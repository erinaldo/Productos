...
namespace Pdf417lib
{
    ...
    public class Pdf417lib
    {
        ...
        {
            sbyte[] sArray = new sbyte[sValue.Length ];
        ...
        public Image DrawBarcodeImage(int ancho)
        {
            if ((this.bitColumns * 2) <= ancho)
                return this.DrawBarcodeImage(2, 3);
            else
                return this.DrawBarcodeImage(1, 3);
        }
        ...
        public Image DrawBarcodeImage(int scale_x, int scale_y)
        {
            ...
            bmp = new Bitmap(this.BitColumns * scale_x, (this.outBits.Length / cols) * scale_y,  System.Drawing.Imaging.PixelFormat.Format1bppIndexed);
            BitmapData bmdn = bmp.LockBits(new Rectangle(0, 0, bmp.Width, bmp.Height), ImageLockMode.ReadWrite, PixelFormat.Format1bppIndexed);
            ...
            for (int k = 0; k < out_Renamed.Length; ++k)
            {
                ...
                for (int j = 0; j < 8; j++)
                {
                    ...
                    for (int xx = 0; xx < scale_x; xx++)
                    {
                        ...
                        {
                            this.SetIndexedPixel(x * scale_x + xx, y * scale_y + yy, bmdn, colorInx);
