package com.duxstar.dacza.datos.generales;
...
import com.duxstar.dacza.datos.basedatos.BaseDatos;
...
public abstract class Entidad
{
		...
		for (final Field campo : this.getClass().getFields())
		{
			if (campo.isAnnotationPresent(Requerido.class))
			{
				...
				if (valor == null)
				{
                    ...
                    String sCampo = (defCampo.msgDescripcion().equals("") ? campo.getName() : defCampo.msgDescripcion());
					...
					throw new ControlError("El campo " + sCampo + " es requerido.", this.getClass().getSimpleName(), campo.getName());
				}
			}
		}
	}
		...
		for (final Field campo : entidad.getClass().getFields())
		{
			if (campo.isAnnotationPresent(Requerido.class))
			{
				...
				if (valor == null)
				{
					...
					//ArrayList<ParametroMSG> params = new ArrayList<ParametroMSG>();
					//params.add(new ParametroMSG((defCampo.msgDescripcion().equals("") ? campo.getName() : defCampo.msgDescripcion()), (defCampo.msgDescripcion().equals("") ? false : true)));
					throw new ControlError("El campo " + sCampo + " es requerido.", entidad.getClass().getSimpleName(), campo.getName());
				}
			}
		}
	...
	{
		String filtroEliminacion = "";
        if (nombreTabla.equalsIgnoreCase("Visita")){
            return filtroEliminacion;
        }
		...
		for (Field campo : getClass().getFields())
		{
			...
			else if (campo.getName().equalsIgnoreCase("TipoFase") && !nombreTabla.toUpperCase().equals("TRANSPROD"))
			{
				filtroEliminacion = "TipoFase <> 1";
				break;
			}else if (campo.getName().equalsIgnoreCase("Activo")  && nombreTabla.toUpperCase().equals("USUARIO") )
            {
                filtroEliminacion = "Activo = 0";
                break;
            }
