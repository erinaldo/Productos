...
public class VinTextBoxScanner extends LinearLayout
{

	public VinTextBoxScanner(Context context, AttributeSet attrs)
	{
		super(context, attrs);
		inicializar();
	}

	public VinTextBoxScanner(Context context)
	{
		super(context);
		inicializar();
	}
	...
	public interface OnVinCodigoIngresadoListener
	{
		// void OnCodigoIngresado(String Codigo);
		void OnVinCodigoIngresado(String Codigo, int codigoLeido);

	}

	private OnVinCodigoIngresadoListener listener;

	// ...

	public void setOnVinCodigoIngresado(OnVinCodigoIngresadoListener l)
	{
		listener = l;
	}

	/************ OnTextChanged *******************************/
	public interface OnVinTextChangedListener
	{
		void OnVinTextChanged(CharSequence s);
	}

	private OnVinTextChangedListener txtChangedlistener;

	// ...

	public void setOnVinTextChanged(OnVinTextChangedListener l)
	{
	...
	private void asignarEventos()
	{
		txtScanner.addTextChangedListener(new TextWatcher()
		{
			...
			public void onTextChanged(CharSequence s, int start, int before, int count)
			{
				try
				{
					...
					{
						txtChangedlistener.OnVinTextChanged(s);
					}
				}
				catch (Exception ex)
				{
		...
		txtScanner.setOnFocusChangeListener(new View.OnFocusChangeListener()
		{
			...
			public void onFocusChange(View v, boolean hasFocus)
			{
				...
				if (!hasFocus)
				{
					...
					{
						listener.OnVinCodigoIngresado(txtScanner.getText().toString(), 2);
					}
				}
			}
		});
		...
		btnScanner.setOnClickListener(mEventoScanVin);
	...
	private OnClickListener mEventoScanVin = new OnClickListener()
	{
		public void onClick(View v)
		{
            ...
            switch(Sesion.get(Sesion.Campo.ModeloDispositivo).toString()){
                case "ALPS":
                    ...
                    if (booCodigoLeido){
                        listener.OnVinCodigoIngresado(txtScanner.getText().toString(), 1);
                    }
                    break;
