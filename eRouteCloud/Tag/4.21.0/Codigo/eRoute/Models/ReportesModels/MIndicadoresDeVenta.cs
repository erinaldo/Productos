...
{
    public class MIndicadoresDeVenta
    {
        ...
        public XtraReport GetReport(string Cedi, string FechaInicial, string FechaFinal, string Vendedor, string Ruta, string NombreCedis, string VendedorSplit, string RutasSplit)
        {
            try
            {
                ...
                //DateTime dFechaIniFiltro = dFechaIni.Date;
                ...
                sConsulta.AppendLine("AND ALM.Clave = '" + Cedi + "' ");
                ...
                sConsulta.AppendLine("GROUP BY Fecha, Vendedor, Ruta order by Ruta, Fecha");

                QueryString = sConsulta.ToString();

                IndicadoresDeVenta report = new IndicadoresDeVenta(QueryString);
                report.pbLogo.Image = Image.FromStream(mStream);
                report.pbLogo.Sizing = DevExpress.XtraPrinting.ImageSizeMode.StretchImage;

                //ReportHeader
                report.lbNombre.Text = sNombreEmpresa;
                report.lbNombreReporte.Text = ValorReferencia.ObtenerDescripcion("REPORTEW", "34", "EM");
                ...
                report.lbFechaFiltro.Text = dFechaIni.ToShortDateString() + (FechaFinal == "null" ? "" : "-" + dFechaFin.ToShortDateString());
                ...
                report.lbFueraFrec.Text = Mensaje.ObtenerDescripcion("XFueraFrec", "EM");
                report.lbFueraFrecV.Text = Mensaje.ObtenerDescripcion("XFueraFrecV", "EM");
    ...
    // public int VisitadosVenta { get; set; }
    // public double Efectividad { get; set; }
    // public int NoVisitados { get; set; }
    // public int FueraFrecuencia { get; set; }
    // public int FueraFrecuenciaV { get; set; }
