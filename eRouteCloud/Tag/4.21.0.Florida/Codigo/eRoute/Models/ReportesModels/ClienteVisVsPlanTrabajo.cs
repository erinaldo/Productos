...
namespace eRoute.Models.ReportesModels
{
    public class ClienteVisVsPlanTrabajo
    {
        ...
        public XtraReport GetReport(int VAVClave, string nombreReport, string pvCondicion, string RutasSplit, string FechaInicial, string FechaFinal, string Cedis, string nombreCedis, string unidadMedida)
        {
            try
            {
                //Logo Empresa
                ...
                report.xrPictureBox2.Image = Image.FromStream(mStream);
                report.xrPictureBox2.Sizing = DevExpress.XtraPrinting.ImageSizeMode.StretchImage;

                //Nombre de la Empresa
                StringBuilder Nombre = new StringBuilder();
                Nombre.AppendLine("SELECT NombreEmpresa AS Nombre FROM Configuracion (NOLOCK)");
                QueryString = "";
                QueryString = Nombre.ToString();
                List<VentasvsAnioAnteriorGeneralModel> Detalle = Connection.Query<VentasvsAnioAnteriorGeneralModel>(QueryString, null, null, true, 600).ToList();
                var Nom = (from N in Detalle select N).ToList().GroupBy(DN => new { DN.Nombre })
                        .SelectMany(NL => NL.Select(NS => new VentasvsAnioAnteriorGeneralModel
                        {
                            Nombre = NS.Nombre
                        })).ToList();
                string DetalleN = "";
                ...
                {
                    DetalleN = item.Nombre;
                }
                report.labelEmpresa.Text = DetalleN;
            ...
            {
                return new ReporteClienteVisVsPlanTrabajo();
            }
        }
    }
