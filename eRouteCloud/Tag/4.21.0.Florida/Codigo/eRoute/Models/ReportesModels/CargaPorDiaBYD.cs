    ...
    public class CargaPorDiaBYD
    {
        ...
        public bool GetReport(string NombreReporte, string NombreEmpresa, int Reporte, string CEDI, String NombreCEDI, string FechaIni, string FechaFin, string RUTClave, string VendedorID, bool PorRuta)
        {
            try
            {
                ...
                sConsulta.AppendLine("SELECT TRP.FechaCaptura, TRP.Folio, TPD.ProductoClave, PRO.Nombre, (ISNULL(TPD.CantidadOriginal, TPD.Cantidad) * PRD.Factor) AS CantidadOriginal, (TPD.Cantidad * PRD.Factor) AS Cantidad, ");
                sConsulta.AppendLine("((TPD.Cantidad * PRD.Factor) * PPV.Precio) AS SubTotal, dbo.FNCalcularImpuestoProducto(TPD.ProductoClave, TRP.FechaCaptura, (TPD.Cantidad * PRD.Factor) * PPV.Precio) AS Impuesto, 0 AS Contenedor ");
