...
namespace eRoute.Models.ReportesModels
{
    public class AnalisisDeSaldoMOO
    {
        ...
        public XtraReport GetReport(string ReportName, string CompanyName, MemoryStream CompanyLogo, string CedisName, string CEDIS, string InitialDate, string Routes, string Sellers, string ObjectName, string Clients, bool ReportFilter, bool ReportType)
        {
            ...
            if (ReportType)
            {
                ...
                sConsulta.AppendLine("WHERE ((TRP.Tipo = 1 AND TRP.TipoFase IN (2,3)) OR (TRP.Tipo = 24 AND TRP.TipoFase <> 0)) AND TRP.Saldo <> 0 ");
                sConsulta.AppendLine(string.Format("AND ALM.AlmacenID = '{0}' ", CEDIS));
                ...
                sConsulta.AppendLine(string.Format("AND (SEC.RUTClave IN (SELECT Datos FROM FNSplit({0}, ',')) OR {0} IS NULL) ", string.IsNullOrEmpty(Routes) ? "NULL" : string.Format("'{0}'", Routes)));
                sConsulta.AppendLine(string.Format("AND (SEC.ClienteClave IN (SELECT Datos FROM FNSplit({0}, ',')) OR {0} IS NULL) ", string.IsNullOrEmpty(Clients) ? "NULL" : string.Format("'{0}'", Clients)));
                ...
                sConsulta.AppendLine("INNER JOIN Almacen ALM (NOLOCK) ON VCH.AlmacenId = ALM.AlmacenID ");
                sConsulta.AppendLine("WHERE PPC.Saldo <> 0 ");
                sConsulta.AppendLine(string.Format("AND ALM.AlmacenID = '{0}' ", CEDIS));
                sConsulta.AppendLine(string.Format("AND (VEN.VendedorID = {0} OR {0} IS NULL)", string.IsNullOrEmpty(Sellers) ? "NULL" : string.Format("'{0}'", Sellers)));
                sConsulta.AppendLine(string.Format("AND (SEC.RUTClave IN (SELECT Datos FROM FNSplit({0}, ',')) OR {0} IS NULL) ", string.IsNullOrEmpty(Routes) ? "NULL" : string.Format("'{0}'", Routes)));
                sConsulta.AppendLine(string.Format("AND (SEC.ClienteClave IN (SELECT Datos FROM FNSplit({0}, ',')) OR {0} IS NULL) ", string.IsNullOrEmpty(Clients) ? "NULL" : string.Format("'{0}'", Clients)));
                ...
                AnalisisSaldosMOODetallado report = new AnalisisSaldosMOODetallado() { DataSource = formatlist };
                ...
                report.reporte.Text = ReportName;
                ...
                report.filterClient.Text = (string.IsNullOrEmpty(Clients) ? "Todos los Clientes" : Clients);
            ...
            else
            {
                ...
                AnalisisSaldosMOODetallado reportG = new AnalisisSaldosMOODetallado() { DataSource = formatlist };
                ...
                reportG.reporte.Text = ReportName;
                ...
                reportG.filterClient.Text = (string.IsNullOrEmpty(Clients) ? "Todos los Clientes" : Clients);
