    ...
    public class R275CumplimientoVisRutGHR
    {
        ...
        public bool GetReportRuta(string FechaIni, string FechaFin, string Rutas, string nombreCedis)
        {
            try
            {
                ...
                sConsulta.AppendLine("select AGV.ALMClave AS OFVTA, AGV.ALMNombre AS Sucursal, '' AS UnidDeNeg, AGV.RUTClave AS Ruta, '40' + AGV.VendedorId AS Agente, ");
                sConsulta.AppendLine("(select V.Nombre from Vendedor V where V.VendedorID = AGV.VendedorId) AS NombreAgente, ");
