app.controller('r262VentasController', ['valorReferencia', '$http', 'cfpLoadingBar', function (valorReferencia, $http, cfpLoadingBar) {
    ...
    self.Submit = function () {
        ...
        var sellers = [];
        if (!self.selectAllSellers) {
            sellers = GetArrayValues(self.sellers);
        }

        if (routes.length <= 0 && !self.selectAllRoutes && sellers.length <= 0 && !self.selectAllSellers) {
            ...
            sellers = (self.selectAllSellers ? null : sellers);
            var data = {
