...
app.controller('almacenCtrl', ["$scope", "$rootScope", "$http", "$window",'valorReferencia', "messageBox", "translationService", "centroBox", "autoBox", function ($scope, $rootScope, $http, $window, valorReferencia, messageBox, translationService, centroBox, autoBox) {
    ...
    $scope.ObtenerCamiones = function() {
        $http({
    ...
    $scope.addCamion = function (placa,clave,desc,checked) {
        if (checked == "YES") {
            $scope.camionDataNuevo.push({ "Placa": placa, "Clave": clave, "Descripcion": desc });
        } else {
            angular.forEach($scope.camionDataNuevo, function (value, key) {
                if (value.Placa == placa) {
                    $scope.camionDataNuevo.splice(key, 1);
                }
            });
        }   
    }
    ...
    $scope.AddFinalC = function () {
        for (var i = 0; i < $scope.camionDataNuevo.length; i++) {
            var aux = false;
            for (var j = 0; j < $scope.camionData.length; j++) {
                if ($scope.camionDataNuevo[i].Clave === $scope.camionData[j].Clave) {
                    aux = true;
                }
            }
            if (!aux) {
                $scope.camionData.push({ 'Placa': $scope.camionDataNuevo[i].Placa, 'Clave': $scope.camionDataNuevo[i].Clave, 'Descripcion': $scope.camionDataNuevo[i].Descripcion });
            }
        }
        //$scope.Ordenar();
        $scope.camionDataNuevo = [];
    ...
    //Eliminar camion
    $scope.ValidarEliminarCamion = function (placa) {        
