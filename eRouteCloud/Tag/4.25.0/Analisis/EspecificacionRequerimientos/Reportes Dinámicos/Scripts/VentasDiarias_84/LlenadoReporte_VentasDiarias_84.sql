...
SET @NombreCodigoReporte = 'VentasDiarias_84'
SET @NombreReporte = 'Ventas Diarias (Ferguez)'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[stpr_VentasDiarias_84] @filterSeller = {0}, @filterDateIni = {1}' WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[stpr_VentasDiarias_84] @filterSeller = {0}, @filterDateIni = {1}')
	END
