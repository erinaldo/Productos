...
SET @NombreCodigoReporte = 'ResumenVentasDiarias_67'
SET @NombreReporte = 'Resumen De Ventas Diarias'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[stpr_ResumenVentasDiarias_67] @filterCEDIS = {0}, @filterStartDate = {1}, @filterEndDate = {2}' WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[stpr_ResumenVentasDiarias_67] @filterCEDIS = {0}, @filterStartDate = {1}, @filterEndDate = {2}')
	END
