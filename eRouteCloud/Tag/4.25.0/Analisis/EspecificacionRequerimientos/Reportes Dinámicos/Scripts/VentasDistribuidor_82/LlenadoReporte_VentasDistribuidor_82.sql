...
SET @NombreCodigoReporte = 'VentasDistribuidor_82'
SET @NombreReporte = 'Ventas Distribuidor'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[stpr_VentasDistribuidor_82] @filtroRutas = {0},@filtroVendedores = {1}, @filtroFechaInicio = {2}, @filtroFechaFin = {3}'  WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[stpr_VentasDistribuidor_82] @filtroRutas = {0},@filtroVendedores = {1}, @filtroFechaInicio = {2}, @filtroFechaFin = {3}')
	END
...
IF NOT EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[FiltrosDinamicos] (NOLOCK) WHERE [RepDinID] = @IdReporte)
	...
	SELECT TOP 1 [RepDinID], 2, 'RMAE' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
	INSERT INTO [dbo].[FiltrosDinamicos]([RepDinID], [TipoFiltro], [Opciones])
	SELECT TOP 1 [RepDinID], 3, 'RMAE' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
	INSERT INTO [dbo].[FiltrosDinamicos]([RepDinID], [TipoFiltro], [Opciones])
