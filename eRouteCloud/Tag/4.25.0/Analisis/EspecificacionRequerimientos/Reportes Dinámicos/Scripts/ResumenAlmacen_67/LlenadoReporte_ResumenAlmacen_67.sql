...
SET @NombreCodigoReporte = 'ResumenAlmacen_67'
SET @NombreReporte = 'Resumen De Almacen'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[stpr_ResumenAlmacen_67] @filterCEDIS = {0}, @filterSeller = {1}, @filterStartDate = {2}, @filterEndDate = {3}' WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[stpr_ResumenAlmacen_67] @filterCEDIS = {0}, @filterSeller = {1}, @filterStartDate = {2}, @filterEndDate = {3}')
	END
...
IF NOT EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[FiltrosDinamicos] (NOLOCK) WHERE [RepDinID] = @IdReporte)
	BEGIN
		...
		SELECT TOP 1 [RepDinID], 1, 'RSAI' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
		INSERT INTO [dbo].[FiltrosDinamicos]([RepDinID], [TipoFiltro], [Opciones])
		SELECT TOP 1 [RepDinID], 3, 'RSAI' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
		INSERT INTO [dbo].[FiltrosDinamicos]([RepDinID], [TipoFiltro], [Opciones])
