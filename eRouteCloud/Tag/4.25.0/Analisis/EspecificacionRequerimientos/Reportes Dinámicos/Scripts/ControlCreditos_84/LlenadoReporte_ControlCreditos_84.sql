...
SET @NombreCodigoReporte = 'ControlCreditos_84'
SET @NombreReporte = 'Control Creditos'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[stpr_ControlCreditos_84] @filterRoute = {0}, @filterStartDate = {1}, @filterEndDate = {2}, @filterClient = {3}'  WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[stpr_ControlCreditos_84] @filterRoute = {0}, @filterStartDate = {1}, @filterEndDate = {2}, @filterClient = {3}')
	END
...
IF NOT EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[FiltrosDinamicos] (NOLOCK) WHERE [RepDinID] = @IdReporte)
	...
	SELECT TOP 1 [RepDinID], 2, 'RMAI' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
	INSERT INTO [dbo].[FiltrosDinamicos]([RepDinID], [TipoFiltro], [Opciones])
	SELECT TOP 1 [RepDinID], 4, 'REAI' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
	INSERT INTO [dbo].[FiltrosDinamicos]([RepDinID], [TipoFiltro], [Opciones])
	SELECT TOP 1 [RepDinID], 7, 'RMAI' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
END
