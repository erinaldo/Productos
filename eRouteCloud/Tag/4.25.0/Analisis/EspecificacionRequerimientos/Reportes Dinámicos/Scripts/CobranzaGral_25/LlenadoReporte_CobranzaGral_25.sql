...
SET @NombreCodigoReporte = 'CobranzaGral_25'
SET @NombreReporte = 'Cobranza (Yupik)'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[stpr_CobranzaGral_25] @FILTROCEDI = {0}, @FILTROVENDEDOR = {1}, @FILTROFECHAINI = {2}, @FILTROFECHAFIN = {3}, @FILTROCLIENTES = {4}' WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[stpr_CobranzaGral_25] @FILTROCEDI = {0}, @FILTROVENDEDOR = {1}, @FILTROFECHAINI = {2}, @FILTROFECHAFIN = {3}, @FILTROCLIENTES = {4}')
	END
...
IF NOT EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[FiltrosDinamicos] (NOLOCK) WHERE [RepDinID] = @IdReporte)
	...
	SELECT TOP 1 [RepDinID], 1, 'RSA' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
	INSERT INTO [dbo].[FiltrosDinamicos]([RepDinID], [TipoFiltro], [Opciones])
	SELECT TOP 1 [RepDinID], 3, 'RSA' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
	INSERT INTO [dbo].[FiltrosDinamicos]([RepDinID], [TipoFiltro], [Opciones])
	SELECT TOP 1 [RepDinID], 4, 'REAI' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
	INSERT INTO [dbo].[FiltrosDinamicos]([RepDinID], [TipoFiltro], [Opciones])
	SELECT TOP 1 [RepDinID], 7, 'OM' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
END
