...
namespace eRoute.Controllers.API
{
    ...
    public class VendedoresController : ApiController
    {
        ...
        //Obtener
        [HttpGet]
        [ActionName("ObtenerVendedoresCedi")]
        public IHttpActionResult ObtenerVendedoresCedi(string CEDI)
        {
            DateTime Factual = DateTime.Now;

            var vendedores = (
                from ven in db.Vendedor
                from vch in db.VENCentroDistHist
                .Where(c => c.AlmacenId.Equals(CEDI) && c.VendedorId.Equals(ven.VendedorID) && (c.VCHFechaInicial <= Factual.Date && c.FechaFinal >= Factual.Date))
                select new { Seleccionado = false, ven.VendedorID, ven.Nombre, ven.MCNClave }).ToList();
