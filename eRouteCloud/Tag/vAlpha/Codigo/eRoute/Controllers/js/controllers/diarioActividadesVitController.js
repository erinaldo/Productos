(function () {
    angular
        .module('eRouteApp')
        .controller('diarioActividadesVitController', diarioActividadesVitController)
    diarioActividadesVitController.$inject = ['$scope', '$http', '$mdToast', '$q', '$timeout', 'NgMap', 'cfpLoadingBar', '$filter', "menuServices", "reportServices", "valorReferencia"];

    function diarioActividadesVitController($scope, $http, $mdToast, $q, $timeout, NgMap, cfpLoadingBar, $filter, menuServices, reportServices, valorReferencia) {
        ...
        //ctrl.group = 'Seller';
        //ctrl.GetSellersAll = GetSellersAll;
        ctrl.GetSellersAll = GetSellersAll;

        function GetSellersAll(selectedItem) {
            if (selectedItem == undefined)
                selectedItem = "";
            $http({
                url: url + '/api/GetSeller?Cedis=' + selectedItem.replace(/\+/g, "%2B") + '&estado=true',
                method: 'GET',
                headers: {
                    Authorization: window.sessionStorage.getItem('Token'),
                    'Content-Type': 'application/json'
                ...
                ctrl.sellersAll = response.data;
            }, function errorCallBack(response) {

            });
            //console.log(JSON.stringify("tam = " + selectAll + " " + url));
        }
        ...
        function SubmitForm(Cedis, dateStatus, Routes, Clientes, pvEsFecha, campoFecha) {
            ...
            if (countRoutes <= 0 || countSellers <= 0 || countSellersAll <= 0) {
