...
namespace eRoute.Models.ReportesModels
{
    public class MLiquidacionPDR
    {
        ...
        public XtraReport GetReport(string NombreReporte, string NombreEmpresa, MemoryStream LogoEmpresa, string NombreCEDIS, string CEDIS, string EstatusFecha, string FechaInicial, string Vendedores)
        {
            try
            {
                ...
                sConsulta.AppendLine("	WHERE CONVERT(date, Dia.FechaCaptura, 112) = '" + FechaInicial + "' ");
                ...
                sConsulta.AppendLine("	    AND CONVERT(date, Dia.FechaCaptura, 112) = '" + FechaInicial + "' ");
                ...
                sConsulta.AppendLine("AND TP.CFVTipo = '2' AND CONVERT(date, TP.FechaCaptura, 112) <> '" + FechaInicial + "' ");
                ...
                sConsulta.AppendLine("AND ((VIS.VendedorId IN (SELECT Datos FROM FNSplit((" + Vendedores + "), ','))) OR " + Vendedores + " IS NULL) ");
                sConsulta.AppendLine("INNER JOIN Dia (NOLOCK) ON Dia.DiaClave = Vis.DiaClave AND CONVERT(date, Dia.FechaCaptura, 112) = '" + FechaInicial + "'");
                ...
                //DESGLOSE
                sConsulta.Clear();
                sConsulta.AppendLine("SELECT SUM(Cantidad) As Cantidad, Tipo, ('$' + Descripcion) AS Descripcion, VendedorID, (SUM(Cantidad) * CAST(Descripcion AS float)) AS Subtotal FROM( ");
                ...
                sConsulta.AppendLine("AND CONVERT(date, PL.FechaPreliquidacion, 112) = '" + FechaInicial + "') DE ");
                ...
                sConsulta.AppendLine("INNER JOIN PLIBancario PLI (NOLOCK) ON PLI.PLIId = PL.PLIId AND CONVERT(date, PL.FechaPreliquidacion, 112) = '" + FechaInicial + "' ");
                sConsulta.AppendLine("AND ((PL.VendedorId IN (SELECT Datos FROM FNSplit((" + Vendedores + "), ','))) OR " + Vendedores + " IS NULL) ");
                ...
                report.filtro.Text = (Vendedores == "" ? "Todos los Vendedores" : Vendedores).Replace("'", "");
                report.centro.Text = NombreCEDIS;
                report.fecha.Text = fInicio.ToShortDateString();
