(function () {
    angular
        .module('eRouteApp')
        .controller('pedidosPreventaDetalladoController', pedidosPreventaDetalladoController)
    pedidosPreventaDetalladoController.$inject = ['$scope', '$http', '$mdToast', '$q', '$timeout', 'NgMap', 'cfpLoadingBar', '$filter', "menuServices", "reportServices", "valorReferencia"];

    function pedidosPreventaDetalladoController($scope, $http, $mdToast, $q, $timeout, NgMap, cfpLoadingBar, $filter, menuServices, reportServices, valorReferencia) {
        ...
        ctrl.inactivosClic = function (selectedItem) {
            var c = $scope.inactv;
            ...
            if (!c || c == undefined) {
                if (selectedItem == undefined)
                    selectedItem = "";
                $http({
                    url: url + '/api/GetSeller?Cedis=' + selectedItem.replace(/\+/g, "%2B") + "&inactivos=" + c,
                    method: 'GET',
                    headers: {
                        Authorization: window.sessionStorage.getItem('Token'),
                        'Content-Type': 'application/json'
        ...
        //            CENClave: '', promocion: '', Presupuesto: ctrl.presupuesto
        //        }
        ...
        //            window.open(url + "/Reports/Ver?VAVClave=" + data.VAVClave + "&Cedis=" + data.Cedis.replace(/\+/g, "%2B") + "&nombreReport=" + data.nombreReport + "&nombreCedis=" + data.nombreCedis + "&FechaInicial=" + data.FechaInicial + "&FechaFinal=" + data.FechaFinal + "&dateStatus=" + data.Rango + "&Rutas=" + data.Rutas + "&Clientes=" + data.Clientes + "&Vendedor=" + data.Vendedor + "&detallado="
        //                + data.reportType + "&unidadMedida=" + data.unidadMedida
        //               + "&CENClave=" + data.CENClave + "&promocion=" + data.promocion + "&Presupuesto=" + data.Presupuesto
        //                , "_blank");
        ...
        //envia el formulario completo
        function SubmitForm(VendedorId, Cedis, dateStatus, Routes, Clientes, pvEsFecha, campoFecha) {
        ...
        function SelectAllChekboxesSeller(selectAll) {
            if (selectAll == undefined || selectAll == false) {
                ...
                console.log(JSON.stringify("tam = " + selectAll.Checked));
        ...
        //$scope.InactivosChange = function (selectAll) {
        //    console.log(item)
        //    if (selectAll == undefined || selectAll == false) {
        //        $scope.userDropDown = false;
        ...
        function ClearAllRouteCheckBoxes() {
            ...
            if ($scope.fil.length > 0) {
                ...
                angular.forEach(ctrl.sellersAll, function (item) {
                    item.Checked = false;
                    item.Disabled = true;
                });
                angular.forEach(ctrl.sellers, function (item) {
        ...
        //bloquea  el listado de rutas y vendedores, cuando se selecciona un vendedor inactivo
        ctrl.ClearRouteSellerCheckBoxes = ClearAllCheckBoxes;
        ...
        function ClearAllCheckBoxes() {
            ...
            angular.forEach(ctrl.sellersAll, function (item) {
                if (item.Checked == true) {
                    $scope.filAll.push(item.Checked);
                }
            });

            if ($scope.filAll.length > 0) {
                angular.forEach(ctrl.routes, function (item) {
                    item.Checked = false;
                    item.Disabled = true;
                });
