(function () {
    angular
        .module('eRouteApp')
        .controller('cargaPorDiaBYDController', cargaPorDiaBYDController)
    cargaPorDiaBYDController.$inject = ['$scope', '$http', '$mdToast', '$q', '$timeout', 'NgMap', 'cfpLoadingBar', '$filter', "menuServices", "reportServices", "valorReferencia"];

    function cargaPorDiaBYDController($scope, $http, $mdToast, $q, $timeout, NgMap, cfpLoadingBar, $filter, menuServices, reportServices, valorReferencia) {
        ...
        function SubmitForm(Cedis, RUTClave, VendedorId) {
            //console.log(ctrl.State);
            var countRoutes;
            angular.forEach(ctrl.routes, function (item) {
                if (item.RUTClave == RUTClave) {
                    item.Checked = true;
                    countRoutes++;
                }
                else {
