    ...
    public class TiemposEnRuta
    {
        ...
        public XtraReport GetReport(string NombreReporte, string NombreEmpresa, MemoryStream LogoEmpresa, string NombreCEDIS, string CEDIS, string EstatusFecha, string FechaInicial, string FechaFinal, string Rutas, string Vendedores, bool FiltroReporte)
        {
            try
            {
                ...
                sConsulta.AppendLine("group by HoraInicialJornada,HoraFinalJornada,ALNClave, ALNNombre, TMP.VisitaClave, VENNombre, FechaHoraInicial, RUTClave, ClienteClave, CodigoLeido, VAD.Descripcion,");
                ...
                KilometrajeString.AppendLine("INNER JOIN Vendedor VEN (NOLOCK) ON CAV.VendedorId = VEN.VendedorId " + string.Format("AND CONVERT(date, CAV.FechaHoraInicial, 112) BETWEEN '{0}' AND '{1}' ", FechaInicial, FechaFinal));
                KilometrajeString.AppendLine("INNER JOIN (SELECT DISTINCT VendedorId, RUTClave, ClaveCEDI FROM AgendaVendedor (NOLOCK) A inner join dia on dia.DiaClave = A.DiaClave where " + string.Format("dia.fechacaptura BETWEEN '{0}' AND '{1}' ", FechaInicial, FechaFinal) +") AGV ON CAV.VendedorId = AGV.VendedorId ");
                KilometrajeString.AppendLine("INNER JOIN Almacen ALN (NOLOCK) ON AGV.ClaveCEDI = ALN.Clave " + string.Format("and ALN.AlmacenID = {0} ", CEDIS));
                KilometrajeString.AppendLine("INNER JOIN Ruta RUT (NOLOCK) ON AGV.RUTClave = RUT.RUTClave and RUT.RUTClave <>'RUT001' " + string.Format("AND ((RUT.RUTClave IN (SELECT Datos FROM FNSplit({0}, ','))) OR {0} IS NULL) ", Rutas));
                ...
                //KilometrajeString.AppendLine(string.Format("and RUT.RUTClave <>'RUT001' and ALN.AlmacenID = {0} ", CEDIS));
