    ...
    public class IndicadoresOJAI
    {
        ...
        private ArchivoXlsModel GenerarExcel()
        {
            ...
            if (bDetallado)
            {
                ...
                try
                {
                    num = nObjetivoVentaDiariaCajas / nTotalObjetivoVisitasEfectivas;
                }
                catch (Exception)
                {
                    num = 0;
                }
                MyOpenXML.createCell(24, ref Rows, GetExcelColumnName(columna) + "24", CellValues.Number, Math.Round(((float)num), 2).ToString(), 2);
                try
                {
                    num = nLogroVentaDiariaCajas / nLogroVisitasEfectivas;
                    if (Double.IsNaN(num))
                        num = 0;
                }
                catch (Exception)
                ...
                MyOpenXML.createCell(25, ref Rows, GetExcelColumnName(columna) + "25", CellValues.Number, Math.Round(((float)num), 2).ToString(), 2);
                ...
                try
                {
                    num = nVentaPesosDia / nLogroVentaDiariaCajas;
                    if (Double.IsNaN(num))
                        num = 0;
                }
                catch (Exception)
                ...
                MyOpenXML.createCell(28, ref Rows, GetExcelColumnName(columna) + "28", CellValues.Number, Math.Round((num), 2).ToString(), 2);
            ...
            else
            {
                ...
                MyOpenXML.createRow(25, ref Rows);
                MyOpenXML.createCell(25, ref Rows, "B25", CellValues.String, Mensaje.ObtenerDescripcion("XLogroDropSize", "EM"), 3);
                num = (float)(lstIndicadoresRutaGral[0].VentaCajas / lstIndicadoresRuta[0].DiasVentaMes) / lstIndicadoresRutaGral[0].VisitasEfectivas;
                if (Double.IsNaN(num))
                {
                    num = 0;
                }
                MyOpenXML.createCell(25, ref Rows, "C25", CellValues.Number, Math.Round((num), 2).ToString(), 2);
