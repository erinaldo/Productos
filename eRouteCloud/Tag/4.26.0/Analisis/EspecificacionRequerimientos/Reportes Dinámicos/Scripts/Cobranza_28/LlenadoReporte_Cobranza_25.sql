...
SET @NombreCodigoReporte = 'Cobranza_28'
SET @NombreReporte = 'Cobranza (La Costeña)'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[stpr_Cobranza_28] @FILTROCEDI = {0}, @FILTRORUTAS = {1}, @FILTROFECHAINI = {2}, @FILTROFECHAFIN = {3}' WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[stpr_Cobranza_28] @FILTROCEDI = {0}, @FILTRORUTAS = {1}, @FILTROFECHAINI = {2}, @FILTROFECHAFIN = {3}')
	END
...
IF NOT EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[FiltrosDinamicos] (NOLOCK) WHERE [RepDinID] = @IdReporte)
	...
	INSERT INTO [dbo].[FiltrosDinamicos]([RepDinID], [TipoFiltro], [Opciones])
	SELECT TOP 1 [RepDinID], 2, 'RMA' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
	INSERT INTO [dbo].[FiltrosDinamicos]([RepDinID], [TipoFiltro], [Opciones])
