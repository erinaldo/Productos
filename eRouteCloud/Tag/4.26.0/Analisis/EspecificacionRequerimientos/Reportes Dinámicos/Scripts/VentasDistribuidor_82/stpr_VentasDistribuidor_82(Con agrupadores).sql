...
SET NOCOUNT ON
	...
	*/
	...
	Select   CASE WHEN 
				 ROW_NUMBER() OVER
					  (PARTITION BY FechaCaptura ORDER BY FechaCaptura Asc) =1
				   THEN FechaCaptura 
				 ELSE null
			   END [Fecha|FORMAT%{0:dd/mm/yyyy}],
			   CASE WHEN 
				 ROW_NUMBER() OVER
					  (PARTITION BY FechaCaptura,Distribuidor ORDER BY FechaCaptura Asc,Distribuidor ASC) =1
				   THEN Distribuidor 
				 ELSE ''
			   END as [Distribuidor] ,
			   CASE WHEN 
				 ROW_NUMBER() OVER
					  (PARTITION BY FechaCaptura,Distribuidor,Vendedor ORDER BY FechaCaptura Asc,Distribuidor ASC,Vendedor ASC) =1
				   THEN Vendedor 
				 ELSE ''
			   END as  [Vendedor] , 
			   CASE WHEN 
	...
	from #Preventa PRE
	order by FechaCaptura,ROW_NUMBER() OVER (PARTITION BY FechaCaptura,Distribuidor ORDER BY FechaCaptura Asc,Distribuidor ASC),
	ROW_NUMBER() OVER (PARTITION BY FechaCaptura,Distribuidor,Vendedor ORDER BY FechaCaptura Asc,Distribuidor ASC,Vendedor ASC),  ROW_NUMBER() OVER (PARTITION BY FechaCaptura, Distribuidor,Vendedor,  Ruta ORDER BY FechaCaptura Asc, Distribuidor ASC,Vendedor ASC,  Ruta ASC), 
