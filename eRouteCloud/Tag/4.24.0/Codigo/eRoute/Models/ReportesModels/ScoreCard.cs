...
namespace eRoute.Models.ReportesModels
{
    public class ScoreCard
    {
        ...
        public bool GetReport(string NombreReporte, string NombreEmpresa, string Fecha, string AlmacenClave, string Esquemas, string AlmacenNombre, string EsquemasNombre)
        {
            try
            {
                //GetDataLayer();
                //Session session = new Session();
                //SelectedData sprocData = session.ExecuteSproc("stpr_CrearUsuarioReportes");
                ...
                if (!Esquemas.Equals(""))
                    filtroEsquemaProd = "AND ESQ2.EsquemaIdPadre IN (" + Esquemas + ")";
                ...
                sConsulta.AppendLine("INNER JOIN Esquema ESQ (NOLOCK) ON CLIESQ.EsquemaID = ESQ.EsquemaID AND ESQ.EsquemaID IN (SELECT * FROM dbo.BuscaIdsEsquema(ESQ.EsquemaID)) AND ESQ.Nivel = 1 ");
                sConsulta.AppendLine("AND DATEPART(WK, D.FechaCaptura) = DATEPART(WK, '" + dFechaIni.Date.ToString("s") + "') ");
                sConsulta.AppendLine("AND DATEPART(YYYY, D.FechaCaptura) = DATEPART(YYYY, '" + dFechaIni.Date.ToString("s") + "') ");
                sConsulta.AppendLine("AND ALM.AlmacenID = '" + AlmacenClave + "' ");
                ...
                sConsulta.AppendLine("INNER JOIN ClienteEsquema CLIESQ (NOLOCK) ON AGV.ClienteClave = CLIESQ.ClienteClave ");
                sConsulta.AppendLine("INNER JOIN Esquema ESQ (NOLOCK) ON CLIESQ.EsquemaID = ESQ.EsquemaID AND ESQ.EsquemaID IN (SELECT * FROM dbo.BuscaIdsEsquema(ESQ.EsquemaID)) AND ESQ.Nivel = 1 ");
                sConsulta.AppendLine("AND DATEPART(WK, D.FechaCaptura) BETWEEN DATEPART(WK, '" + dFechaIni.Date.ToString("s") + "')-4 AND DATEPART(WK, '" + dFechaIni.Date.ToString("s") + "')-1 ");
                sConsulta.AppendLine("AND DATEPART(YYYY, D.FechaCaptura) = DATEPART(YYYY, '" + dFechaIni.Date.ToString("s") + "') ");
                sConsulta.AppendLine("AND ALM.AlmacenID = '" + AlmacenClave + "' ");
                sConsulta.AppendLine("GROUP BY AGV.ClienteClave, CLIESQ.EsquemaID, ESQ.Nombre ");
                ...
                sConsulta.AppendLine("AND DATEPART(WK, D.FechaCaptura) = DATEPART(WK, '" + dFechaIni.Date.ToString("s") + "') ");
                sConsulta.AppendLine("AND DATEPART(YYYY, D.FechaCaptura) = DATEPART(YYYY, '" + dFechaIni.Date.ToString("s") + "') ");
                sConsulta.AppendLine("AND AGV.RUTClave IN (SELECT VRUT.RUTClave FROM Almacen ALM (NOLOCK) ");
                ...
                sConsulta.AppendLine("CASE WHEN SUM(T.ConVenta) > 0 THEN CASE WHEN (CAST(SUM(T.Visitados) AS float)/SUM(T.ConVenta))*100 >= 80 THEN (SELECT Descripcion FROM MENDetalle (NOLOCK) WHERE MENClave = 'XBien' AND MEDTipoLenguaje = dbo.FNObtenerLenguaje()) ");
                sConsulta.AppendLine("ELSE (SELECT Descripcion FROM MENDetalle (NOLOCK) WHERE MENClave = 'XMal' AND MEDTipoLenguaje = dbo.FNObtenerLenguaje()) END else (SELECT Descripcion FROM MENDetalle (NOLOCK) WHERE MENClave = 'XMal' AND MEDTipoLenguaje = dbo.FNObtenerLenguaje()) end AS Promedio4W ");
                ...
                sConsulta.AppendLine("INNER JOIN ClienteEsquema CLIESQ (NOLOCK) ON VIS.ClienteClave = CLIESQ.ClienteClave AND CLIESQ.TipoEstado = 1 ");
                sConsulta.AppendLine("INNER JOIN Esquema ESQ (NOLOCK) ON CLIESQ.EsquemaID = ESQ.EsquemaID AND ESQ.EsquemaID IN (SELECT * FROM dbo.BuscaIdsEsquema(ESQ.EsquemaID)) AND ESQ.Nivel = 1 ");
                sConsulta.AppendLine("INNER JOIN Vendedor VEN (NOLOCK) ON VEN.VendedorID = VIS.VendedorID ");
                sConsulta.AppendLine("INNER JOIN VENCentroDistHist VCDH (NOLOCK) ON VCDH.VendedorId = VEN.VendedorID AND D.FechaCaptura BETWEEN VCDH.VCHFechaInicial AND VCDH.FechaFinal ");
                ...
                sConsulta.AppendLine("INNER JOIN TransProdDetalle TPD (NOLOCK) ON TRP.TransProdID = TPD.TransProdID AND TRP.Tipo = 1 AND TRP.TipoFase = 1 ");
                sConsulta.AppendLine("INNER JOIN ClienteEsquema CLIESQ (NOLOCK) ON TRP.ClienteClave = CLIESQ.ClienteClave AND CLIESQ.TipoEstado = 1 ");
                sConsulta.AppendLine("INNER JOIN Esquema ESQ (NOLOCK) ON CLIESQ.EsquemaID = ESQ.EsquemaID AND ESQ.EsquemaID IN (SELECT * FROM dbo.BuscaIdsEsquema(ESQ.EsquemaID)) AND ESQ.Nivel = 1 ");
                ...
                sConsulta.AppendLine("INNER JOIN Esquema ESQ2 (NOLOCK) ON PROESQ.EsquemaID = ESQ2.EsquemaID AND ESQ2.Nivel = 2 " + filtroEsquemaProd);
                ...
                sConsulta.AppendLine("AND DATEPART(YYYY, D.FechaCaptura) = DATEPART(YYYY, '" + dFechaIni.Date.ToString("s") + "') ");
                sConsulta.AppendLine("AND ALM.AlmacenID = '" + AlmacenClave + "' ");
                sConsulta.AppendLine("--AND PROESQ.EsquemaID = '' ");
                ...
                sConsulta.AppendLine("AND DATEPART(WK, D.FechaCaptura) = DATEPART(WK, '" + dFechaIni.Date.ToString("s") + "') ");
                sConsulta.AppendLine("AND DATEPART(YYYY, D.FechaCaptura) = DATEPART(YYYY, '" + dFechaIni.Date.ToString("s") + "') ");
                sConsulta.AppendLine("AND VIS.RUTClave IN (SELECT VRUT.RUTClave FROM Almacen ALM (NOLOCK) ");
                sConsulta.AppendLine("INNER JOIN VENCentroDistHist VCD (NOLOCK) ON ALM.AlmacenID = VCD.AlmacenId ");
                sConsulta.AppendLine("INNER JOIN VenRut VRUT (NOLOCK) ON VRUT.VendedorID = VCD.VendedorId ");
                sConsulta.AppendLine("AND ALM.AlmacenID = '" + AlmacenClave + "') ");
                ...
                sConsulta.AppendLine("CASE WHEN SUM(T.Agendados) > 0 THEN CASE WHEN (CAST(SUM(T.Visitados) AS float)/SUM(T.Agendados))*100 >= 80 THEN (SELECT Descripcion FROM MENDetalle (NOLOCK) WHERE MENClave = 'XBien' AND MEDTipoLenguaje = dbo.FNObtenerLenguaje()) ");
                sConsulta.AppendLine("ELSE (SELECT Descripcion FROM MENDetalle (NOLOCK) WHERE MENClave = 'XMal' AND MEDTipoLenguaje = dbo.FNObtenerLenguaje()) END else (SELECT Descripcion FROM MENDetalle (NOLOCK) WHERE MENClave = 'XMal' AND MEDTipoLenguaje = dbo.FNObtenerLenguaje()) end AS Promedio4W ");
                sConsulta.AppendLine("FROM ( ");
                ...
                sConsulta.AppendLine("INNER JOIN ClienteEsquema CLIESQ (NOLOCK) ON TRP.ClienteClave = CLIESQ.ClienteClave AND CLIESQ.TipoEstado = 1 ");
                sConsulta.AppendLine("INNER JOIN Esquema ESQ (NOLOCK) ON CLIESQ.EsquemaID = ESQ.EsquemaID AND ESQ.EsquemaID IN (SELECT * FROM dbo.BuscaIdsEsquema(ESQ.EsquemaID)) AND ESQ.Nivel = 1 ");
                sConsulta.AppendLine("INNER JOIN ProductoDetalle PRODE (NOLOCK) ON TPD.ProductoClave = PRODE.ProductoClave AND PRODE.ProductoDetClave = TPD.ProductoClave ");
                sConsulta.AppendLine("INNER JOIN ProductoEsquema PROESQ (NOLOCK) ON TPD.ProductoClave = PROESQ.ProductoClave ");
                sConsulta.AppendLine("INNER JOIN Esquema ESQ2 (NOLOCK) ON PROESQ.EsquemaID = ESQ2.EsquemaID AND ESQ2.Nivel = 2 " + filtroEsquemaProd);
                sConsulta.AppendLine("INNER JOIN Visita VIS (NOLOCK) ON TRP.VisitaClave = VIS.VisitaClave ");
                sConsulta.AppendLine("INNER JOIN Dia D (NOLOCK) ON D.DiaClave = VIS.DiaClave ");
                ...
                sConsulta.AppendLine("INNER JOIN ProductoDetalle PRODE (NOLOCK) ON TPD.ProductoClave = PRODE.ProductoClave AND PRODE.ProductoDetClave = TPD.ProductoClave ");
                sConsulta.AppendLine("INNER JOIN ProductoEsquema PROESQ (NOLOCK) ON TPD.ProductoClave = PROESQ.ProductoClave ");
                sConsulta.AppendLine("INNER JOIN Esquema ESQ2 (NOLOCK) ON PROESQ.EsquemaID = ESQ2.EsquemaID AND ESQ2.Nivel = 2 AND ESQ2.Nombre != 'MIX BODDA' " + filtroEsquemaProd);
                sConsulta.AppendLine("INNER JOIN Visita VIS (NOLOCK) ON TRP.VisitaClave = VIS.VisitaClave ");
                sConsulta.AppendLine("INNER JOIN Dia D (NOLOCK) ON D.DiaClave = VIS.DiaClave ");
                sConsulta.AppendLine("INNER JOIN Vendedor VEN (NOLOCK) ON VEN.VendedorID = VIS.VendedorID ");
                sConsulta.AppendLine("INNER JOIN VENCentroDistHist VCDH (NOLOCK) ON VCDH.VendedorId = VEN.VendedorID AND D.FechaCaptura BETWEEN VCDH.VCHFechaInicial AND VCDH.FechaFinal ");
                ...
                sConsulta.AppendLine("THEN (SELECT Descripcion FROM MENDetalle (NOLOCK) WHERE MENClave = 'XArriba' AND MEDTipoLenguaje = dbo.FNObtenerLenguaje()) ELSE (SELECT Descripcion FROM MENDetalle (NOLOCK) WHERE MENClave = 'XAbajo' AND MEDTipoLenguaje = dbo.FNObtenerLenguaje()) END ");
                sConsulta.AppendLine("ELSE (SELECT Descripcion FROM MENDetalle (NOLOCK) WHERE MENClave = 'XAbajo' AND MEDTipoLenguaje = dbo.FNObtenerLenguaje()) END AS TotalVtaVSPromedio4W ");
                sConsulta.AppendLine("FROM ( ");
                ...
                sConsulta.AppendLine("INNER JOIN ProductoDetalle PRODE (NOLOCK) ON TPD.ProductoClave = PRODE.ProductoClave AND PRODE.ProductoDetClave = TPD.ProductoClave ");
                sConsulta.AppendLine("INNER JOIN ProductoEsquema PROESQ (NOLOCK) ON TPD.ProductoClave = PROESQ.ProductoClave ");
                sConsulta.AppendLine("INNER JOIN Esquema ESQ2 (NOLOCK) ON PROESQ.EsquemaID = ESQ2.EsquemaID AND ESQ2.Nivel = 2 AND ESQ2.Nombre = 'MIX BODDA' ");
                sConsulta.AppendLine("INNER JOIN Visita VIS (NOLOCK) ON TRP.VisitaClave = VIS.VisitaClave ");
                sConsulta.AppendLine("INNER JOIN Dia D (NOLOCK) ON D.DiaClave = VIS.DiaClave ");
                sConsulta.AppendLine("INNER JOIN Vendedor VEN (NOLOCK) ON VEN.VendedorID = VIS.VendedorID ");
                sConsulta.AppendLine("INNER JOIN VENCentroDistHist VCDH (NOLOCK) ON VCDH.VendedorId = VEN.VendedorID AND D.FechaCaptura BETWEEN VCDH.VCHFechaInicial AND VCDH.FechaFinal ");
                ...
                sConsulta.AppendLine("INNER JOIN ClienteEsquema CLIESQ (NOLOCK) ON VIS2.ClienteClave = CLIESQ.ClienteClave AND CLIESQ.TipoEstado = 1 ");
                sConsulta.AppendLine("INNER JOIN Esquema ESQ (NOLOCK) ON CLIESQ.EsquemaID = ESQ.EsquemaID AND ESQ.EsquemaID IN (SELECT * FROM dbo.BuscaIdsEsquema(ESQ.EsquemaID)) AND ESQ.Nivel = 1 ");
                sConsulta.AppendLine("INNER JOIN ProductoDetalle PRODE (NOLOCK) ON TPD.ProductoClave = PRODE.ProductoClave AND PRODE.ProductoDetClave = TPD.ProductoClave ");
                sConsulta.AppendLine("INNER JOIN ProductoEsquema PROESQ (NOLOCK) ON TPD.ProductoClave = PROESQ.ProductoClave ");
                sConsulta.AppendLine("INNER JOIN Esquema ESQ2 (NOLOCK) ON PROESQ.EsquemaID = ESQ2.EsquemaID AND ESQ2.Nivel = 2 ");
                sConsulta.AppendLine("INNER JOIN Vendedor VEN (NOLOCK) ON VEN.VendedorID = VIS2.VendedorID ");
                sConsulta.AppendLine("INNER JOIN VENCentroDistHist VCDH (NOLOCK) ON VCDH.VendedorId = VEN.VendedorID AND D.FechaCaptura BETWEEN VCDH.VCHFechaInicial AND VCDH.FechaFinal ");
                sConsulta.AppendLine("INNER JOIN Almacen ALM (NOLOCK) ON VCDH.AlmacenId = ALM.AlmacenID ");
                sConsulta.AppendLine("AND DATEPART(WK, D.FechaCaptura) = DATEPART(WK, '" + dFechaIni.Date.ToString("s") + "') ");
                sConsulta.AppendLine("AND DATEPART(YYYY, D.FechaCaptura) = DATEPART(YYYY, '" + dFechaIni.Date.ToString("s") + "') ");
                sConsulta.AppendLine("AND VIS2.RUTClave = VIS.RUTClave ");
                ...
                sConsulta.AppendLine("INNER JOIN VENCentroDistHist VCDH (NOLOCK) ON VCDH.VendedorId = VEN.VendedorID AND D.FechaCaptura BETWEEN VCDH.VCHFechaInicial AND VCDH.FechaFinal ");
                sConsulta.AppendLine("INNER JOIN Almacen ALM (NOLOCK) ON VCDH.AlmacenId = ALM.AlmacenID ");
                sConsulta.AppendLine("AND DATEPART(WK, D.FechaCaptura) = DATEPART(WK, '" + dFechaIni.Date.ToString("s") + "') --FILTRAR POR LA SEMANA SELECCIONADA EN EL FILTRO ");
                sConsulta.AppendLine("AND DATEPART(YYYY, D.FechaCaptura) = DATEPART(YYYY, '" + dFechaIni.Date.ToString("s") + "') --FILTRAR POR EL AÑO TAMBIEN, SINO SE TRAE SEMANAS DE TODOS LOS AÑOS ");
                sConsulta.AppendLine("AND ALM.AlmacenID = '" + AlmacenClave + "' ");
                sConsulta.AppendLine("UNION ALL ");
                sConsulta.AppendLine("SELECT 0 AS PzasPedido, ISNULL(SUM(TPD.Cantidad*PRODE.Factor), 0) AS PzasDev ");
                sConsulta.AppendLine("FROM TransProd TRP (NOLOCK) ");
                sConsulta.AppendLine("INNER JOIN TransProdDetalle TPD (NOLOCK) ON TRP.TransProdID = TPD.TransProdID AND TRP.Tipo = 3 AND TRP.TipoFase = 1 ");
                sConsulta.AppendLine("INNER JOIN ClienteEsquema CLIESQ (NOLOCK) ON TRP.ClienteClave = CLIESQ.ClienteClave AND CLIESQ.TipoEstado = 1 ");
                sConsulta.AppendLine("INNER JOIN Esquema ESQ (NOLOCK) ON CLIESQ.EsquemaID = ESQ.EsquemaID AND ESQ.EsquemaID IN (SELECT * FROM dbo.BuscaIdsEsquema(ESQ.EsquemaID)) AND ESQ.Nivel = 1 ");
                sConsulta.AppendLine("INNER JOIN ProductoDetalle PRODE (NOLOCK) ON TPD.ProductoClave = PRODE.ProductoClave AND PRODE.ProductoDetClave = TPD.ProductoClave ");
                sConsulta.AppendLine("INNER JOIN ProductoEsquema PROESQ (NOLOCK) ON TPD.ProductoClave = PROESQ.ProductoClave ");
                ...
                sConsulta.AppendLine("INNER JOIN VENCentroDistHist VCD (NOLOCK) ON ALM.AlmacenID = VCD.AlmacenId ");
                sConsulta.AppendLine("INNER JOIN VenRut VRUT (NOLOCK) ON VRUT.VendedorID = VCD.VendedorId ");
                sConsulta.AppendLine("AND ALM.AlmacenID = '" + AlmacenClave + "')GROUP BY VIS.RUTClave ");
                sConsulta.AppendLine(") AS T ");
                sConsulta.AppendLine("GROUP BY T.RUTClave ");
                sConsulta.AppendLine("UNION ALL ");
                ...
                sConsulta.AppendLine("INNER JOIN ProductoDetalle PRODE (NOLOCK) ON IMD.ProductoClave = PRODE.ProductoClave AND PRODE.ProductoDetClave = IMD.ProductoClave ");
        ...
        //    string conn = ConfigurationManager.ConnectionStrings["eRouteConnection"].ConnectionString;
