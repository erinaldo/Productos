(function () {
    angular
        .module('eRouteApp')
        .controller('liquidacionMorController', liquidacionMorController)
    liquidacionMorController.$inject = ['$scope', '$http', '$mdToast', '$q', '$timeout', 'NgMap', 'cfpLoadingBar', '$filter', "menuServices", "reportServices", "valorReferencia"];

    function liquidacionMorController($scope, $http, $mdToast, $q, $timeout, NgMap, cfpLoadingBar, $filter, menuServices, reportServices, valorReferencia) {
        ...
        function GetSellers() {
            $http({
                url: url + '/api/GetSeller?Cedis=' + null,
                method: 'GET',
                headers: {
        ...
        function GetDates() {
            $http({
                ...
                _.remove(ctrl.dateState, function (n) { return n.Descripcion != "Igual" });
            }, function errorCallBack(response) {
        ...
        function SubmitForm(Cedis, dateStatus, Routes, Clientes, pvEsFecha, campoFecha) {
            ...
            if (countSellers <= 0) {
                alert("Seleccione por lo menos un vendedor");
                cfpLoadingBar.complete();
            }
