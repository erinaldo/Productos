(function () {
    angular
        .module('eRouteApp')
        .controller('reporteMovimientosProductoController', reporteMovimientosProductoController)
    reporteMovimientosProductoController.$inject = ['$scope', '$http', 'cfpLoadingBar', '$filter', "valorReferencia"];

    function reporteMovimientosProductoController($scope, $http, cfpLoadingBar, $filter, valorReferencia) {
        ...
        function GetClients(sellers) {
            Seller = "'" + sellers + "'";
            $http({
                url: url + '/api/GetClients?sellers=' + Seller,
                method: 'GET',
                headers: {
                    Authorization: window.sessionStorage.getItem('Token'),
                    'Content-Type': 'application/json'
        ...
        self.SubmitForm = function () {
            var clientes = [];
            var vendedores = [];
            vendedores = GetArrayValues(self.sellers, $scope.Clave);
            if (!$scope.selectAll) {
            ...
            if (vendedores.length <= 0 && !$scope.selectAll) {
