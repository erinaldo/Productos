(function () {
    angular
        .module('eRouteApp')
        .controller('r002VentasController', r002VentasController)
    r002VentasController.$inject = ['$scope', '$http', 'cfpLoadingBar', '$filter', "valorReferencia"];

    function r002VentasController($scope, $http, cfpLoadingBar, $filter, valorReferencia) {
        ...
        self.GetClients = function (clientScheme, routes, sellers) {
            ...
            $http.post(url + '/Reports/GetObjectString', {
                ...
                $http({
                    url: url + '/API/GetModel/GetClients?clientScheme=' + (clientScheme ? clientScheme.replace(/\+/g, '%2B') : clientScheme) + '&routes=' + dataRoutes + '&sellers=' + (sellers ? sellers.replace(/\+/g, '%2B') : sellers),
                    method: 'GET',
                    headers: {
                        Authorization: window.sessionStorage.getItem('Token'),
                        'Content-Type': 'application/json'
        ...
        self.SubmitForm = function () {
            ...
            var clients = [];
            if (!$scope.EsquemaId || !self.selectAllClients) {
