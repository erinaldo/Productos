(function () {
    angular
        .module('eRouteApp')
        .controller('efectividadPorRutaController', efectividadPorRutaController)
    efectividadPorRutaController.$inject = ['$scope', '$http', '$mdToast', '$q', '$timeout', 'NgMap', 'cfpLoadingBar', '$filter', "menuServices", "reportServices", "valorReferencia"];

    function efectividadPorRutaController($scope, $http, $mdToast, $q, $timeout, NgMap, cfpLoadingBar, $filter, menuServices, reportServices, valorReferencia) {
        ...
        self.GetRoutes = function (selectedItem, estado) {
            if (selectedItem != undefined) {
                ...
                $http({
                    url: url + '/API/GetModel/GetRoutes?cedis=' + selectedItem + "&estado=" + estado,
                    method: 'GET',
                    headers: {
                        Authorization: window.sessionStorage.getItem('Token'),
                        'Content-Type': 'application/json'
