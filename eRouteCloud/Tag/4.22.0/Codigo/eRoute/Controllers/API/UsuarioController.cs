    ...
    public class UsuarioController : ApiController
    {
        ...
        public bool Grabar(Usuario usuario) //valor = msg
        {
            ...
            if (usuario != null)
            {
                ...
                cUsuario.ClaveAcceso = usuario.ClaveAcceso;
        ...
        public IHttpActionResult ObtenerUsuario(string USUId)
        {
            ...
            if(rutas == 0)
            {
                var usr = (from user in db.Usuario
                          ...
                          select new { user.USUId, user.Clave, user.Nombre, ConfTerminal = terminales, ConfRuta = rutas, Tipo = user.Tipo.ToString(), user.DiasLimite, user.ClaveAcceso, user.AlmacenID, user.FechaMod, user.PoliticaId, user.PERClave }
                );
                return Json(usr);
            }
            else
            {
                var usr = (from user in db.Usuario
                           ...
                           where user.USUId == USUId
                           select new { user.USUId, user.Clave, user.Nombre, ConfTerminal = terminales, ConfRuta = rutas, TipoRuta = ruta.Tipo.ToString(), Tipo = user.Tipo.ToString(), user.DiasLimite, user.ClaveAcceso, user.AlmacenID, user.FechaMod, user.PoliticaId, user.PERClave });
                return Json(usr);
            }
        }
