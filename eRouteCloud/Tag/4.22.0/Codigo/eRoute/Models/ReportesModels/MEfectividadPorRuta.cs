...
namespace eRoute.Models.ReportesModels
{
    public class MEfectividadPorRuta
    {
        ...
        public XtraReport GetReport(string pvCondicion, string RutasSplit, string VendedoresSplit, string ClientesSplit, string FechaInicial, string FechaFinal, string Cedis, string NombreCedis, bool detallado)
        {
            try
            {
                ...
                else
                {
                    ...
                    string LogoQuery = "SELECT Logotipo FROM Configuracion";
                    report.lbTotalCodigoBarras.DataBindings.Add("Text", null, "ClientesVisitadosCL");
                    byte[] byteArrayIn = Connection.Query<byte[]>(LogoQuery, null, null, true, 9000).FirstOrDefault();
                    MemoryStream mStream = new MemoryStream(byteArrayIn);
                    report.logo1.Image = Image.FromStream(mStream);
                    report.logo1.Sizing = DevExpress.XtraPrinting.ImageSizeMode.StretchImage;
            ...
            {
                //reporte pedido
                return null;//new ReporteMEfectividadPorRuta();
            }
        }
