...
namespace eRoute.Models.ReportesModels
{
    public class TiemposEnRuta
    {
        ...
        public XtraReport GetReport(string Rutas, string RutasSplit, string FechaInicial, string FechaFinal, string Cedis, string Vendedor) 
        {
            try
            {
                #region reporte
                ...
                DateTime dFechaFin;
                if (FechaFinal == "")
                    dFechaFin = dFechaIni.Date.AddSeconds(86399);
                else
                ...
                ////report.strWhereVendedores(Vendedor, "VEN.VendedorID"), report.strWhereRutas(Rutas, "RUT.RUTClave"), report.strWhereCedis(Cedis, "ALN.AlmacenID"), report.GetDateQuery(dateStatus, FechaInicial, FechaFinal, false, "Dia.FechaCaptura"));
                sConsulta2.AppendLine("INNER JOIN Vendedor VEN (NOLOCK) ON VIS.VendedorId = VEN.VendedorId " + (Vendedor != "" ? "AND VEN.VendedorID IN (" + Vendedor + ")" : ""));
                sConsulta2.AppendLine("INNER JOIN Dia (NOLOCK) ON DIA.DiaClave = VIS.DiaClave and Dia.FechaCaptura between '" + dFechaIni.ToString("s") + "' and '" + dFechaFin.ToString("s") + "'");
                sConsulta2.AppendLine("INNER JOIN Ruta RUT (NOLOCK) ON RUT.RUTClave = VIS.RUTClave AND RUT.RUTClave <> 'RUT001' " + (Rutas != "" ? "AND RUT.RUTClave IN(" + Rutas + ")" : ""));
                ...
                KilometrajeString.AppendLine("and CAV.FechaHoraInicial between '" + dFechaIni.ToString("s") + "' and '" + dFechaFin.ToString("s") + "'");
                if (Rutas != "")
                    sConsulta.AppendLine("and RUT.RUTClave in (" + Rutas + ") ");
