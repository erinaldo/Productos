    ...
    public class GetRoutesController : ApiController
    {
        ...
        public IHttpActionResult GET(string Cedis, string inactivos)
        {
            try
            {
                ...
                {
                    QueryString = "select R.RUTClave, R.Descripcion from Ruta R (nolock) inner join Almacen (nolock) A on R.AlmacenID = A.AlmacenID where R.TipoEstado = 1 and A.AlmacenID IN (" + Cedis + ") ";
                }
                else
                {
