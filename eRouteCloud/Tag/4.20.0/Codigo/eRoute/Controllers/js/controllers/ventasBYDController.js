(function () {
    ...
    ventasBYDController.$inject = ['$scope', '$http', '$mdToast', '$q', '$timeout', 'NgMap', 'cfpLoadingBar', '$filter', "menuServices", "reportServices"];

    function ventasBYDController($scope, $http, $mdToast, $q, $timeout, NgMap, cfpLoadingBar, $filter, menuServices, reportServices) {
        ...
        ctrl.clients = GetClientsT();
        ctrl.GetClientsT = GetClientsT;
        function GetClientsT(selectedItem) {
                if (selectedItem == undefined)
                    selectedItem = "x";
            $http({
        ...
        function GetClientsR(Route) {
            ...
            if(Route != undefined)
                ...
                Cedis = "x"
            $http({
                url: url + '/api/GetClients?Id=Todos&Routes=' + Route + '&Cedis=' + Cedis,
                method: 'GET',
                headers: {
                    Authorization: window.sessionStorage.getItem('Token'),
                    'Content-Type': 'application/json'
                ...
                $http.post(url + '/Reports/GetCondition', data, { ignoreLoadingBar: true })
                    .success(function (data, status, headers, config) {
                        if (ctrl.Todos == false)
                            ...
                            window.open(url + "/Reports/Ver?VAVClave=" + data.VAVClave + "&Cedis=" + data.Cedis.replace(/\+/g, "%2B") + "&nombreReport=" + data.nombreReport + "&nombreCedis=" + data.nombreCedis + "&FechaInicial=" + data.FechaInicial + "&FechaFinal=" + data.FechaFinal + "&dateStatus=" + data.Rango + "&Rutas=" + ctrl.RUTClave + "&Clientes=" + "" + "&Vendedor=" + ctrl.VendedorId + "&detallado=" + data.reportType + "&unidadMedida=" + data.unidadMedida, "_blank");
                        cfpLoadingBar.complete();
                    })
