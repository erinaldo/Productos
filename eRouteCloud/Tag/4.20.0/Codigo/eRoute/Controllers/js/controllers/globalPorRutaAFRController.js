(function () {
    angular
        .module('eRouteApp')
        .controller('globalPorRutaAFRController', globalPorRutaAFRController)
    globalPorRutaAFRController.$inject = ['$scope', '$http', '$mdToast', '$q', '$timeout', 'NgMap', 'cfpLoadingBar', '$filter', "menuServices", "reportServices", "valorReferencia"];

    function globalPorRutaAFRController($scope, $http, $mdToast, $q, $timeout, NgMap, cfpLoadingBar, $filter, menuServices, reportServices, valorReferencia) {
            ...
            if (countRutas <= 0) {
                ...
                .success(function (data, status, headers, config) {
                    if (data.Cedis == undefined)
                        data.Cedis = "";
                    //window.open(url + "/Reports/Ver?VAVClave=" + data.VAVClave + "&Cedis=" + data.Cedis + "&FechaInicial=" + data.FechaInicial + "&FechaFinal=" + data.FechaFinal + "&dateStatus=" + data.Rango + "&Rutas=" + data.Rutas + "&detallado=false", "_blank");
