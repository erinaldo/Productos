(function () {
    angular
        .module('eRouteApp')
        .controller('clientesEsquemaVITController', clientesEsquemaVITController)
    clientesEsquemaVITController.$inject = ['$scope', '$http', '$mdToast', '$q', '$timeout', 'NgMap', 'cfpLoadingBar', '$filter', "menuServices", "reportServices"];

    function clientesEsquemaVITController($scope, $http, $mdToast, $q, $timeout, NgMap, cfpLoadingBar, $filter, menuServices, reportServices) {
        ...
        self.SubmitForm = function () {
            ...
            if (countSchemes <= 0) {
                ...
                .success(function (data, status, headers, config) {
                    window.open(url + "/Reports/Ver?VAVClave=" + self.VAVClave + "&Cedis=" + self.selectedItem.value.replace(/\+/g, "%2B") + "&nombreCedis=" + self.selectedItem.display + "&Productos=" + data.Productos + "&nombreProductos=" + data.NombreProductos, "_blank");
                    cfpLoadingBar.complete();
                })
