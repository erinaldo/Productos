...
.controller('productosController', ["$scope", "$rootScope", "$http", 'valorReferencia', "messageBox", "translationService", "defaultBox", function ($scope, $rootScope, $http, valorReferencia, messageBox, translationService, defaultBox) {
    ...
    vm.checar = function (EsquemaID, Nombre, Clave /*Abreviatura*/) {
        var existe = false;
        //if (vm.EsquemasList <= 0) {
        //    vm.EsquemasList.push({ EsquemaID: EsquemaID, Nombre: Nombre, ProductoClave: vm.ProductoClavePRO, Abreviatura: Abreviatura });
        //} else {
                    ...
                    if (PRUTipo == "" || PRUTipo === undefined) {
                        ...
                        if (value.PRUTipoUnidad == PRUTipo) {
                            ...
                            if (existe == false) {
                                ...
                                value.ProductoDetalle.push({ Index: $scope.PRUTipoIndexAux, NombreProducto: Nombre, ProductoClave: vm.ProductoClavePRO, PRUTipoUnidad: PRUTipo, ProductoDetClave: ProductoClave, Prestamo: '', Factor: value.ProductoDetalle.length + 1 });
                            }
    ...
    vm.validarEliminarProductoRelacionado = function (index, PRUTipo, Producto) {
        if (PRUTipo === undefined) {
            ...
            if (PRUTipo != "") {
                angular.forEach($scope.cUnidadesList, function (value, key) {
                    if (value.PRUTipoUnidad == PRUTipo) {
                        angular.forEach(value.ProductoDetalle, function (v, k) {
                            if (v.ProductoDetClave == Producto) {
                                value.ProductoDetalle.splice(k, 1);
                            }
                        });
                    }
                }, this);
    ...
    vm.ObtenerDetalleImpuestoProducto = function (ProductoClave) {
        if (ProductoClave != "") {
            ...
            $http({
                ...
                imp = data;
                //vm.impList = data;
                //angular.forEach(data, function (proImp) {
                //    vm.TipoEstadoImp = proImp.TipoEstado;
