app.controller('r271DocumentoSaldoDIVController', ['valorReferencia', '$http', 'cfpLoadingBar', function (valorReferencia, $http, cfpLoadingBar) {
    ...
    self.GetSellers = function (selectedItem, state) {
        ...
        $http({
            url: url + '/API/GetModel/GetSellers?cedis=' + selectedItem.replace(/\+/g, '%2B') + '&state=' + state,
            method: 'GET',
            headers: {
                Authorization: window.sessionStorage.getItem('Token'),
                'Content-Type': 'application/json'
    ...
    self.CountSellers = function () {
        ...
        if (count !== 0) {
            if (count !== self.sellers.length) {
                self.GetClientSeller(null, null, self.sellers);
                self.selectAllSellers = false;
            } else if (count === self.sellers.length) {
                self.GetClientSeller(null, null, self.sellers);
                self.selectAllSellers = true;
            }
        }
    }
    ...
    self.GetClients = function (clientScheme, routes, sellers) {
        ...
        $http.post(url + '/Reports/GetObjectString', {
            ...
            $http({
                url: url + '/API/GetModel/GetClients?clientScheme=' + clientScheme + '&routes=' + dataRoutes.data + '&sellers=' + sellers,
                method: 'GET',
                headers: {
                    Authorization: window.sessionStorage.getItem('Token'),
                    'Content-Type': 'application/json'
    ...
    self.GetClientSeller = function (clientScheme, routes, sellers) {
        self.showErrorForm = false;
        self.clients = undefined;
        self.querySchemes = '';
        self.queryClients = '';
        self.selectAllClients = false;
        $http.post(url + '/Reports/GetObjectString', {
            Object: GetArrayValues(sellers)
        }).then(function successCallBack(dataSellers) {
            $http({
                url: url + '/API/GetModel/GetClients?clientScheme=' + clientScheme + '&routes=' + routes + '&sellers=' + dataSellers.data,
                method: 'GET',
                headers: {
                    Authorization: window.sessionStorage.getItem('Token'),
                    'Content-Type': 'application/json'
    ...
    self.Submit = function () {
        ...
        if (self.selectedClientScheme != undefined) {
            esquema = GetArrayValues(self.schemes, self.selectedClientScheme);
            clientes = GetArrayValues(self.clients);
        }
        if (!self.selectAllClients && clientes != null) {
            clientes = GetArrayValues(self.clients);
        }
        
        
        if (rutas.length <= 0 && vendedores.length <= 0) {
