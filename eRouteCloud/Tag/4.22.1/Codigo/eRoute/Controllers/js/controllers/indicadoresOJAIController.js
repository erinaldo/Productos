(function () {
    angular
        .module('eRouteApp')
        .controller('indicadoresOJAIController', indicadoresOJAIController)
    indicadoresOJAIController.$inject = ['$scope', '$http', '$mdToast', '$q', '$timeout', 'NgMap', 'cfpLoadingBar', '$filter', "menuServices", "reportServices", "valorReferencia"];

    function indicadoresOJAIController($scope, $http, $mdToast, $q, $timeout, NgMap, cfpLoadingBar, $filter, menuServices, reportServices, valorReferencia) {
        ...
        function SubmitForm(Cedis, dateStatus, Esquemas, pvEsFecha, campoFecha) {
            ...
            angular.forEach(ctrl.routes, function (item) {
                if (item.RUTClave == ctrl.RUTClave) {
                    item.Checked = true;
                    countRoutes++;
                }
                else {
            ...
            if (ctrl.State === undefined) {
                ...
                $http.post(url + '/Reports/GetCondition', data, { ignoreLoadingBar: true })
                    .success(function (data, status, headers, config) {
                        window.open(url + "/Reports/Ver?VAVClave=" + data.VAVClave + "&Cedis=" + data.Cedis.replace(/\+/g, "%2B") + "&nombreReport=" + data.nombreReport + "&nombreCedis=" + data.nombreCedis + "&FechaInicial=" + data.FechaInicial + "&FechaFinal=" + data.FechaFinal + "&dateStatus=" + data.Rango + "&Rutas=" + data.Rutas + "&detallado=" + data.reportType + "&extra1=" + ctrl.ObjetivoVentaMensualCajas + "&extra2=" + ctrl.ObjetivoVisitasEfectivas, "_blank")
                        cfpLoadingBar.complete();
                    })
