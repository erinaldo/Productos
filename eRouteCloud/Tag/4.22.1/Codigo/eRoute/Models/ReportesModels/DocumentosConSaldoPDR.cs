...
{
    public class DocumentosConSaldoPDR
    {
        private IDbConnection Connection = new SqlConnection(ConfigurationManager.ConnectionStrings["eRouteConnection"].ConnectionString);
        private string QueryString = "";

        public XtraReport GetReport(string pvCondicion, string RutasSplit, string VendedoresSplit, string ClientesSplit, string Cedis, string nombreCedis)
        {
            try
            {
                ...
                sConsulta.AppendLine("CLI.Clave as CLIClave, CLI.RazonSocial as CLINombre, CLI.NombreCorto as CLINCorto, TRP.Folio, Dia.FechaCaptura, TRP.FechaCobranza, TRP.Total - isnull(TT.TotalDevConsignacion, 0) AS Total, TRP.Saldo ");
                ...
                sConsulta.AppendLine("ORDER BY ALM.Clave, USU.Clave, RUT.RUTClave, CLI.Clave, Dia.FechaCaptura asc, TRP.Folio asc ");
                ...
                List<DocumentosConSaldoPDRModel> Documentos = Connection.Query<DocumentosConSaldoPDRModel>(QueryString, null, null, true, 600).ToList();
                ...
                List<DocumentosConSaldoPDRModel> formatlist = new List<DocumentosConSaldoPDRModel>();
                    ...
                    foreach(var gVendedor in vendedorList)
                    {
                        ...
                        foreach (var gRuta in rutaList)
                        {
                            ...
                            foreach (var gCliente in clienteList)
                            {
                                ...
                                {
                                    formatlist.Add(new DocumentosConSaldoPDRModel
                                    {
                                        ...
                                        CLINCorto = objetoAgrupado.CLINCorto,
                ...
                ReporteDocumentosConSaldoPDR report = new ReporteDocumentosConSaldoPDR();
                ...
                report.detalleCLINCorto.DataBindings.Add("Text", report.DataSource, "CLINCorto");
    ...
    class DocumentosConSaldoPDRModel
    {
        ...
        public string CLINCorto { get; set; }
