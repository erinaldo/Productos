...
namespace eRoute.Models.ReportesModels
{
    public class IndicadoresOJAI
    {
        ...
        public bool GetReport(string AlmacenNombre, string Fecha, string RUTClave, bool Detallado, int ObjetivoVentaMensualCajas, int ObjetivoVisitasEfectivas)
        {
            try
            {
                DateTime dFecha;
        ...
        private ArchivoXlsModel GenerarExcel()
        {
            ...
            MyOpenXML.createCell(2, ref Rows, "B2", CellValues.String, ValorReferencia.ObtenerDescripcion("REPORTEW", "244", "EM"), 4);

            MyOpenXML.createRow(3, ref Rows);
            MyOpenXML.createCell(3, ref Rows, "B3", CellValues.String, sAlmacenNombre, 4);

            MyOpenXML.createRow(4, ref Rows);
            MyOpenXML.createCell(4, ref Rows, "B4", CellValues.String, Mensaje.ObtenerDescripcion("XSemana", "EM") + " " + lstIndicadoresRuta[0].Semana.ToString() + ": " + lstIndicadoresRuta[0].FechaInicial.ToShortDateString() + " al " + lstIndicadoresRuta[0].FechaFinal.ToShortDateString(), 4);

            MyOpenXML.createRow(5, ref Rows);
            MyOpenXML.createCell(5, ref Rows, "B5", CellValues.String, (bDetallado ? Mensaje.ObtenerDescripcion("XDetallado", "EM") : Mensaje.ObtenerDescripcion("XGeneral", "EM")), 4);
