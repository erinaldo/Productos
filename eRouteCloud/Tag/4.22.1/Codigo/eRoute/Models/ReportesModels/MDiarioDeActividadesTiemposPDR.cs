...
namespace eRoute.Models.ReportesModels
{
    public class MDiarioDeActividadesTiemposPDR
    {
        ...
        public XtraReport GetReport(string pvCondicion, string pvCondicion1, string RutasSplit, string FechaInicial, string FechaFinal, string NomCEDI)
        {
            ...
            try
            {
                ...
                bool pvKilomoetraje = Connection.Query<int>("SELECT TOP 1 Kilometraje FROM Vendedor VEN (NOLOCK) " + pvCondicion + " ORDER BY MFechaHora DESC", null, null, true, 600).FirstOrDefault() == 1;
                ...
                sConsulta.AppendLine("(select isnull(sum(Total),0) from TransProd TP ");
                sConsulta.AppendLine("inner join Dia on TP.DiaClave = Dia.DiaClave and Dia.FechaCaptura = Fecha and TP.tipo=1 and TP.tipofase<>0");
                sConsulta.AppendLine("inner join Visita Vis on Vis.VisitaClave = TP.VisitaClave and Vis.DiaClave = Dia.DiaClave and Vis.VendedorID = (Select top 1 VendedorID from #TmpDiario_1)) as TotalV,");
                sConsulta.AppendLine("(select isnull(sum(Total),0) from TransProd TP");
                sConsulta.AppendLine("inner join Dia on TP.DiaClave = Dia.DiaClave and Dia.FechaCaptura = Dateadd(dd, -7, Fecha) and TP.tipo=1 and TP.tipofase<>0");
                sConsulta.AppendLine("inner join Visita Vis on Vis.VisitaClave = TP.VisitaClave and Vis.DiaClave = Dia.DiaClave and Vis.VendedorID = (Select top 1 VendedorID from #TmpDiario_1)) as TotalM,");
                ...
                sConsulta.AppendLine("from camionvendedor VEN");
