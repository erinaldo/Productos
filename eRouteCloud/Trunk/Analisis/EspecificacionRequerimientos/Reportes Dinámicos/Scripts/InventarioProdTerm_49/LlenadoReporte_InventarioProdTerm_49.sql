...
SET @NombreCodigoReporte = 'InventarioProdTerm_49'
SET @NombreReporte = 'Inventario De Producto Terminado'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[stpr_InventarioProdTerm_49] @filterCEDIS = {0}, @filterStartDate = {1}' WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[stpr_InventarioProdTerm_49] @filterCEDIS = {0}, @filterStartDate = {1}')
	END
