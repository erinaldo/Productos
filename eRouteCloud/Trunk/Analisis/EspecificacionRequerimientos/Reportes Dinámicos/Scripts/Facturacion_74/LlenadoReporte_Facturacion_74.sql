...
SET @NombreCodigoReporte = 'Facturacion_74'
SET @NombreReporte = 'Facturacion'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[stpr_Facturacion_74] @filterSeller = {0}, @filterStartDate = {1}, @filterEndDate = {2}, @filterCustomer = {3}' WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[stpr_Facturacion_74] @filterSeller = {0}, @filterStartDate = {1}, @filterEndDate = {2}, @filterCustomer = {3}')
	END
...
IF NOT EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[FiltrosDinamicos] (NOLOCK) WHERE [RepDinID] = @IdReporte)
	BEGIN
		...
		INSERT INTO [dbo].[FiltrosDinamicos]([RepDinID], [TipoFiltro], [Opciones])
		SELECT TOP 1 [RepDinID], 7, 'OSAI' FROM [dbo].[ReporteDinamico] WHERE [RepDinID] = @IdReporte
	END
