...
SET @NombreCodigoReporte = 'GastosVendedor_0'
SET @NombreReporte = 'Gastos Del Vendedor'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[stpr_GastosVendedor_0] @filterCEDIS = {0}, @filterSellers = {1}, @filterStartDate = {2}, @filterDateDate = {3}'  WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[stpr_GastosVendedor_0] @filterCEDIS = {0}, @filterSellers = {1}, @filterStartDate = {2}, @filterDateDate = {3}')
	END
