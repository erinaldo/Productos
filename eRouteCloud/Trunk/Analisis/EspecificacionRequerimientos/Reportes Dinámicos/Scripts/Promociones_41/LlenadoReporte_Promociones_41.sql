...
SET @NombreCodigoReporte = 'Promociones_41'
SET @NombreReporte = 'Promociones'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[sptr_Promociones_41] @filterCedi = {0}, @filterDateIni = {1}, @filterDateEnd = {2}' WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[sptr_Promociones_41] @filterCedi = {0}, @filterDateIni = {1}, @filterDateEnd = {2}')
	END
