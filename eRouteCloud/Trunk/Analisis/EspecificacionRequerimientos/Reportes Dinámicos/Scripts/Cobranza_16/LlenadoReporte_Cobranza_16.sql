...
SET @NombreCodigoReporte = 'Cobranza_16'
SET @NombreReporte = 'Cobranza (CHO)'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[stpr_Cobranza_16] @FILTROCEDI = {0}, @FILTRORUTAS = {1}, @FILTROFECHAINI = {2}, @FILTROFECHAFIN = {3}' WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[stpr_Cobranza_16] @FILTROCEDI = {0}, @FILTRORUTAS = {1}, @FILTROFECHAINI = {2}, @FILTROFECHAFIN = {3}')
	END
