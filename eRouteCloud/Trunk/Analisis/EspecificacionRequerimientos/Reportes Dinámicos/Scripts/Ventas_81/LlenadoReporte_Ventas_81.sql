...
SET @NombreCodigoReporte = 'Ventas_81'
SET @NombreReporte = 'Ventas'

IF EXISTS (SELECT TOP 1 [RepDinID] FROM [dbo].[ReporteDinamico] WHERE [Nombre] = @NombreCodigoReporte)
	BEGIN
		UPDATE [dbo].[ReporteDinamico] SET [Consulta] = 'EXEC [dbo].[stpr_Ventas_81] @filtroVendedores = {0}, @filtroFechaInicio = {1}, @filtroFechaFin = {2}'  WHERE [Nombre] = @NombreCodigoReporte
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[ReporteDinamico]([Nombre], [Consulta]) VALUES
		(@NombreCodigoReporte, 'EXEC [dbo].[stpr_Ventas_81] @filtroVendedores = {0}, @filtroFechaInicio = {1}, @filtroFechaFin = {2}')
	END
