...
{
    public class TiemposEnRutaMED
    {
        private IDbConnection Connection = new SqlConnection(ConfigurationManager.ConnectionStrings["eRouteConnection"].ConnectionString);
        private string QueryString = "";
        private DateTime fInicio;
        private DateTime fFin;
        TiemposRutaReportMED report = new TiemposRutaReportMED();
        SubKilometrosMED kilometraje = new SubKilometrosMED();
        ...
        public XtraReport GetReport(string NombreReporte, string NombreEmpresa, MemoryStream LogoEmpresa, string NombreCEDIS, string CEDIS, string EstatusFecha, string FechaInicial, string FechaFinal, string Rutas, string Vendedores, bool FiltroReporte)
        {
            ...
            //Antes de realizar un cambio de formato en esta fecha, favor de revisar con Paula Rojas
            ...
            Rutas = (Rutas == "" ? "null" : string.Format("'{0}'", Rutas));
            CEDIS = (CEDIS == "" ? "null" : string.Format("'{0}'", CEDIS));
            Vendedores = (Vendedores == "" ? "null" : string.Format("'{0}'", Vendedores));
            ...
            sConsulta.AppendLine("select VIS.VisitaClave, VIS.DiaClave, VIS.ClienteClave, VIS.VendedorId, VIS.RutClave, VIS.FechaHoraInicial, VIS.FechaHoraFinal, VIS.CodigoLeido, VIS.DistanciaGPS, VCH.AlmacenId");
            ...
            sConsulta.AppendLine(string.Format("where VIS.RUTClave <>'RUT001' and VCH.AlmacenID = {0} ", CEDIS));
            sConsulta.AppendLine(string.Format("and Dia.FechaCaptura BETWEEN '{0}' AND '{1}'", FechaInicial, FechaFinal));
            sConsulta.AppendLine(string.Format(" AND ((VIS.RUTClave IN (SELECT Datos FROM FNSplit({0}, ','))) OR {0} IS NULL) ", Rutas));
            ...
            sConsulta.AppendLine(string.Format("where AGV.RUTClave <>'RUT001' and ALN.AlmacenID = {0} ", CEDIS));
            sConsulta.AppendLine(string.Format("and Dia.FechaCaptura BETWEEN '{0}' AND '{1}'", FechaInicial, FechaFinal));
            sConsulta.AppendLine(string.Format(" AND ((AGV.RUTClave IN (SELECT Datos FROM FNSplit({0}, ','))) OR {0} IS NULL) ", Rutas));
            sConsulta.AppendLine(") as t");
            ...
            sConsulta.AppendLine("select case when VEN.JornadaTrabajo = 1 then isnull(VEJ.VEJFechaInicial,VIS.FechaHoraInicial) else VIS.FechaHoraInicial end as HoraInicialJornada, case when VEN.JornadaTrabajo = 1 then isnull(VEJ.FechaFinal,VIS.FechaHoraFinal) else VIS.FechaHoraFinal end as HoraFinalJornada, VIS.VisitaClave, VIS.DiaClave, VIS.ClienteClave, VIS.VendedorId, VIS.RutClave, VIS.FechaHoraInicial, VIS.FechaHoraFinal, ");
            sConsulta.AppendLine("Case when (Select LimiteGPS from AseguramientoVisita where MCNClave = VEN.MCNClave) >= VIS.DistanciaGPS then '*' else '' end as Presencial, VIS.CodigoLeido,  ");
            ...
            sConsulta.AppendLine("max(SECOrden) as Secuencia, CLIClave as Clave, CLIRazonSocial as NombreCliente, LocalidadPoblacion, CodigoNoLeido=Presencial, ");
            ...
            sConsulta.AppendLine("Case when (Select top 1 count(transprodId) from TransProd (NOLOCK) TRP where TRP.VisitaClave = TMP.VisitaClave and TRP.ClienteClave = TMP.ClienteClave and TRP.Tipo = 1 and TRP.TipoFase <> 0)> 0 then 'Si' else 'No' end as Venta, ");
            sConsulta.AppendLine("Case when(Select top 1 count(transprodId) from TransProd (NOLOCK)TRP where TRP.VisitaClave1 = TMP.VisitaClave and TRP.ClienteClave = TMP.ClienteClave and TRP.Tipo = 1 and TRP.TipoFase in (2, 3)) > 0 then 'Si' else 'No' end as Surtido, ");
            sConsulta.AppendLine("convert(varchar(25), isnull(VAD.Descripcion,0)) as Concepto, ");
            ...
            sConsulta.AppendLine("left join ImproductividadVenta IMV on IMV.VisitaClave = TMP.VisitaClave AND IMV.DIACLAVE=TMP.DIACLAVE ");
            sConsulta.AppendLine("left join VAVDescripcion VAD on IMV.TipoMotivo = VAD.VAVClave and VAD.VARCodigo = 'MOTIMPRO' and VAD.VADTipoLenguaje = 'EM' ");

            sConsulta.AppendLine("group by HoraInicialJornada,HoraFinalJornada,ALNClave, ALNNombre, TMP.VisitaClave, VENNombre, FechaHoraInicial, RUTClave, ClienteClave, CodigoLeido, Presencial, VAD.Descripcion,");
            ...
            Connection.Open();
            List<TiemposEnRutaMEDModel> User = Connection.Query<TiemposEnRutaMEDModel>(QueryString, null, null, true, 9000).ToList();
            Connection.Close();
            ...
            sConsulta2.AppendLine(string.Format("INNER JOIN Vendedor VEN (NOLOCK) ON VIS.VendedorId = VEN.VendedorId AND ((VEN.VendedorId IN (SELECT Datos FROM FNSplit(({0}), ','))) OR {0} IS NULL) ", Vendedores));
            sConsulta2.AppendLine(string.Format("INNER JOIN Dia (NOLOCK) ON DIA.DiaClave = VIS.DiaClave and Dia.FechaCaptura BETWEEN '{0}' AND '{1}'", FechaInicial, FechaFinal));
            sConsulta2.AppendLine(string.Format("INNER JOIN Ruta RUT (NOLOCK) ON RUT.RUTClave = VIS.RUTClave AND RUT.RUTClave <> 'RUT001' AND ((RUT.RUTClave IN (SELECT Datos FROM FNSplit({0}, ','))) OR {0} IS NULL) ", Rutas));
            ...
            sConsulta2.AppendLine(string.Format("LEFT JOIN Almacen ALN (NOLOCK) ON VCH.AlmacenId = ALN.AlmacenID AND ALN.AlmacenID = {0}", CEDIS));
            ...
            sConsulta2.AppendLine("LEFT JOIN AseguramientoVisita AV (NOLOCK) on VEN.MCNClave = AV.MCNClave ");
            sConsulta2.AppendLine("WHERE AGV.ClienteClave IS NULL and VIS.DistanciaGPS <= AV.LimiteGPS ");
            ...
            sConsulta2.AppendLine("LEFT JOIN AgendaVendedor AGV (NOLOCK) ON AGV.DiaClave = VIS.DiaClave AND AGV.ClienteClave = VIS.ClienteClave AND AGV.RUTClave = VIS.RUTClave AND AGV.VendedorId = VIS.VendedorID ");
            sConsulta2.AppendLine("LEFT JOIN AseguramientoVisita AV (NOLOCK) on VEN.MCNClave = AV.MCNClave ");
            sConsulta2.AppendLine("WHERE AGV.ClienteClave IS NULL AND TRP.TransProdID IS NULL and VIS.DistanciaGPS <= AV.LimiteGPS ");
            sConsulta2.AppendLine("GROUP BY VIS.RutClave, VIS.VendedorId, VIS.DiaClave ");
            ...
            Connection.Open();
            List<ClientesFueraFrecuenciaPresencialMED> ClientesFFP = Connection.Query<ClientesFueraFrecuenciaPresencialMED>(QueryString, null, null, true, 9000).ToList();
            Connection.Close();

            StringBuilder sConsulta3 = new StringBuilder();
            sConsulta3.AppendLine("SET ANSI_WARNINGS OFF ");
            sConsulta3.AppendLine("SET NOCOUNT ON ");
            sConsulta3.AppendLine("SELECT VEN.Nombre AS Vendedor, RES.SINVENTA AS SinVenta, RES.NOVISITADOS + RES.VISITADOS AS TotalClientes, ");
            sConsulta3.AppendLine("RES.NOVISITADOS, RES.VISITADOS, ");
            sConsulta3.AppendLine("CASE WHEN CAST((RES.NOVISITADOS + RES.VISITADOS) AS FLOAT) = 0 THEN 0  ELSE (CAST(RES.VISITADOS AS FLOAT) / CAST((RES.NOVISITADOS + RES.VISITADOS) AS FLOAT)) * 100 END AS VISITAEfectiva, ");
            sConsulta3.AppendLine("CASE WHEN RES.VISITADOS = 0 THEN 0 ELSE CAST((RES.VISITADOS - RES.SINVENTA) AS FLOAT)/ CAST(RES.VISITADOS AS FLOAT) * 100 END AS EfectividadCompra, ");
            sConsulta3.AppendLine("RUT.RUTClave + ' ' + RUT.Descripcion AS Ruta, ALN.Clave + ' ' + ALN.Nombre AS CEDI ");
            sConsulta3.AppendLine("FROM( ");
            sConsulta3.AppendLine("SELECT RutClave, VendedorId, DiaClave, SUM(VISITADOS) AS VISITADOS, SUM(NOVISITADOS) AS NOVISITADOS, SUM(SINVENTA) AS SINVENTA ");
            sConsulta3.AppendLine("FROM( ");
            sConsulta3.AppendLine("SELECT VIS.RutClave, VIS.VendedorId, VIS.DiaClave, ");
            sConsulta3.AppendLine("COUNT(DISTINCT VIS.ClienteClave) AS VISITADOS, 0 AS NOVISITADOS, 0 AS SINVENTA");
            sConsulta3.AppendLine("FROM Visita VIS (NOLOCK) ");
            sConsulta3.AppendLine(string.Format("INNER JOIN Vendedor VEN (NOLOCK) ON VIS.VendedorId = VEN.VendedorId AND ((VEN.VendedorId IN (SELECT Datos FROM FNSplit(({0}), ','))) OR {0} IS NULL) ", Vendedores));
            sConsulta3.AppendLine(string.Format("INNER JOIN Dia (NOLOCK) ON DIA.DiaClave = VIS.DiaClave and Dia.FechaCaptura BETWEEN '{0}' AND '{1}'", FechaInicial, FechaFinal));
            sConsulta3.AppendLine(string.Format("INNER JOIN Ruta RUT (NOLOCK) ON RUT.RUTClave = VIS.RUTClave AND RUT.RUTClave <> 'RUT001' AND ((RUT.RUTClave IN (SELECT Datos FROM FNSplit({0}, ','))) OR {0} IS NULL) ", Rutas));
            sConsulta3.AppendLine("INNER JOIN VENCentroDistHist VCH (NOLOCK) ON VCH.VendedorId = VEN.VendedorID AND Dia.FechaCaptura BETWEEN VCH.VCHFechaInicial AND VCH.FechaFinal ");
            sConsulta3.AppendLine(string.Format("LEFT JOIN Almacen ALN (NOLOCK) ON VCH.AlmacenId = ALN.AlmacenID AND ALN.AlmacenID = {0}", CEDIS));
            sConsulta3.AppendLine("LEFT JOIN AgendaVendedor AGV (NOLOCK) ON AGV.DiaClave = VIS.DiaClave AND AGV.ClienteClave = VIS.ClienteClave AND AGV.RUTClave = VIS.RUTClave AND AGV.VendedorId = VIS.VendedorID ");
            sConsulta3.AppendLine("LEFT JOIN AseguramientoVisita AV (NOLOCK) on VEN.MCNClave = AV.MCNClave ");
            sConsulta3.AppendLine("WHERE AGV.ClienteClave IS NULL and VIS.DistanciaGPS > AV.LimiteGPS ");
            sConsulta3.AppendLine("GROUP BY VIS.RutClave, VIS.VendedorId, VIS.DiaClave ");
            sConsulta3.AppendLine("UNION ");
            sConsulta3.AppendLine("SELECT VIS.RutClave, VIS.VendedorId, VIS.DiaClave, ");
            sConsulta3.AppendLine("0 AS VISITADOS, 0 AS NOVISITADOS, COUNT(DISTINCT VIS.ClienteClave) AS SINVENTA ");
            sConsulta3.AppendLine("FROM Visita VIS (NOLOCK) ");
            sConsulta3.AppendLine(string.Format("INNER JOIN Vendedor VEN (NOLOCK) ON VIS.VendedorId = VEN.VendedorId AND ((VEN.VendedorId IN (SELECT Datos FROM FNSplit(({0}), ','))) OR {0} IS NULL) ", Vendedores));
            sConsulta3.AppendLine(string.Format("INNER JOIN Dia (NOLOCK) ON DIA.DiaClave = VIS.DiaClave and Dia.FechaCaptura BETWEEN '{0}' AND '{1}'", FechaInicial, FechaFinal));
            sConsulta3.AppendLine(string.Format("INNER JOIN Ruta RUT (NOLOCK) ON RUT.RUTClave = VIS.RUTClave AND RUT.RUTClave <> 'RUT001' AND ((RUT.RUTClave IN (SELECT Datos FROM FNSplit({0}, ','))) OR {0} IS NULL) ", Rutas));
            sConsulta3.AppendLine("INNER JOIN VENCentroDistHist VCH (NOLOCK) ON VCH.VendedorId = VEN.VendedorID AND Dia.FechaCaptura BETWEEN VCH.VCHFechaInicial AND VCH.FechaFinal ");
            sConsulta3.AppendLine("LEFT JOIN TransProd TRP (NOLOCK) ON TRP.VisitaClave = VIS.VisitaClave AND TRP.DiaClave = VIS.DiaClave AND TRP.Tipo = 1 AND TRP.TipoFase <> 0 ");
            sConsulta3.AppendLine(string.Format("LEFT JOIN Almacen ALN (NOLOCK) ON VCH.AlmacenId = ALN.AlmacenID AND ALN.AlmacenID = {0}", CEDIS));
            sConsulta3.AppendLine("LEFT JOIN AgendaVendedor AGV (NOLOCK) ON AGV.DiaClave = VIS.DiaClave AND AGV.ClienteClave = VIS.ClienteClave AND AGV.RUTClave = VIS.RUTClave AND AGV.VendedorId = VIS.VendedorID ");
            sConsulta3.AppendLine("LEFT JOIN AseguramientoVisita AV (NOLOCK) on VEN.MCNClave = AV.MCNClave ");
            sConsulta3.AppendLine("WHERE AGV.ClienteClave IS NULL AND TRP.TransProdID IS NULL and VIS.DistanciaGPS > AV.LimiteGPS ");
            sConsulta3.AppendLine("GROUP BY VIS.RutClave, VIS.VendedorId, VIS.DiaClave ");
            sConsulta3.AppendLine(") AS T ");
            sConsulta3.AppendLine("GROUP BY RutClave, VendedorId, DiaClave ");
            sConsulta3.AppendLine(") AS RES ");
            sConsulta3.AppendLine("INNER JOIN Vendedor VEN (NOLOCK) ON VEN.VendedorID = RES.VendedorID ");
            sConsulta3.AppendLine("INNER JOIN (SELECT DISTINCT DiaClave, VendedorId, ClaveCEDI FROM AgendaVendedor (NOLOCK)) AGV ON RES.VendedorId = AGV.VendedorId AND RES.DiaClave = AGV.DiaClave ");
            sConsulta3.AppendLine("INNER JOIN Ruta RUT (NOLOCK) ON RUT.RUTClave = RES.RUTClave ");
            sConsulta3.AppendLine("INNER JOIN Almacen ALN (NOLOCK) ON AGV.ClaveCEDI = ALN.Clave ");
            sConsulta3.AppendLine("SET NOCOUNT OFF ");
            QueryString = "";

            QueryString = sConsulta3.ToString();

            Connection.Open();
            List<ClientesFueraFrecuenciaNoPresencialMED> ClientesFFNP = Connection.Query<ClientesFueraFrecuenciaNoPresencialMED>(QueryString, null, null, true, 9000).ToList();
            Connection.Close();

            var lista = (from c in User
                         select c).ToList();

            var listaClientesP = (from c in ClientesFFP
                                  select c).ToList();

            var listaClientesNP = (from c in ClientesFFNP
                                   select c).ToList();

            var s = (from gr in lista group gr by new { gr.CEDI, gr.Vendedor, gr.Ruta, gr.Fecha } into grupo select grupo);
            var cliffP = (from gr in listaClientesP group gr by new { gr.Vendedor, gr.Ruta } into grupo select grupo);
            var cliffNP = (from gr in listaClientesNP group gr by new { gr.Vendedor, gr.Ruta } into grupo select grupo);
            List<TiemposEnRutaMEDModel> formatlist = new List<TiemposEnRutaMEDModel>();
            ...
            long vtaspresenciales = 0;
            long vtasnopresenciales = 0;
            ...
            foreach (var grupo in s)
            {
                ...
                {
                    formatlist.Add(new TiemposEnRutaMEDModel
                    {
                        ...
                        //clientes fuera de la frecuencia Presenciales
                        SinVentaFP = objetoAgrupado.SinVentaFP,
                        TotalClientesFP = objetoAgrupado.TotalClientesFP,
                        NOVISITADOSFP = objetoAgrupado.NOVISITADOSFP,
                        VISITADOSFP = objetoAgrupado.VISITADOSFP,
                        VISITAEfectivaFP = objetoAgrupado.VISITAEfectivaFP,
                        EfectividadCompraFP = objetoAgrupado.EfectividadCompraFP,
                        //clientes fuera de la frecuencia No Presenciales
                        SinVentaFNP = objetoAgrupado.SinVentaFNP,
                        TotalClientesFNP = objetoAgrupado.TotalClientesFNP,
                        NOVISITADOSFNP = objetoAgrupado.NOVISITADOSFNP,
                        VISITADOSFNP = objetoAgrupado.VISITADOSFNP,
                        VISITAEfectivaFNP = objetoAgrupado.VISITAEfectivaFNP,
                        EfectividadCompraFNP = objetoAgrupado.EfectividadCompraFNP
                    });
                    if (firstFromList)
                    {
                    ...
                    if (objetoAgrupado.CodigoNoLeido == "*")
                        if (objetoAgrupado.Venta == "Si")
                            vtaspresenciales += 1;

                    if (objetoAgrupado.CodigoNoLeido == "")
                        if (objetoAgrupado.Venta == "Si")
                            vtasnopresenciales += 1;
                }
                ...
                if (!grupo.Key.Equals(s.Last().Key))
                {
                    ...
                    if (!grupo.LastOrDefault().Ruta.Equals(s.ElementAt(index).Key.Ruta))
                    {
                        ...
                        formatlist.Last().vtaspresenciales = vtaspresenciales;
                        formatlist.Last().vtasnopresenciales = vtasnopresenciales;

                        formatlist.Last().TiempoRecorridoS = string.Format("{0:D2}:{1:D2}:{2:D2}", (long)formatlist.Last().TiempoRecorrido.TotalHours, formatlist.Last().TiempoRecorrido.Minutes, formatlist.Last().TiempoRecorrido.Seconds);
                        formatlist.Last().TiempoVisitaS = string.Format("{0:D2}:{1:D2}:{2:D2}", (long)formatlist.Last().TiempoVisita.TotalHours, formatlist.Last().TiempoVisita.Minutes, formatlist.Last().TiempoVisita.Seconds);
                        formatlist.Last().TiempoTransitoTotalS = string.Format("{0:D2}:{1:D2}:{2:D2}", (long)formatlist.Last().TiempoTransitoTotal.TotalHours, formatlist.Last().TiempoTransitoTotal.Minutes, formatlist.Last().TiempoTransitoTotal.Seconds);
                        formatlist.Last().TiempoPromedioS = string.Format("{0:D2}:{1:D2}:{2:D2}", (long)formatlist.Last().TiempoPromedio.TotalHours, formatlist.Last().TiempoPromedio.Minutes, formatlist.Last().TiempoPromedio.Seconds);

                        //seccion clientes fuera de frecuencia
                        foreach (var clientesp in cliffP)
                        {
                            if (grupo.LastOrDefault().Ruta.Equals(clientesp.LastOrDefault().Ruta) && grupo.LastOrDefault().Vendedor.Equals(clientesp.LastOrDefault().Vendedor))
                            {
                                formatlist.Last().VISITADOSFP = clientesp.Sum(c => c.VISITADOS);
                                formatlist.Last().SinVentaFP = clientesp.Sum(c => c.SinVenta);
                                formatlist.LastOrDefault().EfectividadCompraFP = clientesp.Sum(c => c.EfectividadCompra) / clientesp.Count();
                            }
                        }

                        foreach (var clientesnp in cliffNP)
                        {
                            if (grupo.LastOrDefault().Ruta.Equals(clientesnp.LastOrDefault().Ruta) && grupo.LastOrDefault().Vendedor.Equals(clientesnp.LastOrDefault().Vendedor))
                            {
                                formatlist.Last().VISITADOSFNP = clientesnp.Sum(c => c.VISITADOS);
                                formatlist.Last().SinVentaFNP = clientesnp.Sum(c => c.SinVenta);
                                formatlist.LastOrDefault().EfectividadCompraFNP = clientesnp.Sum(c => c.EfectividadCompra) / clientesnp.Count();
                            }
                        }
                        tiempoRecorrido = 0;
                        tiempoVisita = 0;
                ...
                else
                {
                    ...
                    foreach (var clientesp in cliffP)
                    {
                        if (grupo.LastOrDefault().Ruta.Equals(clientesp.LastOrDefault().Ruta) && grupo.LastOrDefault().Vendedor.Equals(clientesp.LastOrDefault().Vendedor))
                        {
                            ...
                            vtaspresenciales += (clientesp.Sum(c => c.VISITADOS) - clientesp.Sum(c => c.SinVenta));
                        }
                    }
                    ...
                    foreach (var clientesnp in cliffNP)
                    {
                        if (grupo.LastOrDefault().Ruta.Equals(clientesnp.LastOrDefault().Ruta) && grupo.LastOrDefault().Vendedor.Equals(clientesnp.LastOrDefault().Vendedor))
                        {
                            ...
                            vtasnopresenciales += (clientesnp.Sum(c => c.VISITADOS) - clientesnp.Sum(c => c.SinVenta));
                        }
                    }
                }
            }
            ...
            string dateInicial = fInicio.Date.ToShortDateString();
            string dateFinal = fFin.Date.ToShortDateString();
            report.labelfechaheader.Text = dateInicial + (dateFinal == dateInicial ? "" : " - " + dateFinal);

            Connection.Open();
            byte[] byteArrayIn = Connection.Query<byte[]>("SELECT Logotipo FROM Configuracion (NOLOCK)", null, null, true, 9000).FirstOrDefault();
            ...
            ////////ClientesFueradeFrecuencia Presencial
            report.VisitadosFP.DataBindings.Add("Text", report.DataSource, "VISITADOSFP");
            report.VisitadosSVFP.DataBindings.Add("Text", report.DataSource, "SinVentaFP");
            report.EfectividadCompraFP.DataBindings.Add("Text", report.DataSource, "EfectividadCompraFP", "{0:0.00}");
            ////////

            ////////ClientesFueradeFrecuencia No Presencial
            report.VisitadosFNP.DataBindings.Add("Text", report.DataSource, "VISITADOSFNP");
            report.VisitadosSVFNP.DataBindings.Add("Text", report.DataSource, "SinVentaFNP");
            report.EfectividadCompraFNP.DataBindings.Add("Text", report.DataSource, "EfectividadCompraFNP", "{0:0.00}");
            ////////

            ////////Ventas Transfer
            report.PresencialesT.DataBindings.Add("Text", report.DataSource, "vtaspresenciales");
            report.NoPresencialesT.DataBindings.Add("Text", report.DataSource, "vtasnopresenciales");
            ////////
            ...
            KilometrajeString.AppendLine("INNER JOIN VenRut VR (NOLOCK) ON RUT.RUTClave = VR.RUTClave AND VEN.VendedorID = VR.VendedorID AND VR.TipoEstado = 1 ");
            KilometrajeString.AppendLine(string.Format("where RUT.RUTClave <>'RUT001' and ALN.AlmacenID = {0} ", CEDIS));
            KilometrajeString.AppendLine(string.Format("AND CONVERT(date, CAV.FechaHoraInicial, 112) BETWEEN '{0}' AND '{1}' ", FechaInicial, FechaFinal));
            KilometrajeString.AppendLine(string.Format("AND ((RUT.RUTClave IN (SELECT Datos FROM FNSplit({0}, ','))) OR {0} IS NULL) ", Rutas));
            ...
            Connection.Open();
            List<KilometrajeTiemposEnRutaMEDModel> Km = Connection.Query<KilometrajeTiemposEnRutaMEDModel>(QueryStringKm, null, null, true, 9000).ToList();
            Connection.Close();
            ...
            var kilolista = lista2.GroupBy(l => new { l.CEDI, l.Vendedor, l.Ruta, l.Fecha })
                    .SelectMany(cl => cl.Select(cs => new KilometrajeTiemposEnRutaMEDModel
                    {
                        CEDI = cs.CEDI,
    ...
    class TiemposEnRutaMEDModel
    {
        ...
        //Clientes fuera de la frecuencia Presencial
        public long SinVentaFP { get; set; }
        public long TotalClientesFP { get; set; }
        public long NOVISITADOSFP { get; set; }
        public long VISITADOSFP { get; set; }
        public Decimal VISITAEfectivaFP { get; set; }
        public Decimal EfectividadCompraFP { get; set; }
        public long vtaspresenciales { get; set; }
        //Clientes fuera de la frecuencia No Presencial
        public long SinVentaFNP { get; set; }
        public long TotalClientesFNP { get; set; }
        public long NOVISITADOSFNP { get; set; }
        public long VISITADOSFNP { get; set; }
        public Decimal VISITAEfectivaFNP { get; set; }
        public Decimal EfectividadCompraFNP { get; set; }
        public long vtasnopresenciales { get; set; }
    }

    class ClientesFueraFrecuenciaPresencialMED
    {
        public string Vendedor { get; set; }
    ...
    class ClientesFueraFrecuenciaNoPresencialMED
    {
        public string Vendedor { get; set; }
        public long SinVenta { get; set; }
        public long TotalClientes { get; set; }
    ...
    class KilometrajeTiemposEnRutaMEDModel
    {
        public string CEDI { get; set; }
        public string Vendedor { get; set; }
        public string Ruta { get; set; }
