...
namespace eRoute.Models.ReportesModels
{
    public class Pedidos
    {
        ...
        public XtraReport GetReport(string ReportName, string CompanyName, MemoryStream CompanyLogo, string CedisName, string CEDIS, string InitialDate, string FinalDate, string Routes, string Sellers, string ObjectName, string Clients, bool ReportFilter, bool ReportType)
        {
            ...
            if (ReportType)
            {
                ...
                sConsulta.AppendLine("WHERE TRP.Tipo = 1 AND TRP.TipoFase IN (1,7) ");
                sConsulta.AppendLine(string.Format("AND Dia.FechaCaptura BETWEEN '{0}' AND '{1}' ", InitialDate, FinalDate));
                sConsulta.AppendLine(string.Format("AND (VEN.VendedorID = {0} OR {0} IS NULL) ", string.IsNullOrEmpty(Sellers) ? "NULL" : string.Format("'{0}'", Sellers)));
                sConsulta.AppendLine(string.Format("AND (VIS.RUTClave IN (SELECT Datos FROM FNSplit({0}, ',')) OR {0} IS NULL) ", string.IsNullOrEmpty(Routes) ? "NULL" : string.Format("'{0}'", Routes)));
                sConsulta.AppendLine(string.Format("AND (CLI.ClienteClave IN (SELECT Datos FROM FNSplit({0}, ',')) OR {0} IS NULL) ", string.IsNullOrEmpty(Clients) ? "NULL" : string.Format("'{0}'", Clients)));
                ...
                sConsulta.AppendLine(string.Format("WHERE ALN.AlmacenID = '{0}') TMP ", CEDIS));
                ...
                ReportePedidos report = new ReportePedidos() { DataSource = formatlist };

                List<ProductosPedidosModel> listSub = Connection.Query<ProductosPedidosModel>(GetProducts(CEDIS, InitialDate, FinalDate, Routes, Sellers, Clients), null, null, true, 600).ToList();

                DateTime fInicio = DateTime.Parse(InitialDate);
                DateTime fFin = DateTime.Now;
                if (String.IsNullOrEmpty(FinalDate) || FinalDate == "null")
                {
                    FinalDate = "";
                }
                else
                {
                    fFin = DateTime.Parse(FinalDate);
                    FinalDate = " - " + fFin.Date.ToShortDateString();
                ...
                report.empresa.Text = CompanyName;
                report.reporte.Text = ReportName + " Detallado";



                //ReportHeader
                report.headerlabelcedis.Text = CedisName;
                report.labelfechaheader.Text = fInicio.Date.ToShortDateString() + FinalDate;
                if (ReportFilter)
                {
                    report.labelFilter.Text = "Vendedor: ";
                    report.filter.Text = ObjectName;
                }
                else
                {
                    report.labelFilter.Text = "Ruta(s): ";
                    report.filter.Text = (string.IsNullOrEmpty(Routes) ? "Todas las Rutas" : Routes);
                }
            ...
            else
            {
                ...
                ReportePedidoGeneral reportG = new ReportePedidoGeneral() { DataSource = sd };
                ...
                reportG.logo.Image = Image.FromStream(CompanyLogo);
                reportG.logo.Sizing = DevExpress.XtraPrinting.ImageSizeMode.StretchImage;

                reportG.empresa.Text = CompanyName;
                reportG.reporte.Text = ReportName + " Detallado";



                //ReportHeader
                reportG.headerlabelcedis.Text = CedisName;
                reportG.labelfechaheader.Text = fInicio.Date.ToShortDateString() + FinalDate;
                if (ReportFilter)
                {
                    reportG.labelFilter.Text = "Vendedor: ";
                    reportG.filter.Text = ObjectName;
                }
                else
                {
                    reportG.labelFilter.Text = "Ruta(s): ";
                    reportG.filter.Text = (string.IsNullOrEmpty(Routes) ? "Todas las Rutas" : Routes);
                }
        ...
        public string GetProducts(string CEDIS, string InitialDate, string FinalDate, string Routes, string Sellers, string Clients)
        {
            StringBuilder sConsulta = new StringBuilder();
