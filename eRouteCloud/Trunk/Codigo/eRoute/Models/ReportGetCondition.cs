...
namespace eRoute.Models
{
    ...
    {
        public ConditionModel Get(List<ReportFilterModel> Lotes, List<GetRoutesModel> Routes, List<Clients> Clientes, List<GetSellerModel> Seller, List<GetProductsModel> Products, List<GetProductsSchemeModel> ProductsSchema, string nombreReport = "", string Cedis = "", string nombreCedis = "", string dateStatus = "", string init = "", string end = "", int VAVClave = 0, bool reportType = false, string unidadMedida = "", string CENClave = "", string promocion = "", string Presupuesto = "", string NombreProductos = "")
        {
        ...
        public string Get(List<ReportFilterModel> Objeto, bool change = false)
        {
            if (Objeto.Count() != 0)
            {
        ...
        public void GetExportToExcel(XtraReport report, string name)
        {
            string path = string.Format(@"{0}\Downloads\{1}{2:yyyyMMddHHmmss}.xls", Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), name, DateTime.Now);
            string xlsExportFile = path;
