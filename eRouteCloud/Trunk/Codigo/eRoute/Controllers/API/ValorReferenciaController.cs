...
namespace eRoute.Controllers.API
{
    public class ValorReferenciaController : ApiController
    {
        ...
        [HttpGet]
        [ActionName("ObtenerFiltros")]
        public IHttpActionResult ObtenerFiltros()
        {
            IDbConnection Connection = new SqlConnection(ConfigurationManager.ConnectionStrings["eRouteConnection"].ConnectionString);
            List<object> List = Connection.Query<object>("SELECT CAST([VAVClave] AS [int]) AS [VAVClave] FROM [dbo].[VAVDescripcion] (NOLOCK) WHERE [VARCodigo] = 'TIPFIR'  ORDER BY [VAVClave]").ToList();
            return Json(List);
        }

        [HttpGet]
        [ActionName("ObtenerFiltrosDinamicos")]
        public IHttpActionResult ObtenerFiltrosDinamicos(string RepDinID)
        {
            IDbConnection Connection = new SqlConnection(ConfigurationManager.ConnectionStrings["eRouteConnection"].ConnectionString);
            List<object> List = Connection.Query<object>(
                string.Format(
                    "SELECT CAST([FD].[TipoFiltro] AS [int]) AS [TipoFiltro], [FD].[Opciones], [VD].[Descripcion]"
                    + " FROM [dbo].[FiltrosDinamicos] AS [FD] (NOLOCK)"
                    + " INNER JOIN [dbo].[VAVDescripcion] AS [VD] (NOLOCK) ON [FD].[TipoFiltro] = [VD].[VAVClave] AND [VD].[VARCodigo] = 'TIPFIR' AND [VD].[VADTipoLenguaje] = 'EM'"
                    + " WHERE [FD].[RepDinID] = {0} ORDER BY [TipoFiltro]"
                    , RepDinID)).ToList();
            return Json(List);
        }
    }
