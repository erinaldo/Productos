...
namespace eRoute.Controllers.API
{
    public class SegmentoController : ApiController
    {
        ...
        {
            int nivelAux = (nivel == null ? 0 : Convert.ToInt32((nivel)));
        ...
        [HttpGet]
        [ActionName("GetActiveSchemas")]
        public IHttpActionResult GetActiveSchemas(int Type, string SchemeId = null, int Level = 0)
        {
            List<cEsquema> squemas = (
                from schema in db.Esquema
                where schema.Nivel == Level && schema.Tipo == Type && schema.TipoEstado == 1 && schema.EsquemaIDPadre == SchemeId
                select new cEsquema
                {
                    Clave = schema.Clave,
                    Nombre = schema.Nombre,
                    Abreviatura = schema.Abreviatura,
                    EsquemaID = schema.EsquemaID,
                    EsquemaIDPadre = schema.EsquemaIDPadre,
                    Tipo = schema.Tipo,
                    Orden = schema.Orden,
                    Clasificacion = schema.Clasificacion,
                    NivelHijo = schema.Nivel.ToString()
                }).ToList();
            return Json(squemas);
        }
