...
namespace eRoute.Controllers.API
{
    public class ProductoController : ApiController
    {
        ...
        //  BUSCA PRODUCTOS QUE PUEDEN ESTAR COMO CONTENEDOR
        [HttpGet]
        [ActionName("BuscarProductoContenedor")]
        public IHttpActionResult BuscarProductoContenedor(string ProductoClave)
        {
            string lenguaje = ObtenerLenguaje();

            var Productos = (from P in db.Producto
                             join VAV1 in db.VAVDescripcion on "PROTIPO" equals VAV1.VARCodigo
                             where P.Tipo.ToString() == VAV1.VAVClave && VAV1.VADTipoLenguaje == lenguaje
                             join VAV2 in db.VAVDescripcion on "PROFASE" equals VAV2.VARCodigo
                             where P.TipoFase.ToString() == VAV2.VAVClave && VAV2.VADTipoLenguaje == lenguaje
                             where P.TipoFase == 1
                             // IN ()
                             && (from D1 in db.ProductoDetalle
                                     // NOT IN ()
                                 where !(from D2 in db.ProductoDetalle
                                         where D2.ProductoClave != D2.ProductoDetClave
                                         select D2.ProductoClave).Contains(D1.ProductoClave)
                                         && D1.ProductoClave != ProductoClave
                                 select D1.ProductoClave).Contains(P.ProductoClave)
                             select new
                             {
                                 P.ProductoClave,
                                 P.Id,
            ...
            //var Pro = (from D in db.ProductoDetalle
            //                 where !(from DE in db.ProductoDetalle
            //                        where DE.ProductoClave != DE.ProductoDetClave
            //                        select DE.ProductoClave).Contains(D.ProductoClave)
            //                 select D.ProductoClave
            //    ).ToList();

            //var NoProductos = (from D in db.ProductoDetalle
            //                   where D.ProductoClave != D.ProductoDetClave
            //                   select D.ProductoClave
            //    ).ToList();
        ...
        public IHttpActionResult obtenerProductosRelacionados()
        {
            ...
            var producto = (from pro in db.Producto
                            join V1 in db.VAVDescripcion on "PROTIPO" equals V1.VARCodigo
                            where pro.Tipo.ToString() == V1.VAVClave && V1.VADTipoLenguaje == lenguaje
                            join V2 in db.VAVDescripcion on "PROFASE" equals V2.VARCodigo
                            where pro.TipoFase.ToString() == V2.VAVClave && V2.VADTipoLenguaje == lenguaje
                            where pro.Contenido == true && pro.TipoFase == 1
                            select new { pro.ProductoClave, pro.Id, pro.Tipo, V1.Descripcion, pro.Nombre, pro.NombreLargo, pro.TipoFase, Descripcion2 = V2.Descripcion }).ToList();
