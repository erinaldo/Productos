app.controller('r087VentaPorClienteALTController', ['valorReferencia', '$http', 'cfpLoadingBar', function (valorReferencia, $http, cfpLoadingBar) {
    ...
    self.Submit = function () {
        ...
        var clients = [];
        if (!self.selectAllClients) {
            clients = GetArrayValues(self.clients);
        }
        if (self.selectedClientScheme != undefined) {
            clientScheme = GetArrayValues(self.schemes, self.selectedClientScheme);
        }
        if (routes.length <= 0 && !self.selectAllRoutes && self.selectedSeller === undefined) {
            ...
            clients = (self.selectAllClients ? null : clients);
            var data = {
