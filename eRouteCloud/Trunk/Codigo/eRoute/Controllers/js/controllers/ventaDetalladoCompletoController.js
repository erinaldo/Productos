app.controller('ventaDetalladoCompletoController', ['$scope', '$mdToast', '$q', '$timeout', 'valorReferencia', '$http', 'cfpLoadingBar', function ($scope, $mdToast, $q, $timeout, valorReferencia, $http, cfpLoadingBar) {
    ...
    function GetClients(Routes) {
        $http.post(url + '/Reports/GetRoutesStiring', {
            ...
            $http({
                url: url + '/api/GetClients?Id=' + $scope.abc.currentNode.id + "&Routes=" + response.data + '&Cedis=' + "",
                method: 'GET',
                headers: {
                    Authorization: window.sessionStorage.getItem('Token'),
                    'Content-Type': 'application/json'
