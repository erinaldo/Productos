...
app.controller('clienteCtrl', ["$scope", "$rootScope", "$http", 'valorReferencia', "translationService", "defaultBox", "seleccionEsquema", "messageBox", function ($scope, $rootScope, $http, valorReferencia, translationService, defaultBox, seleccionEsquema, messageBox) {
    ...
    $scope.AddClientSchema = function (branch) {
        const expanded = branch && branch['expanded'];
        const SchemeId = branch && branch['EsquemaID']
        const level = branch && branch['level'];
        branch && Object.assign(branch, { icons: { iconLeaf: "fas fa-spinner fa-spin" } });
        if (!branch || (expanded && branch['children'].length === 0)) {
            seleccionEsquema.Get(1, SchemeId, level).then(function (data) {
                if (!branch) {
                    $scope.tree_data = data;
                } else if (expanded && data.length > 0) {
                    Object.assign(branch, { children: data });
                } else if (data.length === 0) {
                    Object.assign(branch, { icons: { iconLeaf: "fa fa-file" } });
                }
