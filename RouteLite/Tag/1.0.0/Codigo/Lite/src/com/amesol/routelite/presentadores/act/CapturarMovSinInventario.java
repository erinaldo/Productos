...
public class CapturarMovSinInventario extends Presentador
{
	...
	private void generarDetalle(String productoClave, int TipoUnidad, Float Cantidad, String Motivo)
	{
		try
		{
			...
			StringBuilder byRefMensaje = new StringBuilder();
			transProdGeneral = Transacciones.Pedidos.ActualizarGenerarPedido(getTransaccionesIds(), moduloMovDetalle, byRefMensaje, Integer.parseInt(Motivo));
			if (byRefMensaje.length() >0){
				mVista.mostrarAdvertencia(byRefMensaje.toString());
			}
			byRefMensaje= null;
	...
	private void generarDetalle(TransProdDetalle transProdDetalle, boolean refrescarListado)
	{
		try
		{
			...
			transProdGeneral = Transacciones.Pedidos.ActualizarGenerarPedido(getTransaccionesIds(), moduloMovDetalle, byRefMensaje, 13);
			if (byRefMensaje.length() >0){
				mVista.mostrarAdvertencia(byRefMensaje.toString());
			}
			byRefMensaje = null;
	...
	{
		String lstTrpTipo = ValoresReferencia.getStringVAVClave("TRPTIPO", "23");
