...
public class AtenderClientes extends Presentador
{
	...
	public void ValidarDatosGPS(DatosGPS datosGPS)
	{
		...
		if ((Boolean) oParametros.get("TieneCoordenadas"))
		{
			AseguramientoVisita aseguramiento = obtenerAseguramiento();
			if (aseguramiento.TipoAseguramiento == 2 || aseguramiento.TipoAseguramiento == 3)
			{
		...
		else
		{
			...
			else
			{
				...
				try
				{
					...
					clidom.CoordenadaX = (float) datosGPS.getLongitud();
					clidom.CoordenadaY = (float) datosGPS.getLatitud();
					clidom.ReferenciaDom = "AtenderClientes_ValidarDatosGPS";
					clidom.Enviado = false;
					BDVend.guardarInsertar(clidom);
					BDVend.commit();
				}
	...
	public void validarTextCodigoBarra()
	{
		AseguramientoVisita aseguramiento = obtenerAseguramiento();
		if (aseguramiento.TipoContrasena != 0)
		{
			// si el tipo de contraseña es diferente de Ninguna deshabilitar
			mVista.habilitaDeshabilitaCodigoBarras(false);
		}
	}
	...
	public String validarClienteAgendado(Cliente cliente)
	{
		try
		{
			...
			if (cliente.IdElectronico.equals(""))
				return "";
			oParametros.put("FueraFrecuencia", true);
			ISetDatos cli = Consultas.ConsultasCliente.obtenerAgendado(cliente, (Ruta) Sesion.get(Campo.RutaActual));
			if (cli.getCount() > 0)
			{
	...
	public boolean validarMostrarAutorizacion(boolean[] array)
	{
		...
		{
			if (((aseguramiento.TipoAseguramiento == 1 || aseguramiento.TipoAseguramiento == 3)) && Integer.parseInt(oParametros.get("CodigoLeido").toString()) == 0)
			{// !(Boolean)oParametros.get("CodigoLeido")){

				if (aseguramiento.TipoAseguramiento == 3)
					array[1] = true;
			...
			{
				if (aseguramiento.TipoAseguramiento != 2 && aseguramiento.TipoAseguramiento != 3 && aseguramiento.TipoContrasena == 2 && Integer.parseInt(oParametros.get("CodigoLeido").toString()) == 0)
					return true;
				else
					array[1] = true;
				return false;
