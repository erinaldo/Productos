...
public class BuscaProducto extends Vista implements IBuscaProducto, CalcularPorKiloListener
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		try
		{
			...
			listProductos.setChoiceMode(ListView.CHOICE_MODE_SINGLE);
			...
			if (event.getAction() == KeyEvent.ACTION_UP)
			{
				if (keyCode == KeyEvent.KEYCODE_ENTER)
				{
					try
					{
						...
						{
							if (mAccion.equals(Enumeradores.Acciones.ACCION_OBTENER_PRODUCTOS_SELECCIONADOS))
								mostrarProductos(Consultas.ConsultasProducto.obtenerProductosExistencia(filtroEsquema + filtroProducto, precioClave, false, transProdIds, tipoTransProd, (tipoValidarExistencia == TiposValidacionInventario.NoValidar ? false : true), tipoMovimiento, esquemasModulo, ubicacionExistencia));
							else
								mostrarProductos(Consultas.ConsultasProducto.obtenerProductosExistencia(filtroEsquema + filtroProducto, precioClave, filtrar, transProdIds, tipoTransProd, (tipoValidarExistencia == TiposValidacionInventario.NoValidar ? false : true), tipoMovimiento, esquemasModulo, ubicacionExistencia));
						}
						return true;
					}
					catch (Exception e)
	...
	public boolean onKeyDown(int keyCode, KeyEvent event)
	{
		switch (keyCode)
		{
			case KeyEvent.KEYCODE_BACK:
				...
				if((mFragment = getFragmentManager().findFragmentByTag(TAG_CALC)) != null){
					closeFragment(mFragment);
					return true;
				}
				finalizar();
				return true;
