	...
	public String recuperarTransProdIdSalida()
	{
		try
		{
			...
			if (transProdSalida == null)
			{
				...
				transProdSalida.ListaPrecios = oPrecio;
				transProdSalida.PCEPrecioClave = oPrecio.PrecioClave;
...
/*	private void apartarInventarioEntrada(String productoClave, float cantidad)
	...
	private TransProdDetalle generarDetalle(Producto producto, int TipoUnidad, Float Cantidad, short TipoMotivo)
	{
		try
		{
			TransProdDetalle transProdDetalle = TransaccionesDetalle.Cambios.GenerarDetalleCambio(transProdGeneral.TransProdID, producto.ProductoClave, TipoUnidad, Cantidad, 0, transProdGeneral.ListaPrecios.PrecioClave);
			if (transProdDetalle == null)
	...
	private TransProdDetalle generarDetalleSalida(Producto producto, int TipoUnidad, Float Cantidad)
	{
		try
		{
			...
			TransProdDetalle transProdDetalle = TransaccionesDetalle.Cambios.GenerarDetalleCambio(transProdSalida.TransProdID, producto.ProductoClave, TipoUnidad, Cantidad, 0, transProdSalida.ListaPrecios.PrecioClave);
			if (transProdDetalle == null)
				throw new ControlError("MDB042302");
	...
	public boolean eliminarMovimiento()
	{
		...
		{	
			...
			mVista.getSaldoCambiosVendedor().SaldoUsado -=saldoReversa;
			BDVend.guardarInsertar(mVista.getSaldoCambiosVendedor());
