...
import com.amesol.routelite.actividades.BluetoothChatService;
...
public abstract class Vista extends FragmentActivity
{
	...
	public void mostrarPreguntaReintentarImpresion(String mensaje, final Hashtable<String, ContentValues> archivosAImprimir, final File impresion, final int numeroCopias){
		...
		dialogo.setButton(msgSi, new DialogInterface.OnClickListener()
		{
			...
			{
				dialog.dismiss();
				reintentaImpresion(archivosAImprimir, impresion, (short)numeroCopias);
			}
		});
		dialogo.setButton2(msgNo, new DialogInterface.OnClickListener()
		{
	...
	protected void reintentaImpresion(Hashtable<String, ContentValues> archivosAImprimir, File impresion, short numeroCopias){
		try{
			new Recibos().imprimirArchivos(archivosAImprimir, impresion, (IVista) this, numeroCopias);
		}catch(Exception ex){
			mostrarError(ex.getMessage());
			impresionFinalizada(false,"");
		}
