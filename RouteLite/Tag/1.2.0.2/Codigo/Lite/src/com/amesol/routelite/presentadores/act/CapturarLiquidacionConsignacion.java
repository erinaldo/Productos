...
//				consigna.Saldo+= trDetalle.Total;
		...
		TransProd nuevaDevolucion = transProdDev == null ? 
		...
		consigna.MUsuarioID = nuevaDevolucion.MUsuarioID;
	...
	private void liquidacionCancelacion(){
		try{
			...
			for(VistaConsignacionLiquidacion producto: detalleProductos){
				...
				{
					partidaVta++;
					impuestoDetalle = Impuestos.Calcular(impuestos, producto.Precio * producto.Liquidar, producto.Precio);
				...
				{
					partidaDev++;
					impuestoDetalle = Impuestos.Calcular(impuestos, producto.Precio * producto.Devoluciones, producto.Precio);
					...
					if(!generaConsignacion)
					{
						/* Actualizar inventario */
						if(!Inventario.ActualizarInventario(producto.ProductoClave, producto.TipoUnidad, producto.Devoluciones, 
								transDev.Tipo, transDev.TipoMovimiento, ValoresReferencia.getValor("TRPMOT", String.valueOf(transDev.TipoMotivo)).getGrupo(), vendedor.AlmacenID, false)){
							throw new Exception("No se pudo actualizar inventario");
						}
					}else
					{
...
//		transprod.add(consigna.TransProdID);
	...
	public void imprimirDocumentos() throws ControlError, Exception{
		...
		recibos.imprimirRecibos(generarDocsAImprimir(), true, mVista);
	}
