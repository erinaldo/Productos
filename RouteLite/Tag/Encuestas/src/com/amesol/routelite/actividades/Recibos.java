...
public class Recibos
{
	...
	public static void imprimirReporte(int reporte, Cliente cliente, Boolean logoSoloPrimerRecibo, IVista vista) throws ControlError, Exception
	{
		String nombreArchivo = "";
		Hashtable<String, ContentValues> archivosGenerados = new Hashtable<String, ContentValues>();
		...
		nombreArchivo = generarArchivoReporte(reporte, impresion.getAbsolutePath(), reciboAct, imprimeLogo, byRefMsgError, cliente);
	...
	private static String generarArchivoReporte(int reporte, String directorioArchivo, Recibo recibo, boolean imprimeLogo, String[] byRefMsgError, Cliente cliente) throws Exception
	{
		String nombreArchivo = "";
		try
		{
...
//		vt.setVerticalLayout(document.getPageSize().getWidth() - 40, document.getPageSize().getHeight() - 200, document.getPageSize().getHeight()-220, 40, 16);
		...
		cadena+= completaHasta(texto, 10, TipoAlineacion.IZQUIERDA, true);
...
//		cadena = "";
		...
		while(pedidos.moveToNext())
		{
			...
			cadena+= completaHasta(pedidos.getString("Cliente"), 20, TipoAlineacion.IZQUIERDA, false);
			...
			cadena+= completaHasta(pedidos.getString("Ruta"), 18, TipoAlineacion.IZQUIERDA, false);
			volumen+= pedidos.getFloat("Volumen");
			total+= pedidos.getFloat("Monto");
			cadena+= completaHasta(Generales.getFormatoDecimal(pedidos.getFloat("Volumen"), 2), 8, TipoAlineacion.IZQUIERDA, false);
			cadena+= completaHasta("$ ".concat(Generales.getFormatoDecimal(pedidos.getFloat("Monto"), "#,###,##0.00")), 10, TipoAlineacion.IZQUIERDA, true);
//			cadena+= "\r\n";
...
//			vt.go();
		...
		cadena = completaHasta(Generales.getFormatoDecimal(volumen, 2).concat("  "), 122, TipoAlineacion.DERECHA, false);
		cadena+= completaHasta("$ ".concat(Generales.getFormatoDecimal(total, "#,###,##0.00")), 10, TipoAlineacion.IZQUIERDA, true);
