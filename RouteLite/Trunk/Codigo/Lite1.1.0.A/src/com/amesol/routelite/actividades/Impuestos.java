...
public class Impuestos
{
	...
	public static float Calcular(Impuesto[] Impuestos, float PrecioBase, float PrecioUnitario) throws ControlError
	{
		...
		for (int i = 0; i < Impuestos.length; i++)
		{
			...
			{
				Impuestos[i].Importe = Impuestos[i].Valor;
		...
		for (int i = 0; i < oTransProd.TransProdDetalle.size(); i++)
		{
			...
			if (oTransProdDetalle.Promocion == 1)
			{
				...
				if (arrayImpuestos != null && arrayImpuestos.length > 0)
				{
					...
					float ImpuestoImporte = Impuestos.Calcular(arrayImpuestos, subtotal, oTransProdDetalle.Precio);
