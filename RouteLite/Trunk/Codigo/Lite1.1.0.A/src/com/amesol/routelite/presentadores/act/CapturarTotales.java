		...
		try
		{
			...
			{ // recorrer todas las transacciones generadas para calcular
				...
				{ // si es el primero asignar todos los valores a los campos
					...
					if(oTrp.Tipo == TiposTransProd.PEDIDO){ //no habilitar el tipo pedido para los pedidos
				...
				if (oTrp.TipoFase != 0 && oTrp.TipoFase != 3 &&  !(Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.REPARTO && oTrp.TipoFase == 2) )
			...
			if(Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO){
				...
				for(TransProdDetalle oTpd : oTrp.TransProdDetalle){
					if(((Cliente) Sesion.get(Campo.ClienteActual)).Prestamo){
						Inventario.ActualizarInventario(oTpd.ProductoClave, oTpd.TipoUnidad, oTpd.Cantidad, oTrp.Tipo, TiposMovimientos.SALIDA, ((Vendedor) Sesion.get(Campo.VendedorActual)).AlmacenID,"",false,((Cliente) Sesion.get(Campo.ClienteActual)).ClienteClave,oTpd.PrestamoVendido,mVista.getSurtir());
					}else{
						Inventario.ActualizarInventario(oTpd.ProductoClave, oTpd.TipoUnidad, oTpd.Cantidad, oTrp.Tipo, TiposMovimientos.SALIDA, ((Vendedor) Sesion.get(Campo.VendedorActual)).AlmacenID,"",false,"",0,mVista.getSurtir());
					}
				}
