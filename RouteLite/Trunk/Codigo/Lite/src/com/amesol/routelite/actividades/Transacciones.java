...
public class Transacciones
{
	...
	public static void recolectarEnvasesAutomaticamente(TransProd oTransProd) throws Exception{
		String folio = null;
		if (oTransProd.DevolucionID != null && oTransProd.DevolucionID != ""){
			...
			if(Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.REPARTO && 
					(oTransProd.VisitaClave1 != null || oTransProd.VisitaClave1 != "") && 
					(oTransProd.TipoFase == TiposFasesDocto.CAPTURA || oTransProd.TipoFase == TiposFasesDocto.CAPTURA_ESCRITORIO)){
				transProd.VisitaClave = oTransProd.VisitaClave1;
			}else{
				transProd.VisitaClave = oTransProd.VisitaClave;
			}
			transProd.DiaClave = oDia.DiaClave;
			transProd.PCEModuloMovDetClave = oTransProd.PCEModuloMovDetClave;
			transProd.Folio = folio;
			transProd.Tipo = (short) oMMD.getTipoTransprod();
			transProd.TipoFase = TiposFasesDocto.CAPTURA;
			transProd.TipoMovimiento = (short) oMMD.getTipoMovimiento();
			transProd.FechaHoraAlta = Generales.getFechaHoraActual();
			transProd.FechaCaptura = oDia.FechaCaptura;
			...
			if (vrRecoleccion.getCount() > 0 || vrRecoleccionNV.getCount() > 0){
				if(vrRecoleccion.moveToNext()){
					transProd.TipoMotivo = vrRecoleccion.getShort(SearchManager.SUGGEST_COLUMN_INTENT_DATA);
				}else if(vrRecoleccionNV.moveToNext()){
					transProd.TipoMotivo = vrRecoleccionNV.getShort(SearchManager.SUGGEST_COLUMN_INTENT_DATA);
				}
			}else{
				transProd.TipoMotivo = 0;
			}
			transProd.Total = 0;
			transProd.Enviado = false;
			transProd.MFechaHora = Generales.getFechaHoraActual();
			transProd.MUsuarioID = oUsuario.USUId;
			
			//actualizar venta
			String grupoMotivo = null;
			oTransProd.DevolucionID = transProd.TransProdID;
			if(vrRecoleccion.getCount() == 1 || (vrRecoleccion.getCount() > 0 || vrRecoleccionNV.getCount() > 0)){
				grupoMotivo = "Venta";
			}else if(vrRecoleccionNV.getCount() > 0 && vrRecoleccion.getCount() <= 0){
				grupoMotivo = "No Venta";
			}
			
			//TODO sperez CUROLGEN05 punto 2: loop de productos
	...
	public static class Pedidos
	{
		...
		public static boolean GenerarDetalle(ModuloMovDetalle moduloMovDetalle, TransProd transProdGeneral, HashMap<String, TransProd> transacciones, String ProductoClave, int TipoUnidad, float Cantidad, float CantidadOriginal,float PrecioEspecial) throws Exception
		{
			...
			TransProdDetalle transProdDetalle = TransaccionesDetalle.Pedidos.GenerarDetallePedido(transProdGeneral.TransProdID, ProductoClave, TipoUnidad, Cantidad, CantidadOriginal, transProdGeneral.CadenaListasPrecios, PrecioEspecial, null);
