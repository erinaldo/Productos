...
public class Print {
	...
	public Print(Context context) {
		...
		if (!initPrint())
			Toast.makeText(context, "init print failed", 100).show();
		else
			Toast.makeText(context, "init print success", 100).show();
	}
	...
	public void printEAN8Barcode(String data) {
		...
		if (data.length() > 8 || data.length() < 7) {
			Toast.makeText(mContext, "ERROR:The length of EAN8 is error", 100)
					.show();
			return;
		}
		if (data.length() == 8) {
	...
	public void printEAN13Barcode(String data) {
		...
		if (data.length() > 13 || data.length() < 12) {
			Toast.makeText(mContext, "ERROR:The length of EAN13 is error", 100)
					.show();
			return;
		}
		if (data.length() == 13) {
