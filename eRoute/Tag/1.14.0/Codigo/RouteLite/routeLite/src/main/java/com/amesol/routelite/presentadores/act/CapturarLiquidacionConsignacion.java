...
//				consigna.Saldo+= trDetalle.Total;
	...
	private void liquidacionCancelacion(){
		try{
			...
			Cliente cliente = (Cliente)Sesion.get(Campo.ClienteActual);
			String folio = null;
			int decimales = Integer.parseInt(((CONHist)Sesion.get(Sesion.Campo.CONHist)).get("DecimalesImporte").toString());
			
			abonosAnteriores =  consigna.Total - consigna.Saldo;
			abonosAnteriores = Generales.getRound(abonosAnteriores, decimales);
