...
public final class Consultas2
{
    ...
    public static class ConsultasActivos {
        ...
        public static ISetDatos obtenerActivos(String clienteClave)throws Exception
        {
            StringBuilder sConsulta = new StringBuilder();
            sConsulta.append("select Activo.ActivoClave as _id, Activo.ActivoClave || ' - ' || Activo.Nombre as Descripcion, Activo.Tipo, Activo.TipoFase ");
            sConsulta.append("from Activo ");
            sConsulta.append("left join ActivoClienteHist on Activo.ActivoClave=ActivoClienteHist.ActivoClave ");
            sConsulta.append("where Activo.ActivoClave = ActivoClienteHist.ActivoClave ");
            sConsulta.append("and ActivoClienteHist.TipoEstado=1 ");
            sConsulta.append("and ClienteClave='" + clienteClave + "' ");
            sConsulta.append("and ActivoClienteHist.ActivoClienteHistId in ");
            sConsulta.append("(select ActivoClienteHistId from ActivoClienteHist where Fecha in (select max(Fecha) from ActivoClienteHist where ActivoClave=Activo.ActivoClave and ClienteClave='" + clienteClave + "')) ");
