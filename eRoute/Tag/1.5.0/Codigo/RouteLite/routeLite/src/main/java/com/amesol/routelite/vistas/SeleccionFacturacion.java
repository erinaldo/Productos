...
//		mTexto.setText(Mensajes.get("XDatosFacturacion"));
	...
	public String[] getMetodosPagoSeleccionados(){
		...
		while(iterador.hasNext())
		{
			...
			metodosPago.append(pago.Cuenta).append(existeOtro);
			metodosPago.append(ValoresReferencia.getDescripcion("TBANCO", String.valueOf(pago.TipoBanco))).append(existeOtro);
		}
...
//				{
					...
					if("1".equals(motCfg.get("MensajeImpresion")))
					{
						...
						return;
					}else if (motCfg.get("MensajeImpresion").equals("2")){
	...
	private String validaAvance(){
		switch(indiceActivo)
		{
			...
			case METODOS_PAGO_INDEX:
				if(((SEMHist)((Spinner)findViewById(R.id.spnSubEmpresa)).getSelectedItem()).VersionCFD == 4){
