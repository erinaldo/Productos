...
public class CapturaTotales extends Vista implements ICapturaTotales
{
        ...
        public void onDateSet(DatePicker view, int selectedYear, int selectedMonth, int selectedDay) {
            try {
                ...
                if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("DiasMaxDeSurtido")) {
                    ...
                    if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("DiasMaxDeSurtido", moduloMovDetalle.ModuloMovDetalleClave.toString()) == null) {
                            ...
                            if (!edDesc1.getText().toString().equals(".") && !edDesc2.getText().toString().equals(".")) {
                                if (!edDesc1.getText().toString().equals("") && !edDesc2.getText().toString().equals("")) {
                                    ...
                                    if (!(des1 > 100 || des1 < 0) && !(des2 > 100 || des2 < 0)) {
                                        ...
                                        edTotalVentaDes.setText("$ " +  Generales.getFormatoDecimal(((SubTotal - totalesCalculadora.get("SubTotal")) +  subTotalDescuento) + ((Impuestos - totalesCalculadora.get("Impuesto")) +  impuestoDescuento) ,"#,##0.00"));
                                    } else {
                                        AlertDialog.Builder bb = new AlertDialog.Builder(ctx);
    ...
    public void setFechaEntregaDefault() {
        ...
        } else if (getTipoPedido() == TipoPedido.POSFECHADO || ValoresReferencia.getValor("PEDTIPO",String.valueOf(getTipoPedido())).getGrupo().equalsIgnoreCase("PagoAnticipado")|| getTipoPedido() == TipoPedido.CONSIGNACION || (mPresenta.getTipo() == TiposTransProd.MOV_SIN_INV_EV && motConf.get("MSIEVPreventa").toString().equals("1")) || (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.PREVENTA  && mPresenta.getTipo() == TiposTransProd.PEDIDO)) {
