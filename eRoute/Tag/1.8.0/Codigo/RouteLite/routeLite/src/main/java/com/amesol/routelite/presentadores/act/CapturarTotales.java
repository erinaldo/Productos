...
public class CapturarTotales extends Presentador
{
	...
	public void asignarGuardarValores() throws Exception
	{
        ...
        if( (mVista.getEsNuevo() || mVista.getModificandoAutoventa()) && getTipo() == TiposTransProd.PEDIDO){
		...
		while (it.hasNext())
		{
			...
			if ((!mVista.getPedidoAdicional().trim().equals("")||!mVista.getObservaciones().trim().equals("") ||!mVista.getObservaciones2().trim().equals("")) && mVista.getEsNuevo())
			{
				TRPVtaAcreditada pedidoAdicional = new TRPVtaAcreditada();
				pedidoAdicional.TransProdID = oTrp.TransProdID;
