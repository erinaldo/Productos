...
{
	public static class CuadreEnvase{
        //estructura utilizada para el cuadre automatico de envase
		private String productoClave;
		private int tipoUnidad;
		private float cantidad;
		
		public CuadreEnvase(String productoClave, int tipoUnidad, float cantidad){
		...
		//try{
			...
			for(ProductoDetalle prod : Consultas2.ConsultasProductoDetalle.obtenerProductosConPrestamo(producto.ProductoClave, tipoUnidad)){
					...
					if(ppc.Saldo >= CantidadUM){
						...
						float cant = 0;
						if(!producto.Contenido){ //para manejar envase asociado a un producto
							cant = CantidadUM;
