...
public final class Consultas2
{
	...
	public static class ConsultasProductoDetalle{
        ...
        public static ISetDatos obtenerDetallesTicketPedidoPDF (String transprodId) throws Exception {
            String query = "SELECT P.NombreLargo, TPD.ProductoClave, TPD.TipoUnidad, TPD.Cantidad, TPD.Subtotal, " +
                    "TPD.Total FROM TransProdDetalle as TPD inner join Producto as P on P.ProductoClave = TPD.ProductoClave " +
	...
	public static class ConsultasEncuestas
	{
        ...
        public static int obtenerSaltoAnterior(String CENClave, int CEPNumero2) throws Exception
        {
            int CEPN2 = 0;
            ISetDatos config = BDVend.consultar("Select * from CENPregunta where CEPNumero = (Select CEPNumero1 from CEPRespSalto where CENClave = '"+CENClave+"' and CEPNumero2 = "+CEPNumero2+") and CENClave = '"+CENClave+"'");
            if(config.moveToFirst())
            {
                CEPN2 = config.getInt("Orden");
            }
            config.close();
            return CEPN2;
