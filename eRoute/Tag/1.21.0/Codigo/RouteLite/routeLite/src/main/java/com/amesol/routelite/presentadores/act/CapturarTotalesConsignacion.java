...
public class CapturarTotalesConsignacion extends Presentador
{
	...
	public void iniciar()
	{
		try
		{
			...
			{ // recorrer todas las transacciones generadas para calcular
				...
				{ // si es el primero asignar todos los valores a los campos
					...
					{ // si no es nuevo, cargar los datos del transprod
						...
						TransProd Consignacion = new TransProd();
						Consignacion.TransProdID = oTrp.TransProdID;
						Consignacion.TransProdID = SeleccionarConsignacion.TransprodConsig;
						BDVend.recuperar(Consignacion);
						if (Consignacion.isRecuperado()) {
							mVista.setTipoTurno(Consignacion.TipoTurno);
							if(!SeleccionarConsignacion.ConsigFact){
								Cfactura = false;
							}
						}
					}
            ...
            //boolean bExigirPedidoAdicional = false;
            //if (((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).existeParametro("ExigirPedidoAdicionalConsig"))
            //    bExigirPedidoAdicional = ((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).get("ExigirPedidoAdicionalConsig").toString().equals("1");
            mVista.mostrarPedidoAdicional(((Cliente)Sesion.get(Campo.ClienteActual)).ExigirPedidoAdicional); //&& bExigirPedidoAdicional);
			boolean MostrarCFact = false;
			if (((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).existeParametro("ConsignacionCampoFacturar"))
				MostrarCFact = ((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).get("ConsignacionCampoFacturar").toString().equals("1");
			mVista.mostrarConsignacionFactura(MostrarCFact);
			mVista.EditarConsignacionFactura(Cfactura);
	...
	public void asignarGuardarValores() throws Exception
	{
        ...
        //boolean bExigirPedidoAdicional = false;
        //if (((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).existeParametro("ExigirPedidoAdicionalConsig") && ((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).get("ExigirPedidoAdicionalConsig").toString().equals("1")) {
            //bExigirPedidoAdicional = true;
        if (cliente.ExigirPedidoAdicional)
        {
            if (mVista.getPedidoAdicional().equals("")){
                mVista.setFocus("pedido adicional");
                throw new ControlError("BE0001", new String[]
                        { Mensajes.get("XPedidoAdicionalCons"), cliente.FormatoPedidoAdicional });
            }
		...
		while (it.hasNext())
		{
            ...
            //if (bExigirPedidoAdicional && cliente.ExigirPedidoAdicional) {
            if (cliente.ExigirPedidoAdicional) {
                if (!mVista.getPedidoAdicional().trim().equals("") && mVista.getEsNuevo()) {
                    ...
                    if (pedidoAdicional.isRecuperado()) {
                        ...
                        if (!mVista.getPedidoAdicional().trim().equals("")) {
                            pedidoAdicional.PedidoAdicional = mVista.getPedidoAdicional();
                            pedidoAdicional.MFechaHora = Generales.getFechaHoraActual();
                            pedidoAdicional.MUsuarioID = ((Usuario) Sesion.get(Campo.UsuarioActual)).USUId;
                            pedidoAdicional.Enviado = false;
        ...
        Vendedor vendedor = (Vendedor)Sesion.get(Campo.VendedorActual);
        if (vendedor.ValidaLimiteConsignas == 1 && vendedor.LimiteConsignas > 0){
            Consultas.ConsultasVendedor.InsertarClienteConConsignaAb(vendedor, cliente);
        }
	}
	...
	public void recalcular() throws Exception{
		final TransProd transProdVta;
		if(CapturarLiquidacionConsignacion.transVtaRespaldo != null){
			transProdVta = CapturarLiquidacionConsignacion.transVtaRespaldo;
		}
		else {
			transProdVta = transacciones.get(0);
		}
	...
	public void eliminarClienteDeConsignaAb(){
		try {
			Vendedor vendedor = (Vendedor) Sesion.get(Campo.VendedorActual);
			Cliente cliente = (Cliente) Sesion.get(Campo.ClienteActual);
			if (vendedor.ValidaLimiteConsignas == 1 && vendedor.LimiteConsignas > 0) {
