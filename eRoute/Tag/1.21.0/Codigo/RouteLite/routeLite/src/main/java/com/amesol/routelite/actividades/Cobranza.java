...
public class Cobranza
{
	...
	public static class VistaDocumentos
	{
		...
		public VistaDocumentos(String transprodid, String folio, Date fecha, float total, float saldo, Date fechaCobranza, float descuentoProntoPago, String referencia)
		{
			...
			if (Generales.getFechaActual().before(Generales.sumarAFecha(fechaCobranza,Calendar.DAY_OF_YEAR, diasTolerancia)) || Generales.getFechaActual().equals(Generales.sumarAFecha(fechaCobranza,Calendar.DAY_OF_YEAR, diasTolerancia)))
				ImporteDescPP = (saldo*descuentoProntoPago)/100;
			else
				ImporteDescPP = 0;
	...
	public static class VistaDetalle
	{
		...
		private boolean pagoBillpocket;

		public VistaDetalle(String abdid, int tipopago, String pago, String monedaid, String moneda, float importe, int tipobanco, String banco, String cuenta, String referencia, Date fechacheque, String observaciones)
		{
			ABDId = abdid;
			TipoPago = tipopago;
			Pago = pago;
		...
		public VistaDetalle(String abdid, int tipopago, String pago, String monedaid, String moneda, float importe, int tipobanco, String banco, String cuenta, String referencia, Date fechacheque, String observaciones, boolean pagoBillpocket)
		{
			ABDId = abdid;
			TipoPago = tipopago;
			Pago = pago;
			...
			FechaCheque = fechacheque;
            Observaciones = observaciones;
			this.pagoBillpocket = pagoBillpocket;
		}
		...
		public boolean isPagoBillpocket() {
			return pagoBillpocket;
		}

		public void setPagoBillpocket(boolean pagoBillpocket) {
			this.pagoBillpocket = pagoBillpocket;
		}
	}
		...
		if (buscar)
		{
			...
			while (itABT.hasNext())
			{
				...
				if (docto.TransProdID.equals(transProdId))
				{
                    ...
                    docto.SaldoInsoluto = Generales.getRound(transProd.Saldo - importe,2);
	...
	public static void agregarDetalle(Abono abono, VistaDetalle vistaDet)
	{
        ...
        if (vistaDet.Cuenta.length() > 0){
            detalle.Cuenta = vistaDet.Cuenta;
        }
		...
		detalle.PagoBillpocket = vistaDet.pagoBillpocket;
	...
	public static void guardarCobranza(Abono abono, String Folio, ArrayList<String> transProdIds, VistaDetalle det) throws Exception
	{
		try
		{
			...
			abono.ABNId = KeyGen.getId();
			abono.Folio = Folio;
            abono.SubEmpresaId=Consultas.ConsultasAbono.obtenerSubEmpresaIdDocumentos(transProdIds);
			...
			if (dsDocs.moveToFirst())
			{
				...
				do
				{
                    VistaDocumentos doc = new VistaDocumentos(dsDocs.getString("TransProdID"), "", dsDocs.getDate("Fecha"), 0, Generales.getRound(dsDocs.getFloat("Saldo"), 2), dsDocs.getDate("FechaCobranza"),(dsDocs.getColumnIndex("DescProntoPago") >0 ? dsDocs.getFloat(dsDocs.getColumnIndex("DescProntoPago")):0 ), "");
					listaDocs.add(doc);
				}
					...
					while (itDocs.hasNext())
					{
						...
						if (Generales.getRound(doc.getSaldo() - doc.getImporteDescPP(), 2) <= dTotalDecr)
						{
                            ...
                            if (doc.getImporteDescPP() > 0){
                                ...
                                Consultas.ConsultasAbono.ActualizaDescProntoPagoAplicado(doc.getTransprodID(), doc.getImporteDescPP(), true);
                            }
						}
						else
						{
	...
	public static void guardarCobranza(Abono abono, String Folio, ArrayList<String> transProdIds, VistaDetalle det, HashMap<String, Float> hmImportesxDoc) throws Exception
	{
		try
		{
...
*/
			ArrayList<VistaDocumentos> listaDocs = new ArrayList<Cobranza.VistaDocumentos>();
			ISetDatos dsDocs =Consultas.ConsultasAbono.obtenerDocumentosCriterio(transProdIds, (short) Integer.parseInt((String) ((CONHist) Sesion.get(Campo.CONHist)).get("TipoCobranza")));
			if (dsDocs.moveToFirst())
			{
			...
			if (listaDocs.toArray().length > 0)
			{
				...
				if (dTotalDecr > 0)
				{
					...
					{
						VistaDocumentos doc = itDocs.next();
						if (Generales.getRound(hmImportesxDoc.get(doc.getTransprodID()), 2) >0 ) {
							if (Generales.getRound(hmImportesxDoc.get(doc.getTransprodID()), 2) <= dTotalDecr) {
								guardarDocumento(abono, doc.getTransprodID(), hmImportesxDoc.get(doc.getTransprodID()), false, false);
								Transacciones.Pedidos.ActualizarSaldo(doc.getTransprodID(), -hmImportesxDoc.get(doc.getTransprodID()));

								dTotalDecr = Generales.getRound(dTotalDecr - hmImportesxDoc.get(doc.getTransprodID()), 2);
								hmImportesxDoc.put(doc.getTransprodID(), 0f);
							...
							/*if (doc.getImporteDescPP() > 0){
								...
								hmImportesxDoc.put(doc.getTransprodID(), Generales.getRound(hmImportesxDoc.get(doc.getTransprodID()) - dTotalDecr, 2));
								dTotalDecr = 0;
							}
						}
						if (dTotalDecr <= 0)
				...
				if (dTotalDecrChequeNormal > 0)
				{
					...
					while (itDocs.hasNext())
					{
						...
						if (Generales.getRound(hmImportesxDoc.get(doc.getTransprodID()), 2) > 0)
						{
							...
							if (Generales.getRound(hmImportesxDoc.get(doc.getTransprodID()), 2) <= dTotalDecrChequeNormal)
							{
								guardarDocumento(abono, doc.getTransprodID(), hmImportesxDoc.get(doc.getTransprodID()), blnHuboPrevios, false);
								Transacciones.Pedidos.ActualizarSaldo(doc.getTransprodID(), -hmImportesxDoc.get(doc.getTransprodID()));
								guardarCheque(abono, doc.getTransprodID(), hmImportesxDoc.get(doc.getTransprodID()), 0, false);

								dTotalDecrChequeNormal = Generales.getRound(dTotalDecrChequeNormal - (hmImportesxDoc.get(doc.getTransprodID())), 2);
								hmImportesxDoc.put(doc.getTransprodID(), 0f);
							...
							else
							{
								...
								hmImportesxDoc.put(doc.getTransprodID(),Generales.getRound(hmImportesxDoc.get(doc.getTransprodID()) - dTotalDecrChequeNormal, 2));
								dTotalDecrChequeNormal = 0;
							}
				...
				if (dTotalDecrChequePosfechado > 0)
				{
					...
					while (itDocs.hasNext())
					{
						...
						if (Generales.getRound(hmImportesxDoc.get(doc.getTransprodID()), 2) > 0)
						{
							...
							if (Generales.getRound(hmImportesxDoc.get(doc.getTransprodID()), 2) <= dTotalDecrChequePosfechado)
							{
								guardarDocumento(abono, doc.getTransprodID(), hmImportesxDoc.get(doc.getTransprodID()) - doc.getImporteDescPP(), blnHuboPrevios, false);
								// No actualiza el saldo del transprod porque es
								// posfechado
								guardarCheque(abono, doc.getTransprodID(), 0, hmImportesxDoc.get(doc.getTransprodID()), true);

								dTotalDecrChequePosfechado = Generales.getRound(dTotalDecrChequePosfechado - (hmImportesxDoc.get(doc.getTransprodID())), 2);
								hmImportesxDoc.put(doc.getTransprodID(), 0f);
							...
							else
							{
								...
								hmImportesxDoc.put(doc.getTransprodID(), Generales.getRound(hmImportesxDoc.get(doc.getTransprodID()) - dTotalDecrChequePosfechado, 2));
								dTotalDecrChequePosfechado = 0;
							}
...
*/
    ...
    public static void guardarCobranzaPagoAnticipado(Abono abono, TransProd transProd, Visita visita) throws Exception
    {
        try
        {
            ...
            abt.SaldoInsoluto=Generales.getRound(transProd.Saldo - abd.Importe,2);
	...
	public static void eliminarCobranza(String abnid) throws Exception
	{
		...
		if (!Clientes.validarLimiteCredito(cliente, abono) && cliente.LimiteCreditoDinero != 0)
			...
			while (itABT.hasNext())
			{
                ...
                if (aplicarDescProntoPago){
                    ...
                    if (descPP > 0) {
                        ...
                        Consultas.ConsultasAbono.ActualizaDescProntoPagoAplicado(abt.TransProdID, 0, false);
                    }
                }
	...
	{
		String sUsuarioID = ((Usuario) Sesion.get(Campo.UsuarioActual)).USUId;
        String sABNHistId = KeyGen.getId();

		AbonoHist abnHist = new AbonoHist();
        abnHist.ABNHistId = sABNHistId;
