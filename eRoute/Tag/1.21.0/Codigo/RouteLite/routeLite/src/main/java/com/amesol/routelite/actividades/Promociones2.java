...
public class Promociones2
{
	...
	public void TerminoPromocionRegalo() throws Exception
	{
		if (reglaActual != null)
		{
			ActualizaProducto(reglaActual.oProducto.ProductoClave, reglaActual.oPromocion.PromocionClave, reglaActual.oReglaApp.PromocionReglaID, false);

		}
		else if (reglaAcumActual != null)
		{
			ActualizaProducto(reglaAcumActual.oProducto.ProductoClave, reglaAcumActual.oPromocion.PromocionClave, reglaAcumActual.oReglaApp.PromocionReglaID, true);
		}
		SiguientePromocion();
	}
	...
	private void ActualizarDescBonif(String PromocionClave, String ProductoClave, float Porcentaje, String PromocionReglaID,boolean bAcumulado) throws Exception
	{
		int redondeo = 2;
		...
		while (oDetalles.hasNext())
		{
			...
			if (ProductoClave.equals(oTPD.ProductoClave) && (oTPD.Promocion != 2 && oTPD.Promocion != 3) && oTPD.getSubTotal() > 0)
			{
				...
				}else if(oPromociones.containsKey(PromocionClave) &&  oPromociones.get(PromocionClave).TipoAplicacion == Enumeradores.Promocion.TipoAplicacion.Bonificacion && (oPromociones.get(PromocionClave).Tipo == Enumeradores.Promocion.Tipo.Producto ||oPromociones.get(PromocionClave).Tipo == Enumeradores.Promocion.Tipo.Cliente)) {
					oTPD.DescuentoImp = Generales.getRound(oTPD.DescuentoImp + ((oTPD.Precio * oTPD.Cantidad) * Porcentaje), redondeo);
					oTPD.Subtotal = Generales.getRound(oTPD.Subtotal - ((oTPD.Precio * oTPD.Cantidad) * Porcentaje), redondeo);
				}else {
				...
				BDVend.guardarInsertar(oTPD);
				ActualizarProductosAplicados(bAcumulado ? "" : ProductoClave);
			}
		}
	}

	private void ActualizarBonificacionDirecta(String PromocionClave, String ProductoClave, float Bonificacion, String PromocionReglaID,boolean bAcumulado) throws Exception
	{
		int redondeo = 2;
...
//				Descuento oDescuento = Descuentos.BuscarDescuentosProductos(oTPD.ProductoClave);
//				oTPD.DescuentoImp = Descuentos.CalcularDescuentosProducto(oDescuento, oTPD.Cantidad * oTPD.Precio, oTPD.Cantidad);
//				oTPD.Subtotal = (oTPD.Precio * oTPD.Cantidad);

				oTransProd.Promocion = true;
				oTPD.Promocion = 1;
				oTPD.DescuentoImp = Generales.getRound(oTPD.DescuentoImp + Bonificacion, redondeo);
				oTPD.Subtotal = Generales.getRound(oTPD.Subtotal - Bonificacion, redondeo);
				...
				oTPP.PromocionClave = PromocionClave;
				oTPP.PromocionReglaID = PromocionReglaID; //TODO: promocionreglaid
				oTPP.PromocionImp = Generales.getRound(Bonificacion, redondeo);
				oTPP.MFechaHora = Generales.getFechaHoraActual();
				oTPP.MUsuarioID = sUsuarioId;
				oTPP.Enviado = false;
				oTPD.TrpPrp.add(oTPP);
	...
	private void ActualizaPuntos(String PromocionClave, int TipoRango, String ProductoClave, float Porcentaje, boolean bAcumulado) throws Exception
	{
		ListIterator<TransProdDetalle> oDetalles = oTransProd.TransProdDetalle.listIterator();
        float nPuntos = 0;
		while (oDetalles.hasNext())
	...
	private void ActualizaProducto(String productoClave, String promocionClave, String PromocionReglaID, boolean bAcumulado) throws Exception
	{
		ListIterator<TransProdDetalle> oDetalles = oTransProd.TransProdDetalle.listIterator();
    ...
    private void ActualizarDescProntoPago(String PromocionClave, String ProductoClave, float Porcentaje, boolean bAcumulado) throws Exception
    {
        ListIterator<TransProdDetalle> oDetalles = oTransProd.TransProdDetalle.listIterator();
        while (oDetalles.hasNext())
        {
	...
	public class Regla
	{
		...
		private boolean OtorgarDescuento() throws Exception
		{
			...
			ActualizarDescBonif(oPromocion.PromocionClave, oProducto.ProductoClave, nPorcentaje, oReglaApp.PromocionReglaID, false);
				...
				case Enumeradores.Promocion.TipoRegla.Grupo:
					switch (oPromocion.TipoRango)
					{
						case Enumeradores.Promocion.TipoRango.Cantidad:
							...
							ActualizarBonificacionDirecta(oPromocion.PromocionClave, oProducto.ProductoClave, nImporteDesc * nCantidadGrupo, oReglaApp.PromocionReglaID,false);
							AfectoImpuestos = true;
							return true;
						case Enumeradores.Promocion.TipoRango.Importe:
		...
		private boolean OtorgarPuntos() throws Exception
		{
			...
			ActualizaPuntos(oPromocion.PromocionClave, oPromocion.TipoRango, oProducto.ProductoClave, nPorcentaje, false);
        ...
        private boolean OtorgarDescuentoProntoPago() throws Exception
        {
            ...
            ActualizarDescProntoPago(oPromocion.PromocionClave, oProducto.ProductoClave, nPorcentaje,false);
            return true;
        }
	}
...
//                        nCantidad = oPromocion.ProductosKit.get(oProducto.ProductoClave);
			...
			ActualizarDescBonif(oPromocion.PromocionClave, oProducto.ProductoClave, nPorcentaje, oReglaApp.PromocionReglaID,true);
		...
		private boolean OtorgarPuntos() throws Exception
		{
			...
			if (nPuntosAplicar > 0)
			{
				ActualizaPuntos(oPromocion.PromocionClave, oPromocion.TipoRango, oProducto.ProductoClave, nPorcentaje,true);
			}
        ...
        private boolean OtorgarDescuentoProntoPago() throws Exception
        {
            ...
            ActualizarDescProntoPago(oPromocion.PromocionClave, oProducto.ProductoClave, nPorcentaje,true);
            return true;
        }
	}
