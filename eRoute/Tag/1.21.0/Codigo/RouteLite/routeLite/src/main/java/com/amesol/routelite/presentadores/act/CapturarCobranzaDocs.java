...
public class CapturarCobranzaDocs extends Presentador {
    ...
    public void ObtenerDocumentosSubEmpresa(String sSubEmpresaId) {
        try {
            Cliente cliente = (Cliente) Sesion.get(Campo.ClienteActual);
            Cobranza.VistaDocumentos[] oDocumentos;
            oDocumentos = Consultas.ConsultasAbono.obtenerDocumentosPorCobrarPorSubEmpresa(cliente.ClienteClave, (short) Integer.parseInt((String) ((CONHist) Sesion.get(Campo.CONHist)).get("TipoCobranza")), "Folio, Fecha", sSubEmpresaId);
            mVista.mostrarCobranzaDocs(oDocumentos);
        } catch (Exception e) {
            mVista.mostrarError(e.getMessage());
        }
