...
public class CapturaTotales extends Vista implements ICapturaTotales
{
    ...
    boolean mostrarCampoNoRemision = false;
    ...
    private OnClickListener mTerminar = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            try {
                ...
                //Si es reparto, se esta surtiendo y el error es de limite de credito, no se habilita el boton, ya que en ese punto
                //ya se afecto el inventario y se permitia descontarlo doble.
                //En ese caso  el vendedor tendra que dar atras hasta salirse
                if (!(Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO && (getSurtir() || getModificando()) && ex.getMessage()!=null && ex.getMessage().contains("E0599"))){
                    btn.setEnabled(true);
                }else{
                    //Si hubo error de limite
                    Sesion.set(Campo.ResultadoActividad,"ErrorLimiteCredito" );
                }
            }
        }
    };
    ...
    private OnItemSelectedListener mFormaVenta = new OnItemSelectedListener() {
        ...
        public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
            ...
            if (nCFVTipoIni != nCFVTipoAnterior && nCFVTipoAnterior != 0) {
                boolean bSobreprecio = false;
                for (TransProd tran : mPresenta.getTransacciones()) {
                    if (tran.AplicaSobreprecio) {
                        bSobreprecio = true;
                        break;
                    }
                }
                if (bSobreprecio)
                {
                    for (TransProd tran : mPresenta.getTransacciones())
                        tran.CFVTipo = item.getShort(0);
                    setResultado(Enumeradores.Resultados.RESULTADO_BIEN, "I0321");
                    finalizar();
                }
            }
            nCFVTipoAnterior = nCFVTipoIni;
    ...
    private OnChangedListener mPorcentaje = new OnChangedListener() {
        ...
        @Override
        public void onChanged(NumberPicker picker, int oldVal, int newVal) {
            //El control no detecta cuando cambia 0 despues de regresar del detalle del pedido
            NumberPicker importe= (NumberPicker) findViewById(R.id.npDescVendedor);
            ...
            if (oldVal != newVal) { // || newVal == 0){
                ...
                //Se agrega else para corregir problema de deteccion en el control de cambio a 0
            }else if(newVal == 0 && importe.getCurrentDecimal()>0){
                if (separarTotalesSubEmpresa){
                    Spinner spn = (Spinner) findViewById(R.id.spFolio);
                    HashMap<String,TransProd> spinnerMap = (HashMap<String,TransProd>)spn.getTag();
                    mPresenta.calcularDescVendedor(spinnerMap.get(folioSeleccionado), 0);
                }else{
                    mPresenta.calcularDescVendedor(0);
                }
            }
        }
    };
    ...
    public void onCreate(Bundle savedInstanceState) {
        try {
            ...
            if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("MostrarCampoNoRemision")) {
                if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("MostrarCampoNoRemision").equalsIgnoreCase("1")) {
                    mostrarCampoNoRemision = true;
                }
            }
            ...
            if (mostrarCampoNoRemision){
                LinearLayout layNoRemision = (LinearLayout) findViewById(R.id.layNoRemision);
                layNoRemision.setVisibility(View.VISIBLE);
                texto = (TextView) findViewById(R.id.lblNoRemision);
                texto.setText(Mensajes.get("XNoRemision"));
            }
            ...
            if (aTransProdIds != null && aTransProdIds.size() > 0) {
                ...
                if (oParametros != null) {
                    if (oParametros.containsKey("ProdSinExistencia")) {
                        HashMap<String, Float> prodSinExistencia = (HashMap<String, Float>)oParametros.get("ProdSinExistencia");
                        mPresenta.setProdSinExistencia(prodSinExistencia);
                    }
                }
                mPresenta.iniciar();
            }
        ...
        try {
            if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("TipoPagoCredito") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("TipoPagoContado")) {
                if (!((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("TipoPagoCredito").equals("0") || !((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("TipoPagoContado").equals("0")) {
                    spFormaVenta.setOnItemSelectedListener(new OnItemSelectedListener() {
                        @Override
                        public void onItemSelected(AdapterView<?> parentView, View selectedItemView, int position, long id) {
                            try {
                                String sMensaje = "";
                                int nCFVTipo = Consultas.ConsultasCliFormaVenta.obtenerFormaVentaInicial(((Cliente)Sesion.get(Campo.ClienteActual)).ClienteClave);
                                if (nCFVTipo == 1 || nCFVTipo == 0) {
                                    if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("PagoVacioContado")) {
                                        if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("PagoVacioContado").equals("1")) {
                                            sMensaje =  Mensajes.get("XSeleccionarOpcion");
                                        }
                                    }
                                }
                                String[] FormasPagoContado = {};
                                String FormasPagoCo = "";
                                String[] FormasPagoCredito = {};
                                String FormasPagoCr = "";
                                FormasPagoCredito = ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("TipoPagoCredito").toString().trim().split(",");
                                FormasPagoContado = ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("TipoPagoContado").toString().trim().split(",");
                                for(int i = 0; i < FormasPagoCredito.length; i ++){
                                    FormasPagoCr +=  ("'" + FormasPagoCredito[i].trim() + "',");
                                }
                                FormasPagoCr = FormasPagoCr.substring(0, FormasPagoCr.length() - 1);
                                for(int i = 0; i < FormasPagoContado.length; i ++){
                                    FormasPagoCo +=  ("'" + FormasPagoContado[i].trim() + "',");
                                }
                                FormasPagoCo = FormasPagoCo.substring(0, FormasPagoCo.length() - 1);

                                if(!((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("TipoPagoCredito").equals("0") && id == 2){
                                    formasPago = Consultas.ConsultasValorReferencia.obtenerValoresReferencia("PAGO", sMensaje, FormasPagoCr);
                                }
                                if(!((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("TipoPagoCredito").equals("0") && id == 1){
                                    formasPago = Consultas.ConsultasValorReferencia.obtenerValoresReferencia("PAGO", sMensaje, FormasPagoCo);
                                }
                                cargarFormasPago();
                            } catch (Exception ex) {
                            }
	...
	private void HabilitarControles(boolean habilitar) {
        ...
        if (mostrarCampoNoRemision){
            edit = (EditText) findViewById(R.id.txtNoRemision);
            edit.setEnabled(habilitar);
        }
    ...
    public void validarDesctoVendedor(float subTDetalle, float descuentoImp) throws Exception {
        ...
        if (surtir && !((MOTConfiguracion) Sesion.get(Campo.MOTConfiguracion)).get("ModificarVenta").toString().equals("1"))
            habilitar = false;
        /*
    ...
    private void llenarPedidoActual(TransProd oTrp){
        try{
            ...
            if (pedidoAdicional != null) {
                ...
                if (mostrarCampoNoRemision)
                    setRemision(pedidoAdicional.Remision == null ? "" : pedidoAdicional.Remision);
            }
    ...
    public String getRemision() {
        EditText texto = (EditText) findViewById(R.id.txtNoRemision);
        return texto.getText().toString().trim();
    }

    public void setRemision(String remision) {
        EditText texto = (EditText) findViewById(R.id.txtNoRemision);
        texto.setText(remision);
    }
    ...
    public void setFocus(String campo) {
        ...
        } else if (campo.toLowerCase().equals("remision")) {
            EditText remision = (EditText) findViewById(R.id.txtNoRemision);
        ...
        }else if (campo.toLowerCase().equals("tipo turno")) {
            Spinner spinTipoTurno = (Spinner) findViewById(R.id.spTurno);
            spinTipoTurno.requestFocus();
        }
    }
    ...
    private void obtenerValoresPorReferencia() throws Exception {
        ...
        if (fPagoCliente.equals(""))
            formasPago = Consultas.ConsultasValorReferencia.obtenerValoresReferencia("PAGO", sMensaje);
        else
            formasPago = Consultas.ConsultasValorReferencia.obtenerValoresReferencia("PAGO", sMensaje, fPagoCliente);
    ...
    public void solicitarImprimirTicket(){
        ...
        if (motConfig.equals("0")) {
            // si no esta configurada la pregunta salir
            ValidarGenerarConsigna();
            //setResultado(Enumeradores.Resultados.RESULTADO_BIEN);
            //finalizar();
        } else if (motConfig.equals("1")) {
            mostrarPreguntaSiNo(Mensajes.get("P0103"), 8);
        } else if (motConfig.equals("2") || motConfig.equals("3") || motConfig.equals("4")) {
...
//                    else if (Sesion.get(Campo.ArchivosPDFxEnviar) == null && Short.valueOf(((MOTConfiguracion) Sesion.get(Campo.MOTConfiguracion)).get("MensajeImpresion").toString()) == 4){
//                        try
//                        {
//                            mPresenta.WhatsappTicket();
////                            this.mostrarMensaje(Mensajes.get("I0307").replace("$0$", Mensajes.get("XCorreoWhats")), 0, 0);
////                            this.finalizar();
//                        }
//                        catch (Exception e)
//                        {
...
//                    }
                ...
                if (resultCode == Enumeradores.Resultados.RESULTADO_BIEN) {
                    if (Short.valueOf(((MOTConfiguracion) Sesion.get(Campo.MOTConfiguracion)).get("MensajeImpresion").toString()) == 3) {
                        ...
                        if (Short.valueOf(((MOTConfiguracion) Sesion.get(Campo.MOTConfiguracion)).get("MensajeImpresion").toString()) == 4) {
                            try
                            {
                                ConfiguracionLocal conf = (ConfiguracionLocal) Sesion.get(Sesion.Campo.ConfiguracionLocal);
                                Dispositivo.EncenderApagarWiFi(getApplicationContext(), true, 4);
                                mPresenta.WhatsappTicket();
                            }
                            catch (Exception e)
                            {
...
*/
    ...
    private void ValidarGenerarConsigna(){
        try {
            if(Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO && getModificando()){
                if (((ConfigParametro)Sesion.get(Campo.ConfigParametro)).existeParametro("ConvertirPedParcEnConsigna") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("ConvertirPedParcEnConsigna").equals("1")) {
                    if (mPresenta.validarPedidoEnConsigna()){
                        mostrarPreguntaSiNo(Mensajes.get("P0262"), 3);
                    }
                }else{
                    setResultado(Enumeradores.Resultados.RESULTADO_BIEN);
                    finalizar();
    ...
    public void calcularPromocionesConsigna(TransProd transProd)
    {
        try
        {
            HashMap<String, TransProd> hsTransProd = new HashMap<>();
            hsTransProd.put(transProd.SubEmpresaId, transProd);
            Sesion.set(Campo.ArrayTransProd, hsTransProd);
            //siguiente = false;
            try
            {
            ...
            promociones = new Promociones2((TransProd) transProd, (Vista)this, false);
            promociones.setOnTerminarAplicacionListener(new Promociones2.onTerminarAplicacionListener()
            {
                ...
                {
                    try {
                        mPresenta.terminarConsignacion();
                        setResultado(Enumeradores.Resultados.RESULTADO_BIEN);
                        finalizar();
                    }catch (Exception e){}
                }
    ...
    public void mostrarPromocionProducto(String promocionClave, String promocionReglaID, int CantidadGrupoApp, String SubEmpresaId, int cantidad, String productoDisparador, String cadenaListasPrecios)
    {
        ...
        oParametros.put("TipoValidarExistencia", com.amesol.routelite.actividades.Enumeradores.Inventario.TiposValidacionInventario.ValidacionRestrictiva);
        oParametros.put("ProductoDisparador", productoDisparador);
        oParametros.put("CadenaListasPrecio", cadenaListasPrecios);
        iniciarActividadConResultado(IAplicacionPromocion.class, Enumeradores.Solicitudes.SOLICITUD_MOSTRAR_PROMOCIONES_APLICADAS, Enumeradores.Acciones.ACCION_APLICAR_PROMOCIONES, oParametros);
    }
