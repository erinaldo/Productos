...
import com.amesol.routelite.datos.AddendaDetalle;
...
public class SeleccionFacturacion extends Vista implements ISeleccionFacturacion
{
	...
	public static final int DATOS_COMPLEMENTO_INDEX = 5;
	...
	private int saltarMetodoPago = 0;
	...
	public void iniciar()
	{
		...
		mTexto = (TextView) findViewById(R.id.lblDatosComplemento);
		mTexto.setText(Mensajes.get("XComplemento"));
	...
	public void initSpinners()
	{
		...
		mPresentador.initSpinner((Spinner) findViewById(R.id.spnTipoPago));
	...
	public void cargarDetalleFactura(TransProd factura, TRPDatoFiscal datoFiscal)
	{
		...
		SeleccionarFacturacion.VistaSpinner vsTipoPago = mPresentador.getTipoPago();
		if (vsTipoPago !=null){
			ArrayAdapter<SeleccionarFacturacion.VistaSpinner> adapterTipoPago = new ArrayAdapter<SeleccionarFacturacion.VistaSpinner>(this, android.R.layout.simple_spinner_item, new SeleccionarFacturacion.VistaSpinner[]{vsTipoPago});
			((Spinner)findViewById(R.id.spnTipoPago)).setAdapter(adapterTipoPago);
		}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event)
	{
		switch (keyCode)
		{
			case KeyEvent.KEYCODE_BACK:
				if (indiceActivo == METODOS_PAGO_INDEX){
					if(validarSalida() && getFormaDeVentaSeleccionada() == 2 && saltarMetodoPago == 1){
						saltarMetodoPago = 0;
						try{
							if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("SeleccionarMetodoPagoCredito")) {
								if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("SeleccionarMetodoPagoCredito").equals("0")) {
	...
	@Override
	public int getTipoDePagoSeleccionado()
	{
		try{
			int index =Integer.parseInt(((SeleccionarFacturacion.VistaSpinner)((Spinner) findViewById(R.id.spnTipoPago)).getSelectedItem()) .getId());
			return index;
		}catch (Exception ex){
			mostrarError("Error al obtener TipoPago " + ex.getMessage());
		}
		return 0;
	}
	@Override
...
//				{
				...
				if("1".equals(motCfg.get("MensajeImpresion")))
				{
					...
					return;
				}else if (motCfg.get("MensajeImpresion").equals("2") || motCfg.get("MensajeImpresion").equals("3") || motCfg.get("MensajeImpresion").equals("4")){
		...
		public void onClick(View v)
		{
			if(indiceActivo == DATOS_FACTURACION_INDEX)
			{
				if (validarDatosCliente()) {
					if (!mPresentador.requiereAddendaCliente()) {
						FinalizarFactura();
					} else {
						Continuar();
					}
				}
			} else if (indiceActivo == DATOS_COMPLEMENTO_INDEX){
				if (!soloLectura) {
					String errorAddenda = mPresentador.validarDatosAddenda();
					if (errorAddenda.equals("")) {
						FinalizarFactura();
					} else
						mostrarError(errorAddenda);
				} else
					FinalizarFactura();
			} else {
				...
				{
					Continuar();
					if(getFormaDeVentaSeleccionada() == 2 && saltarMetodoPago == 0){
						saltarMetodoPago = 1;
						try{
							if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("SeleccionarMetodoPagoCredito")) {
								if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("SeleccionarMetodoPagoCredito").equals("0")) {
				...
				if(layoutParent.getChildAt(index).getVisibility() != View.GONE)
				{
					...
					switch(indiceActivo){
						case PEDIDOS_INDEX:
							...
							break;
						case DATOS_COMPLEMENTO_INDEX:
							mPresentador.cargarDetalleAdenda(soloLectura);
							break;
					}
					break;
				}
...
//					}
			...
			try{
				if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("LongitudCampoIdFiscal")) {
					if (!((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("LongitudCampoIdFiscal").equals("0")) {
						int LongitudId = Integer.parseInt(((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("LongitudCampoIdFiscal"));
						if (LongitudId == 0)
							LongitudId = 14;
						else
							LongitudId = LongitudId + 1;
						if (((TextView) findViewById(R.id.txtRFC)).getText().toString().replace("-", "").trim().length() > LongitudId) {
							mostrarAdvertencia(Mensajes.get("E0718").replace("$0$", "RFC").replace("$1$", String.valueOf(LongitudId)));
							return;
						}
					}
				}
			...
			try {
				...
				mostrarAdvertencia("Error al obtener parámetro LongitudCampoCodigoPostal");
			}

			if (lengthCodigoPostal > 0){
				if (((TextView) findViewById(R.id.txtCodigoPostalDF)).getText().toString() == null || ((TextView) findViewById(R.id.txtCodigoPostalDF)).getText().toString().equals("")){
					String[] param = {Mensajes.get("XCodigoPostal"), lengthCodigoPostal.toString()};
					mostrarAdvertencia(Mensajes.get("E1029", param));
					return;
				}
				else if (((TextView) findViewById(R.id.txtCodigoPostalDF)).getText().toString().length() != lengthCodigoPostal){
					String[] param = {Mensajes.get("XCodigoPostal"), lengthCodigoPostal.toString()};
					mostrarAdvertencia(Mensajes.get("E1029", param));
					return;
				}
	...
	private OnItemSelectedListener listenerSpinners = new OnItemSelectedListener()
	{
		...
		public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3)
		{
			...
			if(arg0.getId() == R.id.spnSubEmpresa) {
			...
			}else if(arg0.getId() == R.id.spnFormaPago){
					int iCFVTipo = ((CLIFormaVenta)arg0.getItemAtPosition(arg2)).CFVTipo ;
					if (iCFVTipo>0){
						mPresentador.initSpinner((Spinner)findViewById(R.id.spnTipoPago ), String.valueOf(iCFVTipo));
					}
			}
		}
	...
	public void limpiarControlesAddenda(){
		LinearLayout layComplemento = (LinearLayout) findViewById(R.id.layComplemento);
		LinearLayout layDatos = (LinearLayout) layComplemento.findViewById(R.id.layDatosComplemento);
		int maxIndex = layDatos.getChildCount() - 1;
		for(int index = maxIndex; index > 0; index--) {
			layDatos.removeViewAt(index);
		}
	}

	public void agregarDatoTextoAddenda(AddendaDetalle detalle){
		try {
			LinearLayout elemTexto = (LinearLayout) this.getLayoutInflater().inflate(R.layout.elemento_texto, null);
			TextView lblElem = (TextView) elemTexto.findViewById(R.id.textView1);
			lblElem.setText(detalle.Etiqueta);

			EditText txtElem = (EditText) elemTexto.findViewById(R.id.editText1);
			if (soloLectura) {
				txtElem.setText(detalle.Respuesta);
				txtElem.setEnabled(false);
			} else {
				if (detalle.LongMax > 0)
					txtElem.setFilters(new InputFilter[]{new InputFilter.LengthFilter(detalle.LongMax)});
				txtElem.setTag(detalle.ADDDId);
			}

			LinearLayout layComplemento = (LinearLayout) findViewById(R.id.layComplemento);
			LinearLayout layDatos = (LinearLayout) layComplemento.findViewById(R.id.layDatosComplemento);
			layDatos.addView(elemTexto);
		}catch (Exception ex){
			ex.getMessage();
		}
	}

	public void agregarDatoNumericoAddenda(AddendaDetalle detalle){
		LinearLayout elemTexto = (LinearLayout) this.getLayoutInflater().inflate(R.layout.elemento_texto, null);
		TextView lblElem = (TextView) elemTexto.findViewById(R.id.textView1);
		lblElem.setText(detalle.Etiqueta);
		...
		if (soloLectura) {
			...
			if (detalle.TipoDato == 2)
				txtElem.setInputType(InputType.TYPE_CLASS_NUMBER);
			else
				txtElem.setInputType(InputType.TYPE_CLASS_NUMBER | InputType.TYPE_NUMBER_FLAG_DECIMAL);
			txtElem.setTag(detalle.ADDDId);
		}
		LinearLayout layComplemento = (LinearLayout) findViewById(R.id.layComplemento);
		LinearLayout layDatos = (LinearLayout) layComplemento.findViewById(R.id.layDatosComplemento);
	...
	public void agregarDatoFechaAddenda(AddendaDetalle detalle){
		LinearLayout elemFecha = (LinearLayout) this.getLayoutInflater().inflate(R.layout.elemento_fecha, null);
		TextView lblElem = (TextView) elemFecha.findViewById(R.id.textView1);
		lblElem.setText(detalle.Etiqueta);

		Button btnElem = (Button) elemFecha.findViewById(R.id.button1);
		if (soloLectura) {
			btnElem.setText(detalle.Respuesta);
			btnElem.setEnabled(false);
		}
		else{
			btnElem.setText("");
			btnElem.setOnClickListener(mDatoFechaAddenda);
			btnElem.setTag(detalle.ADDDId);
		}

		//elemFecha.setTag(detalle.ADDDId);
		LinearLayout layComplemento = (LinearLayout) findViewById(R.id.layComplemento);
		LinearLayout layDatos = (LinearLayout) layComplemento.findViewById(R.id.layDatosComplemento);
		layDatos.addView(elemFecha);
	}

	public void agregarDatoListaAddenda(AddendaDetalle detalle){
		LinearLayout elemLista = (LinearLayout) this.getLayoutInflater().inflate(R.layout.elemento_lista, null);
		TextView lblElem = (TextView) elemLista.findViewById(R.id.textView1);
		lblElem.setText(detalle.Etiqueta);


		Spinner spnElem = (Spinner) elemLista.findViewById(R.id.lista1);

		String[] valores;
		if (soloLectura)
			valores = new String[]{detalle.Respuesta};
		else
			valores = detalle.Valor.split("\\|");

		ArrayAdapter<String> adapterSpn = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, valores);
		spnElem.setAdapter(adapterSpn);

		if (soloLectura)
			spnElem.setEnabled(false);
		else
			spnElem.setTag(detalle.ADDDId);

		LinearLayout layComplemento = (LinearLayout) findViewById(R.id.layComplemento);
		LinearLayout layDatos = (LinearLayout) layComplemento.findViewById(R.id.layDatosComplemento);
		layDatos.addView(elemLista);
	}

	public String validarDatosAddenda(ArrayList<AddendaDetalle> addendaDetalle){
		LinearLayout layComplemento = (LinearLayout) findViewById(R.id.layComplemento);
		String error = "";
		for (AddendaDetalle det : addendaDetalle) {
			String errorDet = "";
			switch (det.TipoDato){
				case 1: //Texto
					EditText txtTexto = (EditText) layComplemento.findViewWithTag(det.ADDDId);
					String texto = txtTexto.getText().toString().trim();
					if (det.Requerido && texto.length() == 0)
						errorDet = Mensajes.get("BE0001").replace("$0$", det.Etiqueta) + "\n";

					if (errorDet.equals("") && (det.LongMin > 0 || det.LongMax > 0))
						if (texto.length() < det.LongMin || texto.length() > det.LongMax)
							errorDet = Mensajes.get("E1026").replace("$0$", det.Etiqueta).replace("$1$", String.valueOf(det.LongMin)).replace("$2$", String.valueOf(det.LongMax)) + "\n";

					if (errorDet.equals("") && texto.length() > 0)
						det.Respuesta = texto;
					break;
				case 2: //Entero
				case 3: //Decimal
					EditText txtNumero = (EditText) layComplemento.findViewWithTag(det.ADDDId);
					String numero = txtNumero.getText().toString().trim();
					if (det.Requerido && numero.length() == 0)
						errorDet = Mensajes.get("BE0001").replace("$0$", det.Etiqueta) + "\n";

					if (errorDet.equals("") && numero.length() > 0) {
						if (det.TipoDato == 2)
							det.Respuesta = String.valueOf(Integer.parseInt(numero));
						else
							det.Respuesta = numero;
					}
					break;
				case 4: //Fecha
					Button btnFecha = (Button) layComplemento.findViewWithTag(det.ADDDId);
					String fecha = btnFecha.getText().toString().trim();
					if (det.Requerido && fecha.length() == 0)
						errorDet = Mensajes.get("BE0001").replace("$0$", det.Etiqueta) + "\n";

					if (errorDet.equals("") && fecha.length() > 0)
						det.Respuesta = fecha;
					break;
				case 5: //Lista
					Spinner spnLista = (Spinner) layComplemento.findViewWithTag(det.ADDDId);
					String seleccion = spnLista.getSelectedItem().toString();

					if (det.Requerido && seleccion.length() == 0)
						errorDet = Mensajes.get("BE0001").replace("$0$", det.Etiqueta) + "\n";

					if (errorDet.equals("") && seleccion.length() > 0)
						det.Respuesta = seleccion;
					break;
			}
			error += errorDet;
		}
	...
	String sTagDatoFechaActual;
	private OnClickListener mDatoFechaAddenda = new OnClickListener() {

		@SuppressWarnings("deprecation")
		@Override
		public void onClick(View v) {
			sTagDatoFechaActual = v.getTag().toString();
			showDialog(999);
		}
	};
	...
	protected Dialog onCreateDialog(int id) {
		...
		return new DatePickerDialog(this, mDatoFechaAddendaListener, anio, mes, dia);
	...
	private DatePickerDialog.OnDateSetListener mDatoFechaAddendaListener = new DatePickerDialog.OnDateSetListener()
	{
		...
		public void onDateSet(DatePicker view, int selectedYear, int selectedMonth, int selectedDay) {
			try {
				...
				Date fecha = new Date(selectedYear, selectedMonth, selectedDay);
				LinearLayout layComplemento = (LinearLayout) findViewById(R.id.layComplemento);
				//ScrollView scrollDatos = (ScrollView) layComplemento.findViewById(R.id.scrollDatos);
				Button btnElem = (Button) layComplemento.findViewWithTag(sTagDatoFechaActual);
				btnElem.setText(fecha.getYear() + new SimpleDateFormat("-MM-dd").format(fecha));
				//yyyy-MM-dd
