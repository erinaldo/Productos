...
//				oTPD.Subtotal = (oTPD.Precio * oTPD.Cantidad);
				...
				}if(oPromociones.containsKey(PromocionClave) &&  oPromociones.get(PromocionClave).TipoAplicacion == Enumeradores.Promocion.TipoAplicacion.Bonificacion && (oPromociones.get(PromocionClave).Tipo == Enumeradores.Promocion.Tipo.Producto ||oPromociones.get(PromocionClave).Tipo == Enumeradores.Promocion.Tipo.Cliente)) {
					oTPD.DescuentoImp = Generales.getRound(oTPD.DescuentoImp + ((oTPD.Precio * oTPD.Cantidad) * Porcentaje), redondeo);
					oTPD.Subtotal = Generales.getRound(oTPD.Subtotal - ((oTPD.Precio * oTPD.Cantidad) * Porcentaje), redondeo);
				}else {
