...
public class TransaccionesDetalle
{
	...
	public static TransProdDetalle GenerarDetalleBusqueda(String ProductoClave, int TipoUnidad, float Cantidad, float Precio,String PrecioClave, String Lote)
	{
        ...
        if ((PrecioClave != null && PrecioClave.length()>0) || (Lote != null && Lote.length()>0)){
	...
	public static TransProdDetalle ActualizarTipoMotivoTransaccion(TransProdDetalle transprodDetalle, Short tipoMotivo, short tipoTransprod, Float cantidad, boolean cancelar, boolean manejarInventario) throws Exception
	{
		TransProdDetalle tpd = transprodDetalle;
		tpd.TipoMotivo = tipoMotivo;
        if (manejarInventario) {
	...
	public static class Pedidos {
        ...
        public static TransProdDetalle GenerarDetalleCanjes(String TransProdId, String ProductoClave, int TipoUnidad, float Cantidad, int Puntos, StringBuilder byRefError) throws Exception {
            ...
            transProdDetalle.Cantidad = Cantidad;
            transProdDetalle.Partida = Consultas.ConsultasTransProdDetalle.obtenerPartida(TransProdId) + 1;
            transProdDetalle.Precio = Puntos;
            transProdDetalle.Subtotal = Cantidad * Puntos;
            transProdDetalle.Impuesto = 0f;
            transProdDetalle.Total = Cantidad * Puntos;
            transProdDetalle.MFechaHora = Generales.getFechaHoraActual();
            transProdDetalle.MUsuarioID = sUsuarioID;
        ...
        public static TransProdDetalle GenerarDetalleConsignaDePedido(String TransProdId, String ProductoClave, int TipoUnidad, float Cantidad, float Precio, String PrecioClave) throws Exception {
            //StringBuilder sPrecioClave = new StringBuilder();
            float nPrecio = Precio; //ListaPrecio.BuscarPrecio(ProductoClave, Short.parseShort(String.valueOf(TipoUnidad)), listasPrecios, sPrecioClave,Cantidad);
            if (nPrecio >= 0) {
                String sUsuarioID = ((Usuario) Sesion.get(Campo.UsuarioActual)).USUId;
        ...
        public static boolean EliminarDetalleAjustesInventario(String TransProdId, boolean Carga) throws Exception {
            ...
            boolean afectarInventario = true;
            if (Carga || trp.Tipo == 4 || trp.Tipo == 7) {
        ...
        public static TransProdDetalle GuardarDetalleRegaladosYCanjes(TransProd oTransProd, String ProductoClave, int TipoUnidad, float Cantidad, String PromocionClave, int TipoValidacionInventario, boolean PendienteEntrega, String ProductoDisparador, Float Precio, String PrecioClave, String Lote) throws Exception {
            ...
            if (Precio>0 && PrecioClave != null && PrecioClave.length()>0){
                esCanje=true;
            }
            String sUsuarioID = ((Usuario) Sesion.get(Campo.UsuarioActual)).USUId;
            ...
            if (TipoValidacionInventario != TiposValidacionInventario.NoValidar) {
                AtomicReference<Float> existencia = new AtomicReference<Float>();
                StringBuilder error = new StringBuilder();
                boolean validaExistencia;
                if(inventarioPorLotes)
                    validaExistencia = InventarioLotes.ValidarExistencia(ProductoClave, TipoUnidad, Lote, Cantidad, (short) TiposMovimientos.SALIDA, (short) oTransProd.Tipo, existencia, error);
                else
                    validaExistencia = Inventario.ValidarExistencia(ProductoClave, TipoUnidad, Cantidad, (short) TiposMovimientos.SALIDA, (short) oTransProd.Tipo, existencia, error);
            ...
            if (entregado > 0) {
                ...
                if(inventarioPorLotes)
                    transProdDetalleID = Consultas.ConsultasPromocion.recuperarTransProdDetalleIDPromocion(oTransProd.TransProdID,ProductoClave, TipoUnidad, PromocionClave, Lote );
                else
                    transProdDetalleID = Consultas.ConsultasPromocion.recuperarTransProdDetalleIDPromocion(oTransProd.TransProdID,ProductoClave, TipoUnidad, PromocionClave );
                ...
                if (Precio > 0){
                    transProdDetalle.Precio = Precio;
                    transProdDetalle.Subtotal = Generales.getRound(Precio * transProdDetalle.Cantidad, 2);
                    Impuesto[] listaImpuestos = new Impuesto[] {};
                    Cliente oCliente = (Cliente) Sesion.get(Campo.ClienteActual);
                ...
                if (!transProdDetalle.isRecuperado())
                ...
                if(!inventarioPorLotes)
                    ...
                    InventarioLotes.ActualizarInventario(ProductoClave, TipoUnidad, Lote, entregado, oTransProd.Tipo, TiposMovimientos.SALIDA);
            }
            ...
            if (esCanje && transProdDetalle != null ){
                ...
                if(inventarioPorLotes)
                {
                    ...
                    tpdExtra.Lote = Lote;
                    tpdExtra.TransProdDetalleID = transProdDetalle.TransProdDetalleID;
                    tpdExtra.TransProdID = oTransProd.TransProdID;
                    BDVend.recuperar(tpdExtra);
                    tpdExtra.MUsuarioID = sUsuarioID;
                    tpdExtra.MFechaHora = Generales.getFechaHoraActual();
                    BDVend.guardarInsertar(tpdExtra);
                    if (!tpdExtra.isRecuperado()) {
                        transProdDetalle.TPDDatosExtra.add(tpdExtra);
                    }
                }
            }
