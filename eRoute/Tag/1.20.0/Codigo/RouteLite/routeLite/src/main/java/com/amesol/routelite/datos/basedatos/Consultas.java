...
//            return BDTerm.consultar(consulta.toString());
		...
		public static ISetDatos obtenerCantidadesPorProducto(String sTransProdId, String sProductoClave)
		{
			...
			{
				StringBuilder sConsulta = new StringBuilder();
				sConsulta.append("SELECT SUM(TPD.Cantidad * PRD.Factor) AS Cantidad, SUM(TPD.Subtotal) AS Subtotal, SUM(TPD.Impuesto) AS Impuesto, SUM(TPD.Precio) as Precio ");
				sConsulta.append("FROM TransProdDetalle TPD ");
				sConsulta.append("INNER JOIN ProductoDetalle PRD ON TPD.ProductoClave = PRD.ProductoClave AND TPD.TipoUnidad = PRD.PRUTipoUnidad and PRD.ProductoClave = PRD.ProductoDetClave ");
...
//			consulta.append("WHERE TP.TransProdID= '" +MSIEV+ "') AS MSIEV ON UV.ProductoClave = MSIEV.ProductoClave");
	...
	public static final class ConsultasLiquidacionNOR {
		...
		public static ISetDatos obtenerTipoCobranza() throws Exception{
			...
			if (fechas[0] != null && fechas[1] != null) {
				consulta.append("SELECT ");
				consulta.append("	CLI.PublicoGeneral, IFNULL(CLI.DatosExtra, 0) AS DatosExtra, SUM(ABT.Importe) AS Importe ");
				consulta.append("FROM TransProd T ");
				consulta.append("	INNER JOIN Visita VIS ON T.VisitaClave = VIS.VisitaClave ");
				consulta.append("	INNER JOIN Cliente CLI ON VIS.ClienteClave = CLI.ClienteClave ");
				consulta.append("	LEFT JOIN AbnTrp ABT ON T.TransProdID = ABT.TransProdID ");
				consulta.append("WHERE T.Tipo = 1 AND T.TipoFase <> 0 AND T.CFVTipo = 1 ");
				consulta.append("	AND T.FechaCaptura BETWEEN '" + Generales.getFormatoFecha(fechas[0], "yyyy-MM-dd HH:mm:ss") +  "' AND '" + Generales.getUltimaHora(fechas[1], "yyyy-MM-dd HH:mm:ss") +  "' ");
				consulta.append("GROUP BY CLI.PublicoGeneral, CLI.DatosExtra ");
