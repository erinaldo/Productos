...
//				int ortcion  = getIntent().getIntExtra("orientacion", 0);
//				setRequestedOrientation(ortcion);
				break;
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (idReporte ==  Enumeradores.REPORTEA.ESTADO_DE_CUENTA){
            try {
                if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("ImprimirEdoCta")) {
                    if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("ImprimirEdoCta").equals("0")) {
                        return false;
                    }
                }
                else
    ...
    public boolean onOptionsItemSelected(MenuItem menuItem) {
		...
		if(menuItem == this.mnuImprimir){
			if (bImprimiendo) return false;
			if (Sesion.get(Sesion.Campo.Imprimiendo) == null || !Boolean.parseBoolean(Sesion.get(Campo.Imprimiendo).toString())) {
				try {
					Sesion.set(Sesion.Campo.Imprimiendo, true);
					imprimir();
				} catch (Exception e) {
					mostrarMensaje(e.getMessage());
					e.printStackTrace();
					Sesion.set(Sesion.Campo.Imprimiendo, false);
				}
			}
		}else{
			try{
