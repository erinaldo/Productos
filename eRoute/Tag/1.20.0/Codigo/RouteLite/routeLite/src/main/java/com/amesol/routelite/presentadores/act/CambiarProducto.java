...
public class CambiarProducto extends Presentador
{
	...
	public void iniciar()
	{
		try
		{
			...
			mVista.documentosPorConfirmar(transProdGeneral.TipoFase);
    ...
    public void guardarManejoLoteCaducidad(HashMap<String, Date> hmCaducidades) throws Exception
    {
        if(((ConfigParametro)Sesion.get(Campo.ConfigParametro)).existeParametro("CapturaCadLote", Sesion.get(Campo.ModuloMovDetalleClave).toString()) && ((ConfigParametro)Sesion.get(Campo.ConfigParametro)).get("CapturaCadLote", Sesion.get(Campo.ModuloMovDetalleClave).toString()).equals("1")) {
            ...
            while (it.hasNext()) {
				...
				TPDDatosExtra mTPDDatosExtras = new TPDDatosExtra();
				mTPDDatosExtras.TransProdID = getTransProd().TransProdID;
				mTPDDatosExtras.TransProdDetalleID = e.getKey();
				if (!mVista.getEsNuevo()){
					BDVend.recuperar(mTPDDatosExtras);
				}
				TransProdDetalle TPD = new TransProdDetalle();
				TPD.TransProdDetalleID = e.getKey();
				TPD.TransProdID = getTransProd().TransProdID;
				if(mTPDDatosExtras.Lote != null){
					Consultas2.ConsultasReportesDEL.actualizarLoteCaducidad(mTPDDatosExtras.TransProdID, mTPDDatosExtras.TransProdDetalleID, mTPDDatosExtras.Lote);
				}

				BDVend.recuperar(TPD);
				mTPDDatosExtras.Lote = Generales.getFormatoFecha(e.getValue(),"ddMMyyyy") + TPD.getProductoClave();

				LoteCaducidad LC = new LoteCaducidad();
				LC.Lote = mTPDDatosExtras.Lote;
				BDVend.recuperar(LC);
				if(!LC.isRecuperado())
				{
					LC.Lote = mTPDDatosExtras.Lote;
					LC.ProductoClave = TPD.ProductoClave;
					LC.Caducidad = e.getValue();
					LC.MFechaHora = Generales.getFechaHoraActual();
					LC.MUsuarioID = ((Usuario)Sesion.get(Campo.UsuarioActual)).USUId;
					LC.Enviado = false;
					BDVend.guardarInsertar(LC);
				}

				mTPDDatosExtras.MFechaHora = Generales.getFechaHoraActual();
				mTPDDatosExtras.MUsuarioID = ((Usuario)Sesion.get(Campo.UsuarioActual)).USUId;
				mTPDDatosExtras.Enviado = false;
				BDVend.guardarInsertar(mTPDDatosExtras);
...
/*	private void apartarInventarioEntrada(String productoClave, float cantidad)
	...
	public boolean agregarProductoUnidad(Producto producto, int TipoUnidad, Float Cantidad, short TipoMotivo, boolean busqueda, final StringBuilder refError)
	{
		try
		{
			...
			if (mVista.getCambioAutomatico())
			{
				...
				transProdDetalle = TransaccionesDetalle.ActualizarTipoMotivoTransaccion(transProdDetalle, TipoMotivo, moduloMovDetalle.TipoTransProd, transProdDetalle.Cantidad, false, true);
			}
			else
			{
		...
		try
		{
			...
			//{
				...
				if (mAccion.equals(Enumeradores.Acciones.ACCION_CAMBIAR_PRODUCTO_SALIDA))
				{
					...
					tpd = TransaccionesDetalle.Cambios.CalcularTotales(tpd, transProdGeneral.CadenaListasPrecios);
				...
				else
				{
					...
					res = TransaccionesDetalle.ActualizarTipoMotivoTransaccion(tpd, tpd.TipoMotivo, moduloMovDetalle.TipoTransProd, cantidad, true, true);
					...
					TransaccionesDetalle.ActualizarTipoMotivoTransaccion(tpd, tpd.TipoMotivo, moduloMovDetalle.TipoTransProd, cantidad, false, true);
	...
	public void eliminarDetalle(TransProdDetalle tpd)
	{
		try
		{
			...
			if (mAccion.equals(Enumeradores.Acciones.ACCION_CAMBIOS_PRODUCTO_ENTRADA))
			{
				...
				if (TransaccionesDetalle.ActualizarTipoMotivoTransaccion(tpd, tpd.TipoMotivo,  moduloMovDetalle.TipoTransProd, Float.valueOf(0), true, true) == null){
					return;
				}
                ...
                if(((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("CapturaCadLote", Sesion.get(Campo.ModuloMovDetalleClave).toString()) && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("CapturaCadLote", Sesion.get(Campo.ModuloMovDetalleClave).toString()).equals("1")) {
					...
					tpe.TransProdID = tpd.TransProdID;
					tpe.TransProdDetalleID = tpd.TransProdDetalleID;
                    ...
                    BDVend.recuperar(mlc);*///Se quito por el folio 5295
                    if (tpe.isRecuperado()){
						LoteCaducidad lc = new LoteCaducidad();
						lc.Lote = tpe.Lote;
						BDVend.recuperar(lc);
	...
	public boolean eliminarMovimiento()
	{
		try
		{
			...
			for (TransProdDetalle tpd : transProdGeneral.TransProdDetalle)
			{
				...
				TransaccionesDetalle.ActualizarTipoMotivoTransaccion(tpd, tpd.TipoMotivo,  moduloMovDetalle.TipoTransProd, Float.valueOf(0), true, true);
			}
			...
			for (TransProdDetalle tpd : transProdSalida.TransProdDetalle)
			{
                ...
                if(((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("CapturaCadLote", Sesion.get(Campo.ModuloMovDetalleClave).toString()) && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("CapturaCadLote", Sesion.get(Campo.ModuloMovDetalleClave).toString()).equals("1")) {
                    TPDDatosExtra tpe = new TPDDatosExtra();
					tpe.TransProdID = tpd.TransProdID;
					tpe.TransProdDetalleID = tpd.TransProdID;
					BDVend.recuperar(tpe);
					/*ManejoLotesCaducidad mlc = new ManejoLotesCaducidad();
                    mlc.TransProdID = tpd.TransProdID;
	...
	public boolean actualizarMotivo(TransProdDetalle tpd, short motivo)
	{
		try
		{
            ...
            TransProdDetalle res;
            // actualizar tipo motivo (salida)
            res = TransaccionesDetalle.ActualizarTipoMotivoTransaccion(tpd, tpd.TipoMotivo, moduloMovDetalle.TipoTransProd, Float.valueOf(0), true, true); //salida
            if (res == null) return false;
            // actualizar tipo motivo (entrada)
            tpd = TransaccionesDetalle.ActualizarTipoMotivoTransaccion(tpd, motivo,  moduloMovDetalle.TipoTransProd, tpd.Cantidad, false, true);
            ...
            for (TransProdDetalle tpd1 : getTransProd().TransProdDetalle)
            {
                ...
                {
                    // actualizar tipo motivo (salida)
                    TransProdDetalle otpd = TransaccionesDetalle.ActualizarTipoMotivoTransaccion(tpd1, tpd1.TipoMotivo,  moduloMovDetalle.TipoTransProd, Float.valueOf(0), true, true); //salida
                    // actualizar tipo motivo (entrada)
                    otpd = TransaccionesDetalle.ActualizarTipoMotivoTransaccion(tpd1, motivo, moduloMovDetalle.TipoTransProd, tpd1.Cantidad, false, true);
                    BDVend.guardarInsertar(otpd);
                    refrescar = true;
                }
            }
	...
	public String validarExistenciaCambiosPorConfirmar(short tipoMovimiento){

		StringBuilder refError = new StringBuilder();

		TransProd transProd = transProdGeneral;

		for (TransProdDetalle tpd : transProd.getTransProdDetalle()) {
			AtomicReference<Float> existencia = new AtomicReference<Float>();
			if(!Inventario.ValidarExistencia(tpd.ProductoClave, tpd.TipoUnidad, tpd.Cantidad, Float.valueOf(0), tipoMovimiento, (short) moduloMovDetalle.TipoTransProd, false, existencia, refError)){
				break;
			}
		}

		return refError.toString();
	}

	public String guardarCambiosPorConfirmar(short tipoMovimiento){
		StringBuilder refError = new StringBuilder();
		...
		try {
			for (TransProdDetalle tpd : transProd.getTransProdDetalle()) {
				Inventario.ActualizarInventario(tpd.ProductoClave, tpd.TipoUnidad, tpd.Cantidad, moduloMovDetalle.TipoTransProd, tipoMovimiento, ((Vendedor) Sesion.get(Campo.VendedorActual)).AlmacenID);
			}

			transProd.TipoFase = Enumeradores.TiposFasesDocto.CAPTURA;
			transProd.VisitaClave1 = ((Visita) Sesion.get(Campo.VisitaActual)).VisitaClave;
			transProd.DiaClave1 = ((Dia) Sesion.get(Campo.DiaActual)).DiaClave;
			...
			refError.append(ex.getMessage());
		}

		return refError.toString();
	}

	public String cancelarCambiosPorConfirmar(){
		StringBuilder refError = new StringBuilder();

		try {

			transProdGeneral.TipoFase = Enumeradores.TiposFasesDocto.CANCELADO;
			...
			transProdSalida.TipoFase = Enumeradores.TiposFasesDocto.CANCELADO;
			transProdSalida.VisitaClave1 = ((Visita) Sesion.get(Campo.VisitaActual)).VisitaClave;
			transProdSalida.MUsuarioID = ((Usuario) Sesion.get(Campo.UsuarioActual)).USUId;
			transProdSalida.MFechaHora = Generales.getFechaHoraActual();
			transProdSalida.Enviado = false;
			BDVend.guardarInsertar(transProdSalida);
	...
	public boolean esPendienteConfirmar(String transProdID){
		try{
			TransProd transProd = new TransProd();
			transProd.TransProdID = transProdID;
			BDVend.recuperar(transProd);

			if(transProd.TipoFase == 15){//Pendiente por confirmar
				return true;
			}
