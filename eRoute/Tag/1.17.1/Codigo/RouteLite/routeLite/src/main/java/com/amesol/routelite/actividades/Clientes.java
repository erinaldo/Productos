	...
	public static List<Map<String, String>> generarListaInfo(Cliente oCliente)
	{
		try
		{
			...
			while (domicilios.hasNext())
			{
				...
				//domicilio.Calle + " " + domicilio.Numero + " " + domicilio.NumInt + " " + domicilio.Localidad;
			...
			//////////////////
			m = new HashMap<String, String>();
			m.put("descripcion", "Localidad");
			m.put("valor", localidad);
			lista.add(m);

			if(!municipio.equals(localidad)){
				m = new HashMap<String, String>();
				m.put("descripcion", "Poblacion");
				m.put("valor", municipio);
				lista.add(m);
			}
			/////////////////////
