...
public class RegistroTiemposMuertos extends Vista implements IRegistroTiemposMuertos
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		try {
			TIMIdAnterior = Consultas.ConsultasTiempoMuerto.obtenerTiempoMuerto(((Dia) Sesion.get(Campo.DiaActual)).DiaClave, ((Ruta) Sesion.get(Campo.RutaActual)).RUTClave);
			if(TIMIdAnterior.length() > 0){
				tiempoMuerto.TIMId = TIMIdAnterior;
				BDVend.recuperar(tiempoMuerto);
				texto = (TextView) findViewById(R.id.lblFechaInicio);
				texto.setText(Generales.getFormatoFecha(tiempoMuerto.FechaHoraInicial,"dd/MM/yyyy hh:mm:ss"));
				iniciarCronometro();
				cambiarVista();
			}
		}catch(Exception ex){
	...
	public void iniciarCronometro()
	{
		try
		{
			...
			if(TIMIdAnterior.length() == 0) {
				...
				baseCronometro = SystemClock.elapsedRealtime();
			}else {
				baseCronometro = Generales.getFechaHoraActual().getTime() - tiempoMuerto.FechaHoraInicial.getTime();
				baseCronometro = SystemClock.elapsedRealtime() - baseCronometro;
			}
			cronometro.setBase(baseCronometro);
			cronometro.start();
	...
	protected void onPause(){
		...
		try {
			Button boton = (Button) findViewById(R.id.btnIniciar);
			if(boton.isEnabled() == false && TIMIdAnterior == "" && !terminar){
				SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
				TextView texto = (TextView) findViewById(R.id.lblFechaActual);
				tiempoMuerto.FechaHoraFinal = dateFormat.parse(texto.getText().toString());
				tiempoMuerto.MFechaHora = tiempoMuerto.FechaHoraInicial;
				tiempoMuerto.MUsuarioID = ((Usuario) Sesion.get(Campo.UsuarioActual)).USUId;
				BDVend.guardarInsertar(tiempoMuerto);
				BDVend.commit();
			}
