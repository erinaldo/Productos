...
public class ServidorComunicaciones extends Vista implements IServidorComunicaciones
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		if (getIntent().getExtras() != null)
		{
			...
			}else if(hm.containsKey("FolioDocto")){
				presenta = new ServirComunicaciones(this, accion,hm.get("FolioDocto").toString());
			}
			else if (hm.containsKey("Tablas")) //Recibir info
			{
                ...
                if (hm.containsKey("Cobranza") ){
                    cobranza = Boolean.parseBoolean(hm.get("Cobranza").toString());
                }
                if (hm.containsKey("Promociones") ){
                    promociones = Boolean.parseBoolean(hm.get("Promociones").toString());
                }
				if (hm.containsKey("Productos") ){
					productos = Boolean.parseBoolean(hm.get("Productos").toString());
				}
                boolean continuar = false;
                if (hm.containsKey("Continuar") ){
                    continuar = Boolean.parseBoolean(hm.get("Continuar").toString());
                }

				presenta = new ServirComunicaciones(continuar, this, accion, hm.get("Tablas").toString(), recargas, precios, cobranza, promociones, productos);
