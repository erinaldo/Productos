...
public class BaseDatos
{
    ...
    private boolean bHabilitarLog = false;
    private boolean bGuardarLog = false;
    private String sLogFileName = "";
    private LogFile logFile;
    private String sLogOrigen = "";
	...
	public synchronized void crearInstancia() throws Exception
	{
		...
		try
		{
            ...
            if (bHabilitarLog) {
                logFile = new LogFile(sLogFileName);
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | crearInstancia " + archivo.getName() );
            }
		}
	...
	public synchronized void crearInstanciaSinRelaciones() throws Exception
	{
		...
		try
		{
            ...
            if (bHabilitarLog) {
                logFile = new LogFile(sLogFileName);
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | crearInstanciaSinRelaciones");
            }
		}
		catch (SQLiteException ex)
		{
	...
	public void cerrar()
	{
        ...
        if (bHabilitarLog && bGuardarLog) {
            logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "cerrar");
        }
	}
	...
	public void COMInsertarSinHijos(String nombreTabla, String campos, ContentValues contenido) throws Exception
	{
		...
		try
		{
            ...
            if (bHabilitarLog && bGuardarLog) {
                String sContenido = "";
                for (String colName : contenido.keySet()) {
                    sContenido += colName + ": " + contenido.get(colName) + ",";
                }
                if (sContenido.length()>0)
                    sContenido = sContenido.substring(0, sContenido.length()-1);
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "COMInsertarSinHijos" + " | " + nombreTabla + "[ " + sContenido + "]");
            }
		...
		{
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "COMInsertarSinHijos-Exception" + " | " + e1.getMessage() + ": " + nombreTabla);
	...
	public void COMguardarSinHijos(Entidad entidad) throws Exception
	{
		...
		try
		{
			...
			{
				List<String> valores = new ArrayList<String>();
                String camposLlave = obtenerCamposWhereLlavePrincipal(entidad, valores);
                ...
                if (bHabilitarLog && bGuardarLog) {
                    ...
                    for (String llave : valores)
                        sLlaves += llave + ",";
                    if (sLlaves.length()>0)
                        sLlaves = sLlaves.substring(0, sLlaves.length()-1);

                    logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "COMguardarSinHijos" + " | " + nombreTabla + "[" + sContenido + "] - [" + camposLlave + ":" + sLlaves + "]");
                }

                getBD().update(nombreTabla, contenido, camposLlave, valores.toArray(new String[valores.size()]));
			}
		}
		catch (SQLiteConstraintException e1)
		{
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "COMguardarSinHijos-Exception" + " | " + e1.getMessage() + ": " + nombreTabla);
			throw new Exception(e1.getMessage() + " :" + nombreTabla);
		}
		guardarInsertarHijos(entidad);
	}

    public void soloInsertar(Entidad entidad) throws Exception
    {
        ...
        try
        {
            String campos = obtenerCamposInsertSinOriginal(entidad, contenido);
            if (contenido.size() > 0)
            {
                if (bHabilitarLog && bGuardarLog) {
                    String sContenido = "";
                    ...
                    if (sContenido.length()>0)
                        sContenido = sContenido.substring(0, sContenido.length()-1);
                    logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "soloInsertar" + " | " + nombreTabla + "[ " + sContenido + "]");
                }
        ...
        {
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "soloInsertar-Exception" + " | " + e1.getMessage() + ": " + nombreTabla);
	...
	public void guardarInsertar(Entidad entidad) throws Exception
	{
			...
			if (existe)
			{
				...
				if (contenido.size() > 0)
				{
                    ...
                    if (bHabilitarLog && bGuardarLog) {
                        ...
                        logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "guardarInsertar" + " | " + nombreTabla + "[" + sContenido + "] - [" + camposLlave + ":" + sLlaves + "]");
                    }
			...
			else
			{
				...
				if (contenido.size() > 0)
				{
                    if (bHabilitarLog && bGuardarLog) {
                        ...
                        if (sContenido.length()>0)
                            sContenido = sContenido.substring(0, sContenido.length()-1);
                        logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "guardarInsertar" + " | " + nombreTabla + "[ " + sContenido + "]");
                    }
		...
		{
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "guardarInsertar-Exception" + " | " + e1.getMessage() + ": " + nombreTabla);
    ...
    public void guardarInsertarSinOriginales(Entidad entidad) throws Exception
    {
        ...
        try
        {
            if (existe)
            {
                contenido = obtenerCamposUpdateSinOriginal(entidad);
                if (contenido.size() > 0)
                {
                    List<String> valores = new ArrayList<String>();
                    String camposLlave = obtenerCamposWhereLlavePrincipal(entidad, valores);
                    if (bHabilitarLog && bGuardarLog) {
                        ...
                        logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "guardarInsertarSinOriginales" + " | " + nombreTabla + "[" + sContenido + "] - [" + camposLlave + ":" + sLlaves + "]");
                    }
            ...
            else
            {
                ...
                if (contenido.size() > 0)
                {
                    if (bHabilitarLog && bGuardarLog) {
                        ...
                        if (sContenido.length()>0)
                            sContenido = sContenido.substring(0, sContenido.length()-1);
                        logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "guardarInsertarSinOriginales" + " | " + nombreTabla + "[ " + sContenido + "]");
                    }
        ...
        {
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "guardarInsertarSinOriginales-Exception" + " | " + e1.getMessage() + ": " + nombreTabla);
            throw new Exception(e1.getMessage() + " :" + nombreTabla);
        }
        guardarInsertarHijos(entidad);
    }
	...
	public void eliminar(Entidad entidad, String filtro) throws Exception
	{
		...
		{
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "eliminar" + " | " + nombreTabla + "[ " + filtro + "]");
			getBD().delete(nombreTabla, filtro, null);
		}
		catch (SQLiteConstraintException e1)
		{
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "eliminar-Exception" + " | " + e1.getMessage() + ": " + nombreTabla);
			throw new Exception(e1.getMessage() + " :" + nombreTabla);
		}
	}
	...
	public void eliminarTodo(Entidad entidad) throws Exception
	{
		...
		{
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "eliminarTodo" + " | " + nombreTabla);
			getBD().delete(nombreTabla, null, null);
		}
		catch (SQLiteConstraintException e1)
		{
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "eliminarTodo-Exception" + " | " + e1.getMessage() + ": " + nombreTabla);
			throw new Exception(e1.getMessage() + " :" + nombreTabla);
		}
	}
	...
	private void iniciarTransaccion() throws Exception
	{
		...
		{
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "iniciarTransaccion");
	...
	public void commit() throws Exception
	{
        ...
        if (bHabilitarLog && bGuardarLog)
            logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "TransaccionCommit");
	...
	public void rollback() throws Exception
	{
        ...
        if (bHabilitarLog && bGuardarLog)
            logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "TransaccionRollback");

		getBD().endTransaction();
		mEntidadesPendientes.clear();
	}

    private ContentValues obtenerCamposUpdateSinOriginal(Entidad entidad) throws IllegalArgumentException, IllegalAccessException
    {
        ...
        for (Field campo : entidad.getClass().getFields())
        {
            if ((campo.isAnnotationPresent(Campo.class)) || (campo.isAnnotationPresent(LlaveForanea.class)))
            {
                ...
                //{
					...
					/*No quitar los valores nulos, ya que a veces es necesario asignarlos
					 * Si te da error un nulo cambialo desde tu logica de afuera a otro valor*/
                if (valor == null){
                    valores.putNull(nombreCampo);
                }else{
...
//                }
    ...
    private String obtenerCamposInsertSinOriginal(Entidad entidad, ContentValues valores) throws IllegalArgumentException, IllegalAccessException
    {
        //HashMap<String, Object> ori = entidad.getOriginales();
        if (valores == null)
            valores = new ContentValues();
	...
	protected ISetDatos consultar(String consultaSQL, Object[] parametros) throws Exception
	{
		try
		{
            ...
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "consultar" + " | " + consultaSQL + (sParams != "" ? "[ " + sParams + "]" : "") );
		...
		{
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "COMInsertarSinHijos-Exception" + " | " + ex.getMessage() + ": " + consultaSQL);
			ex.printStackTrace();
			return null;
		}
	}
	...
	public String ejecutarEscalar(String consultaSQL, Object[] parametros) throws Exception
	{
        ...
        if (bHabilitarLog && bGuardarLog)
            logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "ejecutarEscalar" + " | " + consultaSQL +  (sParams != "" ? "[ " + sParams + "]" : "") );
	...
	{
        if (bHabilitarLog && bGuardarLog)
            logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "ejecutarEscalarDate" + " | " + consultaSQL);
	...
	{
        if (bHabilitarLog && bGuardarLog)
            logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "ejecutarEscalarInteger" + " | " + consultaSQL);
	...
	{
        if (bHabilitarLog && bGuardarLog)
            logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "ejecutarEscalarObject" + " | " + consultaSQL);
	...
	{
		try {
			if(comando.contains("DELETE") || comando.contains("UPDATE")) {
				iniciarTransaccion();
			}
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "ejecutarComando" + " | " + comando);
			getBD().execSQL(comando);
		}catch (Exception ex){
            if (bHabilitarLog && bGuardarLog)
                logFile.appendLog(Generales.getFechaHoraActualStr("hh:mm:ss") + " | " + sLogOrigen + " | " + "ejecutarComando-Exception" + " | " + ex.getMessage() + ": " + comando);
		}
	}

    public void setHabilitarLog(boolean bHabilitar){
        bHabilitarLog = bHabilitar;
    }

    public void setGuardarLog(boolean bGuardar){
        bGuardarLog = bGuardar;
    }

	public boolean getHabilitarLog(){
    ...
    public void setLogOrigen(String sOrigen){
        sLogOrigen = sOrigen;
    }

	public void writeInLog(String texto){
		try{
			if (bHabilitarLog && bGuardarLog){
				logFile.appendLog(texto + "\r\n");
			}
		}catch(Exception ex){
	...
	public String getDirHistoricosLog(){
		try{
			return logFile.getDirHistoricos();
		}catch(Exception ex){
			return "";
		}

	}
	public void enviarCorreoLog(Vista vista){
		try{
			if (bHabilitarLog) {
				logFile.enviarLogXCorreo(vista);
			}
		}catch(Exception ex){
