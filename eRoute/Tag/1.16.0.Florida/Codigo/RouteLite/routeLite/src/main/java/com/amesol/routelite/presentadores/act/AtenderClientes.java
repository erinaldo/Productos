...
public class AtenderClientes extends Presentador
{
    ...
    private Boolean ObtenerCoordenadasCliente() {
        try {
            ...
            if (cli.getCount() > 0) {
                ...
                cli.close();
                if(oParametros.get("LongitudCliente") != 0 && oParametros.get("LongitudCliente") != 0) {
    ...
    public void llevameACliente(){
        if (ObtenerCoordenadasCliente())
            mVista.navegarAPuntoGPS((Float) oParametros.get("LatitudCliente"), (Float) oParametros.get("LongitudCliente"));
    }
	...
	public String validarClienteAgendado(Cliente cliente)
	{
		try
		{
			...
			if (cliente.Clave.contains(((Ruta)Sesion.get(Campo.RutaActual)).RUTClave)) return "";
    ...
    public void validarTiempoCliente()
    {
        try
        {
            BDVend.setOrigenLog("AtenderClientes");
            BDVend.writeInLog("AtenderClientes:validarTiempoCliente") ;
