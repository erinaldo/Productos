                            ...
                            if (oProm.AplicarKit) {
                                ...
                                if (oProm.TipoRango == Enumeradores.Promocion.TipoRango.Cantidad) {
									if (dsPromocionProducto.get(oProm.PromocionClave).get(indice).Cantidad>0) {
										...
										//Se agrego ya que si la cantidad esta en un rango entre 0-1 (con decimales) lo manda a 0
										//por lo cual no entra en esta posterior validacion
										if(nCantKit == 0)
										{
			...
			if (!promocionActual.PermiteCascada) {
                ...
                if (bJerarquiaPorTipoAplicacion) {
                    ...
                    if (aPromocionProducto != null &&  nIndiceTemp < aPromocionProducto.size()){
                        do {
                            if (oPromociones.get(aPromocionProducto.get(nIndiceTemp).PromocionClave).Tipo != Enumeradores.Promocion.Tipo.Sobreprecio)
