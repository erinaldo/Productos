...
public class TransProd extends Entidad implements Parcelable
{
    ...
    @Hijos(tablaHijos = ProductoNegado.class)
    public List<ProductoNegado> ProductoNegado;

	public TransProd()
	{
		TransProdDetalle = new ArrayList<TransProdDetalle>();
        ProductoNegado = new ArrayList<ProductoNegado>();
	};

	private HashMap<Integer, Precio>  ListaPrecios;
	public String CadenaListasPrecios = "";
	public float DescuentoImpuestoCliente;
	public float DescuentoImpuestoVendedor;
	public TRPVtaAcreditada PedidoAdicional;
    public boolean AplicaSobreprecio;
	public boolean AplicoRedondeoImpuestos;
    public float TotalInicial;

	public void setListaPrecios(HashMap<Integer, Precio> listaPrecios){
		ListaPrecios = listaPrecios; 
		CadenaListasPrecios = "";
		for(Precio p : listaPrecios.values()) {
		    CadenaListasPrecios +="'" + p.PrecioClave + "',"  ;
		}
		if (CadenaListasPrecios.length()>0){
			CadenaListasPrecios = CadenaListasPrecios.substring(0, CadenaListasPrecios.length()-1);
		}
	}
	
	public HashMap<Integer, Precio> getListaPrecios(){
		return ListaPrecios;
	}
