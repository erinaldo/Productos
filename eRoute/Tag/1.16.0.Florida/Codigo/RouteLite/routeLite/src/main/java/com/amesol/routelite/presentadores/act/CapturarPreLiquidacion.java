...
public class CapturarPreLiquidacion extends Presentador
{
	...
	public void iniciar()
	{
		try
		{
			...
			{
				if (Consultas.ConsultaPreLiquidacion.obtenerVentasSinSaldar() == 0)
				{
				...
				else
				{
					...
					mVista.mostrarAdvertencia(Mensajes.get("E0121"));
				}
			}
			else
			{
	...
	public boolean validarTotal()
	{
		...
		try
		{
			...
			//while (mDetallePre.moveToNext())
			//{
			//	total = mDetallePre.getFloat("Total");
			//	if (mDetallePre.getString("Referencia") == null)
			//		total *= (Float.parseFloat(ValoresReferencia.getDescripcion("DENOMINA", mDetallePre.getString("TipoEfectivo"))));
			//	TotalPreLiquidacion = Generales.getRound( TotalPreLiquidacion - total, 2);
			//}
