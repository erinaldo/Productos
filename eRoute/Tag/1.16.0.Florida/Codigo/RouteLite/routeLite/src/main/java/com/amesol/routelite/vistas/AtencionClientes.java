...
public class AtencionClientes extends Vista implements IAtencionClientes {
    ...
    ProgressDialog progressDialogTimbre;
    Handler mHandlerEsperandoTimbre;

    public static boolean token_valido = false;
    ...
    private OnItemClickListener mSeleccion = new OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
            ...
            try {
                ...
                if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("ConfirmarVisitaCliente")) {
                    if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("ConfirmarVisitaCliente").equalsIgnoreCase("1")) {
                        Cursor cliente = (Cursor) parent.getItemAtPosition(position);
                        cliente.moveToPosition(position);
                        mostrarPreguntaSiNo(Mensajes.get("P0260").replace("$0$", cliente.getString(2)), 4);
                        return;
                    }
                }

                if(mPresenta.validarCodigoBarra()){
                    mostrarError(Mensajes.get("E0783"));
                    HabilitarPantalla(true);
                    return;
                }
            } catch (Exception ex) {
    ...
    boolean actualizandoFacturasTimbradas = false;
    boolean habilitarScanner = true;
    boolean vieneGps = false;
    ...
    private boolean actualizandoAgenda = false;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //Al reiniciar la pantalla, se reinicia la bandera token_valido,
        //ya que una vez aceptado un cliente, todos los demas podian entrar sin ser validados
    ...
    public void resultadoActividad(int requestCode, int resultCode, Intent data) {
        if (requestCode == Enumeradores.Solicitudes.SOLICITUD_GPS) {
                ...
                if (resultCode == Enumeradores.Resultados.RESULTADO_MAL) {
                    if (data != null) {
                        ...
                        if (mensajeError != null) {
                            ...
                            Sesion.set(Sesion.Campo.DetonarEnvioParcialAutomatico, "0");
                        }
                    }
                } else {
                    if (!actualizandoAgenda) {
                        try {
                            ...
                            } else if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("TimbradoCFDIMovil") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("TimbradoCFDIMovil").toString().equals("1")) {
                                solicitarFacturasTimbradas();
                            }
                        } catch (Exception ex) {
                            if (ex != null && ex.getMessage() != null) {
                                mostrarError(ex.getMessage());
                            } else {
                                mostrarError("Error al sincronizar");
                            }
                        }
                    }/*else{
                        mPresenta.mostrarClientes(mPresenta.obtenerVistaListado(), null);
                    }*/
                }
                enviando = false;
                return;
            }else if (actualizandoAgenda) {
                mPresenta.mostrarClientes(mPresenta.obtenerVistaListado(), null);
            }
            ...
            if (actualizandoConfirmacionPedidos) {
                if (resultCode == Enumeradores.Resultados.RESULTADO_MAL) {
                    if (data != null) {
                        ...
                        if (mensajeError != null) {
                            mostrarError(mensajeError);
                            //Esto da error cuando continuas con otra de las opciones, mejor se muestra el error y continua
                            //finalizar();
                            //iniciarActividad(IAtencionClientes.class, mensajeError);
                        }
                    }
            ...
            if (actualizandoFacturasTimbradas) {
                if (resultCode == Enumeradores.Resultados.RESULTADO_MAL) {
                    if (data != null) {
                        String mensajeError = (String) data.getExtras().getString("mensajeIniciar");
                        if (mensajeError != null) {
                ...
                actualizandoFacturasTimbradas = false;
                return;
            }
    ...
    public void solicitarFacturasTimbradas(){
        try{
            long espera = 10000;
            ...
            mHandlerEsperandoTimbre= new Handler();
            progressDialogTimbre = new ProgressDialog(this);
            progressDialogTimbre.setMessage(Mensajes.get("I0325"));
            progressDialogTimbre.setProgressStyle(ProgressDialog.STYLE_SPINNER);
            progressDialogTimbre.setCancelable(false);
            progressDialogTimbre.show();
            progressDialogTimbre.setOnDismissListener(new DialogInterface.OnDismissListener() {
                @Override
                public void onDismiss(DialogInterface dialog) {
                    mHandlerEsperandoTimbre.removeCallbacksAndMessages(null);
                }
            });
            mHandlerEsperandoTimbre.postDelayed(new Runnable()
            {
                @Override
                public void run() {
                    progressDialogTimbre.dismiss();
                    HashMap<String, String> parametros = new HashMap<String, String>();
                    String listadoFacturas = "";
                    try{
                        listadoFacturas = Consultas.ConsultasTransProd.obtenerFacturasConTimbradoPendiente("");
                    }catch (Exception ex){
                        if (ex!=null && ex.getMessage()!= null){
                            mostrarError(ex.getMessage());
                        }else{
                    ...
                    if (listadoFacturas.length() > 0) {
                        ...
                        actualizandoFacturasTimbradas  = true;
                    } else {
            ...
            if (ex!= null && ex.getMessage()!= null){
                ...
                mostrarError("Error al solicitar facturas timbradas");
            }
        }
    ...
    public void respuestaMensaje(RespuestaMsg respuesta, int tipoMensaje) {
        ...
        // autorizacion
        {
            ...
            if (respuesta.equals(RespuestaMsg.Si)) {
                ...
                //Si el cliente es público general y no se logro leer el GPS permite entrar a la visita
                //sin token
                if (((Cliente)Sesion.get(Campo.ClienteActual)).PublicoGeneral == 1){
                    mPresenta.SeleccionVisita();
                }else {
                    mPresenta.configuracionContrasena();
                }
                // iniciarActividad(ISeleccionVisita.class,
                // Enumeradores.Acciones.ACCION_VISITAR_CLIENTE_VISITA, null,
                // false);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        try {
            ...
            if (Sesion.get(Campo.DetonarEnvioParcialAutomatico).toString()=="1"){
                onOptionsItemSelected(menu.findItem(Enumeradores.ACTMAC.ENVIO_PARCIAL));
            }else{
                Sesion.set(Sesion.Campo.DetonarEnvioParcialAutomatico, "0");
            }
        } catch (Exception e) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case Enumeradores.ACTMAC.ENVIO_PARCIAL:
                ...
                try{
                    if ((((Ruta) Sesion.get(Campo.RutaActual)).Inventario && Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.PREVENTA) || (((MOTConfiguracion) Sesion.get(Campo.MOTConfiguracion)).get("SincronizarConfirmPedido").equals("1")) ||  (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("TimbradoCFDIMovil") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("TimbradoCFDIMovil").toString().equals("1"))) {
            ...
            case Enumeradores.ACTMAC.IMPRIMIR_CLIENTES:
                ...
                return true;
            case Enumeradores.ACTMAC.ACTUALIZAR_AGENDA:
                actualizandoAgenda = true;
                try{
//                    if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("ActualizarClientesAgenda"))
//                        if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("ActualizarClientesAgenda").toString().equals("1"))
//                            iniciarActividadConResultado(IServidorComunicaciones.class, Enumeradores.Solicitudes.SOLICITUD_SERVIDOR_COMUNICACIONES, Enumeradores.Acciones.ACCION_RECIBIR_ACT_CLIENTES_AGENDA, null);
                    iniciarActividadConResultado(IServidorComunicaciones.class, Enumeradores.Solicitudes.SOLICITUD_SERVIDOR_COMUNICACIONES, Enumeradores.Acciones.ACCION_RECIBIR_INFO_CLIENTES_NUEVOS, null);
                }catch(Exception e){
                }
                return true;
            default:
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
        ...
        if (v instanceof ListView && !optionsFlag) {
            ...
            menu.getItem(10).setTitle(Mensajes.get("XClientesPorRecolectar"));
            ...
            switch (item.getItemId()) {
               ...
               /*checarNFC();
                ...
                case R.id.novisitadosrec:
                    ...
                    return true;
                case R.id.porrecolectar:
                    mPresenta.mostrarClientes(VistaClientes.VISTA_CLIENTES_POR_RECOLECTAR, null);
                    return true;
                default:
                    return super.onOptionsItemSelected(item);
            }
    ...
    @Override
    public void vieneGps(boolean valor) {
        vieneGps = valor;
    }
    ...
    public void mostrarClientes(ISetDatos clientes, int vista) {
        ...
        switch (vista) {
            ...
            case VistaClientes.VISTA_CLIENTES_POR_RECOLECTAR:
                lblTitulo.setText(Mensajes.get("XClientesPorRecolectar") + ": " + clientes.getCount());
                break;
            default:
                break;
        }
            ...
            if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO && clientesPedido.equals("1")) {
                ...
                if (cClientes.getCount() != 0) {
                    ...
                    while (cClientes.moveToNext()) {
                        ClientesVistaReparto nuevo = new ClientesVistaReparto(cClientes.getString(cClientes.getColumnIndex(SearchManager.SUGGEST_COLUMN_TEXT_1)), cClientes.getString(cClientes.getColumnIndex("Contacto")), cClientes.getString(cClientes.getColumnIndex(SearchManager.SUGGEST_COLUMN_TEXT_2)), cClientes.getString(cClientes.getColumnIndex("HorarioVisita")), cClientes.getInt(cClientes.getColumnIndex("PedidosXSurtir")), cClientes.getString(cClientes.getColumnIndex(SearchManager.SUGGEST_COLUMN_INTENT_DATA)));
                        cli.add(nuevo);
                    }
                    ClientesVistaReparto[] clientesReparto = cli.toArray(new ClientesVistaReparto[cli.size()]);

                    ClientesAdapter adpt = new ClientesAdapter(this, R.layout.lista_clientes, clientesReparto);
                    lista_4.setAdapter(adpt);
                } else {
                    lista_4.setAdapter(null);
                }
            } else {
                SimpleCursorAdapter adapter = new SimpleCursorAdapter(this, R.layout.lista_clientes, cClientes, new String[]
                        {SearchManager.SUGGEST_COLUMN_TEXT_1, "Contacto", SearchManager.SUGGEST_COLUMN_TEXT_2, "HorarioVisita"}, new int[]
                        {R.id.texto1, R.id.texto2, R.id.texto3, R.id.texto4});
    ...
    private static class ClientesVistaReparto {
        ...
        public ClientesVistaReparto(String texto1, String texto2, String texto3, String texto4, int pedidoXsurtir, String clienteClave) {
            _texto1 = texto1;
            _texto2 = texto2;
            _texto3 = texto3;
            _texto4 = texto4;
