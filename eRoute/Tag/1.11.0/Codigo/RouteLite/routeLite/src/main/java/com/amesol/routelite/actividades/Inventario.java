...
public class Inventario
{
	...
	public static boolean ValidarExistencia(String productoClave, int tipoUnidad, Float cantidad, Float cantidadAnterior, short tipoMovimiento, short tipoTransProd, boolean cancelacion, AtomicReference<Float> refExistencia, final StringBuilder error)
	{
		...
		{
			if (tipoTransProd == TiposTransProd.PEDIDO || tipoTransProd == TiposTransProd.AJUSTES || tipoTransProd == TiposTransProd.DESCARGAS || tipoTransProd == TiposTransProd.DEVOLUCIONES_MANUALES || tipoTransProd == TiposTransProd.CAMBIOS || tipoTransProd == TiposTransProd.CARGAS || tipoTransProd == TiposTransProd.VENTA_CONSIGNACION)
			{
	...
	public static void CrearMovimientoAutomatico(int tipoEnvioMovimientosAutomaticos ) throws Exception {
		try{
			...
			if(tipoEnvioMovimientosAutomaticos == TipoEnvioMovimientosAutomaticos.DescargaAutomatica) //confirmar el folio solo si hay detalle
				Folios.confirmar(15);
	...
	public static boolean ActualizarInventario(String productoClave, int tipoUnidad, float cantidad, int tipoTransProd, int tipoMovimiento, String almacenID, String grupoMotivo, boolean cancelacion, String clienteClave, float prestamoVendido, boolean surtirPedido, int tipoFase) throws Exception
	{
		...
		if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.VENTA || (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.REPARTO && (tipoTransProd == TiposTransProd.CARGAS || tipoTransProd == TiposTransProd.CAMBIOS || tipoTransProd == TiposTransProd.DESCARGAS || tipoTransProd == TiposTransProd.DEVOLUCIONES_CLIENTE || tipoTransProd == Enumeradores.TiposTransProd.DEVOLUCIONES_MANUALES|| tipoTransProd == TiposTransProd.VENTA_CONSIGNACION || (tipoTransProd == TiposTransProd.PEDIDO && !surtirPedido ) || (tipoTransProd == TiposTransProd.NO_DEFINIDO && clienteClave.equals("") && tipoFase == 0 && !grupoMotivo.equals("")) )))
		{
			...
			{
				if (!cancelacion && (tipoTransProd == TiposTransProd.PEDIDO || tipoTransProd == TiposTransProd.CAMBIOS || tipoTransProd == TiposTransProd.AJUSTES || tipoTransProd == TiposTransProd.DESCARGAS || tipoTransProd == TiposTransProd.VENTA_CONSIGNACION))
					tipoMovInventario = TiposMovInventario.SALIDA_DISPONIBLE;
				else if (cancelacion && (tipoTransProd == TiposTransProd.PEDIDO || tipoTransProd == TiposTransProd.CAMBIOS || tipoTransProd == TiposTransProd.AJUSTES || tipoTransProd == TiposTransProd.DESCARGAS || tipoTransProd == TiposTransProd.VENTA_CONSIGNACION))
					tipoMovInventario = TiposMovInventario.ENTRADA_DISPONIBLE;
			...
			while (dtProductoDetalle.moveToNext()){
				...
				if(dtProductoDetalle.getString("ProductoDetClave").equalsIgnoreCase(productoClave)){
					if(cantidadxFactor - prestamoVendido > 0){
                        ...
                        //producto.ProductoClave = inv.ProductoClave;
