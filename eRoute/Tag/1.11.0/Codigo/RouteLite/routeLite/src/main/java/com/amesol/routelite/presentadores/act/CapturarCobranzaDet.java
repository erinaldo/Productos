...
public class CapturarCobranzaDet extends Presentador
{
	...
	public void generarCobranza() throws Exception
	{
		...
		ValorReferencia[] valoresConFecha = ValoresReferencia.getValores("PAGO", "CP");
		for (int i = 0; i < valoresConFecha.length; i++)
			tiposPagoConFecha.add(Integer.parseInt(valoresConFecha[i].getVavclave()));

		valoresConFecha = ValoresReferencia.getValores("PAGO", "TR");
		for (int i = 0; i < valoresConFecha.length; i++)
			tiposPagoConFecha.add(Integer.parseInt(valoresConFecha[i].getVavclave()));

		valoresConFecha = ValoresReferencia.getValores("PAGO", "FP");
		for (int i = 0; i < valoresConFecha.length; i++)
			tiposPagoConFecha.add(Integer.parseInt(valoresConFecha[i].getVavclave()));
		...
		valores.close();
		mVista.capturarCobranzaDet(tiposPagoConFecha);
	...
	public void guardarDetallePago(DatosAbnDetalle datosABD) throws Exception
	{
		...
		Cobranza.VistaDetalle oVistaDet = new Cobranza.VistaDetalle(KeyGen.getId(), tipoPago, pago, monedaId, moneda, importe, tipoBanco, banco, referencia, fechaCheque, observaciones);
    ...
    public ArrayList<vistaDesgloseProntoPago> obtenerDesgloseProntoPago() throws Exception
    {
        ...
        {
			descPP = (dsDocumentos.getFloat("Saldo") * dsDocumentos.getFloat("Porcentaje"))/ 100;
            desgloseProntoPago.add(new vistaDesgloseProntoPago(dsDocumentos.getString("Folio"),
                    ...
                    Generales.getFormatoDecimal(descPP, "$#,##0.00"),
                    Generales.getFormatoDecimal(dsDocumentos.getFloat("Saldo") - descPP, "$#,##0.00")));
        }
