...
public final class Consultas
{
		...
		public static SortedMap<String,Esquema> obtenerEsquemasJerarquia(ServicesCentral.TiposEsquemas Tipo, String filtro) throws Exception
		{
				...
				while (datos.moveToNext())
				{
					...
					if (datos.getInt("Nivel") == 1){
						...
						//if (hm.containsKey(datos.getString("EsquemaIdPadre"))){
	...
	public static final class ConsultasImpuesto
	{
		...
		{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT  I.Abreviatura || ' ' || CASE WHEN I.TipoValor = 1 THEN  Round(IMV.Valor,2) || '%' ELSE Round(IMV.Valor,2) END as Abreviatura, SUM(TPDI.ImpuestoImp) as ImpuestoImp FROM TPDImpuesto TPDI inner join TransProd TP on TP.TransProdID = TPDI.TransProdID ").
...
//				sConsulta.append("AND Visita.DiaClave='" + diaClave + "'");
        ...
        public static ISetDatos obtenerDiferenciaEnvase(String sTransprodIds) throws Exception {
            StringBuilder consulta = new StringBuilder();
            consulta.append("select t.ProductoDetClave, t.TipoUnidad, sum(VentaTotalLiquido) as VentaTotalLiquido, sum(VentaTotalEnvase) as VentaTotalEnvase, (sum(VentaTotalLiquido)-sum(VentaTotalEnvase)) as Diferencia from ( ");
            consulta.append("select pd.ProductoDetClave, pu.PRUTipoUnidad as TipoUnidad, SUM(td.Cantidad) as VentaTotalLiquido, 0 as VentaTotalEnvase ");
            ...
            consulta.append("union all ");
            consulta.append("select pd.ProductoDetClave,td.TipoUnidad, 0 as VentaTotalLiquido, SUM(Cantidad) as VentaTotalEnvase ");
            consulta.append("from TransProdDetalle td ");
            consulta.append("inner join ProductoDetalle pd on td.ProductoClave=pd.ProductoClave and td.TipoUnidad=pd.PRUTipoUnidad and td.ProductoClave=pd.ProductoDetClave and pd.Prestamo=1 ");
            consulta.append("where td.TransProdID in (" + sTransprodIds + ") ");
