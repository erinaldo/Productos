...
public class CobranzaDetAdapter extends ArrayAdapter<Cobranza.VistaDetalle>
{
	...
	public CobranzaDetAdapter(Context context, int textViewResourceId, CapturarCobranzaDet mPresenta, ArrayList<Cobranza.VistaDetalle> objects, LinkedList<Cobranza.VistaSpinner> pagos, LinkedList<Cobranza.VistaSpinner> monedas, LinkedList<Cobranza.VistaSpinner> bancos, ArrayList<Integer> TiposPagoConFecha, boolean habilitar, boolean mostrarCuenta)
	{
	...
	public View getView(int position, View convertView, ViewGroup parent)
	{
		...
		if (convertView == null)
		{
			...
			else
			{
				...
				holder.spnMoneda.setOnItemSelectedListener(new OnItemSelectedListener()
				{
					...
					public void onItemSelected(AdapterView<?> parent, View view, int pos, long id)
					{
						...
						{
							if (mPresenta.validarDetalleInexistente(oDet.getTipoPago(), ((Cobranza.VistaSpinner) parent.getItemAtPosition(pos)).getId(), oDet.getTipoBanco(), oDet.getReferencia(), oDet.getObservaciones()))
							{
			...
			else
			{
				...
				holder.spnBanco.setOnItemSelectedListener(new OnItemSelectedListener()
				{
					...
					public void onItemSelected(AdapterView<?> parent, View view, int pos, long id)
					{
						...
						{
							if (mPresenta.validarDetalleInexistente(oDet.getTipoPago(), oDet.getMonedaId(), Integer.parseInt(((Cobranza.VistaSpinner) parent.getItemAtPosition(pos)).getId()), oDet.getReferencia(),oDet.getObservaciones()))
							{
            ...
            if (!mostrarCuenta){
                TableRow tableRowCta = (TableRow) fila.findViewById(R.id.tableRowCta);
                tableRowCta.setVisibility(View.GONE);
            }
            else {
                holder.lblCuenta = (TextView) fila.findViewById(R.id.lblCuenta);
                holder.lblCuenta.setText(Mensajes.get("XNoCuenta"));
                holder.txtCuenta = (EditText) fila.findViewById(R.id.txtCuenta);
                holder.txtCuenta.setTag(position);
                if (!habilitar)
                    holder.txtCuenta.setEnabled(false);
                else
                {
                    holder.txtCuenta.setOnFocusChangeListener(new OnFocusChangeListener()
                    {
                        ...
                        public void onFocusChange(View v, boolean hasFocus)
                        {
                            if (!hasFocus)
                            {
                                ...
                                {
                                    if (mPresenta.validarDetalleInexistente(oDet.getTipoPago(), oDet.getMonedaId(), oDet.getTipoBanco(), ((EditText) v).getText().toString(),oDet.getObservaciones()))
                                    {
            ...
            holder.lblObservaciones = (TextView) fila.findViewById(R.id.lblObservaciones);
            holder.lblObservaciones.setText(Mensajes.get("XObservaciones"));
            holder.txtObservaciones = (EditText) fila.findViewById(R.id.txtObservaciones);
            holder.txtObservaciones.setTag(position);
            if (!habilitar)
                holder.txtObservaciones.setEnabled(false);
            else {
                holder.txtObservaciones.setOnFocusChangeListener(new OnFocusChangeListener() {
                    ...
                    public void onFocusChange(View v, boolean hasFocus) {
                        if (!hasFocus) {
                            ...
                            if (oDet.getObservaciones() != null && !oDet.getObservaciones().equals(((EditText) v).getText().toString())) {
                                if (mPresenta.validarDetalleInexistente(oDet.getTipoPago(), oDet.getMonedaId(), oDet.getTipoBanco(), oDet.getReferencia(), ((EditText) v).getText().toString())) {
                                    oDet.setObservaciones(((EditText) v).getText().toString());
                                } else {
                                    holder.txtObservaciones.setText(oDet.getObservaciones());
                                }
                            }
                        }
                    }
			...
			if (habilitar)
			{
				holder.txtFechaCheque.setOnClickListener(new OnClickListener()
				{
					...
					public void onClick(View v)
					{
						...
						if (tiposPagoConFecha.contains(oDet.getTipoPago()))
						{
							detalleSel = oDet;
		...
		try
		{
			...
			if (mostrarCuenta) {
				holder.txtCuenta.setText(detalle.getCuenta());
				if (habilitar) {
					if (detalle.getTipoPago() != 1) {
						holder.txtCuenta.setEnabled(true);
					} else
						holder.txtCuenta.setEnabled(false);
				}
			}
            ...
            holder.txtObservaciones.setText(detalle.getObservaciones());
            holder.txtObservaciones.setEnabled(habilitar);
