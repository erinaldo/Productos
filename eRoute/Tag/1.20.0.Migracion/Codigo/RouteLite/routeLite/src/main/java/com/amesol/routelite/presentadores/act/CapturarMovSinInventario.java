...
public class CapturarMovSinInventario extends Presentador
{
	...
	private void generarDetalle(String productoClave, int TipoUnidad, Float Cantidad, Short Motivo)
	{
		try
		{
			...
			StringBuilder byRefMensaje = new StringBuilder();
			transProdGeneral = Transacciones.Pedidos.ActualizarGenerarMovSinInv(getTransaccionesIds(), moduloMovDetalle, byRefMensaje, Motivo);
			...
			TransProdDetalle transProdDetalle = TransaccionesDetalle.Pedidos.GenerarDetalleMovsGenerales(transProdGeneral.TransProdID, productoClave, TipoUnidad, Cantidad, Motivo, transProdGeneral.CadenaListasPrecios, byRefError);
