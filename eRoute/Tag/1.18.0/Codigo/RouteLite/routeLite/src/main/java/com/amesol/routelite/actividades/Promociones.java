...
//Esta clase no se usa...se sustituye por Promociones2
	...
	public boolean Preparar() throws Exception
	{
		...
		while (oDetalles.hasNext()){
			...
			if (oTPD.Promocion == 1 && oTPD.Total != 0){
				...
				float nPrecio = ListaPrecio.BuscarPrecio(oTPD.ProductoClave, (short) oTPD.TipoUnidad, oTransProd.CadenaListasPrecios);
				if (nPrecio >= 0){				
					oTPD.Precio = nPrecio;
					if(oTPD.Cantidad == 0){
		...
		private boolean OtorgarPrecio() throws Exception{
			...
			while (oDetalles.hasNext()){
				...
				if (oProducto.ProductoClave.equals(oTPD.ProductoClave) && oTPD.Promocion != 2 && oTPD.getSubTotal() > 0){
					//Se manda solo la lista perteneciente a la lista que sustituye el precio, porque no hay jerarquia en este caso
