	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		lv.setOnItemClickListener(new OnItemClickListener()  //este metodo se convoca al ser precionado un elemento de la lista
		{
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id)
			{
                ...
                intent.putExtra("puntos", item.getPuntos());
	...
	private ArrayList<ItemEncuesta> obtenerItems() //en este metodo cargamos los datos de la bd a la lista
	{
		ArrayList<ItemEncuesta> items = new ArrayList<ItemEncuesta>();
		 ...
		 for(int x = 0; x <= prueba.length-1; x++)
		 {
			 ...
			 id = prueba[x].getString("CENClave").toString();
			 descripcion = prueba[x].getString("Descripcion").toString();
			 imagen = prueba[x].getBlob(2);
             puntos = prueba[x].getInt("Puntos");
			 
			 if(prueba[x].getColumnCount() == 4)
			 {
				 fase = "Sin contestar";
				 horaFin = "";
			 }
			 ...
			 {
				 fase = sEncuesta.faseString(prueba[x].getInt("Fase"));
				 horaFin = sEncuesta.fechaString(prueba[x].getDate("HoraFin"));
			 }
			 

			 items.add(new ItemEncuesta(1, imagen, descripcion, fase, horaFin, id, puntos));
