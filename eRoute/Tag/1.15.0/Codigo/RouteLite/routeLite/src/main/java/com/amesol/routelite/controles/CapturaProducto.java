...
public class CapturaProducto extends LinearLayout {
	...
	// ************************************************ constructores
	// ************************************************
	public CapturaProducto(Context context, AttributeSet attrs) throws Exception {
		super(context, attrs);
		inicializar();
	}

	public CapturaProducto(Context context) throws Exception
	{
		super(context);
		inicializar();
	}
	...
	private android.view.View.OnClickListener mBuscarProducto = new android.view.View.OnClickListener()
	{
		...
		{
			String Codigo = obtenerCodigoValido(txtScanner.getTexto());
			Regexp = true;
            if(Codigo.length() != 0 && ((CONHist) Sesion.get(Campo.CONHist)).get("TipoClaveProducto").toString().equals("2")){
                ...
                buscarProducto(StringUtils.leftPad(Codigo,DigitoClaveProd,"0"), false);
            }else {
                buscarProducto(Codigo, false);
            }
		}
	};
					...
					if(Regexp && BusquedaEsp){
						...
						if(Arrays.asList(Actividades).contains(String.valueOf(tipoModulo))){
							...
							if(AlInicio)
								...
								RegexProducto = "^"+RegexProducto+cadenaBusqueda.substring(0,AlMedio).toUpperCase()+"";
								Expresion = "%"+Expresion+cadenaBusqueda.substring(0,AlMedio).toUpperCase()+"%";
							}

							int CampoActual = 0;
							if (!cadenaBusqueda.equals("")){
								if(Arrays.asList(Campos).contains("a") || Arrays.asList(Campos).contains("b") || Arrays.asList(Campos).contains("c") || Arrays.asList(Campos).contains("d")){
									filtroProducto = "( ";
									if(Arrays.asList(Campos).contains("a")){
										if(CampoActual>0)
											filtroProducto += " or ";
										filtroProducto += " Producto.Id like '" + Expresion + "'";
										CampoActual += 1;
									}
									if(Arrays.asList(Campos).contains("b")){
										if(CampoActual>0)
											filtroProducto += " or ";
										filtroProducto += " Producto.ProductoClave like '" + Expresion + "'";
										CampoActual += 1;
									}
									if(Arrays.asList(Campos).contains("c")){
										if(CampoActual>0)
											filtroProducto += " or ";
										filtroProducto += " Producto.Nombre like '" + Expresion + "'";
										CampoActual += 1;
									}
									if(Arrays.asList(Campos).contains("d")){
										if(CampoActual>0)
											filtroProducto += " or ";
										filtroProducto += " Producto.NombreLargo like '" + Expresion + "'";
									}
									filtroProducto += " )";
								}
								ExpRegex = Consultas.ConsultasProducto.ObtenerProductosRegex(filtroProducto, 1);

								if(Arrays.asList(Campos).contains("e") && (ExpRegex == null || ExpRegex.getCount() == 0)){
									filtroProducto = " ProductoUnidad.CodigoBarras like '" + Expresion + "'";
									ExpRegex = Consultas.ConsultasProducto.ObtenerProductosRegex(filtroProducto, 2);
								}

								String ProductosPatron = "(";
								if(ExpRegex.getCount() > 0){
									while(ExpRegex.moveToNext()){
										if(!AlInicio && !AlFinal){
											Pattern Pat = Pattern.compile(RegexProducto);
											String Campo = ExpRegex.getString("CodigoBarras").toString().trim();
											if(Pat.matcher(Campo).matches()){
												ProductosPatron+= "'"+ExpRegex.getString("ProductoClave")+"', ";
											}
										}
										else{
											ProductosPatron+= "'"+ExpRegex.getString("ProductoClave")+"',";
									...
									ProductosPatron = ProductosPatron.substring(0, ProductosPatron.length() - 1);
									ProductosPatron += ")";
									oProducto = Consultas.ConsultasProducto.obtenerProductoIdentico(ProductosPatron, RegexProducto);
								}
							}
						}
						else
	...
	public boolean validarCaptura()
	{
		...
		if (manejoDobleUnidad){
			...
			if(txtCantidad.getText().toString().length() > 17){
				mVista.mostrarError(Mensajes.get("E0623", Mensajes.get("XCantidad"), Mensajes.get("XNumero")));
				txtCantidad.requestFocus();
				return false;
			}
