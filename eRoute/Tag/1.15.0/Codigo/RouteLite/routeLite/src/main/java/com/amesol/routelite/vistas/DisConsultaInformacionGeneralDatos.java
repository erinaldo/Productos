...
public class DisConsultaInformacionGeneralDatos extends Vista implements IDisInformacionGeneralDatos {
    ...
    private ArrayList<ListaDatos> obtenerItems(ISetDatos datos) {
       ...
       if(datos.moveToNext()){
           ...
           informacion = datos.getString("EfectividadCompra") +"% ("+ datos.getString("FrecuenciaCompra")+ " de cada "+ datos.getString("TotalVisitas") +")" ;
           items.add(new ListaDatos(dato, informacion));
           ...
           informacion = "Descuento actual " + datos.getString("DescActual") + " Descuento Propuesto " + datos.getString("DescProp");
           items.add(new ListaDatos(dato, informacion));
        }
