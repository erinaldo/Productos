...
public class SeleccionarPedido extends Presentador
{
	...
	public void iniciar()
	{
		try
		{
			...
			try
			{
				if (mAccion.equals(com.amesol.routelite.presentadores.Enumeradores.Acciones.ACCION_MOSTRAR_PEDIDOS)|| mAccion.equals(Acciones.ACCION_MOSTRAR_PEDIDOS_POR_SURTIR )){
					if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.VENTA){
						ValidarVencimientoVenta();
					}else{
						if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("AplicarValidacionVencimientoVenta",((Vendedor)Sesion.get(Campo.VendedorActual)).MCNClave ) && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("AplicarValidacionVencimientoVenta", ((Vendedor)Sesion.get(Campo.VendedorActual)).MCNClave).equals("1")) {
							ValidarVencimientoVenta();
						}
					}
				}
			...
			else
			{
				...
				if (mAccion.equals(Acciones.ACCION_MOSTRAR_PEDIDOS))
                    ...
                    if (m.get("VentaReparto").toString().equals("0")) {
                        mVista.setResultado(Enumeradores.Resultados.RESULTADO_MAL, Mensajes.get("I0187"));
                        mVista.finalizar();
                        return;
                    }
                    else
                    ...
                    mVista.iniciarActividadConResultado(ICapturaCanje.class, 0, Acciones.ACCION_CAPTURA_CANJE);
	...
	public void modificar(SeleccionarPedido.VistaPedidos pedido)
	{
		if (mAccion.equals(Acciones.ACCION_MOSTRAR_PEDIDOS) || mAccion.equals(Acciones.ACCION_MOSTRAR_PEDIDOS_POR_SURTIR))
		{
            ...
            End If*/
					...
					oTRP.TransProdID = pedido.getTransprodID();
					if  (oTRP.Tipo == Enumeradores.TiposTransProd.PEDIDO && m.get("RecAutoEnvVenta").equals("1") ) {
						if ((Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.VENTA) || (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.REPARTO && oTRP.TipoFase == TiposFasesDocto.SURTIDO)) {
							try {
								TransProd oDev = new TransProd();
								oDev.TransProdID = oTRP.DevolucionID;
								BDVend.recuperar(oDev);
								String grupoDev = ValoresReferencia.getValor("TRPMOT", String.valueOf(oDev.TipoMotivo)).getGrupo();
								Consultas.ConsultasInventario.ValidarInventarioRecoleccionAutomatica(oTRP, oTRP.DevolucionID, (grupoDev.toUpperCase().equals("RECOLECCIÓN") ? "Venta" : "NoVenta"));
							} catch (Exception ex) {
								mVista.mostrarError(Mensajes.get("E0789").replace("$0$", ex.getMessage()));
								return;
							}
						}
					}
		...
		else if (mAccion.equals(Acciones.ACCION_MOSTRAR_DEVOLUCIONES))
		{
			...
			HashMap<String, String> oParametros = new HashMap<String, String>();
			oParametros.put("TransProdId", pedido.getTransprodID());
            oParametros.put("Recoleccion", pedido.getExtras());
		...
		else if (mAccion.equals(Acciones.ACCION_CAPTURAR_CARGAS))
		{
            ...
            if(!registroTransferido(pedido.getTransprodID(),false)){
                ...
                oParametros.put("TransProdId", pedido.getTransprodID());
                oParametros.put("Modificar", "true");
                mVista.iniciarActividadConResultado(ICapturaCanje.class, 0, Acciones.ACCION_CAPTURA_CANJE, oParametros);
                mVista.finalizar();
            }
        }
	}

	public void confirmar(SeleccionarPedido.VistaPedidos pedido){
		try{
			if (mAccion.equals(Enumeradores.Acciones.ACCION_CAPTURAR_CARGAS) || mAccion.equals(Acciones.ACCION_CAPTURAR_DESCARGA) || mAccion.equals(Acciones.ACCION_CAPTURAR_DEVOLUCION))
			{
				mVista.setResultado(Enumeradores.Resultados.RESULTADO_BIEN);
				mVista.finalizar();
				HashMap<String, String> oParametros = new HashMap<String, String>();
				oParametros.put("TransProdId", pedido.getTransprodID());
				oParametros.put("Confirmar", "true");
				mVista.iniciarActividadConResultado(ICapturaMovConInventario.class, 0, mAccion, oParametros);
			}
		}catch(Exception ex){
			mVista.mostrarError(ex.getMessage());
		}
		...
		try 
		{
			if (mAccion.equals(Enumeradores.Acciones.ACCION_MOSTRAR_PEDIDOS) || mAccion.equals(Enumeradores.Acciones.ACCION_MOSTRAR_PEDIDOS_POR_SURTIR) || mAccion.equals(Acciones.ACCION_CANCELAR_MOVS_ENVIADOS ))
			{
				...
				{ // si se permite cancelar, validar si tiene abonos
					...
					{
						if  (oTpd.Tipo == Enumeradores.TiposTransProd.PEDIDO && m.get("RecAutoEnvVenta").equals("1") ) {
							if ((Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.VENTA) || (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.REPARTO && oTpd.TipoFase == TiposFasesDocto.SURTIDO)) {
								try {
									if (oTpd.DevolucionID != "" && oTpd.DevolucionID != null) {
										TransProd oDev = new TransProd();
										oDev.TransProdID = oTpd.DevolucionID;
										BDVend.recuperar(oDev);
										String grupoDev = ValoresReferencia.getValor("TRPMOT", String.valueOf(oDev.TipoMotivo)).getGrupo();
										Consultas.ConsultasInventario.ValidarInventarioRecoleccionAutomatica(oTpd, oTpd.DevolucionID, (grupoDev.toUpperCase().equals("RECOLECCIÓN") ? "Venta" : "NoVenta"));
									}
			...
			else if (mAccion.equals(Enumeradores.Acciones.ACCION_MOSTRAR_CAMBIOS))
			{
				...
				else { // si se permite cancelar, validar si tiene abonos
					HashMap<String, String> oParametros = new HashMap<String, String>();
					oParametros.put("TransProdId", pedido.getTransprodID());
			...
			}else if (mAccion.equals(Enumeradores.Acciones.ACCION_CAPTURA_CANJE))
            {
                if(!registroTransferido(pedido.getTransprodID(),true)){
                    HashMap<String, String> oParametros = new HashMap<String, String>();
                    oParametros.put("TransProdId", pedido.getTransprodID());
                    oParametros.put("Eliminar", "true");
                    mVista.iniciarActividadConResultado(ICapturaCanje.class, Solicitudes.SOLICITUD_ELIMINAR_CANJE, Acciones.ACCION_CAPTURA_CANJE, oParametros);
                }
		...
		if (((CONHist) Sesion.get(Campo.CONHist)).get("TipoLimiteCredito").toString().equals("2"))
		{
			...
			if (oCliente.VencimientoVenta)
			{
				...
				if ((short) Integer.parseInt((String) ((CONHist) Sesion.get(Campo.CONHist)).get("TipoCobranza")) == 2)
					tipoCobranza = (((Cliente) Sesion.get(Campo.ClienteActual)).TipoFiscal == 1 ? 1: 0) ;
				else
					tipoCobranza = (short) Integer.parseInt((String) ((CONHist) Sesion.get(Campo.CONHist)).get("TipoCobranza"));

				//0 = facturas ; 1 = Ventas
				if (tipoCobranza == 1){
					valor = Consultas.ConsultasTransProd.obtenerVentasVencidas(oCliente);
					if (valor > 0)
					{
						throw new ControlError("E0631");
					}
				} else if (tipoCobranza == 0)
				{
					...
					valor = Consultas.ConsultasTransProd.obtenerFacturasVencidas(oCliente);
					if (valor > 0)
					{
						throw new ControlError("E0631");
					}
    ...
    public void AutorizaMovimiento (){
        mVista.iniciarActividadConResultado(IAutorizaMovimiento.class, Enumeradores.Solicitudes.SOLICITUD_AUTORIZARMOVIMIENTO, Enumeradores.Acciones.ACCION_CAPTURAR_CARGAS);
        return;
    }
}
