		...
		public static Cursor obtenerProductosExistencia(String filtro, String listasPrecios, boolean filtrarProductos, String transProdIds, int tipoTransProd, boolean validarExistencia, int tipoMovimiento, String esquemasModulo, int ubicacionExistencia, boolean excluirCanjes) throws Exception
		{
			...
			while (limit + 6000 < rows)
			{
				...
				if (tipoTransProd == TiposTransProd.DEVOLUCIONES_MANUALES)
					consulta.append("select distinct " + (inventarioPorLotes ? "PRO.ProductoClave || PRU.PRUTipoUnidad || IFNULL(LOT.Lote,'') " : "PRO.ProductoClave") + " as _id, PRO.DecimalProducto, PRO.ProductoClave,PRO.Nombre,PRO.NombreLargo,case when PRO.Id is null then '' else PRO.Id end as Id,case when INV.Disponible is null then 0 else (INV.NoDisponible)/PDE.Factor end as Existencia, ");
					...
					consulta.append("select distinct " + (inventarioPorLotes ? "PRO.ProductoClave || PRU.PRUTipoUnidad || IFNULL(LOT.Lote,'') " : "PRO.ProductoClave") + " as _id, PRO.DecimalProducto, PRO.ProductoClave,PRO.Nombre,PRO.NombreLargo,case when PRO.Id is null then '' else PRO.Id end as Id,case when "+ubicacion+" is null then 0 else ("+ubicacion+")/PDE.Factor end as Existencia, ");
				}
					...
					consulta.append("select distinct " + (inventarioPorLotes ? "PRO.ProductoClave || PRU.PRUTipoUnidad || IFNULL(LOT.Lote,'') " : "PRO.ProductoClave") + " as _id, PRO.DecimalProducto, PRO.ProductoClave,PRO.Nombre,PRO.NombreLargo,case when PRO.Id is null then '' else PRO.Id end as Id,case when INV.Disponible is null then 0 else (INV.Disponible-INV.NoDisponible-INV.Contenido)/PDE.Factor end as Existencia, ");
				else
		...
		public static ISetDatos obtenerDetalleRecibo(String id, short tipo, short tipoRecibo, String tabla, String campos, String campoLlave, String tablaOrden, String campoOrden, boolean agruparPorSubEmpresa, boolean seccionProdPromo)
		{
			try
			{
				...
				{ // Pedido, Consignacion, Mov sin Inv s/Visita, MovSinInv
					// c/Visita
					campos = campos.replace("TransProdDetalle.Importe", "(TransProdDetalle.Total/TransProdDetalle.Cantidad) as Importe ");
					campos = campos.replace("TransProdDetalle.DescuentoImp", "ifnull(TransProdDetalle.DescuentoImp, 0) as DescuentoImp ");
					consulta.append("Select ");
					consulta.append(campos);
					consulta.append(" from ");
					consulta.append(tabla);
...
//            return BDTerm.consultar(consulta.toString());
        ...
        public static ISetDatos obtenerInventarioGeneral (String subEmpresaId) throws  Exception {
			...
			if (((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase != null && ((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase.length()>0) {
				...
				consulta.append("select pro.ProductoClave, pro.Nombre, inv.Disponible as Exis, (inv.Disponible - inv.NoDisponible - inv.Apartado - inv.Contenido) as Disp, (inv.NoDisponible + inv.Apartado) as 'No D', 0 as Precio ");
			}
        ...
        public static ISetDatos obtenerInventarioDetallado(String subEmpresaId) throws Exception{
			...
			if (((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase != null && ((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase.length()>0) {
				...
				consulta.append("select pro.ProductoClave, pro.Nombre, inv.Disponible as Exis, (inv.Disponible - inv.NoDisponible - inv.Apartado - inv.Contenido) as Disp, (inv.NoDisponible + inv.Apartado) as 'No D', pru.PRUTipoUnidad, prd.Factor, 0 as Precio ");
			}
			consulta.append("from Inventario inv ");
			consulta.append("inner join Producto pro on inv.ProductoClave = pro.ProductoClave ");
			consulta.append("inner join ProductoUnidad pru on pro.ProductoClave = pru.ProductoClave ");
...
//				sConsulta.append("AND Visita.DiaClave='" + diaClave + "'");
		...
		public static ISetDatos obtenerEnvaseRecAutomatica(String sTransprodId) throws Exception {
			...
			consulta.append("group by pd.ProductoDetClave ");
...
/*			sConsulta.append("select distinct PRM.PromocionClave, PRE.ProductoClave, 0 as Jerarquia from Promocion PRM, ProductoEsquema PRE");
        ...
        public static String recuperarTransProdDetalleIDPromocion(String transProdID, String productoClave, int tipoUnidad,  String promocionClave) throws Exception{
            ...
            return BDVend.bd.ejecutarEscalar("Select TransProdDetalleID from TransProdDetalle where TransProdID ='" + transProdID + "' and ProductoClave = '" + productoClave + "' and TipoUnidad=" + tipoUnidad + " and PromocionClave='" + promocionClave + "' ");
        }
		public static String recuperarTransProdDetalleIDPromocion(String transProdID, String productoClave, int tipoUnidad, String promocionClave, String Lote) throws Exception{
			return BDVend.bd.ejecutarEscalar("Select TPD.TransProdDetalleID from TransProdDetalle TPD inner join TPDDatosExtra TPE on TPD.TransProdID ='" + transProdID + "' and TPD.ProductoClave = '" + productoClave + "' and TPD.TipoUnidad=" + tipoUnidad + " and TPD.PromocionClave='" + promocionClave + "' and TPE.TransProdDetalleID = TPD.TransProdDetalleID and TPE.Lote = '" +Lote+ "' ");
		}
...
//			consulta.append("WHERE TP.TransProdID= '" +MSIEV+ "') AS MSIEV ON UV.ProductoClave = MSIEV.ProductoClave");
	...
	public static final class ConsultasPedidosConfirmadosPRS{
		...
		public static ISetDatos obtenerReporteConfirmadosPRS(String filtroFecha, String clienteClave)throws  Exception {
			...
			sConsulta.append("where TRP.Tipo = 1 and TRP.TipoFase in (1,11,12) ");
			if (!clienteClave.equals("")) {
				sConsulta.append(" and VIS.ClienteClave = '" + clienteClave + "' ");
			}
			if (!filtroFecha.equals("")) {
	...
	public static class ConsultasVentasOSO{
		...
		public static ISetDatos obtenerConsignas(String TransprodID) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("Select distinct TD.ProductoClave, P.Nombre, TD.Cantidad, (TD.Precio + IFNULL(TI.ImpuestoPU,0)) as PrecioUnitario, TD.Cantidad * (TD.Precio + IFNULL(TI.ImpuestoPU,0)) as SubTotal ");
			consulta.append("FROM TransProd T ");
		...
		public static ISetDatos obtenerDevolucionConsignas(String TransprodID) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("Select distinct TD.ProductoClave, P.Nombre, TD.Cantidad, (TD.Precio + IFNULL(TI.ImpuestoPU,0)) as PrecioUnitario, TD.Cantidad * (TD.Precio + IFNULL(TI.ImpuestoPU,0)) as SubTotal ");
			consulta.append("FROM TrpTpd TT ");
			consulta.append("inner join TransProd TDev on TT.TransprodID = TDev.TransprodID and TDev.Tipo = 3 and TDev.TipoFase = 2 and TDev.TransProdID = '"+TransprodID+"' ");
			...
			consulta.append("inner join Transprod TC on TT.TransProdID1 = TC.TransProdID and TC.Tipo = 24 and TC.TipoFase in (6,8) ");
			consulta.append("ORDER BY TD.ProductoClave ");
			return BDVend.consultar(consulta.toString());
		}
	}
