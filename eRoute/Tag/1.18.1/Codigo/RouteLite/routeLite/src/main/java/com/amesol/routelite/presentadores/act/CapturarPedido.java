...
public class CapturarPedido extends Presentador
{
	...
	ArrayList<TransProdDetalle> prodSinExistencia;
			...
			{ //se paso como parametro un TransProdId, cargar el pedido
				...
				for(TransProd tran : transacciones.values()){
                    ...
                    if(( (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO && tran.TipoFase == TiposFasesDocto.SURTIDO) || Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.VENTA)
                            ...
                            && getTipoTransProd() == TiposTransProd.PEDIDO && ((oMC.get("CuadreDeEnvase").toString().equals("1") && oCliente.Prestamo) || oMC.get("RecAutoEnvVenta").toString().equals("1")) ){
	...
	public void agregarProductoUnidad(String productoClave, String subEmpresaId, int TipoUnidad, Float Cantidad, Float PrecioEspecial, Boolean modificacionPrecio, String ListaPrecioEspecial)
	{
		...
		if (modificacionPrecio){
			generarDetalle(productoClave, subEmpresaId, TipoUnidad, Cantidad, PrecioEspecial, false, (float) 0, null,ListaPrecioEspecial);
		}else{
			generarDetalle(productoClave, subEmpresaId, TipoUnidad, Cantidad, PrecioEspecial, false, (float) 0, null,null);
		}
	...
	public void agregarProductoUnidad(String productoClave, String subEmpresaId, int TipoUnidad, Float Cantidad, Float PrecioEspecial, Float CantidadOriginal)
	{
		...
		generarDetalle(productoClave, subEmpresaId, TipoUnidad, Cantidad, PrecioEspecial, true, CantidadOriginal, null,null);
	} 
	...
	public void agregarProductoUnidad(String productoClave, String subEmpresaId, int TipoUnidad, Float Cantidad, Float PrecioEspecial, String TransProdDetalleID)
	{
		...
		generarDetalle(productoClave, subEmpresaId, TipoUnidad, Cantidad, PrecioEspecial, false, (float) 0, TransProdDetalleID,null);
	}
	...
	public void agregarProductoUnidad(String productoClave, String subEmpresaId, int TipoUnidad, Float Cantidad, Float PrecioEspecial, Float CantidadOriginal, String TransProdDetalleID)
	{
		...
		generarDetalle(productoClave, subEmpresaId, TipoUnidad, Cantidad, PrecioEspecial, true, CantidadOriginal, TransProdDetalleID,null);
	}
	...
	private void generarDetalle(Producto producto, TransProdDetalle transProdDetalle, boolean refrescarListado)
	{
		...
		try
		{
			...
			if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.VENTA || Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO || (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.PREVENTA && ((Ruta) Sesion.get(Campo.RutaActual)).Inventario))
			{
                if (!(producto.Contenido && producto.Venta)){
					if (inventarioPorLotes)
						InventarioLotes.ActualizarInventario(transProdDetalle.ProductoClave, transProdDetalle.TipoUnidad, transProdDetalle.TPDDatosExtra.get(0).Lote, transProdDetalle.Cantidad, moduloMovDetalle.TipoTransProd, moduloMovDetalle.TipoMovimiento);
					else
			...
			if((Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.VENTA || Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO) &&
                    (transProdGeneral.Tipo == TiposTransProd.PEDIDO || transProdGeneral.Tipo == TiposTransProd.VENTA_CONSIGNACION)){
				if(cli.Prestamo)
	...
	private void generarDetalle(String productoClave, String subEmpresaId, int TipoUnidad, Float Cantidad, Float PrecioEspecial, boolean setCantidadOriginal, Float CantidadOriginal, String TransProdDetalleID, String ListaPrecioEspecial)
	{
	...
	public void eliminarDetalle(String transProdId, String transProdDetalleId, String subEmpresaId, String productoClave, int TipoUnidad, float Cantidad, boolean validarTrpSinDetalle)
	{
		...
		try
		{
			...
			if (inventarioPorLotes){
				...
				if (tpd.TPDDatosExtra.size()>0) {
					lote = tpd.TPDDatosExtra.get(0).Lote;
				}
			}
			TransaccionesDetalle.Pedidos.EliminarDetalle(transProdId, transProdDetalleId);
			...
			if (tipoValidacionExistencia != TiposValidacionInventario.NoValidar)
			{
				...
				if((Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO) && mVista.getReparto() && !mVista.getSurtir() ){
					...
					if(producto.Contenido){
						...
						if (inventarioPorLotes){
							InventarioLotes.ActualizarInventario(productoClave, TipoUnidad, lote,Cantidad, moduloMovDetalle.TipoTransProd, moduloMovDetalle.TipoMovimiento, vendedor.AlmacenID, true);
						}else{
							Inventario.ActualizarInventario(productoClave, TipoUnidad, Cantidad, moduloMovDetalle.TipoTransProd, moduloMovDetalle.TipoMovimiento, vendedor.AlmacenID, true);
						}
					}
	...
	public ArrayList<TransProdDetalle> getProdSinExistencia() { return prodSinExistencia; }
		...
		if (docsTransProd.hasNext())
		{
			...
			while(oTpd.hasNext()){
				...
				if(!Inventario.ValidarExistenciaDifNoDisponible(tpd.ProductoClave, tpd.TipoUnidad, tpd.Cantidad, existencia, error)){
					...
					tpd.CantidadInventario = existencia.get();
	...
	public void modificarPedido(){
		...
		try
		{
			...
			prodSinExistencia = validarInventarioASurtirModificar();
	...
	public void convertirPedidoEnConsigna(){
		try {
			...
			ArrayList<TransProdDetalle> prodSinExistencia = validarInventarioASurtir();
			if(prodSinExistencia.size() != 0) {
				mVista.mostrarError(Mensajes.get("E1014"));
				return;
			}

			StringBuilder mensaje = new StringBuilder();
			transProdConsig = Transacciones.generarConsignaDePedido(transProdGeneral, mensaje);
			if (mensaje.length()>0){
				mVista.mostrarAdvertencia(mensaje.toString());
				return;
			}
			mensaje = null;
			Transacciones.copiarDetalleDePedido(transProdGeneral, transProdConsig);

			eliminarApartadoASurtir();
			actualizarInventarioConsigna(transProdConsig);
			actualizarEnvaseConsigna(transProdConsig);

			if (validarLimitePrestamoEnvase()){
	...
	public void eliminarApartadoASurtir()throws Exception {
		BDVend.setOrigenLog("CapturarPedido:eliminarApartadoASurtir");
		//ArrayList<TransProdDetalle> aRes = new ArrayList<TransProdDetalle>();
		Iterator<TransProd> docsTransProd = getHashMapTransacciones().values().iterator();
		if (docsTransProd.hasNext())
		{
			...
			while(oTpd.hasNext()){
				...
				//StringBuilder error = new StringBuilder();
				TransProdDetalle tpd = oTpd.next();
				Inventario.ActualizarInventario(tpd.ProductoClave, tpd.TipoUnidad, tpd.Cantidad, 1, 2, vendedor.AlmacenID, true);
			}
		}
	}

	public void actualizarInventarioConsigna(TransProd consigna)throws Exception {
		BDVend.setOrigenLog("CapturarPedido:actualizarInventarioConsigna");
		ArrayList<TransProdDetalle> aRes = new ArrayList<TransProdDetalle>();
		Iterator<TransProdDetalle> oTpd = consigna.getTransProdDetalle().iterator();
		while(oTpd.hasNext()){
			AtomicReference<Float> existencia = new AtomicReference<Float>();
			StringBuilder error = new StringBuilder();
			TransProdDetalle tpd = oTpd.next();
			Inventario.ActualizarInventario(tpd.ProductoClave, tpd.TipoUnidad, tpd.Cantidad, 24, 2, vendedor.AlmacenID);
		}
	}

	public void actualizarEnvaseConsigna(TransProd consigna)throws Exception{
		BDVend.setOrigenLog("CapturarPedido:actualizarEnvaseConsigna");
		Cliente cli = ((Cliente)Sesion.get(Campo.ClienteActual));
		if (cli.Prestamo) {
			Iterator<TransProdDetalle> oTpd = consigna.getTransProdDetalle().iterator();
			while (oTpd.hasNext()) {
				...
				producto.ProductoClave = tpd.ProductoClave;
				BDVend.recuperar(producto);
				ManejoEnvase.manejoEnvase(producto, tpd.TipoUnidad, tpd.Cantidad, tpd, consigna);
			}
		}
	}

	private boolean validarLimitePrestamoEnvase() throws Exception {
		try {
			AtomicReference<String> mensaje = new AtomicReference<>();
			ManejoEnvase.validarLimitePrestamoEnvase(mensaje, "", false);
			if (mensaje.get() != null) {
				mVista.setMensajeLimiteEnvase(true, false);
				mVista.mostrarError(mensaje.get());
				return false;
			}
		} catch (ControlError e) {
			mVista.setMensajeLimiteEnvase(true, true);
			mVista.mostrarError(e.getMessage());
			return false;
		}
		return true;
	...
	private void validarLimiteCredito() throws Exception{
		...
		if((tipoLimiteCredito == 2 || tipoLimiteCredito == 4) && cfv != null && cfv.ValidaLimite){
			...
			if((tipoCobranza == 1 || tipoCobranza == 2) && cliente.TipoFiscal == 1){
				tipoTrp = 1;
				saldoCliente = Consultas.ConsultasTransProd.obtenerSaldoPedidosConsignaciones(cliente, "'" + transProdConsig.TransProdID + "'") + transProdConsig.Total;
				...
				saldoCliente+= transProdConsig.Total;
				tipoTrp = 8;
			}
			...
			if(valor < 0)
			{
				...
				if(Math.abs(valor) > riesgo){
					if(tipoLimiteCredito == 2){
						mVista.setMensajeLimiteCredito(true, true);
						throw new ControlError("E0737", String.valueOf(Math.abs(valor)));
					}else{
						mVista.setMensajeLimiteCredito(true, false);
	...
	public void imprimirTicketConsignacion() throws Exception
	{
		...
		try {
			if (((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).existeParametro("NumImpresiones", mmdClave)) {
				numImpresiones = Short.parseShort (((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).get("NumImpresiones", mmdClave).toString());
			}
		}catch (Exception ex){
	...
	public List<Map<String, String>> generarDocsAImprimir()
	{
		...
		ISetDatos datos = Consultas.ConsultasTransProd.obtenerTransProdAImprimir("'" + transProdConsig.TransProdID + "'");
