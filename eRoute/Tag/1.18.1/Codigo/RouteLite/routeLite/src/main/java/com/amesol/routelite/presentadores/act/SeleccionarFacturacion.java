...
public class SeleccionarFacturacion extends Presentador
{
	...
	private String sADDId;
	private ArrayList<AddendaDetalle> aAddendaDetalle;
	...
	public  VistaSpinner getTipoPago(){
		VistaSpinner vs;
		TRPVtaAcreditada tva = new TRPVtaAcreditada();
		tva.TransProdID = factura.TransProdID;
		try{
			BDVend.recuperar(tva);
			if (tva != null && !tva.FolioEntrega.equals("")){
				vs = new VistaSpinner(tva.FolioEntrega, ValoresReferencia.getDescripcion("TIPAGO", tva.FolioEntrega ));
				return vs;
			}
		}catch (Exception ex){
			mVista.mostrarError("Error al recuperar TipoPago " + ex.getMessage() );
		}
		return null;
	}
	/**
	 ...
	 * @throws ControlError
	 */
    public Boolean validarInicioFacturacion() throws ControlError{
        boolean bHuboError=false;
        try{
	...
	public boolean crearFactura(Cliente cliente, boolean guardaCliente)
	{
		...
		try
		{
			...
			TRPVtaAcreditada trpVtaAcreditada = crearTRPVtaAcreditada(tpFactura);
			BDVend.guardarInsertar(trpVtaAcreditada);

			crearAddendaFactura(tpFactura);
			crearTRPAddenda(tpFactura);
			...
			if("0".equals(conHist.get("TipoCobranza")) || ("2".equals(conHist.get("TipoCobranza")) && cliente.TipoFiscal == 2))
			{
				...
				//Si no es cobranza a facturas, el saldo debe ser 0
				tpFactura.Saldo = 0;
				BDVend.guardarInsertar(tpFactura);
			}
	...
	private TRPVtaAcreditada crearTRPVtaAcreditada(TransProd factura) throws Exception
	{
		TRPVtaAcreditada trpVtaAcreditada = new TRPVtaAcreditada();

		trpVtaAcreditada.TransProdID = factura.TransProdID;
		trpVtaAcreditada.FolioEntrega =mVista.getTipoDePagoSeleccionado() ;
		trpVtaAcreditada.Observaciones = "Eliminar al actualizar";
		trpVtaAcreditada.Enviado = false;
		trpVtaAcreditada.MUsuarioID = ((Usuario) Sesion.get(Campo.UsuarioActual)).USUId;
		trpVtaAcreditada.MFechaHora = Generales.getFechaHoraActual();
...
//		}
        ...
        trpFiscal.UsoCFDI = mVista.getUsoCFDSeleccionado();
	...
	private void crearAddendaFactura(TransProd factura){
		try {
			for (AddendaDetalle det : aAddendaDetalle) {
				if (det.Respuesta != null) {
					AddendaFactura adf = new AddendaFactura();
					adf.TransProdID = factura.TransProdID;
					adf.ADDDId = det.ADDDId;
					adf.Valor = det.Respuesta;
					adf.MFechaHora = Generales.getFechaHoraActual();
					adf.MUsuarioID = ((Usuario) Sesion.get(Campo.UsuarioActual)).USUId;
					BDVend.guardarInsertar(adf);
				}
			}
		}catch (Exception e){}
	}

	private void crearTRPAddenda(TransProd factura){
		try {
			for (AddendaDetalle det : aAddendaDetalle) {
				if (det.Respuesta != null) {
					TRPAddenda ta = new TRPAddenda();
					ta.TransProdID = factura.TransProdID;
					ta.ADDId = sADDId;
					ta.MFechaHora = Generales.getFechaHoraActual();
					ta.MUsuarioID = ((Usuario) Sesion.get(Campo.UsuarioActual)).USUId;
					BDVend.guardarInsertar(ta);
				}
			}
		}catch (Exception e){}
	}
	...
	private void aplicarPagoAutomatico(TransProd factura, boolean cancelar) throws Exception{
		...
		if(cancelar){
            ...
            oAbn.SubEmpresaId=factura.SubEmpresaId;
			oAbn.Folio = Folio;
            ...
            oAbt.SaldoInsoluto= (factura.Saldo-factura.Total);
	...
	public void validarAccion(int itemId) throws Exception
	{
		...
		switch(itemId)
		{
			...
			case R.id.imprimir:
				...
				BDVend.recuperar(datoFiscal);*/
				mVista.mostrarPreguntaImpresionTimbrado();
				break;
		...
		try
		{
			...
			switch (spinner.getId())
			{
				...
				case R.id.spnFormaPago:
					...
					break;
                case R.id.spnUsoCFDI:
                    int cteUsoCFDI = ((Cliente) Sesion.get(Campo.ClienteActual)).UsoCFDI;
                    LinkedList<VistaSpinner> oValores = obtenerValoresSAT("TUSOCFDI", null);
                    ArrayAdapter<VistaSpinner> vAdapter = new ArrayAdapter<VistaSpinner>((Vista) mVista, android.R.layout.simple_spinner_item, oValores);
                    vAdapter.setDropDownViewResource(android.R.layout.simple_spinner_item);
                    spinner.setAdapter(vAdapter);
                    if (cteUsoCFDI != 0) {
                        for (int i = 0; i < spinner.getCount(); i++) {
                            if (((VistaSpinner) spinner.getItemAtPosition(i)).id.equals(String.valueOf(cteUsoCFDI))) {
				...
				case R.id.spnTipoPago:
					if (subEmpresaId.length<=0) break;
					ValorReferencia[] tiposPago = ValoresReferencia.getValores("TIPAGO",subEmpresaId[0]);

					LinkedList<VistaSpinner> valores= new LinkedList<VistaSpinner>();
					VistaSpinner vsContado =null;
					VistaSpinner vsCredito=null;
					for (ValorReferencia v : tiposPago) {
						if (v.getVavclave().equals("1")){
							vsContado = new VistaSpinner(v.getVavclave(),v.getDescripcion());
							valores.add(vsContado);
						}else if (v.getVavclave().equals("2")){
							vsCredito = new VistaSpinner(v.getVavclave(),v.getDescripcion());
							valores.add(vsCredito);
						}else{
							valores.add(new VistaSpinner(v.getVavclave(),v.getDescripcion()));
						}

					}
					ArrayAdapter<VistaSpinner>adapterTipoPago = new ArrayAdapter<VistaSpinner>((Vista) mVista, android.R.layout.simple_spinner_item, valores);
					spinner.setAdapter(adapterTipoPago);

					//Si CFVTipo = 1, seleccionar por default PUE
					if ( subEmpresaId[0].equals("2")){
						if(vsCredito != null) {
							spinner.setSelection(adapterTipoPago.getPosition(vsCredito));
						}
					}else{
						if(vsContado != null) {
							spinner.setSelection(adapterTipoPago.getPosition(vsContado));
						}
					}
					break;
				default:
    ...
    public static LinkedList<VistaSpinner> obtenerValoresSAT(String varCodigo, String vavClave)
    {
        try {
            LinkedList<VistaSpinner> valores = new LinkedList<VistaSpinner>();
            ...
            if (var != null && var.length > 0) {
                ...
                for (ValorReferencia v : sm.values()) {
                    valores.add(new VistaSpinner(v.getVavclave(), v.getClaveSAT() + " - " + v.getDescripcionSAT()));
                }
            }
	...
	public List<Map<String, String>> generarDocsAImprimir()
	{
		...
		while (c.moveToNext())
		{
			...
			registro.put("ConfigFactura", oConHist.get("TicketConfigFactura").toString());
			if (datoFiscal != null && datoFiscal.TransProdID.equals(factura.TransProdID) && datoFiscal.UUID != null){
				registro.put("UUID", datoFiscal.UUID);
			}else if(facturaSeleccionada !=null && facturaSeleccionada.UUID != null && facturaSeleccionada.UUID.length()>0){
				registro.put("UUID", facturaSeleccionada.UUID);
			}
			tabla.add(registro);
		}
	...
	public boolean requiereAddendaCliente() {
		try{
			Cliente cliente = (Cliente) Sesion.get(Campo.ClienteActual);
			sADDId = Consultas.ConsultasAddenda.obtenerAddendaCliente(cliente.ClienteClave);
			return (!sADDId.equals(""));
		}catch (Exception e){
			return false;
		}
	}

	public void cargarDetalleAdenda(boolean soloLectura){
		try{
			ISetDatos dtDetalle;
			if (soloLectura)
				dtDetalle = Consultas.ConsultasAddenda.obtenerDetalleAddendaFactura(factura.TransProdID);
			else
			 	dtDetalle = Consultas.ConsultasAddenda.obtenerDetalleAddenda(sADDId);

			if (dtDetalle.getCount() > 0){
				aAddendaDetalle = new ArrayList<AddendaDetalle>();
				mVista.limpiarControlesAddenda();
				while (dtDetalle.moveToNext()){
					AddendaDetalle det = new AddendaDetalle();
					det.ADDDId = dtDetalle.getString("ADDDId");
					det.TipoDato = dtDetalle.getInt("TipoDato");
					det.LongMin = dtDetalle.getInt("LongMin");
					det.LongMax = dtDetalle.getInt("LongMax");
					det.Etiqueta = dtDetalle.getString("Etiqueta");
					det.Valor = dtDetalle.getString("Valor");
					det.Requerido = dtDetalle.getBoolean("Requerido");
					if (soloLectura)
						det.Respuesta = dtDetalle.getString("Respuesta");

					aAddendaDetalle.add(det);

					switch (det.TipoDato)
					{
						case 1: //Texto
							mVista.agregarDatoTextoAddenda(det);
							break;
						case 2: //Entero
						case 3: //Decimal
							mVista.agregarDatoNumericoAddenda(det);
							break;
						case 4: //Fecha
							mVista.agregarDatoFechaAddenda(det);
							break;
						case 5: //Lista
							mVista.agregarDatoListaAddenda(det);
							break;
					}
				}
			}
	...
	public String validarDatosAddenda(){
		return mVista.validarDatosAddenda(aAddendaDetalle);
	}
}
