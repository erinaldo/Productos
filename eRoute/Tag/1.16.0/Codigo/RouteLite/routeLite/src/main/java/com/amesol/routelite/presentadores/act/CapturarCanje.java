	...
	private void generarDetalle(TransProdDetalle transProdDetalle, boolean refrescarListado)
	{
		try
		{
			...
			if (transProd.PCEPrecioClave == null || transProd.PCEPrecioClave.length()<=0){
				if (((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase != null) {
                    transProd.PCEPrecioClave = ((Vendedor) Sesion.get(Campo.VendedorActual)).ListaPrecioBase;
			...
			transProdDetalle = TransaccionesDetalle.Pedidos.GenerarDetalleMovGenerales(transProdDetalle, transProd.CadenaListasPrecios) ;
			if (tipoValidacionExistencia != TiposValidacionInventario.NoValidar)
			{
