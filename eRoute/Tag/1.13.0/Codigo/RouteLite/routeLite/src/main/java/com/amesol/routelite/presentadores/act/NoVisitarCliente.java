...
public class NoVisitarCliente extends Presentador
{
		...
		if ((mAccion != null) && (mAccion.equals(Enumeradores.Acciones.ACCION_NO_VENTA))) {
            ...
            try {
                if (Consultas.ConsultasImproductividades.existeImproductividadEnVisita(((Visita) Sesion.get(Campo.VisitaActual)).VisitaClave, ((Visita) Sesion.get(Campo.VisitaActual)).DiaClave)) {
                    ImproductividadVenta oImp = new ImproductividadVenta();
                    oImp.VisitaClave = ((Visita) Sesion.get(Campo.VisitaActual)).VisitaClave;
                    oImp.DiaClave = ((Visita) Sesion.get(Campo.VisitaActual)).DiaClave;
                    BDVend.recuperar(oImp);

                    mVista.setMotivoNoVisita(oImp.TipoMotivo);
                    mVista.setComentario(oImp.Comentario);
                    if (oImp.IdImagenEvidencia != null && oImp.IdImagenEvidencia != "")
                        mVista.setImagen(oImp.IdImagenEvidencia);
                }
            }catch (Exception e){
                e.printStackTrace();
            }
	...
	public void mContinuar()
	{
		...
		if ((mAccion != null) && (mAccion.equals(Enumeradores.Acciones.ACCION_NO_VENTA)))
			if (validar())
			{
				guardarImproductividad();
				//mVista.finalizar();
                //ImprimirTicket ();
			}
	}

	private void guardarImproductividad()
	{
		try
		{
            ...
            BDVend.recuperar(mImproductividadVenta);
            ...
            mImproductividadVenta.IdImagenEvidencia = mVista.getIdImagen();
