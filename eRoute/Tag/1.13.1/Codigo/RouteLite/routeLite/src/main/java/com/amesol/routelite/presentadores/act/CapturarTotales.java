			...
			{ // recorrer todas las transacciones generadas para calcular
				...
				{ // si es el primero asignar todos los valores a los campos
					...
					{ // si es nuevo, asignar valores por default
						...
						if (((MOTConfiguracion) Sesion.get(Campo.MOTConfiguracion)).get("PrecioSegunCFVTipo").equals("1") && oTrp.CFVTipo != null && oTrp.CFVTipo>0){
							mVista.setFormaVenta(oTrp.CFVTipo);
						}else {
	...
	public void calcularDescVendedor(float descto)
	{
		...
		try
		{
			...
			{ // recorrer todas las transaccion
				...
				mVista.recalcularTotales(oTrp);
                Impuestos.RecalcularGlobal(oTrp);
        ...
        try{
            ...
            while (TransprodDetalle.moveToNext()){
                ...
                for (int i = 0; i < arrayImpuestosCalculados.length; i++){
                    Consultas.ConsultasImpuesto.ActualizaImpDesGlb(TransprodDetalle.getString("TransProdID"), TransprodDetalle.getString("TransProdDetalleID"),arrayImpuestosCalculados[i].ImpuestoClave, arrayImpuestosCalculados[i].Importe);
                }
            }
        }
        catch (Exception ex){
        ...
        for (int i = 0; i < Impuestos.length; i++){
            if (Impuestos[i].TipoValor == ServicesCentral.TiposValoresAplicacionImpuestos.Porcentaje.value){
                if(Impuestos[i].TipoAplicacion == ServicesCentral.TiposAplicacionImpuestos.SubtotalConImpuestos.value){
                    Impuestos[i].Importe = (pvBase + nTotalImpuesto) * (Impuestos[i].Valor / 100);
                    Impuestos[i].ImportePU = (pvPU + nTotalImpuestoPU) * (Impuestos[i].Valor / 100);
                }else if(Impuestos[i].TipoAplicacion == ServicesCentral.TiposAplicacionImpuestos.SubtotalSinImpuestos.value){
                    Impuestos[i].Importe = pvBase * (Impuestos[i].Valor / 100);
                    Impuestos[i].ImportePU = pvPU * (Impuestos[i].Valor / 100);
                }
            }else if (Impuestos[i].TipoValor == ServicesCentral.TiposValoresAplicacionImpuestos.Importe.value){
                Impuestos[i].Importe = (Impuestos[i].Valor * pvCantidadUnitaria);
						...
						if (clifv.ValidaLimite)
						{
							...
							if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.VENTA || Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO)
							{
								...
								if (trueType == 1)
								{
                                    ...
                                    //valorConsulta = String.valueOf(valor);
                            ...
                            //float valor1 = valor;
							valor = limite - valor;
                            //throw new ControlError("E0599", "clientePagoID: " + clientePagoID + " CFVTipo: " + CFVTipo + " opcion: " + opcion + " totalInicialCredito: " + String.valueOf(totalInicialCredito) + " total: " + String.valueOf(total) + " valorConsulta: " + valorConsulta + " valor: " + String.valueOf(valor1) + " limite: " + String.valueOf(limite) + " valorfin: " + String.valueOf(valor));
							if (valor < 0)
							{
                                ...
                                //throw new ControlError("E0599", "valor: " + String.valueOf(Math.abs(valor)) + " riesgo: " + String.valueOf(riesgo));
                            ...
                            /*else
                                throw new ControlError("E0599", "valor: " + String.valueOf(Math.abs(valor)));*/
						}
						break;
					}
				}
