...
public class CapturaInicioFinJornada extends Vista implements ICapturaInicioFinJornada
{
        ...
        if (finJornada == true) {
            ...
            if (oMotConf.get("RegistrarTripulacion").equals("1")) {
                try{
                    ...
                    LlenaUsuariosTripulacion();
                    lstUsuariosTrip.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                            String sClaveUsuario;
                            Cursor cUsuario = (Cursor) parent.getItemAtPosition(position);
                            cUsuario.moveToPosition(position);
                            sClaveUsuario = cUsuario.getString(0);

                            String sTipoRol=cUsuario.getString(1);
                            Spinner spinTipoRol = (Spinner) dialogViewRegistrarTripulacion.findViewById(R.id.spTipoRol);
                            SimpleCursorAdapter adapter = (SimpleCursorAdapter) spinTipoRol.getAdapter();
                            for (int i = 0; i < adapter.getCount(); i++) {
                                Cursor c = (Cursor) adapter.getItem(i);
                                if (c.getString(0).equals(String.valueOf(sTipoRol))) {
                                    Spinner spinM = (Spinner) dialogViewRegistrarTripulacion.findViewById(R.id.spTipoRol);
                            ...
                            txtUsuarioBusqueda.setText(sClaveUsuario);
                            spTiposRol.requestFocusFromTouch();
                            txtUsuarioBusqueda.setEnabled(false);
                        }
                    });
    ...
    private void cargarTipoRol() {
        try {
            ISetDatos TiposRol= Consultas.ConsultasValorReferencia.obtenerValoresReferencia("TIPOTRI", "");
    ...
    public void LlenaUsuariosTripulacion(){
        try{
            ...
            SimpleCursorAdapter adapter = new SimpleCursorAdapter(this, R.layout.lista_usuario_trip, cUsuariosTrip, new String[] { "_id", "TipoTripulacion", "USUId" }, new int[] { R.id.txtCol1, R.id.txtCol2, R.id.txtCol3 });
