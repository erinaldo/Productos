	...
	public static List<Map<String, String>> generarListaInfo(Cliente oCliente)
	{
		try
		{
			...
			while (domicilios.hasNext())
			{
				...
				if (domicilio.Tipo == 2)
					domPuntoEnt = domicilio.Calle + " " + (domicilio.Numero == null ? "" :domicilio.Numero + " ")  + (domicilio.NumInt ==null ? "" : domicilio.NumInt + " ") + (domicilio.Localidad == null ? "" : domicilio.Localidad );

				if (domicilio.Tipo == 1)
					domFiscal = domicilio.Calle + " " + (domicilio.Numero == null ? "" :domicilio.Numero + " ")  + (domicilio.NumInt ==null ? "" : domicilio.NumInt + " ")  + (domicilio.Localidad == null ? "" : domicilio.Localidad );
			}
