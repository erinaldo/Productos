...
public class CapturarTotales extends Presentador
{
	...
	HashMap<String, Float> prodSinExistencia;
	ArrayList<TransProdDetalle> productoAConsignacion;
	TransProd transProdConsig;
			...
			{ // recorrer todas las transacciones generadas para calcular
				...
				{ // si es el primero asignar todos los valores a los campos
					...
					{ // si no es nuevo, cargar los datos del transprod
						...
						mVista.setRemision(pedidoAdicional.Remision);
	...
	public void asignarGuardarValores() throws Exception
	{
		...
		BDVend.writeInLog("CapturarTotales:asignarGuardarValores_1");
		...
		if (mVista.getTipoTurno() == -1){
			mVista.setFocus("tipo turno");
			throw new ControlError("E0936", Mensajes.get("XTurno"));
		}

        if( (mVista.getEsNuevo() || mVista.getModificandoAutoventa() || mVista.getSurtir() || mVista.getTipoPedido() == TipoPedido.BACKORDER) && getTipo() == TiposTransProd.PEDIDO){
		...
		while (it.hasNext())
		{
			...
			if (mVista.getTipoPedido() == TipoPedido.BACKORDER) {
				oTrp.TipoPedido = TipoPedido.NORMAL;
            ...
            if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.PREVENTA ){
                if(oTrp.Tipo == TiposTransProd.PEDIDO || oTrp.Tipo == TiposTransProd.MOV_SIN_INV_EV){
                    if(ValoresReferencia.getValor("PEDTipo",oTrp.TipoPedido.toString()).getGrupo().equalsIgnoreCase("PAGOANTICIPADO") ){
                        oTrp.Saldo = Generales.getRound(oTrp.Total,2);
                    }
                }
			...
			if ((!mVista.getPedidoAdicional().trim().equals("")||!mVista.getObservaciones().trim().equals("") ||
					!mVista.getObservaciones2().trim().equals("") || !mVista.getFolioNegociacion().trim().equals("")) ||
					(mostrarCampoNoRemision && !mVista.getRemision().trim().equals("")) && mVista.getEsNuevo())
			{
				TRPVtaAcreditada pedidoAdicional = new TRPVtaAcreditada();
				pedidoAdicional.TransProdID = oTrp.TransProdID;
				if(!mVista.getEsNuevo())
				...
				if (mostrarCampoNoRemision)
					pedidoAdicional.Remision = mVista.getRemision();
			...
			else if (!mVista.getEsNuevo())
			{
                ...
                if (pedidoAdicional.isRecuperado()){
                    ...
                    if (!mVista.getPedidoAdicional().trim().equals("") || !mVista.getObservaciones().trim().equals("") ||
							!mVista.getObservaciones2().trim().equals("") || !mVista.getFolioNegociacion().trim().equals("") ||
							(mostrarCampoNoRemision && !mVista.getRemision().trim().equals(""))) {
                        pedidoAdicional.PedidoAdicional = mVista.getPedidoAdicional();
						if (mostrarCampoNoRemision)
							pedidoAdicional.Remision = mVista.getRemision();
                        pedidoAdicional.Observaciones = mVista.getObservaciones();
...
//									&& (Integer.parseInt(Sesion.get(Campo.TipoIndiceModuloMovDetalleClave).toString())) == 9 && oTrp.CFVTipo == Enumeradores.FormasDeVenta.CONTADO && oTrp.ClientePagoId.equals("1"))
			...
			if (oTrp.Tipo == TiposTransProd.PEDIDO && motConfig.get("RecAutoEnvVenta").equals("1")){
				if ((Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.VENTA) || (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.REPARTO)){
					Transacciones.recolectarEnvasesAutomaticamente(oTrp, null,false);
				}
			}
			...
			if (oTrp.Tipo == TiposTransProd.PEDIDO && motConfig.get("RecAutoEnvVenta").equals("1") && Consultas.ConsultasTransProd.obtenerEnvaseRecAutomatica(oTrp.TransProdID).getCount()>= 1 && (oTrp.DevolucionID == "" || oTrp.DevolucionID == null) ){
				if ((Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.VENTA) || (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == TiposModulos.REPARTO)){
					Transacciones.recolectarEnvasesAutomaticamente(oTrp, Consultas.ConsultasTransProd.obtenerEnvaseRecAutomatica(oTrp.TransProdID),false);
				}
			}
			//Folio CAI 5869
		...
		it = transacciones.iterator();
		while (it.hasNext())
		{
			TransProd oTrp = it.next();
			bEliminados |= Consultas.ConsultasVentasInconsistentes.eliminarImpuestosDuplicados(oTrp.TransProdID);
		}
	...
	public void calcularDescVendedor(float descto)
	{
		...
		try
		{
			...
			{ // recorrer todas las transaccion
				...
				BDVend.guardarInsertar(oTrp);
				Boolean[] aplicoRangos = new Boolean[1];
                CalcularImpDesGlb(oTrp, oTrp.DescVendPor, aplicoRangos);
				if (aplicoRangos[0]){
					bAplicoRangos = true;
				}
					...
					if (clifv.CFVTipo == 2)
					{
						...
						if (clifv.ValidaLimite)
						{
							...
							if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.VENTA || Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO)
							{
								...
								{
									valor = Consultas.ConsultasTransProd.obtenerSaldoNoCobrarVentas(oCliente, oTransprod.TransProdID);
									valor += total -  totalInicialCredito;
								}
							}
	...
	public void WhatsappTicket() throws Exception
	{
		BDVend.setOrigenLog("CapturarTotales:imprimirTicket");
		Recibos recibo = new Recibos();
		...
		recibo.WhatsappRecibos(generarDocsAImprimir(), false, mVista, numImpresiones);
...
*/
	...
	public void setProdSinExistencia(HashMap<String, Float> hsProdSinExistencia){
		prodSinExistencia = hsProdSinExistencia;
	}

	public boolean validarPedidoEnConsigna(){
		productoAConsignacion = new ArrayList<>();
		transacciones.get(0);
		//Iterator<TransProd> it = transacciones.iterator();
		//while (it.hasNext()) {
			TransProd oTrp = transacciones.get(0);
			Iterator<TransProdDetalle> itDet = oTrp.getTransProdDetalle().iterator();
			while (itDet.hasNext()){
				TransProdDetalle oTpd = itDet.next();
				if (prodSinExistencia.containsKey(oTpd.TransProdDetalleID))
					oTpd.CantidadInventario = prodSinExistencia.get(oTpd.TransProdDetalleID);
				else
					oTpd.CantidadInventario = oTpd.CantidadOriginal;

				if (oTpd.CantidadInventario > oTpd.Cantidad){
					oTpd.Cantidad = oTpd.CantidadInventario - oTpd.Cantidad;
					productoAConsignacion.add(oTpd);
				}
			}
		//}

		if (productoAConsignacion.size() > 0){
			return true;
		}
		return false;
	}

	public void convertirPedidoEnConsigna(){
		try{
			ValorReferencia oValor = ValoresReferencia.getValor("TRPMOT", "12");
			if (oValor == null || !oValor.getGrupo().toUpperCase().equals("CONSIGNACION")) {
				mVista.mostrarError(Mensajes.get("I0284"));
				return;
			}
			...
			BDVend.recuperar(oCliente, CLIFormaVenta.class);
			Iterator<CLIFormaVenta> it = oCliente.CLIFormaVenta.iterator();
			while (it.hasNext() && !bCredito){
				CLIFormaVenta clifv = it.next();
				if (clifv.CFVTipo == 2){
					bCredito = true;
					diasCredito = (short)clifv.DiasCredito;
				}
			}
			if (!bCredito){
				if (((CONHist) Sesion.get(Campo.CONHist)).get("ConsignaContado").toString().equals("0")){
					mVista.mostrarError(Mensajes.get("I0341"));
					return;
				}
			}

			StringBuilder mensaje = new StringBuilder();
			transProdConsig = Transacciones.generarConsignaDePedidoParcial(transacciones.get(0), diasCredito, mensaje);
			if (mensaje.length()>0){
				mVista.mostrarAdvertencia(mensaje.toString());
				return;
			}
			mensaje = null;

			Iterator<TransProdDetalle> itTpd = productoAConsignacion.iterator();
			while (itTpd.hasNext()) {
				TransProdDetalle tpd = itTpd.next();
				BDVend.recuperar(tpd, TPDDatosExtra.class);
				TransProdDetalle tpdCons = TransaccionesDetalle.Pedidos.GenerarDetalleConsignaDePedido(transProdConsig.TransProdID, tpd.ProductoClave, tpd.TipoUnidad, tpd.Cantidad, tpd.Precio, tpd.TPDDatosExtra.get(0).PrecioClave);
				transProdConsig.TransProdDetalle.add(tpdCons);

				Producto oProd = new Producto();
				oProd.ProductoClave = tpdCons.ProductoClave;
				BDVend.recuperar(oProd);
				if (!(oProd.Contenido && oProd.Venta)){
					Inventario.ActualizarInventario(tpdCons.ProductoClave, tpdCons.TipoUnidad, tpdCons.Cantidad, transProdConsig.Tipo, transProdConsig.TipoMovimiento, vendedor.AlmacenID);
				}
				...
				if(cli.Prestamo){
					...
					Producto producto = new Producto();
					producto.ProductoClave = tpdCons.ProductoClave;
					BDVend.recuperar(producto);
					ManejoEnvase.manejoEnvase(producto, tpdCons.TipoUnidad, tpdCons.Cantidad, tpdCons, transProdConsig);
				}
				//BDVend.guardarInsertar(tpdCons);
			}

			Transacciones.Pedidos.CalcularTotalesPedido(transProdConsig);
			BDVend.guardarInsertar(transProdConsig);

			//FALTA APLICAR PROMOCIONES
			mVista.calcularPromocionesConsigna(transProdConsig);
	...
	public void terminarConsignacion()throws Exception {
		Transacciones.Pedidos.CalcularTotalesPedido(transProdConsig);
		transProdConsig.Saldo = transProdConsig.Total;
		BDVend.guardarInsertar(transProdConsig);
