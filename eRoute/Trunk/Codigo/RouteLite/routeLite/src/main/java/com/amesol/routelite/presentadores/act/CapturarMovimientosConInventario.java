...
public class CapturarMovimientosConInventario extends Presentador
{
	...
	public void iniciar()
	{
		try
		{
			...
			if ((mAccion.equals(Enumeradores.Acciones.ACCION_CAPTURAR_DESCARGA)))
			{
				if (((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).existeParametro("CalcularKardexUnidad")) {
					calcularKardexUnidad = ((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).get("CalcularKardexUnidad").toString().equals("1");
					if (calcularKardexUnidad)
						Consultas.ConsultasKardexUnidad.generarInventarioKardex();
				}
			}
	...
	public String consultarUnidadProductoExistente(String TransProdID, String ProductoClave, short TipoUnidad)
	{
		String Cantidad = "0";
		...
		try
		{
			for (int a = 0; a < transProdID.length; a++)
			{
				ISetDatos mManejo;
				if (calcularKardexUnidad)
					mManejo = ConsultasCantidad.ObtenerCantidad(transProdID[a], ProductoClave, TipoUnidad);
				else
					mManejo = ConsultasCantidad.OptenerCantidad(transProdID[a], ProductoClave);
	...
	private void generarDetalle(String productoClave, int TipoUnidad, Float Cantidad, Float CantidadOriginal, String Motivo)
	{
		...
		try
		{
			...
			{
				//5614: Revisar si se leyó factor en Codigo de Barras
				if(transProdGeneral.Tipo == Enumeradores.TiposTransProd.CARGAS && mVista.getLeerFactorEnCodigoBarras() ) {
					if (mVista.getFactorLeidoProductoActual() > 0) {
						...
						tde.Factor = mVista.getFactorLeidoProductoActual();
						tde.MUsuarioID = transProdDetalle.MUsuarioID;
	...
	public boolean ValidarAfectarInventarioConfirmacionCarga(){
		try{
			...
			for(TransProdDetalle oTpd : transProdGeneral.TransProdDetalle) {
				if (mVista.getLeerFactorEnCodigoBarras()) {
					int tipoUnidad = oTpd.TipoUnidad;
					float factor = 1;
					if (oTpd.TPDDatosExtra == null || oTpd.TPDDatosExtra.size() <= 0) {
						BDVend.recuperar(oTpd, TPDDatosExtra.class);
					}
					if (oTpd.TPDDatosExtra != null && oTpd.TPDDatosExtra.size() > 0) {
						if (oTpd.TPDDatosExtra.get(0).Factor > 0) {
							factor = oTpd.TPDDatosExtra.get(0).Factor;
							tipoUnidad = Consultas.ConsultasProducto.obtenerUnidadMinima(oTpd.ProductoClave);
							//Obtener el contenedor para incrementarlo, ya que al enviar piezas a la función, no lo hace
							if (tipoUnidad != oTpd.TipoUnidad) {
								ProductoUnidad pru = new ProductoUnidad();
								pru.ProductoClave = oTpd.ProductoClave;
								pru.PRUTipoUnidad = Short.parseShort(String.valueOf(oTpd.TipoUnidad));
								BDVend.recuperar(pru);
								if (pru.Contenedor != null && pru.Contenedor.length() > 0) {
									int tipoUnidadMinContenedor = Consultas.ConsultasProducto.obtenerUnidadMinima(oTpd.ProductoClave);
									Inventario.ActualizarInventario(pru.Contenedor, tipoUnidadMinContenedor, oTpd.Cantidad, moduloMovDetalle.TipoTransProd, moduloMovDetalle.TipoMovimiento, vendedor.AlmacenID);
								}
							}
						}
					}
					Inventario.ActualizarInventario(oTpd.ProductoClave, tipoUnidad, oTpd.Cantidad * factor, moduloMovDetalle.TipoTransProd, moduloMovDetalle.TipoMovimiento, vendedor.AlmacenID);
				} else {
