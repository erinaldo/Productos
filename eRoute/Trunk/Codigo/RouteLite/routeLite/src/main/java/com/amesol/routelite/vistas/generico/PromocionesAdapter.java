		...
		if(convertView == null){
			...
			fila = 	inflater.inflate(textViewResourceId, null);
			 ...
			 Promocion vr = getItem(position);
				...
				try {
					inventarioPorLotes = (((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).existeParametro("InventarioPorLotes") && ((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).get("InventarioPorLotes").equals("1"));
				}catch(Exception e){}

				if(inventarioPorLotes)
					vr.SeleccionProducto = true;
				...
				//String sProductosCol2="";
					...
					for(int i=0;i<TodasPromociones.length;i++)
					{
						if(TodasPromociones[i].PromocionClave.equals(vr.PromocionClave))
						{
                            ...
                            TextView txtCantidadKit = new TextView(getContext());
							TextView txtdato = new TextView(getContext());
							txtnombre.setText(TodasPromociones[i].ProductoNombre.trim());

                            if(TodasPromociones[i].CantidadKit != 0){
                                if (vr.TipoRango == Enumeradores.Promocion.TipoRango.Importe)
                                    txtCantidadKit.setText(Generales.getFormatoDecimal(TodasPromociones[i].CantidadKit, "#,##0.00"));
                                else
                                    txtCantidadKit.setText(String.valueOf(TodasPromociones[i].CantidadKit));
                            }
                            else{
                                txtCantidadKit.setText("");
                            }
                            ...
                            txtCantidadKit.setMaxLines(1);
                            txtCantidadKit.setLayoutParams(param);

							txtdato.requestLayout();
                            txtCantidadKit.requestLayout();
							txtnombre.requestLayout();
							
							llh.addView(txtnombre); //agregar el nombre a la vista horizontal
                            llh.addView(txtCantidadKit); //agregar la cantidad a la vista horizontal
