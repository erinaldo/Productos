...
import com.amesol.routelite.vistas.ConsultaInventario.ListaInventarioLotes;
...
public final class Consultas
{
	...
	public static final class ConsultasVendedor
	{
        ...
        public static int ObtenerClientesConConsignaAb(Vendedor vendedor) throws Exception{
            String Consulta = "select count(ClienteClave) as TotalClientes ";
            Consulta += "from VendedorConsigAb ";
            Consulta += "where VendedorID = '" + vendedor.VendedorId + "' ";

            return BDVend.bd.ejecutarEscalarInteger(Consulta);
        }

        public static boolean ExisteClienteConConsignaAb(Vendedor vendedor, Cliente cliente) throws Exception{
            String Consulta = "select count(*)";
            Consulta += "from VendedorConsigAb ";
            Consulta += "where VendedorID = '" + vendedor.VendedorId + "' and ClienteClave = '" + cliente.ClienteClave + "'";

            return (BDVend.bd.ejecutarEscalarInteger(Consulta) > 0);
        }

        public static void InsertarClienteConConsignaAb(Vendedor vendedor, Cliente cliente) throws Exception{
            if (!ExisteClienteConConsignaAb(vendedor, cliente)) {
                String cmd = "insert into VendedorConsigAb (VendedorID, ClienteClave) values ('" + vendedor.VendedorId + "', '" + cliente.ClienteClave + "')";
                BDVend.bd.ejecutarComando(cmd);
            }
        }

        public static void EliminarClienteConConsignaAb(Vendedor vendedor, Cliente cliente) throws Exception{
            //if (!ExisteClienteConConsignaAb(vendedor, cliente)) {
                String cmd = "delete from VendedorConsigAb where VendedorID = '" + vendedor.VendedorId + "' and ClienteClave = '" + cliente.ClienteClave + "'";
                BDVend.bd.ejecutarComando(cmd);
            //}
        }

        public static boolean validarClienteConConsignasAbiertas(Vendedor vendedor, Cliente cliente) throws Exception {
            StringBuilder consulta = new StringBuilder();
            ...
            consulta.append("where trp.Tipo = 24 and trp.TipoFase = 2 and vis.VendedorID = '" + vendedor.VendedorId + "' and vis.ClienteClave = '" + cliente.ClienteClave + "' ");
	...
	public static final class ConsultasRuta
	{
		...
		public static short getFolioClteNvo(String RUTClave) throws Exception
		{
			ISetDatos datos = BDVend.consultar("SELECT FolioClienteNvo FROM ruta where RUTClave  = '" + RUTClave + "'");
			short res = 0;
			if (datos.moveToNext())
			{
		...
		public static boolean ExisteClaveCliente(String Clave) throws Exception
		{
			ISetDatos datos = BDVend.consultar("SELECT * FROM cliente where clave  = '" + Clave + "'");
			boolean res = false;
			if (datos.getCount() > 0)
			{
				res = true;
	...
	public static final class ConsultasCliente
	{
		public static ISetDatos obtenerVisitados(Dia diaActual, Ruta rutaActual, String filtro) throws Exception
		{
			...
			sConsulta.append("SELECT ClienteClave AS _id, ClienteClave AS " + SearchManager.SUGGEST_COLUMN_INTENT_DATA + ", Clave || ' - ' || RazonSocial AS " + SearchManager.SUGGEST_COLUMN_TEXT_1 + ", CASE WHEN NombreContacto IS NULL OR NombreContacto = ''  THEN 'Sin datos de Contacto' ELSE NombreContacto END AS Contacto, Calle || ' ' || Numero AS " + SearchManager.SUGGEST_COLUMN_TEXT_2 + ", HorarioVisita ");
			...
			sConsulta.append("WHERE Agenda.RUTClave='" + rutaActual.RUTClave + "' and Agenda.DiaClave = '" + diaActual.DiaClave + "' ");
		...
		{
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("SELECT AGN.ClienteClave AS _id, AGN.ClienteClave AS " + SearchManager.SUGGEST_COLUMN_INTENT_DATA + ", Clave || ' - ' || RazonSocial AS " + SearchManager.SUGGEST_COLUMN_TEXT_1 + ", CASE WHEN NombreContacto IS NULL OR NombreContacto = ''  THEN 'Sin datos de Contacto' ELSE NombreContacto END AS Contacto, Calle || ' ' || Numero AS " + SearchManager.SUGGEST_COLUMN_TEXT_2 + ", ifnull(AGN.HorarioVisita, '') as HorarioVisita ");
			if(Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO){
				//si es reparto, agregar esta columna para saber si tiene pedidos pendientes por surtir
				sConsulta.append(",case when (select case when count(*) > 0 then 1 else 0 end as PedidoXSurtir ");
				sConsulta.append("from TransProd TRP inner join Visita VIS on TRP.VisitaClave = VIS.VisitaClave and TRP.DiaClave = VIS.DiaClave ");
		...
		public static ISetDatos obtenerHistoricoVisitas() throws Exception
		{
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("SELECT");
			sConsulta.append("\tHVIS.ClienteClave AS _id, HVIS.ClienteClave AS suggest_intent_data, HVIS.ClienteClave || ' - ' || HVIS.RazonSocial AS suggest_text_1,");
			sConsulta.append("\tCASE WHEN C.NombreContacto IS NULL OR C.NombreContacto = ''  THEN 'Sin datos de Contacto' ELSE C.NombreContacto END AS Contacto,");
			sConsulta.append("\tCD.Calle || ' ' || CD.Numero AS suggest_text_2 ");
			sConsulta.append("FROM HistoricoVisita HVIS");
			sConsulta.append("\tLEFT JOIN Cliente C ON HVIS.ClienteClave = C.Clave");
			sConsulta.append("\tLEFT JOIN ClienteDomicilio CD ON C.ClienteClave = CD.ClienteClave AND CD.Tipo = 2 ");
			sConsulta.append("ORDER BY HVIS.ClienteClave");
		...
		public static ISetDatos obtenerBackOrder(Dia diaActual, Ruta rutaActual, String filtro) throws Exception
		{
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("SELECT ClienteClave AS _id, ClienteClave AS " + SearchManager.SUGGEST_COLUMN_INTENT_DATA + ", Clave || ' - ' || RazonSocial AS " + SearchManager.SUGGEST_COLUMN_TEXT_1 + ", CASE WHEN NombreContacto IS NULL OR NombreContacto = ''  THEN 'Sin datos de Contacto' ELSE NombreContacto END AS Contacto, Calle || ' ' || Numero AS " + SearchManager.SUGGEST_COLUMN_TEXT_2 + ", HorarioVisita ");
			if(Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO){
			...
			sConsulta.append("WHERE Agenda.DiaClave='" + diaActual.DiaClave + "' AND Agenda.RUTClave='" + rutaActual.RUTClave + "' AND Cliente.ClienteClave in (select v.clienteclave ");
			sConsulta.append("from transprod t inner join visita v on t.visitaclave=v.visitaclave and t.diaclave=v.diaclave ");
			sConsulta.append("where t.Tipo = 1 and t.FechaEntrega <= '" + diaActual.FechaCaptura + "' and t.FechaSurtido is null and t.TipoPedido = 9) ");
			sConsulta.append("GROUP BY Agenda.ClienteClave, Cliente.Clave, Cliente.RazonSocial, Cliente.NombreContacto, ClienteDomicilio.Calle, ClienteDomicilio.Numero, ifnull(Agenda.HorarioVisita, '')) as t ");
			if (filtro != null)
				sConsulta.append("WHERE Clave || ' - ' || RazonSocial LIKE '%" + filtro + "%' OR NombreContacto LIKE '%" + filtro + "%' OR Calle || ' ' || Numero LIKE '%" + filtro + "%' ");
			sConsulta.append("ORDER BY Orden");
		...
		public static ISetDatos obtenerMensajesCliente(String clienteClave, String tipoModulo) throws Exception
		{
			...
			sConsulta.append("select CAST(MMS.MDBMensajeTipo AS nvarchar) || CAST(MMS.Numero AS nvarchar) as _id, CAST(MMS.MDBMensajeTipo, nvarchar) || CAST(MMS.Numero, nvarchar) || ' - ' || MMS.Descripcion as Descripcion ");
		...
		public static String obtenerClienteDomicilioIdFiscal(String clienteClave) throws Exception {
			return (BDVend.bd.ejecutarEscalar("select ClienteDomicilioId from ClienteDomicilio where ClienteClave = '" + clienteClave + "' and Tipo = 1"));
		}

		public static float obtenerSaldoDocumentos(boolean incluirContado){
			try {
				float saldoDocumentos = 0;
				CONHist conh = (CONHist) Sesion.get(Campo.CONHist);
				...
				if (conh.get("TipoCobranza").toString().equals("1") || (conh.get("TipoCobranza").toString().equals("2") && cli.TipoFiscal == 1)) {
					if (cli.ActualizaSaldoCheque)
						saldoDocumentos = Consultas2.ConsultasTransProd.obtenerSaldoVentas(cli.ClienteClave, incluirContado);
					else
						saldoDocumentos = Consultas2.ConsultasTransProd.obtenerSaldoVentasNoCheque(cli.ClienteClave, incluirContado);
					...
					if (cli.ActualizaSaldoCheque) {
						if (Integer.parseInt(Sesion.get(Sesion.Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.PREVENTA)
							saldoDocumentos = Consultas2.ConsultasTransProd.obtenerSaldoFacturas(cli.ClienteClave, incluirContado);
						else
							saldoDocumentos = Consultas2.ConsultasTransProd.obtenerSaldoFacturas(cli.ClienteClave, incluirContado) + Consultas2.ConsultasTransProd.obtenerSaldoVtasNoFact(cli.ClienteClave, incluirContado);
					} else {
						if (Integer.parseInt(Sesion.get(Sesion.Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.PREVENTA)
							saldoDocumentos = Consultas2.ConsultasTransProd.obtenerSaldoFacturasNoCheque(cli.ClienteClave, incluirContado);
						else
							saldoDocumentos = Consultas2.ConsultasTransProd.obtenerSaldoFacturasNoCheque(cli.ClienteClave, incluirContado) + Consultas2.ConsultasTransProd.obtenerSaldoVtasNoFact(cli.ClienteClave, incluirContado);
					}
				}
	...
	/*CAI 4978*/
	public static final class ConsultasPromociones
	{
		public static ISetDatos obtenerVentasARecalcular(String VisitaClave) throws Exception
		{
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("Select distinct T.TransprodId, TD.TransprodDetalleID");
			sConsulta.append("from Visita V ");
			sConsulta.append("Inner join Transprod T on V.VisitaClave = T.VisitaClave and V.DiaClave = T.DiaClave and T.TipoFase <> 3 ");
			sConsulta.append("Inner join TransprodDetalle TD on T.TransProdID = TD.TransProdID ");
			sConsulta.append("Inner join TrpPrp TP on T.TransProdID = TP.TransProdID and TD.TransProdDetalleID = TP.TransProdDetalleID ");
			sConsulta.append("where V.VisitaClave = '" + VisitaClave + "' and (TD.DescuentoImp is null or TD.DescuentoImp = 0 or TD.DescuentoImp = '')");
			return BDVend.consultar(sConsulta.toString());
		}

		public static Float obtenerImportePromocion(String TransprodID, String TransprodDetalleID) throws Exception
		{
			ISetDatos datos = BDVend.consultar("SELECT SUM(PromocionImp) FROM TrpPrp where TransprodId = '" + TransprodID +"' and TransprodDetalleId = '" + TransprodDetalleID +"'");
			float res = 0;
			if (datos.moveToNext())
			{
				res = datos.getFloat(0);
		...
		public static ISetDatos obtenerImpuestosDuplicados(String TransprodID, String TransprodDetalleID) throws Exception
		{
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("Select TransprodID, TransprodDetalleID, ImpuestoClave, Count(ImpuestoClave) ");
			sConsulta.append("from TPDImpuesto ");
			sConsulta.append("where TransprodId = '" + TransprodID + "' and TransprodDetalleId = '" + TransprodDetalleID + "' ");
			sConsulta.append("group by TransprodID, TransprodDetalleID, ImpuestoClave ");
			sConsulta.append("having count(ImpuestoClave) > 1 ");
			return BDVend.consultar(sConsulta.toString());
		}

		public static String obtenerImpuestoDetalle(String TransprodID, String TransprodDetalleID, String ImpuestoClave) throws Exception
		{
			ISetDatos datos = BDVend.consultar("Select top 1 TPDImpuestoID from TPDImpuesto where TransprodId = '"+TransprodID+"' and TransprodDetalleId = '"+TransprodDetalleID+"' and ImpuestoClave = '"+ImpuestoClave+"' order by MFechahora desc");
			String res = "";
			if (datos.moveToNext())
			{
		...
		public static Float obtenerSubTotal(String TransprodID) throws Exception
		{
			ISetDatos datos = BDVend.consultar("SELECT SUM(SubTotal) FROM TransprodDetalle where TransprodId = '" + TransprodID +"'");
			float res = 0;
			if (datos.moveToNext())
			{
				res = datos.getFloat(0);
		...
		public static Float obtenerImpuestos(String TransprodID) throws Exception
		{
			ISetDatos datos = BDVend.consultar("SELECT SUM(ImpDesGlb) FROM TPDImpuesto where TransprodId = '" + TransprodID +"'");
			float res = 0;
			if (datos.moveToNext())
			{
				res = datos.getFloat(0);
	...
	public static final class ConsultasCantidad
	{
		...
		public static Float ObtenerCantidad(String TransProdIDs, String ProductoClave,Float Factor) throws Exception
		{
			try
			{
				String res = BDVend.getBD().ejecutarEscalar("Select TPD.Cantidad From TransProdDetalle TPD left join TPDDatosExtra TDE on TDE.TransProdID = TPD.TransProdID and TDE.TransProdDetalleID = TPD.TransProdDetalleID Where TPD.ProductoClave='" + ProductoClave + "' And TPD.TransProdID in(" + TransProdIDs + ") and IFNULL(TDE.Factor,0)=" + Factor);
				return Float.parseFloat((res.toString().equals("")? "0" :res.toString()));
			}
			catch (Exception ex)
			{
				ex.printStackTrace();
	...
	public static final class ConsultasProducto
	{
		...
		public static Producto obtenerProductoIdentico(String cadenaBusqueda, String Regex) throws Exception
		{
			ISetDatos setDatos = BDVend.consultar("Select * from Producto where " + Regex);
			Producto producto = null;
			if ((setDatos != null) && (setDatos.moveToFirst()) && (setDatos.getCount() > 0))
			{
				producto = (Producto) BDVend.instanciar(Producto.class, setDatos);
		...
		public static ISetDatos buscarCodigoBarras(String codigoBarras, String listasPrecios , int tipoTransProd) throws Exception
		{
			StringBuilder consulta = new StringBuilder();

			boolean filtrar =false;
			if (tipoTransProd == TiposTransProd.PEDIDO && Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) != Enumeradores.TiposModulos.PREVENTA ){
			...
			consulta.append("case when INV.Disponible is null then 0 else (INV.Disponible-INV.Apartado-INV.NoDisponible-INV.Contenido)/ProductoDetalle.Factor end as Existencia ");
			consulta.append("FROM ProductoDetalle ");
			...
			if( inventarioPorLotes){
				consulta.append("left join InventarioLotes INV on Producto.ProductoClave = INV.ProductoClave and INV.TipoUnidad = ProductoUnidad.PRUTipoUnidad ");
			}else{
				consulta.append("left join Inventario INV on Producto.ProductoClave = INV.ProductoClave ");
			}

			if (filtrar){
				consulta.append("where not PPV.PrecioClave is null and (case when INV.Disponible is null then 0 else (INV.Disponible-INV.Apartado-INV.NoDisponible-INV.Contenido)/ProductoDetalle.Factor end)>0 ");
			}
		...
		public static Cursor obtenerProductosExistencia(String filtro, String listasPrecios, boolean filtrarProductos, String transProdIds, int tipoTransProd, boolean validarExistencia, int tipoMovimiento, String esquemasModulo, int ubicacionExistencia, boolean excluirCanjes) throws Exception
		{
			...
			boolean filtrarConteoInventario = false;
			...
			while (limit + 6000 < rows)
			{
				...
				if (tipoTransProd == TiposTransProd.CAMBIOS || tipoTransProd == TiposTransProd.DEVOLUCIONES_CLIENTE || tipoTransProd == TiposTransProd.DEVOLUCIONES_MANUALES || tipoTransProd == TiposTransProd.MOV_SIN_INV_SV || tipoTransProd == TiposTransProd.DESCARGAS || tipoTransProd == TiposTransProd.CARGAS)
				{
					...
					aplicarFiltrosExistencia = false;
				}else if(tipoTransProd == TiposTransProd.CONTEO_INVENTARIO){
					filtrarConteoInventario = true;
				}

				consulta.append("select distinct " + (inventarioPorLotes ? "PRO.ProductoClave || PRU.PRUTipoUnidad || IFNULL(LOT.Lote,'') " : "PRO.ProductoClave") + " as _id, PRO.DecimalProducto, PRO.ProductoClave,PRO.Nombre,PRO.NombreLargo,case when PRO.Id is null then '' else PRO.Id end as Id, ");
				...
				if (inventarioPorLotes) {
					consulta.append(", LOT.Lote, strftime('%d/%m/%Y', LOT.Caducidad) as Caducidad ");
					consulta.append(",iFNULL((Select sum(Cantidad) from TransProdDetalle TPD inner join TPDDatosExtra TDE ");
					consulta.append("on TPD.TransProdID = TDE.TransProdID and TPD.TransProdDetalleID = TDE.TransProdDetalleID ");
					consulta.append("where TPD.TransProdID in(" + transProdIds + ") and TPD.ProductoClave = PRO.ProductoClave and INV.Lote = TDE.Lote and IFNULL(TPD.Promocion,0) <> 2),0) as CantidadActual ");
				}
				...
				if (inventarioPorLotes) {
					consulta.append("left join InventarioLotes INV on PRO.ProductoClave = INV.ProductoClave AND PRU.PRUTipoUnidad = INV.TipoUnidad ");
					consulta.append("left join LoteCaducidad LOT on INV.Lote = LOT.Lote ");
				}
				else {
					if (calcularKardexUnidad)
				...
				if (filtrarConteoInventario){
					consulta.append("left join ConteoInventarioDet CID on CID.ContId in(" + transProdIds + ") and CID.ProductoClave = PRO.ProductoClave ");
				}else {
					if(!inventarioPorLotes) {
				...
				if (filtrarConteoInventario) {
					filtro += (filtro.length() > 0 ? "AND" : "") + " CID.ContID is null ";
				}else{
					if(!inventarioPorLotes){
					...
					consulta.append((listasPrecios.equals("") ? " WHERE " :  (filtro.length()>0 ? " AND " : "")) + filtro);
				...
				if (aplicarFiltrosExistencia && Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) != Enumeradores.TiposModulos.PREVENTA)
					if (inventarioPorLotes){
						if (tipoTransProd == TiposTransProd.DEVOLUCIONES_MANUALES)
							consulta.append(" GROUP BY PRO.ProductoClave, PRU.PRUTipoUnidad, LOT.Lote HAVING ((INV.NoDisponible)/PDE.Factor)>0 ");
						else if (tipoTransProd == TiposTransProd.DESCARGAS && Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO)
							consulta.append(" GROUP BY PRO.ProductoClave, PRU.PRUTipoUnidad, LOT.Lote HAVING ((INV.Disponible-INV.NoDisponible-INV.Contenido)/PDE.Factor)>0 ");
						else
							consulta.append(" GROUP BY PRO.ProductoClave, PRU.PRUTipoUnidad, LOT.Lote HAVING ((INV.Disponible-INV.Apartado-INV.NoDisponible-INV.Contenido)/PDE.Factor)>0 or (iFNULL((Select sum(Cantidad) from TransProdDetalle TPD inner join TPDDatosExtra TDE on TPD.TransProdID = TDE.TransProdID and TPD.TransProdDetalleID = TDE.TransProdDetalleID where TPD.TransProdID in(" + transProdIds + ") and TPD.ProductoClave = PRO.ProductoClave and INV.Lote = TDE.Lote ),0)>0) ");
					}else{
						if (tipoTransProd == TiposTransProd.DEVOLUCIONES_MANUALES)
				...
				if (((ConfigParametro)Sesion.get(Campo.ConfigParametro)).get("OrdenBusqProducto").length()>0){
					...
					if (inventarioPorLotes)
						consulta.append("ORDER BY PRO.ProductoClave, PRU.PRUTipoUnidad, LOT.Caducidad  ");
					else
	...
	public static final class ConsultasImpresionTicket
	{
		...
		public static ISetDatos obtenerDetalleRecibo(String id, short tipo, short tipoRecibo, String tabla, String campos, String campoLlave, String tablaOrden, String campoOrden, boolean agruparPorSubEmpresa, boolean seccionProdPromo)
		{
			try
			{
				...
				if (campos.toUpperCase().contains("TPDDATOSEXTRA"))
				{
	...
	public static final class ConsultasVisita
	{
		...
		public static void eliminarVisitasSinMovimientos(boolean accesoSistema){
			try{
				String consulta = "select VisitaClave, DiaClave, ClienteClave, VendedorId, RUTClave, FechaHoraInicial, FechaHoraFinal from Visita where Enviado = 0 order by FechaHoraInicial desc";
				ISetDatos visitas = BDVend.consultar(consulta.toString());
				String visitaClave;
				String diaClave;
				String clienteClave;
				String vendedorID;
				...
				Date fechaHoraFinal;
				int tiempoMin = 0;
				boolean ultimaVisita = true;
				double tiempoVisita = 0;
				if (accesoSistema)
					tiempoMin = Consultas.ConsultasCliente.obtenerTiempoMinVisita();
				while (visitas.moveToNext()) {
					visitaClave = visitas.getString("VisitaClave");
					diaClave = visitas.getString("DiaClave");
					clienteClave = visitas.getString("ClienteClave");
					vendedorID = visitas.getString("VendedorId");
					rutClave = visitas.getString("RUTClave");
					fechaHoraInicial = visitas.getDate("FechaHoraInicial");
					fechaHoraFinal = visitas.getDate("FechaHoraFinal");
					tiempoVisita = (fechaHoraFinal.getTime() - fechaHoraInicial.getTime()) / (60.0 * 1000.0);
					if(ultimaVisita && tiempoMin > 0 && tiempoVisita < tiempoMin)
					{
						ultimaVisita = false;
						continue;
					}
					if (!hayMovimientos(visitaClave)) {
						Visita visita = new Visita();
						visita.VisitaClave = visitaClave;
						visita.DiaClave = diaClave;
						BDVend.recuperar(visita);
						BDVend.eliminar(visita);
						//BDVend.eliminar(visita.VisitaHist.get(visita.VisitaHist.size() - 1));
						if (visita.GPSLeido){
							PuntoGPS puntoGPS = ConsultasPuntoGPS.obtenerPuntoGPS(visitaClave, diaClave);
							if (puntoGPS != null){
								BDVend.eliminar(puntoGPS);
							}
						}

						Agenda agenda = ConsultasAgenda.obtenerAgendaPorDiaClienteRutaVendedor(diaClave, rutClave, vendedorID, clienteClave);
						if (agenda != null && !ConsultasVisita.existenVisitas(diaClave, clienteClave))
						{
	...
	public static final class ConsultasImpuesto
	{
		...
		public static ISetDatos BuscarPorProductoYRango(String ProductoClave, Short TipoImpuesto, Date FechaActual,Float TRPSubtotal) throws Exception
		{
			String Consulta = "SELECT Impuesto.ImpuestoClave, Impuesto.Abreviatura, Impuesto.Jerarquia, Impuesto.TipoAplicacion, Impuesto.TipoValor, ImpuestoVig.Valor, Impuesto.ValidaRFC, ImpuestoVig.RedondeoDecimales, IFNULL(ImpuestoVig.RangoInferior,0) as RangoInferior, IFNULL(ImpuestoVig.RangoSuperior,0) as RangoSuperior FROM ProductoImpuesto ";
			Consulta += " INNER JOIN Impuesto ON ProductoImpuesto.ImpuestoClave = Impuesto.ImpuestoClave";
			Consulta += " INNER JOIN ImpuestoVig ON Impuesto.ImpuestoClave = ImpuestoVig.ImpuestoClave";
			Consulta += " WHERE ProductoImpuesto.ProductoClave = '" + ProductoClave + "' ";
			Consulta += " AND '" + Generales.getFormatoFecha(FechaActual, "yyyy-MM-dd'T'HH:mm:ss") + "'>=ImpuestoVig.FechaInicial AND '" + Generales.getFormatoFecha(FechaActual, "yyyy-MM-dd'T'HH:mm:ss") + "'<=ImpuestoVig.FechaFinal";
			Consulta += " AND ImpuestoVig.TipoImpuesto=" + Short.toString(TipoImpuesto);
			Consulta += " AND " +  TRPSubtotal + " between IFNULL(ImpuestoVig.RangoInferior,0) and IFNULL(ImpuestoVig.RangoSuperior,999999999) ";
			Consulta += " ORDER BY Impuesto.Jerarquia";
			return BDVend.consultar(Consulta);

		}
		public static boolean ExistenImpuestosConRango() throws Exception
		{
			ISetDatos datos = BDVend.consultar("SELECT count(*) as ImpConRango from ImpuestoVig where DATETIME('now')  between FechaInicial and FechaFinal and (IFNULL(RangoInferior,0)>0 or IFNULL(RangoSuperior,0)>0) ");
			boolean res = false;
			if (datos.moveToNext())
			{
				if (datos.getShort("ImpConRango") >0){
					res = true;
				}
			}
			datos.close();
	...
	public static final class ConsultasTransProd
	{
		...
		public static ISetDatos obtenerSaldoClienteEfectivo(String clienteClaves) throws Exception{
			...
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT IFNULL(CLI.Clave, '') AS Clave, IFNULL(CLI.RazonSocial, '') AS RazonSocial, IFNULL(SUM(Saldo - ROUND(CASE WHEN TRCP.AbnChequePosfechado IS NULL THEN 0 ELSE TRCP.AbnChequePosfechado END, 2) + ");
		...
		public static ISetDatos obtenerPreLiquidacionCreditosNIC(String diaClave, String claveCliente) throws Exception{

			StringBuilder consulta = new StringBuilder();
			consulta.append("select distinct ad.TipoPago,sum( abt.Importe ) as 'Importe' from Abono a  ")
					.append("inner join Visita v on a.VisitaClave = v.VisitaClave and v.DiaClave = '"+diaClave+"' ")
					.append("inner join Cliente c on v.ClienteClave = c.ClienteClave  " + (claveCliente != null?"and c.ClienteClave = '"+claveCliente+"'":""))
					.append("inner join ABNDetalle ad on a.ABNId = ad.ABNId and (ad.TipoPago = 1 or ad.TipoPago = 2 or ad.TipoPago = 10) ")
					.append("inner join AbnTrp abt on a.ABNId = abt.ABNId ")
					.append("inner join Dia d on v.DiaClave = d.DiaClave ")
					.append("inner join TransProd t on abt.TransProdId = t.TransProdId  and t.FechaCaptura != d.FechaCaptura and t.CFVTipo = '2' ")
					.append("group by ad.TipoPago order by ad.TipoPago asc ");
		...
		public static ISetDatos obtenerPreLiquidacionVentasNIC(String diaClave, String claveCliente, Integer Tipo) throws Exception{
			...
			consulta.append("select distinct ad.TipoPago,sum( abt.Importe ) as 'Importe',sum(t.Saldo) as 'Credito' from Abono a  ")
					...
					.append("inner join Dia d on v.DiaClave = d.DiaClave ");
			if(Tipo == 0 ){
				consulta.append("inner join TransProd t on abt.TransProdId = t.TransProdId  and t.FechaCaptura = d.FechaCaptura ");
			}
			else{
				consulta.append("inner join TransProd t on abt.TransProdId = t.TransProdId and t.diaclave1 = v.DiaClave ");
			}
			consulta.append("group by ad.TipoPago order by ad.TipoPago asc ");
		...
		public static ISetDatos obtenerSubEmpresas() throws Exception {
			...
			StringBuilder consulta = new StringBuilder();
			consulta.append("select SubEmpresaId, NombreEmpresa, RFC, Calle, Numero, Colonia, Ciudad, ifnull(NumeroInterior,''), Region from SubEmpresa");
...
//            return BDTerm.consultar(consulta.toString());
		...
		public static ISetDatos obtenerInventarioGeneral (String subEmpresaId, boolean inventarioCero) throws  Exception {
			String ordenTicketInvGen = "pro.ProductoClave";
			if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("OrdenTicketInvGen")) {
				ordenTicketInvGen = ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("OrdenTicketInvGen");
			}
			StringBuilder consulta = new StringBuilder();

			consulta.append("select pro.ProductoClave, pro.Nombre, pro.Id, inv.Disponible as Exis, (inv.Disponible - inv.NoDisponible - inv.Apartado - inv.Contenido) as Disp, (inv.NoDisponible + inv.Apartado) as 'No D', max(ifnull(ppv.Precio, 0)) as Precio ");
			...
			consulta.append("left join PrecioProductoVig ppv on pru.ProductoClave = ppv.ProductoClave and pru.PRUTipoUnidad = ppv.PRUTipoUnidad and datetime('now') BETWEEN ppv.PPVFechaInicio and ppv.FechaFin ");
			...
			consulta.append("order by " + ordenTicketInvGen);
        ...
        public static ISetDatos obtenerInventarioDetallado(String subEmpresaId, boolean inventarioCero) throws Exception{
			...
			StringBuilder consulta = new StringBuilder();
			if (((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase != null && ((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase.length()>0) {
				consulta.append("select pro.ProductoClave, pro.Nombre, pro.Id, inv.Disponible as Exis, (inv.Disponible - inv.NoDisponible - inv.Apartado - inv.Contenido) as Disp, (inv.NoDisponible + inv.Apartado) as 'No D', pru.PRUTipoUnidad, prd.Factor, max(ifnull(ppv.Precio, 0)) as Precio ");
			}else{
				consulta.append("select pro.ProductoClave, pro.Nombre, pro.Id, inv.Disponible as Exis, (inv.Disponible - inv.NoDisponible - inv.Apartado - inv.Contenido) as Disp, (inv.NoDisponible + inv.Apartado) as 'No D', pru.PRUTipoUnidad, prd.Factor, 0 as Precio ");
			}
			consulta.append("from Inventario inv ");
			consulta.append("inner join Producto pro on inv.ProductoClave = pro.ProductoClave ");
			consulta.append("inner join ProductoUnidad pru on pro.ProductoClave = pru.ProductoClave ");
		...
		public static SeleccionarPedidoBackOrder.VistaPedidos[] obtenerPedidosBackOrder(Visita visita, Cliente cliente) throws Exception
		{
			ArrayList<SeleccionarPedidoBackOrder.VistaPedidos> pedidos = new ArrayList<SeleccionarPedidoBackOrder.VistaPedidos>();
			StringBuilder consulta = new StringBuilder();
			ISetDatos fases = Consultas.ConsultasValorReferencia.obtenerValoresReferencia("TRPFASE", "");
			consulta.append("select TRP.TransprodID, TVA.FolioEntrega, TRP.TipoFase, case when TRP.TipoPedido = 1 then TRP.DiaClave1 else TRP.DiaClave end as Fecha, ");
			consulta.append("0, '', TRP.TipoPedido, case when TRP.TipoPedido = 1 then TRP.Folio else '' end as FolioPedido ");
			consulta.append("from TransProd TRP ");
			consulta.append("inner join visita VIS on VIS.VisitaClave = TRP.VisitaClave AND VIS.DiaClave = TRP.DiaClave and VIS.ClienteClave = '" + cliente.ClienteClave + "'");
			consulta.append("left join TRPVtaAcreditada TVA on TRP.TransprodID = TVA.TransprodID ");
			consulta.append("where TRP.Tipo = 1 and (TRP.TipoPedido = 9 or (TRP.TipoPedido = 1 and TRP.VisitaClave1 = '" + visita.VisitaClave + "' and TRP.DiaClave1 = '" + visita.DiaClave + "')) ");

			ISetDatos datos = BDVend.consultar(consulta.toString());
			String faseConfirmado = Mensajes.get("XConfirmado");
			String fasePedido = "";
			String folioConfirmado = "";
			while (datos.moveToNext())
			{
				folioConfirmado = datos.getString(7);
				if (!folioConfirmado.equals(""))
					fasePedido = faseConfirmado;
				else {
					int fase = datos.getInt(2) + 1;
					fases.moveToPosition(fase - 1);
					fasePedido = fases.getString(2);
				}
				SeleccionarPedidoBackOrder.VistaPedidos pedido = new SeleccionarPedidoBackOrder.VistaPedidos(datos.getString(0), datos.getString(1), datos.getInt(2), fasePedido, datos.getString(3), datos.getInt(4), datos.getString(5), datos.getInt(6), folioConfirmado);
				pedidos.add(pedido);
			}
			...
			return pedidos.toArray(new SeleccionarPedidoBackOrder.VistaPedidos[pedidos.size()]);
		}

		public static SeleccionarPedidoBackOrder.VistaPedidos[] ObtenerDetallesBackOrder(String TransprodID) throws Exception
		{
			ArrayList<SeleccionarPedidoBackOrder.VistaPedidos> pedidos = new ArrayList<SeleccionarPedidoBackOrder.VistaPedidos>();
			StringBuilder consulta = new StringBuilder();
			ISetDatos unidades = Consultas.ConsultasValorReferencia.obtenerValoresReferencia("UNIDADV", "");
			consulta.append("select TPD.TransprodID, PRO.ProductoClave, TPD.TipoUnidad, TPD.MFechaHora as Fecha, TPD.Cantidad, PRO.Nombre ");
			...
			while (datos.moveToNext())
			{
				...
				unidades.moveToPosition(fase - 1);
				//public VistaPedidos(String transprodid, String folio, int tipofase, String fase, String fecha, int cantidad, String descripcion, String folioConfirmado)
				SeleccionarPedidoBackOrder.VistaPedidos pedido = new SeleccionarPedidoBackOrder.VistaPedidos(datos.getString(0), datos.getString(1), datos.getInt(2), unidades.getString(2), datos.getString(3), datos.getInt(4), datos.getString(5));
				pedidos.add(pedido);
			}
		...
		public static int obtenerFacturasVencidas(Cliente oCliente) throws Exception
		{
			StringBuilder consulta = new StringBuilder();
			int resultado = 0;
			short tipoMov =0;
...
//				sConsulta.append("AND Visita.DiaClave='" + diaClave + "'");
		...
		public static SeleccionarConsignacion.VistaConsignacion[] obtenerConsignacionesPorCliente(Cliente cliente) throws Exception
		{
			...
			consulta.append("and VIS.ClienteClave = '" + cliente.ClienteClave + "'");
		...
		public static ISetDatos obtenerDiferenciaEnvase(String sTransprodIds) throws Exception {
			...
			consulta.append("(sum(VentaTotalLiquido)-sum(VentaTotalEnvase)) as Recolectar, DecimalProducto from ( ");
			...
			consulta.append("union all ");
			consulta.append("select pd.ProductoDetClave,td.TipoUnidad, 0 as VentaTotalLiquido, SUM(Cantidad) as VentaTotalEnvase, 0 as Recolectar, pr.DecimalProducto ");
			consulta.append("from TransProdDetalle td ");
			consulta.append("inner join ProductoDetalle pd on td.ProductoClave=pd.ProductoClave and td.TipoUnidad=pd.PRUTipoUnidad and td.ProductoClave=pd.ProductoDetClave and pd.Prestamo=1 ");
			consulta.append("inner join Producto pr on pd.ProductoDetClave=pr.ProductoClave ");
			consulta.append("where td.TransProdID in (" + sTransprodIds + ") ");
		...
		public static ISetDatos obtenerEnvaseRecAutomatica(String sTransprodId) throws Exception {
			StringBuilder consulta = new StringBuilder();

			consulta.append("select pd.ProductoDetClave, pd2.PRUTipoUnidad as TipoUnidad, sum(tpd.Cantidad * pd.Factor) as Recolectar ");
			consulta.append("from TransProdDetalle tpd ");
			consulta.append("inner join ProductoDetalle pd on tpd.ProductoClave= pd.Productoclave and tpd.ProductoClave <> pd.ProductoDetClave and tpd.TipoUnidad = pd.PRUTipoUnidad and pd.Prestamo=1 ");
			consulta.append("inner join ProductoDetalle pd2 on pd.ProductoDetClave = pd2.ProductoClave and pd.ProductoDetClave = pd2.ProductoDetClave and pd.PRUTipoUnidad= pd2.PRUTipoUnidad and pd2.Prestamo=1 ");
			consulta.append("where tpd.TransProdId='" + sTransprodId + "' ");

			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerPedidosConDevolucionEnvasePendiente() throws Exception {
			StringBuilder consulta = new StringBuilder();
			consulta.append("select distinct trp.TransProdId ");
			consulta.append("from transprod trp ");
			consulta.append("inner join transproddetalle tpd on trp.transprodid=tpd.TransProdID ");
			consulta.append("inner join productodetalle prd on tpd.productoclave=prd.productoclave and prd.productoclave<>prd.productodetclave  and prd.Prestamo=1  ");
			consulta.append("where trp.tipo=1 and  trp.TipoFase = 2 and  trp.devolucionid is null ");
			consulta.append("and ((DiaClave in(Select DiaClave from Dia where FueraFrecuencia = 0)) or (DiaClave1 in(Select DiaClave from Dia where FueraFrecuencia = 0))) ");
		...
		public static ISetDatos obtenerDiasFrecuencia() throws Exception{
			...
			StringBuilder consulta = new StringBuilder();
			consulta.append("select D.DiaClave from Dia D where D.FueraFrecuencia = 0 and date( substr(D.DiaClave, 7, 4) || '-' || substr(D.DiaClave, 4, 2) || '-' || substr(D.DiaClave, 1, 2) ) <= date('now', 'localtime')");
        ...
        public static ISetDatos obtenerCobranzaPorCliente(String clienteClave) throws Exception{
            ...
            StringBuilder consulta = new StringBuilder();
            consulta.append("select distinct c.Clave, ifnull(c.NombreCorto, c.RazonSocial) as NombreCorto, t.TransProdID,t.Folio,t.FechaCaptura,t.Total as Importe,t.Saldo from TransProd t ")
                    .append("inner join Visita v on t.VisitaClave = v.VisitaClave ")
                    .append("inner join Cliente c on v.ClienteClave = c.ClienteClave and c.ClienteClave = '" + clienteClave + "' ")
        ...
        public static ISetDatos obtenerResumenCobranzaPorCliente(String diaClave, String clienteClave) throws Exception{
            ...
            StringBuilder consulta = new StringBuilder();
            consulta.append("")
		            .append("SELECT c.Clave, ifnull(c.NombreCorto, c.RazonSocial) as NombreCorto, t.Folio, t.CFVTipo AS FormaVenta , t.Total AS Importe, t.Saldo, SUM(at.Importe)as Abono ")
		            .append("FROM Abono a ")
		            .append("INNER JOIN Visita v ON a.VisitaClave = v.VisitaClave  and v.DiaClave = '" + diaClave + "' ")
		            .append("INNER JOIN Cliente c ON v.ClienteClave = c.ClienteClave and c.ClienteClave = '" + clienteClave + "' ")
		...
		public static ISetDatos obtenerDetallesTicketGuacamaya(String transProdId, int tipo) throws Exception{
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("Select TPD.TransProdID, TPD.TransProdDetalleID, PRO.ProductoClave, PRO.NombreLargo, TPD.Cantidad, TPD.TipoUnidad, TPD.Precio,TPD.Precio * TPD.Cantidad as Importe, ifnull(DES.Nombre, '') as DESNombre, TPD.DescuentoPor, TPD.Subtotal, ifnull(TPD.Promocion, 0) as Promocion ");
			sConsulta.append("from TransProd TRP ");
			sConsulta.append("inner join TransProdDetalle TPD on TRP.TransProdID = TPD.TransProdID ");
			sConsulta.append("inner join Producto PRO ON TPD.ProductoClave = PRO.ProductoClave ");
			sConsulta.append("left join Descuento DES ON TPD.DescuentoClave= DES.DescuentoClave ");
			sConsulta.append("where TRP.Tipo = 1 and " + (tipo == 1 ? "TRP.TransProdID" : "TRP.FacturaID") + " = '" + transProdId + "' and TPD.Cantidad > 0 ");
			sConsulta.append("order by TPD.ProductoClave, TPD.TipoUnidad ");
			return BDVend.consultar(sConsulta.toString());
		}

		public static ISetDatos obtenerPromosDetTicketGuacamaya(String transProdId, String transProdDetalleId) throws Exception{
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("select PRM.Nombre, ifnull(TrpPrp.PromocionImp, '') as PromocionImp, PRM.TipoAplicacion ");
			sConsulta.append("from TrpPrp ");
			sConsulta.append("inner join Promocion PRM on PRM.PromocionClave = TrpPrp.PromocionClave ");
			sConsulta.append("where TransProdID = '" + transProdId + "' and TransProdDetalleID = '" + transProdDetalleId + "' ");
			return BDVend.consultar(sConsulta.toString());
		}

		public static ISetDatos obtenerDescuentosTicketGuacamaya(String transProdId, int tipo) throws Exception{
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("select DES.Nombre,sum(TDE.DesImporte) as DesImporte ");
			sConsulta.append("from TransProd TRP ");
			sConsulta.append("inner join TpdDes TDE on TRP.TransProdID = TDE.TransProdId ");
			sConsulta.append("inner join Descuento DES on DES.DescuentoClave = TDE.DescuentoClave ");
			sConsulta.append("where TRP.Tipo = 1 and " + (tipo == 1 ? "TRP.TransProdID" : "TRP.FacturaID") + " = '" + transProdId + "' ");
			sConsulta.append("group by DES.DescuentoClave, DES.Nombre,TDE.Jerarquia ");
			sConsulta.append("order by TDE.Jerarquia ");
		...
		public static ISetDatos obtenerDetalle(String transProdId) throws Exception {
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT TPD.TransProdDetalleID, TPD.ProductoClave, TPD.TipoUnidad, TPD.Cantidad ");
			consulta.append("FROM TransProdDetalle TPD ");
			consulta.append("WHERE TPD.TransProdId = '" + transProdId + "' ");
			return BDVend.consultar(consulta.toString());
		}
	...
	public static final class ConsultasTransProdDetalle
	{
		...
		public static ISetDatos obtenerDetalle(String transProdId) throws Exception
		{
			...
			consulta.append("case when INV.Disponible is null then 0 else CAST(((INV.Disponible-INV.Apartado-INV.NoDisponible-INV.Contenido)/PDE.Factor) as float) end as Existencia, PRO.DecimalProducto,");
		...
		public static ISetDatos obtenerDetalleLotes(String transProdId) throws Exception
		{
			// obtener todos los productos, excepto aquellos que se otorgaron
			// como promocion
			String clienteClave = ((Cliente)Sesion.get(Campo.ClienteActual)).ClienteClave;
			...
			consulta.append("PRO.Venta, PRO.Contenido, P.Nombre, PRO.Tipo as TipoProducto, P.PrecioClave, LOT.Lote, strftime('%d/%m/%Y', LOT.Caducidad) as Caducidad ");
			consulta.append("from TransProdDetalle TPD inner join Producto PRO on PRO.ProductoClave = TPD.ProductoClave ");
			consulta.append("inner join productodetalle PDE on PRO.ProductoClave = PDE.ProductoClave AND PDE.ProductoDetClave = PRO.ProductoClave AND TPD.TipoUnidad = PDE.PRUTipoUnidad ");
			consulta.append("left join TPDDatosExtra TPE on TPD.TransProdID = TPE.TransProdID and TPD.TransProdDetalleID = TPE.TransProdDetalleID ");
			consulta.append("left join InventarioLotes INV on PRO.ProductoClave = INV.ProductoClave AND PDE.PRUTipoUnidad = INV.TipoUnidad and TPE.Lote = INV.Lote ");
			consulta.append("left join LoteCaducidad LOT ON INV.Lote = LOT.Lote ");
			consulta.append("left join Precio P on TPE.PrecioClave = P.PrecioClave ");
			// consulta.append("where TPD.TransProdID in (" + transProdId +
			// ") AND TPD.Total > 0 AND TPD.Promocion != 2 order by TPD.ProductoClave");
		...
		public static ISetDatos obtenerDetalleCarga(String transProdId) throws Exception
		{
			StringBuilder consulta = new StringBuilder();
			consulta.append("Select TPD.TransProdDetalleID as _id, TPD.TransProdID as mTransProdID, TPD.ProductoClave as ProductoClave, TPD.Cantidad as Cantidad, PRO.Nombre as Descripcion, TPD.TipoUnidad as TipoUnidad, TPD.Total as Total, IFNULL(TDE.Factor,0) as Factor ");
			consulta.append("from TransProdDetalle TPD inner join Producto PRO on PRO.ProductoClave = TPD.ProductoClave ");
			consulta.append("inner join productodetalle PDE on PRO.ProductoClave = PDE.ProductoClave AND PDE.ProductoDetClave = PRO.ProductoClave AND TPD.TipoUnidad = PDE.PRUTipoUnidad ");
			consulta.append("left join inventario INV on PRO.ProductoClave = INV.ProductoClave AND INV.AlmacenID = '" + ((Vendedor) Sesion.get(Campo.VendedorActual)).AlmacenID + "' ");
			consulta.append("left join TPDDatosExtra TDE on TPD.TransProdID = TDE.TransProdID and TPD.TransProdDetalleID = TDE.TransProdDetalleID ");
			// consulta.append("where TPD.TransProdID in (" + transProdId +
			// ") AND TPD.Total > 0 AND TPD.Promocion != 2 order by TPD.ProductoClave");
			consulta.append("where TPD.TransProdID in ('" + transProdId + "')  order by TPD.ProductoClave");
			return BDVend.consultar(consulta.toString());
		...
		public static ISetDatos obtenerDetalle_MovDobleUnidad(String transProdId) throws Exception
		{
			...
			consulta.append("PRU.DecimalProducto as DecimalProducto, IFNULL(TDE.UnidadAlterna,0) as UnidadAlterna, IFNULL(TDE.CantidadAlterna,0) as CantidadAlterna, IFNULL(PRU2.DecimalProducto,0) as DecimalProductoAlterna, TPD.Total as Total ");
			consulta.append("from TransProdDetalle TPD inner join Producto PRO on PRO.ProductoClave = TPD.ProductoClave ");
		...
		{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT TransProdID, TransProdDetalleId, Cantidad, TipoMotivo, ifnull(CantidadOriginal, Cantidad) as CantidadOriginal " +
					"FROM TransProdDetalle " +
					"WHERE ProductoClave = '" + productoClave + "' AND TipoUnidad = " + tipoUnidad + " AND TransProdId in (" + transProdsId + ") AND (IFNULL(Promocion,0) <> 2) ");
			ISetDatos datos = BDVend.consultar(consulta.toString());
			Object res[];
			if (datos.getCount() > 0)
			{
				...
				res[4] = datos.getFloat("CantidadOriginal");
		...
		public static Object[] obtenerDetallePorProductoClaveUnidadFactor(String productoClave, String tipoUnidad, String transProdsId, float Factor) throws Exception
		{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT TPD.TransProdID, TPD.TransProdDetalleId,TPD.Cantidad,TPD.TipoMotivo, IFNULL(TDE.Factor,0)as Factor FROM TransProdDetalle TPD left join TPDDatosExtra TDE on TPD.TransProdID = TDE.TransProdID and TPD.TransProdDetalleID = TDE.TransProdDetalleID WHERE TPD.ProductoClave = '" + productoClave + "' AND TPD.TipoUnidad = " + tipoUnidad + " AND TPD.TransProdId in (" + transProdsId + ") AND (IFNULL(TPD.Promocion,0) <> 2) AND IFNULL(TDE.Factor,0)=" + Factor);
			ISetDatos datos = BDVend.consultar(consulta.toString());
			Object res[];
			if (datos.getCount() > 0)
			{
		...
		public static float obtenerCantidadOriginal(String transProdId, String transProdDetalleId) throws Exception
		{
			float cantidad = 0;
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT ifnull(CantidadOriginal, Cantidad) as CantidadOriginal FROM TransProdDetalle WHERE TransProdId = '" + transProdId + "' and TransProdDetalleId = '" + transProdDetalleId + "' ");
			ISetDatos datos = BDVend.consultar(consulta.toString());
			datos.moveToNext();
			// if(datos.getCount() > 0)
			cantidad = datos.getFloat("CantidadOriginal");
			datos.close();
			return cantidad;
		}
		...
		public static ISetDatos obtenerDetallesTicketCostena(String transProdId) throws Exception{
			...
			consulta.append("(TPD.total - ifnull((select sum(DesImporte) from TpdDes TDD where TPD.TransProdId = TDD.TransProdId and TPD.TransProdDetalleId = TDD.TransProdDetalleId),0)) / TPD.Cantidad as Precio, ");
			consulta.append("ifnull(( ");
			consulta.append("SELECT group_concat(CASE WHEN IMP.Abreviatura = 'IEPS' THEN 'C' ELSE CASE WHEN IMP.Abreviatura = 'IVA' THEN 'I' END END) as Impuesto ");
			consulta.append("FROM TPDImpuesto TPDI ");
			consulta.append("INNER JOIN Impuesto IMP on IMP.ImpuestoClave = TPDI.ImpuestoClave ");
			consulta.append("WHERE TPD.TransProdId = TPDI.TransProdId and TPD.TransProdDetalleId = TPDI.TransprodDetalleId ");
			consulta.append("),'') as Impuesto ");
			consulta.append("FROM TransprodDetalle TPD ");
			consulta.append("inner join Producto PRO on TPD.ProductoClave = PRO.ProductoClave ");
			consulta.append("WHERE TPD.TransProdID = '" + transProdId + "' ");
...
//			return datos;
		...
		public static TransProdDetalle obtenerTPDProductoUnidadLote(String transProdid, String productoClave, int tipoUnidad, String lote) throws Exception
		{
			...
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT TPD.TransProdID, TPD.TransProdDetalleID  ").
					append("from TransProdDetalle TPD ").
					append("inner join TPDDatosExtra TDE on TPD.TransProdID = TDE.TransProdID and TPD.TransProdDetalleID = TDE.TransProdDetalleID  ").
					append("WHERE TPD.TransProdID in(").append(transProdid).append(") and TPD.ProductoClave='" + productoClave + "' and TPD.TipoUnidad=" + String.valueOf(tipoUnidad) + " and TDE.Lote='" + lote + "' and IFNULL(TPD.Promocion,0)<>2 ");

			ISetDatos sdTpd =  BDVend.consultar(consulta.toString());
			if (sdTpd.moveToNext()){
				otpd.TransProdID = sdTpd.getString("TransProdID");
				otpd.TransProdDetalleID = sdTpd.getString("TransProdDetalleID");
				BDVend.recuperar(otpd);
			}
			sdTpd.close();
	...
	public static final class ConsultasCargas
	{
		...
		public static ISetDatos obtenerDetalleCargasPorTransferir()
		{
			...
			try
			{
				if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("InventarioPorLotes") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("InventarioPorLotes").equals("1")) {
					Consulta = "select distinct TPD.ProductoClave, TPD.TipoUnidad, TDE.Lote, sum(TPD.Cantidad) as Cantidad, PRO.DecimalProducto ";
					Consulta += "from TransProd TRP ";
					Consulta += "inner join TransProdDetalle TPD ON TRP.TransProdId = TPD.TransProdId and TRP.Tipo = 2 and TRP.TipoFase = 5 ";
					Consulta += "inner join Producto PRO on TPD.ProductoClave = PRO.ProductoClave ";
					Consulta += "inner join TPDDatosExtra TDE on TPD.TransProdID = TDE.TransProdID and TPD.TransProdDetalleID = TDE.TransProdDetalleID ";
					Consulta += "group by TPD.ProductoClave, TPD.TipoUnidad, TDE.Lote, PRO.DecimalProducto ";
				}
				else {
					Consulta = "Select distinct TPD.ProductoClave, TPD.TipoUnidad, sum(TPD.Cantidad) as Cantidad, PRO.DecimalProducto ";
					Consulta += "from TransProd ";
					Consulta += "TP Inner Join TransProdDetalle TPD ON TP.TransProdId=TPD.TransProdId and TP.Tipo=2 and TP.TipoFase=5 and TP.MUsuarioId='" + ((Vendedor) Sesion.get(Campo.VendedorActual)).USUId + "' ";
					Consulta += "Inner join Producto PRO on TPD.ProductoClave = PRO.ProductoClave ";
					Consulta += "group by TPD.ProductoClave, TPD.TipoUnidad, PRO.DecimalProducto ";
				}
				return BDVend.consultar(Consulta);
			}
			catch (Exception e)
		...
		public static ISetDatos obtenerDetalleInventarioABordo()
		{
			...
			{
				if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("InventarioPorLotes") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("InventarioPorLotes").equals("1")) {
					Consulta = "Select distinct TPD.TransProdId, TPD.TransProdDetalleId, TPD.ProductoClave, TPD.TipoUnidad, TDE.Lote, sum(TPD.Cantidad) as Cantidad, PRO.DecimalProducto ";
					Consulta += "from TransProd TP ";
					Consulta += "Inner Join TransProdDetalle TPD ON TP.TransProdId=TPD.TransProdId and TP.Tipo=23 and TP.TipoFase<>0 and TPD.Cantidad >0 and TP.MUsuarioId='" + ((Vendedor) Sesion.get(Campo.VendedorActual)).USUId + "' ";
					Consulta += "Inner join Producto PRO on TPD.ProductoClave = PRO.ProductoClave ";
					Consulta += "inner join TPDDatosExtra TDE on TPD.TransProdID = TDE.TransProdID and TPD.TransProdDetalleID = TDE.TransProdDetalleID ";
					Consulta += "group by TPD.ProductoClave, TPD.TipoUnidad, TDE.Lote, PRO.DecimalProducto ";
				}else
				{
					Consulta = "Select distinct TPD.TransProdId, TPD.TransProdDetalleId, TPD.ProductoClave, TPD.TipoUnidad, sum(TPD.Cantidad) as Cantidad, PRO.DecimalProducto ";
					Consulta += "from TransProd TP ";
					Consulta += "Inner Join TransProdDetalle TPD ON TP.TransProdId=TPD.TransProdId and TP.Tipo=23 and TP.TipoFase<>0 and TPD.Cantidad >0 and TP.MUsuarioId='" + ((Vendedor) Sesion.get(Campo.VendedorActual)).USUId + "' ";
					Consulta += "Inner join Producto PRO on TPD.ProductoClave = PRO.ProductoClave ";
					Consulta += "group by TPD.ProductoClave, TPD.TipoUnidad, PRO.DecimalProducto ";
...
/*			sConsulta.append("select distinct PRM.PromocionClave, PRE.ProductoClave, 0 as Jerarquia from Promocion PRM, ProductoEsquema PRE");
		...
		public static PromocionDetalle[] obtenerVistaDetalle(Promocion promocion, Integer cantidadGrupo, String productosDisparadores) throws Exception
		{
			...
			consulta.append("select PRA.PromocionClave as PromocionClave,PRO.ProductoClave as ProductoClave, PRO.Nombre as Nombre, PRA.PRUTipoUnidad as PRUTipoUnidad, (PRA.Cantidad * " + cantidadGrupo + ") as Cantidad, PRO.DecimalProducto, PDE.Factor ");
			if(inventarioPorLotes)
				consulta.append(", IL.Lote, strftime('%d/%m/%Y',LC.Caducidad) as Caducidad, ");
			else
				consulta.append(", '' as Lote, '' as Caducidad, ");
			consulta.append("PRA.Precio ");
			consulta.append("from Producto PRO ");
			consulta.append("inner join PromocionAplicacion PRA on PRO.ProductoClave = PRA.ProductoClave ");
			consulta.append("and PRA.PromocionClave = '" + promocion.PromocionClave + "' and PRA.PromocionReglaId='" + promocion.PromocionReglaIdApp + "' ");
			...
			if(inventarioPorLotes) {
				consulta.append("inner join InventarioLotes IL on IL.ProductoClave = PRA.ProductoClave and PRA.PRUTipoUnidad = IL.TipoUnidad and (IL.Disponible - IL.Apartado - IL.NoDisponible - IL.Contenido) >= (PRA.Cantidad * " + cantidadGrupo +")");
				consulta.append("inner join LoteCaducidad LC on IL.Lote = LC.Lote ");
			}
			ISetDatos datos = BDVend.consultar(consulta.toString());
			while (datos.moveToNext())
			{
				...
				PromocionDetalle pedido = new PromocionDetalle(datos.getString("PromocionClave"),
						...
						datos.getString("Lote"),
						datos.getString("Caducidad"),
		...
		public static String recuperarTransProdDetalleIDPromocion(String transProdID, String productoClave, int tipoUnidad, String promocionClave, String Lote) throws Exception{
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("Select TPD.TransProdDetalleID ");
			sConsulta.append("from TransProdDetalle TPD ");
			sConsulta.append("inner join TPDDatosExtra TPE on TPD.TransProdID ='" + transProdID + "' and TPD.ProductoClave = '" + productoClave + "' ");
			sConsulta.append("and TPD.TipoUnidad=" + tipoUnidad + " and TPD.PromocionClave='" + promocionClave + "' ");
			sConsulta.append("and TPE.TransProdDetalleID = TPD.TransProdDetalleID and TPE.Lote = '" +Lote+ "' ");
			return BDVend.bd.ejecutarEscalar(sConsulta.toString());
		}
	...
	public static final class ConsultasProductoNegado
	{
		...
		public static String obtenerProductoNegadoXPromocion(String transProdId, String promocionClave){
			try{
				String productoNegado = "";
				ISetDatos prodNegado = BDVend.consultar("select ProductoClave, Cantidad, TipoUnidad from ProductoNegado where TransProdId = '" + transProdId + "' and PromocionClave = '" + promocionClave + "'");
				if (prodNegado.getCount()>0) {
					while (prodNegado.moveToNext()) {
						productoNegado += prodNegado.getString("ProductoClave") + " (" + prodNegado.getString("Cantidad") + " " + ValoresReferencia.getDescripcion("UNIDADV", prodNegado.getString("TipoUnidad")) + ") " + "\n";
					}
					//productoNegado = productoNegado.substring(0, productoNegado.length()-2);
				}
	...
	public static final class ConsultasTPDDesVendedor
	{
		...
		public static ISetDatos obtenerTPDDesVendedor(String TransProdId, String TransProdDetalleId) throws Exception
		{
			return BDVend.consultar("select * from TPDDesVendedor where TransProdID = '" + TransProdId + "' and TransProdDetalleID = '" + TransProdDetalleId + "'");
		}
	}
	...
	public static final class ConsultasCuotas
	{
		...
		public static ISetDatos obtenerCuotaCumplida(String cuoClave, String vendedorID) throws Exception
		{
			return BDVend.consultar("SELECT * FROM CuotaCumplida WHERE VendedorID = '" + vendedorID + "' and CuoClave='" + cuoClave + "'");
		}
		...
		public static ISetDatos obtenerCUOEsquemasPorEsquemas(String CUOClave) throws Exception
		{
			return BDVend.consultar("SELECT DISTINCT EsquemaId FROM CUOEsquema WHERE CUOClave = '" + CUOClave + "'");
		}
		...
		public static ISetDatos obtenerCUOProductoPorProductoClaveTipo(String CUOClave) throws Exception
		{
			return BDVend.consultar("SELECT CUOClave, ProductoClave FROM CUOProducto WHERE CUOClave = '" + CUOClave + "'");
		}
		...
		public static ISetDatos obtenerCUOClientePorClienteClaveTipo(String CUOClave, String ClienteClave) throws Exception
		{
			return BDVend.consultar("SELECT CUOCliente.* FROM CUOCliente WHERE CUOClave = '" + CUOClave + "' AND ClienteClave = '" + ClienteClave + "'");
		}
		...
		public static ISetDatos obtenerUnidadReinicio(String CUOClave) throws Exception
		{
			StringBuilder consulta = new StringBuilder();
			consulta.append("Select D.FechaCaptura ");
			consulta.append("from Dia D ");
			consulta.append("inner join Cuota C on D.Frecuencia = C.UnidadReinicio and C.CUOClave = '" + CUOClave + "' ");
			return BDVend.consultar(consulta.toString());
		}
	}
	...
	public static final class ConsultasAbono
	{
		...
		public static Cobranza.VistaDocumentos[] obtenerDocumentosEnCobranza(String abnid) throws Exception
		{
			...
			consulta.append("from ( select TRP.TransProdId, TRP.Folio, case when TRP.Tipo = 1 or TRP.Tipo = 24 then ifnull(TRP.FechaSurtido, TRP.FechaCaptura) else FechaFacturacion end as Fecha, Total, Saldo, ");
			consulta.append("(select sum(Total) from TrpTpd TTP where TTP.TransProdId1 = TRP.TransProdId) as DevolucionCons, ifnull(TRP.FechaCobranza, case when TRP.Tipo = 1 or TRP.Tipo = 24 then TRP.FechaCaptura else TRP.FechaFacturacion end) as FechaCobranza, TRP.Notas  ");
			if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("AplicarDescProntoPago") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("AplicarDescProntoPago").equals("1")) {
			...
			while (datos.moveToNext())
			{
				Cobranza.VistaDocumentos documento = new Cobranza.VistaDocumentos(datos.getString(0), datos.getString(1), datos.getDate(2), datos.getFloat(3), datos.getFloat(4), datos.getDate(5), (datos.getColumnIndex("DescProntoPago") >0 ? datos.getFloat(datos.getColumnIndex("DescProntoPago")):0 ), datos.getString(6));
		...
		public static Cobranza.VistaDocumentos[] obtenerDocumentosPorCobrar(String clienteClave, short tipoCobranza,String camposOrderBy) throws Exception
		{
			...
			consulta.append("(Round(case WHEN TRC.AbnChequePosfechado is null THEN 0 ELSE TRC.AbnChequePosfechado END,2)) as Cheque, ifnull(TRP.FechaCobranza, case when TRP.Tipo = 1 or TRP.Tipo = 24 then TRP.FechaCaptura else TRP.FechaFacturacion end) AS FechaCobranza, TRP.Notas ");
			if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("AplicarDescProntoPago") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("AplicarDescProntoPago").equals("1")) {
				consulta.append(", TDC.Porcentaje as Porc, (TRP.Saldo * TDC.Porcentaje)/100 as Importe ");
			}
			consulta.append(" from TransProd TRP ");
			consulta.append("inner join Visita VIS on TRP.VisitaClave = VIS.VisitaClave and TRP.DiaClave = VIS.DiaClave ");
			if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("AplicarDescProntoPago") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("AplicarDescProntoPago").equals("1")) {
        ...
        public static Cobranza.VistaDocumentos[] obtenerDocumentosPorCobrarPorSubEmpresa(String clienteClave, short tipoCobranza, String camposOrderBy, String sSubEmpresaId) throws Exception
        {
            ...
            consulta.append(" from ( select TRP.TransProdId, TRP.Folio, case when TRP.Tipo = 1 or TRP.Tipo = 24 then ifnull(TRP.FechaSurtido, TRP.FechaCaptura) else FechaFacturacion end as Fecha, Total, Saldo, ");
            consulta.append("(select sum(Total) from TrpTpd TTP where TTP.TransProdId1 = TRP.TransProdId) as DevolucionCons, ");
            consulta.append("(Round(case WHEN TRC.AbnChequePosfechado is null THEN 0 ELSE TRC.AbnChequePosfechado END,2)) as Cheque, ifnull(TRP.FechaCobranza,TRP.FechaCaptura) as FechaCobranza, TRP.Notas ");
            if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("AplicarDescProntoPago") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("AplicarDescProntoPago").equals("1")) {
                consulta.append(", TDC.Porcentaje as Porc, TDC.Importe ");
            }
            consulta.append(" from TransProd TRP ");
        ...
        public static float obtenerDescuentoProntoPago(ArrayList<String> transprodids) throws Exception
        {
			...
			consulta.append("select ifnull(sum(round((TRP.Saldo * TDC.Porcentaje) / 100, 2)), 0) as Descuento ");
		...
		{
			StringBuilder consulta = new StringBuilder();
			consulta.append("Select TransProd.TransprodId, ROUND(Saldo  - (CASE WHEN TRC.AbnChequePosfechado is null THEN 0 ELSE TRC.AbnChequePosfechado END),2) as Saldo, ifnull(TransProd.FechaCobranza, case when TransProd.Tipo = 1 or TransProd.Tipo = 24 then TransProd.FechaCaptura else TransProd.FechaFacturacion end) as FechaCobranza ");
		...
		public static ISetDatos obtenerDocumentosCriterio(ArrayList<String> transprodids, short tipoCobranza) throws Exception
		{
			...
			consulta.append("ROUND(Saldo  - (CASE WHEN TRC.AbnChequePosfechado is null THEN 0 ELSE TRC.AbnChequePosfechado END),2) as Saldo, ifnull(TransProd.FechaCobranza, " + (trueType == 1 ? "FechaCaptura" : "FechaFacturacion") + ") as FechaCobranza ");
			if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("AplicarDescProntoPago") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("AplicarDescProntoPago").equals("1")) {
                consulta.append(", TDC.Porcentaje as DescProntoPago, TDC.Importe as ImporteDescPP ");
			}
			consulta.append("from transprod ");
        ...
        public static void ActualizaDescProntoPagoAplicado(String sTransprodId, float nImporteDesc, boolean bAplicado) throws Exception
        {
            StringBuilder sConsulta = new StringBuilder();
            sConsulta.append("update TRPDescCalculadora ");
            sConsulta.append("set Importe=" + nImporteDesc + ", AplicadoCobranza=" + (bAplicado ? "1" : "0") + ", Enviado=0, ");
	...
	public static final class ConsultasInventario
	{
		...
		public static boolean ValidarInventarioRecoleccionAutomatica(TransProd pvTransprod, String sDevolucionIDs, String sTipo) throws Exception{
			ISetDatos dsDevolucion;
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("select d.ProductoClave ");
			sConsulta.append("from transproddetalle d ");
			sConsulta.append("inner join inventario i on d.productoclave= i.productoclave ");
			sConsulta.append("inner join ProductoDetalle PD on d.ProductoClave = PD.ProductoClave and d.ProductoClave = PD.ProductoDetClave and d.TipoUnidad = PD.PRUTipoUnidad ");

			if(sTipo.equals("Venta"))
				sConsulta.append("where (d.cantidad * PD.Factor) > (i.disponible - i.NoDisponible - i.Apartado - i.contenido) ");
			else
				sConsulta.append("where (d.cantidad * PD.Factor) > (i.disponible - i.Apartado - i.contenido) ");
			sConsulta.append("and transprodid in ('" + (!sDevolucionIDs.equals("") ? sDevolucionIDs : pvTransprod.DevolucionID) + "')");

			dsDevolucion = BDVend.consultar(sConsulta.toString());
			if (dsDevolucion.getCount() > 0){
				StringBuilder sProductos = new StringBuilder();
				while (dsDevolucion.moveToNext())
					sProductos.append(dsDevolucion.getString("ProductoClave") + ",");
				throw new Exception(sProductos.toString().substring(0, sProductos.toString().length() - 1));
			}
	...
	public static final class ConsultasInventarioLotes
	{
		public static ISetDatos obtenerProductoInventario(String productoClave, Integer tipoUnidad, String lote) throws Exception
		{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT ProductoDetalle.ProductoClave, ProductoDetalle.ProductoDetClave, ProductoDetalle.Factor, Producto.Venta as PROVenta, Producto.Contenido as PROContenido, ");
			consulta.append("INV.Disponible, INV.NoDisponible, INV.Apartado, INV.Contenido, INV.Pedido ");
			consulta.append("FROM ProductoDetalle ");
			consulta.append("inner join Producto on ProductoDetalle.ProductoClave = Producto.ProductoClave ");
			consulta.append("left join InventarioLotes INV on INV.ProductoClave = ProductoDetalle.ProductoDetClave and INV.TipoUnidad = ProductoDetalle.PRUTipoUnidad and INV.Lote = '" + lote + "'");
			consulta.append("WHERE ProductoDetalle.ProductoClave='" + productoClave + "' and ProductoDetalle.PRUTipoUnidad=" + tipoUnidad);
			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerInventarioPedido(String productoClave, Integer tipoUnidad, String lote) throws Exception
		{
			StringBuilder consulta = new StringBuilder();
			consulta.append("select INV.Pedido ");
			consulta.append("from ProductoDetalle PRD ");
			consulta.append("inner join InventarioLotes INV on INV.ProductoClave = PRD.ProductoClave and INV.ProductoClave = PRD.ProductoDetClave and INV.TipoUnidad = PRD.PRUTipoUnidad ");
			consulta.append("where INV.Lote = '" + lote + "' and INV.ProductoClave = '"+ productoClave +"' and INV.TipoUnidad = " + tipoUnidad + " ");
			return BDVend.consultar(consulta.toString());
		}

		public static float obtenerInventarioTotalUnitario (String subEmpresaId) throws  Exception {
			StringBuilder consulta = new StringBuilder();
			/*consulta.append("select distinct pv.ProductoClave, pv.Precio, inv.disponible, inv.Factor from PrecioProductoVig pv ")
					.append("inner join (select distinct pd.ProductoClave, pd.PRUTipoUnidad, i.Disponible, pd.Factor from inventario i ")
					.append("inner join Producto p on i.ProductoClave = p.ProductoClave and p.SubEmpresaId = '" + subEmpresaId + "' ")
				...
				consulta.append("select ifnull(sum(Precio * (Disponible * Factor)), 0) as TotalUnitario ");
				consulta.append("from ( ");
				consulta.append("select inv.ProductoClave, sum(inv.Disponible) as Disponible, prd.Factor, ifnull(ppv.Precio, 0) as Precio ");
				consulta.append("from InventarioLotes inv ");
				consulta.append("inner join Producto pro on inv.ProductoClave = pro.ProductoClave ");
				consulta.append("inner join ProductoUnidad pru on pro.ProductoClave = pru.ProductoClave ");
				consulta.append("inner join ( ");
				consulta.append("select prd.ProductoClave, prd.PRUTipoUnidad, min(prd.Factor) as Factor ");
				consulta.append("from ProductoDetalle prd ");
				consulta.append("inner join InventarioLotes inv on prd.ProductoClave = inv.ProductoClave and prd.ProductoClave = prd.ProductoDetClave ");
				consulta.append("group by prd.ProductoClave ");
				consulta.append(") as prd on pru.ProductoClave = prd.ProductoClave and pru.PRUTipoUnidad = prd.PRUTipoUnidad ");
				consulta.append("left join (select ProductoClave, PRUTipoUnidad, MAX(Precio) as Precio from PrecioProductoVig group by ProductoClave, PRUTipoUnidad) ppv on pru.ProductoClave = ppv.ProductoClave and pru.PRUTipoUnidad = ppv.PRUTipoUnidad ");
				consulta.append("where pro.SubEmpresaId = '" + subEmpresaId + "' ");
				consulta.append("group by inv.ProductoClave ");
				consulta.append(") as t ");
		...
		public static ListaInventarioLotes[] obtenerListaInventario()
		{
			ArrayList<ListaInventarioLotes> listInventario = null;
			try
			{
				StringBuilder query = new StringBuilder();
				//query.append("select i.ProductoClave, p.Nombre, pd.PRUTipoUnidad, i.disponible as Existencia,(i.disponible - i.nodisponible - i.Apartado -i.contenido - i.pedido) as Disponible, (i.nodisponible + i.Apartado + i.Pedido) as NoDisponible, i.NoDisponible as MalEstado, pu.kglts ");
				query.append("select i.ProductoClave, p.Nombre, pu.PRUTipoUnidad, i.Lote, l.Caducidad, i.disponible as Existencia, ");
				query.append("(i.disponible - i.nodisponible - i.Apartado - i.contenido) as Disponible, ");
				query.append("(i.nodisponible + i.Apartado) as NoDisponible, i.NoDisponible as MalEstado, pu.kglts ");
				query.append("from InventarioLotes i ");
				query.append("inner join Producto p on i.ProductoClave = p.ProductoClave ");
				query.append("inner join productounidad pu  on pu.productoclave = i.productoclave and pu.PRUTipoUnidad = i.TipoUnidad ");
				query.append("inner join LoteCaducidad l on i.Lote = l.Lote ");
				query.append("where  pu.TipoEstado = 1 and i.disponible > 0 ");
				query.append("order by i.ProductoClave, pu.PRUTipoUnidad, l.Caducidad ");
				ISetDatos dataQuery = BDVend.consultar(query.toString());
				listInventario = new ArrayList<ListaInventarioLotes>();

				String productoClave = "";
				int tipoUnidad = 0;
				ListaInventarioLotes itemInventory = new ListaInventarioLotes();
				boolean bNuevo = false;
				while (dataQuery.moveToNext())
				{
					if (!productoClave.equals(dataQuery.getString("ProductoClave")) || tipoUnidad != dataQuery.getInt("PRUTipoUnidad")){
						if(bNuevo)
							listInventario.add(itemInventory);
						itemInventory = new ListaInventarioLotes();
						itemInventory.productoClave = dataQuery.getString("ProductoClave");
						itemInventory.productoNombre = dataQuery.getString("Nombre");
						itemInventory.tipoUnidad = dataQuery.getInt("PRUTipoUnidad");
						itemInventory.detalles = new ArrayList<ConsultaInventario.ListaInventarioLotesDetalle>();
						bNuevo = true;
					}
					ConsultaInventario.ListaInventarioLotesDetalle det = new ConsultaInventario.ListaInventarioLotesDetalle();
					det.lote = dataQuery.getString("Lote");
					det.caducidad = Generales.getFormatoFecha(dataQuery.getDate("Caducidad"), "dd/MM/yy");
					det.existencia = dataQuery.getFloat("Existencia");
					det.disponible = dataQuery.getFloat("Disponible");
					det.noDisponible = dataQuery.getFloat("NoDisponible");
					det.malEstado = dataQuery.getFloat("MalEstado");
					det.kgLts = dataQuery.getFloat("KgLts");
					itemInventory.detalles.add(det);

					productoClave = dataQuery.getString("ProductoClave");
					tipoUnidad = dataQuery.getInt("PRUTipoUnidad");
				}
				if(bNuevo)
					listInventario.add(itemInventory);
			...
			if (listInventario.size() > 0)
			{
				return listInventario.toArray(new ListaInventarioLotes[listInventario.size()]);
			}
			else
				return null;
		}

		public static ISetDatos obtenerProductoInventarioLotes(String productoClave)throws Exception
		{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT ProductoDetalle.ProductoClave as _id, ProductoDetalle.ProductoClave, INV.TipoUnidad, INV.Lote, LC.Caducidad, ProductoDetalle.Factor, Producto.Venta as PROVenta, Producto.Contenido as PROContenido, ");
			consulta.append("INV.Disponible - INV.NoDisponible - INV.Apartado - INV.Contenido as Existencia ");
			consulta.append("FROM ProductoDetalle ");
			consulta.append("inner join Producto on ProductoDetalle.ProductoClave = Producto.ProductoClave ");
			consulta.append("inner join InventarioLotes INV on INV.ProductoClave = ProductoDetalle.ProductoDetClave and INV.TipoUnidad = ProductoDetalle.PRUTipoUnidad ");
			consulta.append("inner join LoteCaducidad LC on INV.Lote = LC.Lote and LC.ProductoClave = Producto.ProductoClave ");
			consulta.append("WHERE ProductoDetalle.ProductoClave='" + productoClave + "' ");
			return BDVend.consultar(consulta.toString());
		}
	}
	...
	public static final class ConsultaPreLiquidacion
	{
		public static void verificarMonto() throws Exception {
			...
			if (dsPreliquidacion.getCount() > 0){
				...
				consulta.append("select IFNULL(sum(abd.Importe),0) as Importe ");
				consulta.append("from Abono abn ");
	...
	public static final class ConsultasKilometraje
	{
		...
		public static CamionVendedor obtenerCamiondeVendedor() throws Exception
		{
			...
			consulta.append("where Enviado=0 ");
			consulta.append("order by FechaHoraInicial desc ");
			consulta.append("limit 1 ");
			ISetDatos datos = BDVend.consultar(consulta.toString());
			while (datos.moveToNext())
			{
	...
	public static final class ConsultaRegistroInicioFin
	{
		...
		public static boolean validarMovimientosPendientes(String tipo) throws Exception{
			boolean valido = false;
			StringBuilder consulta = new StringBuilder();

			consulta.append("SELECT TransProdID FROM TransProd WHERE ");
			consulta.append("TipoFase = 15 AND Tipo = " +tipo);
			ISetDatos datos = BDVend.consultar(consulta.toString());
			if(datos.getCount() > 0)
				valido = true;
		...
		public static String obtenerDiferenciasInventario() throws Exception
		{

			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT ProductoClave, SUM(Existencia) AS Existencia, (Select Disponible from Inventario where ProductoClave = PRODUCTOS.ProductoClave) As Inventario, SUM(Existencia) - (Select Disponible from Inventario where ProductoClave = PRODUCTOS.ProductoClave) as Diferencia ");
			consulta.append("FROM ( ");
			consulta.append("SELECT P.ProductoClave AS ProductoClave, ");
			consulta.append("SUM( ");
			consulta.append("CASE WHEN T.TipoFase = 1 AND T.Tipo = 23 THEN TPD.Cantidad * PD.Factor ELSE 0 END + ");
			consulta.append("CASE WHEN T.TipoFase = 1 AND T.Tipo = 2 THEN TPD.Cantidad * PD.Factor ELSE 0 END + ");
			consulta.append("CASE WHEN T.TipoFase = 1 AND T.Tipo = 3 THEN TPD.Cantidad * PD.Factor ELSE 0 END + ");
			consulta.append("CASE WHEN T.TipoFase = 1 AND T.Tipo = 9 AND T.TipoMovimiento = 1 THEN TPD.Cantidad * PD.Factor ELSE 0 END ");
			consulta.append(") - ");
			consulta.append("SUM( ");
			consulta.append("CASE WHEN T.TipoFase IN (2, 3) AND T.Tipo = 1 THEN TPD.Cantidad * PD.Factor ELSE 0 END + ");
			consulta.append("CASE WHEN T.TipoFase = 2 AND T.Tipo = 24 THEN TPD.Cantidad * PD.Factor ELSE 0 END + ");
			consulta.append("CASE WHEN T.TipoFase = 1 AND T.Tipo = 4 THEN TPD.Cantidad * PD.Factor ELSE 0 END + ");
			consulta.append("CASE WHEN T.TipoFase = 1 AND T.Tipo = 6 THEN TPD.Cantidad * PD.Factor ELSE 0 END + ");
			consulta.append("CASE WHEN T.TipoFase = 1 AND T.Tipo = 7 THEN TPD.Cantidad * PD.Factor ELSE 0 END + ");
			consulta.append("CASE WHEN T.TipoFase = 1 AND T.Tipo = 9 AND T.TipoMovimiento = 2 THEN TPD.Cantidad * PD.Factor ELSE 0 END ");
			consulta.append(") AS Existencia ");
			consulta.append("FROM TransProd T ");
			consulta.append("INNER JOIN TransProdDetalle TPD ON T.TransProdID = TPD.TransProdID ");
			consulta.append("INNER JOIN Producto P ON TPD.ProductoClave = P.ProductoClave ");
			consulta.append("INNER JOIN ProductoDetalle PD ON P.ProductoClave = PD.ProductoClave AND TPD.TipoUnidad = PD.PRUTipoUnidad AND PD.ProductoDetClave = P.ProductoClave ");
			consulta.append("WHERE ((T.Tipo IN (2, 3, 9, 1, 24, 4, 6, 7) AND T.DiaClave = '27/11/2020') OR (T.Tipo = 23)) AND T.TipoFase IN (1, 2, 3) ");
			consulta.append("GROUP BY P.ProductoClave ");
			consulta.append("UNION ");
			consulta.append("SELECT P.ProductoClave AS ProductoClave, ");
			consulta.append("SUM( ");
			consulta.append("CASE WHEN T.TipoFase = 1 AND T.Tipo = 23 THEN TPD.Cantidad * PD.Factor ELSE 0 END + ");
			...
			consulta.append("FROM TransProd T ");
			consulta.append("INNER JOIN TransProdDetalle TPD ON T.TransProdID = TPD.TransProdID ");
			consulta.append("INNER JOIN ProductoDetalle PD ON TPD.ProductoClave = PD.ProductoClave AND TPD.TipoUnidad = PD.PRUTipoUnidad AND PD.ProductoDetClave <> TPD.ProductoClave ");
			consulta.append("INNER JOIN Producto P ON PD.ProductoDetClave = P.ProductoClave ");
			consulta.append("WHERE ((T.Tipo IN (2, 3, 9, 1, 24, 4, 6, 7) AND T.DiaClave = '27/11/2020') OR T.Tipo = 23) AND T.TipoFase IN (1, 2, 3) ");
			consulta.append("GROUP BY P.ProductoClave ");
			consulta.append(") PRODUCTOS ");
			consulta.append("GROUP BY PRODUCTOS.ProductoClave ");

			String prod = "";
			ISetDatos datos = BDVend.consultar(consulta.toString());
			while (datos.moveToNext())
			{
				if(datos.getInt("Diferencia") != 0)
					prod += datos.getString("ProductoClave") + " ,";
			}
			datos.close();
	...
	public static final class ConsultasReparto
	{
		public static ISetDatos obtenerPedidos(){
			...
			{
				if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("InventarioPorLotes") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("InventarioPorLotes").equals("1")) {
					Consulta = "select t.Tipo, td.ProductoClave, td.TipoUnidad, te.Lote, sum(td.Cantidad) as Cantidad ";
					Consulta += "from TransProdDetalle td ";
					Consulta += "inner join TransProd t on t.TransProdID = td.TransProdID ";
					Consulta += "inner join TPDDatosExtra te on td.TransProdID = te.TransProdID and td.TransProdDetalleID = te.TransProdDetalleID ";
					...
					Consulta += "group by t.Tipo, td.ProductoClave, td.TipoUnidad, te.Lote ";
				}else {
					Consulta = "select t.Tipo, td.ProductoClave, td.TipoUnidad, sum(td.Cantidad) as Cantidad ";
					Consulta += "from TransProdDetalle td ";
					Consulta += "inner join TransProd t on t.TransProdID = td.TransProdID ";
					Consulta += "where (Tipo=1 or Tipo=24) and (TipoFase=1 or TipoFase=7) ";
					if (tiposPagoAnticipado.length() > 0) {
...
//	}
	...
	public static final class ConsultasStock{
		public static String obtenerUltimaVisita(String ClienteClave, boolean MSIEV) throws Exception{
			...
			consulta.append("WHERE V.ClienteClave = '" + ClienteClave + "' AND ");
			if(MSIEV){
		...
		public static ISetDatos obtenerDetalleStock(String ClienteClave) throws Exception{
			StringBuilder consulta = new StringBuilder();

//			consulta.append("SELECT UV.ProductoClave AS _id, UV.Nombre AS Nombre, UV.TipoUnidad AS TipoUnidad, Stock AS Cantidad FROM ");
...
//			consulta.append("WHERE TP.TransProdID= '" +MSIEV+ "') AS MSIEV ON UV.ProductoClave = MSIEV.ProductoClave");
			...
			consulta.append("Select SV.VisitaClave, SV.ProductoClave as _id, P.Nombre as Nombre, PU.PRUTipoUnidad as TipoUnidad, Stock as Cantidad ");
			consulta.append("from StockVisita as SV ");
			consulta.append("inner join Producto as P on P.ProductoClave = SV.ProductoClave ");
			consulta.append("inner join ProductoUnidad as PU on PU.ProductoClave = P.ProductoClave and SV.ClienteClave = '"+ClienteClave+"' ");
			consulta.append("order by SV.DiaClave desc");
	...
	public static final class ConsultasPedidosAdicional{
		public static int obtenerCuantosEsquemas(String esquemas) throws Exception{
			StringBuilder consulta = new StringBuilder();
			int cuantos = 0;
			Cliente oCliente = (Cliente)Sesion.get(Campo.ClienteActual);
			consulta.append("SELECT COUNT(Esq.EsquemaID) AS Cuantos ");
			consulta.append("FROM Esquema Esq ");
			consulta.append("INNER JOIN ClienteEsquema CE ON Esq.EsquemaID = CE.EsquemaID ");
			consulta.append("AND Esq.EsquemaID ");
			consulta.append("IN ('" +esquemas+ "') AND Esq.Tipo = 1 ");
			consulta.append("AND CE.ClienteClave = '" +oCliente.ClienteClave+ "'");
		...
		public static int obtenerClientesEsquemas(String Esquemas, String clienteClave) throws Exception{
			int cuantos = 0;
			StringBuilder consulta = new StringBuilder();

			consulta.append("SELECT COUNT(Esq.EsquemaID) AS Cuantos FROM Esquema Esq ");
			consulta.append("INNER JOIN ClienteEsquema CliEsq ON Esq.Tipo = '1' ");
			consulta.append("AND (Esq.EsquemaID = CliEsq.EsquemaID OR Esq.EsquemaIDPadre = CliEsq.EsquemaID) ");
			consulta.append("AND CliEsq.ClienteClave = '" +clienteClave+ "' AND Esq.EsquemaID IN ('" +Esquemas+ "')");
		...
		public static Boolean existeVentas(String visitaClave) throws Exception{
			Boolean valido = true;
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT COUNT(TransProdID) AS Cuantos FROM TransProd ");
			consulta.append("WHERE Tipo = '1' AND TipoFase <> '0' AND VisitaClave = '" +visitaClave+ "'");

			ISetDatos datos = BDVend.consultar(consulta.toString());
			if(datos.moveToFirst())
				valido = (datos.getInt("Cuantos") > 0 ? true : false);

			return valido;
		}

		public static int obtenerPedAdicionalVacios(String visitaClave, Boolean existe) throws Exception{
			int cuantos = 0;
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT COUNT(Trp.TransProdID) AS Cuantos FROM TransProd Trp ");
			consulta.append("INNER JOIN TRPVtaAcreditada TrpVta ON Trp.TransProdID = TrpVta.TransProdID AND Trp.Tipo = '1' ");
			consulta.append("AND Trp.TipoFase <> '0' ");
			if(existe)
				consulta.append(" AND (TrpVta.PedidoAdicional = NULL OR TrpVta.PedidoAdicional = '') ");
			consulta.append("AND Trp.VisitaClave = '" +visitaClave+ "'");
		...
		public static String obtenerPedidoAdicional(String transprodID) throws Exception{
			String pedido = "";
			ISetDatos datos = BDVend.consultar("SELECT PedidoAdicional FROM TRPVtaAcreditada WHERE TransProdID = '" +transprodID+ "'");
			if(datos.moveToFirst())
				pedido = datos.getString("PedidoAdicional");

			return pedido;
		}

		public static void actualizarPedidoAdicional(String transProdID, String pedAdicional, String usuarioID) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("UPDATE TRPVtaAcreditada SET PedidoAdicional = '" +pedAdicional+ "'");
			consulta.append(",MUsuarioID = '" +usuarioID+ "'");
			consulta.append(" WHERE TransProdID = '" +transProdID+ "'");
			BDVend.ejecutarComando(consulta.toString());
		}
	...
	public static final class ConsultasPedidosConfirmadosPRS{

		public static String obtenerIdsConfirmadosPRS() throws  Exception {
			StringBuilder sConsulta = new StringBuilder();
			String sConfirmadosPRS ="";

			sConsulta.append("Select TransProdID from TransProd where Tipo = 1 and TipoFase = 12 ");
			ISetDatos datos = BDVend.consultar(sConsulta.toString());
			while (datos.moveToNext())
			{
				sConfirmadosPRS += "'" + datos.getString("TransProdID") + "',";
			}
			if(sConfirmadosPRS.length()>0){
				sConfirmadosPRS = sConfirmadosPRS.substring(0, sConfirmadosPRS.length() - 1);
			}
			datos.close();

			return sConfirmadosPRS;
		}

		public static String obtenerFolioPRS(String transProdId) throws  Exception {
			StringBuilder sConsulta = new StringBuilder();
			String sFolioPRS ="";
			sConsulta.append("Select PedidoAdicional from TRPVtaAcreditada where TransProdID ='"  + transProdId + "' ");
			ISetDatos datos = BDVend.consultar(sConsulta.toString());
			...
			{
				datos.moveToNext();
				sFolioPRS =  datos.getString("PedidoAdicional");
			}
			datos.close();
			return sFolioPRS;
		}

		public static String obtenerBackOrderDisConfirmacion() throws  Exception {
			StringBuilder sConsulta = new StringBuilder();
			String sBackOrderPRS ="";

			sConsulta.append("select V.ClienteClave from TransProd TP inner join Visita V on TP.VisitaClave = V.VisitaClave and TP.DiaClave = V.DiaClave and TP.DiaClave = '"+((Dia)Sesion.get(Campo.DiaActual)).DiaClave+"'");
			sConsulta.append("where TP.Tipo = 1 and TP.TipoFase = 1 and TP.TipoPedido = 9 ");
			ISetDatos datos = BDVend.consultar(sConsulta.toString());
			while (datos.moveToNext())
			{
				sBackOrderPRS += "'" + datos.getString("ClienteClave") + "' ,";
			}
			if(sBackOrderPRS.length()>0){
				sBackOrderPRS = sBackOrderPRS.substring(0, sBackOrderPRS.length() - 2);
			}
			datos.close();

			return sBackOrderPRS;
		}

		public static ISetDatos obtenerReporteConfirmadosPRS(String filtroFecha, String clienteClave)throws  Exception {
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("SELECT TRP.TransProdID,CLI.ClienteClave, CLI.Clave, CLI.RazonSocial,  TVA.FolioCliente as ClienteSAP,  TRP.FechaCaptura as FechaPedido, IFNULL(TVA.PedidoAdicional,'') as PedidoSAP, TRP.Total as ValorNeto, TVA.Observaciones as Estatus, TVA.FolioNegociacion, ");
			sConsulta.append("TPD.ProductoClave, PRO.Nombre, TPD.TipoUnidad, TPD.CantidadOriginal ,TPD. Cantidad, TPD.Total, IFNULL(TVA.Observaciones2,0) as TotalAntImp, TRP.Folio, TRP.ClientePagoId, TRP.TipoFase  ");
			sConsulta.append("FROM TransProd TRP ");
			sConsulta.append("inner join Visita VIS on VIS.VisitaClave = TRP.VisitaClave and VIS.DiaClave = TRP.DiaClave ");
			sConsulta.append("inner join TransProdDetalle TPD on TRP.TransProdID = TPD.TransProdID ");
			sConsulta.append("inner join Cliente CLI on CLI.ClienteClave = VIS.ClienteClave ");
			sConsulta.append("inner join Producto PRO on PRO.ProductoClave = TPD.ProductoClave ");
			sConsulta.append("left join TRPVtaAcreditada TVA on TRP.TransProdID = TVA.TransProdID ");
			sConsulta.append("where TRP.Tipo = 1 and TRP.TipoFase in (1,11,12) and (TRP.TipoPedido <> 9 or (TRP.TipoPedido = 9 and TRP.FacturaId is not null)) ");
			if (!clienteClave.equals("")) {
				sConsulta.append(" and VIS.ClienteClave = '" + clienteClave + "' ");
			}
			if (!filtroFecha.equals("")) {
			...
			sConsulta.append("order by CLI.ClienteClave, TRP.FechaCaptura ");
    ...
    public static final class ConsultaReporteVentas{
		...
		public static ISetDatos obtenerVentasGeneral(String diaClave)throws Exception {
			...
			consulta.append("SELECT T.Folio AS Folio, IFNULL(C.NombreCorto, '') AS NombreCorto, T.Total AS Total ");
			consulta.append("FROM TransProd T ");
			consulta.append("INNER JOIN Visita V ON IFNULL(T.VisitaClave1, T.VisitaClave) = V.VisitaClave AND IFNULL(T.DiaClave1, T.DiaClave) = V.DiaClave ");
			consulta.append("INNER JOIN Cliente C ON V.ClienteClave = C.ClienteClave ");
			consulta.append("WHERE T.Tipo = 1 AND T.TipoFase IN (" + (tipoModulo == Enumeradores.TiposModulos.PREVENTA ? "1" : "2, 3") + ") ");
			consulta.append("AND V.DiaClave = '" + diaClave + "' AND V.VendedorId = '" + ((Vendedor) Sesion.get(Campo.VendedorActual)).VendedorId + "' ");
	...
	public static final class ConsultaReporteVentasMEL{

		public static ISetDatos obtenerVentasGeneralMEL(String diaClave)throws Exception {
			StringBuilder consulta = new StringBuilder();
			int tipoModulo = Integer.parseInt(Sesion.get(Campo.TipoModulo).toString());

			consulta.append("select t.Folio as Folio, c.NombreCorto as NombreCorto, t.Total as Total, t.TipoFase as TipoFase ");
			consulta.append("from TransProd t ");
			consulta.append("inner join Visita v on ifnull(t.VisitaClave1,t.VisitaClave)=v.VisitaClave and ifnull(t.DiaClave1,t.DiaClave)=v.DiaClave ");
			consulta.append("inner join Cliente c on v.ClienteClave=c.ClienteClave ");
			consulta.append("where t.Tipo=1 and t.TipoFase in (2,0) ");
		...
		public static ISetDatos obtenerVentasPorProductoPorPrecioMEL(String diaClave)throws Exception {
			StringBuilder consulta = new StringBuilder();
			int tipoModulo = Integer.parseInt(Sesion.get(Campo.TipoModulo).toString());

			consulta.append("select td.TipoUnidad as TipoUnidad, td.Precio as Precio, sum(td.Cantidad) as Cantidad, t.TipoFase as TipoFase ");
			consulta.append("from TransProd t ");
			consulta.append("inner join Visita v on ifnull(t.VisitaClave1,t.VisitaClave)=v.VisitaClave and ifnull(t.DiaClave1,t.DiaClave)=v.DiaClave ");
			consulta.append("inner join TransProdDetalle td on t.TransProdID=td.TransProdID  ");
			consulta.append("where t.Tipo=1 and t.TipoFase in (" + (tipoModulo == Enumeradores.TiposModulos.PREVENTA ? "1" : "2,3") + ")");
		...
		public static ISetDatos obtenerVentasKiloLitrosMEL(String diaClave)throws Exception {
			StringBuilder consulta = new StringBuilder();
			int tipoModulo = Integer.parseInt(Sesion.get(Campo.TipoModulo).toString());

			consulta.append("select sum(td.Cantidad * ifnull(pu.KgLts,0)) as KiloLitros, t.TipoFase as TipoFase ");
			consulta.append("from TransProd t ");
			consulta.append("inner join Visita v on ifnull(t.VisitaClave1,t.VisitaClave)=v.VisitaClave and ifnull(t.DiaClave1,t.DiaClave)=v.DiaClave ");
			consulta.append("inner join TransProdDetalle td on t.TransProdID=td.TransProdID ");
			consulta.append("inner join ProductoUnidad pu on td.ProductoClave=pu.ProductoClave and td.TipoUnidad=pu.PRUTipoUnidad ");
			consulta.append("where t.Tipo=1 and t.TipoFase in (2) ");
			consulta.append("and v.DiaClave='"+ diaClave +"' ");
		...
		public static ISetDatos obtenerVentasDetalladoMEL(String diaClave)throws Exception {
			StringBuilder consulta = new StringBuilder();
			int tipoModulo = Integer.parseInt(Sesion.get(Campo.TipoModulo).toString());

			consulta.append("select t.Tipo as Tipo, ifnull(c.Clave,'') || ' - ' || ifnull(c.RazonSocial,'') as Cliente, t.Folio as Folio, p.ProductoClave || ' - ' || p.Nombre as Producto, td.TipoUnidad as TipoUnidad, td.Cantidad as Cantidad, t.TipoFase as TipoFase,  ");
			consulta.append("case when ifnull(td.Promocion,0)=2 then '*' else td.Precio end as Precio, ");
			consulta.append("case when ifnull(td.Promocion,0)=2 then '' else ((td.Subtotal+td.Impuesto)-ifnull(tds.DesImporte,0)-ifnull(tdsv.DesImporte,0)-ifnull(tds.DesImpuesto,0)-ifnull(tdsv.DesImpuesto,0)) end as Total ");
			consulta.append("from TransProd t ");
			consulta.append("inner join Visita v on ifnull(t.VisitaClave1,t.VisitaClave)=v.VisitaClave and ifnull(t.DiaClave1,t.DiaClave)=v.DiaClave ");
		...
		public static ISetDatos obtenerVentasPorProductoMEL(String diaClave)throws Exception {
			StringBuilder consulta = new StringBuilder();
			int tipoModulo = Integer.parseInt(Sesion.get(Campo.TipoModulo).toString());

			consulta.append("select p.ProductoClave || ' - ' || p.Nombre as Producto, td.TipoUnidad as TipoUnidad, sum(td.Cantidad) as Cantidad, t.TipoFase as TipoFase,");
			consulta.append("sum(td.Total) as Total ");
			consulta.append("from TransProd t ");
			consulta.append("inner join Visita v on ifnull(t.VisitaClave1,t.VisitaClave)=v.VisitaClave and ifnull(t.DiaClave1,t.DiaClave)=v.DiaClave ");
			consulta.append("inner join Cliente c on v.ClienteClave=c.ClienteClave ");
			...
			consulta.append("left join TPDDesVendedor tdsv on td.TransProdID=tdsv.TransProdId and td.TransProdDetalleID=tdsv.TransProdDetalleId ");
			consulta.append("inner join Producto p on td.ProductoClave=p.ProductoClave ");
			consulta.append("where t.Tipo=1 and t.TipoFase in (2) and ifnull(td.Promocion,0)<>2 ");
			consulta.append("and v.DiaClave='"+ diaClave +"' ");
			consulta.append("group by p.ProductoClave, p.Nombre, td.TipoUnidad ");
			consulta.append("order by p.ProductoClave, p.Nombre ");
		...
		public static ISetDatos obtenerVentasPromocionalesMEL(String diaClave)throws Exception {
			StringBuilder consulta = new StringBuilder();
			int tipoModulo = Integer.parseInt(Sesion.get(Campo.TipoModulo).toString());
	...
	public static final class ConsutasNotaVentaUNI{
		...
		public static ISetDatos obtenerPedidos(int tipoModulo, String clienteClave, String visitaClave) throws Exception
		{
			...
			consulta.append("Select TP.Folio, P.ProductoClave, P.Nombre, TPD.Cantidad, TPD.Precio, TPD.Subtotal, sum(ifnull(ti.ImpuestoPU, 0)) as Impuesto ");
			...
			consulta.append("group by TP.Folio, P.ProductoClave, P.Nombre, TPD.Cantidad, TPD.Precio, TPD.Subtotal ");
	...
	public static final class ConsultasLiquidacionDis{
			...
			if (fechas[0] != null && fechas[1] != null) {
				...
				switch (tipoConsulta) {
					case 1:
						...
						consulta.append("from TransProd ");
						consulta.append("inner join Visita on ifnull(TransProd.VisitaClave1, TransProd.VisitaClave) = Visita.VisitaClave and ifnull(TransProd.DiaClave1, TransProd.DiaClave) = Visita.DiaClave ");
						consulta.append("inner join Dia on Visita.DiaClave = Dia.DiaClave ");
						consulta.append("inner join TransProdDetalle on TransProd.TransProdID = TransProdDetalle.TransProdID ");
						consulta.append("inner join Producto on TransProdDetalle.ProductoClave = Producto.ProductoClave ");
						consulta.append("where TransProd.Tipo in (1,24)  and TransProd.TipoFase = 2  ");
						consulta.append("and Dia.FechaCaptura between '" + Generales.getFormatoFecha(fechas[0], "yyyy-MM-dd HH:mm:ss") +  "' and '" + Generales.getUltimaHora(fechas[1], "yyyy-MM-dd HH:mm:ss") +  "' ");
		...
		public static float obtenerTotalCredito() throws Exception{
			...
			if (fechas[0] != null && fechas[1] != null) {
				...
				consulta.append("inner join Visita on ifnull(TransProd.VisitaClave1, TransProd.VisitaClave) = Visita.VisitaClave and ifnull(TransProd.DiaClave1, TransProd.DiaClave) = Visita.DiaClave ");
				consulta.append("inner join Dia on Visita.DiaClave = Dia.DiaClave ");
				consulta.append("where TransProd.TipoFase = 2 ");
		...
		public static float obtenerTotalContado() throws Exception{
			...
			if (fechas[0] != null && fechas[1] != null) {
				...
				consulta.append("where transprod.cfvtipo=1 and transprod.tipo=1 and transprod.tipofase = 2 ");
				consulta.append("and dia.FechaCaptura between '" + Generales.getFormatoFecha(fechas[0], "yyyy-MM-dd HH:mm:ss") + "' and '" + Generales.getUltimaHora(fechas[1], "yyyy-MM-dd HH:mm:ss") + "' ");
		...
		public static float obtenerTotalCobranza() throws Exception{
			...
			if (fechas[0] != null && fechas[1] != null) {
				...
				consulta.append("inner join abntrp on abntrp.abnid=abono.abnid  ");
				consulta.append("inner join transprod on abntrp.transprodid=transprod.transprodid ");
				consulta.append("where transprod.tipofase = 2 and ((transprod.tipo=1 and transprod.cfvtipo=2) or transprod.tipo=24)");
				consulta.append("and dia.FechaCaptura between '" + Generales.getFormatoFecha(fechas[0], "yyyy-MM-dd HH:mm:ss") + "' and '" + Generales.getUltimaHora(fechas[1], "yyyy-MM-dd HH:mm:ss") + "' ");
	...
	public static final class ConsultasLiquidacionNOR {

		public static ISetDatos obtenerDatosLiquidacion(boolean soloProductos, int tipoTransprod) throws Exception {
			...
			if (fechas[0] != null && fechas[1] != null) {
				...
				if (soloProductos) {
					consulta.append("SELECT ");
					consulta.append("	T.Tipo, TPD.ProductoClave, P.Nombre, TPD.TipoUnidad, SUM(CASE WHEN IFNULL(TPD.Promocion, 0) <> 2 THEN TPD.Cantidad ELSE 0 END) AS Cantidad, SUM(CASE WHEN TPD.Promocion = 2 THEN TPD.Cantidad ELSE 0 END) AS Promocion, SUM(TPD.DescuentoImp) AS DescuentoDet ");
					consulta.append("FROM TransProd T ");
					consulta.append("	INNER JOIN TransProdDetalle TPD ON T.TransProdID = TPD.TransProdID ");
					consulta.append("	INNER JOIN Producto P ON TPD.ProductoClave = P.ProductoClave ");
					consulta.append("	LEFT JOIN TpdDes TD ON TPD.TransProdID = TD.TransProdId AND TPD.TransProdDetalleID = TD.TransProdDetalleId ");
					consulta.append("WHERE T.Tipo = " + (tipoTransprod == 1 ? "1 AND T.TipoFase <> 0 " : "24 AND T.TipoFase = 2 "));
					consulta.append("AND T.FECHACAPTURA BETWEEN '" + Generales.getFormatoFecha(fechas[0], "yyyy-MM-dd HH:mm:ss") +  "' AND '" + Generales.getUltimaHora(fechas[1], "yyyy-MM-dd HH:mm:ss") +  "' ");
					consulta.append("GROUP BY T.Tipo, TPD.ProductoClave, P.Nombre, TPD.TipoUnidad ");
					consulta.append("ORDER BY P.Nombre, TPD.TipoUnidad ");
				} else {
					consulta.append("SELECT ");
					consulta.append("	SUM(T.Total) AS Total, SUM(T.DescuentoVendedor) AS DescuentoVendedor, SUM(T.DescuentoImp) AS DescuentoImp ");
					consulta.append("FROM TransProd T ");
					consulta.append("WHERE T.Tipo IN (1, 24) AND T.TipoFase <> 0 ");
					consulta.append("AND T.FECHACAPTURA BETWEEN '" + Generales.getFormatoFecha(fechas[0], "yyyy-MM-dd HH:mm:ss") +  "' AND '" + Generales.getUltimaHora(fechas[1], "yyyy-MM-dd HH:mm:ss") +  "' ");
				}
		...
		public static float obtenerTotalCredito() throws Exception{
			...
			if (fechas[0] != null && fechas[1] != null) {
				consulta.append("SELECT ");
				consulta.append("	CASE WHEN IFNULL(SUM(T.total), 0) = 1 THEN 0 ELSE SUM(T.total) END AS Total ");
				consulta.append("FROM TransProd T ");
				consulta.append("WHERE T.TipoFase <> 0 ");
				consulta.append("	AND ((T.Tipo = 1 AND T.CFVTipo = 2) OR T.Tipo = 24) ");
				consulta.append("	AND T.FechaCaptura BETWEEN '" + Generales.getFormatoFecha(fechas[0], "yyyy-MM-dd HH:mm:ss") +  "' AND '" + Generales.getUltimaHora(fechas[1], "yyyy-MM-dd HH:mm:ss") +  "' ");
		...
		public static ISetDatos obtenerTipoCobranza() throws Exception{
			float total = 0;
			StringBuilder consulta = new StringBuilder();
			Date[] fechas = new Date[2];
			Consultas.ConsultasDia.obtenerRangoAgenda(fechas);
			if (fechas[0] != null && fechas[1] != null) {
				consulta.append("SELECT EsqPublicoGeneral, EsqDatosExtra, sum(Importe) AS Importe FROM ( ");
				consulta.append("	SELECT ");
				consulta.append("		CASE WHEN ifnull((SELECT count(*) FROM ClienteEsquema CE INNER JOIN Esquema E ON CE.EsquemaID = E.EsquemaID WHERE E.Nivel = 1 AND CLI.ClienteClave = CE.ClienteClave AND E.Nombre = 'Publico General'), 0) > 0 THEN 1 ELSE 0 END AS EsqPublicoGeneral, ");
				consulta.append("		CASE WHEN ifnull((SELECT count(*) FROM ClienteEsquema CE INNER JOIN Esquema E ON CE.EsquemaID = E.EsquemaID WHERE E.Nivel = 1 AND CLI.ClienteClave = CE.ClienteClave AND E.Nombre = 'Factura Punto Venta'), 0) > 0 THEN 1 ELSE 0 END AS EsqDatosExtra, ");
				consulta.append("		ABT.Importe AS Importe ");
				consulta.append("	FROM TransProd AS T ");
				consulta.append("		INNER JOIN Visita AS VIS ON T.VisitaClave = VIS.VisitaClave ");
				consulta.append("		INNER JOIN Cliente AS CLI ON VIS.ClienteClave = CLI.ClienteClave ");
				consulta.append("		LEFT JOIN AbnTrp AS ABT ON T.TransProdID = ABT.TransProdID ");
				consulta.append("	WHERE T.Tipo = 1 AND T.TipoFase <> 0 AND T.CFVTipo = 1 ");
				consulta.append("		AND T.FechaCaptura BETWEEN '" + Generales.getFormatoFecha(fechas[0], "yyyy-MM-dd HH:mm:ss") +  "' AND '" + Generales.getUltimaHora(fechas[1], "yyyy-MM-dd HH:mm:ss") +  "' ");
				consulta.append(") AS tmp ");
				consulta.append("GROUP BY EsqPublicoGeneral, EsqDatosExtra ");
		...
		public static float obtenerCobranza(boolean Contado) throws Exception{
			float total = 0;
			StringBuilder consulta = new StringBuilder();
			Date[] fechas = new Date[2];
			Consultas.ConsultasDia.obtenerRangoAgenda(fechas);
			if (fechas[0] != null && fechas[1] != null) {
				consulta.append("SELECT ");
				consulta.append("	IFNULL(SUM(ABD.Importe), 0) AS Total ");
				consulta.append("FROM Abono A ");
				consulta.append("	INNER JOIN Dia D ON A.DiaClave = D.DiaClave ");
				consulta.append("	INNER JOIN ABNDETALLE ABD ON A.ABNId = ABD.ABNId ");
				consulta.append("	INNER JOIN Visita V ON A.VisitaClave = V.VisitaClave AND A.DiaClave = V.DiaClave ");
				consulta.append("	INNER JOIN AbnTrp ABT ON A.ABNId = ABT.ABNId ");
				consulta.append("	INNER JOIN TransProd T ON ABT.TransProdID = T.TransProdID ");
				consulta.append("WHERE T.TipoFase <> 0 " + (Contado == true ? "AND T.CFVTipo = 1 AND T.Tipo = 1": "AND ((T.Tipo = 1 AND T.CFVTipo = 2) OR T.Tipo = 24) "));
				consulta.append("	AND D.FechaCaptura BETWEEN '" + Generales.getFormatoFecha(fechas[0], "yyyy-MM-dd HH:mm:ss") +  "' AND '" + Generales.getUltimaHora(fechas[1], "yyyy-MM-dd HH:mm:ss") +  "' ");
    ...
    public static final class ConsultasReportesBYD {
        ...
        public static ISetDatos obtenerVisitasRealizadas(String DiaClave) throws Exception{
			...
			consulta.append("count(distinct case when (VIS.CodigoLeido = 0) then VIS.ClienteClave else null end) as VisitasSinLector ");
            consulta.append("from Visita VIS ");
            consulta.append("left join ( ");
            if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.PREVENTA) {
                consulta.append("   select distinct VisitaClave, DiaClave ");
        ...
        public static ISetDatos obtenerClientesConVenta(String DiaClave) throws Exception {
            ...
            consulta.append("sum(case when vis.CodigoLeido = 1 then 0 else 1 end) as CodigoNoLeido, ");
        ...
        public static ISetDatos obtenerClientesSinVenta(String DiaClave) throws Exception {
			...
			consulta.append("case when vis.CodigoLeido = 1 then 1 else 0 end as CodigoLeido ");
            consulta.append("from Visita vis ");
		...
		public static ISetDatos obtenerCargas(String transProdId, boolean contenedor) throws Exception{
			...
			if(contenedor){
				...
				consulta.append("Select p.ProductoClave ,p.Nombre as Articulo, (tpd.Cantidad * IFNULL(tde.Factor, pd.Factor)) as Piezas , (ppv.Precio * (tpd.Cantidad * IFNULL(tde.Factor, pd.Factor))) AS Monto,ppv.Precio as PrecioBase,c.TipoImpuesto ");
				consulta.append("from TransProd tp  ");
				consulta.append("inner join Vendedor v on v.USUid = tp.MUsuarioId ");
				consulta.append("inner join Cliente c on v.ClienteModelo = c.ClienteClave ");
				consulta.append("inner join TransProdDetalle tpd on tp.TransProdID = tpd.TransProdID ");
	...
	public static final class ConsultasTicketCOM{
		...
		public static ISetDatos getPromocionProducto(String transProdDetalleID, String clienteClave) throws Exception{

			StringBuilder consulta = new StringBuilder();

			consulta.append("SELECT  ");
			consulta.append("	PRP.PromocionClave, PR.Importe, PR.Porcentaje, P.ProductoClave, E.Nombre, PRO.TipoAplicacion, PRP.PromocionImp, TPD.Cantidad, TPD.Precio ");
			consulta.append("FROM TransProd TRP");
			consulta.append("	INNER JOIN TransProdDetalle TPD ON TRP.TransProdID = TPD.TransProdID");
			consulta.append("	INNER JOIN Producto P ON TPD.ProductoClave = P.ProductoClave");
			consulta.append("	LEFT JOIN TrpPrp PRP ON TPD.TransProdID = PRP.TransProdID AND TPD.TransProdDetalleID = PRP.TransProdDetalleID");
			consulta.append("	LEFT JOIN PromocionRegla PR ON PRP.PromocionClave = PR.PromocionClave AND PRP.PromocionReglaID = PR.PromocionReglaID");
			consulta.append("	LEFT JOIN Promocion PRO ON PR.PromocionClave = Pro.PromocionClave");
			consulta.append("	LEFT JOIN PromocionDetalle PRD ON PR.PromocionClave = PRD.PromocionClave");
			consulta.append("	LEFT JOIN Esquema E ON PRD.EsquemaID = E.EsquemaID ");
			consulta.append("WHERE TPD.TransProdDetalleID IN ( " + transProdDetalleID + " ) AND (PRP.PromocionClave LIKE 'PR%' OR PRP.PromocionClave LIKE 'DD%') ");
			consulta.append("	AND E.EsquemaID IN (SELECT EsquemaID FROM ClienteEsquema WHERE ClienteClave = '" + clienteClave + "') ");
			consulta.append("ORDER BY TPD.ProductoClave, PRP.PromocionClave DESC ");
			consulta.append("LIMIT 1");
			consulta.append("");
		...
		public static ISetDatos getPromocionProductoDD(String transProdDetalleID, String clienteClave) throws Exception{
			...
			consulta.append("SELECT  ");
			//consulta.append("	PRP.PromocionClave, PR.Importe, PR.Porcentaje, P.ProductoClave, E.Nombre, PRO.TipoAplicacion, PRP.PromocionImp, TPD.Cantidad, TPD.Precio");
			consulta.append("	P.ProductoClave, E.Nombre, SUM(PRP.PromocionImp) AS PromocionImp, SUM(TPD.Cantidad) AS Cantidad ");
			consulta.append("FROM TransProd TRP");
			consulta.append("	INNER JOIN TransProdDetalle TPD ON TRP.TransProdID = TPD.TransProdID");
			consulta.append("	INNER JOIN Producto P ON TPD.ProductoClave = P.ProductoClave");
			consulta.append("	LEFT JOIN TrpPrp PRP ON TPD.TransProdID = PRP.TransProdID AND TPD.TransProdDetalleID = PRP.TransProdDetalleID");
			...
			consulta.append("	LEFT JOIN Promocion PRO ON PR.PromocionClave = Pro.PromocionClave");
			consulta.append("	LEFT JOIN Esquema E ON PRD.EsquemaID = E.EsquemaID ");
			consulta.append("WHERE PRP.TransProdDetalleID IN ( " + transProdDetalleID + " ) AND PRP.PromocionClave LIKE 'DD%' AND E.EsquemaID IN (SELECT EsquemaID FROM ClienteEsquema WHERE ClienteClave = '" + clienteClave + "') ");
			consulta.append("GROUP BY P.ProductoClave, E.Nombre ");
			consulta.append("ORDER BY TPD.ProductoClave, PRP.PromocionClave DESC");
			consulta.append("");
...
//		}
	...
	public static final class ConsultasTicketPDFMederi {

		public static ISetDatos obtenerDetalles(String TransprodId) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("select P.ProductoClave, P.Nombre, TD.Cantidad, TD.Precio, (TD.Cantidad * TD.Precio) as Subtotal, TD.Promocion, IFNULL(TD.DescuentoImp,0) as Descuento1, IFNULL(TpD.DesImporte,0) as Descuento2, IFNULL(TpV.DesImporte,0) as Descuento3, ");
			consulta.append("case when TD.Promocion = 2 then 'Promocion' else 'Normal' end as Tipo ");
			consulta.append("from TransProd T ");
			consulta.append("inner join TransprodDetalle TD on T.TransprodId = TD.TransprodId and T.TransprodId = '" + TransprodId + "' and T.Tipo = 1 and T.TipoFase = 1 ");
			consulta.append("inner join Producto P on TD.ProductoClave = P.ProductoClave ");
			consulta.append("left join TpdDes TpD on T.TransprodId = TpD.TransprodID and TD.TransprodDetalleId = TpD.TransprodDetalleId ");
			consulta.append("left join TpdDesVendedor TpV on T.TransprodId = TpV.TransprodId and TD.TransprodDetalleId = TpV.TransprodDetalleId order by P.ProductoClave, TD.Promocion asc ");
			return BDVend.consultar(consulta.toString());
		}
	}
	...
	public static final class ConsultasConteoInventario{
		public static ISetDatos obtenerConteoInventario()throws Exception {
			StringBuilder consulta = new StringBuilder();
			consulta.append("Select ContId as _id,'Registro de Conteo' as Nombre, FechaHoraAlta, TipoFase, Enviado from ConteoInventario where FechaHoraAlta between '" + Generales.getPrimerHora(((Dia)Sesion.get(Campo.DiaActual)).FechaCaptura) + "' and '" + Generales.getUltimaHora(((Dia)Sesion.get(Campo.DiaActual)).FechaCaptura) + "' ");

			return BDVend.consultar(consulta.toString());
		}
		public static ISetDatos obtenerConteoInventarioDetalle(String ContId) throws Exception
		{

			StringBuilder consulta = new StringBuilder();
			consulta.append("Select CID.ProductoClave as _id, CID.*,  PRO.Nombre as Descripcion, PRO.DecimalProducto as DecimalProducto ");
			consulta.append("from ConteoInventarioDet CID ");
			consulta.append("inner join Producto PRO on CID.ProductoClave = PRO.ProductoClave ");
			consulta.append("where CID.ContId ='" + ContId + "' ");
			return BDVend.consultar(consulta.toString());
		}
	}
	...
	public static final class ConsultasReportesFamo{
		public static double obtenerTotalVentas(String CFVTipo, String DiaClave, String Tipo) throws Exception{
			StringBuilder consulta = new StringBuilder();
			/*consulta.append("SELECT IFNULL(SUM(TP.Total),0) AS Total FROM TransProd TP ");
			consulta.append("WHERE TP.Tipo = " +Tipo+ " AND ");
			consulta.append("TipoFase <> 0 ");*/
			consulta.append("SELECT IFNULL(SUM(TP.Total),0) AS Total FROM TransProd TP ");
			consulta.append("INNER JOIN Visita V ON V.VisitaClave = TP.VisitaClave AND TP.Tipo = " +Tipo+ " AND ");
			consulta.append("TipoFase <> 0 AND V.DiaClave = '" +DiaClave+ "' ");
			if(Tipo.equals("1") || Tipo.equals("2"))
				consulta.append("AND TP.CFVTipo = " +CFVTipo);

			ISetDatos datos = BDVend.consultar(consulta.toString());
			datos.moveToFirst();
			return datos.getDouble("Total");
		}

		public static double obtenerTotalCobranza() throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT IFNULL(SUM(AT.Importe),0) AS Total FROM AbnTrp AT ");
			consulta.append("INNER JOIN TransProd TP ON TP.TransProdId = AT.TransProdID ");
			consulta.append("AND TP.Tipo = 1 AND TP.TipoFase <> 0 AND TP.CFVTipo = 2 ");
	...
	public static final class ConsultasReportesNGO{
		public static ISetDatos obtenerEncabezadoNGO(String almacenId) throws Exception{
			return BDVend.consultar("SELECT Nombre, Telefono, Domicilio FROM Almacen WHERE AlmacenId = '" +almacenId+ "'");
		}

		public static ISetDatos obtenerClienteDomicilio(String clienteClave) throws Exception{
			return BDVend.consultar("SELECT Calle, Numero, NumInt FROM ClienteDomicilio WHERE Tipo = 2 AND ClienteClave = '" +clienteClave+ "'");
		}

		public static float obtenerSaldoCliente(String clienteClave) throws Exception{
			float saldo = 0f;
			ISetDatos datosSaldo = BDVend.consultar("SELECT IFNULL(SUM(Saldo),0) AS Saldo FROM TransProd WHERE Tipo = 1 AND TipoFase = 2 AND ClienteClave = '" +clienteClave+ "'");

			while(datosSaldo.moveToNext())
				saldo = datosSaldo.getFloat("Saldo");

			return saldo;
		}

		public static ISetDatos obtenerVentas(String visitaClave, String DiaClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT TransProdId, TransProdID AS ID,Folio, CFVTipo, Total, Notas, IFNULL(TipoTurno, 0) AS TipoTurno ");
			consulta.append("FROM TransProd ");
			consulta.append("WHERE COALESCE(VisitaClave1, VisitaClave) = '" +visitaClave+ "' AND COALESCE(DiaClave1, DiaClave) = '" +DiaClave+ "' ");
			consulta.append("AND Tipo = 1 AND TipoFase <> 0");

			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerBonificacion(String visitaClave, String diaClave, String motivosNoVta) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT DISTINCT T.TransProdId, T.TransProdId as _id, CASE WHEN MT.TipoIndice=2 THEN T.Notas ELSE T.Folio END AS Folio ");
			consulta.append("FROM TransProd T ");
			consulta.append("INNER JOIN TransProdDetalle TPD ON T.TransProdID = TPD.TransProdID ");
			consulta.append("INNER JOIN ModuloMovDetalle MMD ON T.PCEModuloMovDetClave = MMD.ModuloMovDetalleClave ");
			consulta.append("INNER JOIN ModuloTerm MT ON MMD.ModuloClave = MT.ModuloClave ");
			consulta.append("WHERE T.VisitaClave = '" +visitaClave+ "' AND T.DiaClave = '" +diaClave+ "' ");
			consulta.append("AND T.Tipo = 9 AND T.TipoFase <> 0 AND T.TipoMovimiento = 1 ");
			if(motivosNoVta.length() > 0)
				consulta.append("AND TPD.TipoMotivo IN (" +motivosNoVta+ ")");

			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerDetallesVentas(String transProdId, String motivosNoVta) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT TPD.ProductoClave, PRO.Nombre, TPD.Cantidad, TPD.Precio + TPD.Impuesto AS Precio, TPD.Subtotal + TPD.Impuesto AS Importe ");
			consulta.append("FROM TransProdDetalle TPD ");
			consulta.append("INNER JOIN Producto PRO ON PRO.ProductoClave = TPD.ProductoClave ");
			consulta.append("WHERE TPD.TransProdId = '" +transProdId+ "' ");
			if(motivosNoVta.length() > 0)
		...
		public static boolean muestraCodBarras() throws Exception{
			ISetDatos datos = BDTerm.consultar("SELECT MostrarCodBarras FROM Recibo WHERE Tipo=1 AND Predeterminado=1");
			boolean muestra = false;
			while(datos.moveToNext())
				muestra = datos.getBoolean("MostrarCodBarras");

			return muestra;
		}

		public static ISetDatos obtenerCobranza(String visitaClave, String diaClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT TRP.CFVTipo, SUM(ABT.Importe) AS Total ");
			consulta.append("FROM Abono ABN ");
			consulta.append("INNER JOIN AbnTrp ABT ON ABN.ABNId = ABT.ABNId ");
			consulta.append("INNER JOIN TransProd TRP ON ABT.TransProdId = TRP.TransProdId ");
			consulta.append("WHERE ABN.VisitaClave='" +visitaClave+ "' AND ABN.DiaClave='" +diaClave+ "' ");
			consulta.append("GROUP BY TRP.CFVTipo");

			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerEnvase(String tipoMotivoVta, String tipoMotivoNoVta, String DiaClave, String VisitaClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) != Enumeradores.TiposModulos.REPARTO){
				if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.PREVENTA){
					consulta.append("SELECT T.ProductoClave, T.Nombre, ");
					consulta.append("SUM(T.Bonificacion) AS Bonificacion, SUM(T.Venta) AS Venta, SUM(T.Devolucion) AS Devolucion ");
					consulta.append("FROM ( ");
				}
				consulta.append("SELECT PRO.ProductoClave, PRO.Nombre, ");
				consulta.append("sum(case TRP.Tipo when 9 then TPD.Cantidad else 0 end) as Bonificacion, ");
				consulta.append("sum(case TRP.Tipo when 1 then TPD.Cantidad else 0 end) as Venta, ");
				consulta.append("sum(case TRP.Tipo when 3 then TPD.Cantidad else 0 end) as Devolucion ");
				consulta.append("from TransProd TRP ");
				consulta.append("inner join TransProdDetalle TPD on TRP.TransProdId = TPD.TransProdId ");
				consulta.append("inner join Producto PRO on TPD.ProductoClave = PRO.ProductoClave and PRO.Contenido = 1 ");
				consulta.append("where (TRP.Tipo = 1 or (TRP.Tipo = 3 " +(tipoMotivoVta.length() > 0 ? "and TPD.TipoMotivo in (" +tipoMotivoVta+ ")" : "")+ ") ");
				consulta.append("or (TRP.Tipo = 9 and TRP.TipoMovimiento = 1 " +(tipoMotivoNoVta.length() > 0 ? "and TPD.TipoMotivo in (" +tipoMotivoNoVta+ ")" : "")+ ") ");
				consulta.append(") and TRP.TipoFase <> 0 ");
				consulta.append("and coalesce(TRP.DiaClave1,TRP.DiaClave) = '" +DiaClave+ "' and coalesce(TRP.VisitaClave1,TRP.VisitaClave) = '" +VisitaClave+ "' ");
				consulta.append("group by PRO.ProductoClave, PRO.Nombre ");
				if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.PREVENTA){
					consulta.append("union all ");
					consulta.append("select PRO.ProductoClave, PRO.Nombre, ");
					consulta.append("0 as Bonificacion, ");
					consulta.append("0 as Venta, ");
					consulta.append("sum(TPD.Cantidad) as Devolucion ");
					consulta.append("from TransProd TRP ");
					consulta.append("inner join TransProdDetalle TPD on TRP.TransProdId = TPD.TransProdId ");
					consulta.append("inner join ProductoDetalle pd on TPD.ProductoClave=pd.ProductoClave and TPD.TipoUnidad=pd.PRUTipoUnidad and TPD.ProductoClave<>pd.ProductoDetClave ");
					consulta.append("inner join Producto PRO on pd.ProductoDetClave = PRO.ProductoClave and PRO.Contenido = 1 ");
					consulta.append("where ((TRP.Tipo = 1 and TRP.TipoFase=1) or (TRP.Tipo=9 and TRP.TipoMovimiento=1)) ");
					consulta.append("and coalesce(TRP.DiaClave1,TRP.DiaClave) = '" +DiaClave+ "' and coalesce(TRP.VisitaClave1,TRP.VisitaClave) = '" +VisitaClave+ "' ");
					consulta.append("group by PRO.ProductoClave, PRO.Nombre ");
					consulta.append(") as t ");
					consulta.append("group by t.ProductoClave, t.Nombre ");
				}
			}else{
				consulta.append("select PRO.ProductoClave, PRO.Nombre, ");
				consulta.append("sum(case when TRP.Tipo=1 and TRP.TIpoFase=2 and TRP.FacturaId is not null then TPD.Cantidad else 0 end) as Bonificacion, ");
				consulta.append("sum(case when TRP.Tipo=1 and TRP.TIpoFase=2 and TRP.FacturaId is null then TPD.Cantidad else 0 end) as Venta, ");
				consulta.append("sum(case when TRP.Tipo=3 then TPD.Cantidad else 0 end) as Devolucion ");
				consulta.append("from TransProd TRP ");
				consulta.append("inner join TransProdDetalle TPD on TRP.TransProdId = TPD.TransProdId ");
				consulta.append("inner join Producto PRO on TPD.ProductoClave = PRO.ProductoClave and PRO.Contenido = 1 ");
				consulta.append("where ((TRP.Tipo = 1 and TRP.TipoFase in (2,3)) or (TRP.Tipo = 3 " +(tipoMotivoVta.length() > 0 ? "and TPD.TipoMotivo in (" +tipoMotivoVta+ ")" : "")+ ") ");
				consulta.append("or (TRP.Tipo = 9 and TRP.TipoMovimiento = 1 " +(tipoMotivoNoVta.length() > 0 ? "and TPD.TipoMotivo in (" +tipoMotivoNoVta+ ")": "")+ ") ");
				consulta.append(") and TRP.TipoFase <> 0 ");
				consulta.append("and coalesce(TRP.DiaClave1,TRP.DiaClave) = '" +DiaClave+ "' and coalesce(TRP.VisitaClave1,TRP.VisitaClave) = '" +VisitaClave+ "' ");
				consulta.append("group by PRO.ProductoClave, PRO.Nombre ");
		...
		public static float obtenerSaldo(String diaClave, String visitaClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			float saldo = 0f;
			consulta.append("SELECT SUM(Saldo) AS Saldo FROM TransProd WHERE Tipo=1 AND TipoFase <> 0 AND CFVTipo = 2 AND COALESCE(DiaClave1,DiaClave)='" +diaClave+ "' ");
			consulta.append("AND COALESCE(VisitaClave1, VisitaClave) ='" +visitaClave+ "'");

			ISetDatos datos = BDVend.consultar(consulta.toString());
			while(datos.moveToNext())
				saldo += datos.getDouble("Saldo");

			return saldo;
		}

		public static ISetDatos obtenerMovimientos(String tipoMotivoNoVta) throws Exception{
			StringBuilder consulta = new StringBuilder();
			String diaClavesEnFrecuencia = "select  diaclave from dia where fuerafrecuencia=0";

			if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO){
				consulta.append("select TPD.ProductoClave, PRO.Nombre, ");
				consulta.append("sum(case when TRP.Tipo=1 and TRP.CFVTipo=1 and TRP.TipoFase in (2,3) and TRP.FacturaId is null then TPD.Cantidad else 0 end) as Contado, ");
				consulta.append("sum(case when TRP.Tipo=1 and TRP.TipoFase = 0 then TPD.Cantidad else 0 end) as Cancelado, ");
				consulta.append("sum(case when TRP.Tipo=1 and TRP.CFVTipo=2 and TRP.TipoFase in (2,3) and TRP.FacturaId is null then TPD.Cantidad else 0 end) as Credito, ");
				consulta.append("sum(case when TRP.Tipo=1 and TRP.TipoFase in (2,3) and TRP.FacturaId is not null then TPD.Cantidad else 0 end) as Bonificacion, ");
				consulta.append("sum(case when TRP.Tipo=7 then TPD.Cantidad else 0 end) as Devolucion ");
				consulta.append("from TransProd TRP ");
				consulta.append("inner join TransProdDetalle TPD on TRP.TransProdId = TPD.TransProdId ");
				consulta.append("inner join Producto PRO on TPD.ProductoClave = PRO.ProductoClave ");
				consulta.append("where (TRP.Tipo = 1 or (TRP.Tipo = 9 and TRP.TipoFase <> 0 and TRP.TipoMovimiento = 1 " +(tipoMotivoNoVta.length() > 0 ? "and TPD.TipoMotivo in (" +tipoMotivoNoVta+ ")" : "")+ ") or (TRP.Tipo = 7 and TRP.TipoFase <> 0)) ");
				consulta.append("and PRO.Contenido = 0 and (TRP.DiaClave in (" +diaClavesEnFrecuencia+ ") or TRP.DiaClave1 in (" +diaClavesEnFrecuencia+ "))");
				consulta.append("group by TPD.ProductoClave, PRO.Nombre ");
			}else{
				consulta.append("select TPD.ProductoClave, PRO.Nombre, ");
				consulta.append("sum(case when TRP.Tipo = 1 and TRP.CFVTipo = 1 and TRP.TipoFase <> 0 then TPD.Cantidad else 0 end) as Contado, ");
				consulta.append("sum(case when TRP.Tipo = 1 and TRP.TipoFase = 0 then TPD.Cantidad else 0 end) as Cancelado, ");
				consulta.append("sum(case when TRP.Tipo = 1 and TRP.CFVTipo = 2 and TRP.TipoFase <> 0 then TPD.Cantidad else 0 end) as Credito, ");
				consulta.append("sum(case when TRP.Tipo = 9 then TPD.Cantidad else 0 end) as Bonificacion, ");
				consulta.append("sum(case when TRP.Tipo = 7 then TPD.Cantidad else 0 end) as Devolucion ");
				consulta.append("from TransProd TRP ");
				consulta.append("inner join TransProdDetalle TPD on TRP.TransProdId = TPD.TransProdId ");
				consulta.append("inner join Producto PRO on TPD.ProductoClave = PRO.ProductoClave ");
		...
		public static ISetDatos obtenerTotalesMov() throws Exception{
			StringBuilder consulta = new StringBuilder();
			String diaClavesEnFrecuencia = "select  diaclave from dia where fuerafrecuencia=0";

			consulta.append("select sum(case when TRP.CFVTipo = 1 and PRO.Contenido = 0 then TPD.Total else 0 end) + sum(case when TRP.CFVTipo = 2 and PRO.Contenido = 0 then TPD.Total else 0 end) + sum(case when PRO.Contenido = 1 then TPD.Total else 0 end) as Total, ");
			consulta.append("sum(case when TRP.CFVTipo = 1 and PRO.Contenido = 0 then TPD.Total else 0 end) as Contado, ");
			consulta.append("sum(case when TRP.CFVTipo = 2 and PRO.Contenido = 0 then TPD.Total else 0 end) as Credito, ");
			consulta.append("sum(case when PRO.Contenido = 1 then TPD.Total else 0 end) as Envase ");
			consulta.append("from TransProd TRP ");
			consulta.append("inner join TransProdDetalle TPD on TRP.TransProdId = TPD.TransProdId ");
			consulta.append("inner join Producto PRO on TPD.ProductoClave = PRO.ProductoClave ");
			consulta.append("where TRP.Tipo = 1 and TRP.TipoFase <> 0 and (TRP.DiaClave in (" + diaClavesEnFrecuencia + ") or TRP.DiaClave1 in (" + diaClavesEnFrecuencia + ")) ");

			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerEnvasesMov(String tipoMotivoVta, String tipoMotivoNoVta) throws Exception{
			StringBuilder consulta = new StringBuilder();
			String diaClavesEnFrecuencia = "select  diaclave from dia where fuerafrecuencia=0";
			if (Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) == Enumeradores.TiposModulos.REPARTO){
				consulta.append("select PRO.ProductoClave, PRO.Nombre, ");
				consulta.append("sum(case when TRP.Tipo=1 and TRP.TipoFase in (2,3) and TRP.FacturaId is not null then TPD.Cantidad else 0 end) as Bonificacion, ");
				consulta.append("sum(case when TRP.Tipo=1 and TRP.TipoFase in (2,3) and TRP.FacturaId is null then TPD.Cantidad else 0 end) as Venta, ");
				consulta.append("sum(case TRP.Tipo when 3 then TPD.Cantidad else 0 end) as Recoleccion, ");
				consulta.append("sum(case TRP.Tipo when 7 then TPD.Cantidad else 0 end) as Devolucion ");
				consulta.append("from TransProd TRP ");
				consulta.append("inner join TransProdDetalle TPD on TRP.TransProdId = TPD.TransProdId ");
				consulta.append("inner join Producto PRO on TPD.ProductoClave = PRO.ProductoClave and PRO.Contenido = 1 ");
				consulta.append("where (TRP.Tipo = 1 or (TRP.Tipo = 3 " +(tipoMotivoVta.length() > 0 ? "and TPD.TipoMotivo in (" +tipoMotivoVta+ ")" : "") + ") or TRP.Tipo = 7 ");
				consulta.append("or (TRP.Tipo = 9 and TRP.TipoMovimiento = 1 " +(tipoMotivoNoVta.length() > 0 ? "and TPD.TipoMotivo in (" +tipoMotivoNoVta+ ")" : "") + ") ");
				consulta.append(") and TRP.TipoFase <> 0 ");
				consulta.append("and (TRP.DiaClave in (" +diaClavesEnFrecuencia+ ") or TRP.DiaClave1 in (" +diaClavesEnFrecuencia+ ")) ");
				consulta.append("group by PRO.ProductoClave, PRO.Nombre ");
			}else{
				consulta.append("select PRO.ProductoClave, PRO.Nombre, ");
				consulta.append("sum(case TRP.Tipo when 9 then TPD.Cantidad else 0 end) as Bonificacion, ");
		...
		public static ISetDatos obtenerPrecios(String listaPrecioClave) throws Exception{
			StringBuilder consulta = new StringBuilder();

			consulta.append("SELECT Producto.ProductoClave as _id, Producto.ProductoClave, Producto.Nombre, Producto.ProductoClave || ' ' || Producto.Nombre AS Descripcion, PRUTipoUnidad, Precio ");
			//consulta.append("SELECT Producto.ProductoClave as _id, Producto.ProductoClave || ' ' || Producto.Nombre AS Descripcion, Precio ");
			consulta.append("FROM PrecioProductoVig ");
			consulta.append("INNER JOIN  Producto  ON Producto.ProductoClave = PrecioProductoVig.ProductoClave ");
			consulta.append("WHERE PrecioClave ='" +listaPrecioClave+ "' AND DATE('Now') between PrecioProductoVig.PPVFechaInicio and PrecioProductoVig.FechaFin ");
			consulta.append("order by Producto.ProductoClave ");

			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerListasPrecios(String clienteClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("select distinct PCE.PrecioClave, P.Nombre  from ClienteEsquema CE ");
			consulta.append("inner join PrecioClienteEsquema PCE on PCE.EsquemaId = CE.EsquemaID ");
			consulta.append("and CE.ClienteClave = '" +clienteClave+ "' ");
			consulta.append("inner join Precio P ON P.PrecioClave = PCE.PrecioClave ");
	...
	public static class ConsultaNotaVentaFerguez {
		public static ISetDatos obtenerVendedor(String vendedorId) throws  Exception {
			StringBuilder consulta = new StringBuilder();
			consulta.append("select RUTClave from VenRut where VendedorID = '" + vendedorId + "'");
			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerDomicilioCliente(String clienteClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("select Calle, ifnull('#'+numero, '') as numero  from ClienteDomicilio where Tipo = 2 and ClienteClave = '" + clienteClave +"'");
			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerVentaDetalle(String transprodId) throws Exception {
			StringBuilder consulta = new StringBuilder();
			consulta.append("select p.Nombre, td.TipoUnidad, td.Cantidad, td.Precio, td.Promocion,");
			consulta.append("ifnull((select SUM(ImpuestoPU) from TPDImpuesto where TransProdDetalleID = td.TransProdDetalleID),0) as Imp,");
			consulta.append("(select ppv.PrecioSugerido from PrecioProductoVig ppv");
			consulta.append(" inner join PrecioClienteEsquema pce on pce.PrecioClave = ppv.PrecioClave and ppv.ProductoClave =td.ProductoClave and ppv.PRUTipoUnidad =td.TipoUnidad and ppv.PPVFechaInicio<=DATETIME('now')");
			consulta.append("and pce.ModuloMovDetalleClave in(select ModuloMovDetalleClave from ModuloMovDetalle where TipoIndice =9)");
			consulta.append(" inner join ClienteEsquema ce on ce.EsquemaID = pce.EsquemaID limit 1) as PrecioSugerido ");

			consulta.append(" from  TransProdDetalle td ");
			consulta.append(" inner join Producto p on p.ProductoClave = td.ProductoClave and td.TransProdId = '" + transprodId + "'");
			consulta.append(" order by p.Nombre");

			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerDesgloseDescuento(String transprodId) throws Exception {
			StringBuilder consulta = new StringBuilder();
			consulta.append("select y.Nombre, y.ProductoClave, SUM(y.precio) as Precio, SUM(y.Descuento) as Descuento, SUM(y.promocion) as Promocion, sum(y.Imp) as Imp from (");
			consulta.append("select p.Nombre, p.ProductoClave, td.Precio,");
			consulta.append("ifnull((select SUM(ImpuestoPU) from TPDImpuesto where TransProdDetalleID = td.TransProdDetalleID),0) as Imp,");
			consulta.append("ifnull((select SUM(PromocionImp) from TrpPrp where TransProdDetalleID = td.TransProdDetalleID and PromocionClave like 'C%'),0) as Descuento,");
			consulta.append("ifnull((select SUM(PromocionImp) from TrpPrp where TransProdDetalleID = td.TransProdDetalleID and PromocionClave like 'P%'),0) as Promocion");
			consulta.append(" from  TransProdDetalle td");
			consulta.append(" inner join Producto p on p.ProductoClave = td.ProductoClave and td.TransProdId = '"+transprodId+"') y");
			consulta.append(" group by y.ProductoClave, y.Nombre");
			consulta.append(" order by y.Nombre");

			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerVentasTransProd(String visitaClave) throws Exception {
			StringBuilder consulta = new StringBuilder();
			consulta.append("select t.TransProdID, t.CFVTipo, t.Folio, t.Subtotal, t.Total,");
			consulta.append("ifnull((select SUM(PromocionImp) from TrpPrp where PromocionClave like 'C%' and TransProdId = t.TransProdId),0) as Des,");
			consulta.append("ifnull((select SUM(PromocionImp) from TrpPrp where PromocionClave like 'P%' and TransProdId = t.TransProdId),0) as Prom ");
			consulta.append(" from TransProd t where IFNULL(t.VisitaClave1, t.VisitaClave) = '" + visitaClave + "'");
			consulta.append(" and t.Tipo = 1 and t.TipoFase <> 0");

			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerConsignacion(String visitaClave) throws Exception {
			StringBuilder consulta = new StringBuilder();
			consulta.append("select Folio, TransProdID, ");
			consulta.append("ifnull((select SUM(PromocionImp) from TrpPrp where PromocionClave like 'C%' and TransProdId = t.TransProdId),0) as Desc,");
			consulta.append("ifnull((select SUM(PromocionImp) from TrpPrp where PromocionClave like 'P%' and TransProdId = t.TransProdId),0) as Prom");
			consulta.append(" from TransProd t where Tipo = 24 and TipoFase = 2 and VisitaClave = '" + visitaClave + "'");

			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerCobranza(String visitaClave) throws Exception {
			StringBuilder consulta = new StringBuilder();
			consulta.append("select a.ABNId, ad.TipoPago, atp.Importe, t.Folio from Abono a");
			consulta.append(" inner join ABNDetalle ad on ad.ABNId = a.ABNId and a.VisitaClave ='"+visitaClave+"'");
			consulta.append(" inner join AbnTrp atp on atp.ABNId = a.ABNId");
			consulta.append(" inner join TransProd t on t.TransProdID = atp.TransProdID");
			consulta.append(" order by t.Folio asc");

			return BDVend.consultar(consulta.toString());
		}

		public static String obtenerFormaPago(String vavclave) throws Exception {
			StringBuilder consulta = new StringBuilder();
			consulta.append("select IFNULL(ClaveSat,'') as ClaveSAT, IFNULL(DescripcionSAT, '') as DescripcionSAT from VARValor vr");
			consulta.append(" inner join VAVDescripcion vd on vd.VAVClave = vr.VAVClave and vd.VARCodigo = 'PAGO' and vr.VAVClave = '"+ vavclave +"'");

			String formaPago = "";
			ISetDatos datos = BDTerm.consultar(consulta.toString());
			if(datos.getCount() > 0) {
				datos.moveToFirst();
				formaPago = datos.getString("ClaveSAT") + "-" + datos.getString("DescripcionSAT");
			}
			return formaPago;
		}

		public static int mostrarSaldoEnvase(String clave, String cliente) throws  Exception {
			StringBuilder consulta = new StringBuilder();
			int cuantos = 0;

			consulta.append("select COUNT(*) as cuantos from Esquema e");
			consulta.append(" inner join ClienteEsquema ce on ce.EsquemaID = e.EsquemaID and ce.ClienteClave = '"+cliente+"' and e.Clave ='"+clave+"'");
			ISetDatos dato = BDVend.consultar(consulta.toString());
			if(dato.getCount() > 0 ){
				dato.moveToFirst();
				cuantos = dato.getInt("cuantos");
			}

			return cuantos;
		}

		public static ISetDatos obtenerEnvase(String visitaClave, String cliente) throws Exception{
			StringBuilder consulta = new StringBuilder();

			consulta.append("select y.Nombre, y.ProductoClave, SUM(y.Ini) as Ini, SUM(y.Car) as Car, SUM(y.Abo) as Abo, SUM(y.Vta) as Vta,");
			consulta.append("ifnull((select pv.Precio from PrecioClienteEsquema pce");
			consulta.append("		inner join ClienteEsquema  ce on ce.EsquemaID = pce.EsquemaID and ce.ClienteClave = '"+cliente+"' and");
			consulta.append(" pce.ModuloMovDetalleClave in(select ModuloMovDetalleClave from ModuloMovDetalle where TipoIndice = 9)");
			consulta.append(" inner join PrecioProductoVig pv on pv.ProductoClave = y.ProductoClave and pv.PRUTipoUnidad = y.TipoUnidad and pv.PPVFechaInicio <= DATETIME('now') limit 1 ");
			consulta.append("),0) as Precio");

			consulta.append(" from(");

			consulta.append(" select p.Nombre, p.ProductoClave, 1 as TipoUnidad, SUM(pp.saldo) as Ini, 0 as Car, 0 as Abo, 0 as Vta from ProductoPrestamoCli pp");
			consulta.append(" inner join Producto p on p.ProductoClave = pp.ProductoClave and pp.ClienteClave ='"+cliente+"'");
			consulta.append(" group by p.Nombre, p.ProductoClave");

			consulta.append(" union all");

			//Consignacion de envase realizadas en visitas anteriores
			consulta.append(" select p.Nombre,p.ProductoClave, td.TipoUnidad, sum(td.cantidad) as Ini, 0 as Car, 0 as Abo, 0 as Vta from TransProd t");
			consulta.append(" inner join TransProdDetalle td on td.TransProdID = t.TransProdID and t.Tipo =24 and t.TipoFase = 2 and VisitaClave<> '"+visitaClave+"' and t.VisitaClave1 = null");
			consulta.append(" inner join Producto p on p.ProductoClave = td.ProductoClave and p.Contenido = 1 and p.Venta = 1");
			consulta.append(" group by p.Nombre, p.ProductoClave, td.TipoUnidad");

			consulta.append(" union all");

			//13.3	Ventas y Consignaciones de envase asociado a producto terminado, realizadas en la visita actual
			consulta.append(" select p.Nombre, p.ProductoClave, td.TipoUnidad, 0 as Ini, SUM(td.cantidad) as Car, 0 as Abo, 0 as Vta from TransProd t");
			consulta.append(" inner join TransProdDetalle td on t.TransProdID = td.TransProdID and ifnull(t.VisitaClave1, t.VisitaClave)='"+visitaClave+"'");
			consulta.append(" and t.Tipo in(1,24) and (t.TipoFase = 2 or t.TipoFase = 3)");
			consulta.append(" inner join ProductoDetalle pd on pd.ProductoClave = td.ProductoClave and PRUTipoUnidad = td.TipoUnidad and ProductoDetClave = pd.ProductoClave and pd.Prestamo = 1");
			consulta.append(" inner join Producto p on p.ProductoClave = pd.ProductoClave");
			consulta.append(" group by p.Nombre, p.ProductoClave, td.TipoUnidad");
			...
			//Devoluciones
			consulta.append(" select p.Nombre, p.ProductoClave, td.TipoUnidad, 0 as Ini, 0 as Car, SUM(td.cantidad) as Abo, 0 as Vta from TransProd t");
			consulta.append(" inner join TransProdDetalle td on td.TransProdID = t.TransProdID and t.VisitaClave ='"+visitaClave+"' and t.Tipo = 3 and t.TipoFase = 1");
			consulta.append(" inner join Producto p on p.ProductoClave = td.ProductoClave and p.Contenido = 1 and p.Venta = 1");
			consulta.append(" group by p.Nombre, p.ProductoClave, td.TipoUnidad");

			consulta.append(" union all");

			//13.5	Ventas de envase provenientes de una consignación realizadas en la visita actual:
			consulta.append(" select p.Nombre, p.ProductoClave, td.TipoUnidad, 0 as Ini, 0 as Car, 0 as Abo, SUM(td.Cantidad) as Vta from TransProd t");
			consulta.append(" inner join TransProdDetalle td on td.TransProdID = t.TransProdID and t.TransProdID in(select FacturaID from TransProd where Tipo = 24)");
			consulta.append(" and t.VisitaClave ='"+visitaClave+"' and t.Tipo = 1 and t.TipoFase = 2");
			consulta.append(" inner join Producto p on p.ProductoClave = td.ProductoClave and p.Contenido = 1 and p.Venta = 1");
			consulta.append(" group by p.Nombre, p.ProductoClave, td.TipoUnidad ) y");

			consulta.append(" group by y.Nombre, y.ProductoClave, y.TipoUnidad");

			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerVentasCredito(String cliente) throws Exception{
			return BDVend.consultar("select ifnull(SUM(saldo),0) as Saldo, count(*) as cuantos from TransProd where Tipo = 1 and TipoFase <> 0 and CFVTipo = 2 and ClienteClave = '" + cliente +"'");
		}

		public static ISetDatos obtenerConsignasSaldo(String cliente) throws  Exception {
			StringBuilder consulta = new StringBuilder();

			consulta.append("select ifnull(SUM(saldo),0) as Saldo from TransProd where TransProdID in(");
			consulta.append("select distinct t.TransProdID from TransProd t");
			consulta.append(" inner join TransProdDetalle td on td.TransProdID = t.TransProdID and t.Tipo = 24 and t.TipoFase = 2 and t.ClienteClave = '"+cliente+"'");
			consulta.append(" inner join ProductoDetalle pd on pd.ProductoClave = td.ProductoClave and pd.PRUTipoUnidad = td.TipoUnidad and pd.ProductoDetClave = td.ProductoClave)");
	...
	public static class ConsultasVentasOSO{
		public static String ObtenerTransprodSubEmpresa(String VisitaClave, int Tipo) throws Exception{
			String TransprodId = "";
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT T.TransprodId ");
			consulta.append("FROM TransProd T ");
			if(Tipo == 1){
				consulta.append("where T.VisitaClave = '" + VisitaClave + "' Limit 1 ");
			}
			else
				consulta.append("where T.VisitaClave1 = '" + VisitaClave + "' Limit 1 ");
			ISetDatos Trans = BDVend.consultar(consulta.toString());

			if (Trans.getCount() > 0) {
				Trans.moveToFirst();
				TransprodId = Trans.getString(0);
			}
			return TransprodId;
		}

		public static ISetDatos ObtenerTransprodVisita(String VisitaClave, int Tipo) throws Exception{
			String TransprodId = "";
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT distinct T.TransprodId ");
			consulta.append("FROM TransProd T ");
			if(Tipo == 1){
				consulta.append("where T.VisitaClave = '" + VisitaClave + "' ");
			}
			else if(Tipo == 2){
				consulta.append("where T.VisitaClave1 = '" + VisitaClave + "' ");
			}
			else
				consulta.append("where T.VisitaClave = '" + VisitaClave + "' or T.VisitaClave1 = '" + VisitaClave + "' ");
			consulta.append("order by T.Folio ");
			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerDetallesVenta(String TransprodID, int TipoFase, String ClienteClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT TD.ProductoClave, P.Nombre, TD.Cantidad, case when TD.Promocion = 1 then(TD.Precio + IFNULL(SUM(TI.ImpuestoPU), 0)) else (select Precio from PrecioProductoVig where productoclave = TD.ProductoClave and PrecioClave =(Select EsquemaID from ClienteEsquema where clienteClave = '"+ClienteClave+"' ) limit 1)end as PrecioUnitario, ((TD.Precio + IFNULL(SUM(TI.ImpuestoPU), 0)) * TD.Cantidad) as SubTotal, IFNULL(TD.Promocion,0) as Promocion ");
			consulta.append("FROM TransProd T ");
			consulta.append("INNER JOIN TransProdDetalle TD on T.TransprodId = TD.TransProdID AND T.TransProdID = '"+TransprodID+"' ");
			consulta.append("INNER JOIN Producto P on TD.ProductoClave = P.ProductoClave  and P.Contenido <> 1 ");
			consulta.append("LEFT JOIN TPDImpuesto TI on T.TransProdID = TI.TransProdID and TD.TransProdDetalleID = TI.TransProdDetalleID  ");
			if( TipoFase != 3)
				consulta.append("WHERE T.Tipo = 1 and T.TipoFase = "+TipoFase+" ");
			else
				consulta.append("WHERE T.Tipo = 1 and T.TipoFase = 2 ");
			consulta.append("group by TD.ProductoClave, P.Nombre, TD.Cantidad ");
			consulta.append("ORDER BY TD.ProductoClave");
			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerVentaEnvase(String TransprodID) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT distinct TD.Cantidad, P.Nombre, TD.Total ");
			consulta.append("FROM TransProd T ");
			consulta.append("INNER JOIN TransProdDetalle TD on T.TransprodId = TD.TransProdID AND T.TransProdID = '"+TransprodID+"' ");
			consulta.append("INNER JOIN Producto P on TD.ProductoClave = P.ProductoClave and P.Contenido = 1 ");
			consulta.append("INNER JOIN ProductoDetalle PD on P.ProductoClave = PD.ProductoClave and PD.ProductoClave = PD.ProductoDetClave ");
			consulta.append("WHERE T.Tipo = 1 and T.TipoFase <> 0 ");
			consulta.append("ORDER BY TD.ProductoClave");
			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerDevolucionEnvase(String VisitaClave, int Tipo) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT distinct SUM(TD.Cantidad) as Cantidad, P.Nombre as Envase, SUM(TD.Total) as Importe ");
			consulta.append("FROM Visita V ");
			if(Tipo == 1)
				consulta.append("INNER JOIN TransProd T on V.VisitaClave = T.VisitaClave and V.VisitaClave = '"+VisitaClave+"' ");
			else
				consulta.append("INNER JOIN TransProd T on V.VisitaClave = T.VisitaClave and V.VisitaClave = '"+VisitaClave+"' ");
			consulta.append("INNER JOIN TransProdDetalle TD on T.TransprodId = TD.TransProdID ");
			consulta.append("INNER JOIN Producto P on TD.ProductoClave = P.ProductoClave and P.Contenido = 1 and P.Venta = 1 ");
			consulta.append("INNER JOIN ProductoDetalle PD on P.ProductoClave = PD.ProductoClave and PD.ProductoClave = PD.ProductoDetClave ");
			consulta.append("WHERE T.Tipo = 3 and T.TipoFase <> 0 ");
			consulta.append("GROUP BY P.Nombre ");
			consulta.append("ORDER BY TD.ProductoClave");
			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerBonificaciones(String TransprodID, String ClienteClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("Select distinct TD.ProductoClave, TD.Cantidad, IFNULL(PR.Nombre, (Select Nombre from Promocion Promo where PromocionClave = TD.PromocionClave LIMIT 1)) as Nombre, ");
			consulta.append("(-1*IFNULL(IFNULL(TP.PromocionImp,(Select Precio from PrecioProductoVig where ProductoClave = TD.ProductoClave and PrecioClave = (Select ListaPrecioBase from Vendedor where VendedorID =(Select VendedorID from Visita where VisitaClave = T.VisitaClave Limit 1) Limit 1)Limit 1)),0)) as ImporteBase, ");
			consulta.append("(-1*IFNULL(Case when TP.PromocionImp = 0 then (select Precio from PrecioProductoVig where productoclave = TD.ProductoClave and PrecioClave =(Select EsquemaID from ClienteEsquema where clienteClave = '"+ClienteClave+"' ) limit 1) else TP.PromocionImp end,(Select Precio from PrecioProductoVig where ProductoClave = TD.ProductoClave and PrecioClave = (Select ListaPrecioBase from Vendedor where VendedorID =(Select VendedorID from Visita where VisitaClave = T.VisitaClave Limit 1) Limit 1)Limit 1))) as Importe ");
			consulta.append("FROM TransProd T ");
			consulta.append("INNER JOIN TransProdDetalle TD on T.TransprodId = TD.TransProdID AND T.TransProdID = '"+TransprodID+"' ");
			consulta.append("inner join Producto P on TD.ProductoClave = P.ProductoClave ");
			consulta.append("left join TrpPrp TP on T.TransProdID = TP.TransProdID and TD.TransProdDetalleID = TP.TransProdDetalleID ");
			consulta.append("left join Promocion PR on TP.PromocionClave = PR.PromocionClave ");
			consulta.append("where TP.PromocionClave is not null and TP.PromocionClave like 'B%' ");
			consulta.append("ORDER BY TD.ProductoClave");
			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerBonificacionesPrev(String TransprodID, String ClienteClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("Select distinct TD.ProductoClave, TD.Cantidad, IFNULL(PR.Nombre, (Select Nombre from Promocion Promo where PromocionClave = TD.PromocionClave LIMIT 1)) as Nombre, ");
			consulta.append("(-1*IFNULL(IFNULL(TP.PromocionImp,(Select Precio from PrecioProductoVig where ProductoClave = TD.ProductoClave and PrecioClave = (Select ListaPrecioBase from Vendedor where VendedorID =(Select VendedorID from Visita where VisitaClave = T.VisitaClave Limit 1) Limit 1)Limit 1)),0)) as ImporteBase, ");
			consulta.append("(-1*IFNULL(Case when TP.PromocionImp = 0 then (select Precio from PrecioProductoVig where productoclave = TD.ProductoClave and PrecioClave =(Select EsquemaID from ClienteEsquema where clienteClave = '"+ClienteClave+"' ) limit 1) else TP.PromocionImp end,(Select Precio from PrecioProductoVig where ProductoClave = TD.ProductoClave and PrecioClave = (Select ListaPrecioBase from Vendedor where VendedorID =(Select VendedorID from Visita where VisitaClave = T.VisitaClave Limit 1) Limit 1)Limit 1))) as Importe ");
		...
		public static ISetDatos obtenerPromociones(String TransprodID, String ClienteClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("Select distinct TD.ProductoClave, IFNULL((Select Cantidad from TransprodDetalle where TransprodId = T.TransprodId and PromocionClave = TP.PromocionClave and Promocion = 2),TD.Cantidad) as Cantidad, IFNULL(PR.Nombre, (Select Nombre from Promocion Promo where PromocionClave = TD.PromocionClave LIMIT 1)) as Nombre, ");
			consulta.append("(-1*IFNULL(IFNULL(TP.PromocionImp,(Select Precio from PrecioProductoVig where ProductoClave = TD.ProductoClave and PrecioClave = (Select ListaPrecioBase from Vendedor where VendedorID =(Select VendedorID from Visita where VisitaClave = T.VisitaClave Limit 1) Limit 1)Limit 1)),0)) as ImporteBase, ");
			consulta.append("(-1*IFNULL(Case when TP.PromocionImp = 0 then (select Precio from PrecioProductoVig where productoclave = TD.ProductoClave and PrecioClave =(Select EsquemaID from ClienteEsquema where clienteClave = '"+ClienteClave+"' ) limit 1) else TP.PromocionImp end,(Select Precio from PrecioProductoVig where ProductoClave = TD.ProductoClave and PrecioClave = (Select ListaPrecioBase from Vendedor where VendedorID =(Select VendedorID from Visita where VisitaClave = T.VisitaClave Limit 1) Limit 1)Limit 1))) as Importe ");
			consulta.append("FROM TransProd T ");
			consulta.append("INNER JOIN TransProdDetalle TD on T.TransprodId = TD.TransProdID AND T.TransProdID = '"+TransprodID+"' ");
			...
			consulta.append("left join TrpPrp TP on T.TransProdID = TP.TransProdID and TD.TransProdDetalleID = TP.TransProdDetalleID ");
			consulta.append("left join Promocion PR on TP.PromocionClave = PR.PromocionClave ");
			consulta.append("where TP.PromocionClave is not null and TP.PromocionClave not like 'B%' ");
			consulta.append("ORDER BY TD.ProductoClave");
			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerPromocionesPrev(String TransprodID, String ClienteClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("Select distinct TD.ProductoClave, IFNULL((Select Cantidad from TransprodDetalle where TransprodId = T.TransprodId and PromocionClave = TP.PromocionClave and Promocion = 2),TD.Cantidad) as Cantidad, IFNULL(PR.Nombre, (Select Nombre from Promocion Promo where PromocionClave = TD.PromocionClave LIMIT 1)) as Nombre, ");
			consulta.append("(-1*IFNULL(IFNULL(TP.PromocionImp,(Select Precio from PrecioProductoVig where ProductoClave = TD.ProductoClave and PrecioClave = (Select ListaPrecioBase from Vendedor where VendedorID =(Select VendedorID from Visita where VisitaClave = T.VisitaClave Limit 1) Limit 1)Limit 1)),0)) as ImporteBase, ");
			consulta.append("(-1*IFNULL(Case when TP.PromocionImp = 0 then (select Precio from PrecioProductoVig where productoclave = TD.ProductoClave and PrecioClave =(Select EsquemaID from ClienteEsquema where clienteClave = '"+ClienteClave+"' ) limit 1) else TP.PromocionImp end,(Select Precio from PrecioProductoVig where ProductoClave = TD.ProductoClave and PrecioClave = (Select ListaPrecioBase from Vendedor where VendedorID =(Select VendedorID from Visita where VisitaClave = T.VisitaClave Limit 1) Limit 1)Limit 1))) as Importe ");
		...
		public static ISetDatos obtenerConsignas(String TransprodID, String ClienteClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("Select distinct TD.ProductoClave, P.Nombre, TD.Cantidad, IFNULL((select Precio from PrecioProductoVig where productoclave = TD.ProductoClave and PrecioClave =(Select EsquemaID from ClienteEsquema where clienteClave = '"+ClienteClave+"' ) limit 1),(Select Precio from PrecioProductoVig where ProductoClave = TD.ProductoClave and PrecioClave = (Select ListaPrecioBase from Vendedor where VendedorID =(Select VendedorID from Visita where VisitaClave = T.VisitaClave Limit 1) Limit 1)Limit 1)) as ImporteBase ");
			consulta.append("FROM TransProd T ");
			consulta.append("inner join TransProdDetalle TD on T.TransprodId = TD.TransProdID and T.Tipo = 24 and T.TipoFase = 2 and T.TransProdID = '"+TransprodID+"' ");
			consulta.append("inner join Producto P on TD.ProductoClave = P.ProductoClave ");
//			consulta.append("inner join TPDImpuesto TI on T.TransProdID = TI.TransProdID and TD.TransProdDetalleID = TI.TransProdDetalleID ");
			consulta.append("ORDER BY TD.ProductoClave ");
			return BDVend.consultar(consulta.toString());
		}

		public static ISetDatos obtenerDevolucionConsignas(String TransprodID, String ClienteClave) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("Select distinct TD.ProductoClave, P.Nombre, TD.Cantidad, IFNULL((select Precio from PrecioProductoVig where productoclave = TD.ProductoClave and PrecioClave =(Select EsquemaID from ClienteEsquema where clienteClave = '"+ClienteClave+"' ) limit 1),(Select Precio from PrecioProductoVig where ProductoClave = TD.ProductoClave and PrecioClave = (Select ListaPrecioBase from Vendedor where VendedorID =(Select VendedorID from Visita where VisitaClave = TDev.VisitaClave Limit 1) Limit 1)Limit 1)) as ImporteBase ");
			consulta.append("FROM TrpTpd TT ");
			consulta.append("inner join TransProd TDev on TT.TransprodID = TDev.TransprodID and TDev.Tipo = 3 and TDev.TipoFase = 2 ");
			consulta.append("inner join TransProdDetalle TD on TDev.TransprodId = TD.TransProdID ");
			consulta.append("inner join Producto P on TD.ProductoClave = P.ProductoClave ");
//			consulta.append("inner join TPDImpuesto TI on TDev.TransProdID = TI.TransProdID and TD.TransProdDetalleID = TI.TransProdDetalleID ");
			consulta.append("inner join Transprod TC on TT.TransProdID1 = TC.TransProdID and TC.Tipo = 24 and TC.TipoFase in (6,8) and TC.TransProdID = '"+TransprodID+"' ");
			consulta.append("ORDER BY TD.ProductoClave ");
			return BDVend.consultar(consulta.toString());
		}
	}

	public static final class ConsultasAddenda {

		public static String obtenerAddendaCliente(String clienteClave) throws Exception {
			String addId = "";
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("select ADDId from AddendaCliente adc ");
			sConsulta.append("inner join AddendaDetalle adt on adc.ADDId = adt.ADDId ");
			sConsulta.append("where ClienteClave = '" + clienteClave + "' and adt.Tipo = 2 ");
			sConsulta.append("limit 1 ");
			ISetDatos dtAddenda = BDVend.consultar("select ADDId from AddendaCliente where ClienteClave = '" + clienteClave + "'");

			if (dtAddenda.getCount() > 0) {
				dtAddenda.moveToFirst();
				addId = dtAddenda.getString("ADDId");
			}
			return addId;
		}

		public static ISetDatos obtenerDetalleAddenda(String addId) throws Exception {
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("select ADDDId, TipoDato, LongMin, LongMax, Etiqueta, Valor, Requerido ");
			sConsulta.append("from AddendaDetalle  ");
			sConsulta.append("where ADDId = '" + addId + "' and Tipo = 2 ");

			ISetDatos dtDetalle = BDVend.consultar(sConsulta.toString());
			return dtDetalle;
		}

		public static ISetDatos obtenerDetalleAddendaFactura(String transProdId) throws Exception {
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("select adt.ADDDId, adt.TipoDato, adt.LongMin, adt.LongMax, adt.Etiqueta, adt.Valor, adt.Requerido, adf.Valor as Respuesta ");
			sConsulta.append("from AddendaFactura adf ");
			sConsulta.append("inner join AddendaDetalle adt on adf.ADDDId = adt.ADDDId ");
			sConsulta.append("where adf.TransProdId = '" + transProdId + "' ");
	...
	public static final class ReporteInventarioNOR {

		public static ISetDatos ObtenerInventario(String diaClave) throws Exception {
			StringBuilder consulta = new StringBuilder();
			consulta = new StringBuilder();
			consulta.append("Select pro.ProductoClave || '-' || p.Nombre as Nombre,  ");
			consulta.append("(select ifnull(sum(td.cantidad),0) from transprod t inner join TransProdDetalle td on t.TransProdID = td.TransProdID and t.tipo in (2, 23) and t.tipofase = 1 and td.ProductoClave = pro.ProductoClave) as InventarioInicial, ");
			consulta.append("(select ifnull(sum(td.cantidad),0) from transprod t inner join TransProdDetalle td on t.TransProdID = td.TransProdID and t.tipo in (1) and t.tipofase = 2 and td.ProductoClave = pro.ProductoClave where DiaClave = '" + diaClave + "') as Ventas, ");
			consulta.append("(select ifnull(sum(td.cantidad),0) from transprod t inner join TransProdDetalle td on t.TransProdID = td.TransProdID and t.tipo in (9) and t.tipofase = 1 and t.tipomovimiento = 1 and td.ProductoClave = pro.ProductoClave where DiaClave = '" + diaClave + "') as Devoluciones ");
			consulta.append("from ");
			consulta.append("( ");
			consulta.append("select distinct tpd.ProductoClave ");
			consulta.append("from TransProd tp ");
			consulta.append("inner join TransProdDetalle tpd on tp.TransProdID = tpd.TransProdID ");
			consulta.append("where tp.tipo in (2, 23) and tp.tipofase = 1 ");
			consulta.append(") pro ");
			consulta.append("inner join Producto p on pro.productoclave = p.productoclave ");
			ISetDatos datos = BDVend.consultar(consulta.toString());
			return datos;
		}
	}

	public static final class ConsultasReportesMelbrin {

		//	REPORTE 57 EFECTIVIDAD POR RUTAS

		public static ISetDatos obtenerClientesEnAgendaMEL(String ruta, String diaClave) throws Exception{

			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT DISTINCT ")
					.append("	C.ClienteClave, C.RazonSocial, time(V.FechaHoraInicial) AS Inicio, time(V.FechaHoraFinal) AS Fin ")
					.append("FROM Cliente C ")
					.append("	INNER JOIN Agenda A on C.ClienteClave = A.ClienteClave AND A.RutClave = '" + ruta + "' ")
					.append("	INNER JOIN Visita V on C.ClienteClave = V.ClienteClave AND V.RUTClave = '" + ruta + "' AND V.DiaClave = '" + diaClave + "' ")
					.append("ORDER BY time(V.FechaHoraInicial) asc ");

			return  BDVend.consultar(consulta.toString());
		}
	}

	public static final class ConsultaReporteVentasFRI {
		...
		public static ISetDatos obtenerVentasGeneralTipo(String diaClave, int tipoVenta)throws Exception {
			StringBuilder consulta = new StringBuilder();
			int tipoModulo = Integer.parseInt(Sesion.get(Campo.TipoModulo).toString());

			consulta.append("SELECT T.Folio AS Folio, C.NombreCorto AS NombreCorto, T.Total AS Total, T.CFVTipo, T.TipoTurno ");
			consulta.append("FROM TransProd T");
			consulta.append("\tINNER JOIN Visita V ON IFNULL(T.VisitaClave1, T.VisitaClave) = V.VisitaClave AND IFNULL(T.DiaClave1, T.DiaClave) = V.DiaClave");
			consulta.append("\tINNER JOIN Cliente C ON V.ClienteClave = C.ClienteClave ");
			consulta.append("WHERE T.Tipo = 1 AND T.TipoFase IN (" + (tipoModulo == Enumeradores.TiposModulos.PREVENTA ? "1" : "2,3") + ") ");
			consulta.append("AND V.DiaClave = '"+ diaClave +"' ");
			if (tipoVenta == 1){
				consulta.append("AND T.TipoTurno = 0 AND T.CFVTipo = 1 ");
			} else if (tipoVenta == 2){
				consulta.append("AND T.TipoTurno = 0 AND T.CFVTipo = 2 ");
			} else if (tipoVenta == 3){
				//FACTURAS
				consulta.append("AND T.TipoTurno = 1 ");
			}
			consulta.append("ORDER BY T.Folio");
		...
		public static ISetDatos obtenerVentasDetalladoTipo(String diaClave, int tipoVenta)throws Exception {
			StringBuilder consulta = new StringBuilder();
			int tipoModulo = Integer.parseInt(Sesion.get(Campo.TipoModulo).toString());

			consulta.append("SELECT");
			consulta.append("\tT.Tipo AS Tipo, IFNULL(C.Clave, '') || ' - ' || IFNULL(C.RazonSocial, '') AS Cliente, T.Folio AS Folio, P.ProductoClave || ' - ' || P.Nombre AS Producto, TD.TipoUnidad AS TipoUnidad, TD.Cantidad AS Cantidad,");
			consulta.append("\tCASE WHEN IFNULL(TD.Promocion, 0) = 2 THEN '*' ELSE TD.Precio END AS Precio, ");
			consulta.append("\tCASE WHEN IFNULL(TD.Promocion, 0) = 2 THEN '' ELSE ((TD.Subtotal + TD.Impuesto) - IFNULL(TDS.DesImporte, 0) - IFNULL(TDSV.DesImporte, 0) - IFNULL(TDS.DesImpuesto, 0) - IFNULL(TDSV.DesImpuesto, 0)) END AS Total,");
			consulta.append("\tT.CFVTipo, T.TipoTurno ");
			consulta.append("FROM TransProd T");
			consulta.append("\tINNER JOIN Visita V ON IFNULL(T.VisitaClave1, T.VisitaClave) = V.VisitaClave AND IFNULL(T.DiaClave1, T.DiaClave) = V.DiaClave");
			consulta.append("\tINNER JOIN Cliente C ON V.ClienteClave = C.ClienteClave");
			consulta.append("\tINNER JOIN TransProdDetalle TD ON T.TransProdID = TD.TransProdID");
	...
	public static final class ConsultaReporteHistoricoVisitas_MED {

		public static ISetDatos obtenerHistoricoVisitasGeneral(String filtroFechas, String clientesSeleccionados)throws Exception {
			StringBuilder consulta = new StringBuilder();

			consulta.append("SELECT ");
			consulta.append("\tHV.ClienteClave, HV.RazonSocial, HV.NumVentas, HV.NumVisitas ");
			consulta.append("FROM HistoricoVisita HV ");
			consulta.append("WHERE " + filtroFechas + " ");
			if (!clientesSeleccionados.isEmpty()){
				consulta.append("\tAND HV.ClienteClave IN (" + clientesSeleccionados + ") ");
			}
			consulta.append("ORDER BY HV.ClienteClave");

			ISetDatos datos = BDVend.consultar(consulta.toString());
			return datos;
		}

		public static ISetDatos obtenerHistoricoVisitasDetallado(String filtroFechas, String clientesSeleccionados)throws Exception {
			StringBuilder consulta = new StringBuilder();

			consulta.append("SELECT ");
			consulta.append("\tHV.ClienteClave, HV.RazonSocial, HV.NumVentas, HV.NumVisitas, HV.VtasPresencial, HV.VtasNoPresencial,");
			consulta.append("\tHV.VisitaPresencial, HV.VisitaNoPresencial ");
			consulta.append("FROM HistoricoVisita HV ");
			consulta.append("WHERE " + filtroFechas + " ");
			if (!clientesSeleccionados.isEmpty()){
				consulta.append("\tAND HV.ClienteClave IN (" + clientesSeleccionados + ") ");
