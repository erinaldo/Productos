...
public class BuscaProducto extends Vista implements IBuscaProducto, CalcularPorKiloListener
{
	...
	private boolean excluirMensajePrecio = false;
	private int selectedItem = 0;
	private boolean busquedaFiltro = false;

	private boolean modifcarPrecio = false;
	private String clavePrecioMod;
	private Float precioMod;
	...
	private HashMap<InventarioLotesKey, TransProdDetalle> seleccionTPDLotes; // = new HashMap<InventarioLotesKey, TransProdDetalle>();
    ...
    private boolean bBusquedaSimple = false;

    private boolean bExcluirCanjes = false;

	private boolean bManejoDobleUnidad = false;
	...
	public void onCreate(Bundle savedInstanceState)
	{
		try {
			...
			bManejoDobleUnidad =  Boolean.parseBoolean(m.get("ManejoDobleUnidad").toString().equals("0") ? "false" : "true");
			...
			if (inventarioPorLotes)
				seleccionTPDLotes = new HashMap<InventarioLotesKey, TransProdDetalle>();
			else
				seleccionTPD = new HashMap<String, TransProdDetalle>();
			/*
            ...
            if (oParametros != null) {
                ...
                if (oParametros.get("ListaPrecios") != null && !oParametros.get("ListaPrecios").toString().equals("")) {
					...
					if(oParametros.get("ExcluirMensajePrecio") != null && oParametros.get("ExcluirMensajePrecio").toString().equals("true")){
						excluirMensajePrecio= true;
					}
                ...
                if (oParametros.get("ExcluirCanjes") != null) {//Excluir los productos Tipo Canje
                    bExcluirCanjes  = Boolean.parseBoolean(oParametros.get("ExcluirCanjes").toString());//Excluir los productos Tipo Canje
                }
				if (oParametros.get("ProductoEspecifico")!= null){
					productoEspecifico =oParametros.get("ProductoEspecifico").toString();
					if(productoEspecifico.length()>0) {
						filtroProducto = "PRO.ProductoClave='" +  productoEspecifico + "'";
					}
				}
            }
            ...
            if (((Vendedor) Sesion.get(Campo.VendedorActual)).MostrarEsquema && !busquedaFiltro) {
                ...
                if (mAccion.equals(Enumeradores.Acciones.ACCION_OBTENER_PRODUCTOS_SELECCIONADOS))
                    productos = Consultas.ConsultasProducto.obtenerProductosExistencia(filtroEsquema + filtroProducto, precioClave, filtrar, transProdIds, tipoTransProd, (tipoValidarExistencia == TiposValidacionInventario.NoValidar ? false : true), tipoMovimiento, esquemasModulo, ubicacionExistencia, bExcluirCanjes);//Envase Devolucion Cliente
                else if (mAccion.equals(Enumeradores.Acciones.ACCION_OBTENER_BUSQUEDA_SIMPLE)) {
                    btn.setVisibility(View.GONE);
                    bBusquedaSimple = true;
					if (bManejoDobleUnidad){
						productos = Consultas.ConsultasProducto.obtenerProductosExistenciaDobleUnidad(filtroEsquema + filtroProducto, precioClave, filtrar, transProdIds, tipoTransProd, (tipoValidarExistencia == TiposValidacionInventario.NoValidar ? false : true), tipoMovimiento, esquemasModulo, ubicacionExistencia, bExcluirCanjes);

					}else{
						productos = Consultas.ConsultasProducto.obtenerProductosExistencia(filtroEsquema + filtroProducto, precioClave, false, transProdIds, tipoTransProd, (tipoValidarExistencia == TiposValidacionInventario.NoValidar ? false : true), tipoMovimiento, esquemasModulo, ubicacionExistencia, bExcluirCanjes);
					}
                } else
                    productos = Consultas.ConsultasProducto.obtenerProductosExistencia(filtroEsquema + filtroProducto, precioClave, filtrar, transProdIds, tipoTransProd, (tipoValidarExistencia == TiposValidacionInventario.NoValidar ? false : true), tipoMovimiento, esquemasModulo, ubicacionExistencia, bExcluirCanjes);//Envase Devolucion Cliente
			...
			mostrarError("Error de nulos en la busqueda ",3);
		}
		catch (ControlError e)
		{
			mostrarError(e.getMessage(),3);
		}
		catch (Exception ex)
		{
	    ...
	    public void onDestroy() {
          ...
          try {
              ...
              if (listProductos.getAdapter() != null) {
                  stopManagingCursor(((Cursor) (((SimpleCursorAdapter) listProductos.getAdapter()).getCursor())));
                  ((Cursor) (((SimpleCursorAdapter) listProductos.getAdapter()).getCursor())).close();
              }
          }catch(Exception ex){
              mostrarError("Error al cerrar Cursores");
          }
	    }
	...
	public void onWindowFocusChanged(boolean hasFocus)
	{
		...
		if (hasFocus)
			if (!bEsVisible)
			{
				...
				if (eCantidades != null && eCantidades.size()>0) {
				...
				if (keyCode == KeyEvent.KEYCODE_ENTER)
				{
					try
					{
						...
						if (!cargando)
						{
							...
							if (mAccion.equals(Enumeradores.Acciones.ACCION_OBTENER_PRODUCTOS_SELECCIONADOS))
								productos = Consultas.ConsultasProducto.obtenerProductosExistencia(filtroEsquema + filtroProducto, precioClave, false, transProdIds, tipoTransProd, (tipoValidarExistencia == TiposValidacionInventario.NoValidar ? false : true), tipoMovimiento, esquemasModulo, ubicacionExistencia, bExcluirCanjes);//Envase Devolucion Cliente
                            else if (mAccion.equals(Enumeradores.Acciones.ACCION_OBTENER_BUSQUEDA_SIMPLE))
								if (bManejoDobleUnidad) {
									productos = Consultas.ConsultasProducto.obtenerProductosExistenciaDobleUnidad(filtroEsquema + filtroProducto, precioClave, filtrar, transProdIds, tipoTransProd, (tipoValidarExistencia == TiposValidacionInventario.NoValidar ? false : true), tipoMovimiento, esquemasModulo, ubicacionExistencia, bExcluirCanjes);
								}else{
				...
				if (data != null){
					try{
						...
						if (!esquemasModulo.equals("")){
							...
							if (mAccion.equals(Enumeradores.Acciones.ACCION_OBTENER_PRODUCTOS_SELECCIONADOS))
								productos = Consultas.ConsultasProducto.obtenerProductosExistencia(filtroEsquema + filtroProducto, precioClave, filtrar, transProdIds, tipoTransProd, (tipoValidarExistencia == TiposValidacionInventario.NoValidar ? false : true),tipoMovimiento, esquemasModulo, ubicacionExistencia, bExcluirCanjes);
							else
								productos = Consultas.ConsultasProducto.obtenerProductosExistencia(filtroEsquema + filtroProducto, precioClave, filtrar, transProdIds, tipoTransProd, (tipoValidarExistencia == TiposValidacionInventario.NoValidar ? false : true),tipoMovimiento, esquemasModulo, ubicacionExistencia, bExcluirCanjes);
						}
	...
	public void respuestaMensaje(RespuestaMsg respuesta, int tipoMensaje)
	{
		...
		else if (respuesta.equals(Enumeradores.RespuestaMsg.OK) && errorDeCaptura)
		{
			...
			if(respuesta == RespuestaMsg.Si){
				if (inventarioPorLotes)
					aceptarCantidadPorLotes(etCant);
				else
	...
	{
		if (productos !=null) {
			cantidadProductos = productos.getCount();
		}else{
			cantidadProductos = 0;
		}
        ...
        try {
            ...
            mostrarError("Error al limpiar cursores");
        }
		...
		try
		{
			final MySimpleCursorAdapter adapter;
			...
			{
				adapter = new MySimpleCursorAdapter(this, R.layout.lista_busqueda_producto_lote, productos,
						new String[]
								{ "Unidad", "ProductoClave", "Nombre", "Lote", "Caducidad", "Existencia", "Precio", "Cantidad" },
						new int[]
								{ R.id.txtUnidad, R.id.txtClave, R.id.txtDescripcion, R.id.txtLote, R.id.txtCaducidad, R.id.txtExistencia, R.id.txtPrecio, R.id.npCantidad });
			}else{
				adapter = new MySimpleCursorAdapter(this, R.layout.lista_busqueda_producto, productos,
						new String[]
								{ "Unidad", "ProductoClave", "Nombre", "Existencia", "Precio", "Cantidad", "PrecioSugerido", "SaldoEnvase", "ValorPuntos", "ProductoClave" },
						new int[]
								{ R.id.txtUnidad, R.id.txtClave, R.id.txtDescripcion, R.id.txtExistencia, R.id.txtPrecio, R.id.npCantidad, R.id.txtPrecioSugerido, R.id.txtSaldoEnvase, R.id.txtValorPuntos, R.id.imgProducto });
			}
	...
	private android.view.View.OnClickListener mAgregar = new android.view.View.OnClickListener()
	{
		...
		public void onClick(View v)
		{
			...
			{
				if (inventarioPorLotes)
					Sesion.set(Campo.ResultadoActividad, seleccionTPDLotes);
				else
	...
	private class MySimpleCursorAdapter extends SimpleCursorAdapter
	{
		...
		public View getView(final int pos, View v, ViewGroup parent)
		{
			...
			String clave = (((TextView) v.findViewById(R.id.txtClave)).getText().toString() + "|" + ((TextView) v.findViewById(R.id.txtUnidad)).getTag().toString() + "|" + ((TextView) v.findViewById(R.id.txtExistencia)).getText().toString() + "|" + (modifcarPrecio ? precioMod : (c.isNull(c.getColumnIndex("Precio")) ? null : c.getFloat(c.getColumnIndex("Precio")))) + "|" + c.getInt(c.getColumnIndex("DecimalProducto")) + "|" + pos + "|" + (modifcarPrecio ? clavePrecioMod : c.getString(c.getColumnIndex("PrecioClave"))));
			if (inventarioPorLotes)
				clave += "|" + ((TextView) v.findViewById(R.id.txtLote)).getText().toString() + "|" + (c.isNull(c.getColumnIndex("CantidadActual")) ? null : c.getFloat(c.getColumnIndex("CantidadActual")));

			modifcarPrecio = false;
			et.setTag(clave);
            ...
            if (bBusquedaSimple) {
                v.setOnClickListener(new View.OnClickListener() {
                                         ...
                                         @Override
                                         public void onClick(View v) {
                                             setResultado(Enumeradores.Resultados.RESULTADO_BIEN, ((TextView) v.findViewById(R.id.txtClave)).getText().toString());
                                             finalizar();
                                         }
                                     }
                );
	...
	private void aceptarCantidad(EditText et){
		//Estructura del tag ProductoClave|Unidad|Existencia|Precio|Decimales|posicion|PrecioClave
		String[] llave = et.getTag().toString().split("\\|");
        ...
        String sPrecioClave ="";
        if (!llave[6].equals("null"))
            sPrecioClave = llave[6];
		if(modifcarPrecio && productoClave.equals(claveProducto)){
			precio = precioMod;
			sPrecioClave = clavePrecioMod;
			modifcarPrecio = false;
		}
		...
		try
		{
					...
					else
					{
						if (tipoValidarExistencia != TiposValidacionInventario.NoValidar)
						{
							if(tipoTransProd == TiposTransProd.NO_DEFINIDO && ubicacionExistencia != 0){
                                ...
                                float cantidad = Float.parseFloat(et.getText().toString());
                                ...
                                if ((tipoTransProd == Enumeradores.TiposTransProd.PEDIDO || tipoTransProd == Enumeradores.TiposTransProd.VENTA_CONSIGNACION) && Integer.parseInt(Sesion.get(Campo.TipoModulo).toString()) != Enumeradores.TiposModulos.PREVENTA){
                                    ...
                                    if (mPresenta.validarEnvase(productoClave))
                                    {
                                        saldoPrestamoIni = mPresenta.obtenerSaldoDeEnvase(productoClave);
                                        if (saldoPrestamoIni < 0)
                                            saldoPrestamoIni = 0;

                                        if (saldoPrestamoIni < cantidad)
                                            cantidad -= saldoPrestamoIni;
                                        else
                                            validar = false;
                                    }
                                }
                                ...
                                if (validar) {
                                    if (!Inventario.ValidarExistencia(productoClave, unidad, cantidad, tipoMovimiento, tipoTransProd, existencia, error)) {
                                        if (tipoValidarExistencia == TiposValidacionInventario.ValidacionInformativa) {
                                            mostrarAdvertencia(error.toString());
                                        } else {
                                            errorDeCaptura = true;
							...
							if (validarPrecio) {
								...
								Float precioVol = ListaPrecio.BuscarPrecio(productoClave, unidad, "'" + sPrecioClave + "'", sbListaAplicada, seleccionTPD.get(productoClave).Cantidad);
								if (precioVol !=null && precioVol != precio  && precioVol != precio) {
									seleccionTPD.get(productoClave).Precio = precioVol;
								}
							}
			...
			else if (et.getText().toString().length() > 0 && Float.parseFloat(et.getText().toString()) > 0)
			{
				...
				if (precio == null && validarPrecio && !excluirMensajePrecio)
				{
						errorDeCaptura = true;
						posicionError = pos;
				...
				else
				{
					if (tipoValidarExistencia != TiposValidacionInventario.NoValidar)
					{
						...
						if (validarPrecio) {
							StringBuilder sbListaAplicada = new StringBuilder();
							Float precioVol = ListaPrecio.BuscarPrecio(productoClave, unidad, "'" + sPrecioClave + "'", sbListaAplicada, Float.parseFloat(et.getText().toString()));
							if (precioVol !=null && precioVol != precio  && precioVol != precio) {
								precio = precioVol;
							}
						}
						seleccionTPD.put(productoClave, TransaccionesDetalle.GenerarDetalleBusqueda(productoClave, unidad, Generales.getRound(Float.parseFloat(et.getText().toString()), decimalProducto), (precio == null ? 0 : precio), sPrecioClave, ""));
						et.setText(Generales.getFormatoDecimal(seleccionTPD.get(productoClave).Cantidad, decimalProducto));
					...
					else
					{
						if (validarPrecio) {
							StringBuilder sbListaAplicada = new StringBuilder();
							Float precioVol = ListaPrecio.BuscarPrecio(productoClave, unidad, "'" + sPrecioClave + "'", sbListaAplicada, Float.parseFloat(et.getText().toString()));
							if (precioVol !=null && precioVol >=0 && precioVol != precio) {
								precio = precioVol;
							}
						}
						seleccionTPD.put(productoClave, TransaccionesDetalle.GenerarDetalleBusqueda(productoClave, unidad, Generales.getRound(Float.parseFloat(et.getText().toString()), decimalProducto), (precio == null ? 0 : precio), sPrecioClave, ""));
			...
			else if (capturarCantidadCero){
				if(cantidadProductos == contadorProductos) {
					seleccionTPD.put(productoClave, TransaccionesDetalle.GenerarDetalleBusqueda(productoClave, unidad, Generales.getRound(Float.parseFloat(et.getText().toString()), decimalProducto), (precio == null ? 0 : precio), sPrecioClave, ""));
				}else{
					contadorProductos ++;
				}
	...
	private void aceptarCantidadPorLotes(EditText et){
		//Estructura del tag ProductoClave|Unidad|Existencia|Precio|Decimales|posicion|PrecioClave|Lote|CantidadActual
		String[] llave = et.getTag().toString().split("\\|");
		String productoClave = llave[0];
		Short unidad = Short.parseShort(llave[1]);
		AtomicReference<Float> existencia = new AtomicReference<Float>();
		...
		String lote = llave[7];
		Float cantidadActual = 0f;
		if (llave.length>=9){
			cantidadActual = Float.parseFloat(llave[8]);
		}
		InventarioLotesKey prodLoteKey = null;
		if (seleccionTPDLotes.size() > 0)
			for (InventarioLotesKey inv: seleccionTPDLotes.keySet()) {
				if (inv.ProductoClave.equals(productoClave) && inv.TipoUnidad == unidad && inv.Lote.equals(lote)) {
					prodLoteKey = inv;
					break;
				}
			}
		if (prodLoteKey == null)
			prodLoteKey = new InventarioLotesKey(productoClave, unidad, lote);

		StringBuilder error = new StringBuilder();
		try
		{
			if (seleccionTPDLotes.containsKey(prodLoteKey))
			{
				if (((TransProdDetalle) seleccionTPDLotes.get(prodLoteKey)).Cantidad != Float.parseFloat(et.getText().toString()))
				{
					if (Float.parseFloat(et.getText().toString()) <= 0)
					{
						seleccionTPDLotes.remove(prodLoteKey);
						et.setText(Generales.getFormatoDecimal(0, decimalProducto));
					}
					else
					{
						if (tipoValidarExistencia != TiposValidacionInventario.NoValidar)
						{
							if(tipoTransProd == TiposTransProd.NO_DEFINIDO && ubicacionExistencia != 0){
								...
								if(ubicacionExistencia == 1)
									...
									grupoMotivo = "NO VENTA";
								if(!InventarioLotes.ValidarExistencia(productoClave, unidad, lote, Float.parseFloat(et.getText().toString()), (short) 0, grupoMotivo, existencia, error)){
									if (tipoValidarExistencia == TiposValidacionInventario.ValidacionInformativa)
									{
										mostrarAdvertencia(error.toString());
									}
								...
								if (validar) {
									if (!InventarioLotes.ValidarExistencia(productoClave, unidad, lote, cantidad - cantidadActual, tipoMovimiento, tipoTransProd, existencia, error)) {
										if (tipoValidarExistencia == TiposValidacionInventario.ValidacionInformativa) {
											mostrarAdvertencia(error.toString());
										} else {
											errorDeCaptura = true;
											posicionError = pos;
											mostrarError(error.toString());
											seleccionTPDLotes.get(prodLoteKey).Cantidad = Generales.getRound(existencia.get(), decimalProducto);
											et.setText(Generales.getFormatoDecimal(existencia.get(), decimalProducto));
											return;
										}
									}
							...
							seleccionTPDLotes.get(prodLoteKey).Cantidad = Generales.getRound(Float.parseFloat(et.getText().toString()), decimalProducto);
							if (validarPrecio) {
								StringBuilder sbListaAplicada = new StringBuilder();
								Float precioVol = ListaPrecio.BuscarPrecio(productoClave, unidad, "'" + sPrecioClave + "'", sbListaAplicada, seleccionTPDLotes.get(prodLoteKey).Cantidad);
								if (precioVol !=null && precioVol != precio  && precioVol != precio) {
									seleccionTPDLotes.get(prodLoteKey).Precio = precioVol;
								}
							}
							et.setText(Generales.getFormatoDecimal(seleccionTPDLotes.get(prodLoteKey).Cantidad, decimalProducto));
						}
						else
						{
							seleccionTPDLotes.get(prodLoteKey).Cantidad = Generales.getRound(Float.parseFloat(et.getText().toString()), decimalProducto);
			...
			else if (et.getText().toString().length() > 0 && Float.parseFloat(et.getText().toString()) > 0)
			{
				...
				else
				{
					if (tipoValidarExistencia != TiposValidacionInventario.NoValidar)
					{
						...
						seleccionTPDLotes.put(prodLoteKey, TransaccionesDetalle.GenerarDetalleBusqueda(productoClave, unidad, Generales.getRound(Float.parseFloat(et.getText().toString()), decimalProducto), (precio == null ? 0 : precio), sPrecioClave, lote));
						et.setText(Generales.getFormatoDecimal(seleccionTPDLotes.get(prodLoteKey).Cantidad, decimalProducto));
	...
	{
		boolean inventarioPorLotes;
		boolean imagenesProducto;
		public vista()
		{
			try {
				inventarioPorLotes = (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("InventarioPorLotes") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("InventarioPorLotes").equals("1"));
				imagenesProducto = (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("BusquedaConImagenes") && ((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("BusquedaConImagenes").equals("1"));
			}catch(Exception e){}
		}
		...
		public boolean setViewValue(View view, Cursor cursor, int columnIndex)
		{
			...
			switch (viewId)
			{
				case R.id.txtUnidad:
                    ...
                    if (sColor != "")
                        unidad.setTextColor(Color.parseColor(sColor));
					...
					break;
				case R.id.txtLote:
					TextView lote = (TextView) view;
					lote.setText((cursor.getString(columnIndex) == null ? "": cursor.getString(columnIndex)));
					break;
				case R.id.txtCaducidad:
					TextView caducidad = (TextView) view;
				...
				case R.id.txtExistencia:
					...
					if (tipoValidarExistencia == TiposValidacionInventario.NoValidar || bManejoDobleUnidad)
					{
					...
					if (!cursor.isNull(8)){
						...
						if(!inventarioPorLotes) {
							if (tipoTransProd != TiposTransProd.CANJES) {
								lParams = (LinearLayout.LayoutParams) precio.getLayoutParams();
								lParams.weight = .30f;
								precio.setLayoutParams(lParams);
							}
						}
					}
					else{
				...
				case R.id.txtPrecioSugerido:
					...
					if(tipoTransProd != TiposTransProd.PEDIDO && ((tipoTransProd == TiposTransProd.MOV_SIN_INV_EV && ((MOTConfiguracion)Sesion.get(Campo.MOTConfiguracion)).get("MSIEVPreventa").equals("0")) || tipoTransProd == TiposTransProd.CANJES)){ //solo se muestra el precio sugerido para los pedidos
						...
						if (!cursor.isNull(8)){
							...
							if (tipoTransProd != TiposTransProd.CANJES) {
								lParams = (LinearLayout.LayoutParams) precioSug.getLayoutParams();
								lParams.weight = .30f;
								precioSug.setLayoutParams(lParams);
							}
						}
						else{
							precioSug.setVisibility(View.INVISIBLE);
                ...
                case R.id.txtValorPuntos:
                    TextView puntos = (TextView) view;

                    if (!cursor.isNull(15)){
                        if (tipoTransProd == TiposTransProd.CANJES) {
                            puntos.setText(String.valueOf(cursor.getInt(columnIndex)) + " pts");
                            puntos.setVisibility(View.VISIBLE);
                        }
                        else {
							puntos.setVisibility(View.GONE);
							lParams = (LinearLayout.LayoutParams) puntos.getLayoutParams();
							lParams.weight = 0f;
							puntos.setLayoutParams(lParams);
						}
                    }
                    else{
                        puntos.setVisibility(View.GONE);
				...
				case R.id.imgProducto:
					...
					float RelacionImagenProd = 100;
					if (((ConfigParametro) Sesion.get(Campo.ConfigParametro)).existeParametro("PorcentajeImgProducto")) {
						try{
							RelacionImagenProd = Float.parseFloat(((ConfigParametro) Sesion.get(Campo.ConfigParametro)).get("PorcentajeImgProducto").toString());
						}
						catch(Exception ex){

						}
					}
					File file = new File(conf.get(ArchivoConfiguracion.CampoConfiguracion.DIRECTORIO_TRABAJO).toString()+"/ImgProd/"+cursor.getString(cursor.getColumnIndex("ProductoClave"))+".jpg");
					float ancho = (350*(RelacionImagenProd/100));
					float alto = (350*(RelacionImagenProd/100));
					int width = (int)ancho;
					int height = (int)alto;
					LinearLayout.LayoutParams parms = new LinearLayout.LayoutParams(width,height);
					ImageView producto = (ImageView) view;
					producto.setLayoutParams(parms);
					if(file.exists()){
						Bitmap myBitmap = BitmapFactory.decodeFile(file.getAbsolutePath());
						producto.setImageBitmap(myBitmap);
					}
					else{
						Bitmap myBitmap = BitmapFactory.decodeResource(getResources(), R.drawable.noimage);
						producto.setImageBitmap(myBitmap);
					}
					if(imagenesProducto)
				...
				default:
					if (viewId != R.id.npCantidad)
					{
                        ...
                        if (viewId == R.id.txtClave || viewId == R.id.txtDescripcion) {
                            if (sColor != "")
                                texto.setTextColor(Color.parseColor(sColor));
                        }else if(viewId == R.id.txtSaldoEnvase)
						{
							try{
								if (cursor.isNull(cursor.getColumnIndex("SaldoEnvase")) ){
									texto.setVisibility(View.GONE);
								}
							}catch (Exception ex){
					...
					else
					{
						...
						{
							if (inventarioPorLotes){
								//InventarioLotesKey prodLoteKey = new InventarioLotesKey(cursor.getString(cursor.getColumnIndex("ProductoClave")), cursor.getShort(cursor.getColumnIndex("Unidad")), cursor.getString(cursor.getColumnIndex("Lote")));
								InventarioLotesKey prodLoteKey = null;
								if (seleccionTPDLotes.size() > 0)
									for (InventarioLotesKey inv: seleccionTPDLotes.keySet()) {
										if (inv.ProductoClave.equals(cursor.getString(cursor.getColumnIndex("ProductoClave"))) && inv.TipoUnidad == cursor.getShort(cursor.getColumnIndex("Unidad")) && inv.Lote.equals(cursor.getString(cursor.getColumnIndex("Lote")))) {
											prodLoteKey = inv;
											break;
										}
									}
								if (prodLoteKey == null)
									prodLoteKey = new InventarioLotesKey(cursor.getString(cursor.getColumnIndex("ProductoClave")), cursor.getShort(cursor.getColumnIndex("Unidad")), cursor.getString(cursor.getColumnIndex("Lote")));
								if (seleccionTPDLotes.containsKey(prodLoteKey))
								{
									if (seleccionTPDLotes.get(prodLoteKey).Cantidad != Float.parseFloat(texto.getText().toString()))
									{
										texto.setText(Generales.getFormatoDecimal(seleccionTPDLotes.get(prodLoteKey).Cantidad, Integer.parseInt(cursor.getString(cursor.getColumnIndex("DecimalProducto")))));
									}
								}
								else
								{
									//Si se capturo cantidad de ese lote en los TransProd, mostrarlo desde el inicio
									if ( cursor.getColumnIndex("CantidadActual")>0 && cursor.getFloat(cursor.getColumnIndex("CantidadActual"))>0){
										texto.setText(Generales.getFormatoDecimal(cursor.getFloat(cursor.getColumnIndex("CantidadActual")), Integer.parseInt(cursor.getString(cursor.getColumnIndex("DecimalProducto")))));
									}else {
										texto.setText(Generales.getFormatoDecimal(0, Integer.parseInt(cursor.getString(cursor.getColumnIndex("DecimalProducto")))));
									}
								}
	...
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo)
	{
		...
		if(motConfig.get("AsignarManualListas").equals("1")){
			menu.getItem(1).setTitle(Mensajes.get("XCambiarListaPrecio"));
	...
	public boolean onContextItemSelected(MenuItem item)
	{
		...
		switch(item.getItemId()){
			case R.id.calcular:
				...
				return true;
			case R.id.listaPrecios:
				modificarPrecio(cursor.getString(cursor.getColumnIndex("ProductoClave")),
						cursor.getInt(cursor.getColumnIndex("Unidad")));
				claveProducto = cursor.getString(cursor.getColumnIndex("ProductoClave"));
				return true;
		}
	...
	private void modificarPrecio(final String productoClave, final int unidad){
		try{
			LayoutInflater inflater = (LayoutInflater) BuscaProducto.this
					.getSystemService(Context.LAYOUT_INFLATER_SERVICE);

			View dialogViewModificarListaPrecio = inflater.inflate(R.layout.dialog_listas_precios, null);

			AlertDialog.Builder builder = new AlertDialog.Builder(BuscaProducto.this);
			final ListView lista_precios = (ListView) dialogViewModificarListaPrecio.findViewById(R.id.lstAgenda);

			String sClienteClave = ((Cliente)Sesion.get(Campo.ClienteActual)).ClienteClave;

			ISetDatos precios = Consultas.ConsultasPrecio.obtenerPreciosProducto(productoClave, unidad, sClienteClave);
			...
			lista_precios.setOnItemClickListener(new AdapterView.OnItemClickListener() {
				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
					Cursor producto = (Cursor) (((SimpleCursorAdapter) lista_precios.getAdapter()).getCursor());
					clavePrecioMod = producto.getString(producto.getColumnIndex("_id"));
					precioMod = producto.getFloat(producto.getColumnIndex("Precio"));
					modifcarPrecio = true;

					if(seleccionTPD.containsKey(productoClave)){
						float cantidad = seleccionTPD.get(productoClave).Cantidad;
						seleccionTPD.remove(productoClave);
						seleccionTPD.put(productoClave, TransaccionesDetalle.GenerarDetalleBusqueda(productoClave, unidad, cantidad, precioMod, clavePrecioMod, ""));
					}
