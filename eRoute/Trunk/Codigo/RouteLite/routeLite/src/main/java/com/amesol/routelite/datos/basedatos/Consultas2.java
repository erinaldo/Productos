...
public final class Consultas2
{
	...
	public static class ConsultasTransProd{
		...
		public static float obtenerSaldoVentas(String clienteClave, boolean incluirContado) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT SUM(TRP.Saldo) AS Saldo FROM TransProd TRP ");
			consulta.append("INNER JOIN Visita VIS ON TRP.VisitaClave = VIS.VisitaClave AND TRP.DiaClave = VIS.DiaClave ");
			consulta.append("WHERE VIS.ClienteClave = '" + clienteClave + "' AND TRP.TipoFase != 0 AND ");
			consulta.append((incluirContado ? "TRP.Tipo in (1, 24)" : "((TRP.Tipo = 1 AND TRP.CFVTipo = 2) OR TRP.Tipo = 24)"));
			ISetDatos saldo = BDVend.consultar(consulta.toString());
			float res = 0;
			while(saldo.moveToNext()){
				res += saldo.getFloat("Saldo");
		...
		public static float obtenerSaldoVentasVencidas(Visita visita) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT SUM(TRP.Saldo) AS Saldo FROM TransProd TRP ");
			consulta.append("INNER JOIN Visita VIS ON TRP.VisitaClave = VIS.VisitaClave AND TRP.DiaClave = VIS.DiaClave "); //AND TRP.ClienteClave = VIS.ClienteClave ");
			consulta.append("WHERE ((TRP.Tipo = 1 AND TRP.CFVTipo = 2) OR TRP.Tipo = 24) ");
			consulta.append("AND TRP.TipoFase != 0 AND VIS.ClienteClave = '" + visita.ClienteClave + "'");
		...
		public static float obtenerSaldoVentasNoCheque(String clienteClave, boolean incluirContado) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT case when sum(saldo+(Case When TRC.AbnCheque is null then 0 Else TRC.AbnCheque END)) is null then 0 else sum(saldo+(Case When TRC.AbnCheque is null then 0 Else TRC.AbnCheque END)) end AS Saldo ");
			consulta.append("FROM Transprod TRP inner join visita v on v.visitaclave = TRP.visitaclave and v.diaclave = TRP.diaclave ");
			consulta.append("LEFT JOIN (Select TRPCheque.TransProdID,sum(AbnCheque) as AbnCheque from TRPCheque inner join Abono ABN on ABN.ABNId = TRPCheque.ABNId group by TRPCheque.TransProdID) TRC on TRC.TransProdID = TRP.TransProdID ");
			consulta.append("WHERE v.ClienteClave = '" + clienteClave + "' AND TipoFase != 0 AND ");
			consulta.append((incluirContado ? "Tipo in (1, 24)" : "((Tipo = 1 AND CFVTipo = 2) OR TRP.Tipo = 24)"));
			ISetDatos saldo = BDVend.consultar(consulta.toString());
			float res = 0;
			while(saldo.moveToNext()){
				res += saldo.getFloat("Saldo");
		...
		public static float obtenerSaldoVentasVencidasNoCheque(Visita visita) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT case when sum(saldo+(Case When TRC.AbnCheque is null then 0 Else TRC.AbnCheque END)) is null then 0 else sum(saldo+(Case When TRC.AbnCheque is null then 0 Else TRC.AbnCheque END)) end AS Saldo ");
			consulta.append("FROM Transprod TRP inner join visita v on v.visitaclave = TRP.visitaclave and v.diaclave = TRP.diaclave ");
			consulta.append("Left Join (Select TRPCheque.TransProdID,sum(AbnCheque) as AbnCheque from TRPCheque  inner join Abono ABN on ABN.ABNId = TRPCheque.ABNId group by TRPCheque.TransProdID) TRC on TRC.TransProdID = TRP.TransProdID ");
		...
		public static final float obtenerSaldoFacturas(String clienteClave, boolean incluirContado) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT SUM(TRP.Saldo) AS Saldo ");
			consulta.append("FROM TransProd TRP ");
			consulta.append("INNER JOIN Visita VIS ON TRP.VisitaClave = VIS.VisitaClave AND TRP.DiaClave = VIS.DiaClave ");
			consulta.append("WHERE VIS.ClienteClave = '" + clienteClave + "' AND TRP.TipoFase != 0 AND ");
			consulta.append((incluirContado ? "TRP.Tipo in (8, 24)" : "((TRP.Tipo = 8 AND TRP.CFVTipo = 2) OR TRP.Tipo = 24)"));
			ISetDatos saldo = BDVend.consultar(consulta.toString());
			float res = 0;
			while(saldo.moveToNext()){
				res += saldo.getFloat("Saldo");
		...
		public static final float obtenerSaldoFacturasVencidas(Visita visita) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT SUM(TRP.Saldo) AS Saldo FROM TransProd TRP ");
			consulta.append("INNER JOIN Visita VIS ON TRP.VisitaClave = VIS.VisitaClave AND TRP.DiaClave = VIS.DiaClave "); //AND TRP.ClienteClave = VIS.ClienteClave ");
			consulta.append("AND ((TRP.Tipo = 8 AND TRP.CFVTipo = 2) OR TRP.Tipo = 24) AND TRP.TipoFase != 0 AND VIS.ClienteClave = '" + visita.ClienteClave + "'");
		...
		public static float obtenerSaldoFacturasNoCheque(String clienteClave, boolean incluirContado) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT case when sum(saldo+(Case When TRC.AbnCheque is null then 0 Else TRC.AbnCheque END)) is null then 0 else sum(saldo+(Case When TRC.AbnCheque is null then 0 Else TRC.AbnCheque END)) end AS Saldo ");
			consulta.append("FROM Transprod TRP inner join visita v on v.visitaclave = TRP.visitaclave and v.diaclave = TRP.diaclave ");
			consulta.append("Left Join (Select TRPCheque.TransProdID,sum(AbnCheque) as AbnCheque from TRPCheque  inner join Abono ABN on ABN.ABNId = TRPCheque.ABNId group by TRPCheque.TransProdID) TRC on TRC.TransProdID = TRP.TransProdID ");
		...
		public static float obtenerSaldoFacturasVencidasNoCheque(Visita visita) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT case when sum(saldo+(Case When TRC.AbnCheque is null then 0 Else TRC.AbnCheque END)) is null then 0 else sum(saldo+(Case When TRC.AbnCheque is null then 0 Else TRC.AbnCheque END)) end AS Saldo ");
			consulta.append("FROM Transprod TRP inner join visita v on v.visitaclave = TRP.visitaclave and v.diaclave = TRP.diaclave ");
			consulta.append("Left Join (Select TRPCheque.TransProdID,sum(AbnCheque) as AbnCheque from TRPCheque  inner join Abono ABN on ABN.ABNId = TRPCheque.ABNId group by TRPCheque.TransProdID) TRC on TRC.TransProdID = TRP.TransProdID ");
		...
		public static final float obtenerSaldoVtasNoFact(String clienteClave, boolean incluirContado) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT SUM(TRP.Saldo) AS Saldo FROM TransProd TRP ");
			consulta.append("INNER JOIN Visita VIS ON TRP.VisitaClave = VIS.VisitaClave AND TRP.DiaClave = VIS.DiaClave ");
			consulta.append("WHERE VIS.ClienteClave = '" + clienteClave + "' AND TRP.TipoFase != 0 AND TRP.FacturaID IS NULL AND ");
			consulta.append((incluirContado ? "TRP.Tipo = 1" : "TRP.Tipo = 1 AND TRP.CFVTipo = 2"));
			ISetDatos saldo = BDVend.consultar(consulta.toString());
			float res = 0;
			while(saldo.moveToNext()){
				res += saldo.getFloat("Saldo");
		...
		public static final float obtenerSaldoVtasVencidasNoFact(Visita visita) throws Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT SUM(TRP.Saldo) AS Saldo FROM TransProd TRP ");
			consulta.append("INNER JOIN Visita VIS ON TRP.VisitaClave = VIS.VisitaClave AND TRP.DiaClave = VIS.DiaClave "); //AND TRP.ClienteClave = VIS.ClienteClave ");
			consulta.append("AND TRP.Tipo = 1 AND TRP.CFVTipo = 2 AND TRP.TipoFase != 0 AND TRP.FacturaID IS NULL AND VIS.ClienteClave = '" + visita.ClienteClave + "'");
    ...
    public static class ConsultasActivos {
        ...
        public static boolean existenActivos()throws Exception{
           return BDVend.bd.ejecutarEscalarInteger("select count(*) from Activo") > 0;
        }
	...
	public static class ConsultarActividadesDis {
		...
		public static ISetDatos obtenerObjetivo(String rutClave) throws Exception
		{
			StringBuilder sConsulta = new StringBuilder();
			sConsulta.append("SELECT * FROM DIS_ObjetivoMensual WHERE RutClave = '" +rutClave+ "'");

			return BDVend.consultar(sConsulta.toString());
		}

		public static double obtenerFaltanteCajas(String rutClave, String esquemas) throws Exception
		{
			StringBuilder consulta = new StringBuilder();
			ISetDatos datos;

			consulta.append("SELECT IFNULL(SUM(TPD.Cantidad),0) AS Cantidad ");
			consulta.append("FROM TransProd TP ");
			consulta.append("INNER JOIN TransProdDetalle TPD ON TP.TransProdID = TPD.TransProdID ");
			consulta.append(" AND TP.Tipo = 1 AND TipoFase <> 0 ");
			consulta.append("INNER JOIN Visita V ON V.RUTClave = '" +rutClave+ "' AND V.VisitaClave = TP.VisitaClave ");
			consulta.append("INNER JOIN ProductoEsquema PE ON PE.ProductoClave = TPD.ProductoClave ");
			consulta.append("INNER JOIN Esquema Esq ON PE.EsquemaId = Esq.EsquemaId AND Esq.Nivel = 1 ");
			consulta.append("AND Nombre IN(" +esquemas+ ")");

			datos = BDVend.consultar(consulta.toString());
			datos.moveToFirst();
			return datos.getDouble("Cantidad");
		}
	...
	public static final class ConsultasInventarioLotes{
		public static ISetDatos obtenerLotesCaducidad(String transProdID) throws  Exception{
			StringBuilder consulta = new StringBuilder();
			consulta.append("SELECT TransProdDetalleID, LC.Caducidad FROM TPDDatosExtra TPE ");
			consulta.append("INNER JOIN LoteCaducidad LC ON LC.Lote = TPE.Lote AND TPE.TransProdID = '"+transProdID+"'");
			return BDVend.consultar(consulta.toString());
		}

		public static void guardarTPDDatosExtra(TPDDatosExtra tpd, boolean nuevo) throws  Exception{
			StringBuilder consulta = new StringBuilder();

			if(nuevo) {
				consulta.append("INSERT INTO TPDDatosExtra(TransProdID, TransProdDetalleID, Lote, MFechaHora, MUsuarioID, Enviado) ");
				consulta.append("VALUES('"+tpd.TransProdID+"','"+tpd.TransProdDetalleID+"','"+tpd.Lote+"',strftime('%Y-%m-%d %H:%M:%S')");
				consulta.append(",'"+tpd.MUsuarioID+"',0)");
			}else{
				consulta.append("UPDATE TPDDatosExtra SET Lote='"+tpd.Lote+"',MFechaHora=strftime('%Y-%m-%d %H:%M:%S'),MUsuarioID='"+tpd.MUsuarioID);
				consulta.append("' where TransProdID = '"+tpd.TransProdID +"' AND TransProdDetalleID='"+tpd.TransProdDetalleID+"'");
			}
	...
	public static final class ConsultasReportesDEL{
		public static void actualizarLoteCaducidad(String transProdID, String transProdDetalleID, String Lote) throws Exception{
			BDVend.ejecutarComando("UPDATE TPDDatosExtra SET Lote = NULL where TransProdID = '" +transProdID+ "' and TransProdDetalleID = '"+transProdDetalleID+ "'");
			BDVend.ejecutarComando("DELETE FROM LoteCaducidad WHERE Lote = '"+Lote+"'");
		}
		...
		public static ISetDatos obtenerInventarioGeneral (String subEmpresaId) throws  Exception {
			...
			//if (((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase != null && ((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase.length()>0) {
				consulta.append("select pro.ProductoClave, pro.Nombre, ifnull(sum(inv.Disponible),0) as Exis, ifnull(sum((inv.Disponible - inv.NoDisponible - inv.Apartado - inv.Contenido)),0) as Disp, ifnull(sum((inv.NoDisponible + inv.Apartado)),0) as 'No D', max(ifnull(ppv.Precio, 0)) as Precio ");
			//}else{
			//	consulta.append("select pro.ProductoClave, pro.Nombre, ifnull(sum(inv.Disponible),0) as Exis, ifnull(sum((inv.Disponible - inv.NoDisponible - inv.Apartado - inv.Contenido)),0) as Disp, ifnull(sum((inv.NoDisponible + inv.Apartado)),0) as 'No D', 0 as Precio ");
			//}
		...
		public static float obtenerInventarioTotalUnitario (String subEmpresaId) throws  Exception {
			...
			if (false && ((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase != null && ((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase.length()>0){
				...
				consulta.append("from ( ");
				consulta.append("select inv.ProductoClave, inv.Disponible, prd.Factor, ifnull(ppv.Precio, 0) as Precio ");
				consulta.append("from InventarioLotes inv ");
				consulta.append("inner join Producto pro on inv.ProductoClave = pro.ProductoClave ");
				consulta.append("inner join ProductoUnidad pru on pro.ProductoClave = pru.ProductoClave ");
				consulta.append("inner join ( ");
		...
		public static ISetDatos obtenerInventarioDetallado(String subEmpresaId, boolean total, String productoClave) throws Exception{
			String totales = "inv.Disponible as Exis, (inv.Disponible - inv.NoDisponible - inv.Apartado - inv.Contenido) as Disp, (inv.NoDisponible + inv.Apartado) as 'No D'";
			if(total)
				totales = "ifnull(sum(inv.Disponible),0) as Exis, ifnull(sum((inv.Disponible - inv.NoDisponible - inv.Apartado - inv.Contenido)),0) as Disp, ifnull(sum((inv.NoDisponible + inv.Apartado)),0) as 'No D'";
			StringBuilder consulta = new StringBuilder();
			//if (((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase != null && ((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase.length()>0) {
				consulta.append("select pro.ProductoClave, pro.Nombre, inv.Lote, ifnull(strftime('%d/%m/%Y',LC.Caducidad),'          ') as Caducidad, "+totales+" , pru.PRUTipoUnidad, prd.Factor, max(ifnull(ppv.Precio, 0)) as Precio ");
			//}else{
			//	consulta.append("select pro.ProductoClave, pro.Nombre, inv.Lote, ifnull(strftime('%d/%m/%Y',LC.Caducidad),'          ') as Caducidad, "+totales+", pru.PRUTipoUnidad, prd.Factor, 0 as Precio ");
			//}
			consulta.append("from InventarioLotes inv ");
			consulta.append("inner join Producto pro on inv.ProductoClave = pro.ProductoClave and pro.SubEmpresaId = '" + subEmpresaId + "' ");
			if(!total)
				consulta.append(" and inv.ProductoClave = '" +productoClave+"' ");
			consulta.append("inner join ProductoUnidad pru on pro.ProductoClave = pru.ProductoClave ");
			...
			consulta.append("left join LoteCaducidad LC on inv.ProductoClave = LC.ProductoClave and inv.Lote = LC.Lote ");
			//if (((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase != null && ((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase.length()>0){
				consulta.append("left join (select ProductoClave, PRUTipoUnidad, MAX(Precio) as Precio from PrecioProductoVig group by ProductoClave, PRUTipoUnidad) ppv on pru.ProductoClave = ppv.ProductoClave and pru.PRUTipoUnidad = ppv.PRUTipoUnidad ");
			//	consulta.append("and ppv.PrecioClave = '" + ((Vendedor)Sesion.get(Campo.VendedorActual)).ListaPrecioBase + "' ");
			//}
		...
		public static ISetDatos obtenerInventarioTotalProductos (String subEmpresaId) throws  Exception {
			StringBuilder consulta = new StringBuilder();
			consulta.append("select ifnull(sum(inv.Disponible),0) as Exis, ifnull(sum(inv.Disponible - inv.NoDisponible - inv.Apartado - inv.Contenido),0) as Disp, ifnull(sum(inv.NoDisponible + inv.Apartado),0) as 'No D' ");
			consulta.append("from InventarioLotes inv ");
			consulta.append("inner join Producto pro on inv.ProductoClave = pro.ProductoClave ");
			consulta.append("where pro.SubEmpresaId = '" + subEmpresaId + "' ");
			consulta.append("order by pro.ProductoClave ");
