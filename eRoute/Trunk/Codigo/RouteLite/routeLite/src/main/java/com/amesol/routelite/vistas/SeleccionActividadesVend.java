...
public class SeleccionActividadesVend extends Vista implements ISeleccionActividadesVend
{
    ...
    private int solicitarContrasenaCarga = 0;
	...
	public void onCreate(Bundle savedInstanceState)
	{
        ...
        if (Sesion.get(Campo.SolicitarContrasenaCarga) == null || Sesion.get(Campo.SolicitarContrasenaCarga).equals(""))
            Sesion.set(Campo.SolicitarContrasenaCarga, "1");
	...
	{
		BDVend.setOrigenLog("SeleccionActividadesVend:Fin ActividadActual");
	...
	public void mostrarActividades(ValorReferencia[] valores, String grupo)
	{
		...
		boolean TipoOrden = false;
		for(int x=0;x<valores.length;x++){
			try {
				Integer.parseInt(valores[x].getDescripcion().substring(0,2));
			} catch (NumberFormatException excepcion) {
				TipoOrden = true;
			}
		}
		if(TipoOrden){
			Arrays.sort(valores, new Comparator<ValorReferencia>() {
				@Override
				public int compare(ValorReferencia lhs, ValorReferencia rhs) {
					return Integer.valueOf(lhs.getVavclave().compareTo(rhs.getVavclave()));
				}
			});
		}
		else {
			Arrays.sort(valores, new Comparator<ValorReferencia>() {
				@Override
				public int compare(ValorReferencia lhs, ValorReferencia rhs) {
					return Integer.valueOf(lhs.getDescripcion().substring(0,2).compareTo(rhs.getDescripcion().substring(0,2)));
				}
			});
		}
		...
		lista.setTag(grupo);
		if(TipoOrden){
			Collections.sort(Valores, new Comparator<ValorReferencia>() {
				@Override
				public int compare(ValorReferencia lhs, ValorReferencia rhs) {
					return Integer.valueOf(lhs.getVavclave().compareTo(rhs.getVavclave()));
				}
	...
	private OnItemClickListener mSeleccionar = new OnItemClickListener()
	{
		public void onItemClick(AdapterView<?> parent, View v, int position, long id)
		{
			...
			if (tipoIndice.equals("8") || tipoIndice.equals("15") || tipoIndice.equals("10"))
			{
	...
	private void validarPermisoAcceso(String tipoIndice, int tipoPermiso)//Metodo que valida permisos ldelatorre
	{
		...
		try
		{
            if (Sesion.get(Sesion.Campo.ConfigParametro) != null && ((ConfigParametro)Sesion.get(Sesion.Campo.ConfigParametro)).existeParametro("ValidarAcceso", tipoIndice) && ((ConfigParametro)Sesion.get(Sesion.Campo.ConfigParametro)).get("ValidarAcceso", tipoIndice).equals("1")){
                if (tipoIndice.equals("15")){ //Descarga
                    solicitarContrasenaDescarga = 1;
                    solicitarContrasenaDevoAlmacen = Integer.parseInt(String.valueOf(Sesion.get(Campo.SolicitarContrasenaDevolucionAlmacen)));
                    solicitarContrasenaCarga = 1;
                }
                else if (tipoIndice.equals("8")) { //Dev. Almacen
                    solicitarContrasenaDescarga = Integer.parseInt(String.valueOf(Sesion.get(Campo.SolicitarContrasenaDescarga)));
                    solicitarContrasenaDevoAlmacen = 1;
                    solicitarContrasenaCarga = 1;
                }
                else if (tipoIndice.equals("10")){
                    solicitarContrasenaDescarga = 1;
                    solicitarContrasenaDevoAlmacen = 1;
                    solicitarContrasenaCarga = Integer.parseInt(String.valueOf(Sesion.get(Campo.SolicitarContrasenaCarga)));
                }
			...
			{
				Log.i(null, "Descarga: "+solicitarContrasenaDescarga+" Almacen: "+solicitarContrasenaDevoAlmacen+"Carga: "+solicitarContrasenaCarga);
				if(tipoPermiso == Enumeradores.TipoPermiso.ACCESAR && permiso.equals("") && ((tipoIndice.equals("15") && solicitarContrasenaDescarga == 1) || (tipoIndice.equals("8") && solicitarContrasenaDevoAlmacen == 1) || (tipoIndice.equals("10") && solicitarContrasenaCarga == 1)))
				{
                        ...
                        if (!usuario.PERClave.equals("Admin")) {
                            ...
                            try {
                                //if (!(Sesion.get(Sesion.Campo.ConfigParametro) != null && ((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).existeParametro("ValidarAcceso", tipoIndice) && ((ConfigParametro) Sesion.get(Sesion.Campo.ConfigParametro)).get("ValidarAcceso", tipoIndice).equals("1"))) {
                                    ...
                                    if (tipoIndice.equals("10"))
                                        Sesion.set(Campo.SolicitarContrasenaCarga, "0");
                                //}
                            }catch(Exception e){}
