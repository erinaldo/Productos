...
#End Region
    ...
    Public Sub AgregarFolio()
        If bload Then
            ...
            dtCxC = ModPOS.Recupera_Tabla("st_recupera_global", "@SUCClave", SUCClave, "@Inicio", Inicio, "@Fin", Fin.AddHours(23.9999))
                ...
                Select Case MessageBox.Show("Se generara una Factura Global con todos los documentos seleccionados por " & Format(TxtTotal.Text, "Currency") & " para el Cliente: " & TxtRazonSocial.Text & ", esta de acuerdo?", "Pregunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                    Case DialogResult.Yes
                        ...
                        If VerificaDatoFiscalCte(oCFD) = True Then
                            ...
                            If oCFD.VersionCF = "3.3" Then
                                ModPOS.Ejecuta("st_crea_detalleGlobal", "@FacturaId", FacturaId, "@FACClave", FACClave, "@Desc", IIf(dPorc > 0, dPorc / 100, 0))
                            ...
                            If EnviaFactura = True Then
                                If oCFD.email <> "" Then
                                    ModPOS.SendMail(oCFD.VersionCF, oCFD.email, oCFD.TipoCF, FormatoFactura, CDate(oCFD.Fecha), oCFD.Serie & oCFD.Folio, oCFD.DOCClave, oCFD.total, MailAdress, MailUser, MailPassword, HostSMTP, MailPort, MailSSL, DisplayName, PathXML, SUCClave, TipoCambio, MonedaDesc, MonedaRef)
                                End If
                            End If
    ...
    Private Sub dtFinal_EnabledChanged(sender As Object, e As EventArgs) Handles dtFinal.EnabledChanged
    ...
    Private Sub TxtBusqueda_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TxtBusqueda.KeyPress
        If (Asc(e.KeyChar)) = 13 Then
            If Not TxtBusqueda.Text = vbNullString Then
                Dim dt As DataTable
                dt = ModPOS.SiExisteRecupera("sp_recupera_clavecte", "@Clave", TxtBusqueda.Text.Trim.ToUpper.Replace("'", "''"), "@COMClave", ModPOS.CompanyActual)
                If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then
