...
#End Region
    ...
    Private Sub BtnGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnGuardar.Click
        ...
        If ChkCredito.GetEstado = 0 AndAlso (CajaActiva = True OrElse iAnticipos = 1) And Saldo > 0 Then
            ...
            If CajaActiva = True OrElse iAnticipos = 1 Then
                            ...
                            For i = 0 To b.drAbonos.Length - 1
                                ...
                                If b.drAbonos(i)("Tipo") = 2 AndAlso DateDiff(DateInterval.Day, CDate(b.drAbonos(i)("Fecha")), CDate(dtCargos.Rows(0)("fecha"))) = 0 Then
                                    ...
                                    If bTieneSello = False Then
                                        ...
                                        If b.drAbonos(i)("Saldo") > Saldo Then
                                            ...
                                            ImportePago = Saldo
                                            ImporteAnticipo = b.drAbonos(i)("Saldo") - Saldo
                                        ...
                                        ModPOS.Aplica_Pagos(dtCargo, oCFD.CTEClave, b.drAbonos(i)("ID"), b.drAbonos(i)("TipoPago"), ImportePago, CAJClave, 1)
                                    Else
                                        ModPOS.Aplica_Pagos(dtCargo, oCFD.CTEClave, b.drAbonos(i)("ID"), b.drAbonos(i)("TipoPago"), b.drAbonos(i)("Saldo"), CAJClave, b.drAbonos(i)("Tipo"))
                                    End If
                                Else
                                    ModPOS.Aplica_Pagos(dtCargo, oCFD.CTEClave, b.drAbonos(i)("ID"), b.drAbonos(i)("TipoPago"), b.drAbonos(i)("Saldo"), CAJClave, b.drAbonos(i)("Tipo"))
                                End If
                ...
                Saldo = IIf(dtCargo.Compute("Sum(Saldo)", "Saldo > 0") Is System.DBNull.Value, 0, dtCargo.Compute("Sum(Saldo)", "Saldo > 0"))


                Dim iTPago As Integer
                Dim dTotalCambio, dTotalAbono As Double

                If Saldo > 0 AndAlso CajaActiva = True Then
                    ...
                    Do
                        ...
                        a.SaldoAcumulado = ModPOS.TruncateToDecimal(Saldo, 2)
                        ...
                        If a.DialogResult = DialogResult.OK Then
                            If a.detallePago.Rows.Count > 0 Then
                                ...
                                For i = 0 To a.detallePago.Rows.Count - 1
                                    ModPOS.Aplica_Pagos(dtCargo, oCFD.CTEClave, a.detallePago.Rows(i)("ABNClave"), a.detallePago.Rows(i)("TipoPago"), a.detallePago.Rows(i)("Saldo"), CAJClave, 1)
                                Next
                            ...
                            Saldo -= dTotalAbono - Math.Round(dTotalCambio, 2)

                        End If



                    Loop While TruncateToDecimal(Saldo, 2) > 0
        ...
        Else
            ...
            If ModPOS.TruncateToDecimal(Saldo, 1) > 0 Then
        ...
        If Saldo > 0 Then
            ModPOS.Ejecuta("sp_act_saldo_cte", "@Tipo", 1, "@Cliente", oCFD.CTEClave, "@Importe", Saldo)
        End If
        ...
        Select Case MessageBox.Show("¿Desea imprimir la nota de cargo?", "Pregunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
            Case DialogResult.Yes
                ...
                dtPrinter.Dispose()
                ModPOS.imprimirCargo(FormatCargo, CARClave, Total, TipoCambio, MonedaDesc, MonedaRef, sImpresora, 1, oCFD.VersionCF)
        End Select
        ...
        If oCFD.email <> "" Then
            Select Case MessageBox.Show("¿Desea enviar el documento por correo electrónico?", "Pregunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                Case DialogResult.Yes
                    ModPOS.SendMailCargo(oCFD.VersionCF, oCFD.email, FormatCargo, fechaPath, Folio, CARClave, Total, MailAdress, MailUser, MailPassword, HostSMTP, MailPort, MailSSL, DisplayName, PathXML, TipoCambio, MonedaDesc, MonedaRef, True)
