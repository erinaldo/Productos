...
Public Class Imprimir
    ...
    Public Sub AddHeaderLineDetalle(ByVal Concepto As String, ByVal Importe As Double, ByVal style As FontEstilo, ByVal alinear As Alinear)
        Dim str2 As String = Me.FormateaCadena(alinear.Derecha, Strings.Format(Me.Redondear(Importe, 2).ToString, "Currency"), 12)
        Dim str As String = ""
        ...
        Else
            str = "  "
        End If
        Dim item As String = String.Concat(New String() {Me.FormateaCadena(alinear.Izquierda, Concepto, 18), str, str2})
    ...
    Public Sub AddItem(ByVal DVEClave As String, ByVal GTIN As String, ByVal Nombre As String, ByVal Cantidad As Double, ByVal Precio As Double, ByVal Descuento As Double)
        ...
        If (Descuento = 0) Then
            str = Me.FormateaCadena(Alinear.Derecha, Me.Redondear(Cantidad, 2).ToString, 7)
            str7 = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(Precio, 2).ToString, "Currency"), 10)
            str8 = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear((Precio * Cantidad), 2).ToString, "Currency"), 12)
            str4 = ""
            If (Me.maxChar > &H20) Then
            ...
            If (Descuento > 0) Then
                ...
                If Not dtDescuento Is Nothing Then
                    ...
                    For i = 0 To dtDescuento.Rows.Count() - 1
                        ...
                        str6 = Me.FormateaCadena(Alinear.Izquierda, (Me.Redondear((dtDescuento.Rows(i)("DescuentoPorc") * 100), 2).ToString & "%"), 6)
                        str5 = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(dtDescuento.Rows(i)("DescuentoImp") + dtDescuento.Rows(i)("ImpuestoDesc"), 2).ToString, "Currency"), 10)
                        str4 = ""
                        str8 = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(((Precio - dDescuento) * Cantidad), 2).ToString, "Currency"), 12)
    ...
    Public Sub AddItem(ByVal Tipo As Integer, ByVal DescPor As Double, ByVal Descuento As Double)
        ...
        Dim str6 As String
        If (Descuento > 0) Then
            str6 = Me.FormateaCadena(Alinear.Izquierda, (Me.Redondear((DescPor * 100), 2).ToString & "%"), 6)
            str12 = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(Descuento, 2).ToString, "Currency"), 16)
            str4 = ""
            If (Me.maxChar > &H20) Then
                str4 = Me.Espacio((Me.maxChar - &H20))
            End If
            Select Case Tipo
                Case Is = 1 'Descuento
                    Me.items.Add(String.Concat(New String() {str6, "DESC. CTE.", str4, str12}))
                Case Is = 2 'Bonificacion
                    Me.items.Add(String.Concat(New String() {str6, "BONI. CTE.", str4, str12}))
                Case Is = 3 'Puntos
                    '  Me.items.Add(String.Concat(New String() {str6, "PTOS. CTE.", str4, str12}))
            End Select
        End If
    End Sub
    ...
    Public Sub AddHeaderItem(ByVal Cant As String, ByVal Denominacion As String, ByVal Importe As String, ByVal style As FontEstilo, ByVal alinear As Alinear)
        ...
        Dim item As String = Me.FormateaCadena(alinear.Derecha, Importe, 12)
    ...
    Public Sub AddItemTransferencia(ByVal GTIN As String, ByVal Nombre As String, ByVal Cantidad As Double, ByVal Costo As Double, ByVal Subtotal As Double)
        ...
        str = Me.FormateaCadena(Alinear.Derecha, Me.Redondear(Cantidad, 2).ToString, 7)
        str7 = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(Costo, 2).ToString, "Currency"), 10)
        str8 = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear((Subtotal), 2).ToString, "Currency"), 12)
        str4 = ""
    ...
    Public Sub AddTotal(ByVal Total As Double, ByVal Puntos As Double, ByVal Ahorro As Double, ByVal Articulos As Double, ByVal Recibido As Double, ByVal Cambio As Double, ByVal EstiloTotal As FontEstilo)
        Dim str6 As String = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(Total, 2).ToString, "Currency"), 12)
        Dim str4 As String = Me.FormateaCadena(Alinear.Derecha, Me.Redondear(Puntos, 2).ToString("#,##0.00"), 12)
        Dim str As String = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(Ahorro, 2).ToString, "Currency"), 12)
        Dim str2 As String = Me.FormateaCadena(Alinear.Derecha, Me.Redondear(Articulos, 2).ToString, 12)
        ...
        If (Recibido > 0) Then
            Dim str5 As String = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(Recibido, 2).ToString, "Currency"), 12)
            Dim str3 As String = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(Cambio, 2).ToString, "Currency"), 12)
    ...
    Public Sub AddTotal(ByVal Redondeo As Double, ByVal Total As Double, ByVal Puntos As Double, ByVal Ahorro As Double, ByVal Articulos As Double, ByVal Recibido As Double, ByVal Cambio As Double, ByVal Saldo As Double, ByVal EstiloTotal As FontEstilo)
        Dim str7 As String = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(Total, 2).ToString, "Currency"), 12)
        Dim str4 As String = Me.FormateaCadena(Alinear.Derecha, Me.Redondear(Puntos, 2).ToString("#,##0.00"), 12)
        Dim str As String = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(Ahorro, 2).ToString, "Currency"), 12)
        Dim str2 As String = Me.FormateaCadena(Alinear.Derecha, Me.Redondear(Articulos, 2).ToString, 12)
        Dim str8 As String = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(Saldo, 2).ToString, "Currency"), 12)

        If (Redondeo > 0) Then
            Dim str6 As String = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(Redondeo, 2).ToString, "Currency"), 12)
    ...
    Public Sub AddMetodoPago(ByVal Tipo As String, ByVal Banco As String, ByVal Referencia As String, ByVal linea As Integer)
        If linea = 0 Then
            Me.totales.Add(Me.FormateaCadena(Alinear.Izquierda, ("METODO DE PAGO: "), Me.maxChar) & Me.delimitador & CStr(Me.Style) & Me.delimitador & CStr(Me.fontSize))
        End If
        Me.totales.Add(Me.FormateaCadena(Alinear.Centrado, (Tipo.ToUpper), Me.maxChar) & Me.delimitador & CStr(Me.Style) & Me.delimitador & CStr(Me.fontSize))
        If Banco <> "" Then
            Me.totales.Add(Me.FormateaCadena(Alinear.Centrado, (Banco & IIf(Referencia <> "", ", " & Referencia, "")), Me.maxChar) & Me.delimitador & CStr(Me.Style) & Me.delimitador & CStr(Me.fontSize))
        End If
    End Sub
    ...
    Public Sub AddTotalDevolucion(ByVal Total As Double, ByVal Devolucion As Double, ByVal EstiloTotal As FontEstilo)
        ...
        If (Devolucion >= 0) Then
            Dim str5 As String = Me.FormateaCadena(Alinear.Derecha, Strings.Format(Me.Redondear(Devolucion, 2).ToString, "Currency"), 12)
