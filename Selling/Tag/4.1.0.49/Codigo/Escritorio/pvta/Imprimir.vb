...
Public Class Imprimir
    ...
    Public Sub AddHeaderLineDetalleC(ByVal Concepto As String, ByVal Importe As Double, ByVal style As FontEstilo, ByVal alinear As Alinear)
        ...
        If maxChar < &H20 Then
            str2 = Me.FormateaCadena(alinear.Derecha, TruncateToDecimal(Importe, 2), 12)
            item = String.Concat(New String() {Me.FormateaCadena(alinear.Izquierda, Concepto, 15), "", str2})
        Else
            str2 = Me.FormateaCadena(alinear.Derecha, TruncateToDecimal(Importe, 2), 12)
            item = String.Concat(New String() {Me.FormateaCadena(alinear.Izquierda, Concepto, 18), str, str2})
    ...
    Public Sub AddHeaderLineDetalle(ByVal Concepto As String, ByVal Cantidad As Integer, ByVal Importe As Double, ByVal style As FontEstilo, ByVal alinear As Alinear)

        Dim strConcepto, strCantidad, strImporte As String
        Dim item As String
        Dim str As String = ""
        If (Me.maxChar > &H20) Then
            str = Me.Espacio((Me.maxChar - &H20))
        Else
            str = " "
        End If
        '27 caracteres minimo
        If maxChar < &H20 Then
            strConcepto = Me.FormateaCadena(alinear.Izquierda, Concepto, 11)
            strCantidad = Me.FormateaCadena(alinear.Derecha, TruncateToDecimal(Cantidad, 2).ToString, 4)
            strImporte = Me.FormateaCadena(alinear.Derecha, Strings.Format(TruncateToDecimal(Importe, 2).ToString, "Currency"), 10)
            item = String.Concat(New String() {strConcepto, str, strCantidad, str, strImporte})
        Else 'de 32 o mas caracteres
            strConcepto = Me.FormateaCadena(alinear.Izquierda, Concepto, 15)
            strCantidad = Me.FormateaCadena(alinear.Derecha, TruncateToDecimal(Cantidad, 2).ToString, 5)
            strImporte = Me.FormateaCadena(alinear.Derecha, Strings.Format(TruncateToDecimal(Importe, 2).ToString, "Currency"), 10)
            item = String.Concat(New String() {strConcepto, " ", strCantidad, str, strImporte})
        End If

        Me.headerLines.Add(item & Me.delimitador & CStr(style) & Me.delimitador & CStr(alinear))
    End Sub

    Public Sub AddHeaderLineDetalle(ByVal Concepto As String, ByVal Desc As String, ByVal Descripcion As String, ByVal sImporte As String, ByVal style As FontEstilo, ByVal alinear As Alinear)

        Dim strConcepto, strDesc, strImporte As String
        Dim item As String
        Dim str As String = " "
        Dim i As Integer
        ...
        If IsNumeric(sImporte) Then
            Importe = CDec(sImporte)
        End If
        ...
        If maxChar < &H20 Then
            ...
            strConcepto = Me.FormateaCadena(alinear.Izquierda, Right(Concepto, 13), 13)
            strDesc = Me.FormateaCadena(alinear.Izquierda, Left(Desc, 2), 2)
            If IsNumeric(sImporte) Then
                strImporte = Me.FormateaCadena(alinear.Derecha, Strings.Format(TruncateToDecimal(Importe, 2).ToString, "Currency"), 10)
            Else
                strImporte = Me.FormateaCadena(alinear.Derecha, Left(sImporte, 10), 10)
            End If

            item = String.Concat(New String() {strConcepto, str, strDesc, str, strImporte})

        ElseIf maxChar = &H20 Then

            strConcepto = Me.FormateaCadena(alinear.Izquierda, Right(Concepto, 18), 18)
            strDesc = Me.FormateaCadena(alinear.Izquierda, Left(Desc, 2), 2)
            If IsNumeric(sImporte) Then
                strImporte = Me.FormateaCadena(alinear.Derecha, Strings.Format(TruncateToDecimal(Importe, 2).ToString, "Currency"), 10)
            Else
        ...
        Else
            strConcepto = Me.FormateaCadena(alinear.Izquierda, Right(Concepto, 18), 18)
            strDesc = Me.FormateaCadena(alinear.Izquierda, Left(Descripcion, 2 + i), 2 + i)
            If IsNumeric(sImporte) Then
                strImporte = Me.FormateaCadena(alinear.Derecha, Strings.Format(TruncateToDecimal(Importe, 2).ToString, "Currency"), 10)
            Else
                strImporte = Me.FormateaCadena(alinear.Derecha, Left(sImporte, 10), 10)
    ...
    Public Sub AddItemVale(ByVal Folio As String, ByVal Clave As String, ByVal Modelo As String, ByVal Talla As String, ByVal Color As String, ByVal Cantidad As String, ByVal Precio As String, Optional ByVal DobleFila As Boolean = False)
        '40 Caracteres

        Dim iEspacio As Integer = 0

        If (Me.maxChar < 40) Then
            iEspacio = 40 - maxChar
        End If

        Dim str4, str5, str10, str7, str14, str30, str8 As String

        str4 = Me.FormateaCadena(Alinear.Derecha, Cantidad, 4)
        str5 = Me.FormateaCadena(Alinear.Centrado, Talla, 5)
        str10 = Me.FormateaCadena(Alinear.Izquierda, Color, 7 - iEspacio)
        str7 = Me.FormateaCadena(Alinear.Izquierda, Modelo, 7)
        str8 = Me.FormateaCadena(Alinear.Derecha, Precio, 8)
        str14 = Me.FormateaCadena(Alinear.Izquierda, Folio, 14 - iEspacio)
        Dim pruebas As String = String.Concat(New String() {str7, str10, str5, str8, str4})
        If DobleFila Then
            str14 = Me.FormateaCadena(Alinear.Izquierda, Folio, 14)
            str4 = Me.FormateaCadena(Alinear.Derecha, Cantidad, 7)
            Me.items.Add(String.Concat(New String() {str14}))
            Me.items.Add(String.Concat(New String() {str7, str10, str5, str8, str4}))
        ElseIf Clave = "" AndAlso Modelo = "" Then
            str30 = Me.FormateaCadena(Alinear.Izquierda, Folio, 15 - IIf(Precio = "", 0, iEspacio))
            str10 = Me.FormateaCadena(Alinear.Derecha, Cantidad, 8)
            str8 = Me.FormateaCadena(Alinear.Derecha, Precio, 12 - IIf(Precio = "", iEspacio, 0))
            Me.items.Add(String.Concat(New String() {str30, str8, str10}))
        Else
            Me.items.Add(String.Concat(New String() {str14, str7, str10, str5, str8, str4}))
        End If
    End Sub


    Public Sub AddItemTC(ByVal DVEClave As String, ByVal Modelo As String, ByVal Color As String, ByVal Talla As String, ByVal Cantidad As Double, ByVal Precio As Double, ByVal Descuento As Double)
        ...
        Color = FormateaCadena(Alinear.Izquierda, Color, 11)
        Me.items.Add(Me.FormateaCadena(Alinear.Izquierda, (Modelo & " " & Color & " " & Talla), Me.maxChar))
        If (Descuento = 0) Then
            If maxChar < &H20 Then
    ...
    Public Sub AddItemPicking(ByVal Ubicacion As String, ByVal Clave As String, ByVal Nombre As String, ByVal Cantidad As String)
        Dim str As String
        Dim str4 As String = ""
        Dim str7 As String
        Dim str8 As String
        ...
        If (Me.maxChar < &H20) Then
            str = Me.FormateaCadena(Alinear.Izquierda, Clave, 9)
            str7 = Me.FormateaCadena(Alinear.Derecha, Ubicacion, 9)
            str8 = Me.FormateaCadena(Alinear.Derecha, Cantidad, 9)
        Else
            str = Me.FormateaCadena(Alinear.Izquierda, Clave, 11)
            str7 = Me.FormateaCadena(Alinear.Derecha, Ubicacion, 11)
            str8 = Me.FormateaCadena(Alinear.Derecha, Cantidad, 10)
    ...
    Public Sub AddItemTransferenciaTC(ByVal Modelo As String, ByVal Color As String, ByVal Talla As String, ByVal Cantidad As Double, ByVal Costo As Double, ByVal Subtotal As Double)
        Dim str As String
        Dim str4 As String
        Dim str7 As String
        Dim str8 As String
    ...
    Public Sub AddPagare(ByVal texto As String, ByVal Alineacion As Integer)
        Me.totales.Add(Me.FormateaCadena(Alineacion, texto, Me.maxChar) & Me.delimitador & CStr(Me.Style) & Me.delimitador & CStr(Me.fontSize))
    End Sub
    ...
    Public Sub AddTotalTicket(ByVal Total As Double, ByVal EstiloTotal As FontEstilo, Optional ByVal MON As String = "TOTAL")
        Dim str6 As String = Me.FormateaCadena(Alinear.Derecha, Strings.Format(TruncateToDecimal(Total, 2).ToString, "Currency"), 12)

        Dim str As String = FormateaCadena(Alinear.Derecha, MON, 5)
        Me.totales.Add(Me.FormateaCadena(Alinear.Derecha, (str & str6), (Me.maxChar - 11)) & Me.delimitador & CStr(EstiloTotal) & Me.delimitador & CStr(Me.fontSize + 4))
    End Sub
    ...
    Public Function Espacio(ByVal N As Integer) As String
        Dim str2 As String = ""
        Dim i As Integer
        For i = 0 To N - 1
            str2 = (str2 & " ")
    ...
    Public Function FormateaCadena(ByVal Alineacion As Alinear, ByVal s As String, ByVal length As Integer) As String
    ...
    Public Sub PrintTicket(ByVal impresora As String, ByVal LineasHoja As Integer, ByVal Lineas As Integer, Optional ByVal Generic As Boolean = False)
        ...
        If Generic Then
            ...
            TamañoPersonal = settings.PaperSizes.Item(settings.PaperSizes.Count - 1)
            'TamañoPersonal = New Printing.PaperSize("CKLASS", 284, 15748)
            document.DefaultPageSettings.PaperSize = TamañoPersonal
        ElseIf Lineas > LineasHoja Then
