...
Namespace PickingPrinting
    ...
    Public Class PackingSet
        Inherits PrintDocument
        Public PackingFont As Font
        Public PackingCodeFont As Font
        Public PackingFontBold As Font

        Private PackingSheets As New Collection(Of PackingSheet)()
        Private lssPackingSheetSettings As PickingSheetSettings
        Private NextSheet As Integer
        Private NextPage As Integer
        Private LastPage As Integer
        ...
        Public Sub New(ByVal tipoPapel As PaperSize, _
            ...
            lssPackingSheetSettings = New PickingSheetSettings(tipoPapel, vertical, anchoEtiqueta, altoEtiqueta, noColumnas, espacioColumnas, noFilas, espacioFilas, izqHoja, derHoja, arribaHoja, abajoHoja, izqEtiqueta, derEtiqueta, arribaEtiqueta, abajoEtiqueta)

            Me.DefaultPageSettings = DirectCast(lssPackingSheetSettings, PageSettings)
        End Sub

        Public Sub AddPackingSheet(ByVal NewSheet As PackingSheet)
            PackingSheets.Add(NewSheet)
        End Sub

        Public Function GetSheets() As Collection(Of PackingSheet)
        ...
        Protected Overloads Overrides Sub OnBeginPrint(ByVal e As PrintEventArgs)
            ...
            NextPage = IIf((NextPage < 1), 1, NextPage)
            LastPage = Me.PrinterSettings.ToPage
            NextSheet = (NextPage - 1) * Me.lssPackingSheetSettings.LabelsPerSheet
        End Sub
        ...
        Protected Overloads Overrides Sub OnPrintPage(ByVal e As PrintPageEventArgs)
            ...
            Dim w As Single = 0
            Dim h As Single = 0
            Dim LastSheet As Integer = PackingSheets.Count
            Dim CurrentColumn As Integer
            Dim CurrentRow As Integer

            Dim rectPacking As RectangleF
            Dim HojaPacking As PackingSheet

            w = lssPackingSheetSettings.LabelWidth - (lssPackingSheetSettings.LabelMargins.Left + lssPackingSheetSettings.LabelMargins.Right)
            h = lssPackingSheetSettings.LabelHeight - (lssPackingSheetSettings.LabelMargins.Top + lssPackingSheetSettings.LabelMargins.Bottom)

            For CurrentRow = 0 To lssPackingSheetSettings.LabelRows - 1
                For CurrentColumn = 0 To lssPackingSheetSettings.LabelColumns - 1
                    If NextSheet < LastSheet Then
                        HojaPacking = PackingSheets(NextSheet)

                        x = lssPackingSheetSettings.Margins.Left
                        ' 1/100in
                        y = lssPackingSheetSettings.Margins.Top
                        ' 1/100in
                        x = x + (CurrentColumn * (lssPackingSheetSettings.LabelWidth + lssPackingSheetSettings.LabelColumnSpacing)) + lssPackingSheetSettings.LabelMargins.Left
                        y = y + (CurrentRow * (lssPackingSheetSettings.LabelHeight + lssPackingSheetSettings.LabelRowSpacing)) + lssPackingSheetSettings.LabelMargins.Top

                        rectPacking = New RectangleF(New PointF(x, y), New SizeF(w, h))

                        HojaPacking.DrawPackingSheet(e.Graphics, PackingFont, PackingFontBold, PackingCodeFont, Brushes.Black, rectPacking)
            ...
            Next
            NextPage += 1
            e.HasMorePages = IIf((LastPage = 0), (NextSheet < PackingSheets.Count), (NextPage <= LastPage))
        End Sub
    End Class
