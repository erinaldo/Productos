...
#End Region
    ...
    Private Sub Clock_Tick(sender As Object, e As EventArgs) Handles Clock.Tick
        ...
        dtVenta = ModPOS.Recupera_Tabla("st_recupera_ventas", "@COMClave", ModPOS.CompanyActual)
        ...
        If NumFacturas > 0 Then
            ...
            Dim MonedaDesc, MonedaRef As String
            Dim MONClave As String = ""
            Dim Logo As Integer = 1
            Dim TipoCambio As Decimal
            Dim facturable As Boolean

            For i = 0 To dtVenta.Rows.Count - 1


                If MONClave <> dtVenta.Rows(i)("MONClave") Then
                    dt = ModPOS.Recupera_Tabla("sp_recupera_moneda", "@Moneda", dtVenta.Rows(i)("MONClave"))
                    MONClave = dt.Rows(0)("MONClave")
                    MonedaRef = dt.Rows(0)("Referencia")
                    MonedaDesc = dt.Rows(0)("Descripcion")
                    TipoCambio = dtVenta.Rows(i)("TipoCambio")
                    dt.Dispose()
                End If
                ...
                oCFD.TImpuesto = dtVenta.Rows(i)("TipoImpuesto")
                ...
                If facturable = True Then
                    ...
                    If dtMetodosPago.Rows.Count <> 1 Then
                        ...
                        msg = ModPOS.Ejecuta("st_error_log", "@Modulo", "InvoiceServer", "@TipoDocumento", 1, "@DOCClave", dtVenta.Rows(i)("VENClave"), "@Folio", dtVenta.Rows(i)("Folio"), "@Error", "El cliente " & oCFD.Clave & ", no cuenta con Metodo de Pago o cuenta con mas de uno", "@Usuario", ModPOS.UsuarioActual)
                    Else
                        ...
                        If VerificaDatoFiscalCte(oCFD) = True Then
                            ...
                            If ModPOS.validaFolio(SUCClave, CAJClave, 1, 1) Then
                                Try
                                    ...
                                    Dim dtLogo As DataTable = ModPOS.Recupera_Tabla("st_recupera_logo_fac", "@FACClave", FACClave, "@VENClave", dtVenta.Rows(i)("VENClave"))
                                    Logo = dtLogo.Rows(0)("Logo")
                                    dtLogo.Dispose()

                                    msg = ModPOS.Ejecuta("st_venta_factura", _
                                    ...
                                    If iCredito = 0 Then
                                        ...
                                        If dtAbnSaldo.Rows.Count > 0 Then
                                            ...
                                            For Each row As DataRow In dtAbnSaldo.Select("", "SaldoBase ASC")
                                                ModPOS.Aplica_Pagos(dtFacturas, oCFD.CTEClave, row("ID"), row("TipoPago"), row("SaldoBase"), CAJClave, row("Tipo"), row("Fecha"), idEvento)
                                                oCFD.AplicaAnticipo = True
                                            Next
                                            dtFacturas.Dispose()
                                            msg = ModPOS.Ejecuta("st_elimina_pendiente", "@VENClave", dtVenta.Rows(i)("VENClave"))
                                        Else
                                    ...
                                    If oCFD.VersionCF = "3.3" AndAlso iCredito = 1 Then
                                        ...
                                        msg = ModPOS.Ejecuta("sp_agregar_metodopagofac", _
                                    ...
                                    Else
                                        ...
                                        If oCFD.AplicaAnticipo = True Then
                                        ...
                                        If oCFD.Tipo = 1 Then
                                            ...
                                            Try
                                                ...
                                                If dsRes.Tables("AckErrorApplication").Rows(0)("documentStatus").ToString().ToUpper() = "ACCEPTED" Then
                                                    ...
                                                    msg = ModPOS.Ejecuta("sp_upd_complemento_envio", "@FACClave", oCFD.FACClave)
                                    ...
                                    If iTipoPac <> 0 Then
                                        ...
                                        If oCFD.email <> "" Then
                                            Dim sMsg As String
                                            sMsg = ModPOS.SendMail(oCFD.VersionCF, oCFD.email, oCFD.TipoCF, sFormatoFactura, CDate(oCFD.Fecha), oCFD.Serie & oCFD.Folio, oCFD.DOCClave, oCFD.total, MailAdress, MailUser, MailPassword, HostSMTP, MailPort, MailSSL, DisplayName, PathXML, SUCClave, TipoCambio, MonedaDesc, MonedaRef, Logo, False)
                                            If sMsg <> "" Then
                                                msg = ModPOS.Ejecuta("st_error_log", "@Modulo", "InvoiceServer", "@TipoDocumento", 1, "@DOCClave", dtVenta.Rows(i)("VENClave"), "@Folio", dtVenta.Rows(i)("Folio"), "@Error", "Error de envio:" & sMsg, "@Usuario", ModPOS.UsuarioActual)
                                            End If
                                        End If
                                    Else
                                        msg = ModPOS.Ejecuta("st_error_log", "@Modulo", "InvoiceServer", "@TipoDocumento", 1, "@DOCClave", dtVenta.Rows(i)("VENClave"), "@Folio", dtVenta.Rows(i)("Folio"), "@Error", "Error de Timbrado:", "@Usuario", ModPOS.UsuarioActual)
                                    End If
                                    System.Threading.Thread.Sleep(1000)

                                Catch ex As Exception
                                    msg = ModPOS.Ejecuta("st_error_log", "@Modulo", "InvoiceServer", "@TipoDocumento", 1, "@DOCClave", dtVenta.Rows(i)("VENClave"), "@Folio", dtVenta.Rows(i)("Folio"), "@Error", IIf(ex.ToString.Length > 500, ex.ToString.Substring(0, 500), ex.ToString), "@Usuario", ModPOS.UsuarioActual)
                        ...
                        Else
                            ' Grabar en log de errores de faturacion 
                            msg = ModPOS.Ejecuta("st_error_log", "@Modulo", "InvoiceServer", "@TipoDocumento", 1, "@DOCClave", dtVenta.Rows(i)("VENClave"), "@Folio", dtVenta.Rows(i)("Folio"), "@Error", "El cliente " & oCFD.Clave & " No cuenta con domicilio fiscal valido", "@Usuario", ModPOS.UsuarioActual)
                        End If
                    End If
                End If
            Next
