...
#End Region
    ...
    Private Moneda, MonRefBase As String
    ...
    Private SaldoCliente As Decimal
    Private TotalFactura As Decimal
    Private SaldoBase As Decimal
    Private SaldoVenta As Decimal
    ...
    Public Function CargaDatosTicket(ByVal Periodo As Integer, ByVal Mes As Integer, ByVal Folio As String, ByVal bCargaTickets As Boolean) As Boolean
        ...
        If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then
            ...
            If LstAsignados.Items.Count = 0 Then
                ...
                oCFD.TImpuesto = CInt(dt.Rows(0)("TipoImpuesto"))
            ...
            If MONClave = "" Then
                Dim dtMon As DataTable = ModPOS.Recupera_Tabla("sp_recupera_moneda", "@Moneda", dt.Rows(0)("MONClave"))
                MONClave = dtMon.Rows(0)("MONClave")
                MonedaRef = dtMon.Rows(0)("Referencia")
                MonedaDesc = dtMon.Rows(0)("Descripcion")
            ...
            ElseIf TipoCambio <> CDec(dt.Rows(0)("TipoCambio")) Then
                MessageBox.Show("No es posible agregar a la factura documentos con tipo de cambio distintos", "Información", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                Return False
            ElseIf MONClave <> CStr(dt.Rows(0)("MONClave")) Then
                MessageBox.Show("No es posible agregar a la factura documentos con tipos de moneda diferentes", "Información", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                Return False

            ElseIf oCFD.TImpuesto <> CInt(dt.Rows(0)("TipoImpuesto")) Then
                MessageBox.Show("No es posible agregar a la factura documentos con tipos de impuesto (General/Frontera) ", "Información", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                Return False
            End If
            ...
            If bCargaTickets = True AndAlso idFactura Is Nothing = False Then
    ...
    Private Sub FrmFactura_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not CmbCaja.SelectedValue Is Nothing Then
            ...
            If ClienteInicial = "" Then
                ...
                ClienteInicial = IIf(dt.Rows(0)("Mostrador").GetType.Name = "DBNull", "", dt.Rows(0)("Mostrador"))
                dt.Dispose()
            End If
        End If
    ...
    Private Function crearCF(ByVal FacturaID As String) As Boolean
        ...
        For i = 0 To dt.Rows.Count - 1
            ...
            If oCFD.TipoCF = 1 OrElse oCFD.TipoCF = 2 Then
                ...
                iTipoPAC = crearXML(1, dtPAC, PathXML, oCFD.Serie & oCFD.Folio, oCFD.DOCClave, oCFD.tipoDeComprobante, oCFD, dtConcepto, dtImpuesto, 1, InterfazSalida, dtDetalleImpuesto, dtRetencionDet, dtRetencion, True, TallaColor)
    ...
    '            Case Is >= Redondear(dtFacturas.Rows(i)("SaldoBase"), 2)
    '                ModPOS.Ejecuta("sp_actualiza_saldofac", _
    '                                              "@FACClave", dtFacturas.Rows(i)("ID"), _
    '                                              "@Abono", dtFacturas.Rows(i)("SaldoBase"))

    '                SaldoDisponible -= dtFacturas.Rows(i)("SaldoBase")
    ...
    Private Sub BtnGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnGuardar.Click
        ...
        ElseIf bPedidoCredito = False Then
            ...
            recuperaCliente(oCFD.CTEClave)

            iValidaCredito = ModPOS.ValidaCredido(CtaMaestra, LimiteCredito, DiasCredito, dTotal, TipoCambio)
            ...
            ElseIf iValidaCredito = -1 Then
                ...
                If CreditoDisponible < CDbl(dTotal * TipoCambio) Then
                    MessageBox.Show("No es posible realizar la factura debido a que el importe total del documento sobrepasa el crédito disponible del cliente " & Format(CStr(ModPOS.Redondear(CreditoDisponible, 2)), "Currency") & ", la venta actual lo sobrepasa por " & Format(CStr(ModPOS.Redondear(CDbl(dTotal * TipoCambio) - CreditoDisponible, 2)), "Currency"), "Información", MessageBoxButtons.OK)
                    Exit Sub
                End If
        ...
        If TallaColor = 1 Then
            Dim dtInterfaz As DataTable = Recupera_Tabla("st_recupera_interfaz", "@Interfaz", "Pedidos", "@COMClave", ModPOS.CompanyActual)
            If Not dtInterfaz Is Nothing AndAlso dtInterfaz.Rows.Count > 0 Then
                Dim sFecha As String
                'For aux As Integer = 0 To Ventas.GetUpperBound(0)
                sFecha = DateTime.Now.ToString("yyyyMMdd_HHmmssfff")
                If Ventas(0)("Tipo") <> "Contado" Then
                    ModPOS.Ejecuta(CStr(dtInterfaz.Rows(0)("sp")), "@Folio", idFactura, "@TipoDocumento", 3, "@Path", InterfazSalida, "@Fecha", sFecha)
                End If
            ...
            If dtAbonos.Rows.Count > 0 Then
                For i = 0 To dtAbonos.Rows.Count - 1
                    If CDbl(dtAbonos.Rows(i)("Abonos")) > 0 Then
                        ModPOS.Aplica_Pagos_Tickets(idFactura, CDbl(dtAbonos.Rows(i)("Abonos")), oCFD.CTEClave)
                    End If
                Next
            End If
        ...
        Else
            ...
            ModPOS.Ejecuta("st_saldo_ticketfactura", "@FacturaId", idFactura, "@CTEClave", oCFD.CTEClave)
        End If
        ...
        SaldoBase = IIf(dtFacturas.Compute("Sum(SaldoBase)", "SaldoBase > 0") Is System.DBNull.Value, 0, dtFacturas.Compute("Sum(SaldoBase)", "SaldoBase > 0"))



        If ChkCredito.GetEstado = 0 AndAlso CajaActiva = True AndAlso SaldoBase > 0 Then
                            ...
                            For i = 0 To b.drAbonos.Length - 1
                                ...
                                If b.drAbonos(i)("Tipo") = 2 AndAlso DateDiff(DateInterval.Day, CDate(b.drAbonos(i)("Fecha")), CDate(dtFacturas.Rows(0)("fechaFactura"))) = 0 Then
                                    ...
                                    If bTieneSello = False Then
                                        ...
                                        ModPOS.Aplica_Pagos(dtFacturas, oCFD.CTEClave, b.drAbonos(i)("ID"), b.drAbonos(i)("TipoPago"), ImportePago, CAJClave, 1, b.drAbonos(i)("Fecha"), idEvento)
                                    Else
                                        ModPOS.Aplica_Pagos(dtFacturas, oCFD.CTEClave, b.drAbonos(i)("ID"), b.drAbonos(i)("TipoPago"), b.drAbonos(i)("SaldoBase"), CAJClave, b.drAbonos(i)("Tipo"), b.drAbonos(i)("Fecha"), idEvento)
                        ...
                        If a.DialogResult = DialogResult.OK Then
                            If a.detallePago.Rows.Count > 0 Then
                                ...
                                For i = 0 To a.detallePago.Rows.Count - 1
                                    ModPOS.Aplica_Pagos(dtFacturas, oCFD.CTEClave, a.detallePago.Rows(i)("ABNClave"), a.detallePago.Rows(i)("TipoPago"), a.detallePago.Rows(i)("SaldoBase"), CAJClave, 1, a.detallePago.Rows(i)("FechaPago"), idEvento)
                                Next

                                If TallaColor = 1 Then
                                    For i = 0 To a.detallePago.Rows.Count - 1
                                        imprimirRecibo(a.detallePago.Rows(i)("ABNClave"), "Ticket", True, ImpresoraCajon, Math.Round(a.TotalCambio, 2), " ")
                                    Next
                                    ...
                                    If Not dtInterfaz Is Nothing AndAlso dtInterfaz.Rows.Count > 0 Then
                                        If a.detallePago.Rows.Count > 0 Then
                                            ModPOS.Ejecuta(CStr(dtInterfaz.Rows(0)("sp")), "@Folio", idEvento, "@TipoDocumento", 2, "@Path", InterfazSalida, "@Fecha", sFecha, "@Tipo", 2)
                                        End If
            ...
            Select Case MessageBox.Show("¿Desea enviar el documento por correo electrónico?", "Pregunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                Case DialogResult.Yes
                    ...
                    For j = 0 To dtFactura.Rows.Count - 1
                        ...
                        ModPOS.SendMail(oCFD.VersionCF, oCFD.email, oCFD.TipoCF, dtFactura.Rows(j)("Formato"), CDate(dtFactura.Rows(j)("Fecha")), dtFactura.Rows(j)("Folio"), dtFactura.Rows(j)("FACClave"), dtFactura.Rows(j)("Total"), MailAdress, MailUser, MailPassword, HostSMTP, MailPort, MailSSL, DisplayName, PathXML, SUCClave, TipoCambio, MonedaDesc, MonedaRef, CInt(dtFactura.Rows(j)("Logo")))
                    Next
                    dtFactura.Dispose()
            End Select
        End If
