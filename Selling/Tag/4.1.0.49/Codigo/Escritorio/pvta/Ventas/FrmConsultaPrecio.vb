...
#End Region
    ...
    Private Sub recuperaProducto(ByVal TipoBusqueda As Integer, ByVal Texto As String, ByVal Cantidad As Double)
        ...
        dtCliente = ModPOS.Recupera_Tabla("sp_venta_lista", "@Sucursal", SUCClave, "@Cliente", Cliente, "@TipoCanal", TipoCanal)
        ListaPrecio = dtCliente.Rows(0)("PREClave")
        TImpuesto = dtCliente.Rows(0)("TipoImpuesto")
        dtCliente.Dispose()

        'Busca y recupera los datos del producto
        Dim dtProducto As DataTable = ModPOS.SiExisteRecupera("sp_venta_producto", "CTEClave", Cliente, "@SUCClave", SUCClave, "@Tipo", TipoBusqueda, "@Busca", Texto.Trim.ToUpper.Replace("'", "''"), "@PREClave", ListaPrecio, "@Cantidad", Cantidad, "@Char", cReplace, "@TallaColor", TallaColor)
    ...
    Private Sub TxtMostrador_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TxtMostrador.KeyPress
        If (Asc(e.KeyChar)) = 13 Then
            If Not TxtMostrador.Text = vbNullString Then
                Dim dt As DataTable
                dt = ModPOS.SiExisteRecupera("sp_recupera_clavecte", "@Clave", TxtMostrador.Text.Trim.ToUpper.Replace("'", "''"), "@COMClave", ModPOS.CompanyActual, "@SUCClave", SUCClave)
