Public Class FrmEditAbono
    ...
    Public cancelarPago As Boolean = False
    Public pagoCancelado As Boolean = False
    Public dtDatosPinPad As DataTable
    Public pagoTarjetaAmiga As Boolean = False
...
#End Region
    ...
    Private Sub BtnActualizar_Click(sender As Object, e As EventArgs) Handles BtnActualizar.Click
        If cancelarPago Then
            If terminalPinPad Then
                Cancelar()
            ElseIf pagoTarjetaAmiga Then
                CancelarTarjetaAmiga()
            End If
            Me.Close()
            Exit Sub
        End If
    ...
    Private Sub FrmEditAbono_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Dim numAutorizacion As String = IIf(dt.Rows(0)("NumAutorizacion").GetType.Name <> "DBNull", dt.Rows(0)("NumAutorizacion"), "")
        dt.Dispose()
        ...
        If cancelarPago Then
            Me.Text = "Eliminar Abono"
            CmbBanco.Enabled = False
            TxtNumCta.Enabled = pagoTarjetaAmiga
            TxtReferencia.Enabled = False
            If pagoTarjetaAmiga Then
                TxtReferencia.Text = numAutorizacion
                lblNumCta.Text = "Tarjeta"
                lblFecha.Visible = False
                cmbFechaPago.Visible = False
                lblBeneficiaria.Visible = False
                cmbBeneficiaria.Visible = False
    ...
    Private Sub CancelarTarjetaAmiga()
        ...
        datosTarjetaAmiga.tarjeta = TxtNumCta.Text.Replace("-", "")
        datosTarjetaAmiga.monto = monto
        datosTarjetaAmiga.idComercio = sIdComercio
        datosTarjetaAmiga.idTerminal = sIdTerminal
        datosTarjetaAmiga.idTransaccion = TxtReferencia.Text

        datosTarjetaAmiga = datosTarjetaAmiga.consulta(TarjetaAmiga.Cancelacion)
    ...
    Private Sub Cancelar()
        ...
        Dim importe As Double = dtDatosPinPad.Rows(0)("Importe")
        Dim numAutorizacion As String = dtDatosPinPad.Rows(0)("numAutorizacion").ToString()
        Dim refFinanciera As String = dtDatosPinPad.Rows(0)("referenciaFinanciera")
        Dim secuencia As String = dtDatosPinPad.Rows(0)("secuencia")

        With pagoTarjeta
            .e01Transaccion = PinPad.tipoOperacion.CancelarVenta
            .leerArchivo()
            If PinPadNumero <> "" Then
                .e02Terminal = PinPadNumero
            End If
            .e03Sesion = Format(Today, "MMdd")
            .e04Secuencia = secuencia 'Se debe de poner la misma de la venta
            .e05Importe = pagoTarjeta.formatoImporte(importe)
            ...
            .e15Autoriza = numAutorizacion 'Se debe de poner el numAutorizacion que regreso la venta
            .e16Modo = StrDup(2, " ")
            '.e17CVV2 = StrDup(4, " ")
            '.e18TrackII = "                                        "
            '.e19Numero = StrDup(3, " ")
            ...
            '.e26Filler = "0000"
            .e27Referencia = refFinanciera 'Se pone la referencia financiera que regreso la venta
            '.e28Filler = "0"
            '.e29Filler = "000"
            '.e30Filler = "000"
            '.e31MultiPagos = "0000"
            ...
            If .miPinPad.RespDLL = 0 AndAlso .miPinPad.ClsResponse.C05_CodigoRespuesta = "00" Then
                pagoCancelado = True
            Else
                If .r16Leyenda.Length = 0 Then
                    .r16Leyenda = .miPinPad.Response
                End If
                MessageBox.Show(.r16Leyenda, "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
        End With
    End Sub

    Private Sub cmbTerminal_SelectedValueChanged(sender As Object, e As EventArgs) Handles cmbTerminal.SelectedValueChanged
        If cancelarPago And formaLoad And Not cmbTerminal.SelectedValue Is Nothing Then
