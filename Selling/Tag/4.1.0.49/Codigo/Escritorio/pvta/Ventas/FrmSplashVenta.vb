...
#End Region
    ...
    Private Sub FrmSplashVenta_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If FrmPadre = "Caja" OrElse FrmPadre = "Devolucion" Then
    ...
    Private Sub Entrar()
        ...
        If FrmPadre = "Verificar" Then
            ...
            If dt.Rows(0)("Tipo") = 1 OrElse dt.Rows(0)("Tipo") = 2 Then 'Venta
                ...
                a.TipoCanal = dt.Rows(0)("TipoCanal")
                a.SUCClave = SUCClave
        ...
        Else
            ...
            If Not CmbSucursal.SelectedValue Is Nothing Then
                ...
                If Not cmbPuntodeVenta.SelectedValue Is Nothing Then
                    ...
                    If FrmPadre = "Devolucion" Then

                        Dim ClaveCaja, ALMClave, TICDevolucion, Impresora, Referencia, Recibo, Aplicacion As String
                        Dim reciboTicket As Integer
                        ...
                        If fase = 0 Then
                            ...
                            If ModPOS.Devolucion Is Nothing Then
                                ...
                                ModPOS.Devolucion.Padre = "Menu"
                                ModPOS.Devolucion.ActivaDevolucion = True
                                ModPOS.Devolucion.ActivaCaja = True
                                ModPOS.Devolucion.CAJClave = cmbPuntodeVenta.SelectedValue
                                ModPOS.Devolucion.Referencia = ClaveCaja
                                ...
                                ModPOS.Devolucion.Caja = Referencia
                                ModPOS.Devolucion.Cajon = Cajon
                                ModPOS.Devolucion.bLiquidacion = True
                                ModPOS.Devolucion.WindowState = FormWindowState.Maximized
                                ModPOS.Devolucion.PrintGeneric = PrintGeneric

                            End If
                            ModPOS.Devolucion.Width = Screen.PrimaryScreen.Bounds.Width
                            ModPOS.Devolucion.Height = Screen.PrimaryScreen.Bounds.Height
                    ...
                    ElseIf FrmPadre = "Caja" Then
                        ...
                        Dim Stage, StageCancelacion, Mostrador, PDVClave, StageLU As String
                        Dim FormatoRecibo As String
                        Dim IncluirCierre As Boolean
                        Dim idPos, passCajero As String
                        ...
                        If Fase = 0 Then
                            ...
                            If ModPOS.MtoCXC Is Nothing Then
                                ...
                                ModPOS.MtoCXC.FormatoRecibo = FormatoRecibo
                                ModPOS.MtoCXC.PDVClave = PDVClave
                                ModPOS.MtoCXC.StageLU = StageLU
                                ModPOS.MtoCXC.IncluirCierre = IncluirCierre
                                ModPOS.MtoCXC.idPOS = idPos
                                ModPOS.MtoCXC.idCajero = idCajero
                                ModPOS.MtoCXC.passCajero = passCajero
                            End If
                        ...
                        Else
                            ...
                            ElseIf dt.Rows(0)("Tipo") = 6 Then ' TouchCK
                                If ModPOS.TouchCK Is Nothing Then
                                    ModPOS.TouchCK = New FrmTouchCK
                                    With ModPOS.TouchCK
                                        .StartPosition = FormStartPosition.CenterScreen
                                        .ALMClave = dt.Rows(0)("ALMClave")
                                        ...
                                        .UBCClave = IIf(dt.Rows(0)("StageSurtido").GetType.Name = "DBNull", "", dt.Rows(0)("StageSurtido"))
                                ...
                                ModPOS.TouchCK.Width = Screen.PrimaryScreen.Bounds.Width
                                ModPOS.TouchCK.Height = Screen.PrimaryScreen.Bounds.Height
                                ModPOS.TouchCK.StartPosition = FormStartPosition.CenterScreen
                                ModPOS.TouchCK.ShowDialog()
                                Me.Close()
    ...
    Private Sub BtnAbrir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnAbrir.Click
        ...
        Else
            ...
            Else
                ...
                Dim sProcesador As String = ""
                Dim sID As String = ""

                If Compartir = 1 Then
                    sProcesador = ModPOS.GetProcessorId()
                End If

                dt = Recupera_Tabla("st_recupera_abiertos", "@USRClave", ModPOS.UsuarioActual, "@Procesador", sProcesador)
                numAbiertos = dt.Rows.Count
                If numAbiertos = 1 Then
                    sID = dt.Rows(0)("PDVClave")
                End If
                dt.Dispose()
                If numAbiertos > 0 Then
                    ...
                    If numAbiertos = 1 Then
                        ...
                        dt = ModPOS.Recupera_Tabla("sp_recupera_puntodeventa", "@PDVClave", sID)
                        ...
                        If Liberar = 1 Then
                            If MessageBox.Show("Esta apunto de liberar el punto de venta, asegurese que no este siendo usado en otra por usted mismo en otra ventana del programa o  en otra computadora antes de continuar, ya que podria provocar conflictos con los consecutivos de los folios. Para continuar precione OK o Cancel para salir sin realizar cambios", "Advertencia", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.OK Then
                                ...
                                ModPOS.Ejecuta("sp_actualiza_fase", "@Clave", sID, "@Tipo", Tipo)
                                ...
                                End If
                                Tipo = 1
                                cmbPuntodeVenta.SelectedValue = sID
                            End If
                        Else
                    ...
                    End If
                Else
                    MessageBox.Show("No se encontraron Puntos de Venta Abiertos asociados al Vendedor Actual o al equipo", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            End If
