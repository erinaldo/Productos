...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.GroupBox1.Controls.Add(Me.lblFolioSAP)
        ...
        'lblFolioSAP
        '
        Me.lblFolioSAP.Anchor = CType(((System.Windows.Forms.AnchorStyles.Top Or System.Windows.Forms.AnchorStyles.Left) _
            Or System.Windows.Forms.AnchorStyles.Right), System.Windows.Forms.AnchorStyles)
        Me.lblFolioSAP.Font = New System.Drawing.Font("Microsoft Sans Serif", 9.75!)
        Me.lblFolioSAP.Location = New System.Drawing.Point(76, 21)
        Me.lblFolioSAP.Name = "lblFolioSAP"
        Me.lblFolioSAP.Size = New System.Drawing.Size(139, 17)
        Me.lblFolioSAP.TabIndex = 154
        '
        'BtnStage
        '
        ...
        Me.BtnStage.Location = New System.Drawing.Point(180, 167)
        ...
        Me.TxtStage.Location = New System.Drawing.Point(58, 168)
        ...
        Me.Label11.Location = New System.Drawing.Point(10, 171)
        ...
        Me.txtTotalPza.Location = New System.Drawing.Point(91, 551)
        Me.txtTotalPza.Name = "txtTotalPza"
        Me.txtTotalPza.Size = New System.Drawing.Size(87, 20)
        Me.txtTotalPza.TabIndex = 133
        ...
        Me.Label13.Location = New System.Drawing.Point(13, 554)
...
#End Region
    ...
    Public SUCClave, InterfazSalida As String
    ...
    Public FolioSAP As String

    Public VersionCF As String
    Public dtPac As DataTable
    ...
    Private Transito, ReciboRF, Picking, Packing, SurtidoRF, ticketPicking As Boolean
    Private TCosto As Integer
    Private dtDetalle As DataTable
    Private bLoad As Boolean = False
    Private PROClave, TIKClave As String
    Private TProducto As Integer
    Private Cantidad As Double
    Private Clave, ClaveSAT, Pedimento As String
    Private Costo As Double
    Private NumDecimales As Integer
    Private Disponible As Double
    Private Autoriza As String
    Private Inventario As Boolean = True

    Private alerta(5) As PictureBox
    Private reloj As parpadea

    Public Function crearCF(ByVal TRSClave As String) As Boolean
        Dim oCFD As New CFD
        ...
        dt = ModPOS.Recupera_Tabla("sp_recupera_traslado", "@TRSClave", TRSClave)

        dtConcepto = ModPOS.Recupera_Tabla("st_recupera_concepto_traslado", "@TRSClave", TRSClave)
    ...
    Private Function validaForm() As Boolean
        ...
        ElseIf Me.Padre = "Agregar" Then
            ...
            If ModPOS.SiExite(Con, "sp_valida_PK", "@tabla", "ReciboAlmacen", "@clave", UCase(Trim(Me.TxtFolio.Text)), "@COMClave", ModPOS.CompanyActual) > 0 Then
                Beep()
    ...
    Private Function recuperaProducto(ByVal sClave As String) As Boolean
        ...
        If Not dtProducto Is Nothing AndAlso dtProducto.Rows.Count > 0 Then
            ...
            Pedimento = dtProducto.Rows(0)("Pedimento")
    ...
    Private Function validaExistencia() As Boolean
        ...
        For i = 0 To dtDetalle.Rows.Count - 1
            ...
            If dtDetalle.Rows(i)("VENClave") = "" Then
                ...
                If dtDetalle.Rows(i)("Cantidad") > Disponible Then
                    result = False
                    dtDetalle.Rows(i)("Cantidad") = Disponible
                    dtDetalle.Rows(i)("Total") = Disponible * dtDetalle.Rows(i)("Costo")
    ...
    Private Function validaExistenciaUBC() As Boolean
        Dim result As Boolean = True
        If dtDetalle.Rows.Count > 0 Then
            ...
            foundRows = dtDetalle.Select("TProducto <= 3 and VENClave=''")
            If foundRows.GetLength(0) > 0 Then
                For i = 0 To foundRows.GetUpperBound(0)
                    ...
                    If Cantidad > 0 Then
                        result = False
                        foundRows(i)("Cantidad") -= Cantidad
                        foundRows(i)("Total") = foundRows(i)("Cantidad") * foundRows(i)("Costo")
                    End If
                Next
            End If
        End If
    ...
    Private Sub FrmTraslado_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Padre = "Nuevo" Then
            ...
            dtDetalle = ModPOS.CrearTabla("TrasladoDetalle", _
                                          ...
                                          "Solicitado", "System.Double", _
                                          "Cantidad", "System.Double", _
                                          "ClaveSAT", "System.String", _
                                          "Clave", "System.String", _
                                          "Nombre", "System.String", _
                                          "Pedimento", "System.String")
            ...
            GridDetalle.RootTable.Columns("Solicitado").FormatString = "0.00"
        ...
        Else
            ...
            If TallaColor = 0 Then
                GridDetalle.CurrentTable.Columns("Modelo").Visible = True
                GridDetalle.CurrentTable.Columns("Talla").Visible = True
                GridDetalle.CurrentTable.Columns("TallaUSA").Visible = True
                GridDetalle.CurrentTable.Columns("Color").Visible = True
            End If


            If Estado = 7 Then
                GridDetalle.CurrentTable.Columns("Recibido").Visible = True
            Else
                GridDetalle.CurrentTable.Columns("Recibido").Visible = False
            End If
            GridDetalle.RootTable.Columns("Cantidad").FormatString = "0.00"
        ...
        fc = New Janus.Windows.GridEX.GridEXFormatCondition(GridDetalle.RootTable.Columns("Solicitado"), Janus.Windows.GridEX.ConditionOperator.GreaterThan, GridDetalle.GetValue("Cantidad"))
    ...
    Private Sub TxtClaveProd_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TxtClaveProd.KeyDown
        If e.KeyCode = Keys.Enter Then
            If Not System.String.IsNullOrEmpty(TxtClaveProd.Text) Then
                ...
                If TallaColor = 1 Then
                    Cantidad = IIf(Math.Abs(CDbl(TxtCantidad.Text)) = 0, 1, Math.Abs(CDbl(TxtCantidad.Text)))
                    aceptaCantidad()
                End If
            Else
    ...
    Public Sub aceptaCantidad()
        ...
        If System.String.IsNullOrEmpty(PROClave) Then
            Beep()
            Do
                result = MessageBox.Show("¡La Clave de producto es requerida!", "Error", MessageBoxButtons.OKCancel, MessageBoxIcon.Stop, MessageBoxDefaultButton.Button2)
        ...
        Else
            ...
            Dim foundRows() As System.Data.DataRow
            foundRows = dtDetalle.Select("PROClave = '" & PROClave & "' and VENClave=''")

            If TallaColor = 1 AndAlso foundRows.Length > 0 Then
                Cantidad += foundRows(0)("Cantidad")
            End If

            If Inventario = True Then
                If Cantidad > Disponible Then
                    Do
                        result = MessageBox.Show("La cantidad registrada del producto " & Clave & " excede la cantidad disponible en el almacén origen, el disponible actual es de: " & CStr(Disponible), "Información", MessageBoxButtons.OKCancel, MessageBoxIcon.Information, MessageBoxDefaultButton.Button2)
                    Loop While result = System.Windows.Forms.DialogResult.Cancel
                    Exit Sub
                End If
            End If
            ...
            If cmbSucursalO.SelectedValue Is Nothing Then
                Do
                    result = MessageBox.Show("La Sucursal Origen seleccionada no es valida o es requerida", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Loop While result = System.Windows.Forms.DialogResult.Cancel
                Exit Sub
            End If
            ...
            If foundRows.Length = 0 Then
                ...
                Else
                    Do
                        result = MessageBox.Show("¡La Cantidad de producto es requerida!", "Error", MessageBoxButtons.OKCancel, MessageBoxIcon.Stop)
                    Loop While result = System.Windows.Forms.DialogResult.Cancel
                End If
            ElseIf Cantidad = 0 Then
                'Elimina
                If foundRows(0)("VENClave") = "" Then
            ...
            ElseIf Cantidad <> foundRows(0)("Cantidad") AndAlso foundRows(0)("VENClave") = "" Then
                'actualiza
                foundRows(0)("Solicitado") = Cantidad
    ...
    Private Sub BtnGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnGuardar.Click
        If validaForm() Then
            Dim dt As DataTable
            Dim bConfirma As Boolean = True
            Dim imprimeCFD As Boolean = False
            ...
            If EstadoActual = 1 Then
                ...
                ReciboRF = IIf(dt.Rows(0)("ReciboRF").GetType.Name = "DBNull", False, dt.Rows(0)("ReciboRF"))
                ...
                If Picking = True AndAlso UBCClave = "" Then
                ...
                If Inventario = True Then
                    If validaExistencia() = False Then
                        If MessageBox.Show("La cantidad registrada de uno o más productos excede el disponible en el almacén origen. ¿Desea Cancelar (Cancel) para interrumpir o Continuar (OK) y solo procesar el disponible?", "Pregunta", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = System.Windows.Forms.DialogResult.Cancel Then
                            Exit Sub
                        End If
                    ElseIf Picking = True Then
                        If validaExistenciaUBC() = False Then
                            If MessageBox.Show("La cantidad registrada de uno o más productos excede el disponible en el almacén origen. ¿Desea Cancelar (Cancel) para interrumpir o Continuar (OK) y solo procesar el disponible?", "Pregunta", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = System.Windows.Forms.DialogResult.Cancel Then
                                Exit Sub
                            End If
                        End If
                ...
                If Padre = "Nuevo" Then
                    ...
                    If TallaColor = 1 Then
                        dt = ModPOS.SiExisteRecupera("sp_recupera_alm", "@ALMClave", ALMClave)
                        claveAlm = Microsoft.VisualBasic.Left(dt.Rows(0)("Clave"), 2)
                        dt.Dispose()
                    End If
                    

                    Folio = claveAlm & Referencia & Microsoft.VisualBasic.Right(CStr(Periodo), 2) & Microsoft.VisualBasic.Right("0" & CStr(Mes), 2) & "-" & CStr(Folio)
                    TxtFolio.Text = Folio
                ...
                Else
                    ...
                    For fila = 0 To dtDetalle.Rows.Count - 1
                        If dtDetalle.Rows(fila)("VENClave") = "" Then
                            ModPOS.Ejecuta("sp_inserta_trasladodetalle", _
                           "@DTRSClave", dtDetalle.Rows(fila)("ID"), _
                           "@TRSClave", TRSClave, _
                           "@PROClave", dtDetalle.Rows(fila)("PROClave"), _
                ...
                If TallaColor = 1 AndAlso EstadoActual = 1 Then
                    If MessageBox.Show("¿Desea Confirmar y cerrar el Pedido de Traslado?", "Pregunta", MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.No Then
                        bConfirma = False
                    End If

                End If

                If bConfirma = True Then
                    Dim a As New MeAutorizacion
                    a.Sucursal = cmbSucursalO.SelectedValue
                    a.MontodeAutorizacion = GridDetalle.GetTotal(GridDetalle.CurrentTable.Columns("Total"), Janus.Windows.GridEX.AggregateFunction.Sum)
                    a.StartPosition = FormStartPosition.CenterScreen
                ...
                If EstadoActual = 2 Then 'Cerrado
                    ...
                    ModPOS.GeneraMovInv(2, 8, 8, TRSClave, ALMClave, Folio, Autoriza, Picking)

                    Ejecuta("sp_recibo_traslado", "@TRSClave", TRSClave)
                    ModPOS.GeneraMovInv(1, 8, 8, TRSClave, ALMDestino, Folio, Autoriza, Picking)
                    ...
                    If InterfazSalida <> "" Then
                        ...
                        If dtInterfaz.Rows.Count > 0 Then
                            ModPOS.Ejecuta(CStr(dtInterfaz.Rows(0)("sp")), "@Folio", TRSClave, "@TipoDocumento", 0, "@Path", InterfazSalida, "@Fecha", sFecha)
                        End If
                    End If
                ...
                ElseIf EstadoActual = 5 Then 'Picking
                    ModPOS.calculaRecoleccion(8, TRSClave, ALMClave, UBCClave, IIf(SurtidoRF = True, 1, 0))
                End If
                ...
                If (EstadoActual = 2 OrElse EstadoActual = 4) AndAlso VersionCF = "3.3" Then
                    If MessageBox.Show("¿Deseas generar el comprobante fiscal digital de traslado?", "Pregunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                        imprimeCFD = crearCF(TRSClave)
                    End If
                End If
            End If
                    ...
                    If Not (Packing = True AndAlso tipoEntrega = 3) Then
                        Select Case MessageBox.Show("¿Desea Reimprimir la Hoja de Recolección?", "Pregunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                            Case DialogResult.Yes
                                ModPOS.ImprimirSurtido(8, TRSClave, True, True, SUCClave, TIKClave, ticketPicking, TallaColor)
                        End Select
                    End If
                ElseIf MessageBox.Show("¿Desea Imprimir el Traslado?", "Pregunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                    ...
                    If imprimeCFD = True Then
                        'Imprime CFDI Traslado

                        imprimirCfdiTraslado(TRSClave, iCopias, sImpresora)
                    ...
                    Else
                        If Formato = 1 Then
                            ...
                            Do
                                ModPOS.imprimirTicketTraslado(sTIKClave, True, TRSClave, TallaColor)
                        ...
                        Else
                            imprimirReporteTraslado(TRSClave, False, iCopias, sImpresora)
                        End If
            ...
            Select Case MessageBox.Show("Se eliminara el producto: " & GridDetalle.GetValue("Clave") & ", " & GridDetalle.GetValue("Nombre"), "Advertencia", MessageBoxButtons.YesNo, MessageBoxIcon.Warning)
                Case DialogResult.Yes
                    ...
                    If foundRows.Length = 1 Then
                        ...
                        If foundRows(0)("VENClave") <> "" Then
                            MessageBox.Show("No es posible eliminar el producto debido a que se encuentra bloqueado y comprometido para su envio a una sucursal", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            Exit Sub
                        End If

                        Dim dtCargo As DataTable = ModPOS.SiExisteRecupera("st_verificarCargoProveedor", "@TRSClave", TRSClave, "@PROClave", GridDetalle.GetValue("PROClave"))
                        If Not dtCargo Is Nothing Then
                            If dtCargo.Rows(0)("Cantidad") > 0 Then
                                MessageBox.Show("No se puede eliminar el producto debido a que tiene un cargo a proveedor asociado", "Información", MessageBoxButtons.OK)
                                Exit Sub
                            End If
                        End If
    ...
    Private Sub cmbTipoEntrega_SelectedValueChanged(sender As Object, e As EventArgs) Handles cmbTipoEntrega.SelectedValueChanged
    ...
    Private Sub GridDetalle_KeyDown(sender As Object, e As KeyEventArgs) Handles GridDetalle.KeyDown
                            ...
                            If item.Length = 2 Then
                                If Not IsDBNull(item(0)) AndAlso Not IsDBNull(item(1)) Then
                                    If IsNumeric(item(1)) Then
                                        If recuperaProducto(CStr(item(0)).Trim) Then
                                            Cantidad = CDbl(item(1))
                                            aceptaCantidad()
                                        Else
                                            frmStatusMessage.Close()
                                            MessageBox.Show("El producto " & CStr(item(0)).Trim & " no es valido, el sistema espera que se proporcione la Clave de producto", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                                            Exit For
                                        End If
                                    Else
                                        frmStatusMessage.Close()
