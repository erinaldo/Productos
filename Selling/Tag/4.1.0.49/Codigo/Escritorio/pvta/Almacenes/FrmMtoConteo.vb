            ...
            If sConteoSelected <> "" Then
                ...
                If bIniciar = True Then
                    ...
                    If iEstado = 1 Then
                        ...
                        dt = ModPOS.Recupera_Tabla("st_recupera_conteo_abierto", "@ALMClave", ALMClave)

                        If dt.Rows.Count > 0 Then
                            MessageBox.Show("No es posible iniciar un conteo debido a que se encuentra un conteo previo en proceso o finalizado con diferencias. Finalice y Ajuste dicho conteo para continuar", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            bError = True
                            Exit Sub
                        End If
                        ...
                        If dt Is Nothing Then
                            MessageBox.Show("No es posible iniciar un conteo debido a que no se encuentra con ubicaciones asignadas", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            bError = True
                            Exit Sub
                        Else
                            dt = ModPOS.Recupera_Tabla("st_conteo_asignacion_Usuario", "@CONClave", sConteoSelected)

                            If dt.Rows.Count > 0 Then
                                MessageBox.Show("No es posible iniciar un conteo debido a que no todas las ubicaciones tiene un usuario asignado", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                                bError = True
                                Exit Sub
                            End If
                        End If
                            ...
                            Case DialogResult.Yes
                                ...
                                If bError = False Then
                                    ...
                                    If foundRows.Length > 0 Then
                                        MessageBox.Show("El conteo seleccionado no puede ser iniciado debido a que tiene ubicaciones pendientes de asignar", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                ...
                Else
                    ...
                    If iEstado = 2 OrElse iEstado = 3 Then
                        ...
                        Dim conteoSel As String = sConteoSelected
                        ActualizarGrid()
                        sConteoSelected = conteoSel
    ...
    Private Sub BtnDiferencias_Click(sender As Object, e As EventArgs) Handles BtnDiferencias.Click
        If validaForm() Then
            If sConteoSelected <> "" Then
                ...
                Dim dt As DataTable
                dt = ModPOS.Recupera_Tabla("sp_recupera_conteo", "@CONClave", sConteoSelected)
                If dt.Rows(0)("Estado") = 3 Or dt.Rows(0)("Estado") = 5 Then

                    Dim OpenReport As New Report
                    Dim pvtaDataSet As New DataSet
                    pvtaDataSet.DataSetName = "reportDataSet"
                    pvtaDataSet.Tables.Add(ModPOS.Recupera_Tabla("sp_rep_ajustes", "@CONClave", sConteoSelected, "@Forma", 2))

                        OpenReport.PrintPreview("Ajustes de Inventario", "CRAjustes.rpt", pvtaDataSet, "")
                  

                ElseIf dt.Rows(0)("Estado") = 4 Then

                    Dim OpenReport As New Report
                    Dim pvtaDataSet As New DataSet
                    pvtaDataSet.DataSetName = "reportDataSet"
                    pvtaDataSet.Tables.Add(ModPOS.Recupera_Tabla("sp_rep_ajustes", "@CONClave", sConteoSelected, "@Forma", 1))

                    OpenReport.PrintPreview("Conteo de Inventario", "CRAjustes.rpt", pvtaDataSet, "")

                Else

                    MessageBox.Show("Solo es posible ajuster Conteos que se encuentran Finalizados", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
                dt.Dispose()
