Public Class FrmAjustar
    ...
    Private Sub FrmAjustar_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Padre = "Ajustar" Then
            ...
            btnReubicar.Enabled = True
        Else
            Me.Text = "Resumen del Conteo"
            BtnGuardar.Text = "Guardar"
            Forma = 1
            btnReubicar.Enabled = False
        End If
    ...
    Private Sub BtnGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnGuardar.Click
        If Forma = 1 Then
            ...
            If Not dtConteoDetalle Is Nothing AndAlso dtConteoDetalle.Rows.Count > 0 Then
                ...
                End If
            Else
                ModPOS.Ejecuta("sp_end_conteo", "@CONClave", CONClave, "@Usuario", ModPOS.UsuarioActual)
            ...
            If Not dtConteo Is Nothing AndAlso dtConteo.Rows.Count > 0 Then
                ...
                If a.DialogResult = DialogResult.OK Then
                    If a.Autorizado Then
                        Cursor.Current = Cursors.WaitCursor
                        ModPOS.Ejecuta("sp_ajusta_conteo", "@CONClave", CONClave, "@Referencia", CONClave, "@Usuario", a.Autoriza)
                        ...
                        If InterfazSalida <> "" Then
                            ...
                            If dtInterfaz.Rows.Count > 0 Then
                                ModPOS.Ejecuta(CStr(dtInterfaz.Rows(0)("sp")), "@Folio", CONClave, "@TipoDocumento", 0, "@Path", InterfazSalida, "@Fecha", sFecha)
                            End If
                        End If
    ...
    Private Sub btnReubicar_Click(sender As Object, e As EventArgs) Handles btnReubicar.Click
        'validar si existe un stage de diferencias

        Dim dt As DataTable = ModPOS.Recupera_Tabla("st_valida_stage_conteo", "@CONClave", CONClave)

        If dt.Rows(0)("Stage") = "" Then
            dt.Dispose()
            Beep()
            MessageBox.Show("El Almacén no cuenta con un Stage de Diferencias Asignado", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Exit Sub
        End If
        Cursor.Current = Cursors.WaitCursor
        ModPOS.Ejecuta("st_reubicar_conteo", "@CONClave", CONClave, "@Stage", dt.Rows(0)("Stage"), "@Referencia", CONClave, "@Usuario", ModPOS.UsuarioActual)
        dt.Dispose()
        Cursor.Current = Cursors.Default
        Me.Close()
    End Sub
