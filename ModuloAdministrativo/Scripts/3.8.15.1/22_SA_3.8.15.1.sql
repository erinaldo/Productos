...
/****** Object:  StoredProcedure [dbo].[sel_FacturasBusqueda]    Script Date: 07/19/2011 14:37:53 ******/
...
begin
	...
	left outer join VentasFacturaCFD on Ventas.IdCedis = VentasFacturaCFD.IdCedis and  Ventas.IdTipoVenta = VentasFacturaCFD.IdTipoVenta and 
		Ventas.Serie = VentasFacturaCFD.Serie and Ventas.Folio = VentasFacturaCFD.Folio and VentasFacturaCFD.UUID is not null and VentasFacturaCFD.TransprodIdFactura <> ''''
	left outer join Route.dbo.TransProd Trp on Trp.TransProdId = VentasFacturaCFD.TransProdIdFactura and Trp.TipoFase <> 0
...
begin
	...
	exec( 'SELECT left(v.RutClave, len(v.RutClave)-4) as IdCedis, ''3'' as TipoVenta, isnull(Facturada + '' ('' + T.Folio + '')'', ''N'') as Facturada, isnull(PFA.Fecha, '''') as ''F. Impresión'', 
	case T.CFVTipo 
		when 1 then ''Contado''
		when 2 then ''Crédito''
	end as ''Forma Venta'',
	...
	left outer join RouteADM.dbo.PedidosFacturados PFA on T.TransProdId = PFA.TransProdId 
	...
	GROUP BY t.folio, D.FechaCaptura, Clientes.IdCliente, Clientes.RazonSocial, t.ClienteClave, NombreSucursal, V.RutClave, t.transprodid, Facturada,PFA.Fecha,T.CFVTipo
