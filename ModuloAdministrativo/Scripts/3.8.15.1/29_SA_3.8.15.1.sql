...
/****** Object:  StoredProcedure [dbo].[up_VentasFactura]    Script Date: 07/22/2011 09:49:13 ******/
...
declare @UsuarioId as varchar(16)
SET @Lenguaje = Route.dbo.FNObtenerLenguaje()

set @IdCedisCFD = 1

select @VendedorId = VendedorID from ROUTE.dbo.FOSHist where FSHFechaInicio <= GETDATE() order by FSHFechaInicio desc
select @UsuarioId = USUId from ROUTE.dbo.Vendedor where VendedorID = @VendedorId
...
where SubE.TipoEstado = 1 and FosH.VendedorID = @VendedorId
...
begin tran 
	...
	begin
		...
		begin
			...
			update Route.dbo.FolioSolicitado set MUsuarioID = @UsuarioId, MFechaHora = GETDATE()
			from Route.dbo.SubEmpresa SubE 
			inner join Route.dbo.Folio Folio on SubE.SubEmpresaId = Folio.SubEmpresaId 
			inner join Route.dbo.FolioSolicitado FolS on Folio.FolioID = FolS.FolioID 
			inner join Route.dbo.FOSHist FosH on FosH.FolioID = FolS.FolioID and FosH.FOSId = FolS.FOSId 
...
begin
	...
	exec Route.dbo.SP_CancelarVenta @FolioN, @SerieN, @Fecha, @UsuarioId, 0 
