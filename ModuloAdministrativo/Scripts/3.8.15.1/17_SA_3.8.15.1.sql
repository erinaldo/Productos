...
/****** Object:  StoredProcedure [dbo].[sel_DatosCFD]    Script Date: 07/11/2011 13:08:40 ******/
SET ANSI_NULLS OFF
GO
...
ALTER PROCEDURE [dbo].[sel_DatosCFD]
@IdCedis as bigint,
@IdSurtido as bigint,
@IdTipoVenta as int,
@Folio as bigint,
...
begin
	...
	where SubE.TipoEstado = 1 and FosH.VendedorID = CAST(@IdCedis as varchar(10)) + REPLICATE('0', 4-len(@IdRuta)) + CAST(@IdRuta as varchar(4))
		...
		order by SemHist.SEMHistFechaInicio desc
end
...
if @Opc = 5
begin
	select DirRepMensual, DirDocXML, DirArchivosFacElec, ContrasenaClave, FolS.Aprobacion, FolS.AnioAprobacion
	from Route.dbo.SEMHist SemHist
	inner join Route.dbo.SubEmpresa SubE on SubE.SubEmpresaId = SemHist.SubEmpresaId 
	inner join Route.dbo.Folio Folio on SubE.SubEmpresaId = Folio.SubEmpresaId
	inner join Route.dbo.FolioSolicitado FolS on Folio.FolioID = FolS.FolioID 
	inner join Route.dbo.FOSHist FosH on FosH.FolioID = FolS.FolioID and FosH.FOSId = FolS.FOSId 
	where SubE.TipoEstado = 1 and FosH.VendedorID = 'CFDCed' + CAST(@IdCedis as varchar(10)) and SemHist.DirArchivosFacElec <> ''
	order by SemHist.SEMHistFechaInicio desc
end

if @Opc = 6
begin
	select TOP 1 SemHist.*, v.Descripcion as DescProveedor
	from Route.dbo.SEMHist SemHist
	inner join Route.dbo.VAVDescripcion v ON v.VARCodigo = 'PROTIMB' AND SemHist.ProveedorTimbre = v.VAVClave AND v.VADTipoLenguaje = Route.dbo.FNObtenerLenguaje()
	order by SemHist.SEMHistFechaInicio desc		
end
