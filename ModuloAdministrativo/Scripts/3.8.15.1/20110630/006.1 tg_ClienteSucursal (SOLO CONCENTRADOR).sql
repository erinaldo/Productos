...
-- inserta domicilio de entrega
	...
	update ROUTE.dbo.ClienteDomicilio 
		set Calle = left(I.Calle,64), Numero = left(I.NumExterior,16), NumInt = left(I.NumInterior,16), CodigoPostal = left(I.CP,16), 
		Colonia = left(I.Colonia,64), Localidad = left(I.Localidad,40), Poblacion = left(I.Poblacion,64), Entidad = left(I.Entidad,32), Pais = left(I.Pais,32), GLN = left(TDA_GLN,13), CRTienda = left(I.CRTienda,5), NombreTienda=left(I.NombreSucursal,30)
...
-- inserta domicilio fiscal
	insert into ROUTE.dbo.ClienteDomicilio ( ClienteClave, ClienteDomicilioID, Tipo, Calle, Numero, NumInt, CodigoPostal, ReferenciaDom, Colonia, Localidad, Poblacion, Entidad, Pais, GLN, CRTienda, NombreTienda, MFechaHora, MUsuarioID )
		select cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(5))+IdSucursal, 'D-01', 1, left( CalleF, 64), left(NumExteriorF, 16), left( NumInteriorF, 16), left( CPF, 16), '', left(ColoniaF, 64), left(LocalidadF, 40), left( PoblacionF, 64), left(EntidadF, 32), left(PaisF, 32), '', '', '', getdate(), 'Interfaz' from Inserted where cast(cast (case IdCedis when 1 then 'A' 
			when 2 then 'B' 
			when 3 then 'C' 
			when 4 then 'D' 
			when 5 then 'E'
	...
	update ROUTE.dbo.ClienteDomicilio 
		set Calle = left(I.CalleF,64), Numero = left(I.NumExteriorF,16), NumInt = left(I.NumInteriorF,16), CodigoPostal = left(I.CPF,16), 
		Colonia = left(I.ColoniaF,64), Localidad = left(I.LocalidadF,40), Poblacion = left(I.PoblacionF,64), Entidad = left(I.EntidadF,32), Pais = left(I.PaisF,32), GLN = '', CRTienda = '', NombreTienda=''
	from ROUTE.dbo.ClienteDomicilio as CD, Inserted as I
	where ClienteClave = cast (case IdCedis when 1 then 'A' 
			when 2 then 'B' 
			when 3 then 'C' 
