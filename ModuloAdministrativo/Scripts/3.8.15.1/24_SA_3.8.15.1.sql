...
/****** Object:  StoredProcedure [dbo].[sp_importVentasCPC]    Script Date: 07/19/2011 16:53:32 ******/
...
--set @Folio = 32109
--set @Serie = 'REM01'
--set @TransprodId = '885D7C6499E77184'
--set @TransprodIdFactura = 'A5FEDC3104F947AF'
...
--set @TipoNotaCredito = 0
	...
	DECLARE @IdSucursalFac AS VARCHAR(16)
	DECLARE @IdSucursalVta AS VARCHAR(16)	
	...
	DECLARE @ClienteClaveFac AS VARCHAR(16)
	DECLARE @ClienteClaveVta AS VARCHAR(16)
	...
	SELECT TOP 1 @CobrarVentas = CobrarVentas, @PagoAutomatico = PagoAutomatico, @MonedaID = MonedaID FROM CONHist ORDER BY MFechaHora DESC		
		
	SELECT @Serie = Serie, @Fecha = Fecha,  @SubTotal = Subtotal, @Iva = Iva, @IdSucursalFac = IdSucursal, @DctoImp = DctoImp, @DiasCredito = isnull(DiasVencimiento,0) 
	...
	where FolD.TipoContenido = 2 and MMD.TipoIndice = 25 and MMD.TipoTransProd = 8 and MMD.TipoEstado = 1 and MMD.Baja = 0
	...
	BEGIN
		...
		SET @ClienteClaveFac = CONVERT(VARCHAR(16),@LetraCte + @IdSucursalFac)
		...
		INSERT INTO [Route].[dbo].[Visita]
		 ...
		 VALUES(
				@VisitaClave,@DiaClave,@ClienteClaveFac,@VendedorId,@RUTClave
		...
		declare @SaldoFactura as float
		set @SaldoFactura = @SubTotal + @Iva
		
		--Cobranza (PagoAutomatico, CobrarFacturas y Venta a Contado)			
		IF (@CobrarVentas = 0)
		BEGIN
			IF (@PagoAutomatico = 1 AND @idTipoVenta = 1)
			BEGIN				
				...
				INSERT INTO Abono (ABNId, Folio, FechaCreacion, VisitaClave, DiaClave, FechaAbono, Total, Saldo, MFechaHora, MUsuarioID)
					VALUES(@ABNID, @FolioRoute, @FechaCobranza, @VisitaClave, @DiaClave, @FechaCobranza, @SaldoFactura, 0, GETDATE(), @USUID)
				
				INSERT INTO AbnTrp (ABNId, TransProdID, FechaHora, Importe, Serie, Corecibo, TipoFaseIntSal, MFechaHora, MUsuarioID)
					VALUES(@ABNID, @TransprodIdFactura, @FechaCobranza, @SaldoFactura, '','',1,GETDATE(), @USUID)
				
				INSERT INTO ABNDetalle (ABNId, ABDId, TipoPago, Importe, SaldoDeposito, MonedaId, Referencia, MFechaHora, MUsuarioID)
					VALUES(@ABNID, RIGHT(REPLACE(CONVERT(VARCHAR(36),NEWID()),'-',''),16), 1, @SaldoFactura, 0, @MonedaID, '', GETDATE(), @USUID)
					
				UPDATE TransProd SET Saldo = 0 WHERE TransProdID = @TransprodIdFactura 
			END
			ELSE
				UPDATE Cliente SET SaldoEfectivo = SaldoEfectivo + @SaldoFactura WHERE ClienteClave = @ClienteClaveFac  	
		END
		ELSE
			UPDATE TransProd SET Saldo = 0 WHERE TransProdID = @TransprodIdFactura
		...
		begin			
			select top 1 @IdSucursalFac = ClienteSucursal.IdSucursal, @RFC = RFC, @RazonSocial = RazonSocial, @Calle = isnull(CalleF,''), @NumExt = isnull(NumExteriorF,''), @NumInt = isnull(NumInteriorF,''), @Colonia = isnull(ColoniaF,''),
	...
	BEGIN
		...
		fetch next from PedidosFactura into @idcedis, @IdTipoVenta, @Serie, @Folio, @IdSucursalVta, @CedId, @Tipo
		while (@@fetch_status = 0)
		begin

			SET @TipoFase = 2
			SELECT @Serie = Serie, @Fecha = Fecha,  @SubTotal = Subtotal, @Iva = Iva, @IdSucursalVta = IdSucursal, @DctoImp = DctoImp, @DiasCredito = isnull(DiasVencimiento,0) 
			FROM RouteCPC.dbo.Ventas 
			WHERE IdCedis = @idCedis AND IdTipoVenta = @idTipoVenta AND Folio = @Folio and Serie = @Serie 
			...
			SET @FechaCobranza = DATEADD(DAY, @DiasCredito, @Fecha)	
				...
				SET @ClienteClaveVta = CONVERT(VARCHAR(16),@LetraCte + @IdSucursalVta)
				SELECT @PrestamoCliente = Prestamo FROM Cliente WHERE ClienteClave = @ClienteClaveVta 
				...
				BEGIN
					...
					INSERT INTO [Route].[dbo].[Visita]
						   ...
						   ,[MFechaHora],[MUsuarioID])
					 VALUES(
							@VisitaClave,@DiaClave,@ClienteClaveVta,@VendedorId,@RUTClave
						   ,1,@Fecha,@Fecha,1,0,0,0,NULL
						   ,GETDATE(),@USUID)
					...
					if exists(select Addenda.ADDID 
						...
						where AdCte.ClienteClave  = @ClienteClaveVta)
					begin
					...
					INSERT INTO [Route].[dbo].[TransProd]
						   ...
						   ,[TipoFaseIntSal],[MFechaHora],[MUsuarioID], [SubEmpresaID])
					VALUES(
						   @TransProdId,@VisitaClave,@DiaClave,@ModuloMovDetalleClave,@ClienteClaveVta,'1'
					...
					BEGIN
						...
						BEGIN
							...
							BEGIN
								IF (SELECT COUNT(*) FROM ProductoPrestamoCli WHERE ClienteClave = @ClienteClaveVta  AND ProductoClave = @ProductoEnvase) = 0
									INSERT INTO ProductoPrestamoCli (ClienteClave, ProductoClave, Saldo, MFechaHora, MUsuarioID) 
									VALUES	(@ClienteClaveVta , @ProductoEnvase, CASE WHEN @EsEnvase = 0 THEN @Cantidad ELSE @Cantidad * -1 END, GETDATE(), @USUID)
								ELSE
									UPDATE ProductoPrestamoCli SET Saldo = Saldo + CASE WHEN @EsEnvase = 0 THEN @Cantidad ELSE @Cantidad * -1 END, MFechaHora = GETDATE(), MUsuarioID = @USUID WHERE ClienteClave = @ClienteClaveVta AND ProductoClave = @ProductoEnvase 								
							END
					...
					BEGIN
						...
						END
						ELSE
							UPDATE Cliente SET SaldoEfectivo = SaldoEfectivo + @SaldoVenta WHERE ClienteClave = @ClienteClaveVta 	
					END
					ELSE
						UPDATE TransProd SET Saldo = 0 WHERE TransProdID = @TransProdID 
