...
/****** Object:  StoredProcedure [dbo].[up_FacturaOxxo]    Script Date: 07/21/2011 08:47:50 ******/
...
begin tran 
	...
	DECLARE @TransProdIDRoute varchar(16),@TransProdIdVFC varchar(16),@TransProdIdFacturaVFC varchar(16)
	
	select @TransProdIdVFC  = VFC.TransprodId , @TransProdIdFacturaVFC = VFC.TransprodIdFactura, @TransProdIDRoute = TRP.TransProdID
	from VentasFacturaCFD VFC
	left join Route.dbo.TransProd TRP on VFC.TransprodIdFactura = TRP.TransProdID
	where VFC.IdCedis = @IdCedis and VFC.IdTipoVenta = @IdTipoVenta and VFC.Serie = @Serie and VFC.Folio = @Folio 

	--declare @VendedorId as varchar(20)
	select @VendedorId = 'CFDCed' + CAST(@IdCedis as varchar(10))
	--Este if se hizo para el caso de cuando ya esta el documento con Generado y en route realmente no existe el TransProd
	if ((@TransProdIdVFC is not null) AND (@TransProdIdFacturaVFC is not null) AND (@TransProdIDRoute is null) )
	begin
	
			exec Route.dbo.sp_importVentasCPC @IdCedis, @IdTipoVenta, @Folio, @Serie, @TransProdIdVFC, @TransProdIdFacturaVFC, @VendedorId, 0 
	end
	else
	begin
