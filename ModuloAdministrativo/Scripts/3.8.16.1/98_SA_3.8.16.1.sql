...
/****** Object:  StoredProcedure [dbo].[Gsp_NivelReporte]    Script Date: 09/04/2011 11:55:31 ******/
...
begin
	...
	begin		
		SELECT DISTINCT 
		VEN.IdVendedor AS Número, TPVEN.TipoVendedor AS Tipo, VEN.Nombre + ' ' +  VEN.ApPaterno + ' ' + VEN.ApMaterno  AS [Nombre del Vendedor], 
		VEN.Nomina
		FROM  Vendedores VEN 
		INNER JOIN SurtidosVendedor SURVEN 
		INNER JOIN Surtidos SUR ON SURVEN.IdCedis = SUR.IdCedis AND SURVEN.IdSurtido = SUR.IdSurtido
		INNER JOIN TipoVendedor TPVEN ON SURVEN.IdTipoVendedor = TPVEN.IdTipoVendedor ON VEN.IdCedis = SURVEN.IdCedis AND 
		VEN.IdVendedor = SURVEN.IdVendedor
		INNER JOIN Rutas RUT ON RUT.IdRuta = SUR.IdRuta  and UPPER(RUT.TipoVenta) <> 'PREVENTA'
		WHERE (SUR.IdCedis = @IdCedi) AND (SUR.Fecha BETWEEN DATEADD(day,-DATEPART(dw, @FechaInicial)+1, @FechaInicial ) AND DATEADD(day,-DATEPART(dw, @FechaInicial )+6, @FechaInicial ))
		and SUR.Status = 'C'
		GROUP BY VEN.IdVendedor, TPVEN.TipoVendedor, VEN.ApPaterno, VEN.ApMaterno, VEN.Nombre, VEN.Nomina
		UNION 
		SELECT VEN.IdVendedor AS Número, TPVEN.TipoVendedor AS Tipo, VEN.Nombre + ' ' +  VEN.ApPaterno + ' ' + VEN.ApMaterno AS [Nombre del Vendedor], 
		VEN.Nomina from 
		Ventas VTA 
		...
		INNER JOIN Vendedores VEN ON SURVEN.IdVendedor = VEN.IdVendedor and VEN.IdCedis = SURVEN.IdCedis
		...
		and SUR.Status = 'C' and (SUR.IdCedis = @IdCedi)
		GROUP BY VEN.IdVendedor, TPVEN.TipoVendedor, VEN.ApPaterno, VEN.ApMaterno, VEN.Nombre, VEN.Nomina
		ORDER BY TPVEN.TipoVendedor DESC, [Nombre del Vendedor]
	end
	...
	begin	
		...
		exec( '	select * from ComisionesHistorico 
			where IdCedis =  ' + @IdCedi + ' ' + @Filtro + ' and FechaIni = ''' + @FechaIni + ''' and FechaFin = ''' + @FechaFin + ''' 
			order by FechaIni, Vendedor ' )		
	end
...
begin
	...
	begin			
		exec('
			SELECT     SUR.IdCedis, SUR.Fecha, SUR.IdSurtido, SUR.IdRuta, RUT.Ruta, SURDET.IdProducto AS ''Cve. Prod.'', PROD.Producto AS Producto, SURDET.Surtido AS Carga, 
								  (SURDET.VentaCredito + SURDET.VentaContado) AS Venta, SURDET.Consignacion, SURDET.Recuperacion, SURDET.DevBuena AS ''Dev. Buena'', 
			...
			GROUP BY SURDET.IdProducto, PROD.Producto, SURDET.Surtido, (SURDET.VentaCredito + SURDET.VentaContado), SURDET.Consignacion, SURDET.Recuperacion, SURDET.DevBuena, 
								  SURDET.DevMala, SURDET.Obsequios, SURDET.Precio, SUR.IdCedis, SUR.IdSurtido, SUR.Fecha, RUT.Ruta, SUR.IdRuta, SUR.Status
...
if @ReporteOrigen = 31 			-- Registro de Kilometraje
begin

	If @Tipo = 1 or @Tipo = 2  		-- a nivel de Fecha y Ruta
	begin
		SELECT     distinct SUR.IdRuta as Número, RUT.Ruta
		FROM         Surtidos SUR INNER JOIN
		                      Rutas RUT ON SUR.IdCedis = RUT.IdCedis AND SUR.IdRuta = RUT.IdRuta
	...
	if @Tipo = 10 or @Tipo = 11	-- Reporte a nivel Fecha y Ruta
	begin
		exec('
			SELECT  STRUT.IdCedis, STRUT.Fecha as IdAgrupa, STRUT.Fecha as DescripcionAgrupa, STRUT.Fecha as FechaRegistro, 
					STRUT.IdRuta as NumeroRuta, RUT.Ruta as NombreRuta, 
					CASE isnull(KM.KmInicial, 0) WHEN 0 THEN ''Kilometraje No Registrado'' ELSE '''' END AS Estatus, 
					KM.KmInicial, KM.KmFinal, KM.KmFinal - KM.KmInicial AS Recorridos, KM.Litros, 
                    KM.Importe, (KM.KmFinal - KM.KmInicial) / KM.Litros AS Rendimiento
			FROM	StatusRutas AS STRUT INNER JOIN
					Rutas AS RUT ON STRUT.IdCedis = RUT.IdCedis AND STRUT.IdRuta = RUT.IdRuta LEFT OUTER JOIN
                    Kilometraje AS KM ON STRUT.IdCedis = KM.IdCedis AND STRUT.Fecha = KM.Fecha AND STRUT.IdRuta = KM.IdRuta
			WHERE   (STRUT.IdCedis = ' + @IdCedi + ') 
					AND (STRUT.Fecha BETWEEN ''' + @FechaInicial + ''' AND ''' + @FechaFinal +''' ' + @Filtro + ')
			order by STRUT.IdCedis, STRUT.Fecha, STRUT.IdRuta
		')
	end
	...
	begin
		exec('
			SELECT  STRUT.IdCedis, STRUT.Fecha as FechaRegistro, STRUT.IdRuta as IdAgrupa, STRUT.IdRuta as NumeroRuta, 
					RUT.Ruta as DescripcionAgrupa, RUT.Ruta as NombreRuta, 
					CASE isnull(KM.KmInicial, 0) WHEN 0 THEN ''Kilometraje No Registrado'' ELSE '''' END AS Estatus, 
					KM.KmInicial, KM.KmFinal, KM.KmFinal - KM.KmInicial AS Recorridos, KM.Litros, 
                    KM.Importe, (KM.KmFinal - KM.KmInicial) / KM.Litros AS Rendimiento
			FROM	StatusRutas AS STRUT INNER JOIN
			...
			WHERE   (STRUT.IdCedis = ' + @IdCedi + ') 
					AND (STRUT.Fecha BETWEEN ''' + @FechaInicial + ''' AND ''' + @FechaFinal +''' ' + @Filtro + ')
			order by STRUT.IdCedis, STRUT.IdRuta, STRUT.Fecha
		')
	end
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 98, (SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 98 where  Tipo = 'SA' and Version ='3.8.16.1'
END
