...
/****** Object:  StoredProcedure [dbo].[sel_PreventaRoute]    Script Date: 07/29/2011 08:53:37 ******/
...
begin
	...
	replace(TRP.FOLIO,''-'','''')+''-''+isnull(cast(SUR.IdRuta as varchar),'''')+''-''+isnull(left(cast(SUR.Fecha as varchar),11),'''') as ''Folio-Ruta-FechaSurtido'', 
	VAD.Descripcion as ''Forma de Venta'', TRP.ClienteClave, ClienteSucursal.NombreSucursal, 
	...
	left join Ventas VEN on cast(replace(TRP.folio,''-'','''') as bigint)=VEN.Folio
	left join  Surtidos SUR on VEN.IdSurtido=SUR.IdSurtido
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 28, (SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 28 where  Tipo = 'SA' and Version ='3.8.16.1'
END
GO