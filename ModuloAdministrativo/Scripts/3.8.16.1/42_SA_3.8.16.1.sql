...
/****** Object:  StoredProcedure [dbo].[up_Surtidos]    Script Date: 08/01/2011 17:40:18 ******/
...
if @Opc = 1  
begin
	set @IdSurtido = isnull ( (
	select max(IdSurtido) from (
	select IdSurtido from Surtidos where IdCedis = @IdCedis
	union
	select IdSurtido from SurtidosCancelados where IdCedis = @IdCedis
	)as t
	)+1, 1)	
...
begin
	...
	exec Route.dbo.sp_importVentasADM @IdCedis, @IdSurtido
	...
	insert into SurtidosCancelados 
	select IdCedis, IdSurtido, Fecha, IdRuta, GETDATE() 
	from Surtidos  
	where IdCedis = @IdCedis and IdSurtido = @IdSurtido 
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 42, (SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 42 where  Tipo = 'SA' and Version ='3.8.16.1'
END
GO