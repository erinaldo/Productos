...
/****** Object:  StoredProcedure [dbo].[sel_StatusRutas]    Script Date: 07/25/2011 14:17:17 ******/
...
Begin
	...
	LEFT OUTER JOIN RutasNoLiquidacion AS RUTNLIQ ON RUT.IdCedis = RUTNLIQ.IdCedis AND RUT.IdRuta = RUTNLIQ.IdRuta AND RUTNLIQ.Fecha = @Fecha 
	LEFT OUTER JOIN TipoNoLiquidacion AS TPNLIQ ON RUTNLIQ.IdTipoNoLiquidacion = TPNLIQ.IdTipoNoLiquidacion 
	WHERE RUT.IdCedis = @IdCedis AND RUT.Status = 'A' and SUR.IdSurtido IS NULL and RUTNLIQ.IdTipoNoLiquidacion IS NULL AND RUT.TipoVenta IN ('Venta', 'Reparto')
...
--Todas las Rutas excepto las de TipoVenta preventa
if @Opc = 6 
Begin
	select StatusRutas.IdCedis, StatusRutas.Fecha, StatusRutas.IdRuta as 'No.', Rutas.Ruta as 'Ruta', StatusRutas.Status as 'Estatus', StatusDesc as 'Descripción'
	from StatusRutas
	inner join Rutas on Rutas.IdRuta = StatusRutas.IdRuta and Rutas.IdCedis = @IdCedis
	where StatusRutas.IdCedis = @IdCedis and StatusRutas.Fecha = @Fecha and UPPER(Rutas.TipoVenta) <> 'PREVENTA' 
End
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 8, (SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 8 where  Tipo = 'SA' and Version ='3.8.16.1'
END
GO
