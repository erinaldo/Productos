...
/****** Object:  StoredProcedure [dbo].[up_Ventas]    Script Date: 08/02/2011 09:50:07 ******/
...
--set @Folio = 1
--set @Serie = 'REM01'
--set @Fecha = '07/28/2011'
--set @IdCliente = 1
--set @IdSucursal = '010010103'
--set @Opc = 1
...
begin
	...
	insert into Ventas values (@IdCedis, @IdSurtido, @IdTipoVenta, @Folio, @Serie, @Fecha, @IdCliente, 0, 0, @IdSucursal, 0, @DiasCredito, null)
...
begin
	...
	select @IdCedis, @IdSurtido, @IdTipoVenta, @Folio, IdProducto, @Serie, isnull( sum(Surtido - DevBuena - DevMala - Obsequios - VentaContado - VentaCredito - Consignacion + Recuperacion) , 0),  0, 
	0, 0, 0, isnull( sum(Surtido - DevBuena - DevMala - Obsequios - VentaContado - VentaCredito - Consignacion + Recuperacion) , 0)
...
/****** Object:  StoredProcedure [dbo].[sel_Precio]    Script Date: 07/27/2011 08:18:00 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sel_Precio]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sel_Precio]
GO
...
CREATE PROCEDURE [dbo].[sel_Precio]
@IdLista as int,
...
BEGIN
    ...
    SELECT Precio FROM PreciosDetalle WHERE IdLista = @IdLista AND IdProducto = @IdProducto
END
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 45, (SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 45 where  Tipo = 'SA' and Version ='3.8.16.1'
END
GO