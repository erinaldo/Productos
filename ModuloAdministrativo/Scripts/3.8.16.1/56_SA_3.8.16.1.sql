...
/****** Object:  StoredProcedure [dbo].[sel_StatusRutas]    Script Date: 08/15/2011 10:26:46 ******/
...
Begin
	...
	WHERE RUT.IdCedis = @IdCedis AND RUT.Status = 'A' and SUR.IdSurtido IS NULL AND RUT.TipoVenta IN ('Venta', 'Reparto','Preventa')
...
-- BELTICOS: Se agrega la opción para obtener las rutas de preventa que tienen un surtido en estado pendiente 
if @Opc =6
Begin
	SELECT	RUT.IdCedis, RUT.IdRuta, SUR.IdSurtido,
	(Select Top 1 IdVendedor from SurtidosVendedor SURVEN where SURVEN.IdCedis = 1 and IdSurtido = SUR.IdSurtido) as IdVendedor  
	FROM    Rutas AS RUT
	LEFT OUTER JOIN Surtidos AS SUR ON SUR.IdCedis = RUT.IdCedis AND SUR.IdRuta = RUT.IdRuta AND SUR.Fecha = @Fecha
	LEFT OUTER JOIN RutasNoLiquidacion AS RUTNLIQ ON RUT.IdCedis = RUTNLIQ.IdCedis AND RUT.IdRuta = RUTNLIQ.IdRuta AND RUTNLIQ.Fecha = @Fecha 
	LEFT OUTER JOIN TipoNoLiquidacion AS TPNLIQ ON RUTNLIQ.IdTipoNoLiquidacion = TPNLIQ.IdTipoNoLiquidacion 
	WHERE RUT.IdCedis = @IdCedis AND RUT.Status = 'A' and SUR.Status = 'P' and RUTNLIQ.IdTipoNoLiquidacion IS NULL AND RUT.TipoVenta IN ('Preventa')
End
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 56,(SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 56 where  Tipo = 'SA' and Version ='3.8.16.1'
END
GO