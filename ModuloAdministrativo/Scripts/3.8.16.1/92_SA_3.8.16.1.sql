...
/****** Object:  StoredProcedure [dbo].[sel_ComisionesCalculo]    Script Date: 08/30/2011 22:18:10 ******/
...
CASE WHEN COM.Acumulado = 0 THEN null ELSE (CASE WHEN COM.IdConceptoPago = 1 THEN sum(Cantidad) WHEN COM.IdConceptoPago = 2 THEN FLOOR(SUM(Cantidad/ isnull(ProductosUnidad.Factor,0 ))) WHEN COM.IdConceptoPago = 3 THEN sum(Total) ELSE 0 END / (CASE WHEN COM.Acumulado = 0 THEN 1 ELSE @DiasTrabajados END)) END as PromSemanal,
Inicial, Final, Pago as Factor, 
CASE WHEN COM.IdConceptoPago = 1 THEN sum(Cantidad) WHEN COM.IdConceptoPago = 2 THEN FLOOR(SUM(Cantidad/ isnull(ProductosUnidad.Factor,1 ))) WHEN COM.IdConceptoPago = 3 THEN sum(Total) ELSE 0 END * Pago as 'Pago'
from Surtidos
...
having CASE WHEN COM.IdConceptoPago = 1 THEN sum(Cantidad) WHEN COM.IdConceptoPago = 2 THEN FLOOR(SUM(Cantidad/ isnull(ProductosUnidad.Factor,0 ))) WHEN COM.IdConceptoPago = 3 THEN sum(Total) ELSE 0 END / (CASE WHEN COM.Acumulado = 0 THEN 1 ELSE @DiasTrabajados END) between Inicial and Final  
UNION
--Comisiones x Marca
select Surtidos.IdCedis, Vendedores.IdVendedor , Vendedores.Nombre as Vendedor, @FechaInicial as FechaIni, @FechaFinal as FechaFin, 
COM.IdComEsquema, COM.Esquema, COM.IdConceptoPago,ConceptoPago, 
...
@DiasTrabajados  as DiasTrabajados, 
CASE WHEN COM.Acumulado = 0 THEN null ELSE (CASE WHEN COM.IdConceptoPago = 1 THEN sum(Cantidad) WHEN COM.IdConceptoPago = 2 THEN FLOOR(SUM(Cantidad/ isnull(ProductosUnidad.Factor,0 ))) WHEN COM.IdConceptoPago = 3 THEN sum(VentasDetalle.Total) ELSE 0 END / (CASE WHEN COM.Acumulado = 0 THEN 1 ELSE @DiasTrabajados END)) END as PromSemanal,
Inicial, Final, Pago as Factor, 
CASE WHEN COM.IdConceptoPago = 1 THEN sum(Cantidad) WHEN COM.IdConceptoPago = 2 THEN FLOOR(SUM(Cantidad/ isnull(ProductosUnidad.Factor,1 ))) WHEN COM.IdConceptoPago = 3 THEN sum(VentasDetalle.Total) ELSE 0 END * Pago as 'Pago'
from Ventas  
...
Surtidos.IdCedis, SurtidosVendedor.IdTipoVendedor, COM.TipoVendedor, 
VentasDetalle.IdProducto, Producto, COM.Acumulado, Vendedores.IdVendedor , Vendedores.Nombre
having CASE WHEN COM.IdConceptoPago = 1 THEN sum(Cantidad) WHEN COM.IdConceptoPago = 2 THEN FLOOR(SUM(Cantidad/ isnull(ProductosUnidad.Factor,0 ))) WHEN COM.IdConceptoPago = 3 THEN sum(VentasDetalle.Total) ELSE 0 END / (CASE WHEN COM.Acumulado = 0 THEN 1 ELSE @DiasTrabajados END) between Inicial and Final  
UNION
--Comisiones x Marca
select Surtidos.IdCedis, Vendedores.IdVendedor , Vendedores.Nombre as Vendedor, @FechaInicial as FechaIni, @FechaFinal as FechaFin, 
COM.IdComEsquema, COM.Esquema, COM.IdConceptoPago,ConceptoPago, 
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 92, (SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 92 where  Tipo = 'SA' and Version ='3.8.16.1'
END
