...
/****** Object:  StoredProcedure [dbo].[up_Ventas]    Script Date: 08/17/2011 22:56:02 ******/
...
begin
	...
	set @Folio = ( select isnull( max(Folio)+1, 1) from (
		Select Folio from Ventas where Ventas.IdCedis = @IdCedis and Serie in (@Serie, @SerieD) 
		UNION 
		Select Folio from VentasCanceladas where VentasCanceladas.IdCedis = @IdCedis and VentasCanceladas.Serie in (@Serie, @SerieD) 
		) as Folios
		)

	insert into Ventas (IdCedis,IdSurtido,IdTipoVenta,Folio,Serie,Fecha,IdCliente,Subtotal,Iva,IdSucursal,DctoImp,DiasCredito)
	values (@IdCedis, @IdSurtido, @IdTipoVenta, @Folio, @Serie, @Fecha, @IdCliente, 0, 0, @IdSucursal, 0, @DiasCredito)
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 62, (SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 62 where  Tipo = 'SA' and Version ='3.8.16.1'
END
GO
