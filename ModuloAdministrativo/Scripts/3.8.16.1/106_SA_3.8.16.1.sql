...
/****** Object:  StoredProcedure [dbo].[sel_ArchivoTextoConsulta]    Script Date: 10/28/2011 16:48:42 ******/
...
begin
	...
	FROM    (RutasNoLiquidacion AS RUTNLIQ RIGHT OUTER JOIN            
			...
			inner join CedisVirtuales V on RUT.TipoVenta = V.TipoRuta
	WHERE   (cast(V.IdCedisVirtual as varchar(10)) = @IdCedisVirtual) AND (STRUT.Fecha = @Fecha) AND (STRUT.Status = 'N')
end
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 106, (SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 106 where  Tipo = 'SA' and Version ='3.8.16.1'
END
GO