...
/****** Object:  StoredProcedure [dbo].[sp_tg_ExportManagement]    Script Date: 11/14/2011 17:29:58 ******/
...
BEGIN
	...
	--SET @FechaIN = '9999-12-31 00:00:00.000'
	--SET @FechaFIN = '9999-12-31 00:00:00.000'
	--SET @Vende = '10101'
	...
	BEGIN
		...
		SELECT * INTO #TmpRecuperacion1 FROM (	
		...
		AND TRP.TransProdID NOT IN (SELECT TransProdID FROM RouteADM.dbo.RecuperacionTransProd)
		AND TRP.TransProdId IN (SELECT TransProdID FROM RouteADM.dbo.ConsignasTransProd)
		)AS T

		SELECT * INTO #TmpRecuperacion FROM (	
		SELECT REC.TransProdId, ABN.DiaClave, Dia.FechaCaptura   
		FROM (
		SELECT TRP.TransProdId, (SELECT TOP 1 ABT.ABNId FROM AbnTrp ABT INNER JOIN Abono ABN ON ABT.ABNId = ABN.ABNId 
		INNER JOIN Dia ON ABN.DiaClave = Dia.DiaClave 
		WHERE TRP.TransProdId = ABT.TransProdID 
		ORDER BY Dia.FechaCaptura DESC) AS ABNId
		FROM #TmpRecuperacion1 TRP 
		) AS REC
		INNER JOIN Abono ABN ON REC.ABNId = ABN.ABNId 
		INNER JOIN Dia ON ABN.DiaClave = DIA.DiaClave 
		INNER JOIN Visita VIS ON ABN.VisitaClave = VIS.VisitaClave AND ABN.DiaClave = VIS.DiaClave 
		INNER JOIN Vendedor VEN ON VEN.VendedorID = VIS.VendedorID 
		INNER JOIN Usuario USU ON USU.USUId = VEN.USUId
		WHERE USU.Clave = @Vende
		)AS T
		
		DROP TABLE #TmpRecuperacion1
				
		--Obtiene los diAS a ExpORtar
		DECLARE @CursorVar1 CURSOR  
		SET @CursorVar1 = CURSOR SCROLL DYNAMIC FOR 
			SELECT DISTINCT REC.DiaClave, REC.FechaCaptura
			FROM #TmpRecuperacion REC 
			UNION
			SELECT DISTINCT D.DiaClave, D.FechaCaptura
			FROM TransProd TP
			INNER JOIN Dia D ON D.DiaClave = TP.DiaClave AND TP.DiaClave1 IS NULL
			...
			AND  (U.clave = @Vende) AND TP.TransProdId NOT IN (SELECT TransProdId FROM #TmpRecuperacion)
			UNION 		
			...
			AND ((TRP.MFechaHora BETWEEN @FechaIN AND @FechaFIN) OR (@FechaIN = @FechaIniCompara AND TRP.TipoFaseIntSal IN (0,2)))
			AND (USU.Clave = @Vende) AND TRP.TransProdId NOT IN (SELECT TransProdId FROM #TmpRecuperacion)
		...
		BEGIN 
			...
			BEGIN
				...
				SELECT TOP 1 @ClaveCEDI = ALMClave FROM(
					SELECT DISTINCT   ALM.Clave AS ALMClave
					FROM Visita VIS 
					INNER JOIN Vendedor VEN ON VIS.VendedorID = VEN.VendedorID 
				...
				SELECT TOP 1 @Surtido=CAR.IdSurtido, @IdCarga = CAR.IdCarga
				FROM [RouteADM].dbo.Cargas CAR
				INNER JOIN RouteADM.dbo.Surtidos SUR ON CAR.IdCedis = SUR.IdCedis AND CAR.IdSurtido = SUR.IdSurtido AND CAR.IdRuta = SUR.IdRuta AND CAR.Fecha = SUR.Fecha   
				WHERE  CAR.IdCedis = CAST(@ClaveCEDI AS INT) AND CAR. idRuta= @idRuta AND CAR.Fecha=@Dia AND SUR.Status = 'P'
				...
				BEGIN
					...
					--Reparto
						...
						INNER JOIN Usuario USU ON USU.USUId = VEN.USUId
						WHERE TRP.Tipo = 1 AND TRP.TipoFase = 2 AND TRP.CFVTipo = 2 and TRP.Saldo <> 0 
						AND TRP.TransProdID NOT IN (SELECT TransProdID FROM RouteADM.dbo.ConsignasTransProd)
						AND ((TRP.FechaHoraAlta BETWEEN @FechaIN AND @FechaFIN) OR (@FechaIN = @FechaIniCompara AND TRP.TipoFaseIntSal in (0,2))) 
						AND USU.Clave = @Vende 
						UNION 
						--Venta Directa
						SELECT TRP.TransProdID 
						...
						INNER JOIN Usuario USU ON USU.USUId = VEN.USUId
						WHERE TRP.Tipo = 1 AND TRP.TipoFase = 2 AND TRP.CFVTipo = 2 and TRP.Saldo <> 0						
						AND ((TRP.FechaHoraAlta BETWEEN @FechaIN AND @FechaFIN) OR (@FechaIN = @FechaIniCompara AND TRP.TipoFaseIntSal = 0)) 
						AND USU.Clave = @Vende
					) AS t
														
					DECLARE @crsConsignacion CURSOR  
					SET @crsConsignacion = CURSOR SCROLL DYNAMIC FOR 
					SELECT CAST(ProductoClave AS INT) AS ProductoClave, SUM(cantidad)AS Cantidad
					FROM (
					...
					OPEN  @crsConsignacion
					FETCH NEXT FROM @crsConsignacion INTO @ProductoClave, @Cantidad
					WHILE @@FETCH_STATUS = 0      
					BEGIN 
					
						IF(SELECT COUNT(*) FROM [RouteADM].dbo.SurtidosDetalle WHERE IdCedis = @ClaveCEDI AND IdSurtido = @Surtido AND IdProducto = @ProductoClave)=0
						BEGIN
							INSERT INTO [RouteADM].[dbo].[SurtidosDetalle]([IdCedis],[IdSurtido],[IdProducto],[Fecha],[Surtido],[DevBuena],[DevMala],[Obsequios],[VentaContado],[VentaCredito],[Consignacion],[Recuperacion],[Precio],[Iva])
							VALUES(@ClaveCEDI,@Surtido,CAST(@ProductoClave AS INT),@Dia,0,0,0,0,0,0,@Cantidad,0,0,0)
						END
						ELSE
						BEGIN
							UPDATE [RouteADM].dbo.SurtidosDetalle SET Consignacion = Consignacion + @Cantidad 
							WHERE IdCedis = @ClaveCEDI AND IdSurtido = @Surtido AND IdProducto = @ProductoClave
						END
					...
					CLOSE @crsConsignacion
					DEALLOCATE @crsConsignacion
					
					INSERT INTO RouteADM.dbo.ConsignasTransProd
					SELECT TransProdID, GETDATE() FROM #TmpConsignacion
					...
					DECLARE @crsRecuperacion CURSOR  
					SET @crsRecuperacion = CURSOR SCROLL DYNAMIC FOR 
					SELECT CAST(ProductoClave AS INT) AS ProductoClave, SUM(Cantidad) AS Cantidad
					FROM (
						SELECT TPD.ProductoClave, TPD.Cantidad 
						FROM TransProdDetalle TPD 
						WHERE TPD.TransProdID IN(SELECT TransProdID FROM #TmpRecuperacion WHERE DiaClave = @DiaClave)	
						AND (TPD.Promocion IS NULL OR TPD.Promocion <> 2)						
					) AS T
					GROUP BY ProductoClave
					
					OPEN  @crsRecuperacion
					FETCH NEXT FROM @crsRecuperacion INTO @ProductoClave, @Cantidad
					WHILE @@FETCH_STATUS = 0      
					BEGIN 					
						IF(SELECT COUNT(*) FROM [RouteADM].dbo.SurtidosDetalle WHERE IdCedis = @ClaveCEDI AND IdSurtido = @Surtido AND IdProducto = @ProductoClave)=0
						BEGIN
							INSERT INTO [RouteADM].[dbo].[SurtidosDetalle]([IdCedis],[IdSurtido],[IdProducto],[Fecha],[Surtido],[DevBuena],[DevMala],[Obsequios],[VentaContado],[VentaCredito],[Consignacion],[Recuperacion],[Precio],[Iva])
							VALUES(@ClaveCEDI,@Surtido,CAST(@ProductoClave AS INT),@Dia,0,0,0,0,0,0,0,@Cantidad,0,0)
						END
						ELSE
						BEGIN
							UPDATE [RouteADM].dbo.SurtidosDetalle SET Recuperacion = Recuperacion + @Cantidad 
							WHERE IdCedis = @ClaveCEDI AND IdSurtido = @Surtido AND IdProducto = @ProductoClave
						END
					...
					CLOSE @crsRecuperacion
					DEALLOCATE @crsRecuperacion
			
					INSERT INTO RouteADM.dbo.RecuperacionTransProd
					SELECT TransProdID, GETDATE() FROM #TmpRecuperacion	WHERE DiaClave = @DiaClave				
					...
					SET @CursorVar2 = CURSOR SCROLL STATIC FOR 
						--Contado Reparto
						SELECT TP.Subtotal, TP.Impuesto, TP.DESCuentoVendedor + TP.DescuentoImp AS DESCuento, V.ClienteClave, 
						TP.TransProdID, dbo.FNObtenerSolONumeros(TP.Folio) AS folio, TP.DiasCredito,
						...
						AND  (Us.Clave = @Vende) AND TP.Facturaid IS NULL
						UNION
						--Creditos saldados (Recuperacion)					
						SELECT DISTINCT TRP.Subtotal, TRP.Impuesto, TRP.DescuentoVendedor + TRP.DescuentoImp as Descuento, VPrev.ClienteClave, 
						TRP.TransProdID, dbo.FNObtenerSolONumeros(TRP.Folio) AS Folio, TRP.DiasCredito,
					    ...
					    FROM TransProd TRP
					    INNER JOIN #TmpRecuperacion TMP ON TRP.TransProdId = TMP.TransProdId
					    INNER JOIN Visita VPrev ON TRP.VisitaClave = VPrev.VisitaClave AND TRP.DiaClave = VPrev.DiaClave
						INNER JOIN Dia DPrev ON VPrev.DiaClave = DPrev.DiaClave
						WHERE TMP.DiaClave = @DiaClave
					    UNION									
						--Creditos saldados en la fecha que se surtieron
						...
						WHERE TRP.Tipo = 1 AND TRP.TipoFase = 2 AND TRP.CFVTipo = 2 AND TRP.Saldo = 0 
						AND ((TRP.MFechaHora BETWEEN @FechaIN AND @FechaFIN) OR (@FechaIN = @FechaIniCompara AND TRP.TipoFaseIntSal in (0,2)))
						AND USU.Clave = @Vende AND TRP.TransProdID NOT IN (SELECT TransProdID FROM RouteADM.dbo.ConsignasTransProd) 				
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 114, (SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 114 where  Tipo = 'SA' and Version ='3.8.16.1'
END