...
/****** Object:  StoredProcedure [dbo].[sel_SurtidosDetalle]    Script Date: 09/07/2011 15:47:56 ******/
...
begin
        ...
        (VentaCredito + VentaContado) as 'Venta', Consignacion as 'Consignación', Recuperacion as 'Cob. Consig.',
        DevBuena as 'Dev. Buena', DevMala as 'Dev. Mala', Obsequios as 'Obsequios', Surtido - VentaCredito - VentaContado - DevBuena - DevMala - Obsequios - Consignacion + Recuperacion as 'Inv. Final', 
        ...
        select Rejas.IdCedis, Rejas.IdProducto as 'Cve. Producto', Producto as 'Producto', Surtido as 'Carga', 0 as 'Venta', 0 as 'Consignación', 0 as 'Cob. Consig.',
        ...
        where IdCedis = @IdCedis and IdSurtido = @IdSurtido --and ( Surtido > 0 or Devolucion > 0 )
...
begin
        ...
        if @IdFamiliaRejas <> 0  -- producto reja
                select isnull(Rejas.IdCedis, @IdCedis) as IdCedis, Productos.IdProducto as 'Cve. Producto', Producto as 'Producto', isnull(Surtido,0) as 'Carga', 
                0 as 'Venta', 0 as 'Consignación', 0 as 'Cob. Consig.',
                isnull(Devolucion,0) as 'Dev. Buena', 0 as 'Dev. Mala', 0 as 'Obsequios', isnull(Surtido,0) - isnull(Devolucion,0) as 'Inv. Final', 
				...
				case Divide 
					when 'N' then cast(isnull(Rejas.Devolucion,0) * ISNULL(Factor,0) as bigint) 
					when 'S' then cast(isnull(Rejas.Devolucion,0) / isnull(Factor,1) as bigint) else 0 end as UnidadC,
				case Divide 
					when 'N' then (isnull(Rejas.Devolucion,0) * ISNULL(Factor,0)) - cast(isnull(Rejas.Devolucion,0) * ISNULL(Factor,0) as bigint) 
					when 'S' then (isnull(Rejas.Devolucion,0) / isnull(Factor,1) - cast(isnull(Rejas.Devolucion,0) / isnull(Factor,1) as bigint)) * isnull(Factor,1) else isnull(Rejas.Devolucion,0) end as PiezasC
                from Productos 
                left outer join Rejas on Rejas.IdProducto = Productos.IdProducto and Productos.Status = 'A' and IdCedis = @IdCedis and IdSurtido = @IdSurtido --and ( Surtido > 0 or Devolucion > 0 )
                left outer join ProductosUnidad on ProductosUnidad.IdProducto = Productos.IdProducto 
                where Productos.IdProducto = @IdProducto
                order by cast(Productos.IdProducto as bigint)

        else -- producto no reja
                select SurtidosDetalle.IdCedis, SurtidosDetalle.IdProducto as 'Cve. Producto', Producto as 'Producto', Surtido as 'Carga', 
                (VentaCredito + VentaContado) as 'Venta', Consignacion as 'Consignación', Recuperacion as 'Cob. Consig.',
                DevBuena as 'Dev. Buena', DevMala as 'Dev. Mala', Obsequios as 'Obsequios', 
                Surtido - VentaCredito - VentaContado - DevBuena - DevMala - Obsequios - Consignacion + Recuperacion as 'Inv. Final', 
                ...
                from SurtidosDetalle 
                inner join Productos on SurtidosDetalle.IdProducto = Productos.IdProducto and Productos.Status = 'A'
                left outer join ProductosUnidad on ProductosUnidad.IdProducto = Productos.IdProducto 
                where IdCedis = @IdCedis and IdSurtido = @IdSurtido and SurtidosDetalle.IdProducto = @IdProducto --and Surtido > 0 
                order by cast(SurtidosDetalle.IdProducto as bigint)
end
...
begin
        ...
        SELECT     SURDET.IdCedis, SURDET.IdProducto AS 'Cve. Prod.', PROD.Producto AS Producto, SURDET.Surtido AS Carga, 
                              (SURDET.VentaCredito + SURDET.VentaContado) AS Venta, SURDET.Consignacion as 'Consignacion', 
                              SURDET.Recuperacion as 'Cob. Consig.', SURDET.DevBuena AS 'Dev. Buena', SURDET.DevMala AS 'Dev. Mala', 
                              SURDET.Obsequios AS Obsequios, case sum(VENDET.Cantidad) when 0 then SURDET.Precio else 
                              isnull(sum(VENDET.Subtotal)/sum(VENDET.Cantidad), 0) end  AS Precio, ISNULL(SUM(VENDET.Subtotal), 0) AS SubTotal, 
        ...
        WHERE     (SURDET.IdCedis = @IdCedis) and (SURDET.idsurtido = @IdSurtido) -- AND (SURDET.Surtido > 0) 
        GROUP BY SURDET.IdCedis, SURDET.IdProducto, PROD.Producto, SURDET.Surtido, (SURDET.VentaCredito + SURDET.VentaContado), 
                              SURDET.Consignacion, SURDET.Recuperacion, SURDET.DevBuena, SURDET.DevMala, SURDET.Obsequios, SURDET.Precio
        -- ORDER BY CAST(SURDET.IdProducto AS bigint)

        union

        SELECT     SURDET.IdCedis, SURDET.IdProducto AS 'Cve. Prod.', PROD.Producto AS Producto, SURDET.Surtido AS Carga, 
                              0 AS Venta, 0 as 'Consignacion', 0 as 'Cob. Consig.', SURDET.Devolucion AS 'Dev. Buena', 0 AS 'Dev. Mala', 
...
if @Opc = 5
begin
		declare @Ruta as varchar(10)
		select @Ruta = IdRuta from Surtidos where IdSurtido = @IdSurtido and IdCedis = @IdCedis
		
        select Tipo, isnull( sum(Subtotal),0) as 'Subtotal', isnull( sum(Cantidad * Precio * Iva),0) as 'Iva', isnull( sum(Total),0) as 'Total', 0 as 'Cob. Consig.'
        from VentasTipo 
        inner join VentasDetalle on IdCedis = @IdCedis and IdSurtido = @IdSurtido and VentasDetalle.IdTipoVenta = VentasTipo.IdTipoVenta
        group by Tipo
        union all
        select 3, 0 as 'Subtotal', 0 as 'Iva', 0 as 'Total',
        (select sum(Total) from (
			select Total
			from Route.dbo.TransProd TRP
			inner join Route.dbo.Visita VIS on VIS.VisitaClave = TRP.VisitaClave1 and VIS.DiaClave = TRP.DiaClave1 
			where TransProdID in (select TransProdId from ConsignasTransProd where convert(datetime, convert(varchar(20), Fecha, 103), 103) = @Fecha)
			and Tipo = 1 and TipoFase = 2 and Saldo <> 0 and VIS.RUTClave = cast(@IdCedis as varchar(10)) + replicate('0', 4 - len(@Ruta)) + cast(@Ruta as varchar(10))
			union all
			select Total
			from Route.dbo.TransProd TRP
			inner join Route.dbo.Visita VIS on VIS.VisitaClave = TRP.VisitaClave and VIS.DiaClave = TRP.DiaClave and TRP.VisitaClave1 is null
			where TransProdID in (select TransProdId from ConsignasTransProd where convert(datetime, convert(varchar(20), Fecha, 103), 103) = @Fecha)
			and Tipo = 1 and TipoFase = 2 and Saldo <> 0 and VIS.RUTClave = cast(@IdCedis as varchar(10)) + replicate('0', 4 - len(@Ruta)) + cast(@Ruta as varchar(10))
		)as t) as 'Cob. Consig.'
...
if @Opc = 7
	select isnull(sum(Surtido), 0) as Surtido, isnull(sum(devbuena), 0) as DevBuena, isnull(sum(devmala), 0) as DevMala, isnull(sum(obsequios), 0) as Obsequios, 
	isnull(sum(ventacontado + ventacredito), 0) as Venta, ISNULL(sum(consignacion),0) as Consignacion, ISNULL(sum(recuperacion),0) as 'Cob. Consig.',
	isnull(sum(surtido - devbuena - devmala - obsequios - ventacontado - ventacredito - Consignacion + Recuperacion), 0) as InvFinal, 
	isnull(sum(Entrada), 0) as Entrada, isnull(sum(Salida), 0) as Salida
	from SurtidosDetalle
...
if @Opc = 8
        ...
        WHERE     (SURDET.IdCedis = @IdCedis) and (SURDET.idsurtido = @IdSurtido) AND SURDET.VentaContado + SURDET.VentaCredito  > 0 -- AND (SURDET.Surtido > 0) 
        GROUP BY SURDET.IdCedis, SURDET.IdProducto, PROD.Producto, SURDET.Surtido, SURDET.VentaCredito, SURDET.VentaContado, 
...
begin
        ...
        WHERE     (SURDET.IdCedis = @IdCedis) and (SURDET.idsurtido = @IdSurtido) -- AND (SURDET.Surtido > 0) 
        GROUP BY SURDET.IdCedis, SURDET.IdProducto, PROD.Producto, SURDET.Surtido, (SURDET.VentaCredito + SURDET.VentaContado), 
                              SURDET.Consignacion, SURDET.Recuperacion, SURDET.DevBuena, SURDET.DevMala, SURDET.Obsequios, 
                              SURDET.Precio, Marcas.IdMarca, Marcas.Marca, SURDET.Fecha, SURDET.IdSurtido, Rutas.Ruta, SUR.Status
        -- ORDER BY CAST(SURDET.IdProducto AS bigint)
...
begin
	...
	(SELECT SUM(T.Saldo) FROM
	--Reparto
	(SELECT TRP.TransProdID, TRP.Saldo, TRP.DiaClave, D.FechaCaptura
	FROM Route.dbo.TransProd TRP				
	INNER JOIN Route.dbo.Visita VIS ON VIS.VisitaClave = TRP.VisitaClave1 AND VIS.DiaClave = TRP.DiaClave1 
	...
	INNER JOIN Route.dbo.Dia D ON TRP.DiaClave = D.DiaClave
	INNER JOIN Route.dbo.TransProdDetalle TRPD ON TRP.TransProdID = TRPD.TransProdID
	INNER JOIN Productos PROD ON TRPD.ProductoClave = PROD.IdProducto
	WHERE TRP.Tipo = 1 AND TRP.TipoFase = 2 AND TRP.CFVTipo = 2 and TRP.Saldo <> 0
	AND D.FechaCaptura = @Fecha AND VIS.RUTClave = cast( @IdCedis as varchar(10) ) + replicate('0', 4 - len( @Ruta ) ) + cast( @Ruta as varchar(10) )--LIKE '%'+@Ruta
	AND PROD.IdMarca = @IdMarca
	UNION 
	--Venta Directa
	SELECT TRP.TransProdID, TRP.Saldo, TRP.DiaClave, D.FechaCaptura
	FROM Route.dbo.TransProd TRP
	INNER JOIN Route.dbo.Visita VIS ON VIS.VisitaClave = TRP.VisitaClave AND VIS.DiaClave = TRP.DiaClave
	INNER JOIN Route.dbo.Vendedor VEN ON VEN.VendedorID = VIS.VendedorID 
	INNER JOIN Route.dbo.Usuario USU ON USU.USUId = VEN.USUId
	INNER JOIN Route.dbo.Dia D ON TRP.DiaClave = D.DiaClave
	INNER JOIN Route.dbo.TransProdDetalle TRPD ON TRP.TransProdID = TRPD.TransProdID
	INNER JOIN Productos PROD ON TRPD.ProductoClave = PROD.IdProducto
	WHERE TRP.Tipo = 1 AND TRP.TipoFase = 2 AND TRP.CFVTipo = 2 and TRP.Saldo <> 0 
	AND D.FechaCaptura = @Fecha AND VIS.RUTClave = cast( @IdCedis as varchar(10) ) + replicate('0', 4 - len( @Ruta ) ) + cast( @Ruta as varchar(10) )
	AND PROD.IdMarca = @IdMarca) as T) as 'Cob. Consig.'
	from VentasTipo 
	inner join VentasDetalle on IdCedis = @IdCedis and IdSurtido = @IdSurtido and VentasDetalle.IdTipoVenta = VentasTipo.IdTipoVenta
	...
	select 99 as Tipo, 0 as Subtotal, 0 as Iva, sum(t.Total) as Total,0 as 'Cob. Consig.'
		from (
...
--	inner join Surtidos on Surtidos.IdCedis = Ventas.IdCedis and Surtidos.IdCedis = Ventas.IdCedis 
	...
	where Dia.FechaCaptura= @Fecha and USU.Clave = cast( @IdCedis as varchar(10) ) + replicate('0', 4 - len( @IdRuta ) ) + cast( @IdRuta as varchar(10) )
	and FAC.Tipo = 1 and FAC.TipoFase = 2 and (TRP.CFVTipo = 2 or TRP.CFVTipo is null) 
...
--	inner join Surtidos on Surtidos.IdCedis = Ventas.IdCedis and Surtidos.IdCedis = Ventas.IdCedis 
	...
	--inner join Route.dbo.Vendedor VEN on VEN.VendedorID = VIS.VendedorID
	--inner join Route.dbo.Usuario USU on USU.USUId = VEN.USUId
	--inner join Marcas M on M.IdMarca = 1
	--inner join Route.dbo.Cliente CS on CS.ClienteClave = VIS.ClienteClave 
	--where Dia.FechaCaptura= @Fecha and USU.Clave = cast( @IdCedis as varchar(10) ) + replicate('0', 4 - len( @IdRuta ) ) + cast( @IdRuta as varchar(10) )
	--and FAC.Tipo = 8 and FAC.TipoFase <> 0 and (TRP.CFVTipo = 2 or TRP.CFVTipo is null) 
	--group by M.IdMarca, M.Marca, CS.NombreCorto, FAC.Folio

	select @IdCedis, @IdSurtido, 2 as Orden, 'COBRANZA' as Concepto, M.IdMarca as IdMarca, left(M.Marca,30) Marca, left(CLI.NombreCorto,60) NombreSucursal, FAC.Folio as Folio, sum(FAC.Saldo) as Saldo -- replace(FAC.Folio, 'X', '') as Folio
	from Route.dbo.TransProd FAC 
	inner join Route.dbo.Cliente CLI on FAC.ClienteClave = CLI.ClienteClave
	inner join Route.dbo.Dia Dia on FAC.DiaClave = Dia.DiaClave
	--inner join Route.dbo.Usuario USU on USU.USUId = VEN.USUId
	inner join Marcas M on M.IdMarca = FAC.SubEmpresaID 
	where Dia.FechaCaptura= @Fecha and FAC.MUsuarioId = cast( @IdCedis as varchar(10) ) + replicate('0', 4 - len( @IdRuta ) ) + cast( @IdRuta as varchar(10) )
	and FAC.Tipo = 1 and FAC.TipoFase = 2 and (FAC.CFVTipo = 2 or FAC.CFVTipo is null) and FAC.Saldo<>0
	group by M.IdMarca, M.Marca, CLI.NombreCorto, FAC.Folio
...
begin
        ...
        if @IdFamiliaRejas <> 0  -- producto reja
                ...
                isnull( ( 
                        select (Inicial + Entradas - Salidas + DevBuena ) 
                        from InventarioKardex
                        where IdCedis = @IdCedis and Fecha = @Fecha and InventarioKardex.IdProducto = Productos.IdProducto
                ), 0) as 'Inv. Final', Decimales, Status
                from Productos 
                ...
                isnull( ( 
                        select (Inicial + Entradas - Salidas - VentaContado - VentaCredito - Obsequios + DevMala) 
                        from InventarioKardex
                        where IdCedis = @IdCedis and Fecha = @Fecha and InventarioKardex.IdProducto = Productos.IdProducto
                ), 0) /*-
...
begin
	...
	select Cargas.IdCedis, Cargas.IdSurtido, 'Cargas' as Concepto, cast(Cargas.IdCarga as varchar(10)) as Folio, 
	...
	select SURDET.IdCedis, SURDET.IdSurtido, 'Dev. Buenas' as Concepto, '-' as Folio, -- isnull(SDEV.IdTipoDevolucion,0) as Folio, 
	...
	select SURDET.IdCedis, SURDET.IdSurtido, 'Dev. Malas' as Concepto, case TipoMerma.Imputable when 'C' then 'Cedis' else 'Vend' end as Folio, -- isnull(SDEV.IdTipoMerma,0) as Folio, 
	SUM(Cantidad) as Unidades, SUM(Cantidad * Productos.Conversion) as Conversion, SUM(Cantidad * Precio) as Importe
	from SurtidosDetalle SURDET 
	inner join Productos on Productos.IdProducto = SURDET.IdProducto 
	inner join SurtidosMerma on SURDET.IdCedis = SurtidosMerma.IdCedis and SURDET.IdSurtido = SurtidosMerma.IdSurtido 
		and SURDET.IdProducto = SurtidosMerma.IdProducto 
	inner join TipoMerma on TipoMerma.IdTipoMerma = SurtidosMerma.IdTipoMerma 
	where SURDET.IdCedis = @IdCedis and SURDET.IdSurtido = @IdSurtido 
	group by SURDET.IdCedis, SURDET.IdSurtido, TipoMerma.Imputable 
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 101, (SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 101 where  Tipo = 'SA' and Version ='3.8.16.1'
END
GO