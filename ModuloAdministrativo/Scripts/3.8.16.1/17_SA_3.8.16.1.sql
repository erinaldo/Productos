...
/****** Object:  StORedProcedure [dbo].[sp_tg_expORtManagement]    Script Date: 07/26/2011 09:26:12 ******/
		...
		BEGIN
			...
			BEGIN
				...
				SET @CursorVar2 = CURSOR SCROLL DYNAMIC FOR 
					--Contado Reparto
					SELECT TP.Subtotal, TP.Impuesto, TP.DESCuentoVendedor + TP.DescuentoImp AS DESCuento, V.ClienteClave, TP.TransProdID, dbo.FNObtenerSolONumeros(RIGHT(TP.Folio,@LongFolio)) AS folio, TP.DiasCredito
					FROM TransProd AS TP 
					INNER JOIN Visita V ON V.VisitaClave = TP.VisitaClave1 AND V.DiaClave = TP.DiaClave1 
					INNER JOIN Vendedor VE ON VE.VendedorId = V.VendedorId
					INNER JOIN Usuario AS Us ON Us.USUId = VE.USUId
					...
					SELECT DISTINCT TRP.Subtotal, TRP.Impuesto, TRP.DescuentoVendedor + TRP.DescuentoImp as Descuento, VIS.ClienteClave, TRP.TransProdID, dbo.FNObtenerSolONumeros(RIGHT(TRP.Folio, @LongFolio)) AS Folio, TRP.DiasCredito
					FROM Abono ABN 
					INNER JOIN AbnTrp ABT ON ABN.ABNId = ABT.ABNId
					INNER JOIN TransProd TRP ON TRP.TransProdID = ABT.TransProdID 
					INNER JOIN Visita VIS ON VIS.VisitaClave = ABN.VisitaClave AND VIS.DiaClave = ABN.DiaClave
				...
				OPEN  @CursorVar2
				FETCH NEXT FROM @CursorVar2 INTO @Subtotal, @Impuesto, @Descuento, @ClienteClave, @TransProdID, @Folio, @DiasCredito
				WHILE @@FETCH_STATUS = 0      
				BEGIN 
					...
					FROM (
						...
						FROM ( 
							SELECT TD.ProductoClave, TD.SubTotal, (PRD.FactOR * TD.Cantidad) AS PiezAS, TD.Precio, TD.Impuesto, T.DescVendPor, TD.DescuentoImp,
...
--SELECT * FROM RouteADM.dbo.InventarioKardex 
--SELECT * FROM RouteADM.dbo.SurtidosDetalle 
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 17,(SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 17 where  Tipo = 'SA' and Version ='3.8.16.1'
END
GO