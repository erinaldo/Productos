...
IF NOT EXISTS (SELECT * FROM SYSCOLUMNS WHERE id = OBJECT_ID(N'[dbo].[CedisVirtuales]') AND name = 'FolioConsecutivo')
BEGIN
	BEGIN TRANSACTION
	...
	ALTER TABLE dbo.CedisVirtuales ADD
		FolioConsecutivo bigint NOT NULL CONSTRAINT DF_CedisVirtuales_FolioCons DEFAULT 1

	ALTER TABLE dbo.CedisVirtuales SET (LOCK_ESCALATION = TABLE)
	COMMIT
END
/*go*/go
...
AS
BEGIN
	IF(SELECT COUNT(*) FROM CedisVirtuales WHERE IdCedis = @IdCedisReal) = 0
	BEGIN
		SET NOCOUNT ON;

		declare @IdCedis as varchar(10), @TipoVenta as varchar(20), @IdCedisVirtual as bigint, @Existe as int
		...
		begin
			...
			begin
				...
				insert into CedisVirtuales values(@IdCedis,@TipoVenta,@IdCedisVirtual,1)
			end
			fetch next from InsertaCedis into @IdCedis, @TipoVenta
		end
		close InsertaCedis
...
END
/*go*/go
UPDATE ArchivoTextoCampo SET FormatoEspecial = NULL WHERE IdArchivo = '1' AND IdCampo = '4'
/*go*/go
...
begin
	-- CEDIS VIRTUAL
	select V.IdCedisVirtual, count(STRUT.IdRuta) as TotalRutasFecha, STRUT.Fecha, AVG(V.FolioConsecutivo)
	...
	UPDATE CedisVirtuales SET FolioConsecutivo = FolioConsecutivo + 1 WHERE IdCedisVirtual = @IdCedisVirtual
end
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 113, (SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 113 where  Tipo = 'SA' and Version ='3.8.16.1'
END