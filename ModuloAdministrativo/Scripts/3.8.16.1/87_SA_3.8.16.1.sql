...
/****** Object:  StoredProcedure [dbo].[sel_ComisionesCalculo]    Script Date: 08/30/2011 12:27:30 ******/
...
CASE WHEN COM.Acumulado = 0 THEN null ELSE (CASE WHEN COM.IdConceptoPago = 1 THEN sum(Cantidad) WHEN COM.IdConceptoPago = 2 THEN SUM(Cantidad/ isnull(ProductosUnidad.Factor,0 )) WHEN COM.IdConceptoPago = 3 THEN sum(Total) ELSE 0 END / (CASE WHEN COM.Acumulado = 0 THEN 1 ELSE @DiasTrabajados END)) END as PromSem,
...
BEGIN
	...
	VALUES('SA', GETDATE(), '3.8.16.1', 87, (SELECT  cast(SERVERPROPERTY('productversion') as varchar(50))))
END
ELSE
BEGIN 
	Update VersionBD  set MaximoConsecutivo = 87 where  Tipo = 'SA' and Version ='3.8.16.1'
END
