...
/****** Object:  StoredProcedure [dbo].[up_Ventas]    Script Date: 08/26/2011 20:13:13 ******/
...
declare @ClaveUsuario varchar(16)
...
begin
	...
	select @Folio = isnull(Ultimo + 1, 1) from FoliosRemision where IdCedis = @IdCedis and Serie in (@Serie, @SerieD)

	insert into Ventas values (@IdCedis, @IdSurtido, @IdTipoVenta, @Folio, @Serie, @Fecha, @IdCliente, 0, 0, @IdSucursal, 0, @DiasCredito)	
	
	if exists (select * from FoliosRemision where IdCedis = @IdCedis and Serie in (@Serie, @SerieD))
		update FoliosRemision set Ultimo = @Folio where IdCedis = @IdCedis and Serie in (@Serie, @SerieD)
	else
		insert into FoliosRemision values (@IdCedis, @Serie, @Folio)		
...
begin
	...
	select @Fecha = Fecha, @IdRuta = IdRuta, @ClaveUsuario = 'CFDCed' + CAST(IdCedis as varchar(10))
	from Surtidos 
	...
	delete from VentasFacturaCFD where IdCedis = @IdCedis and IdSurtido = @IdSurtido and IdTipoVenta = @IdTipoVenta and FolioFactura = @Folio 
...
begin
	...
	declare @SerieRem as varchar(5)
	declare @SerieDev as varchar(6)
	declare @ActualizarFolio as bit
	
	select @SerieRem = SerieRemisiones, @SerieDev = SerieDevoluciones from Configuracion where IdCedis = @IdCedis
	set @ActualizarFolio = 0

	if @Serie = @SerieRem or @Serie = @SerieDev
		set @ActualizarFolio = 1

	insert into Ventas values (@IdCedis, @IdSurtido, @IdTipoVenta, @Folio, @Serie, @Fecha, @IdCliente, 0, 0, @IdSucursal, 0, @DiasCredito)	
	
	if @ActualizarFolio = 1
	begin
		if exists (select * from FoliosRemision where IdCedis = @IdCedis and Serie in (@SerieRem, @SerieDev))
			update FoliosRemision set Ultimo = @Folio where IdCedis = @IdCedis and Serie in (@SerieRem, @SerieDev)
		else
			insert into FoliosRemision values (@IdCedis, @SerieRem, @Folio)
	end
