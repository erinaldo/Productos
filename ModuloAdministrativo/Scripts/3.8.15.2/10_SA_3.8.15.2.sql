...
/****** Object:  StoredProcedure [dbo].[up_Pedidos]    Script Date: 08/26/2011 19:38:24 ******/
...
--execute up_Pedidos 5, 773, 0, '19000101', 0, '19000101', 0, 0, '', 0, '', 0, 2

--set @IdCedis = 5
--set @IdPedido = 773
--set @IdTipoVenta = 0
--set @FechaPedido = '19000101'
--set @IdRutaPedido = 0
--set @FechaEntrega = '19000101'
--set @IdRutaEntrega = 0
--set @IdCliente = 0
--set @IdSucursal = ''
--set @IdSurtido = 0
...
begin
	...
	where IdCedis = @IdCedis and SerieFactura = @Serie and FolioFactura = @Folio 
	--and TransprodIdFactura <> '' and not TransprodIdFactura is null
	
	if  not @IdFactura is null
		exec Route.dbo.SP_CancelarFactura @IdFactura, @ClaveUsuario 
	else
...
begin
	...
	begin			
		...
		select @Folio = isnull(Ultimo + 1, 1) from FoliosRemision where IdCedis = @IdCedis and Serie in (@Serie, @SerieDev)
	...
	begin
		if exists (select * from FoliosRemision where IdCedis = @IdCedis and Serie in (@Serie, @SerieDev))
			update FoliosRemision set Ultimo = @Folio where IdCedis = @IdCedis and Serie in (@Serie, @SerieDev)
		else
