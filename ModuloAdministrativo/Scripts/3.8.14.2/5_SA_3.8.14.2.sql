...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[SP_CancelarVenta]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[SP_CancelarVenta]
/*go*/go
...
as Begin
		...
		begin 
				...
				select @TransprodID=t.TransprodID,@CFVTipo=t.CFVTipo,@Total=t.Total, @ClienteClave=V.ClienteClave,@ClientePagoID=max(ven.ClientePagoID),@Saldo=t.Saldo  from transprod  t inner join TransProd ven on t.TransProdID=ven.FacturaID inner join Dia D on D.DiaClave=t.DiaClave inner join Visita V on V.VisitaClave=T.VisitaClave and T.DiaClave=V.DiaClave where t.Folio=@Folio and t.Tipo=8 and D.FechaCaptura=@Fecha group by t.TransprodID,t.CFVTipo,t.Total,V.ClienteClave,t.Saldo
								...
								begin
								delete ABNDetalle from ABNDetalle inner join Abono on ABNDetalle.ABNId=Abono.ABNId inner join AbnTrp on AbnTrp.ABNId=Abono.ABNId where AbnTrp.TransProdID=@TransprodID
								delete Abono from Abono inner join AbnTrp on AbnTrp.ABNId=Abono.ABNId where AbnTrp.TransProdID=@TransprodID
								delete AbnTrp where AbnTrp.TransProdID=@TransprodID
								...
								begin
								delete ABNDetalle from ABNDetalle inner join Abono on ABNDetalle.ABNId=Abono.ABNId inner join AbnTrp on AbnTrp.ABNId=Abono.ABNId  inner join TransProd on TransProd.TransProdID=AbnTrp.TransProdID where TransProd.FacturaID=@TransprodID
								delete Abono from Abono inner join AbnTrp on AbnTrp.ABNId=Abono.ABNId inner join TransProd on TransProd.TransProdID=AbnTrp.TransProdID where TransProd.FacturaID=@TransprodID
								delete AbnTrp From AbnTrp inner join TransProd on TransProd.TransProdID=AbnTrp.TransProdID where TransProd.FacturaID=@TransprodID
								Update TransProd Set Saldo =Total where TransProdID=@TransprodID
