...
/****** Object:  StoredProcedure [dbo].[Gsp_NivelReporte]    Script Date: 12/10/2010 11:59:38 ******/
...
begin
	...
	begin		
		exec ( 'select Clientes.IdCedis, Clientes.IdCliente, isnull( ClientesTipo.Tipo, ''-'') as CTipo, RazonSocial, 
			...
			from FN_VencimientoAlaFecha (' + @IdCedi + ', ''' + @FechaFinal + ''') 
			where FN_VencimientoAlaFecha.IdCliente = Clientes.IdCliente 
			and DiasVencidos >= 1 and DiasVencidos <= 7 
			), 0) as ''1 a 7'', 
			
			isnull((select sum(Saldo)
			from FN_VencimientoAlaFecha (' + @IdCedi + ', ''' + @FechaFinal + ''') 
			where FN_VencimientoAlaFecha.IdCliente = Clientes.IdCliente
			and DiasVencidos >= 8 and DiasVencidos <= 15
			), 0) as ''8 a 15'', 
			
			isnull((select sum(Saldo)
			from FN_VencimientoAlaFecha (' + @IdCedi + ', ''' + @FechaFinal + ''') 
			where FN_VencimientoAlaFecha.IdCliente = Clientes.IdCliente
			and DiasVencidos >= 16 and DiasVencidos <= 30
			), 0) as ''16 a 30'', 
...
begin
	...
	if @Tipo = 10 or @Tipo = 11
	begin		
		exec( 'select FN_VentasPorCliente.IdCedis, FN_VentasPorCliente.IdCliente, isnull( ClientesTipo.Tipo, ''-'') as CTipo, RazonSocial, Grupos.IdGrupo as IdGpo, Familias.IdFamilia as IdFam, 
		...
		group by FN_VentasPorCliente.IdCedis, FN_VentasPorCliente.IdCliente, RazonSocial, FN_VentasPorCliente.IdGrupo, FN_VentasPorCliente.IdFamilia, Grupos.IdGrupo, Familias.IdFamilia, FN_VentasPorCliente.IdProducto, FN_VentasPorCliente.Producto, Precio, ClientesTipo.Tipo 		
...
if @ReporteOrigen = 20   -- COBRANZA TERMINALES
begin 
	...
	if @Tipo = 10 or @Tipo = 11
	begin
		exec ( 'select ''COBRANZA'' as Concepto, Dia.FechaCaptura as Fecha, 1 as IdMarca, left(''MODELO DE ORIENTE'',30) Marca, 
		left(CS.NombreCorto,60) NombreSucursal, TRP.Folio as Folio, sum(ABN.Total) as Total 
		...
		group by Dia.FechaCaptura, CS.NombreCorto, TRP.Folio
		order by Dia.FechaCaptura, CS.NombreCorto, TRP.Folio ')
	end
...
begin
	...
	exec ( 'SELECT  Substring(FNCTA.Cuenta,5,2) AS No, Familias.Familia, FNCTA.Cuenta, FNCTA.Entidad, FNCTA.IdProducto AS Clave, PRO.Producto AS NombreCuenta, CED.Cedis AS NombreEntidad, FNCTA.Piezas, 
		...
		VentasDetalle.Serie = Ventas.Serie collate Traditional_Spanish_CI_AS and VentasDetalle.Folio = Ventas.Folio 
		...
		MC.Serie = Ventas.Serie collate Traditional_Spanish_CI_AS and MC.Folio = Ventas.Folio and MC.Status <> ''B'' and MC.CargoAbono = ''C'' and MC.Fecha between ''' + @FechaInicial + ''' and ''' + @FechaFinal + '''
		left outer join MovimientosFacturas as MA on MA.IdCedis = Ventas.IdCedis and MA.IdTipoVenta = Ventas.IdTipoVenta and 
		MA.Serie = Ventas.Serie collate Traditional_Spanish_CI_AS and MA.Folio = Ventas.Folio and MA.Status <> ''B'' and MA.CargoAbono = ''A'' and MA.Fecha between ''' + @FechaInicial + ''' and ''' + @FechaFinal + '''
