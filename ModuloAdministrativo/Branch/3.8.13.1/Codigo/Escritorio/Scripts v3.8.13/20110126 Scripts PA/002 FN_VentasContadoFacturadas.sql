...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FN_VentasContadoFacturadas]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[FN_VentasContadoFacturadas]
GO

USE [RouteADM]
GO

/****** Object:  UserDefinedFunction [dbo].[FN_VentasContadoFacturadas]    Script Date: 01/26/2011 12:29:01 ******/
SET ANSI_NULLS OFF
GO
...
CREATE FUNCTION [dbo].[FN_VentasContadoFacturadas]
(
	@IdCedis as bigint,
...
AS  
	...
	(
		...
		and Ventas.Serie in (select SerieFacturasCredito from Configuracion where IdCedis = Ventas.IdCedis)
