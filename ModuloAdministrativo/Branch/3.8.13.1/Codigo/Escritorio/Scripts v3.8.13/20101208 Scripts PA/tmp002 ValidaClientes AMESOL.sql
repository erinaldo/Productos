...
--VENTAS CON CLAVE DE CLIENTE = ''
select Ventas.IdCedis, Ventas.IdSurtido, Ventas.Fecha, Ventas.IdCliente, Ventas.Serie, Ventas.Folio,
Ventas.IdSucursal, ClienteSucursal.NombreSucursal, Ventas.IdCliente, ClienteSucursal.IdSucursal, Ventas.Total 
from Ventas 
left outer join ClienteSucursal on ClienteSucursal.IdCedis = Ventas.IdCedis and rtrim(ltrim(ClienteSucursal.IdSucursal)) = rtrim(ltrim(Ventas.IdSucursal))
where Ventas.IdSucursal = ''
order by Ventas.IdCedis, Ventas.Fecha, Ventas.IdSucursal 

--select *
-- --delete 
--from ClienteSucursal where IdCliente = 51
-- IdSucursal in ('011010411', '011010412', '014270001') and IdCliente = 1 and IdCedis = 1
