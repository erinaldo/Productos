...
BEGIN
	...
	where MOV.IdCedis = @IdCedis and MOV.IdMovimiento = @IdMovimiento
	...
	DECLARE @Serie AS VARCHAR(20), @IdTipoVenta AS INT, @FolioADM AS INT, @idRuta as INT, @IdMovimientoDetalle AS INT, @idSucursal AS varchar(30)
	...
	WHERE M.Fecha = @Fecha AND M.IdCedis = @idCedis  and M.IdMovimiento = @IdMovimiento and M.Status = 'A' and M.IdDocumento <> 'CT'
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				SET @RUTClave = 'R' + REPLICATE('0', 2-len(@idRuta)) + cast(@idRuta as varchar(10)) -- CONVERT(VARCHAR(20),@idCedis)+RIGHT('0000'+CONVERT(VARCHAR(4),@idRuta),4)
				...
				INSERT INTO Abono (ABNId, Folio, FechaCreacion, VisitaClave, DiaClave, FechaAbono, Total, Saldo, MFechaHora, MUsuarioID)
					VALUES(@ABNID, @Folio, @Fecha, @VisitaClave, @DiaClave, @Fecha, @Total, 0, GETDATE(), @USUID)
			
				INSERT INTO AbnTrp (ABNId, TransProdID, FechaHora, Importe, Serie, Corecibo, TipoFaseIntSal, MFechaHora, MUsuarioID)
					VALUES(@ABNID, @TransProdID, @Fecha, @Total, '','',1,GETDATE(), @USUID)
				
				INSERT INTO ABNDetalle (ABNId, ABDId, TipoPago, Importe, SaldoDeposito, MonedaId, Referencia, MFechaHora, MUsuarioID)
					VALUES(@ABNID, RIGHT(REPLACE(CONVERT(VARCHAR(36),NEWID()),'-',''),16), 1, @Total, 0, @MonedaID, '', GETDATE(), @USUID)
