...
/****** Object:  StoredProcedure [dbo].[up_FacturasImpresion]    Script Date: 04/28/2011 18:14:18 ******/
...
begin
	...
	exec ( ' insert into VentasFacturadas
		select IdCedis, IdSurtido, IdTipoVenta, Folio, Serie, ''' + @FolioImpresion + ''', ''S'', getdate(), ''' + @Login + '''
...
begin
	...
	exec ( ' insert into PedidosFacturados
		select t.TransProdId, ''' + @FolioImpresion + ''', ''S'', getdate(), ''' + @Login + '''
...
begin
	...
	exec ( ' insert into PedidosFacturados
		select ' + @Filtro + ', ''' + @FolioImpresion + ''', ''S'', getdate(), ''' + @Login + ''' ') 
