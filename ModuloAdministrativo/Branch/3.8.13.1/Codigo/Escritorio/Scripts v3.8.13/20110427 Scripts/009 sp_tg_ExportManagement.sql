...
/****** Object:  StoredProcedure [dbo].[sp_tg_exportManagement]    Script Date: 04/28/2011 19:56:13 ******/
...
BEGIN
	SET DATEFORMAT DMY;
	  ...
	  declare @SerieN as varchar(10), @FolioN as bigint
...
---------------------------------------------------------------------------------------------------------
                        ...
                        BEGIN 
							...
							if(@IdClienteContadoTmp is not null)
                             ...
                             insert into [RouteADM].dbo.Ventas (IdCedis,IdSurtido,IdTipoVenta,Folio,Serie,Fecha,IdCliente,Subtotal,Iva,IdSucursal,DiasCredito,FechaEdicion, Login)
                              values(@ClaveCEDI,@Surtido,@TipoVenta,@Folio,@SerieFactContado,@Dia,@IdClienteContado,@Subtotal,@Impuesto,@ClienteClave, @DiasCredito,GETDATE(),@vende)
                             ...
                             if exists(select TransProdId from RouteADM.dbo.PedidosFacturados where TransProdId = @TransProdId and ISNUMERIC(FolioImpresion) = 1)
                             begin
                             
								select @SerieN = SerieFacturasCredito from RouteADM.dbo.Configuracion where IdCedis = @ClaveCEDI 
								select @FolioN = cast(FolioImpresion as bigint) from RouteADM.dbo.PedidosFacturados where TransProdId = @TransProdId
                             
								if not exists( select Folio from RouteADM.dbo.Ventas where IdCedis = @ClaveCEDI and IdSurtido = @Surtido and IdTipoVenta = @TipoVenta and Folio = @FolioN )
                             		exec RouteADM.dbo.up_VentasFactura @ClaveCEDI, @Surtido, @TipoVenta, @Folio, @SerieFactContado, @FolioN, @SerieN, @Vende, 1 
                             end
                        ...
                        BEGIN 
							...
							if(@IdClienteContadoTmp is not null)
                             ...
                             insert into [RouteADM].dbo.Ventas (IdCedis,IdSurtido,IdTipoVenta,Folio,Serie,Fecha,IdCliente,Subtotal,Iva,IdSucursal, DiasCredito,FechaEdicion, Login)
                              values(@ClaveCEDI,@Surtido,@TipoVenta,@Folio,@Serie,@Dia,@IdClienteContado,@Subtotal,@Impuesto,@ClienteClave, @DiasCredito,GETDATE(),@vende)
