...
Public IdTipoVenta, Folio, RFC, Nombre, Direccion, Tel, IdCliente, IdSucursal, Serie, FolioRem
...
On Error Resume Next
    ...
    StrCmd = "execute sel_VentasDetalle " & IdCedis & ", " & IdSurtido & ", " & IdTipoVenta & ", " & Folio & ", " & IIf(Not LstConfiguraLiquidacion(7), 1, 3)
    If RsC.State Then RsC.Close
    RsC.Open StrCmd, Cnn
    If Not Rep Then LstDPartidas = GetDataLVL(RsC, LstPartidas, 4, IIf(Not LstConfiguraLiquidacion(7), 10, 12), IIf(Not LstConfiguraLiquidacion(7), "0|0|9|7|7|7|7", "0|0|9|7|8|7|7|7|7"))
End Sub
...
On Error GoTo Err_EliminarFact:
    ...
    If MsgBox("¿ Está seguro que desea Eliminar la Factura " & Format(Folio, "0000000") & " del Cliente " & Chr(13) & Chr(10) & Nombre & " por " & LblTotal.Caption & " ?", vbQuestion + vbYesNo, App.Title) = vbYes Then
        ...
        MousePointer = 11
            ...
            StrCmd = "execute sel_VentasRoute " & IdCedis & ", " & IdSurtido & ", " & IdTipoVenta & ", " & Folio & ", 2"
            ...
            StrCmd = "execute up_Ventas " & IdCedis & ", " & IdSurtido & ", " & IdTipoVenta & ", " & Folio & ", '" & Serie & "', '" & FormatDate(Fecha) & "', 0, 0, '" & Usuario & "', 3"
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
    ...
    Select Case LstConfiguraLiquidacion(8)
        Case "MODELOORIENTE":
            With AL_RptFacturaModelo
                ...
                .LblFolio.Caption = FolioRem
                .LblTipoVenta.Caption = "Venta de " & TxtTipoVenta.Text
...
On Error Resume Next
    ...
    TxtDctoImp.Visible = IIf(LstConfiguraLiquidacion(7), True, False)
    TxtDctoPorc.Visible = IIf(LstConfiguraLiquidacion(7), True, False)
    LblDcto(0).Visible = IIf(LstConfiguraLiquidacion(7), True, False)
    LblDcto(1).Visible = IIf(LstConfiguraLiquidacion(7), True, False)
End Sub
...
Dim Existencias, Descuento, DctoCapturado
    ...
    If KeyAscii = 13 Then
        ...
        If LstConfiguraLiquidacion(7) Then
            TxtDctoPorc.SetFocus
        Else
            TxtDctoPorc_KeyPress 13
            TxtIdProd.SetFocus
        End If
...
'        Forma = 6: AL_ProductosBusqueda.Show
    End If
    If KeyAscii = 13 Then
        ValidaProductoVenta
        Exit Sub
...
Dim Existencias, Descuento, DctoCapturado
    ...
    If KeyAscii = 13 Then
        ...
        If Not Rs.EOF Then
            Descuento = Rs.Fields(0)
            If Descuento < CDbl(TxtDctoPorc.Text) Then
                MsgBox "¡ El descuento permitido para el esta Ruta debe ser menor a " & FormatPercent(Descuento / 100, 2) & " !"
                TxtDctoPorc.Text = "0"
                TxtDctoPorc.SetFocus
                Exit Sub
            End If
...
'                    Exit Sub
        ...
        StrCmd = "execute up_VentasDetalle " & IdCedis & ", " & IdSurtido & ", " & IdTipoVenta & ", " & Folio & ", '" & Trim(TxtSerie.Text) & "', '" & FormatDate(Fecha) & "', " & TxtIdProd.Text & _
            ", " & CDbl(TxtCantidad.Text) & ", " & Precio & ", " & Iva & ", 0" & IIf(LstConfiguraLiquidacion(7), ", " & CDbl(TxtDctoPorc.Text) / 100 & ", " & CDbl(TxtDctoImp.Text), "") & ", 1"
        If Rs.State Then Rs.Close
        Rs.Open StrCmd, Cnn
...
'        Rs.Open StrCmd, Cnn
        ...
        TxtCantidad.Text = 0: TxtPrecio.Text = 0: TxtSubtotal.Text = 0: TxtIva.Text = 0: TxtTotal.Text = 0: TxtDctoPorc.Text = 0: TxtDctoImp.Text = 0
        TxtIdProd.Text = "": TxtIdProd.SetFocus: TxtDesc.Text = "": TxtExistencia.Text = 0
        Exit Sub
    End If
