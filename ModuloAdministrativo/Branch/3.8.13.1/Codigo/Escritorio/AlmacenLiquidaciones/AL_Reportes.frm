    ...
    Select Case IdReporte
       ...
       Case 8      'Resumen de Precios
        ...
        Case 14      'Facturas de Contado
            TituloReporteXLS = "Facturas de Contado ( " & NivelReporte & " )"
            'CedisXLS = IdCedis & " - " & NomCedis
            PeriodoXLS = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                "  al  " & Format(RepFechaFinal, ctFechaLarga)
        ...
        Case 24
            CreaArchivoPolizaModeloOriente
            MousePointer = 0
            MsgBox "El Archivo se Creó con Exito...", vbInformation + vbOKOnly, App.Title
            Exit Sub
...
Private Sub btnImprimir_Click()
    ...
    If IdReporte = 25 Or IdReporte = 26 Then
        If ValidaDiaySurtido(CLng(RepIdCedi), 0, CDate(RepFechaInicial)) Then
            If Not Cnn.State Then OpenConn Server, Db, UserDB, PasswordDB
            If Rs.State Then Rs.Close
        ...
        If IdReporte = 25 Then
            If Rs.State Then Rs.Close
            StrCmd = "exec up_ModeloOrienteESEnvase '" & RepIdCedi & "','" & FormatDate(CDate(RepFechaInicial)) & "'"
            Rs.Open StrCmd, Cnn
        End If
    ...
    If Not Rs.EOF Then
        Select Case IdReporte
            ...
            Case 3      'comisiones
                With AL_RptRepComisionesSanMarcos
                    .ImgLogo.Picture = LoadPicture(App.Path & "\LogoEmpresa.jpg"): .ImgLogo.Height = 810: .ImgLogo.Width = 1920
                    .LblTitulo.Caption = "Cálculo de Comisiones por Venta de Productos ( " & NivelReporte & " )"
                    '.DescripcionAgrupacion = DescripcionAgrupacion
                    .LblCedis.Caption = RepIdCedi & " - " & RepNombreCedi
                    .LblDatos.Caption = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                        "  al  " & Format(RepFechaFinal, ctFechaLarga)
            ...
            Case 22      'Saldos del Vendedor
             ...
             Case 25      'Modelo de Oriente Entradas y Salidas
                ...
                StrCmd = "execute sel_ModeloOrienteESEnvase " & RepIdCedi & ", '" & FormatDate(CDate(RepFechaInicial)) & "'"
                If RsC.State Then RsC.Close
                RsC.Open StrCmd, Cnn
                With AL_RptRepModeloOrienteESEnvase
                
                    .ImgLogo.Picture = LoadPicture(App.Path & "\LogoEmpresa.jpg")
                    .LblTitulo.Caption = "Reporte de Entradas y Salidas Envase Modelo de Oriente ( " & NivelReporte & " )"
                    .LblCedis.Caption = RepIdCedi & " - " & RepNombreCedi
                    .LblDatos.Caption = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                        "  al  " & Format(RepFechaFinal, ctFechaLarga)
                    .Printer.Orientation = ddOPortrait: .Printer.PaperSize = 1
                    .DataSrc.DataSourceName = Cnn: .DataSrc.Recordset = RsC
                    .Show 1
                ...
                With AL_RptRepModeloOrienteES
                    ...
                    .LblDatos.Caption = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                        "  al  " & Format(RepFechaFinal, ctFechaLarga)
                    .Printer.Orientation = ddOLandscape: .Printer.PaperSize = 5
                    .DataSrc.DataSourceName = Cnn: .DataSrc.Recordset = Rs
                    .Show 1
...
Sub CreaArchivoPolizaModeloOriente()
Dim NombreArchivo, Base, IdCons As Long

On Error GoTo Err_Poliza:

    MousePointer = 11
    NombreArchivo = "\ReportesXLS\Poliza" & Format(IdCedis, "00") & "-" & Format(FechaInicial, "yyyymmdd") & ".txt"
    FConexion = App.Path & NombreArchivo
    If FileExists(App.Path & NombreArchivo) Then Kill FConexion
    
    Fd = FreeFile
    Base = "I-" & Month(FechaInicial) & "-0" & Format(FechaInicial, "dd") & "  |" & Year(FechaInicial) & "| 1|" & Format(FechaInicial, "yyyymmdd") & "|"
    
    Open FConexion For Output As #Fd
        Print #Fd, "CVW2.0"
        Print #Fd, "1|" & Base & "3|" & DefineCampo("", 15) & "|" & DefineCampo("", 10) & "| 0|" & DefineCampo("", 10) & "|" & DefineCampo("INGRESOS " & Format(FechaInicial, "dd MMMM yyyyy"), 50) & "|" & DefineCampo("", 50) & "| 0|" & DefineCampo("", 15) & "|" & DefineCampo("0.00000", 16) & "|" & DefineCampo("", 15) & "|" & DefineCampo("", 15) & "|" & DefineCampo("", 15) & "|A| | |" & DefineCampo("1.000000", 16) & "|" & DefineCampo("", 50)
        
        If Rs.State Then Rs.Close
        StrCmd = "execute sel_PolizaModeloOriente '" & RepIdCedi & "','" & FormatDate(CDate(RepFechaInicial)) & "','" & FormatDate(CDate(RepFechaFinal)) & "', 1"
        Rs.Open StrCmd, Cnn
        IdCons = 1
        While Not Rs.EOF
            Print #Fd, "2|" & Base & DefineCampo(IdCons, 4) & "|3|I-" & Month(FechaInicial) & "-0" & Format(FechaInicial, "dd") & "  |" & DefineCampo(Trim(Rs.Fields(3)), 14) & "|" & DefineCampo(Trim(Rs.Fields(5)), 60) & "|" & DefineCampo(IIf(CDbl(Rs.Fields(6)) <> 0, Format(Rs.Fields(6), "0.00000"), Format(Rs.Fields(7), "0.00000")), 16) & "|" & DefineCampo(IIf(CDbl(Rs.Fields(6)) <> 0, "C", "A"), 1) & "|" & DefineCampo("0", 2) & "|" & DefineCampo("0", 2)
            IdCons = IdCons + 1
            Rs.MoveNext
        Wend
    Close #Fd
...
Err_Poliza:
    MousePointer = 0
    MsgBox "Error al crear el Archivo de la Póliza de Modelo de Oriente. Error: " & Err.Description, vbInformation + vbOKOnly, App.Title
    GoTo No_Err_Poliza:

End Sub

Function DefineCampo(Texto, Longitud) As String
    DefineCampo = ""
    For i = 1 To Longitud - Len(Trim(Texto))
        DefineCampo = DefineCampo + " "
    Next
    DefineCampo = CStr(Texto) + DefineCampo
End Function
