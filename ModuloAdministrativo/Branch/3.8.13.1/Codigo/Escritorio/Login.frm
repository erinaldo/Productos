    ...
    Else
        ...
        With MenuAlmacen
            ...
            Else
                ...
                ReDim LstConfiguraLiquidacion(8)
                For i = 10 To 18
...
'On Error Resume Next
    ...
    If MsgBox("¿ Deseas Generar los Periodos ?", vbQuestion + vbYesNo, App.Title) = vbNo Then Exit Sub
    
    MousePointer = 11
    
    Dim i As Date, j As Date, IdPeriodo, IdSemanaNat, IdSemanaLab, IdSemana, DiasMes, Mes, Agno
    Dim NatIni As Date, NatFin As Date
    Dim LabIni As Date, LabFin As Date
    
    IdSemanaNat = 3: IdPeriodo = 11: IdSemanaLab = 2: IdSemana = 577
    NatIni = CDate("10/01/2010"): NatFin = CDate("16/01/2010")
    LabIni = CDate("07/01/2010"): LabFin = CDate("13/01/2010")
    DiasMes = 9: Mes = 1: Agno = 2011
    ...
    While i < j
        ...
        If Month(i) <> Mes Then
            StrCmd = "update Periodos set Per_DiasHab = " & DiasMes & " where month(Per_Fecha)= " & Mes & " and year(Per_Fecha) = " & Year(i - 1)
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
...
'        If Agno <> Year(i) Then
'            IdPeriodo = 0: IdSemanaNat = 1: IdSemanaLab = 1
'            Agno = Year(i)
'        End If
...
'        Debug.Print StrCmd
    ...
    Wend
    MousePointer = 0
    MsgBox "¡ Periodos generados con éxito !", vbInformation, App.Title
End Sub
