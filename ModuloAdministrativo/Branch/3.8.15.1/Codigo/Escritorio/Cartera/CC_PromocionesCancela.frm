...
Begin VB.Form CC_PromocionesCancela 
   ...
   Caption         =   "Cancelación del Acuerdo"
   ClientHeight    =   3960
   ClientLeft      =   45
   ClientTop       =   315
   ClientWidth     =   10515
...
On Error GoTo Err_CancelaPromocion:
    ...
    If MsgBox("¡ Al Cancelar el Movimiento se dará de Baja el Acuerdo seleccionado !. ¿ Desea continuar ?", vbQuestion + vbYesNo, App.Title) = vbNo Then Exit Sub
    ...
    For i = 1 To LstDetalle.ListItems.Count
        If LstDetalle.ListItems(i).Checked Then
            ...
            StrCmd = "execute up_Folio " & CLng(LstDetalle.SelectedItem) & ", '19000101', 0, '', 'B', '" & Usuario & "',  3"
            If RsC.State Then RsC.Close
            RsC.Open StrCmd, Cnn
    ...
    Else
        MousePointer = 0
        MsgBox "¡ Movimientos Cancelados !", vbInformation + vbOKOnly, App.Title
    End If
...
Err_CancelaPromocion:
    MousePointer = 0
    MsgBox "¡ Error al tratar de Cancelar el Acuerdo !", vbInformation + vbOKOnly, App.Title
...
On Error Resume Next
    ...
    Rs.Open StrCmd, Cnn
    LstDDetalle = GetDataLVL(Rs, LstDetalle, 0, 4, "0|0|0|0|3")
