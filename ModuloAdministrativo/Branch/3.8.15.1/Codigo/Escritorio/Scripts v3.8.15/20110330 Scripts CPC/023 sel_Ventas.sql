...
/****** Object:  StoredProcedure [dbo].[sel_Ventas]    Script Date: 04/01/2011 09:29:53 ******/
...
begin
	...
	where Ventas.Serie like '%' + @Serie + '%' and Ventas.Folio like '%' + @Folio + '%' and Ventas.IdCedis in (select IdCedis from UsuariosCedis where Login = @Login) 
...
if @Opc = 4
begin
	exec ( 'select cast(Ventas.IdCedis as varchar(4)) as IdCedis, cast(isnull(Cedis,''-Cedis No Encontrado-'') as varchar(20)) as Cedis, Ventas.IdTipoVenta, Ventas.Fecha, Ventas.Serie, Ventas.Folio, 
	VentasAcredita.FolioEntrega, VentasAcredita.FolioCliente, VentasAcredita.Factura, VentasAcredita.Remision, 	
	Ventas.Total, Ventas.Cargos, Ventas.Abonos, Ventas.Saldo, Ventas.IdCliente, isnull(Clientes.RazonSocial,''-Cliente No Econtrado-'') as RazonSocial,
	Ventas.IdSucursal, isnull(ClienteSucursal.NombreSucursal,''-Sucursal No Econtrada-'') as NombreSucursal, 
	isnull(FacturasOxxo.SerieOX,'''') as SerieOX, isnull(FacturasOxxo.FolioOX,0) as FolioOX, Ventas.Status
...
if @Opc = 5
begin 
	select Serie, Folio from Ventas where IdCedis = @IdCedis and IdTipoVenta = @IdTipoVenta and Serie = @Serie and Folio = @Folio 
end
