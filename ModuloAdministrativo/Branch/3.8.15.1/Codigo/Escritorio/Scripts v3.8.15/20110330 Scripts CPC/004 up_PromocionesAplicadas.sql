...
/****** Object:  StoredProcedure [dbo].[up_PromocionesAplicadas]    Script Date: 03/30/2011 17:56:19 ******/
...
begin
	...
	set Monto = isnull( ( select Monto 
			...
			where FNSaldoMonto.IdCedis = PromocionesAplicadasDetalle.IdCedis and FNSaldoMonto.IdTipoVenta = PromocionesAplicadasDetalle.IdTipoVenta
				and FNSaldoMonto.Serie = PromocionesAplicadasDetalle.Serie and FNSaldoMonto.Folio = PromocionesAplicadasDetalle.Folio ), 0)
	where IdAplicacion = @IdAplicacion and IdPromocion = @IdPromocion -- and Aplicado = ''
	
	update PromocionesAplicadasDetalle set Monto = 0
	where IdAplicacion = @IdAplicacion and IdPromocion = @IdPromocion 
		and IdPromocion = IdPromocionAnterior 
	
	update PromocionesAplicadasDetalle set Monto = 0
	where IdAplicacion = @IdAplicacion and IdPromocion = @IdPromocion 
		and SUBSTRING(Aplicado,1,1) = '3' and IdPromocion in (select IdPromocion from Promociones where IdPromocion = @IdPromocion and Otras = 0)
...
begin
	update PromocionesAplicadasDetalle 
	set Aplicado = '2 | El Monto del Acuerdo: ' + cast(Monto as varchar(10)) + ' excede el saldo: ' + cast(Saldo as varchar(10)), Monto = 0
	where IdAplicacion = @IdAplicacion and IdPromocion = @IdPromocion and Monto > Saldo
	
	delete from PromocionesAplicadasDetalle 
	where Aplicado = '' and Monto = 0 and IdPromocion = @IdPromocion 
end
...
begin
	...
	--delete from PromocionesAplicadasDetalle where IdMovimiento = 0 and IdAplicacion = @IdAplicacion and IdPromocion = @IdPromocion 

	update PromocionesAplicadasDetalle
	set Aplicado = 'E | Error al aplicar Acuerdo'
	where IdAplicacion = @IdAplicacion and IdPromocion = @IdPromocion 
		and ( (Aplicado = '' and Monto > 0) ) --or (SUBSTRING(Aplicado,1,1) = '3') )

end

if @Opc = 6
begin
	delete from PromocionesAplicadas where IdPromocion = @IdPromocion and IdAplicacion = @IdAplicacion 
	delete from PromocionesAplicadasDetalle where IdPromocion = @IdPromocion and IdAplicacion = @IdAplicacion 
end
