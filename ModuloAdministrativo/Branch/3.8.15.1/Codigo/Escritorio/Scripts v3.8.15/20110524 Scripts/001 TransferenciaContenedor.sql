...
/****** Object:  StoredProcedure [dbo].[up_TransferenciaContenedor]    Script Date: 05/25/2011 07:46:32 ******/
...
AS
	...
	declare @FechaInicial as datetime, @FechaFinal as datetime, @Ejecuta as int
	...
	begin
		...
		Insert Into [Contenedor].[RouteADM].DBO.SurtidosImpuestos
		([IdCedis],[IdSurtido],[IdProducto],[Fecha],[IdTipoImpuesto],[TipoAplicacion],[Jerarquia],[Tasa])
		Select [IdCedis],[IdSurtido],[IdProducto],[Fecha],[IdTipoImpuesto],[TipoAplicacion],[Jerarquia],[Tasa]
		From SurtidosImpuestos where IdCedis = @IdCedis and IdSurtido in (SELECT SUR.IdSurtido FROM Surtidos SUR WHERE SUR.IdCedis = @IdCedis and SUR.Fecha = @FechaInicial )
		...
		Insert Into [Contenedor].[RouteADM].DBO.VentasImpuestos
		([IdCedis],[IdSurtido],[IdTipoVenta],[Folio],[IdProducto],[IdTipoImpuesto],[TipoAplicacion],[Jerarquia],[Tasa])
		Select [IdCedis],[IdSurtido],[IdTipoVenta],[Folio],[IdProducto],[IdTipoImpuesto],[TipoAplicacion],[Jerarquia],[Tasa]
		From VentasImpuestos where IdCedis = @IdCedis and IdSurtido in (SELECT SUR.IdSurtido FROM Surtidos SUR WHERE SUR.IdCedis = @IdCedis and SUR.Fecha = @FechaInicial )

		Insert Into [Contenedor].[RouteADM].DBO.VentasPromociones
		([IdCedis],[IdSurtido],[IdTipoVenta],[Folio],[IdProducto],[IdPromocion],[Otras],[Cascada],[DctoPorc],[DctoImp],[Resultado])
		Select [IdCedis],[IdSurtido],[IdTipoVenta],[Folio],[IdProducto],[IdPromocion],[Otras],[Cascada],[DctoPorc],[DctoImp],[Resultado]
		From VentasPromociones where IdCedis = @IdCedis and IdSurtido in (SELECT SUR.IdSurtido FROM Surtidos SUR WHERE SUR.IdCedis = @IdCedis and SUR.Fecha = @FechaInicial )
		...
		delete from [Contenedor].[RouteADM].dbo.PedidosDetalle 
			where IdCedis = @IdCedis and IdPedido in ( select IdPedido 
				from Pedidos 
				where IdCedis = @IdCedis and (FechaEntrega = @FechaInicial or FechaPedido = @FechaInicial ))
				
		delete from [Contenedor].[RouteADM].dbo.PedidosImpuestos  
			where IdCedis = @IdCedis and IdPedido in ( select IdPedido 
				from Pedidos 
				where IdCedis = @IdCedis and (FechaEntrega = @FechaInicial or FechaPedido = @FechaInicial ))

		delete from [Contenedor].[RouteADM].dbo.PedidosPromociones  
			where IdCedis = @IdCedis and IdPedido in ( select IdPedido 
				from Pedidos 
				where IdCedis = @IdCedis and (FechaEntrega = @FechaInicial or FechaPedido = @FechaInicial ))
		...
		from Pedidos where IdCedis = @IdCedis and (FechaEntrega = @FechaInicial or FechaPedido = @FechaInicial )
		...
		from  PedidosDetalle where IdCedis = @IdCedis and IdPedido in (Select IdPedido from Pedidos 
			where IdCedis = @IdCedis and (FechaEntrega = @FechaInicial or FechaPedido = @FechaInicial ))

		Insert into [Contenedor].[RouteADM].DBO.PedidosImpuestos 
		([IdCedis],[IdPedido],[IdTipoVenta],[IdProducto],[IdTipoImpuesto],[TipoAplicacion],[Jerarquia],[Tasa])
		Select [IdCedis],[IdPedido],[IdTipoVenta],[IdProducto],[IdTipoImpuesto],[TipoAplicacion],[Jerarquia],[Tasa]
		from  PedidosImpuestos where IdCedis = @IdCedis and IdPedido in (Select IdPedido from Pedidos 
			where IdCedis = @IdCedis and (FechaEntrega = @FechaInicial or FechaPedido = @FechaInicial ))

		Insert into [Contenedor].[RouteADM].DBO.PedidosPromociones 
		([IdCedis],[IdPedido],[IdTipoVenta],[IdProducto],[IdPromocion],[Otras],[Cascada],[DctoPorc],[DctoImp],[Resultado])
		Select [IdCedis],[IdPedido],[IdTipoVenta],[IdProducto],[IdPromocion],[Otras],[Cascada],[DctoPorc],[DctoImp],[Resultado]
		from  PedidosPromociones where IdCedis = @IdCedis and IdPedido in (Select IdPedido from Pedidos 
			where IdCedis = @IdCedis and (FechaEntrega = @FechaInicial or FechaPedido = @FechaInicial ))
		...
		from VendedoresSaldosFinancia Where IdCedis = @IdCedis and Fecha = @FechaInicial
		...
		from  VendedoresSaldosFinanciaD Where IdCedis = @IdCedis and IdCorto in (Select IdCorto from VendedoresSaldosFinancia
			where IdCedis = @IdCedis and Fecha = @FechaInicial )
	...
	begin
		...
		Insert into [Contenedor].[RouteADM].DBO.ProductosImpuestos 
		Select PImp.IdProducto, PImp.IdTipoImpuesto  
		from ProductosImpuestos PImp
		left outer join [Contenedor].[RouteADM].DBO.ProductosImpuestos PImpC on PImp.IdProducto = PImpC.IdProducto and PImp.IdTipoImpuesto = PImpC.IdTipoImpuesto 
		where PImpC.IdTipoImpuesto is null
		...
		--update [Contenedor].[RouteADM].DBO.ClienteSucursal set [RFC] = CteS.RFC, [RazonSocial] = CteS.[RazonSocial], [CalleF] = CteS.[CalleF], [NumExteriorF] = CteS.[NumExteriorF], 
		--	[NumInteriorF] = CteS.[NumInteriorF], [ColoniaF] = CteS.[ColoniaF], [LocalidadF] = CteS.[LocalidadF], [PoblacionF] = CteS.[PoblacionF], [EntidadF] = CteS.[EntidadF], 
		--	[PaisF] = CteS.[PaisF], [TelefonosF] = CteS.[TelefonosF], [CPF] = CteS.[CPF]
		--from ClienteSucursal CteS, [Contenedor].[RouteADM].DBO.ClienteSucursal as CSCont
		--where CteS.IdCedis = CSCont.IdCedis and CteS.IdSucursal = CSCont.IdSucursal and ( 
		--CteS.[RFC] <> CSCont.[RFC] or CteS.[RazonSocial] <> CSCont.[RazonSocial] or CteS.[CalleF] <> CSCont.[CalleF] or CteS.[NumExteriorF] <> CSCont.[NumExteriorF] or CteS.[NumInteriorF] <> CSCont.[NumInteriorF] 
		--or CteS.[ColoniaF] <> CSCont.[ColoniaF] or CteS.[LocalidadF] <> CSCont.[LocalidadF] or CteS.[PoblacionF] <> CSCont.[PoblacionF] or CteS.[EntidadF] <> CSCont.[EntidadF] 
		--or CteS.[PaisF] <> CSCont.[PaisF] or CteS.[TelefonosF] <> CSCont.[TelefonosF] or CteS.[CPF] <> CSCont.[CPF])

		Insert into [Contenedor].[RouteADM].DBO.ClientesImpuestos 
		Select CImp.IdCedis, CImp.IdCliente, CImp.IdSucursal, CImp.IdProducto, CImp.IdTipoImpuesto  
		from ClientesImpuestos CImp
		left outer join [Contenedor].[RouteADM].DBO.ClientesImpuestos CImpC on CImp.IdCedis = CImpC.IdCedis and CImp.IdCliente = CImpC.IdCliente 
			and CImp.IdSucursal = CImpC.IdSucursal and CImp.IdProducto = CImpC.IdProducto and CImp.IdTipoImpuesto = CImpC.IdTipoImpuesto 
		where CImpC.IdTipoImpuesto is null
	...
	begin
		...
		Select [IdCedis],[IdSurtido],[IdTipoVenta],[Folio],[Serie],[Fecha],[FechaAcredita],[Login],[FechaEntrega],[FolioEntrega],[Observaciones],[FolioCliente],[Remision],[Factura],[Status],[Vencimiento],[StatusTransferido]
