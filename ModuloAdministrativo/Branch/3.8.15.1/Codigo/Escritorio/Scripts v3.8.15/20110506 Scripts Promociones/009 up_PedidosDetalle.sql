...
declare @IdTipoImpuesto as int, @TipoAplicacion as int, @Jerarquia as int, @Impuestos as float, @Fecha as datetime
...
begin
	...
	delete from PedidosPromociones where IdCedis = @IdCedis and IdPedido = @IdPedido and IdProducto = @IdProducto
		
	if @Cantidad <> 0
	begin
		select @Fecha = FechaPedido from Pedidos where IdCedis = @IdCedis and IdPedido = @IdPedido 
	
		exec up_PedidosPromociones @IdCedis, @IdPedido, @IdTipoVenta, @Fecha, @IdProducto, @Cantidad, @Precio, 1
		...
		insert into PedidosDetalle values (@IdCedis, @IdPedido, @IdTipoVenta, @IdProducto, @Cantidad, @Precio, @Iva, @DctoPorc, @DctoImp, @Entregado)
