...
/****** Object:  StoredProcedure [dbo].[sp_importVentaCPGADM]    Script Date: 06/27/2011 18:26:01 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_importVentaCPGADM]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_importVentaCPGADM]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[sp_importVentaCPGADM] 
	@idCedis int, @Fecha as datetime, @idTipoVenta int, @Folio int, @Serie as varchar(10), 
	@TransprodId as varchar(16), @TransprodIdFactura as varchar(16), @VendedorId as varchar(16), @TipoNotaCredito as smallint
AS
BEGIN
	SET NOCOUNT ON;

--declare @idCedis int, @IdPedido int, @idTipoVenta int, @Folio int, @Serie as varchar(10), @TransprodId as varchar(16), @TransprodIdFactura as varchar(16), @VendedorId as varchar(16)
--set @idCedis = 1
--set @IdPedido = 2988
--set @idTipoVenta = 2
--set @Folio = 8
...
--set @VendedorId = 'CFDCed1'
 ...
 --   SELECT @SubTotal = Subtotal, @Iva = Iva, @idSucursal = IdSucursal, @DctoImp = DctoImp, @DiasCredito = isnull(DiasCredito,0) 
	--FROM RouteADM.dbo.Pedidos  
	--WHERE IdCedis = @idCedis AND IdPedido = @IdPedido -- IdSurtido=@IdPedido AND IdTipoVenta = @idTipoVenta AND Folio = @Folio and Serie = @Serie 
	select @DctoImp = SUM(VentasDetalle.DctoImp),
	@Subtotal = isnull(sum(VentasDetalle.Subtotal), 0), 
	@Iva = isnull(sum(VentasDetalle.subtotal*VentasDetalle.iva), 0)
	...
	select @idSucursal = 'CPG', @DiasCredito = 0
	...
	--	WHERE IdCedis = @idCedis AND IdSurtido=@IdPedido AND IdTipoVenta = @idTipoVenta AND Folio = @Folio and Serie = @Serie 	
	--END
	...
	--print convert(varchar(20),@idCedis) + ',' + convert(varchar(20),@IdPedido) + ',' + convert(varchar(20),@idTipoVenta) + ',' + convert(varchar(20),@Folio)
	...
	BEGIN
		...
		--IF(SELECT COUNT(*) FROM RouteADM.dbo.PedidosDetalle WHERE IdCedis = @idCedis AND IdPedido = @IdPedido)>0 --AND IdTipoVenta = @idTipoVenta AND Folio = @Folio AND Serie = @Serie 
		--BEGIN
			...
			BEGIN
				...
				declare  ActImpuestos cursor for
					...
					inner join RouteADM.dbo.VentasDetalle as VentasDetalle on Ventas.IdCedis = VentasDetalle.IdCedis and VentasDetalle.IdSurtido = Ventas.IdSurtido 
						and Ventas.IdTipoVenta = VentasDetalle.IdTipoVenta and VentasDetalle.Folio = Ventas.Folio  
					left outer join RouteADM.dbo.VentasImpuestos as VentasImpuestos on VentasImpuestos.IdCedis = VentasDetalle.IdCedis and VentasDetalle.IdSurtido = VentasImpuestos.IdSurtido 
					...
					left outer join RouteADM.dbo.FN_VentasContadoPOSTFactura(@IdCedis, @Fecha, @Fecha) FNPostFact on Ventas.IdCedis = FNPostFact.IdCedis and Ventas.IdSurtido = FNPostFact.IdSurtido 
						and Ventas.IdTipoVenta = FNPostFact.IdTipoVenta and Ventas.Folio = FNPostFact.Folio 
					where Ventas.IdCedis = @IdCedis and Ventas.Fecha = @Fecha and Ventas.IdTipoVenta = 1
						and FNFact.Folio is null and FNPostFact.Folio is null and VentasDetalle.IdProducto = @IdProducto
					group by VentasDetalle.IdProducto, IdTipoImpuesto, TipoAplicacion, Jerarquia, Tasa
					order by VentasDetalle.IdProducto, IdTipoImpuesto, TipoAplicacion, Jerarquia, Tasa
	...
	BEGIN
		...
		from RouteADM.dbo.ClienteSucursal as ClienteSucursal 
		where IdSucursal = 'CPG'
			
		select @FolioID = Folio.FolioID, @FosID = FosH.FOSId, @TipoVersion = dbo.FNObtenerDescVR('VERFACTE',@Lenguaje,SemHist.VersionCFD), @Aprobacion = Aprobacion, @AnioAprobacion = AnioAprobacion 
