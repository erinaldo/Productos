...
begin
        ...
        if @IdFamiliaRejas <> 0  -- producto reja
                ...
                from Rejas
                inner join Productos on Rejas.IdProducto = Productos.IdProducto and Productos.Status = 'A'
...
begin
	...
	SUM(Cantidad) as Unidades, SUM(Cantidad * Productos.Conversion) as Conversion, SUM(Cantidad * Precio) * (1+SURDET.Iva) as Importe
	from Cargas 
	...
	group by Cargas.IdCedis, Cargas.IdSurtido, Cargas.IdCarga, SURDET.Iva 

	union

	select SURDET.IdCedis, SURDET.IdSurtido, 'Dev. Buenas' as Concepto, '-' as Folio, -- isnull(SDEV.IdTipoDevolucion,0) as Folio, 
	SUM(DevBuena) as Unidades, SUM(DevBuena * Productos.Conversion) as Conversion, SUM(DevBuena * Precio) * (1+SURDET.Iva) as Importe
	...
	group by SURDET.IdCedis, SURDET.IdSurtido, SURDET.Iva 
	...
	SUM(DevMala) as Unidades, SUM(DevMala * Productos.Conversion) as Conversion, SUM(DevMala * Precio) * (1+SURDET.Iva) as Importe
	from SurtidosDetalle SURDET 
	inner join Productos on Productos.IdProducto = SURDET.IdProducto 
	...
	where SURDET.IdCedis = @IdCedis and SURDET.IdSurtido = @IdSurtido 
	group by SURDET.IdCedis, SURDET.IdSurtido, TipoMerma.Imputable, SURDET.Iva  
