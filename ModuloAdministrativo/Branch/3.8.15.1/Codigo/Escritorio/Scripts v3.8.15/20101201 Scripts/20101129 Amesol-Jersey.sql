...
select cast(VEN.IdCedis as varchar(2)) + cast(VEN.IdSurtido as varchar(10)) + VEN.Serie + CAST(VEN.Folio as varchar(10)) Numero, RIGHT(newid(),11) Operacion, VEN.Fecha FRegistro, VEN.Folio Remision, VEN.Fecha FRemision, 
...
VEN.Subtotal, VEN.Iva, VEN.Total, '' Fact_Impresa, case isnull(VA.FolioEntrega,'N') when 'N' then 'N' else 'S' end Acreditada,
...
'' as Estado_ie, '' as Cd, 0 as CargoDist 
from Ventas VEN
inner join Surtidos SUR on SUR.IdCedis = VEN.IdCedis and SUR.IdSurtido = VEN.IdSurtido 
left outer join VentasAcredita VA on VEN.IdCedis = VA.IdCedis and VEN.IdSurtido = VA.IdSurtido and VEN.Serie = VA.Serie and VEN.Folio = VA.Folio 
...
where VEN.IdCedis = @IdCedis and VEN.Fecha = @Fecha and VEN.IdTipoVenta in (1,2)
...
from Ventas VEN
inner join Surtidos SUR on SUR.IdCedis = VEN.IdCedis and SUR.IdSurtido = VEN.IdSurtido 
inner join VentasAcredita VA on VEN.IdCedis = VA.IdCedis and VEN.IdSurtido = VA.IdSurtido and VEN.Serie = VA.Serie and VEN.Folio = VA.Folio 
left outer join Configuracion CONF on CONF.IdCedis = VEN.IdCedis and CONF.SerieFacturasCredito = VEN.Serie
inner join SurtidosVendedor SURV on VEN.IdCedis = SURV.IdCedis and VEN.IdSurtido = SURV.IdSurtido and SURV.IdTipoVendedor in (1,2)
left outer join PreciosListaCliente PCTE on PCTE.IdCedis = VEN.IdCedis and PCTE.IdCliente = VEN.IdCliente 
left outer join PreciosListaRuta PRUTA on PRUTA.IdCedis = VEN.IdCedis and PRUTA.IdRuta = SUR.IdRuta 
where VEN.IdCedis = @IdCedis and VA.FechaAcredita = @Fecha and VEN.IdTipoVenta in (1,2)
...
VENDET.Entregado Recibidas, '' Promocion, 0 SubTotProm, 0 IvaPromo, 0 SubTPromDi, 0 IvaPromDis, '' PromocionCargo, 0 SubPromCgo
from Ventas VEN
inner join Surtidos SUR on SUR.IdCedis = VEN.IdCedis and SUR.IdSurtido = VEN.IdSurtido 
...
order by VEN.IdCedis, VEN.IdSurtido, VEN.Serie, VEN.Folio desc
