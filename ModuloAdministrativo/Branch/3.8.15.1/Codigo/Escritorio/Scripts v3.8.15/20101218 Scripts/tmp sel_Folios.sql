...
/****** Object:  StoredProcedure [dbo].[sel_Folios]    Script Date: 12/20/2010 12:26:13 ******/
...
AS
declare @IdTipo as bigint, @Serie as varchar(10), @FolioExiste as bigint, @FolioInicial as bigint
...
begin
	set @Serie = isnull( (select SerieRemisiones from Configuracion where IdCedis = @IdCedis ), '')
	set @Folio = ( select isnull( max(Ventas.Folio)+1, 1) from Ventas 
		where Ventas.IdCedis = @IdCedis and Serie = @Serie )
