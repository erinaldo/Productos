...
/****** Object:  StoredProcedure [dbo].[Gsp_NivelReporte]    Script Date: 04/03/2011 23:33:38 ******/
...
begin
	...
	begin		
		if @IdCedi <> '0'
			...
			exec ( 'select 0 as IdCedis, Clientes.IdCliente, isnull( ClientesTipo.Tipo, ''-'') as CTipo, 
					(select top 1 RazonSocial from Clientes CS where CS.IdCliente = Clientes.IdCliente) as RazonSocial, 

				isnull((select sum(Saldo)
				from FN_VencimientoAlaFechaG (''' + @FechaFinal + ''') 
				where FN_VencimientoAlaFechaG.IdCliente = Clientes.IdCliente 
				and DiasVencidos < 1 
				), 0) as ''0'', 
				...
				from Clientes 
				left outer join ClientesTipo on Clientes.IdCedis = ClientesTipo.IdCedis and Clientes.IdCliente = ClientesTipo.IdCliente
				where Clientes.IdCliente > 0 ' + @Filtro + ' 
				group by Clientes.IdCliente, ClientesTipo.Tipo
				having (isnull((select sum(Saldo)
				from FN_VencimientoAlaFechaG (''' + @FechaFinal + ''') 
				where FN_VencimientoAlaFechaG.IdCliente = Clientes.IdCliente
				), 0)) <> 0			
...
begin
	...
	begin	
		...
		if @IdCedi <> '0'
			...
			exec( '
			select 0 as IdCedis, '''' Cedis, Clientes.IdCliente, isnull( ClientesTipo.Tipo, ''-'') as CTipo, RazonSocial, 
			
			isnull(sum(SaldoInicial), 0) as ''INICIAL'', isnull(sum(Monto + Cargos), 0) as ''CARGOS'', isnull(sum(Abonos), 0) as ''ABONOS'',
			isnull(sum(SaldoInicial + Monto + Cargos - Abonos ), 0) as ''SALDO''
			from Clientes 
			left outer join ClientesTipo on Clientes.IdCedis = ClientesTipo.IdCedis and Clientes.IdCliente = ClientesTipo.IdCliente   
			left outer join FN_SaldosPorFechaG (''' + @FechaInicial + ''', ''' + @FechaFinal + ''') as FN_SaldosPorFecha on FN_SaldosPorFecha.IdCedis = Clientes.IdCedis and FN_SaldosPorFecha.IdCliente = Clientes.IdCliente
			inner join Cedis on Cedis.IdCedis = Clientes.IdCedis
			where Clientes.IdCliente > 0  ' + @Filtro + '  
			group by Clientes.IdCliente, RazonSocial, ClientesTipo.Tipo  
			having isnull(sum(SaldoInicial), 0) <> 0 or isnull(sum(Monto + Cargos), 0) <> 0 or isnull(sum(Abonos), 0) <> 0 -- isnull(sum(SaldoInicial + Monto + Cargos - Abonos ), 0)  <> 0 ')
	end
...
begin
	...
	begin	
		if @IdCedi <> '0'		
			exec( 'select FN_VentasPorCliente.IdCedis, FN_VentasPorCliente.IdCliente, isnull( ClientesTipo.Tipo, ''-'') as CTipo, RazonSocial, Grupo, Familia, 
			...
			group by FN_VentasPorCliente.IdCedis, FN_VentasPorCliente.IdCliente, RazonSocial, FN_VentasPorCliente.IdGrupo, Grupo, FN_VentasPorCliente.IdFamilia, Familia, FN_VentasPorCliente.IdProducto, FN_VentasPorCliente.Producto, Precio, ClientesTipo.Tipo 		
			...
			exec( 'select 0 as IdCedis, FN_VentasPorCliente.IdCliente, isnull( ClientesTipo.Tipo, ''-'') as CTipo, RazonSocial, Grupo, Familia, 
			FN_VentasPorCliente.IdProducto, FN_VentasPorCliente.Producto, sum(Cantidad) as Piezas, sum(Kilolitros) as Kilolitros, 
			sum(Total) as Importe, Precio as PrecioProm 
			from FN_VentasPorClienteG (''' + @FechaInicial + ''', ''' + @FechaFinal + ''') as FN_VentasPorCliente 
			left outer join ClientesTipo on FN_VentasPorCliente.IdCedis = ClientesTipo.IdCedis and FN_VentasPorCliente.IdCliente = ClientesTipo.IdCliente   
			inner join CLientes on Clientes.IdCedis = FN_VentasPorCliente.IdCedis and Clientes.IdCliente = FN_VentasPorCliente.IdCliente
			inner join Grupos on Grupos.IdGrupo = FN_VentasPorCliente.IdGrupo
			inner join Familias on Familias.IdFamilia = FN_VentasPorCliente.IdFamilia
			group by FN_VentasPorCliente.IdCliente, RazonSocial, FN_VentasPorCliente.IdGrupo, Grupo, FN_VentasPorCliente.IdFamilia, Familia, FN_VentasPorCliente.IdProducto, FN_VentasPorCliente.Producto, Precio, ClientesTipo.Tipo 		
			order by FN_VentasPorCliente.IdCliente, FN_VentasPorCliente.IdGrupo, FN_VentasPorCliente.IdFamilia, FN_VentasPorCliente.IdProducto')
	end
...
begin 
	...
	begin
		...
		WHERE (VACR.FechaAcredita BETWEEN @FechaInicial AND @FechaFinal) 
		ORDER BY Nombre 
	end

	if @Tipo = 10 or @Tipo = 11
	begin
		exec ( '		SELECT     CED.IdCedis, CED.Cedis, VEN.Fecha, VEN.IdCliente, CTE.RazonSocial AS Cliente, VEN.Serie, VEN.Folio, VEN.Serie + '' '' + cast(VEN.Folio as varchar(20)) as SerieFolio,
...
begin 
	...
	If @Tipo = 1 or @Tipo = 2
	begin
		SELECT Distinct VEN.IdCliente, CTE.RazonSocial AS Cliente
		FROM VentasAcredita VACR 
		inner join Ventas VEN on VACR.IdCedis = VEN.IdCedis and VACR.IdTipoVenta = VEN.IdTipoVenta
			and VEN.Serie = VACR.Serie and VEN.Folio = VACR.Folio  
		INNER JOIN Clientes CTE ON CTE.IdCedis = VEN.IdCedis AND VEN.IdCliente = CTE.IdCliente 		
		inner join ClienteSucursal on ClienteSucursal.IdCedis = VEN.IdCedis and ClienteSucursal.IdSucursal = VEN.IdSucursal		
		WHERE (VACR.FechaAcredita BETWEEN @FechaInicial and @FechaFinal) 
		ORDER BY VEN.IdCliente
	end

	if @Tipo = 10 or @Tipo = 11
	begin
		exec ( 'SELECT     VEN.Fecha, VEN.IdCliente as IdCliente, CTE.RazonSocial AS Cliente, 
			VEN.Subtotal AS [SubtotalF], VEN.Iva AS [IVAF], VEN.Total AS [TotalF],
			VACR.FolioEntrega, VACR.FolioCliente, VACR.Remision, VACR.Factura
		FROM VentasAcredita VACR 
		inner join Ventas VEN on VACR.IdCedis = VEN.IdCedis and VACR.IdTipoVenta = VEN.IdTipoVenta
			and VEN.Serie = VACR.Serie and VEN.Folio = VACR.Folio  
		INNER JOIN Clientes CTE ON CTE.IdCedis = VEN.IdCedis AND VEN.IdCliente = CTE.IdCliente 		
		inner join ClienteSucursal on ClienteSucursal.IdCedis = VEN.IdCedis and ClienteSucursal.IdSucursal = VEN.IdSucursal		
		WHERE (VACR.FechaAcredita BETWEEN ''' + @FechaInicial + ''' and ''' + @FechaFinal + ''') ' + @Filtro + ' 
		ORDER BY VEN.Fecha, VEN.IdCliente' )
	end

	if @Tipo = 20 or @Tipo = 21
	begin
		exec ( 'SELECT     VEN.Fecha, VEN.IdSucursal as IdCliente, ClienteSucursal.NombreSucursal AS Cliente, 
			VEN.Subtotal AS [SubtotalF], VEN.Iva AS [IVAF], VEN.Total AS [TotalF],
			VACR.FolioEntrega, VACR.FolioCliente, VACR.Remision, VACR.Factura
		FROM VentasAcredita VACR 
		inner join Ventas VEN on VACR.IdCedis = VEN.IdCedis and VACR.IdTipoVenta = VEN.IdTipoVenta
		...
		inner join ClienteSucursal on ClienteSucursal.IdCedis = VEN.IdCedis and ClienteSucursal.IdSucursal = VEN.IdSucursal		
		WHERE (VACR.FechaAcredita BETWEEN ''' + @FechaInicial + ''' and ''' + @FechaFinal + ''') ' + @Filtro + ' 
		ORDER BY VEN.Fecha, VEN.IdSucursal' )
	end
...
begin 
	...
	If @Tipo < 100
	begin
		SELECT Distinct IdCliente, RazonSocial AS Cliente
		FROM Clientes 
		ORDER BY IdCliente
	end
	...
	begin
		...
		inner join Documentos on Movimientos.IdDocumento = Documentos.IdDocumento 
		inner join DocumentosTipo on DocumentosTipo.IdTipoDocumento = MovimientosFacturas.IdTipoDocumento and DocumentosTipo.IdDocumento = MovimientosFacturas.IdDocumento
		where Folio.Status <> ''B'' and Folio.Fecha between ''' + @FechaInicial + ''' and ''' + @FechaFinal + ''' ' + @Filtro + '  ' + @Ordenamiento + ' 
...
begin 
	...
	begin
		...
		INNER JOIN Clientes CTE ON CTE.IdCedis = VEN.IdCedis AND VEN.IdCliente = CTE.IdCliente 		
		inner join ClienteSucursal on ClienteSucursal.IdCedis = VEN.IdCedis and ClienteSucursal.IdSucursal = VEN.IdSucursal		
		WHERE (VACR.FechaAcredita BETWEEN @FechaInicial and @FechaFinal) and VEN.IdCliente = 3500
		ORDER BY VEN.IdCliente
	end
	...
	begin
		...
		INNER JOIN Clientes CTE ON CTE.IdCedis = VEN.IdCedis AND VEN.IdCliente = CTE.IdCliente 		
		inner join ClienteSucursal on ClienteSucursal.IdCedis = VEN.IdCedis and ClienteSucursal.IdSucursal = VEN.IdSucursal		
		WHERE (VACR.FechaAcredita BETWEEN ''' + @FechaInicial + ''' and ''' + @FechaFinal + ''') AND VEN.IdCliente = ''3500'' ' + @Filtro + ' 
		ORDER BY VEN.Fecha, VEN.IdSucursal' )
	end
	
	if @Tipo = 11
	begin
		select LEYPROD.IdProductoLey 
		+ 
		'PA' + replicate('0',9-LEN(case VAC.Factura 
			when '' then VAC.Remision
			else VAC.Factura 
			end )) + 
			case VAC.Factura 
			when '' then VAC.Remision
		...
		+
		cast(cast(SUM(VENDET.Total) as decimal(9,2)) as varchar(10)) +
		replicate(' ',10-LEN(cast(cast(SUM(VENDET.Total) as decimal(9,2)) as varchar(10)))) --as Importe, 
		+
		replicate('0',8-LEN(case VAC.FolioCliente
			when '' then VAC.FolioEntrega
			else VAC.FolioCliente
			end)) + 
		case VAC.FolioCliente
			when '' then VAC.FolioEntrega
			else VAC.FolioCliente
			end --AS FolioCliente, 
		+
		REPLICATE ('0', 2-LEN(day(VEN.Fecha))) + CAST(day(VEN.Fecha) as varchar(2)) + '/' + 
		REPLICATE ('0', 2-LEN(MONTH(VEN.Fecha))) + CAST(MONTH(VEN.Fecha) as varchar(2)) + '/' + 
		cast(year(VEN.Fecha) as varchar(4)) -- Fecha
		+
		REPLICATE ('0', 13-LEN(LEYSUC.Ley)) + LEYSUC.Ley, -- SucursalLey
		VEN.Fecha 
		from Ventas VEN
		inner join VentasAcredita VAC on VAC.IdCedis = VEN.IdCedis and VAC.Serie = VEN.Serie
			and VAC.IdTipoVenta = VEN.IdTipoVenta and VAC.Folio = VEN.Folio
		inner join RouteCPC.dbo.LEYSucursal LEYSUC on LEYSUC.IdCliente = VEN.IdCliente and LEYSUC.IdSucursal = VEN.IdSucursal 
		inner join VentasDetalle VENDET on VENDET.IdCedis = VEN.IdCedis and VENDET.Serie = VEN.Serie
			and VENDET.IdTipoVenta = VEN.IdTipoVenta and VENDET.Folio = VEN.Folio
		inner join RouteCPC.dbo.LEYProductos LEYPROD on LEYPROD.IdSucursal = VEN.IdSucursal and LEYPROD.IdProducto = VENDET.IdProducto
		where VEN.IdCliente = 3500 and VEN.Fecha between @FechaInicial and @FechaFinal 
		group by VEN.Fecha, LEYPROD.IdProductoLey, VAC.Factura, VAC.Remision, VAC.FolioCliente, VAC.FolioEntrega, LEYSUC.Ley 
		order by VEN.Fecha, LEYPROD.IdProductoLey, VAC.Factura, VAC.Remision, VAC.FolioCliente, VAC.FolioEntrega, LEYSUC.Ley 
	end
