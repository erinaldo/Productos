...
/****** Object:  StoredProcedure [dbo].[sel_FacturasImpresion]    Script Date: 03/23/2011 11:46:39 ******/
...
begin
	...
	begin
		...
		where ' + @Filtro + ' 
		group by VentasDetalle.IdProducto, Productos.Producto, round(VentasDetalle.Precio,3)
...
begin
	if @Piezas = 1
	begin
		exec( 'SELECT Td.ProductoClave as ''Cve. Prod.'', PRO.Nombre as ''Producto'', sum(td.cantidad * PRD.Factor) as ''Cantidad'', round(TD.Precio,3) as ''Precio'', sum(td.subtotal) as ''SubTotal'', sum(td.impuesto) as ''Iva'', sum(td.total) as ''Total'', ''Piezas'' as Unidad
		...
		GROUP BY Td.ProductoClave, PRO.Nombre, round(TD.Precio,3)
	...
	begin
		...
		where ' + @Filtro + ' 
		AND T.Tipo = 1 AND T.TipoFase <> 0 AND T.TipoFase = 1
		GROUP BY Td.ProductoClave, PRO.Nombre, PR.Conversion, round(TD.Precio,3)
		ORDER BY cast(Td.ProductoClave as bigint), PRO.Nombre')
	end
end
...
begin
	...
	begin
		...
		sum(PedidosDetalle.Cantidad) as ''Cantidad'', round(PedidosDetalle.Precio,3) as ''Precio'', sum(PedidosDetalle.Subtotal) as ''Subtotal'', 
		...
		where ' + @Filtro + ' 
		group by PedidosDetalle.IdProducto, Productos.Producto, round(PedidosDetalle.Precio,3)
	...
	begin
		...
		inner join Productos on PedidosDetalle.IdProducto = Productos.IdProducto
		where ' + @Filtro + ' 
		group by PedidosDetalle.IdProducto, Productos.Producto, Productos.Conversion, round(PedidosDetalle.Precio,3)
		order by PedidosDetalle.IdProducto')
	end
end
...
if @Opc = 7
begin
	select '990000001' as IdSucursal, 'XAXX010101000' as RFCS, 'PUBLICO GENERAL' as RazonSocialF, 'BAJA CALIFORNIA NORTE, MÉXICO' as DomicilioS, 'PUBLICO GENERAL' as NombreSucursal
end

if @Opc = 10
begin
	declare @strCedis as varchar(8000), @strFecha as varchar(8000)
	
	if @Piezas = 0
	begin
		set @strCedis = '' set @strFecha = ''
	end
	if @Piezas = 1 -- Por Cedis
		set @strCedis = ' Ventas.IdCedis, '
	if @Piezas = 2 -- Por Fecha
		set @strFecha = ' Ventas.Fecha, '
	if @Piezas = 3 -- Por Cedis y Fecha
	begin
		set @strCedis = ' Ventas.IdCedis, ' set @strFecha = ' Ventas.Fecha, '
	end

	exec( 'select Ventas.IdCedis, Ventas.IdTipoVenta, Ventas.Serie, Ventas.Folio, Ventas.Fecha, Ventas.Total
	from Ventas 
	where ' + @Filtro + ' 
