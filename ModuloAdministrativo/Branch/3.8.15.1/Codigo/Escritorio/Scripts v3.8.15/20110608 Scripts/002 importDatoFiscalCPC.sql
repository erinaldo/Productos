...
/****** Object:  StoredProcedure [dbo].[sp_importDatoFiscalCPC]    Script Date: 06/09/2011 11:15:43 ******/
...
declare @idCedis2 int, @idTipoVenta2 int, @Folio2 int, @Serie2 as varchar(10), @fecha2 as datetime, @TransprodId2 as varchar(16), @Tipo as int
declare @Remision as varchar(20), @PedidoAdicional as varchar(20), @ClienteClave as varchar(20)

select @idCedis2 = @idCedis, @idTipoVenta2 = @idTipoVenta, @Serie2 = @Serie, @Folio2 = @Folio, @TransprodId2 = @TransprodId 
	
set @USUID = @VendedorId 

	DECLARE @PedidosFacturados AS CURSOR 
	SET @PedidosFacturados = CURSOR SCROLL STATIC  
	FOR
		select Vtas.IdCedis, Vtas.IdTipoVenta, Vtas.Serie, Vtas.Folio, 1, VentasFacturaCFD.TransprodID
		...
		left outer join RouteCPC.dbo.VentasFacturaCFD as VentasFacturaCFD on FacturasOxxo.IdCedis = VentasFacturaCFD.IdCedis and FacturasOxxo.IdTipoVenta = VentasFacturaCFD.IdTipoVenta  
			and FacturasOxxo.Serie = VentasFacturaCFD.Serie and FacturasOxxo.Folio = VentasFacturaCFD.Folio
		...
		select Vtas.IdCedis, Vtas.IdTipoVenta, Vtas.Serie, Vtas.Folio, 0, VentasFacturaCFD.TransprodID
		...
		left outer join RouteCPC.dbo.VentasFacturaCFD as VentasFacturaCFD on Vtas.IdCedis = VentasFacturaCFD.IdCedis and Vtas.IdTipoVenta = VentasFacturaCFD.IdTipoVenta  
			and Vtas.Serie = VentasFacturaCFD.Serie and Vtas.Folio = VentasFacturaCFD.Folio
	...
	OPEN @PedidosFacturados
	FETCH NEXT FROM @PedidosFacturados INTO @idcedis, @IdTipoVenta, @Serie, @Folio, @Tipo, @TransprodId
	...
	BEGIN
		...
		begin
			...
			update Route.dbo.TransProd set Folio = @Remision where TransProdID = @TransprodId 
		end
	...
	close @PedidosFacturados
	deallocate @PedidosFacturados		

	select @idCedis = @idCedis2, @idTipoVenta = @idTipoVenta2, @Serie = @Serie2, @Folio = @Folio2, @TransprodId = @TransprodId2 
		...
		select @FolioID = Folio.FolioID, @FosID = FosH.FOSId, @TipoVersion = '2.0', @Aprobacion = Aprobacion, @AnioAprobacion = AnioAprobacion, @Serie = FolS.Serie 
