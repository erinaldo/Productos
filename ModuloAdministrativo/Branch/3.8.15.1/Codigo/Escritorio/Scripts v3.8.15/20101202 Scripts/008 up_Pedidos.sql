...
/****** Object:  StoredProcedure [dbo].[up_Pedidos]    Script Date: 12/02/2010 19:35:18 ******/
...
begin
	...
	set @Folio = (select isnull(MAX(Folio) + 1, 1) from Ventas where IdCedis = @IdCedis and Serie not in (select SerieFacturasCredito from Configuracion where IdCedis = @IdCedis )
