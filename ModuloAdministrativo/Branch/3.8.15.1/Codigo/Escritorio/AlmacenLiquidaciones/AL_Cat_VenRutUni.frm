...
If LblEdicion.Caption = "Consultar" Then Exit Sub
    ...
    Select Case SSTab.Tab
        ...
        Case 1:
            ...
            StrCmd = "execute up_Vendedores " & IdCedis & ", " & Trim(TxtIdVendedor.Text) & ", '" & UCase(Trim(TxtNombre.Text)) & "', '" & UCase(Trim(TxtApPat.Text)) & "', '" & UCase(Trim(TxtApMat.Text)) & "', '" & UCase(Trim(TxtNomina.Text)) & "', " & TIPOVENDEDOR & ", '" & FormatDate(Date) & "', '" & UCase(Trim(TxtUsuario.Text)) & "', " & IIf(Nuevo, 1, 2)
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
            MuestraVendedor
            Nuevo = False
        ...
        Case 2:
            ...
            If Trim(LblEdicion.Caption) = "Nuevo" Then TxtIdRuta.Text = ValidaIdRuta
            ...
            StrCmd = "execute up_Rutas " & IdCedis & ", " & Trim(TxtIdRuta.Text) & ", '" & UCase(Trim(TxtRuta.Text)) & "', " & tiporuta & ", '" & Trim(cmbModelo.Text) & "', '" & ChkCFD.Value & "', '" & FormatDate(Date) & "'," & IIf(Nuevo, 1, 2)
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
            MuestraRutas
            Nuevo = False
...
On Error GoTo Err_CatRutasE:
    ...
    Select Case SSTab.Tab
        ...
        Case 2:
            ...
            StrCmd = "execute up_Rutas " & IdCedis & "," & TxtIdRuta.Text & ", '','','', '0', '19000101', 3"
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
            MuestraRutas
            Nuevo = False
...
On Error Resume Next
    ...
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
    LstDCanales = GetDataLVL(Rs, LstRuta, 0, 7, "0|0|0|0|0|0|0")
End Sub
...
On Error Resume Next
    ...
    MuestraUnidades
    ChkCFD.Enabled = IIf(CFDCedis = "1", False, True)
    SSTab.Tab = 3
    LLENA_COMBOS
End Sub
...
If LstRuta.ListItems.Count <= 0 Then Exit Sub
    ...
    End Select
    ChkCFD.Value = IIf(LstRuta.SelectedItem.ListSubItems(4) = "S", 1, 0)
    LblEdicion.Caption = "Consultar"
    Nuevo = False
End Sub
...
Private Sub TxtIdRuta_Validate(Cancel As Boolean)
On Error Resume Next
    TxtIdRuta.Text = ValidaIdRuta
End Sub
...
On Error Resume Next
    ...
    If Rs.State Then Rs.Close
    StrCmd = "exec up_vendedores 0, 0, '', '', '', '', 0, '19000101', '" & Trim(TxtUsuario.Text) & "', 4"
    Rs.Open StrCmd
    If Not Rs.EOF Then
...
Function ValidaIdRuta() As String
On Error Resume Next
    
ValidaIdRuta = ""
    StrCmd = "select IdRuta from Rutas where IdCedis = " & IdCedis & " and IdRuta = " & Trim(TxtIdRuta.Text)
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
    If Not Rs.EOF Then
        MsgBox "¡ Teclea un Número de Ruta válido !", vbInformation + vbOKOnly, App.Title
        TxtIdRuta.Text = ""
        TxtIdRuta.SetFocus
        Exit Function
    End If
ValidaIdRuta = Trim(TxtIdRuta.Text)
End Function
