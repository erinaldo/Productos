   ...
   Begin TabDlg.SSTab SSTab 
      ...
      Begin VB.Frame FrmOpt 
         ...
         Begin VB.CommandButton btnCalcularSaldo 
            ...
            Width           =   495
         End
         Begin VB.CheckBox ChkDevBuena 
            BackColor       =   &H00FFFFFF&
            Caption         =   "Aumentar Devolución Buena"
            BeginProperty Font 
               Name            =   "Arial"
               Size            =   9
               Charset         =   0
...
Public LstDEntradas
Dim Nuevo As Boolean, LstDTipo, LstDEntradasProds
...
On Error GoTo Err_ActInvInicialFisico:
    ...
    If Opc = 1 Then
        StrCmd = "execute up_InvFisico " & IdCedis & ", '" & FormatDate(Fecha) & "', " & Trim(TxtIdProdF.Text) & ", " & Trim(TxtCantidadF.Text) & ", " & IIf(ChkDevBuena.Value = 1, 1, 0) & ", 1"
    Else
    ...
    StrCmd = "execute up_Bitacora '" & Usuario & "', " & IdCedis & ", 0, 0, 0, 0, '', 0, '" & _
    FormatDate(Fecha) & "', 0, 0, '', " & Trim(TxtIdProdF.Text) & ", 'Actualizar', 'Almacén Inventario Físico. Producto " & Trim(TxtIdProdF.Text) & ", Cantidad = " & Trim(TxtCantidadF.Text) & "', 1"
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
...
'            End If
            ...
            StrCmd = "execute up_Bitacora '" & Usuario & "', " & IdCedis & ", 0, 0, " & Trim(LstDEntradas(1, UBound(LstDEntradas, 2))) & ", 0, '', 0, '" & _
            FormatDate(Fecha) & "', 0, 0, '', 0, 'Insertar', 'Entradas y Salidas Almacén', 1"
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
...
MousePointer = 11
    ...
    Select Case SSTab.Tab
        Case 0:
            ...
            StrCmd = "execute up_Bitacora '" & Usuario & "', " & IdCedis & ", 0, 0, " & CLng(LstEntradas.SelectedItem) & ", 0, '', 0, '" & _
            FormatDate(Fecha) & "', 0, 0, '', 0, 'Aplicar', 'Entradas y Salidas Almacén', 1"
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
        ...
        Case 1:
            ...
            StrCmd = "execute up_InvFisico " & IdCedis & ", '" & FormatDate(Fecha) & "', 0, 0, 0, 1"
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
            
            StrCmd = "execute up_Bitacora '" & Usuario & "', " & IdCedis & ", 0, 0, 0, 0, '', 0, '" & _
            FormatDate(Fecha) & "', 0, 0, '', 0, 'Aplicar', 'Almacén Inventario Físico', 1"
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
...
MousePointer = 11
    ...
    Select Case SSTab.Tab
        Case 0:
            ...
            StrCmd = "execute up_Bitacora '" & Usuario & "', " & IdCedis & ", 0, 0, " & CLng(LstEntradas.SelectedItem) & ", 0, '', 0, '" & _
            FormatDate(Fecha) & "', 0, 0, '', 0, 'Eliminar', 'Entradas y Salidas Almacén', 1"
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
        ...
        Case 1:
            ...
            StrCmd = "execute up_InvFisico " & IdCedis & ", '" & FormatDate(Fecha) & "', -1, 0, 0, 1"
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
            
            StrCmd = "execute up_Bitacora '" & Usuario & "', " & IdCedis & ", 0, 0, 0, 0, '', 0, '" & _
            FormatDate(Fecha) & "', 0, 0, '', 0, 'Eliminar', 'Almacén Inventario Físico', 1"
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
...
On Error GoTo Err_Importar:
    ...
    If DocumentoAplicado Then
        MsgBox "¡ No puedes modificar un Inventario Físico Aplicado !", vbInformation + vbOKOnly, App.Title
        Exit Sub
    End If
    
    If MsgBox("¿ Estás seguro que deseas Importar el Inventario desde archivo ?" & Chr(13) & Chr(10) & "Los Registros Actuales se ELIMINARÁN", vbQuestion + vbYesNo, App.Title) = vbNo Then Exit Sub
    
    StrCmd = "execute up_InventarioFisicoExcel " & IdCedis & ", '" & FormatDate(Fecha) & "'"
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
    CargaInventarioInicialFisico False, 1
...
On Error GoTo Err_ImpAlmacen:
    ...
    Select Case Me.SSTab.Tab
        ...
        Case 1:
            'CargaInventarioFisico
            CargaInventarioInicialFisico True, SSTab.Tab
            AL_RptAlmacenDocumentosIF.Line6.Y1 = AL_RptAlmacenDocumentosIF.Line6.Y1 - 200: AL_RptAlmacenDocumentosIF.Line6.Y2 = AL_RptAlmacenDocumentosIF.Line6.Y2 - 200
            AL_RptAlmacenDocumentosIF.ReportHeader.Height = AL_RptAlmacenDocumentosIF.ReportHeader.Height - 700
            AL_RptAlmacenDocumentosIF.LblTitulo.Caption = IIf(SSTab.Tab = 1, "INVENTARIO FÍSICO", "INVENTARIO INICIAL")
            AL_RptAlmacenDocumentosIF.LblCedis.Caption = "Centro de Distribución: " & IdCedis & " - " & NomCedis
            AL_RptAlmacenDocumentosIF.LblTipo.Caption = "Inventario Físico del " & Format(Fecha, "DDDD dd ""de"" MMMM ""de"" yyyy")
            AL_RptAlmacenDocumentosIF.LblDatos.Caption = ""
            AL_RptAlmacenDocumentosIF.LblStatus.Caption = "Estatus: " & IIf(DocumentoAplicado, "Aplicado", "Sin Aplicar")
        Case 2:
    ...
    If Not Rs.EOF Then
        If SSTab.Tab = 1 Then
            AL_RptAlmacenDocumentosIF.object.DataSrc.DataSourceName = Cnn
            AL_RptAlmacenDocumentosIF.object.DataSrc.Recordset = Rs
        Else
    ...
    If SSTab.Tab = 1 Then
        AL_RptAlmacenDocumentosIF.LblUsuario = "Usuario: " & Usuario
        AL_RptAlmacenDocumentosIF.Printer.Orientation = ddOPortrait
        AL_RptAlmacenDocumentosIF.Printer.PaperSize = 1
        AL_RptAlmacenDocumentosIF.Show
    Else
        AL_RptAlmacenDocumentos.LblUsuario = "Usuario: " & Usuario
...
On Error Resume Next
    ...
    StrCmd = "execute up_InvFisico " & IdCedis & ", '" & FormatDate(Fecha) & "', 0, " & IIf(ChkDevBuena.Value = 1, 1, 0) & ", 0, 2"
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
    
    StrCmd = "execute up_Bitacora '" & Usuario & "', " & IdCedis & ", 0, 0, 0, 0, '', 0, '" & _
    FormatDate(Fecha) & "', 0, 0, '', 0, 'Modifiar', '" & IIf(ChkDevBuena.Value = 1, "Sumar ", "No Considerar ") & " Devolución Buena para el Inventario Físico', 1"
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
...
On Error Resume Next
    ...
    If Opc = 1 Then
        If Not Rep Then LstDInvfisico = GetDataLVL(Rs, LstInvFisico, 2, 7, "0|0|9|9|9|0")
    Else
...
MousePointer = 11
    ...
    StrCmd = "execute up_Bitacora '" & Usuario & "', " & IdCedis & ", 0, 0, " & CLng(LstEntradas.SelectedItem) & ", 0, '', 0, '" & _
    FormatDate(Fecha) & "', 0, 0, '', " & Trim(TxtIdProdES.Text) & ", 'Actualizar', 'Entradas y Salidas Almacén. Producto " & Trim(TxtIdProdES.Text) & ", Cantidad = " & Trim(TxtCantidadES.Text) & "', 1"
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
