VERSION 5.00
Begin {82282820-C017-11D0-A87C-00A0C90F29FC} AL_RptRepLiquidacionSubEmpresa 
   Caption         =   "Liquidación de Rutas"
   ClientHeight    =   11430
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   18960
   Icon            =   "AL_RptRepLiquidacionSubEmpresa.dsx":0000
   StartUpPosition =   2  'CenterScreen
   WindowState     =   2  'Maximized
   _ExtentX        =   33443
   _ExtentY        =   20161
   SectionData     =   "AL_RptRepLiquidacionSubEmpresa.dsx":08CA
End
Attribute VB_Name = "AL_RptRepLiquidacionSubEmpresa"
...
Private Sub GroupFooter1_Format()
    ...
    While Not RsC.EOF
        Select Case RsC.Fields(0)
            Case 2: LblContado.Caption = FormatCurrency(RsC.Fields(3), 2, vbTrue)
            Case 1: LblCredito.Caption = FormatCurrency(RsC.Fields(3), 2, vbTrue)
            Case 99:
    ...
    If LstConfiguraLiquidacion(1) = "S" Then
        StrCmd = "execute sel_Denominacion " & IdCedis & ", " & FldSurtido.Text & ", 0, '', '" & FldIdMarca.Text & "', 4"
        ...
        If Not Rs2.EOF Then
            ...
            LblSaldo.Caption = FormatCurrency(Rs2.Fields(2) - TotalCobranza, 2, vbTrue)
            Select Case Rs2.Fields(2)
                Case 0: LblSaldoE.Caption = "Saldo"
                Case Else: LblSaldoE.Caption = IIf(Rs2.Fields(2) - TotalCobranza < 0, "Faltante", "Sobrante")
            End Select
            LblTotalE.Visible = True: LblEfectivo.Visible = True
            LblSaldoE.Visible = True: LblSaldo.Visible = True
       End If

        StrCmd = "execute sel_Denominacion " & IdCedis & ", " & FldSurtido.Text & ", 0, '', '" & FldIdMarca.Text & "', 5"
    ...
    StrCmd = "execute sel_SurtidosVendedores " & IdCedis & ", " & FldSurtido.Text
...
Private Sub ReportFooter_Format()
    ...
    If LstConfiguraLiquidacion(0) <> "N" Then
        StrCmd = "execute sel_ComisionesLiquidacion " & IdCedis & ", " & FldSurtido.Text & ", '" & FormatDate(FldFechaCorta) & "', '" & FormatDate(FldFechaCorta) & "'"
        If RsCom.State Then RsCom.Close
        RsCom.Open StrCmd, Cnn
    
        SubRptComisiones.object.GroupHeader1.Visible = IIf(LstConfiguraLiquidacion(0) = "G", False, True)
