...
'Amadeus
'*********************************************************************************************************************************************************************************************************************************************************************************
Function ValidaCobranzaAplicadaVentas(IdCed As Long, IdSurt As Long, Fec As Date, Ruta) As Boolean
If (Route) Then
        StrCmd = "select [Route].[dbo].[FNValidarCancelacionVentasADM](" & IdCed & "," & IdSurt & ",'" & FormatDate(Fec) & "'," & Ruta & ")"
        If Rs.State Then Rs.Close
        Rs.Open StrCmd, Cnn
        
        
         If Rs.Fields(0) = True Then
         MsgBox "¡ No puedes eliminar la liquidacion ya que existen abonos aplicados a las ventas de la liquidacion !", vbInformation + vbOKOnly, App.Title
         ValidaCobranzaAplicadaVentas = False
         MousePointer = 0
         Exit Function
        End If
...
End If
        ...
        StrCmd = "          select * from Ventas V"
        StrCmd = StrCmd & " inner join RouteCPC.dbo.MovimientosFacturas M on M.IdCedis=V.IdCedis and V.IdTipoVenta=M.IdTipoVenta and V.Serie=M.Serie collate SQL_Latin1_General_CP1_CI_AS and V.Folio=M.Folio "
        StrCmd = StrCmd & " where M.Status='A' and  V.IdCedis=" & IdCed & " and V.IdSurtido=" & IdSurt
        If Rs.State Then Rs.Close
        Rs.Open StrCmd, Cnn
...
MousePointer = 0
ValidaCobranzaAplicadaVentas = True
End Function
'*********************************************************************************************************************************************************************************************************************************************************************************
'Amadeus
'*********************************************************************************************************************************************************************************************************************************************************************************
Function ValidaCobranzaAplicadaVenta(IdCed As Long, IdSurt As Long, Folio, Serie, Fec As Date, Ruta) As Boolean
If (Route) Then
        StrCmd = " select [Route].[dbo].[FNValidarCancelacionVentaADM](" & Folio & ",'" & Serie & "','" & FormatDate(Fec) & "'," & Ruta & ") "
        If Rs.State Then Rs.Close
        Rs.Open StrCmd, Cnn
         ...
         If Rs.Fields(0) = True Then
         MsgBox "¡ No puedes eliminar la liquidacion ya que existen abonos aplicados a las ventas de la liquidacion !", vbInformation + vbOKOnly, App.Title
         ValidaCobranzaAplicadaVenta = False
         MousePointer = 0
         Exit Function
        End If
...
End If
        ...
        StrCmd = "          select * from Ventas V"
        StrCmd = StrCmd & " inner join RouteCPC.dbo.MovimientosFacturas M on M.IdCedis=V.IdCedis and V.IdTipoVenta=M.IdTipoVenta and V.Serie=M.Serie collate SQL_Latin1_General_CP1_CI_AS and V.Folio=M.Folio "
        StrCmd = StrCmd & " where M.Status='A' and V.IdCedis=" & IdCed & " and V.IdSurtido=" & IdSurt & "and V.Folio=" & Folio & " and V.Serie ='" & Serie & "'"
        If Rs.State Then Rs.Close
        Rs.Open StrCmd, Cnn
...
MousePointer = 0
ValidaCobranzaAplicadaVenta = True
End Function
