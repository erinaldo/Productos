...
On Error GoTo Err_btnArchivo:
    ...
    StrCmd = "execute sel_FacturasBusqueda " & CLng(LstCedis.SelectedItem) & ", '" & IdCte & "', '" & StrRutas & "', '" & Folio & "', '" & FormatDate(DTPFechaInicial.Value) & "', '" & FormatDate(DTPFechaFinal.Value) & "', " & Opc
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
    If Rs.EOF Then MsgBox "¡ Ventas no encontradas con los criterios seleccionados !", vbInformation + vbOKOnly, App.Title
    LstDFacturas = GetDataLVL(Rs, LstFacturas, 2, 14, "0|4|0|0|0|4|0|0|0|0|9|9|3")
...
On Error Resume Next
    ...
    StrCmd = "execute sel_ClientesFacturas " & CLng(LstCedis.SelectedItem) & ",'" & Trim(TxtIdCliente.Text) & "','" & Trim(TxtRFC.Text) & "','" & Trim(TxtRazonSocial.Text) & "', 0, 3"
