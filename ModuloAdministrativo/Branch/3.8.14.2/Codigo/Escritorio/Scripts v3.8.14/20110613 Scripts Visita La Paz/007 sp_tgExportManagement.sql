...
/****** Object:  StoredProcedure [dbo].[sp_tg_exportManagement]    Script Date: 06/17/2011 20:59:48 ******/
...
------------------------CAMBIOS DE PRODUCTO (Obsequios)--------------------------------------------------
                        ...
                        INNER JOIN (
                             ...
                             INNER JOIN ProductoDetalle PD ON PD.ProductoClave = TD.ProductoClave AND PD.ProductoDetClave = TD.ProductoClave AND TD.TipoUnidad = PD.PRUTipoUnidad
...
---------------------------------------------------------------------------------------------------------
                        ...
                        INNER JOIN vendedor VE on VE.VendedorId = V.VendedorId
                        INNER JOIN Usuario as Us on Us.USUId = VE.USUId
                        where TP.tipo=8 and TP.TipoFase <> 0 and  TP.DiaClave= @DiaClave and TF.TransProdId is null
                        AND((TP.MFechaHora between @FechaIn and @FechaFin) OR (@FechaIn = @FechaIniCompara AND TP.TipoFaseIntSal IN (0,2)))
                        AND  (Us.Clave = @vende)
                        ...
                        INNER JOIN vendedor VE on VE.VendedorId = V.VendedorId
                        INNER JOIN Usuario as Us on Us.USUId = VE.USUId
                        where TP.tipo=8 and TP.TipoFase <> 0 and TP.DiaClave= @DiaClave 
                        AND((TP.MFechaHora between @FechaIn and @FechaFin) OR (@FechaIn = @FechaIniCompara AND TP.TipoFaseIntSal IN (0,2)))
                        AND  (Us.Clave = @vende)
	  ...
	  update RouteADM.dbo.SurtidosDetalle set DevBuena = DevBuena - VenD.Cantidad
	  from RouteADM.dbo.SurtidosDetalle SurD, RouteADM.dbo.VentasDetalle VenD
	  where SurD.IdCedis = VenD.IdCedis and SurD.IdSurtido = VenD.IdSurtido and SurD.IdProducto = VenD.IdProducto 
		and SurD.IdCedis = @ClaveCEDI and SurD.IdSurtido = @Surtido 
		and VenD.Serie in ('EBQ', 'EBQX')
