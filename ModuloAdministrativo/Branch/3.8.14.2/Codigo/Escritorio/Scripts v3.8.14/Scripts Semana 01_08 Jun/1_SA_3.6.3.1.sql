...
dbo.FNObtenerCEDIADMInter(Al.Clave)as IdCedi,S.IdSurtido,isnull((select max(IdCheque) from RouteADM.dbo.SurtidosCheque),0) IdCheque,''as IdCajero,'MXP' as IdMoneda ,AD.TipoBanco,AD.Referencia,AD.Importe 
into #TablaTempSurtidosCheque
from Abono A
left join AbnTrp At on At.ABNId=A.ABNId
inner join ABNDetalle AD on A.ABNId=AD.ABNId
inner join Visita Vis on Vis.VisitaClave=A.VisitaClave 
inner join Vendedor V on V.VendedorID=Vis.VendedorID 
...
inner join almacen Al on Al.Almacenid=VC.Almacenid
inner join RouteADM.dbo.Surtidos S on  dbo.FNObtenerCEDIADMInter(Al.Clave)=cast(S.IdCedis AS VARCHAR(255)) and Fecha=CONvERT(DATETIME,CONvERT(VARCHAR(20),FechaAbono,111),111) and idruta=dbo.FNObtenerRutaADMInter(VR.RutClave) and S.Status='P'
where Tipopago<>1 and
((A.FechaAbono between @FechaIn and @FechaFin) OR (@FechaIn = @FechaIniCompara AND At.TipoFaseIntSal=0))
AND (Us.Clave = @vende)  

INSERT INTO RouteADM.dbo.SurtidosCheque
select IdCedi,IdSurtido,IdCheque+fila,IdCajero,IdMoneda,ISNULL(TipoBanco,'0'),Referencia,Importe FROM #TablaTempSurtidosCheque


drop TABLE #TablaTempSurtidosCheque
