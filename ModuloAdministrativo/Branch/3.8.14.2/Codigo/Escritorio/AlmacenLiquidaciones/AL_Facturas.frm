...
'*******************************************
'Amadeus
'*******************************************Agrege Variable Serie
Public IdTipoVenta, Folio, Serie, RFC, Nombre, Direccion, Tel, IdCliente, IdSucursal
...
'Amadeus
'*******************************************
If Not ValidaCobranzaAplicadaVenta(IdCedis, IdSurtido, Folio, Serie, Fecha, IdRuta) Then Exit Sub
    ...
    If MsgBox("¿ Está seguro que desea Eliminar la Factura " & Format(Folio, "0000000") & " del Cliente " & Chr(13) & Chr(10) & Nombre & " por " & LblTotal.Caption & " ?", vbQuestion + vbYesNo, App.Title) = vbYes Then
        ...
        MousePointer = 11
            StrCmd = "execute up_Ventas " & IdCedis & ", " & IdSurtido & ", " & IdTipoVenta & ", " & Folio & ", '', '" & FormatDate(Fecha) & "', 0, 0, 0, 3, 'btnEliminarVenta_Click'"
...
'*******************************************
            ...
            StrCmd = "execute Route.dbo.SP_CancelarVenta '" & Folio & "','" & Serie & "', '" & Format$(Fecha, "yyyy-MM-dd") & "', '" & Usuario & "'," & IdRuta
            If RsC.State Then RsC.Close
            RsC.Open StrCmd, Cnn
...
On Error Resume Next
    If Trim(TxtIdProd.Text) <> "" Then
        ...
        'Valido si el producto pertenece a la misma marca que el resto de productos de la venta
        StrCmd = "select IdMarca from Productos where IdProducto = " & TxtIdProd.Text
        If Rs.State Then Rs.Close
        Rs.Open StrCmd, Cnn
        If Not Rs.EOF Then
            ...
            StrCmd = "select Productos.IdMarca, Marcas.Marca from Productos "
            StrCmd = StrCmd + "inner join Marcas on Productos.IdMarca = Marcas.IdMarca "
            StrCmd = StrCmd + "where IdProducto = (select top 1 IdProducto from VentasDetalle where IdCedis = " & IdCedis & " and IdSurtido = " & IdSurtido & " and IdTipoVenta = " & IdTipoVenta & " and Folio = " & Folio & " and IdProducto <> " & TxtIdProd.Text & ")"
            
            Rs.Close
            Rs.Open StrCmd, Cnn
            
            Dim IdMarcaVta As String
            If Not Rs.EOF Then
                IdMarcaVta = Rs.Fields(0)
                If IdMarca <> IdMarcaVta Then
                    MsgBox "¡ El Producto " & TxtIdProd.Text & " no esta asociado a la Marca " & Rs.Fields(1) & ", a la cual pertenecen los productos previamente capturados !", vbInformation + vbOKOnly, App.Title
                    TxtDesc.Text = "¡ Producto no econtrado !": TxtCantidad.Text = 0: TxtPrecio.Text = 0: TxtSubTotal.Text = 0: TxtIva.Text = 0: TxtTotal.Text = 0: TxtExistencia.Text = 0
                    TxtIdProd.SetFocus
                    Exit Sub
                End If
