...
/****** Object:  StoredProcedure [dbo].[Gsp_NivelReporte]    Script Date: 11/29/2010 21:03:46 ******/
...
begin
	...
	begin			
			exec( '	select ''' + @FechaFinal + ''' as Fecha, IdVendedor, Nomina, Vendedor, SUM(Pago) as Comision, SUM(AbonoMerma) as AbonoMerma, SUM(AbonoVolumen) as AbonoVolumen, SUM(CargoMerma) as CargoMerma,
	...
	begin			
			exec( '	select FN_ComisionesDetalleFecha.Fecha, IdVendedor, Nomina, Vendedor, SUM(Pago) as Comision, SUM(AbonoMerma) as AbonoMerma, SUM(AbonoVolumen) as AbonoVolumen, SUM(CargoMerma) as CargoMerma,
				ISNULL((select top 1 VS.SaldoActual + isnull(SUM(VSFD.Monto),0) + isnull(VSV.Creditos,0) 
					from VendedoresSaldos VS
					left outer join VendedoresSaldosValida VSV on VS.IdCedis = VSV.IdCedis and VS.IdSurtido = VSV.IdSurtido and VS.IdVendedor = VSV.IdVendedor 
					left outer join VendedoresSaldosFinanciaD VSFD on VSFD.IdCedis = VS.IdCedis and VS.IdVendedor = VSFD.IdVendedor and VSFD.Fecha > VS.Fecha  
