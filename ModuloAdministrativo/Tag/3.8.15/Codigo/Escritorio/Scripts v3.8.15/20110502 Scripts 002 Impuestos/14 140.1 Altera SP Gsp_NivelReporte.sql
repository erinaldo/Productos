...
/****** Object:  StoredProcedure [dbo].[Gsp_NivelReporte]    Script Date: 05/03/2011 10:45:38 ******/
	...
	begin		
		exec('
			SELECT     SUR.IdCedis, SUR.IdRuta AS IdAgrupa, RUT.Ruta AS DescripcionAgrupa, PROD.IdFamilia, FAM.Familia, FAM.OrdenImpresion AS FamOrdenImpresion, 
			                      ...
			                      SUM(ISNULL(VVENCON.IVA, 0) + ISNULL(VVENCRED.IVA, 0)) AS Impuestos, SUM(ISNULL(VVENCON.Total, 0) + ISNULL(VVENCRED.Total, 0)) AS Total, 
			...
			SELECT     SUR.IdCedis, SUR.IdRuta AS IdAgrupa, RUT.Ruta AS DescripcionAgrupa, PROD.IdFamilia, FAM.Familia, FAM.OrdenImpresion AS FamOrdenImpresion, 
			                      ...
			                      AS [Dev. Buena], 0 AS [Dev. Mala], 0 AS Obsequios, 0 AS SubTotal, 0 AS Impuestos, 0 AS Total, 0 AS CreditoContado, 0 AS CargaKlts, 0 AS BuenaKlts, 
	...
	begin			
		exec('
			SELECT    SUR.IdCedis, SUR.IdRuta, CED.Cedis, RUT.Ruta, FAM.Familia, PROD.IdProducto AS [Cve. Prod.], PROD.Producto, 
					...
					SUM(ISNULL(VVENCON.IVA, 0) + ISNULL(VVENCRED.IVA, 0)) AS ImpPesos, 
	...
	begin			
		exec('
			SELECT     SUR.IdCedis, VEN.IdVendedor AS IdAgrupa, 
			                      ...
			                      0)) AS SubTotal, SUM(ISNULL(VVENCON.IVA, 0) + ISNULL(VVENCRED.IVA, 0)) AS Impuestos, SUM(ISNULL(VVENCON.Total, 0) + ISNULL(VVENCRED.Total, 0)) 
			...
			SELECT     SUR.IdCedis, VEN.IdVendedor AS IdAgrupa, TPVEN.TipoVendedor + '' - '' + VEN.ApPaterno + '' '' + VEN.ApMaterno + '' '' + VEN.Nombre AS DescripcionAgrupa,
			                      ...
			                      0 AS Impuestos, 0 AS Total, 0 AS CreditoContado, 0 AS CargaKlts, 0 AS BuenaKlts, 0 AS MalaKlts, 0 AS ObsequiosKlts, 0 AS ContadoKlts, 0 AS CreditoKlts, 
	...
	begin			
		exec('
			...
			SELECT     TOP 100 PERCENT SUR.IdCedis, VEN.IdVendedor, CED.Cedis, VEN.Nomina AS Nómina, 
			                      ...
			                      0 AS DevMalaKlts, 0 AS ObsequiosPzas, 0 AS ObsequiosKlts, 0 AS SubTotalPesos, 0 AS ImpPesos, 0 AS TotalPesos, PROD.IdFamilia, 
	...
	begin			--Pendiente por arreglar (agregar una vista que tenga los totales de venta por cliente, credito y contado.... cambiar el reporte)
		exec('	
			SELECT     VSUR.IdCedis, CTE.IdCliente AS IdAgrupa, CTE.RazonSocial AS DescripcionAgrupa, VSUR.IdFamilia, FAM.Familia, 
			                      ...
			                      ISNULL(SUM(VENDET.Cantidad * VENDET.Precio * VENDET.Iva), 0) AS Impuestos, ISNULL(SUM(VENDET.Total), 0) AS Total, 
					SUM(SURDET.VentaCredito) + SUM(SURDET.VentaContado) as CreditoContado,
					SUM(SURDET.Surtido) * PROD.Conversion as CargaKlts, SUM(SURDET.DevBuena) * PROD.Conversion as BuenaKlts, 
					SUM(SURDET.DevMala) * PROD.Conversion as MalaKlts,	SUM(SURDET.Obsequios) * PROD.Conversion as ObsequiosKlts, 
					SUM(SURDET.VentaContado) * PROD.Conversion as ContadoKlts, SUM(SURDET.VentaCredito) * PROD.Conversion as CreditoKlts,
	...
	begin			
		exec('
			SELECT     TOP 100 PERCENT SUR.IdCedis, CED.Cedis, VEN.IdCliente AS Número, CTE.RazonSocial AS Cliente, FAM.Familia, SURDET.IdProducto AS [Cve. Prod.], 
					...
					SUM(ISNULL(VVENCON.IVA, 0) + ISNULL(VVENCRED.IVA, 0)) AS Impuestos, 
	...
	begin			
		exec('
			...
			SELECT     SUR.IdCedis, PROD.IdMarca AS IdAgrupa, MAR.Marca AS DescripcionAgrupa, PROD.IdFamilia, FAM.Familia, 
			                      ...
			                      0 AS Credito, 0 AS Contado, SUM(REJ.Devolucion) AS [Dev. Buena], 0 AS [Dev. Mala], 0 AS Obsequios, 0 AS SubTotal, 0 AS Impuestos, 0 AS Total, 
	...
	begin			
		exec('
			...
			SELECT     TOP 100 PERCENT SUR.IdCedis, PROD.IdMarca, CED.Cedis, MAR.Marca, FAM.Familia,  PROD.IdProducto AS [Cve. Prod.], PROD.Producto, SUM(REJ.Surtido) AS CargaPzas, 
			                      ...
			                      0 AS SubTotalPesos, 0 AS ImpPesos, 0 AS TotalPesos, PROD.IdFamilia, FAM.OrdenImpresion AS FamOrdenImpresion, PROD.Conversion
			FROM         Surtidos SUR INNER JOIN
			                      Cedis CED ON SUR.IdCedis = CED.IdCedis INNER JOIN
			                      Rejas REJ ON SUR.IdCedis = REJ.IdCedis AND SUR.IdSurtido = REJ.IdSurtido AND SUR.IdRuta = REJ.IdRuta LEFT OUTER JOIN
			                      Familias FAM INNER JOIN
...
begin
	...
	begin	
		--exec up_ComisionesHistorico @IdCedi, @FechaInicial, @FechaFinal, '', 3
			
		exec( '	select * from tempComisiones 
			where IdCedis =  ' + @IdCedi + ' ' + @Filtro + ' and Fecha = ''' + @FechaFinal + '''
			order by Fecha, Vendedor ' )		
	end

	if @Tipo = 20 or @Tipo = 21		--Reporte 
	begin			
		--exec up_ComisionesHistorico @IdCedi, @FechaInicial, @FechaFinal, '', 4

		exec( '	select * from tempComisionesDetalle 
			where IdCedis =  ' + @IdCedi + ' ' + @Filtro + ' and Fecha between ''' + @FechaInicial + ''' and ''' + @FechaFinal + '''
			order by Fecha, Vendedor ' )		
	end
...
begin
	...
	begin
		SELECT     SUR.IdCedis, SUR.IdRuta, SUR.Fecha, SUR.IdSurtido AS Surtido, RUT.Ruta, ISNULL(VVTACONT.Subtotal, 0) AS [Venta Contado], ISNULL(VVTACONT.IVA, 
		                      0) AS Impuestos, ISNULL(VVTACONT.Total, 0) AS [Total Contado], ISNULL(VVTACRED.Subtotal, 0) AS [Venta Crédito], ISNULL(VVTACRED.IVA, 0) AS Impuestos, 
			...
			SELECT     SUR.IdCedis, SUR.IdRuta, SUR.Fecha, SUR.IdSurtido AS Surtido, RUT.Ruta, ISNULL( VVTACONT.Total, 0) AS [Venta Contado], 
				...
				- ISNULL( VVTACONT.Total, 0) AS [Diferencia], 						
						ISNULL(VVTACRED.Subtotal, 0) AS [Venta Crédito], ISNULL(VVTACRED.IVA, 0) 
								  AS [Imp Crédito], ISNULL(VVTACRED.Total, 0) AS [Total Crédito],
...
begin
	...
	begin
		SELECT     CED.IdCedis, CED.Cedis, VEN.Fecha, VEN.IdCliente AS [Número de Cliente], CTE.RazonSocial AS Nombre, SUR.IdRuta AS Ruta, VEN.Serie, VEN.Folio, 
		                      VEN.Subtotal AS [Subtotal Factura], VEN.Iva AS [Imp Factura], VEN.Total AS [Total Factura]
	...
	begin
		SELECT     CED.IdCedis, CED.Cedis, VEN.Fecha, VEN.IdCliente AS [Número de Cliente], CTE.RazonSocial AS Nombre, SUR.IdRuta AS Ruta, VEN.Serie, VEN.Folio, 
		                      VEN.Subtotal AS [Subtotal Factura], VEN.Iva AS [Imp Factura], VEN.Total AS [Total Factura], VENDET.IdProducto AS [Clave Producto], PRO.Producto, 
		                      VENDET.Cantidad, VENDET.Precio, VENDET.Subtotal AS [Subtotal Detalle], VENDET.Cantidad * VENDET.Precio * VENDET.Iva AS [Imp Detalle], 
...
begin
	...
	begin
		exec ('select PreciosDetalle.IdLista, PreciosLista.Descripcion, Productos.IdFamilia, Familia, PreciosDetalle.IdProducto, Producto, Precio, Iva as Impuesto, Precio * (1+iva) as Total
		from PreciosDetalle
	...
	begin
		...
		exec ('
			SELECT     PreciosDetalle.IdLista, Productos.IdFamilia, PreciosLista.Descripcion as Lista, Familias.Familia, PreciosDetalle.IdProducto as [Clave de Producto], Productos.Producto, 
			                      PreciosDetalle.Precio, Productos.Iva as Impuesto, PreciosDetalle.Precio * (1 + Productos.Iva) AS Total
...
begin
	...
	begin
		exec('
			SELECT     VENCTE.IdCedis, VENCTE.IdCliente, CTE.RazonSocial, VENCTE.IdProducto, VENCTE.Producto, VENCTE.IdFamilia, 
			                      ...
			                      AS CantidadKlts, SUM(VENCTE.IVA) AS Impuestos, SUM(VENCTE.Subtotal) AS SubTotal, SUM(VENCTE.Total) AS Total, 0 AS IdAgrupa, 
	...
	begin
		exec('
			SELECT     VENCTE.IdCedis, VENCTE.IdFamilia, FAM.OrdenImpresion, VENCTE.Conversion, CED.Cedis, VENCTE.IdCliente AS [Número de Cliente], 
			                      ...
			                      AS [Cantidad (Pzas)], SUM(VENCTE.Conversion * VENCTE.Cantidad) AS [Cantidad (Klts)], SUM(VENCTE.Subtotal) AS SubTotal, SUM(VENCTE.IVA) AS Impuestos, 
	...
	begin
		exec('
			SELECT     VENCTE.IdCedis, VENCTE.IdTipoVenta as IdAgrupa, VENTIP.TipoVenta as DescripcionAgrupa, VENCTE.IdCliente, CTE.RazonSocial, VENCTE.IdProducto,
			                      ...
			                      SUM(VENCTE.Conversion * VENCTE.Cantidad) AS CantidadKlts, SUM(VENCTE.IVA) AS Impuestos, SUM(VENCTE.Subtotal) AS SubTotal, SUM(VENCTE.Total) 
			                      AS Total
			FROM         FN_VentasPorClientePorFactura(' + @IdCedi + ', ''' + @FechaInicial + ''', ''' + @FechaFinal +''') VENCTE INNER JOIN
			                      Familias FAM ON VENCTE.IdFamilia = FAM.IdFamilia INNER JOIN
	...
	begin
		exec('
			SELECT     VENCTE.IdCedis, VENCTE.IdFamilia, FAM.OrdenImpresion, VENCTE.Conversion, VENCTE.IdTipoVenta, CED.Cedis, VENTIP.TipoVenta AS [Tipo de Venta],
			                      ...
			                      AS Impuestos, SUM(VENCTE.Subtotal) AS SubTotal, SUM(VENCTE.Total) AS Total
			FROM         FN_VentasPorClientePorFactura(' + @IdCedi + ', ''' + @FechaInicial + ''', ''' + @FechaFinal +''') VENCTE INNER JOIN
			                      Familias FAM ON VENCTE.IdFamilia = FAM.IdFamilia INNER JOIN
			                      VentasTipo VENTIP ON VENCTE.IdTipoVenta = VENTIP.IdTipoVenta INNER JOIN
			                      Clientes CTE ON VENCTE.IdCedis = CTE.IdCedis AND VENCTE.IdCliente = CTE.IdCliente INNER JOIN
	...
	begin
		exec('
			SELECT     VENCTE.IdCedis, VENCTE.IdTipoVenta, VENTIP.TipoVenta, VENCTE.Serie collate Traditional_Spanish_CI_AS + cast(VENCTE.Folio as varchar(15)) collate Traditional_Spanish_CI_AS as IdAgrupa, 
					VENCTE.Serie collate Traditional_Spanish_CI_AS + '' '' + cast(VENCTE.Folio as varchar(15)) collate Traditional_Spanish_CI_AS + '' ('' +  VENTIP.TipoVenta + '')'' collate Traditional_Spanish_CI_AS as DescripcionAgrupa, VENCTE.Fecha, VENCTE.IdCliente, CTE.RazonSocial,                       
					...
					SUM(VENCTE.IVA) AS Impuestos, SUM(VENCTE.Subtotal) AS SubTotal, SUM(VENCTE.Total) AS Total
			FROM         FN_VentasPorClientePorFactura(' + @IdCedi + ', ''' + @FechaInicial + ''', ''' + @FechaFinal +''') VENCTE INNER JOIN
			                      Familias FAM ON VENCTE.IdFamilia = FAM.IdFamilia INNER JOIN
			                      VentasTipo VENTIP ON VENCTE.IdTipoVenta = VENTIP.IdTipoVenta INNER JOIN
			                      Clientes CTE ON VENCTE.IdCedis = CTE.IdCedis AND VENCTE.IdCliente = CTE.IdCliente
	...
	begin
		exec('
			SELECT     VENCTE.IdCedis, VENCTE.IdTipoVenta, VENCTE.IdFamilia, FAM.OrdenImpresion, VENCTE.Conversion, CED.Cedis, 
			                      ...
			                      SUM(VENCTE.Conversion * VENCTE.Cantidad) AS [Cantidad (Klts)], SUM(VENCTE.Subtotal) AS SubTotal, SUM(VENCTE.IVA) AS Impuestos, SUM(VENCTE.Total) 
			                      AS Total
			FROM         FN_VentasPorClientePorFactura(' + @IdCedi + ', ''' + @FechaInicial + ''', ''' + @FechaFinal +''') VENCTE INNER JOIN
			                      Familias FAM ON VENCTE.IdFamilia = FAM.IdFamilia INNER JOIN
			                      VentasTipo VENTIP ON VENCTE.IdTipoVenta = VENTIP.IdTipoVenta INNER JOIN
	...
	begin
		exec('
			...
			FROM         FN_VentasPorSucursalPorFactura(' + @IdCedi + ', ''' + @FechaInicial + ''', ''' + @FechaFinal +''') VENCTE INNER JOIN
			                      Familias FAM ON VENCTE.IdFamilia = FAM.IdFamilia INNER JOIN
			                      Clientes CTE ON VENCTE.IdCedis = CTE.IdCedis AND VENCTE.IdCliente = CTE.IdCliente INNER JOIN
			                      ClienteSucursal CTESUC ON VENCTE.IdCedis = CTESUC.IdCedis AND 
	...
	begin
		exec('
			SELECT     VENCTE.IdCedis, VENCTE.IdSucursal, VENCTE.IdFamilia, FAM.OrdenImpresion, VENCTE.Conversion, CED.Cedis, 
					...
					SUM(VENCTE.Subtotal) AS SubTotal, SUM(VENCTE.IVA) AS Impuestos, SUM(VENCTE.Total) AS Total
			FROM         FN_VentasPorSucursalPorFactura(' + @IdCedi + ', ''' + @FechaInicial + ''', ''' + @FechaFinal +''') VENCTE INNER JOIN
			                      Familias FAM ON VENCTE.IdFamilia = FAM.IdFamilia INNER JOIN
			                      Clientes CTE ON VENCTE.IdCedis = CTE.IdCedis AND VENCTE.IdCliente = CTE.IdCliente INNER JOIN
			                      ClienteSucursal CTESUC ON VENCTE.IdCedis = CTESUC.IdCedis AND 
	...
	begin
		exec('
			SELECT     VENCTE.IdCedis, VENCTE.IdSucursal, VENCTE.IdFamilia, FAM.OrdenImpresion, VENCTE.Conversion, VENCTE.IdTipoVenta, CED.Cedis, 
			                      ...
			                      AS [Cantidad (Pzas)], SUM(VENCTE.Conversion * VENCTE.Cantidad) AS [Cantidad (Klts)], SUM(VENCTE.IVA) AS Impuestos, SUM(VENCTE.Subtotal) AS SubTotal, 
			                      SUM(VENCTE.Total) AS Total
			FROM         FN_VentasPorSucursalPorFactura(' + @IdCedi + ', ''' + @FechaInicial + ''', ''' + @FechaFinal +''') VENCTE INNER JOIN
			                      Familias FAM ON VENCTE.IdFamilia = FAM.IdFamilia INNER JOIN
			                      VentasTipo VENTIP ON VENCTE.IdTipoVenta = VENTIP.IdTipoVenta INNER JOIN
...
begin
	...
	begin
		...
		left join Route.dbo.Secuencia as Secuencia on Secuencia.RutClave = cast( Rutas.IdCedis as varchar(10) ) + replicate('0', 4 - len( Rutas.IdRuta ) ) + cast( Rutas.IdRuta as varchar(10) )
...
begin
	...
	begin
		exec( 'select VS.IdCedis, VS.IdSurtido, VS.Fecha, case VS.IdTipoSaldo 
				...
				order by V.ApPaterno, V.ApMaterno, V.Nombre, VS.Fecha, VS.IdSurtido ')
	end
...
begin
	...
	begin
		exec( 'SELECT  distinct   CED.IdCedis, CED.Cedis, VEN.Fecha, Marcas.IdMarca, Marcas.Marca, VEN.IdTipoVenta, case VEN.IdTipoVenta when ''1'' then ''Contado'' else ''Crédito'' end as TipoVenta, VEN.IdCliente AS [Número de Cliente], CTE.RazonSocial AS Nombre, SUR.IdRuta AS Ruta, VEN.Serie, VEN.Folio, 
								  VEN.Subtotal, VEN.Iva as Impuestos, VEN.Total, CS.IdSucursal, CS.NombreSucursal as Sucursal,
								 VENDET.IdProducto, PROD.Producto, SUM(VENDET.Cantidad) as Cantidad, 
								 SUM(VENDET.Subtotal) as SubTotalD, SUM(VENDET.Iva) as ImpD, SUM(VENDET.Total) as TotalD
			FROM         Ventas VEN INNER JOIN
								  Surtidos SUR ON SUR.IdCedis = VEN.IdCedis AND SUR.IdSurtido = VEN.IdSurtido INNER JOIN
...
begin
	...
	begin
		SELECT     CED.IdCedis, CED.Cedis, VEN.Fecha, VEN.IdCliente, CTE.RazonSocial AS Cliente, SUR.IdRuta, VEN.Serie, VEN.Folio, VEN.Serie + ' ' + cast(VEN.Folio as varchar(15)) as SerieFolio,
			VEN.IdSucursal, NombreSucursal, VEN.Subtotal AS [SubtotalF], VEN.Iva AS [ImpF], VEN.Total AS [TotalF],
