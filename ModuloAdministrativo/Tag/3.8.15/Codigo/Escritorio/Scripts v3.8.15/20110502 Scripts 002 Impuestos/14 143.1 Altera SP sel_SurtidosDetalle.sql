...
/****** Object:  StoredProcedure [dbo].[sel_SurtidosDetalle]    Script Date: 05/03/2011 15:49:35 ******/
...
begin
        ...
        Precio as 'Precio', Subtotal as 'Subtotal', Surtido*Precio*Productos.Iva as 'Impuestos', Total as 'Total'
        ...
        0 as 'Precio', 0 as 'Subtotal', 0 as 'Impuestos', 0 as 'Total'
        from Rejas 
...
begin
        ...
        0 as 'Precio', 0 as 'Subtotal', 0 as 'Impuestos', 0 as 'Total', 0 as Unidad, Surtido as Piezas
        from Rejas 
...
begin
        ...
        if @IdFamiliaRejas <> 0  -- producto reja
                select isnull(Rejas.IdCedis, @IdCedis) as IdCedis, Productos.IdProducto as 'Cve. Producto', Producto as 'Producto', isnull(Surtido,0) as 'Carga', 0 as 'Venta',
                isnull(Devolucion,0) as 'Dev. Buena', 0 as 'Dev. Mala', 0 as 'Obsequios', isnull(Surtido,0) - isnull(Devolucion,0) as 'Inv. Final', 
                0 as 'Precio', 0 as 'Subtotal', 0 as 'Impuestos', 0 as 'Total', Decimales, @IdFamiliaRejas,
                ...
                select SurtidosDetalle.IdCedis, SurtidosDetalle.IdProducto as 'Cve. Producto', Producto as 'Producto', Surtido as 'Carga', (VentaCredito + VentaContado) as 'Venta',
                DevBuena as 'Dev. Buena', DevMala as 'Dev. Mala', Obsequios as 'Obsequios', Surtido - VentaCredito - VentaContado - DevBuena - DevMala - Obsequios as 'Inv. Final', 
                Precio as 'Precio', Subtotal as 'Subtotal', Surtido*Precio*Productos.Iva as 'Impuestos', Total as 'Total', Decimales, @IdFamiliaRejas, 
                isnull(IdUnidad,'') as IdUnidad, isnull(Factor, 0) as Factor, isnull(Divide, '') as Divide,
				case Divide 
...
begin
        ...
        SELECT     SURDET.IdCedis, SURDET.IdProducto AS 'Cve. Prod.', PROD.Producto AS Producto, SURDET.Surtido AS Carga, 
                              ...
                              0 AS Obsequios, 0 AS Precio, 0 AS SubTotal, 0 AS Impuestos, 0 AS Total
...
if @Opc = 5
        ...
        select Tipo, isnull( sum(Subtotal),0) as 'Subtotal', isnull( sum(Cantidad * Precio * Iva),0) as 'Impuestos', isnull( sum(Total),0) as 'Total'
        from VentasTipo 
...
begin
        ...
        SELECT     SURDET.IdCedis, SURDET.IdProducto AS 'Cve. Prod.', PROD.Producto AS Producto, SURDET.Surtido AS Carga, 
                              ...
                              SURDET.Obsequios AS Obsequios, case sum(VENDET.Cantidad) when 0 then SURDET.Precio else 
                              isnull(sum(VENDET.Subtotal)/sum(VENDET.Cantidad), 0) end AS Precio, ISNULL(SUM(VENDET.Subtotal), 0) AS SubTotal, 
                              ISNULL(SUM(VENDET.Cantidad * VENDET.Precio * VENDET.Iva), 0) AS Impuestos, ISNULL(SUM(VENDET.Total), 0) AS Total, Marcas.IdMarca as IdMarca, Marcas.Marca as Marca, 
        ...
        SELECT     SURDET.IdCedis, SURDET.IdProducto AS 'Cve. Prod.', PROD.Producto AS Producto, SURDET.Surtido AS Carga, 
                              0 AS Credito, 0 AS Contado, SURDET.Devolucion AS 'Dev. Buena', 0 AS 'Dev. Mala', 
                              0 AS Obsequios, 0 AS Precio, 0 AS SubTotal, 0 AS Impuestos, 0 AS Total, Marcas.IdMarca as IdMarca, Marcas.Marca as Marca, 
                              SURDET.Fecha, SURDET.IdSurtido, Rutas.Ruta, case SUR.Status when 'A' then 'Aplicado' else 'Pendiente' end as Estatus
        FROM         Rejas SURDET LEFT OUTER JOIN
                              Productos PROD ON SURDET.IdProducto = PROD.IdProducto LEFT OUTER JOIN
                              VentasDetalle VENDET ON SURDET.IdCedis = VENDET.IdCedis AND SURDET.IdSurtido = VENDET.IdSurtido AND 
...
begin
	...
	select Tipo, isnull( sum(Subtotal),0) as 'Subtotal', isnull( sum(Cantidad * Precio * VentasDetalle.Iva),0) as 'Impuestos', isnull( sum(Total),0) as 'Total'
	from VentasTipo 
	inner join VentasDetalle on IdCedis = @IdCedis and IdSurtido = @IdSurtido and VentasDetalle.IdTipoVenta = VentasTipo.IdTipoVenta
	...
	select 99 as Tipo, 0 as Subtotal, 0 as Impuestos, sum(t.Total) as Total
		from (
