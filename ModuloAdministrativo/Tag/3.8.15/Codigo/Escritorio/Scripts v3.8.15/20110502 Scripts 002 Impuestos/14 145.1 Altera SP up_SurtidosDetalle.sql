...
/****** Object:  StoredProcedure [dbo].[up_SurtidosDetalle]    Script Date: 05/03/2011 15:52:17 ******/
...
begin
	...
	begin 
		...
		set @Iva = isnull( (Select sum(TIM.Tasa) from ProductosImpuestos PIM inner join TipoImpuesto TIM 
							on PIM.IdTipoImpuesto = TIM.IdTipoImpuesto where PIM.IdProducto = @IdProducto), 0)
		...
		if @SurtidoAnterior = 0 and @Surtido <> 0 
			begin
				...
				insert into SurtidosDetalle values (@IdCedis, @IdSurtido, @IdProducto, @Fecha, @Surtido, 0, 0, 0, 0, 0,@Precio, @Iva)
				update InventarioKardex set Surtido = Surtido + @Surtido where IdCedis = @IdCedis and Fecha = @Fecha and IdProducto = @IdProducto 
			end
		else
			begin
...
if @Opc = 3
	begin
		update SurtidosDetalle set DevBuena = Surtido - DevMala - Obsequios - VentaContado - VentaCredito 
