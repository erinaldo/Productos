...
/****** Object:  StoredProcedure [dbo].[sp_importPedidosADM]    Script Date: 06/27/2011 16:28:22 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_importPedidosADM]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_importPedidosADM]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[sp_importPedidosADM] 
	@idCedis int, @IdPedido int, @idTipoVenta int, @Folio int, @Serie as varchar(10), 
...
--set @VendedorId = 'CFDCed1'
	...
	SET @TipoFase = 2
    SELECT @Fecha = FechaPedido,  @SubTotal = Subtotal, @Iva = Iva, @idSucursal = IdSucursal, @DctoImp = DctoImp, @DiasCredito = isnull(DiasCredito,0) 
	...
	SELECT @idRuta = IdRutaPedido  FROM RouteADM.dbo.Pedidos WHERE IdCedis = @idCedis AND IdPedido = @IdPedido	
	...
	from RouteADM.dbo.VentasFacturaCFD 
	where IdCedis = @IdCedis and IdTipoVenta = @IdTipoVenta and IdPedido = @IdPedido 
	...
	BEGIN
		...
		VALUES(
			   ...
			   ,@Fecha,@Fecha,@Fecha,@FechaFacturacion,@Fecha,@FechaPrimera
			   ,0
		...
		BEGIN
			...
			FROM RouteADM.dbo.PedidosDetalle 
			WHERE IdCedis = @idCedis AND IdPedido = @IdPedido --AND IdTipoVenta = @idTipoVenta AND Folio = @Folio AND Serie = @Serie
	...
	BEGIN
		...
		from RouteADM.dbo.Pedidos as Ventas
		...
		where Ventas.IdCedis = @IdCedis and Ventas.IdPedido = @IdPedido -- and Ventas.IdTipoVenta = @IdTipoVenta and Ventas.Folio = @Folio 
