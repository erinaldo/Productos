...
/****** Object:  StoredProcedure [dbo].[sel_PromocionesAplicadasDetalle]    Script Date: 03/31/2011 11:15:49 ******/
...
if @Opc = 0
	Select PMD.IdCedis, Cedis, PMD.IdMovimiento as Folio, Folio.Fecha as Fecha, PMD.IdCliente, isnull(Tipo, '-') as CTipo, RazonSocial, Serie, PMD.Folio, PMD.Fecha as FechaFactura, Saldo, PMD.Monto, 
		--case Aplicado when 'A' then 'Aplicado' else Aplicado end as Aplicado, 
		Aplicado, PMD.IdDocumento, Documento, PMD.IdTipoDocumento, TipoDocumento
...
if @Opc = 1
	Select PMD.IdCedis, Cedis, PMD.IdMovimiento as Folio, PMDA.FechaInicial as Fecha, PMD.IdCliente, isnull(Tipo, '-') as CTipo, RazonSocial, Serie, PMD.Folio, PMD.Fecha as FechaFactura, Saldo, PMD.Monto, 
		--case Aplicado when 'A' then 'Aplicado' else Aplicado end as Aplicado, 
		Aplicado, PMD.IdDocumento, Documento, PMD.IdTipoDocumento, TipoDocumento
	from PromocionesAplicadasDetalle as PMD
	inner join PromocionesAplicadas PMDA on PMDA.IdPromocion = PMD.IdPromocion and PMDA.IdAplicacion = PMD.IdAplicacion 
	inner join Cedis on Cedis.IdCedis = PMD.IdCedis
	inner join Clientes on Clientes.IdCedis = PMD.IdCedis and PMD.IdCliente = Clientes.IdCliente
	left outer join ClientesTipo on Clientes.IdCedis = ClientesTipo.IdCedis and ClientesTipo.IdCliente = Clientes.IdCliente
	inner join Documentos on Documentos.IdDocumento = PMD.IdDocumento
	inner join DocumentosTipo on DocumentosTipo.IdTipoDocumento = PMD.IdTipoDocumento
	--inner join Folio on Folio.Folio = PMD.IdMovimiento 
	where PMD.IdAplicacion = @IdAplicacion and PMD.IdPromocion = @IdPromocion and PMD.Monto = 0
	order by PMD.IdCedis, PMD.IdCliente, FechaFactura
