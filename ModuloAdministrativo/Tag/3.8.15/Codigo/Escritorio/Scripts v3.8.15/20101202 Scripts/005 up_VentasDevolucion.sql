...
/****** Object:  StoredProcedure [dbo].[up_VentasDevoluciones]    Script Date: 12/02/2010 17:17:42 ******/
...
declare @SerieD as varchar(6), @Serie as varchar(6)
...
begin 
	...
	from Ventas 
	where IdCedis = @IdCedis and Serie not in (select SerieFacturasCredito from Configuracion where IdCedis = @IdCedis )
		and Serie not in (select SerieFacturasContado from Configuracion where IdCedis = @IdCedis )
