...
/****** Object:  StoredProcedure [dbo].[GeneraSaldos]    Script Date: 06/15/2011 12:35:13 ******/
...
declare VentasACPC cursor for
	...
	fetch next from VentasACPC into @IdCedis, @IdTipoVenta, @Folio, @Serie, @Fecha, @IdCliente, @Subtotal, @Iva, @DiasCredito, @IdSucursal, @DctoImp, @IdSurtido
		...
		begin
			...
			select IdCedis, IdTipoVenta, @Serie, Folio, IdProducto, IdTipoImpuesto, TipoAplicacion, Jerarquia, Tasa 
	...
	if @IdMovimiento is not null and @IdMovimiento > 0
		exec up_Folio @IdMovimiento, '19000101', 0, '', '', 'SUPER', 5

	deallocate VentasACPC

	-- VALIDA ACREDITACIONES
	select top 1 @FechaFinal = Fecha from RouteADM.dbo.StatusDia where Status = 'C' order by Fecha desc	
	
	declare InsertaVentasAcreditadasPendientes cursor for
		Select VAC.IdCedis, VAC.IdSurtido, VAC.IdTipoVenta, VAC.Folio, VAC.Serie
		...
		From RouteADM.DBO.VentasAcredita VAC
		left outer join VentasAcredita VACC on VACC.IdCedis = VAC.IdCedis and VACC.Serie = VAC.Serie 
	...
	fetch next from InsertaVentasAcreditadasPendientes into @IdCedis, @IdSurtido, @IdTipoVenta, @Folio, @Serie 
	while @@fetch_status = 0
	begin
		
		delete from VentasAcredita where IdCedis = @IdCedis and Serie = @Serie and IdTipoVenta = @IdTipoVenta and Folio = @Folio 
		
		Insert Into VentasAcredita
		Select [IdCedis],[IdTipoVenta],[Folio],[Serie],[Fecha],[FechaAcredita],[Login],[FechaEntrega],[FolioEntrega],[Observaciones],[FolioCliente],[Remision],[Factura],[Status],[Vencimiento]
		From RouteADM.DBO.VentasAcredita where IdCedis = @IdCedis and IdSurtido = @IdSurtido and IdTipoVenta = @IdTipoVenta and Folio = @Folio 
