...
/****** Object:  Trigger [tgRouteU_ClienteSucursal]    Script Date: 06/15/2011 21:04:08 ******/
...
/****** Object:  Trigger [dbo].[tgRouteU_ClienteSucursal]    Script Date: 06/15/2011 21:04:08 ******/
SET ANSI_NULLS ON
GO
...
-- inserta cliente
	insert into ROUTE.dbo.Cliente ( ClienteClave, CNPId, Clave, IdElectronico, IdFiscal, RazonSocial, TipoFiscal, TipoImpuesto, NombreContacto, TelefonoContacto, FechaRegistroSistema, FechaNacimiento, LimiteCreditoDinero, NombreCorto, TipoEstado, LimiteDescuento, SaldoEfectivo, Prestamo, Exclusividad, VigExclusividad, ActualizaSaldoCheque, VencimientoVenta, DiasVencimiento, FechaFactura, DesgloseImpuesto, CorreoElectronico, MFechaHora, MUsuarioID ) 
		select cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal, null, cast (case IdCedis when 1 then 'A' 
			when 2 then 'B' 
			when 3 then 'C' 
			when 4 then 'D' 
			when 5 then 'E'
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal, left(CodigoBarras, 64), left( RFC, 64), left( RazonSocial, 128), 1, 1, left(Contacto, 64), left( Telefonos, 64), getdate(), '20000101', 0, left( NombreSucursal, 32), 1, 0, 0, 0, 0, '20991231', 1, 0, 0, '20991231', 0, '', getdate(), 'Interfaz' 
	from Inserted where cast(cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal as varchar(16))  not in ( select ClienteClave from ROUTE.dbo.Cliente where  ClienteClave = cast(cast (case IdCedis when 1 then 'A' 
			when 2 then 'B' 
			when 3 then 'C' 
			when 4 then 'D' 
			when 5 then 'E'
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal as varchar(16)) )
	...
	where ClienteClave = cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal 
...
-- inserta domicilio de entrega
	insert into ROUTE.dbo.ClienteDomicilio ( ClienteClave, ClienteDomicilioID, Tipo, Calle, Numero, NumInt, CodigoPostal, ReferenciaDom, Colonia, Localidad, Poblacion, Entidad, Pais, GLN, MFechaHora, MUsuarioID )
		select cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal, 'D-02', 2, left( Calle, 64), left(NumExterior, 16), left( NumInterior, 16), left( CP, 16), '', left(Colonia, 64), left(Localidad, 40), left( Poblacion, 64), left(Entidad, 32), left(Pais, 32), left(TDA_GLN,13), getdate(), 'Interfaz' from Inserted where cast(cast (case IdCedis when 1 then 'A' 
			when 2 then 'B' 
			when 3 then 'C' 
			when 4 then 'D' 
			when 5 then 'E'
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal as varchar(16))  not in ( select ClienteClave 
	from ROUTE.dbo.ClienteDomicilio where  ClienteClave = cast(cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal as varchar(16)) and ClienteDomicilioId = 'D-02' )
	...
	where ClienteClave = cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal and Tipo = 2
...
-- inserta domicilio fiscal
	insert into ROUTE.dbo.ClienteDomicilio ( ClienteClave, ClienteDomicilioID, Tipo, Calle, Numero, NumInt, CodigoPostal, ReferenciaDom, Colonia, Localidad, Poblacion, Entidad, Pais, GLN, MFechaHora, MUsuarioID )
		select cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal, 'D-01', 1, left( CalleF, 64), left(NumExteriorF, 16), left( NumInteriorF, 16), left( CPF, 16), '', left(ColoniaF, 64), left(LocalidadF, 40), left( PoblacionF, 64), left(EntidadF, 32), left(PaisF, 32), left(TDA_GLN,13), getdate(), 'Interfaz' from Inserted where cast(cast (case IdCedis when 1 then 'A' 
			when 2 then 'B' 
			when 3 then 'C' 
			when 4 then 'D' 
			when 5 then 'E'
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal as varchar(16))  not in ( select ClienteClave from ROUTE.dbo.ClienteDomicilio where  ClienteClave = cast(cast (case IdCedis when 1 then 'A' 
			when 2 then 'B' 
			when 3 then 'C' 
			when 4 then 'D' 
			when 5 then 'E'
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal as varchar(16)) and ClienteDomicilioId = 'D-01' )
	update ROUTE.dbo.ClienteDomicilio 
	...
	where ClienteClave = cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal  and Tipo = 1
...
-- inserta forma de venta
	insert into ROUTE.dbo.CliFormaVenta ( ClienteClave, CFVTipo, LimiteCredito, DiasCredito,Inicial, CapturaDias, ValidaLimite, ValidaPago, Estado, MFechaHora, MUsuarioID )
		select cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal, '1', 0, 0, 0, 0, 0, 0, 1, getdate(), 'Interfaz' from Inserted where cast(cast (case IdCedis when 1 then 'A' 
			when 2 then 'B' 
			when 3 then 'C' 
			when 4 then 'D' 
			when 5 then 'E'
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal as varchar(16))  not in ( select ClienteClave from ROUTE.dbo.CliFormaVenta where ClienteClave = cast(cast (case IdCedis when 1 then 'A' 
			when 2 then 'B' 
			when 3 then 'C' 
			when 4 then 'D' 
			when 5 then 'E'
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal as varchar(16)) and CFVTipo = '1' )
	insert into ROUTE.dbo.CliFormaVenta ( ClienteClave, CFVTipo, LimiteCredito, DiasCredito,Inicial, CapturaDias, ValidaLimite, ValidaPago, Estado, MFechaHora, MUsuarioID )
		select cast (case IdCedis when 1 then 'A' 
			when 2 then 'B' 
			when 3 then 'C' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal, '2', 99999, 30, 1, 0, 0, 0, 1, getdate(), 'Interfaz' from Inserted where FormaVenta in (2) and cast(cast (case IdCedis when 1 then 'A' 
			when 2 then 'B' 
			when 3 then 'C' 
			when 4 then 'D' 
			when 5 then 'E'
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal as varchar(16)) and CFVTipo = '2' )
	...
	update ROUTE.dbo.CliFormaVenta set Estado = 1, Inicial = 0 where CFVTipo = '1' and ClienteClave in ( select cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal from Inserted )
	update ROUTE.dbo.CliFormaVenta set Inicial = 1 where CFVTipo = '1' and ClienteClave in ( select cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal from Inserted where FormaVenta = 1 )
	update ROUTE.dbo.CliFormaVenta set Estado = 1, Inicial = 1 where CFVTipo = '2' and ClienteClave in ( select cast (case IdCedis when 1 then 'A' 
			...
			when 9 then 'I'
			when 10 then 'J'
			else '' end as varchar(10))+IdSucursal from Inserted where FormaVenta = 2 )
