...
/****** Object:  StoredProcedure [dbo].[sp_importVentasCPC]    Script Date: 06/16/2011 09:29:35 ******/
...
--declare @idCedis int, @idTipoVenta int, @Folio int, @Serie as varchar(10), @TransprodId as varchar(16), @TransprodIdFactura as varchar(16), @VendedorId as varchar(16), @TipoNotaCredito as int
--set @idCedis = 1
--set @idTipoVenta = 2
--set @Folio = 311246
--set @Serie = 'RE'
--set @TransprodId = 'B206C5109D2D4AD9'
--set @TransprodIdFactura = 'B224F8C702033403'
...
--set @TipoNotaCredito = 0
	...
	begin			
		select top 1 @CedId = ClienteSucursal.IdCedis, @idSucursal = ClienteSucursal.IdSucursal
	...
	if LEN(@Folio) <= @Formato 
		SET @FolioRoute = @Serie +  REPLICATE('0',@Formato - LEN(@Folio)) + cast(@Folio as varchar(20))
	else
		SET @FolioRoute = @Serie +  cast(@Folio as varchar(20))
	--RIGHT('0000000'+CONVERT(VARCHAR(8),@Folio),7)
	
	--print convert(varchar(20),@idCedis) + ',' + convert(varchar(20),@Serie) + ',' + convert(varchar(20),@idTipoVenta) + ',' + convert(varchar(20),@Folio)
	...
	BEGIN
		...
		SELECT @TransprodIdFactura, FolioEntrega, FolioCliente, Remision, GETDATE(), @USUID 
