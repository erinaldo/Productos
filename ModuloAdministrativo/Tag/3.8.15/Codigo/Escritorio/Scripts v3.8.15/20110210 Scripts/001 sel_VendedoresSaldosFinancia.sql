...
/****** Object:  StoredProcedure [dbo].[sel_VendedoresSaldosFinancia]    Script Date: 02/10/2011 16:23:44 ******/
...
if @Opc = 6
begin
	select top 1 abs(VS.SaldoActual), isnull(( select sum(VSFD.Monto)
		from VendedoresSaldosFinancia VSF 
		left outer join VendedoresSaldosFinanciaD VSFD on VSF.IdCedis = VSFD.IdCedis and VSF.IdCorto = VSFD.IdCorto and VSFD.Status = 'A' and VSFD.Fecha >= convert(date,GETDATE(),112)
		where VSF.IdCedis = VS.IdCedis and VSF.IdVendedor = VS.IdVendedor and VSF.Status = 'A' ),0) as NoVencido,
		abs(VS.SaldoActual) - isnull(( select sum(VSFD.Monto)
		from VendedoresSaldosFinancia VSF 
		left outer join VendedoresSaldosFinanciaD VSFD on VSF.IdCedis = VSFD.IdCedis and VSF.IdCorto = VSFD.IdCorto and VSFD.Status = 'A' and VSFD.Fecha >= convert(date,GETDATE(),112)
		where VSF.IdCedis = VS.IdCedis and VSF.IdVendedor = VS.IdVendedor and VSF.Status = 'A' ),0) as Saldo
