...
/****** Object:  StoredProcedure [dbo].[sp_importDatoFiscalPedidosADM]    Script Date: 05/04/2011 20:26:27 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_importDatoFiscalPedidosADM]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_importDatoFiscalPedidosADM]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[sp_importDatoFiscalPedidosADM] 
...
declare @USUID as varchar(20)
			...
			WHERE IdCedis = @idCedis AND IdPedido = @idSurtido -- AND IdTipoVenta = @idTipoVenta AND Folio = @Folio AND Serie = @Serie
			...
			BEGIN
				...
				declare  ActImpuestos cursor for
					...
					where VtasImp.IdCedis = @IdCedis and VtasImp.IdPedido = @IdSurtido and VtasImp.IdProducto = @IdProducto --and VtasImp.IdTipoVenta = @IdTipoVenta and VtasImp.Folio = @Folio 
		...
		where Ventas.IdCedis = @IdCedis and Ventas.IdPedido = @IdSurtido -- and Ventas.IdTipoVenta = @IdTipoVenta and Ventas.Folio = @Folio 
