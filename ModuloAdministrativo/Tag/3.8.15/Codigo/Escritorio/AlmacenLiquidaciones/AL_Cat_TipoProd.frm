...
Begin VB.Form AL_Cat_TipoProd 
   ...
   Begin TabDlg.SSTab SSTab 
      ...
      Begin VB.Frame FrmOpt 
         ...
         Begin VB.ComboBox CmbResponsable 
            ...
            EndProperty
            Height          =   360
            ItemData        =   "AL_Cat_TipoProd.frx":1E87
            Left            =   9960
            List            =   "AL_Cat_TipoProd.frx":1E91
            Style           =   2  'Dropdown List
            TabIndex        =   8
            Top             =   480
            Width           =   2055
         End
         Begin VB.OptionButton OptAplica 
            BackColor       =   &H00FFFFFF&
            Caption         =   "No"
            BeginProperty Font 
               Name            =   "Arial"
         ...
         Begin VB.Label Label7 
            AutoSize        =   -1  'True
            BackColor       =   &H00FFFFFF&
            Caption         =   "Descuenta de Liquidación"
            BeginProperty Font 
               Name            =   "Arial"
               Size            =   9.75
               Charset         =   0
   ...
   Begin VB.Image Image1 
      Height          =   30
      Left            =   120
      Picture         =   "AL_Cat_TipoProd.frx":1EA6
      Stretch         =   -1  'True
      Top             =   495
      Visible         =   0   'False
      Width           =   13740
   ...
   Begin VB.Image Image2 
      Height          =   285
      Left            =   10560
      Picture         =   "AL_Cat_TipoProd.frx":24E3
      Top             =   135
      Visible         =   0   'False
      Width           =   3315
   End
...
If LblEdicion.Caption = "Consultar" Then Exit Sub
    ...
    Select Case SSTab.Tab
        ...
        Case 1:
            ...
            If Rs.State Then Rs.Close
            Rs.Open "SELECT * FROM TIPOMERMA WHERE ORDEN = " & Trim(TxtOrden.Text) & " and IdTipoMerma <> " & Trim(TxtIdMerma.Text), Cnn
            If Not Rs.EOF Then MsgBox ("Ya existe este número de orden para el Tipo de Devolución")
            
            StrCmd = "execute up_TipoMerma " & Trim(TxtIdMerma.Text) & ",'" & UCase(Trim(TxtMerma.Text)) & "', " & Trim(TxtOrden.Text) & ",'" & FormatDate(Date) & "', " & IIf(OptAplica(0), 1, 0) & ", " & Mid(CmbResponsable.Text, 1, 1) & ", " & IIf(Nuevo, 1, 2)
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
            MuestraTipoMerma
            Nuevo = False
...
On Error GoTo Err_TProdsE:
    ...
    Select Case SSTab.Tab
        ...
        Case 1:
            ...
            StrCmd = "execute up_TipoMerma  " & Trim(TxtIdMerma.Text) & ",'','','',0,'',3"
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
            MuestraTipoMerma
            Nuevo = False
...
On Error GoTo Err_TProdsN:
    ...
    Select Case SSTab.Tab
        ...
        Case 1:
            TxtIdMerma.Text = "": TxtMerma.Text = "": TxtOrden.Text = "": CmbResponsable.Text = "Vendedor"
            If Rs.State Then Rs.Close
...
On Error Resume Next
    ...
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
    LstDCanales = GetDataLVL(Rs, LstTipoMerma, 0, 6, "1|0|0|0|0|0|0")
End Sub
...
Private Sub LstPresentacion_ItemClick(ByVal Item As MSComctlLib.ListItem)
On Error Resume Next
...
Private Sub LstTipoMerma_ItemClick(ByVal Item As MSComctlLib.ListItem)
On Error Resume Next
...
If LstTipoMerma.ListItems.Count <= 0 Then Exit Sub
    ...
    If LstDCanales(5, LstTipoMerma.SelectedItem.Index - 1) = "S" Then
        OptAplica(0).Value = True
    Else
        OptAplica(1).Value = True
    End If
    CmbResponsable.Text = IIf(Mid(LstDCanales(6, LstTipoMerma.SelectedItem.Index - 1), 1, 1) = "V", "Vendedor", "Cedis")
    Nuevo = False
    LblEdicion.Caption = "Consultar"
End Sub
