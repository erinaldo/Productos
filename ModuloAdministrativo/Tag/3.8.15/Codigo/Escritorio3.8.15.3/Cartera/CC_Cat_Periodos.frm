...
Begin VB.Form CC_Cat_Periodos 
   ...
   Begin VB.Frame FrmOpt 
      ...
      Begin VB.CommandButton btnAbrir 
         ...
         Height          =   495
         Left            =   6960
         Picture         =   "CC_Cat_Periodos.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   6
         Top             =   500
         Width           =   2055
   ...
   End
End
Attribute VB_Name = "CC_Cat_Periodos"
...
On Error Resume Next
    ...
    If MsgBox("¿ Estás seguro que deseas " & IIf(Me.Tag = 1, "Abrir Mes", "Cerrar Mes") & " el mes seleccionado ?", vbQuestion + vbYesNo, App.Title) = vbNo Then Exit Sub
    ...
    Rs.Open StrCmd, Cnn
    If Not Rs.EOF And Me.Tag = 1 Then
    ...
    If Not Me.Tag = 1 Then
        ...
        If Not Rs.EOF Then
            MsgBox "¡ No puedes Cerrar el Periodo cuando todavía existen Movimientos o Facturas Globales sin Aplicar. !", vbInformation + vbOKOnly, App.Title
            Exit Sub
        End If
    End If
    
    StrCmd = "execute up_Periodos " & CLng(lstDCedis(0, cmbCedis.ListIndex - 1)) & ", " & Year(DTPPeriodo.Value) & ", " & Periodo & ", 'C', '" & IIf(Me.Tag = 1, "A", "C") & "', " & IIf(Me.Tag = 1, 1, 2)
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
    ...
    If Not IsEmpty(LstDPeriodos) Then
        For i = 0 To UBound(LstDPeriodos, 2)
            If Month(DTPPeriodo.Value) = LstDPeriodos(4, i) Then
                Me.Tag = IIf(Trim(UCase(LstDPeriodos(3, i))) = "CERRADO", 1, 0)
                Exit For
            End If
        Next
    End If
