...
'            ListaDeSubReportes = GetData(Rs)
        ...
        Case ctFiltro
            Dim Check As Integer, IdDoc As String, IdTipoDoc As String, Data, DataID
            Filtro = "": Check = 0
            
               If (IdReporte >= 1 And IdReporte <= 5) Or (IdReporte >= 10 And IdReporte <= 15) Or IdReporte = 23 Then
                
                For i = 1 To LstSubRep.ListItems.Count
                    If LstSubRep.ListItems(i).Checked Then
                        DataID = Split(LstSubRep.ListItems(i), " | ")
                        Data = Split(DataID(0), " - ")
                        IdDoc = Trim(Data(0))
                        IdTipoDoc = Trim(Data(1))
                        If Check = 0 Then Filtro = " and ( "
                        Filtro = Filtro & "( MovimientosFacturas.IdDocumento = ''" & IdDoc & "'' and MovimientosFacturas.IdTipoDocumento = ''" & IdTipoDoc & "'') or "
                        Check = Check + 1
                    End If
                Next
                ...
                Else
                    ...
                    IdDoc = Trim(Data(0))
                    IdTipoDoc = Trim(Data(1))
                    Filtro = " and MovimientosFacturas.IdDocumento = ''" & IdDoc & "'' and MovimientosFacturas.IdTipoDocumento = ''" & IdTipoDoc & "''"
            ...
            StrCmd = "Gsp_NivelReporte " & IdReporte & "," & TipoReporte & ",'" & IdCedi & "','" & _
                    ...
                    "','" & Filtro & "', '" & IIf((IdReporte >= 1 And IdReporte <= 5) Or (IdReporte >= 10 And IdReporte <= 15) Or IdReporte = 23, "0", FiltroOrdenamiento) & "'"
            Rs.Open StrCmd, Cnn
...
Private Sub btnExcel_Click()
    ...
    Select Case IdReporte
        Case 1 To 5, 10 To 15 ' Movimientos Ventas
            TituloReporteXLS = "Movimientos a Ventas ( " & NivelReporte & " )"
        ...
        Case 21      'Ventas Acreditadas por Ejecutivo
            TituloReporteXLS = "Ventas Acreditadas por Ejecutivo ( " & NivelReporte & " )"
            'CedisXLS = IdCedis & " - " & NomCedis
            PeriodoXLS = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                "  al  " & Format(RepFechaFinal, ctFechaLarga)
            Tipo = " Ventas Acreditadas por Ejecutivo"

        Case 22      'Ventas Al Cobro
            TituloReporteXLS = "Ventas Al Cobro ( " & NivelReporte & " )"
            'CedisXLS = IdCedis & " - " & NomCedis
            PeriodoXLS = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                "  al  " & Format(RepFechaFinal, ctFechaLarga)
            Tipo = " Ventas Al Cobro"

        Case 23      'Ventas Al Cobro
            TituloReporteXLS = "Movimiento a Ventas ( " & NivelReporte & " )"
            'CedisXLS = IdCedis & " - " & NomCedis
            PeriodoXLS = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                "  al  " & Format(RepFechaFinal, ctFechaLarga)
            Tipo = " Movimiento a Ventas"
        
        Case 24      'Ventas CASA LEY
            TituloReporteXLS = "Ventas CASA LEY ( " & NivelReporte & " )"
            'CedisXLS = IdCedis & " - " & NomCedis
            PeriodoXLS = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                "  al  " & Format(RepFechaFinal, ctFechaLarga)
            Tipo = " Ventas CASA LEY"
    ...
    If IdReporte <> 23 Then Filtro = ""
        
    If (ChkSeleccionar.Value = 2 Or IdReporte <= 5) And IdReporte <> 23 Then
    ...
    Dim ChkTodos As Boolean
    ChkTodos = True
    If IdReporte = 23 Then
        FiltroOrdenamiento = ""
        For i = 1 To LstVDatos(0).ListItems.Count
            If LstVDatos(0).ListItems(i).Checked Then
                'construir el filtro
                FiltroOrdenamiento = FiltroOrdenamiento & "''" & ListaDeFiltro(0, i - 1) & "'', "
                ConteoRegistrosChecados = ConteoRegistrosChecados + 1
            Else
                ChkTodos = False
            End If
        Next
        If Trim(FiltroOrdenamiento) <> "" And Not ChkTodos Then
            FiltroOrdenamiento = " and Ventas.IdCliente in (" & Mid(FiltroOrdenamiento, 1, Len(FiltroOrdenamiento) - 2) & ")"
        Else
            FiltroOrdenamiento = ""
        End If
    End If
    ...
    StrCmd = "Gsp_NivelReporte " & IdReporte & "," & EjecutaSQLXLS & ",'" & RepIdCedi & "','" & _
            ...
            IIf(ChkSeleccionar.Value = 2 Or ((IdReporte >= 1 And IdReporte <= 5) Or (IdReporte >= 10 And IdReporte <= 15)) Or IdReporte = 23, Filtro, "") & _
            "', '" & IIf((IdReporte >= 1 And IdReporte <= 5) Or (IdReporte >= 10 And IdReporte <= 15), "1", FiltroOrdenamiento) & "'"
        
    Rs.Open StrCmd, Cnn
    
    If Not Rs.EOF Then
        If IdReporte = 24 And EjecutaSQLXLS = 11 Then
            GeneraTXTLEY
        Else
...
Private Sub btnImprimir_Click()
    ...
    If IdReporte <> 23 Then
    Filtro = ""
        If ChkSeleccionar.Value = 2 Or ((IdReporte >= 1 And IdReporte <= 5) Or (IdReporte >= 10 And IdReporte <= 15)) Then
...
'                    .Printer.PaperWidth = 12240
    ...
    Filtro = IIf(ChkSeleccionar.Value = 2 Or ((IdReporte >= 1 And IdReporte <= 5) Or (IdReporte >= 10 And IdReporte <= 15) Or IdReporte = 23), Filtro, "")
        ...
        Select Case IdReporte
            Case 1 To 5, 10 To 15      'Movimientos
                With CC_RptMovimiento
                    ...
                    .LblTitulo.Caption = "Movimientos de Ventas ( " & NivelReporte & " )"
            ...
            Case 6      'Antiguedad
                With CC_RptAntiguedad
                    .ImgLogo.Picture = LoadPicture(App.Path & "\LogoEmpresa.jpg")
                    .LblTitulo.Caption = "Antiguedad de Saldos ( " & IIf(EjecutaSQL = 30, StrMarca, NivelReporte) & " )"
                    .LblCedis.Caption = RepIdCedi & " - " & RepNombreCedi
                    .LblDatos.Caption = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                        "  al  " & Format(RepFechaFinal, ctFechaLarga)
            ...
            Case 7      'Antiguedad Detallada
                With CC_RptAntiguedadDetalle
                    .ImgLogo.Picture = LoadPicture(App.Path & "\LogoEmpresa.jpg")
                    .LblTitulo.Caption = "Antiguedad de Saldos Detallada ( " & IIf(EjecutaSQL = 30, StrMarca, NivelReporte) & " )"
                    .LblCedis.Caption = RepIdCedi & " - " & RepNombreCedi
                    .LblDatos.Caption = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                        "  al  " & Format(RepFechaFinal, ctFechaLarga)
            ...
            Case 8      'Balanza
                With CC_RptBalanza
                    .ImgLogo.Picture = LoadPicture(App.Path & "\LogoEmpresa.jpg")
                    .LblTitulo.Caption = "Balanza de Clientes ( " & IIf(EjecutaSQL = 30, StrMarca, NivelReporte) & " )"
                    .LblCedis.Caption = RepIdCedi & " - " & RepNombreCedi
                    .LblDatos.Caption = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                        "  al  " & Format(RepFechaFinal, ctFechaLarga)
            ...
            Case 18      'Balanza Detallada
                With CC_RptBalanzaDetallada
                    .ImgLogo.Picture = LoadPicture(App.Path & "\LogoEmpresa.jpg")
                    .LblTitulo.Caption = "Balanza de Clientes Detallada ( " & IIf(EjecutaSQL = 30, StrMarca, NivelReporte) & " )"
                    .LblCedis.Caption = RepIdCedi & " - " & RepNombreCedi
                    .LblDatos.Caption = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                        "  al  " & Format(RepFechaFinal, ctFechaLarga)
            ...
            Case 21      'Créditos por Ejecutivo
                With CC_RptRepVentasAcredita
                    .ImgLogo.Picture = LoadPicture(App.Path & "\LogoEmpresa.jpg")
                    .LblTitulo.Caption = "Ventas Acreditadas por Ejecutivo ( " & NivelReporte & " )"
                    .LblCedis.Caption = RepIdCedi & " - " & RepNombreCedi
                    .LblDatos.Caption = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                        "  al  " & Format(RepFechaFinal, ctFechaLarga)
            ...
            Case 22      'Ventas al Cobro
                With CC_RptVentasAlCobro
                    .ImgLogo.Picture = LoadPicture(App.Path & "\LogoEmpresa.jpg")
                    .LblTitulo.Caption = "Ventas al Cobro ( " & NivelReporte & " )"
                    .LblCedis.Caption = RepIdCedi & " - " & RepNombreCedi
                    .LblDatos.Caption = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                        "  al  " & Format(RepFechaFinal, ctFechaLarga)
            ...
            Case 23      'Movimientos a Ventas (Cargos/Abonos)
                With CC_RptMovimiento
                    .ImgLogo.Picture = LoadPicture(App.Path & "\LogoEmpresa.jpg")
                    .LblTitulo.Caption = "Movimientos a Ventas (Cargos/Abonos) ( " & NivelReporte & " )"
'                    .LblDatos.Caption = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
'                                        "  al  " & Format(RepFechaFinal, ctFechaLarga)
            ...
            Case 24      'Ventas CASA LEY
                With CC_RptVentasLey
                    .ImgLogo.Picture = LoadPicture(App.Path & "\LogoEmpresa.jpg")
                    .LblTitulo.Caption = "Ventas CASA LEY ( " & NivelReporte & " )"
                    .LblCedis.Caption = RepIdCedi & " - " & RepNombreCedi
                    .LblDatos.Caption = "Periodo del  " & Format(RepFechaInicial, ctFechaLarga) & _
                                        "  al  " & Format(RepFechaFinal, ctFechaLarga)
