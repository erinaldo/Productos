...
Public IdCedis, IdMovimiento, IdDocumento, IdTipoDocumento, CargoAbono, Fecha As Date, Aplicado As Boolean
Dim LstDPartidas, LstDClientes, StrIdClientes

Sub MuestraDetalleSaldos()
On Error Resume Next
    StrCmd = "execute sel_MovimientosFacturas " & IdCedis & ", " & IdMovimiento & ", '" & FormatDate(Fecha) & "', '" & IdDocumento & "', 1"
...
On Error Resume Next
    StrCmd = "execute sel_MovimientosFacturas " & IdCedis & ", " & IdMovimiento & ", '" & FormatDate(Fecha) & "', '" & IdDocumento & "', 2"
    If RsC.State Then RsC.Close
    If Not Rep Then
        RsC.Open StrCmd, Cnn
...
ConteoRegistrosChecados = 0: StrIdClientes = ""
    ...
    If Trim(TxtMontoInicial.Text) = "" Or Trim(TxtMontoFinal.Text) = "" Then
        MsgBox "¡ Teclee los montos de las Ventas a saldar !", vbInformation + vbOKOnly, App.Title
        Exit Sub
    End If
    ...
    StrCmd = "execute up_SaldaFacturas " & IdCedis & ", " & IdMovimiento & ", '" & FormatDate(DTPFechaInicial.Value) & "', '" & IdDocumento & "', '" & IdTipoDocumento & "', 'A', '" & FormatDate(DTPFechaInicial.Value) & "', '" & FormatDate(DTPFechaFinal.Value) & "', " & TxtMontoInicial.Text & ", " & TxtMontoFinal.Text & ", '" & StrIdClientes & "', '" & Usuario & "'"
    If RsC.State Then RsC.Close
    RsC.Open StrCmd, Cnn
