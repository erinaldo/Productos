...
Public IdTipoVenta, Serie, Folio, LstDSeries, LstDMetodoPago, Actividad As Integer, IdPedido
...
On Error GoTo Err_Factura_Venta:
    MensajeCFD = ""
    If LstMetodosPago.Visible Then
    ...
    MousePointer = 11
        ...
        If Facturada <> "1" Then
            StrCmd = "execute up_VentasFactura " & IdCedis & ", " & IdSurt & ", " & IdTipoVenta & ", " & Folio & ", '" & Serie & "', 0, '', '" & metodoPago & "','" & banco & "','" & numCta & "', " & Actividad & ", 3"
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
...
'            Rs.Open StrCmd, Cnn
    ...
    MousePointer = 11
        ...
        If Not ValidarMetodos(sMetodos) Then Exit Sub
...
On Error Resume Next
    If IsNull(metodoPago) Then
        ...
        'StrCmd = "exec RouteADM.dbo.sel_ValorReferencia 'PAGO'"
        'StrCmd = "Select clientepagoid as VAVClave, MetodoPago + ' ' + replace(Banco,'*','') + ' ' + replace(NumCtaPago,'*','') as Descripcion from clientepago where idcliente = " & lblIDCliente.Caption & " and idcedis = " & IdCedis
        'If Rs.State Then Rs.Close
        'Rs.Open StrCmd, Cnn
        ...
        LstMetodosPago.ColumnHeaders(1).Width = 5200
...
Function ValidarMetodos(ByVal sMetodos As String) As Boolean
    ...
    StrCmd = "execute sel_VersionCFD '" & LstDSeries(3, LstPartidas.SelectedItem.Index - 1) & "'"
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
    If Not Rs.EOF Then
