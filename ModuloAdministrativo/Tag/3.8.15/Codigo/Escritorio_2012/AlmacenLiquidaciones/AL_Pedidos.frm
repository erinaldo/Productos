...
On Error GoTo Err_ActualizaPed
    ...
    StrCmd = "execute up_Pedidos " & IdCedis & ", " & IdPedido & ", " & LstDTipoVenta(0, cmbTipoVenta.ListIndex - 1) & ", '" & _
            ...
            FormatDate(DTPFechaE.Value) & "', '" & TxtRutaE.Text & "', " & IdCliente & ", '" & IdSucursal & "', 0, '', 0, 1"
    ...
    If LstPedidos.ListItems.Count > 0 Then
        LstPedidos.SetFocus
        LstPedidos.ListItems.item(1).Selected = True
...
'                If MsgBox("La información se perderá. ¿ Desea continuar ?", vbQuestion + vbYesNo, App.Title) = vbYes Then
        ...
        StrConsultas = ""
                    ...
                    If LstDPedidos(25, LstPedidos.SelectedItem.Index - 1) = sSerieFact Then
                            bEliminar = True
                    End If
                  ...
                  If bEliminar Then
                    ...
                    StrConsultas = "select UUID from Route.dbo.TrpDatoFiscal where Transprodid='" + LstDPedidos(28, LstPedidos.SelectedItem.Index - 1) + "' "
             ...
             If Rs.State Then Rs.Close
                Rs.Open StrConsultas, Cnn
                If Not Rs.EOF Then
                            If IsNull(Rs.Fields(0)) Or Rs.Fields(0) = "" Then
                            GoTo Continua:
                            End If
                Else
                GoTo Continua:
        ...
        StrConsultas = ""
                    ...
                    MensajeCFD = ClaseCFDADM.LlamarCFDADM(PathRouteDesktop, LstDPedidos(28, LstPedidos.SelectedItem.Index - 1), RutaXML, 4)
...
Continua:
                ...
                StrCmd = "execute up_Pedidos " & IdCedis & ", " & IdPedido & ", 0, '19000101', 0, '19000101', 0, 0, '', 0, '', 0, 2"
                If Rs.State Then Rs.Close
                Rs.Open StrCmd, Cnn
...
On Error Resume Next
    ...
    If Not ValidaMetodosDePago(LstPedidos.SelectedItem.SubItems(6), IdCedis) Then Exit Sub

    If Not ValidaDetalle() Then Exit Sub
    If Not ValidaModulo("PEDID", True) Then Exit Sub
        
    If IsEmpty(LstDPedidos) Then Exit Sub
        
'    If CFDCedis = "1" Then
'        StrCmd = "execute sel_ValidaUsuarioCedisCFD " & IdCedis & ", " & LstDPedidos(5, LstPedidos.SelectedItem.Index - 1) & ", 1, " & CFDCedis & ", 1"
'        If Rs.State Then Rs.Close
'        Rs.Open StrCmd, Cnn
'    End If
    
'    StrCmd = "execute sel_Series " & IdCedis & ", " & LstDPedidos(5, LstPedidos.SelectedItem.Index - 1) & ", 1, '" & LstDPedidos(25, LstPedidos.SelectedItem.Index - 1) & "', " & LstDPedidos(26, LstPedidos.SelectedItem.Index - 1) & ", '19000101', 3"
'    If Rs.State Then Rs.Close
'    Rs.Open StrCmd, Cnn
'    If Not Rs.EOF Then
'        AL_Facturar.TxtDatos.Text = "Datos de la Factura"
'        MsgBox "¡ Esta Pedido ya está Facturado !", vbInformation + vbOKOnly, App.Title
'        Exit Sub
'    Else
'        AL_Facturar.TxtDatos.Text = "Datos del Pedido"
'    End If
    
    With AL_Facturar
        .TxtCliente.Text = LstDPedidos(9, LstPedidos.SelectedItem.Index - 1) & " - " & LstDPedidos(10, LstPedidos.SelectedItem.Index - 1)
        .TxtRFC.Text = LstDPedidos(19, LstPedidos.SelectedItem.Index - 1)
        .TxtSucursal.Text = LstDPedidos(17, LstPedidos.SelectedItem.Index - 1) & " (CB: " & LstDPedidos(15, LstPedidos.SelectedItem.Index - 1) & ") " & LstDPedidos(16, LstPedidos.SelectedItem.Index - 1)
'        Select Case LstDPedidos(27, LstPedidos.SelectedItem.Index - 1)
'            Case "E": .TxtStatus.Text = "EMITIDA"
'            Case "B": .TxtStatus.Text = "BAJA"
'            Case "": .TxtStatus.Text = ""
'        End Select
        .lblIDCliente = LstPedidos.SelectedItem.SubItems(6)
        
        .TxtRemision.Text = "Pedido - " & LstDPedidos(1, LstPedidos.SelectedItem.Index - 1) & " | " & FormatCurrency(LstDPedidos(14, LstPedidos.SelectedItem.Index - 1), 2, vbTrue)
        
        .Actividad = 2
        .IdPedido = LstDPedidos(1, LstPedidos.SelectedItem.Index - 1)
        .IdTipoVenta = LstDPedidos(2, LstPedidos.SelectedItem.Index - 1)
        .Folio = LstDPedidos(26, LstPedidos.SelectedItem.Index - 1)
        .Serie = LstDPedidos(25, LstPedidos.SelectedItem.Index - 1)
                
        .Left = AL_Pedidos.Left + ((AL_Pedidos.Width - .Width) / 2)
        .Top = AL_Pedidos.Top + ((AL_Pedidos.Height - .Height) / 2)
        .Show
        .ZOrder (0)
    End With
End Sub
...
Private Function ValidaDetalle() As Boolean
    StrCmd = "select IdPedido from PedidosDetalle where IdCedis = '" & IdCedis & "' and IdPedido = '" & LstDPedidos(1, LstPedidos.SelectedItem.Index - 1) & "'"
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
    ...
    If Not Rs.EOF Then
        ValidaDetalle = True
    Else
        MsgBox "No se puede facturar el pedido sin detalle", vbOKOnly + vbInformation
        ValidaDetalle = False
    End If
End Function
...
On Error GoTo ImpPedido:
    ...
    If CFDCedis = "1" And ValidaFactura(LstDPedidos(5, LstPedidos.SelectedItem.Index - 1), LstDPedidos(25, LstPedidos.SelectedItem.Index - 1), LstDPedidos(26, LstPedidos.SelectedItem.Index - 1)) Then
        ...
        StrCmd = "execute sel_VentasDetalle " & IdCedis & ", " & LstDPedidos(1, LstPedidos.SelectedItem.Index - 1) & ", " & LstDPedidos(2, LstPedidos.SelectedItem.Index - 1) & ", " & LstDPedidos(26, LstPedidos.SelectedItem.Index - 1) & ", 4"
        If RsC.State Then RsC.Close
        RsC.Open StrCmd, Cnn
    
        StrCmd = "execute sel_VentasFacturaCFD " & IdCedis & ", " & LstDPedidos(1, LstPedidos.SelectedItem.Index - 1) & ", " & LstDPedidos(2, LstPedidos.SelectedItem.Index - 1) & ", '" & LstDPedidos(25, LstPedidos.SelectedItem.Index - 1) & "', " & LstDPedidos(26, LstPedidos.SelectedItem.Index - 1) & ", 2, 2"
        If Rs.State Then Rs.Close
        Rs.Open StrCmd, Cnn
        If Not Rs.EOF Then
        ...
        If Trim(MensajeCFD) <> "" Then GoTo ImpPedido:
    Else
        With AL_RptPedido
            
            .ImgLogo.Picture = LoadPicture(App.Path & "\LogoEmpresa.jpg")
            .LblTitulo.Caption = "COMPROBANTE DE VENTA"
            .LblCedis.Caption = "PASTEURIZADORA JERSEY DEL NOROESTE S.A. DE C.V. CEDIS: " & NomCedis
...
On Error Resume Next
    ...
    If LstDPedidos(24, LstPedidos.SelectedItem.Index - 1) <> 0 Then
        MsgBox "¡ El pedido ya se ha convertido en Venta !", vbInformation + vbOKOnly, App.Title
        Exit Sub
    End If

    If MsgBox("Una vez convertido en Venta el Pedido ya no podrá modificarse" & Chr(13) & Chr(10) & "¿ Estás seguro que deseas continuar ?", vbQuestion + vbYesNo, App.Title) = vbNo Then Exit Sub
        
    StrCmd = "execute up_Pedidos " & IdCedis & ", " & IdPedido & ", " & LstDTipoVenta(0, cmbTipoVenta.ListIndex - 1) & ", '" & _
            FormatDate(DTPFechaP.Value) & "', '" & TxtRutaP.Text & "', '" & _
            FormatDate(DTPFechaE.Value) & "', '" & TxtRutaE.Text & "', " & IdCliente & ", '" & IdSucursal & "', " & IdSurtido & ", '', 0, 3"
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
...
On Error Resume Next
    If Not IsEmpty(LstDPedidos) Or LstPedidos.ListItems.Count > 0 Then
        ...
        IdSucursal = LstDPedidos(17, LstPedidos.SelectedItem.Index - 1)
        IdTipoVenta = LstDPedidos(2, LstPedidos.SelectedItem.Index - 1)
        
        If LstDPedidos(25, LstPedidos.SelectedItem.Index - 1) <> "" Then
            LblPedido(1).Visible = True
            LblPedido(1).Caption = "Factura: " & LstDPedidos(25, LstPedidos.SelectedItem.Index - 1) & Format(LstDPedidos(26, LstPedidos.SelectedItem.Index - 1), "000000")
        Else
            LblPedido(1).Visible = False
        End If
        
        TxtCliente.Text = LstDPedidos(9, LstPedidos.SelectedItem.Index - 1) & " - " & LstDPedidos(10, LstPedidos.SelectedItem.Index - 1)
        TxtRFC2.Text = LstDPedidos(19, LstPedidos.SelectedItem.Index - 1)
        TxtSucursal.Text = LstDPedidos(17, LstPedidos.SelectedItem.Index - 1) & " (CB: " & LstDPedidos(15, LstPedidos.SelectedItem.Index - 1) & ") " & LstDPedidos(16, LstPedidos.SelectedItem.Index - 1)
...
Dim Opc
    ...
    btnFacturar.Visible = IIf(CFDCedis = "1", True, False)
    ...
    If RsC.State Then RsC.Close
    RsC.Open StrCmd, Cnn
    LstDPedidos = GetDataLVL(RsC, LstPedidos, 3, 18, "0|0|0|4|0|4|0|0|7|7|7|7|0|0|0|0")
    LblPedido(1).Visible = False
'    LstPedidos_ItemClick LstPedidos.SelectedItem
End Sub
...
On Error Resume Next
    ...
    Else
        StrCmd = "execute sel_ClientesFacturas " & IdCedis & ",'" & LstDPedidos(9, LstPedidos.SelectedItem.Index - 1) & "','','" & LstDPedidos(17, LstPedidos.SelectedItem.Index - 1) & "', " & "', " & LstDTipoVenta(0, cmbTipoVenta.ListIndex - 1) & Opc
    End If
    If RsC.State Then RsC.Close
    RsC.Open StrCmd, Cnn
'Debug.Print StrCmd
...
Sub ValidaRutaPedido(Txt As TextBox)
...
Dim Existencias, Descuento, DctoCapturado
    ...
    If KeyAscii = 13 Then
        ...
        If Trim(TxtIdProd.Text) = "" Or Trim(TxtIdProd.Text) = "0" Then
            TxtIdProd.SetFocus
            Exit Sub
        End If
        If LstDPedidos(24, LstPedidos.SelectedItem.Index - 1) <> 0 Or LstDPedidos(26, LstPedidos.SelectedItem.Index - 1) <> 0 Then
...
'        MsgBox "execute sel_Precios " & IdCedis & ", " & IdCliente & ", " & TxtRutaP.Text & ", " & TxtIdProd.Text & ", 0, 0, 0", vbInformation + vbOKOnly, App.Title
        ...
        If Not Rs.EOF Then
            ...
            TxtCantidad.Text = FormatNumber(Rs.Fields(2), 3): TxtPrecio.Text = FormatCurrency(Precio, 3, vbTrue):
...
On Error Resume Next
    ...
    If RsC.State Then RsC.Close
    RsC.Open StrCmd, Cnn
    If Not Rep Then LstDPartidas = GetDataLVL(RsC, LstPartidas, 3, 12, "0|0|9|9|7|8|7|7|7|7")
End Sub
