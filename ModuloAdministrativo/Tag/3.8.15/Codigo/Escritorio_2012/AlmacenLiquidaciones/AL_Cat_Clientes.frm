...
Begin VB.Form AL_Cat_Clientes 
   ...
   Begin TabDlg.SSTab SSTab 
      ...
      Tab(4).Control(0)=   "OptDomicilio(0)"
      Tab(4).Control(1)=   "OptDomicilio(1)"
      Tab(4).Control(2)=   "FrmOpt(4)"
      Tab(4).Control(3)=   "LblOpt(4)"
      ...
      Begin VB.Frame FrmOpt 
         ...
         Begin VB.Frame FrmMetodoPago 
            ...
            Top             =   2280
            Width           =   10815
            Begin MSComctlLib.ListView lstMetodosEliminados 
               Height          =   1095
               Left            =   7080
               TabIndex        =   160
               Top             =   960
            ...
            Begin MSComctlLib.ListView LstMetodosPago 
               ...
               NumItems        =   0
            End
            Begin VB.CommandButton btnEliminarMetodoPago 
               BackColor       =   &H00FFFFFF&
               BeginProperty Font 
                  Name            =   "Arial"
                  Size            =   9.75
               ...
               Height          =   495
               Left            =   9000
               Picture         =   "AL_Cat_Clientes.frx":1EDF
               Style           =   1  'Graphical
               TabIndex        =   152
               Top             =   1080
               Width           =   1695
            End
            Begin VB.CommandButton btnNuevoMetodoPago 
               BackColor       =   &H00FFFFFF&
               BeginProperty Font 
                  Name            =   "Arial"
                  Size            =   9.75
               ...
               Height          =   495
               Left            =   9000
               Picture         =   "AL_Cat_Clientes.frx":25EF
               Style           =   1  'Graphical
               TabIndex        =   151
               Top             =   360
               Width           =   1695
         ...
         Begin VB.ComboBox CboCadena2 
            CausesValidation=   0   'False
            Height          =   360
            ItemData        =   "AL_Cat_Clientes.frx":2C7D
            Left            =   2880
            List            =   "AL_Cat_Clientes.frx":2C7F
            Style           =   2  'Dropdown List
            TabIndex        =   13
            Top             =   1800
            Width           =   2625
...
If LblEdicion.Caption = "Consultar" Then Exit Sub
    ...
    Select Case SSTab.Tab
        ...
        Case 3:
            ...
            Dim z As Integer
            For z = 1 To LstMetodosPago.ListItems.Count
                If LstMetodosPago.ListItems(z).SubItems(4) = "0" Then 'no esta guardado
                    StrCmd = "insert into clientepago (idcliente,idcedis,metodopago,numctapago,banco) " & _
                            "values(" & TxtIdCliente.Text & "," & IdCedis & ",'" & LstMetodosPago.ListItems(z).SubItems(1) & "','" & IIf(LstMetodosPago.ListItems(z).SubItems(2) = "", "*", LstMetodosPago.ListItems(z).SubItems(2)) & "','" & IIf(LstMetodosPago.ListItems(z).SubItems(3) = "", "*", LstMetodosPago.ListItems(z).SubItems(3)) & "')"
                    If Rs.State Then Rs.Close
                    Rs.Open StrCmd, Cnn
                    LstMetodosPago.ListItems(z).SubItems(4) = "1"
                End If
            Next
            For z = 1 To lstMetodosEliminados.ListItems.Count
                StrCmd = "delete ClientePago where ClientePagoID = " & lstMetodosEliminados.ListItems(z).Text
                If Rs.State Then Rs.Close
                Rs.Open StrCmd, Cnn
            Next
        ...
        Case 4
            ...
            If Nuevo Then
                IdSucursal = Replicate(TxtCedis.Text, 2, "0") & Replicate(TxtDatos(1).Text, 8, "0")
            Else
                IdSucursal = TxtDatos(1).Text
            End If
    
            If Nuevo Then
                StrCmd = "select IdSucursal from ClienteSucursal where IdSucursal = '" & IdSucursal & "'"
                If Rs.State Then Rs.Close
                Rs.Open StrCmd, Cnn
                If Not Rs.EOF Then
                    MsgBox "¡ La Sucursal de Cliente con clave " & IdSucursal & " ya existe !" & Chr(13) & Chr(10) & "Teclea otra Clave de Sucursal", vbInformation, App.Title
                    TxtDatos(1).SetFocus
                    TxtDatos(1).Text = ""
                    Exit Sub
                End If
...
'            End If
            ...
            For i = 0 To TxtDatos.Count - 1
                Select Case i
                    Case 3, 8, 15, 13, 10, 24, 16, 19, 22, 30:
                    Case Else:
...
Private Sub btnEliminarMetodoPago_Click()
    If LstMetodosPago.ListItems.Count = 0 Or LblEdicion.Caption = "Consultar" Then Exit Sub
    If MsgBox("¿Desea eliminar el metodo de pago " & LstMetodosPago.SelectedItem.SubItems(1) & "?", vbYesNo + vbQuestion) = vbYes Then
        If LstMetodosPago.SelectedItem.SubItems(4) = "1" Then
            lstMetodosEliminados.ListItems.Add , , LstMetodosPago.SelectedItem.Text
        End If
        LstMetodosPago.ListItems.Remove (LstMetodosPago.SelectedItem.Index)
    End If
End Sub
...
On Error GoTo Err_CatCteN:
    ...
    Select Case SSTab.Tab
        ...
        Case 4:
            ...
            TxtCedis.Visible = True
            TxtDatos(1).Left = 3360
            TxtDatos(1).Width = 1035
            TxtDatos(1).BackColor = &H80000005
            TxtDatos(1).MaxLength = 8


            TxtCedis.Text = Replicate(CStr(IdCedis), 2, "0")
...
Private Sub btnNuevoMetodoPago_Click()
    If LblEdicion.Caption = "Consultar" Then Exit Sub
...
Private Sub cboBanco_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        If Trim(txtNumCta.Text) = "" Then
            MsgBox "El campo Número de Cuenta es requerido", vbOKOnly + vbInformation
            txtNumCta.SetFocus
        ElseIf Trim(cboBanco.Text) = "" Then
            MsgBox "El campo Banco es requerido", vbOKOnly + vbInformation
            cboBanco.SetFocus
        Else
            AgregarMetodoPago cboMetodoPago.Text, Trim(txtNumCta.Text), cboBanco.Text
        End If
    End If
End Sub

Private Sub cboMetodoPago_Click()
    If cboMetodoPago.ListIndex = -1 Then Exit Sub
    If cboMetodoPago.ItemData(cboMetodoPago.ListIndex) = "1" Or cboMetodoPago.ItemData(cboMetodoPago.ListIndex) = "14" Then
        'efectivo o no identificado, deshabilitar numcta y banco
        txtNumCta.Text = ""
...
Sub AgregarMetodoPago(metodoPago As String, numCta As String, banco As String)
    If ValidarMetodoPago(metodoPago) Then
        LstMetodosPago.ListItems.Add , , ""
        LstMetodosPago.ListItems(LstMetodosPago.ListItems.Count).SubItems(1) = metodoPago
        LstMetodosPago.ListItems(LstMetodosPago.ListItems.Count).SubItems(2) = numCta
        LstMetodosPago.ListItems(LstMetodosPago.ListItems.Count).SubItems(3) = banco
        LstMetodosPago.ListItems(LstMetodosPago.ListItems.Count).SubItems(4) = "0"
        cboMetodoPago.ListIndex = -1
        txtNumCta.Text = ""
        cboBanco.ListIndex = -1
        HabilitarMetodoPago False
    End If
End Sub

Function ValidarMetodoPago(metodoPago As String) As Boolean

    If LstMetodosPago.ListItems.Count > 0 And (UCase(metodoPago) = "EFECTIVO" Or UCase(metodoPago) = "NO IDENTIFICADO") Then

        Dim tItem As ListItem
        Set tItem = LstMetodosPago.FindItem(metodoPago, lvwSubItem, , lvwWhole)
        
        If Not tItem Is Nothing Then
            ValidarMetodoPago = False
        Else
...
Private Sub cboMetodoPago_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        If cboMetodoPago.ItemData(cboMetodoPago.ListIndex) = "1" Or cboMetodoPago.ItemData(cboMetodoPago.ListIndex) = "14" Then
            AgregarMetodoPago cboMetodoPago.Text, "", ""
            Exit Sub
        End If
        txtNumCta.SetFocus
    End If
...
Sub HabilitarMetodoPago(habilitado As Boolean)
    cboMetodoPago.Enabled = habilitado
    txtNumCta.Enabled = habilitado
    cboBanco.Enabled = habilitado
End Sub
...
On Error Resume Next
    ...
    'LLENA METODOS DE PAGO
    cboMetodoPago.Clear
    If Rs.State Then Rs.Close
    Rs.Open "exec RouteADM.dbo.sel_ValorReferencia 'PAGO'", Cnn
    While Not Rs.EOF
        cboMetodoPago.AddItem Rs!Descripcion
        cboMetodoPago.ItemData(cboMetodoPago.ListCount - 1) = Rs!VAVClave
        Rs.MoveNext
    Wend
    Rs.Close
    ...
    cboBanco.Clear
    If Rs.State Then Rs.Close
    Rs.Open "exec RouteADM.dbo.sel_ValorReferencia 'TBANCO'", Cnn
    While Not Rs.EOF
    ...
    'LLENA LIST VIEW METODOS DE PAGO
    LstMetodosPago.ListItems.Clear
    LstMetodosPago.ColumnHeaders.Add , , "ClientePagoID"
    LstMetodosPago.ColumnHeaders.Add , , "Método de Pago"
    LstMetodosPago.ColumnHeaders.Add , , "Número de Cuenta"
    LstMetodosPago.ColumnHeaders.Add , , "Banco"
    LstMetodosPago.ColumnHeaders.Add , , "Guardado"
    LstMetodosPago.ColumnHeaders(1).Width = 0
    LstMetodosPago.ColumnHeaders(2).Width = 3000
    LstMetodosPago.ColumnHeaders(3).Width = 2500
    LstMetodosPago.ColumnHeaders(4).Width = 2700
    LstMetodosPago.ColumnHeaders(5).Width = 0
    
    lstMetodosEliminados.ListItems.Clear
    lstMetodosEliminados.ColumnHeaders.Add , , "ClientePagoID"
End Sub
...
Private Sub LstCliente_Click()
    ...
    LimpiarMetodoPago
    If Rs.State Then Rs.Close
    Rs.Open "SELECT clientepagoid,metodopago,numctapago,banco FROM ClientePago WHERE IDCliente = " & CLng(TxtIdCliente.Text) & " AND IDCedis = " & IdCedis, Cnn
    LstMetodosPago.ListItems.Clear
    If Not Rs.EOF Then
        CargarMetodosPago Rs
    End If
...
Sub CargarMetodosPago(metodospago As Recordset)
    While Not Rs.EOF
        LstMetodosPago.ListItems.Add , , Rs!clientepagoid
        LstMetodosPago.ListItems(LstMetodosPago.ListItems.Count).SubItems(1) = Rs!metodoPago
        LstMetodosPago.ListItems(LstMetodosPago.ListItems.Count).SubItems(2) = Replace(Rs!numctapago, "*", "")
        LstMetodosPago.ListItems(LstMetodosPago.ListItems.Count).SubItems(3) = Replace(Rs!banco, "*", "")
        LstMetodosPago.ListItems(LstMetodosPago.ListItems.Count).SubItems(4) = "1"
        Rs.MoveNext
    Wend
End Sub
...
On Error Resume Next
    ...
    Select Case Index
        ...
        Case 1      'para reasignacion de cliente
            ...
            TxtCedis.Visible = False
            TxtDatos(1).Left = 3000
            TxtDatos(1).Width = 1395
            TxtDatos(1).BackColor = &HE0E0E0
            TxtDatos(1).MaxLength = 10
            LblEdicion.Caption = "Actualizar"
...
On Error Resume Next
    If Index = 1 Then
        If Not TxtDatos(Index).Locked Then
            Dim IdSucTemp As String
            IdSucTemp = Replicate(TxtCedis.Text, 2, "0") & Replicate(TxtDatos(1).Text, 8, "0")
            TxtDatos(Index).Text = Replicate(TxtDatos(1).Text, 8, "0")
            StrCmd = "select IdSucursal from ClienteSucursal where IdSucursal = '" & IdSucTemp & "'"
            If Rs.State Then Rs.Close
            Rs.Open StrCmd, Cnn
            If Not Rs.EOF Then
                MsgBox "¡ La Sucursal de Cliente con clave " & IdSucTemp & " ya existe !" & Chr(13) & Chr(10) & "Teclea otra Clave de Sucursal", vbInformation, App.Title
...
Private Sub txtNumCta_LostFocus()
    If Trim(txtNumCta.Text) <> "" Then
        If Not IsNumeric(txtNumCta.Text) Then
            If UCase(Trim(txtNumCta.Text)) <> UCase("No identificado") Then
                MsgBox "El número de cuenta no es valido.", vbExclamation + vbOKOnly
                txtNumCta.SetFocus
                txtNumCta.SelStart = 0
                txtNumCta.SelLength = Len(txtNumCta.Text)
            Else
                cboBanco.Enabled = False
                AgregarMetodoPago cboMetodoPago.Text, "No Identificado", ""
                Exit Sub
            End If
        Else
            cboBanco.Enabled = True
