...
Global CFDCedis, PathRouteDesktop, Facturada, metodoPago, Facturar, banco, numCta
...
Function ValidaMetodosDePago(IdCliente As String, IdCedis As Long) As Boolean
    StrCmd = "Select clientepagoid from RouteADM.dbo.clientepago where idcliente = " & IdCliente & " and idcedis = " & IdCedis
    If Rs.State Then Rs.Close
    Rs.Open StrCmd, Cnn
    If Not Rs.EOF Then
        ValidaMetodosDePago = True
    Else
        MsgBox "No existen métodos de pago configurados para este cliente", vbOKOnly + vbInformation
        ValidaMetodosDePago = False
    End If
End Function

Function ObtenerMetodosDePago(IdCliente As String, IdCedis As Long) As Recordset
    Dim recset As New Recordset
    StrCmd = "Select clientepagoid as VAVClave, MetodoPago + ' ' + replace(Banco,'*','') + ' ' + replace(NumCtaPago,'*','') as Descripcion, MetodoPago, Banco, NumCtaPago from RouteADM.dbo.clientepago where idcliente = " & IdCliente & " and idcedis = " & IdCedis
    If recset.State Then recset.Close
    recset.Open StrCmd, Cnn
    Set ObtenerMetodosDePago = recset
End Function
...
Function ValidaUltimoDiaCerrado(IdCed As Long, Fec As Date) As Boolean
    ...
    Rs.Open StrCmd, Cnn
    If Not Rs.EOF Then
        'comparar como fechas no como string
        If CDate(FormatDate(Fec)) <= CDate(FormatDate(Rs.Fields(0))) Then
...
Function Replicate(sCadena As String, nLongitud As Integer, sCaracter As String) As String
    Dim clen As Integer
    Dim temp As String
    ...
    If (clen < nLongitud) Then
        temp = String((nLongitud - clen), sCaracter) + temp
    End If
    Replicate = temp
End Function
