...
/****** Object:  StoredProcedure [dbo].[sp_tg_exportManagement]    Script Date: 01/20/2011 16:46:47 ******/
...
---------------------------------------------------------------------------------------------------------
                        ...
                        SELECT TP.Subtotal ,TP.Impuesto, V.ClienteClave, TP.TransProdId, TP.CFVTipo,dbo.FNObtenerSoloNumeros(right(TP.Folio,4)) as folio
                        FROM TransProd as TP 
                        INNER JOIN Visita V ON V.VisitaClave = TP.VisitaClave1 AND V.DiaClave = TP.DiaClave1 
                        INNER JOIN vendedor VE on VE.VendedorId = V.VendedorId
                        INNER JOIN Usuario as Us on Us.USUId = VE.USUId
