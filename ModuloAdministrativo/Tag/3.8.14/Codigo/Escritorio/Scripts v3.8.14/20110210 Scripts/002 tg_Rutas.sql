...
/****** Object:  Trigger [tgRouteU_Rutas]    Script Date: 02/10/2011 13:46:33 ******/
...
/****** Object:  Trigger [dbo].[tgRouteU_Rutas]    Script Date: 02/10/2011 13:46:34 ******/
SET ANSI_NULLS ON
GO
...
begin tran sp_Contador
	...
	declare  sp_contador cursor for
		...
		/*Cuentas por Cobrar*/'2TP631GUGIMEZYA') and R.RutClave in (
			select cast( IdCedis as varchar(10) ) + replicate('0', 4 - len( IdRuta ) ) + cast( IdRuta as varchar(10) )
			from Inserted)
...
--
	...
	/*	
	declare InsFolios cursor for
		select distinct FolioId
		from Route.dbo.Folio
	open InsFolios
	
	fetch next from InsFolios into @FolioId
	while (@@fetch_status = 0)
	begin
		insert into Route.dbo.FolioUsuario
		select @FolioId, cast( IdCedis as varchar(10) ) + replicate('0', 4 - len( IdRuta ) ) + cast( IdRuta as varchar(10) ),
		999999, 2, getdate(), 'Interfaz'
		from Inserted where cast( IdCedis as varchar(10) ) + replicate('0', 4 - len( IdRuta ) ) + cast( IdRuta as varchar(10) ) not in (select VendedorId from Route.dbo.FolioUsuario where FolioId = @FolioId )

		insert into Route.dbo.FolioReservacion
		select @FolioId, cast( IdCedis as varchar(10) ) + replicate('0', 4 - len( IdRuta ) ) + cast( IdRuta as varchar(10) ),
		right(newid(),15), getdate(), 0, 999998, 0, 1, getdate(), 'Interfaz'
		from Inserted where cast( IdCedis as varchar(10) ) + replicate('0', 4 - len( IdRuta ) ) + cast( IdRuta as varchar(10) ) not in (select VendedorId from Route.dbo.FolioReservacion where FolioId = @FolioId )
		
		fetch next from InsFolios into @FolioId
	end
	close InsFolios
	deallocate InsFolios	
*/
