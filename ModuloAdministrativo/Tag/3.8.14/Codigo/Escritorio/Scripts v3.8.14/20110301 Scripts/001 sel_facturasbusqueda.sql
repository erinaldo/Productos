...
/****** Object:  StoredProcedure [dbo].[sel_FacturasBusqueda]    Script Date: 03/01/2011 13:16:46 ******/
...
/****** Object:  StoredProcedure [dbo].[sel_FacturasBusqueda]    Script Date: 03/01/2011 13:16:47 ******/
SET ANSI_NULLS ON
GO
...
-- select @IdRuta, @Folio, @IdCliente
	...
	case Ventas.Serie  collate SQL_Latin1_General_CP1_CI_AS
	...
	inner join ClienteSucursal on ClienteSucursal.IdCedis = Clientes.IdCedis and Ventas.IdSucursal = ClienteSucursal.IdSucursal
 	...
 	' + @IdRuta + ' 
	where Ventas.IdCedis = ' + @IdCedis + ' and Ventas.Fecha between  ''' + @FechaInicial + ''' and ''' + @FechaFinal + ''' ' + @Folio + ' ' + @IdCliente + '  
