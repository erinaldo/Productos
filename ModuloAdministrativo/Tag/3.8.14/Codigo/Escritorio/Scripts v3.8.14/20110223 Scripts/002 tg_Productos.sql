...
/****** Object:  Trigger [tgRouteU_Productos]    Script Date: 02/23/2011 11:04:19 ******/
...
/****** Object:  Trigger [dbo].[tgRouteU_Productos]    Script Date: 02/23/2011 11:04:20 ******/
SET ANSI_NULLS ON
GO
...
begin
	...
	insert into ROUTE.dbo.Producto (ProductoClave, Nombre, NombreLargo, Id, Tipo, SubEmpresaID, CodigoBarras, LimiteDescuento, TipoFase, Contenido, Venta, DecimalProducto, CantidadProduccion, UnidadProduccion, MFechaHora, MUsuarioID) 
		select IdProducto, left( Producto, 32 ), left( Producto, 64 ), IdProducto, 1, IdMarca, CodBarras, 0, 1, 0, 0, case Decimales when 1 then 2 else 0 end, 0, 0, getdate(), 'Interfaz' 
	...
	update ROUTE.dbo.Producto set Nombre = left( (select Producto from Inserted where ProductoClave = IdProducto ), 32 ), NombreLargo = left( ( select Producto from Inserted where ProductoClave = IdProducto), 64 ), 
		...
		SubEmpresaID = (select IdMarca from Inserted where ProductoClave = IdProducto ), MFechaHora = getdate()
	...
	insert into ROUTE.dbo.ProductoUnidad select IdProducto, 1, Conversion, 1, getdate(), 'Interfaz' from Inserted where IdProducto not in ( select ProductoClave from ROUTE.dbo.ProductoUnidad where PruTipoUnidad = 1 ) and Decimales = 0
	insert into ROUTE.dbo.ProductoUnidad select IdProducto, 2, 1, 1, getdate(), 'Interfaz' from Inserted where IdProducto not in ( select ProductoClave from ROUTE.dbo.ProductoUnidad where PruTipoUnidad = 1 ) and Decimales = 1
	...
	insert into ROUTE.dbo.ProductoDetalle select IdProducto, 2, IdProducto, 0, 1, getdate(), 'Interfaz' from Inserted where Decimales = 1
