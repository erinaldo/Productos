...
/****** Object:  StoredProcedure [dbo].[Gsp_NivelReporte]    Script Date: 04/15/2011 17:44:15 ******/
...
begin
	...
	begin			
			exec( 'select IdVendedor, Nomina, Vendedor, SUM(Pago) as Comision
	...
	begin			
			exec( 'select IdVendedor, Nomina, Vendedor, SUM(Pago) as Comision, Fecha
				from FN_ComisionesDetalleFecha(' + @IdCedi + ', ''' + @FechaInicial + ''', ''' + @FechaFinal + ''') 
				where IdVendedor <> 0 ' + @Filtro + ' 
...
begin
	...
	begin			
		exec('
			...
			SELECT     SUR.IdCedis, SUR.IdRuta, SUR.Fecha, SUR.IdSurtido AS Surtido, RUT.Ruta, ISNULL( VVTACONT.Total, 0) AS [Venta Contado], ISNULL(SUM(SD.Cantidad * SD.IdDenominacion), 0) AS [Efectivo], 
						ISNULL(SUM(SD.Cantidad * SD.IdDenominacion), 0) - ISNULL( VVTACONT.Total, 0) AS [Diferencia], ISNULL(VVTACRED.Subtotal, 0) AS [Venta Crédito], ISNULL(VVTACRED.IVA, 0) 
...
begin
	...
	begin
		...
		exec( 'select Surtidos.IdCedis, Surtidos.IdRuta, Ruta, Productos.IdMarca, Marca, SurtidosMerma.IdTipoMerma, isnull( TipoMerma, ''No Definido'') as TipoMerma, 
		SurtidosMerma.IdProducto, Producto, sum(Cantidad) as Piezas, sum(Cantidad * Conversion) as KiloLitros
		from Surtidos
		inner join SurtidosMerma on Surtidos.IdCedis = SurtidosMerma.IdCedis and Surtidos.IdSurtido = SurtidosMerma.IdSurtido 
		left outer join TipoMerma on TipoMerma.IdTipoMerma = SurtidosMerma.IdTipoMerma
		...
		inner join Rutas on Rutas.IdCedis = Surtidos.IdCedis and Surtidos.IdRuta = Rutas.IdRuta
		where Surtidos.IdCedis = ' + @IdCedi + ' and Surtidos.Fecha between ''' + @FechaInicial + ''' and ''' + @FechaFinal + '''   ' + @Filtro + '
		group by Surtidos.IdCedis, Surtidos.IdRuta, Ruta, Productos.IdMarca, Marca, SurtidosMerma.IdTipoMerma, TipoMerma, SurtidosMerma.IdProducto, Producto, IdFamilia
		order by Surtidos.IdRuta, Productos.IdMarca, SurtidosMerma.IdTipoMerma, IdFamilia, SurtidosMerma.IdProducto' )
...
begin
	...
	If @Tipo = 10 or @Tipo = 21			-- acumulado
	begin
		exec(  'select SurtidosDenominacion.IdCedis, Surtidos.Fecha, 0 + TipoDenominacion.IdTipoDenominacion as Orden, ''Efectivo '' + TipoDenominacion.TipoDenominacion as Concepto, Monedas.IdMoneda, Monedas.Moneda, 
			...
			group by SurtidosDenominacion.IdCedis, Surtidos.Fecha, Denominaciones.Denominacion, Monedas.IdMoneda, Monedas.Moneda, 
				TipoDenominacion.IdTipoDenominacion, TipoDenominacion.TipoDenominacion
