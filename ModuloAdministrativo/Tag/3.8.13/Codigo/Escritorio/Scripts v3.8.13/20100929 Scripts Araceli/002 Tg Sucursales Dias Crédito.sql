...
/****** Object:  Trigger [dbo].[tgRouteU_ClienteSucursal]    Script Date: 09/20/2010 10:57:23 ******/
SET ANSI_NULLS ON
GO
...
ALTER TRIGGER [dbo].[tgRouteU_ClienteSucursal] ON [dbo].[ClienteSucursal] 
FOR INSERT, UPDATE
AS
...
set @IdCedis = ( select IdCedis from Configuracion where IdCedis in ( select IdCedis from Inserted )  )
...
-- inserta cliente
	insert into ROUTE.dbo.Cliente ( ClienteClave, CNPId, Clave, IdElectronico, IdFiscal, RazonSocial, TipoFiscal, TipoImpuesto, NombreContacto, TelefonoContacto, FechaRegistroSistema, FechaNacimiento, LimiteCreditoDinero, NombreCorto, TipoEstado, LimiteDescuento, SaldoEfectivo, Prestamo, Exclusividad, VigExclusividad, ActualizaSaldoCheque, VencimientoVenta, DiasVencimiento, FechaFactura, DesgloseImpuesto, CorreoElectronico, MFechaHora, MUsuarioID ) 
		select IdSucursal, null, IdSucursal, left(CodigoBarras, 64), left( RFC, 64), left( RazonSocial, 128), 1, 1, left(Contacto, 64), left( Telefonos, 64), getdate(), '20000101', LimiteCredito, left( NombreSucursal, 32), 1, 0, 0, 0, 0, '20991231', 1, 0, DiasCredito, '20991231', 1, '', getdate(), 'Interfaz' 
	...
	update ROUTE.dbo.Cliente 
		...
		LimiteCreditoDinero = LimiteCredito, DiasVencimiento = DiasCredito
	from ROUTE.dbo.Cliente, Inserted
	where ClienteClave = IdSucursal 

-- inserta domicilio de entrega
	insert into ROUTE.dbo.ClienteDomicilio ( ClienteClave, ClienteDomicilioID, Tipo, Calle, Numero, NumInt, CodigoPostal, ReferenciaDom, Colonia, Localidad, Poblacion, Entidad, Pais, MFechaHora, MUsuarioID )
		select IdSucursal, 'D-02', 2, left( Calle, 64), left(NumExterior, 16), left( NumInterior, 16), left( CP, 16), '', left(Colonia, 64), left(Localidad, 64), left( Poblacion, 64), left(Entidad, 32), left(Pais, 32), getdate(), 'Interfaz' from Inserted where cast(IdSucursal as varchar(16))  not in ( select ClienteClave 
...
-- inserta domicilio fiscal
	insert into ROUTE.dbo.ClienteDomicilio ( ClienteClave, ClienteDomicilioID, Tipo, Calle, Numero, NumInt, CodigoPostal, ReferenciaDom, Colonia, Localidad, Poblacion, Entidad, Pais, MFechaHora, MUsuarioID )
		select IdSucursal, 'D-01', 1, left( CalleF, 64), left(NumExteriorF, 16), left( NumInteriorF, 16), left( CPF, 16), '', left(ColoniaF, 64), left(LocalidadF, 64), left( PoblacionF, 64), left(EntidadF, 32), left(PaisF, 32), getdate(), 'Interfaz' from Inserted where cast(IdSucursal as varchar(16))  not in ( select ClienteClave from ROUTE.dbo.ClienteDomicilio where  ClienteClave = cast(IdSucursal as varchar(16)) and ClienteDomicilioId = 'D-01' )
	update ROUTE.dbo.ClienteDomicilio 
...
-- inserta forma de venta
	insert into ROUTE.dbo.CliFormaVenta ( ClienteClave, CFVTipo, LimiteCredito, DiasCredito,Inicial, CapturaDias, ValidaLimite, ValidaPago, Estado, MFechaHora, MUsuarioID )
		select IdSucursal, '1', 0, 0, 0, 0, 0, 0, 1, getdate(), 'Interfaz' from Inserted where FormaVenta in (1) and  cast(IdSucursal as varchar(16))  not in ( select ClienteClave from ROUTE.dbo.CliFormaVenta where ClienteClave = cast(IdSucursal as varchar(16)) and CFVTipo = '1' )
	-- update para dias de crédito y límite de crédito
	update ROUTE.dbo.CliFormaVenta set Estado = 1 where CFVTipo = '1' and ClienteClave in ( select IdSucursal from Inserted where FormaVenta = 1 )
	update ROUTE.dbo.CliFormaVenta set Estado = 0 where CFVTipo <> '1' and ClienteClave in ( select IdSucursal from Inserted where FormaVenta = 1 )

	insert into ROUTE.dbo.CliFormaVenta ( ClienteClave, CFVTipo, LimiteCredito, DiasCredito,Inicial, CapturaDias, ValidaLimite, ValidaPago, Estado, MFechaHora, MUsuarioID )
		select IdSucursal, '2', 99999, 30, 0, 0, 0, 0, 1, getdate(), 'Interfaz' from Inserted where FormaVenta in (2) and cast(IdSucursal as varchar(16))  not in ( select ClienteClave from ROUTE.dbo.CliFormaVenta where  ClienteClave = cast(IdSucursal as varchar(16)) and CFVTipo = '2' )
	update ROUTE.dbo.CliFormaVenta set Estado = 1 where CFVTipo = '2' and ClienteClave in ( select IdSucursal from Inserted where FormaVenta = 2 )
	update ROUTE.dbo.CliFormaVenta set Estado = 0 where CFVTipo <> '2' and ClienteClave in ( select IdSucursal from Inserted where FormaVenta = 2 )

	insert into ROUTE.dbo.ClientePago ( ClienteClave, ClientePagoID, Tipo, TipoBanco, Cuenta, TipoEstado, MFechaHora, MUsuarioID )
		select IdSucursal, '1', 1, 0, '', 1, getdate(), 'Interfaz' from Inserted where cast(IdSucursal as varchar(16))  not in ( select ClienteClave from ROUTE.dbo.ClientePago where  ClienteClave = cast(IdSucursal as varchar(16))  )
