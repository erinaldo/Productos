...
/****** Object:  StoredProcedure [dbo].[up_ModeloOrienteEnvase]    Script Date: 02/09/2011 10:52:46 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[up_ModeloOrienteEnvase]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[up_ModeloOrienteEnvase]
GO
...
CREATE PROCEDURE [dbo].[up_ModeloOrienteEnvase]
@IdCedis as bigint,
@Fecha as datetime
...
----insert into RepModeloOrienteEnvase values (3, ''' + @Fecha + ''', 2, 0, ''Almacén'', ''Huamantla'', ' + @Almacen3 + '  ) 
	...
	insert into RepModeloOrienteEnvase values (3, ''' + @Fecha + ''', 4, 0, ''Vacío'', ''Huamantla'', ' + @Almacen3 + '  ) 	
...
----insert into RepModeloOrienteEnvase values (2, ''' + @Fecha + ''', 102, 0, ''Almacén'', ''Libres'', ' + @Almacen2 + '  ) 
	...
	insert into RepModeloOrienteEnvase values (2, ''' + @Fecha + ''', 104, 0, ''Vacío'', ''Libres'', ' + @Almacen2 + '  ) 
...
insert into RepModeloOrienteEnvase
select 3, Fecha, 200, 0, 'GRAN TOTAL', 'HUAM + LIB', sum(cast(IdProd30 as money)), sum(cast(IdProd31 as money)), sum(cast(IdProd32 as money)), sum(cast(IdProd33 as money)), sum(cast(IdProd34 as money)), sum(cast(IdProd35 as money)), sum(cast(IdProd36 as money))
from RepModeloOrienteEnvase
where Fecha = @Fecha and IdAgrupador in (6, 106)
group by Fecha
