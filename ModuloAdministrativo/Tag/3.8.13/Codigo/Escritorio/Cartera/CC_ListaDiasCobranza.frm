...
On Error Resume Next
    ...
    If LstFecha.SelectedItem.Index > 1 Then
        MsgBox "¡ Debe importar la Cobranza de Terminales en orden de acuerdo a la fecha, de la más antigua a la más reciente !", vbInformation + vbOKOnly, App.Title
        Exit Sub
    Else
        'StrCmd = "execute sel_Movimientos " & Month(CDate(LstFecha.SelectedItem.ListSubItems(1))) & ", " & IdCed & ", 'CT', 2"
        StrCmd = "execute sel_Movimientos '" & Format(CDate(LstFecha.SelectedItem.ListSubItems(1)), "yyyy-mm-dd") & "', " & IdCed & ", 'CT', 2"
        If Rs.State Then Rs.Close
        Rs.Open StrCmd, Cnn
        If Not Rs.EOF Then
            MsgBox "¡ Debe Aplicar los Movimientos de Cobranza de Terminales que estén Pendientes !", vbInformation + vbOKOnly, App.Title
            Exit Sub
        End If
    End If
