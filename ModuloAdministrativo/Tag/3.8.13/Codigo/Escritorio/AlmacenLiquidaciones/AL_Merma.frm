...
On Error Resume Next
    ...
    End If
    If CDbl(TxtFaltante.Text) <> 0 Then
        If MsgBox("¿ Deseas Cancelar la clasificación de la Devolución Mala ?", vbQuestion + vbYesNo, App.Title) = vbYes Then
...
On Error Resume Next
    If CDbl(TxtFaltante.Text) <> 0 Then
        MsgBox "¡ Debe clasificar toda la Devolución Mala !", vbInformation + vbOKOnly, App.Title
        Exit Sub
    Else
        Unload Me
    End If
...
On Error Resume Next
    StrCmd = "execute sel_SurtidosMerma " & IdCedis & ", " & IdSurtido & ", " & CLng(AL_Liquidacion.TxtIdProdD.Text) & ", 2"
    If RsC.State Then RsC.Close
    RsC.Open StrCmd, Cnn
...
On Error Resume Next
    ...
    If MsgBox("¿ Deseas Eliminar el elemento seleccionado ?", vbQuestion + vbYesNo, App.Title) = vbYes Then
        StrCmd = "execute up_SurtidosMerma " & IdCedis & ", " & IdSurtido & ", " & CLng(AL_Liquidacion.TxtIdProdD.Text) & ", " & CLng(LstMermas.SelectedItem) & ", 0, 2"
        If RsC.State Then RsC.Close
        RsC.Open StrCmd, Cnn
        CalculaFaltante
        MuestraDevMala
...
On Error Resume Next
    ...
    StrCmd = "execute up_SurtidosMerma " & IdCedis & ", " & IdSurtido & ", " & CDbl(AL_Liquidacion.TxtIdProdD.Text) & ", " & cmbTipo.ItemData(cmbTipo.ListIndex) & ", " & CDbl(TxtCantidad.Text) & ", 1"
    If RsC.State Then RsC.Close
    RsC.Open StrCmd, Cnn
    CalculaFaltante
    MuestraDevMala
...
On Error Resume Next
    StrCmd = "execute sel_SurtidosMerma " & IdCedis & ", " & IdSurtido & ", " & CDbl(AL_Liquidacion.TxtIdProdD.Text) & ", 2"
    If RsC.State Then RsC.Close
    RsC.Open StrCmd, Cnn
...
Sub MuestraDevMala()
On Error Resume Next
    StrCmd = "execute sel_SurtidosMerma " & IdCedis & ", " & IdSurtido & ", " & CLng(AL_Liquidacion.TxtIdProdD.Text) & ", 1"
    If RsC.State Then RsC.Close
    RsC.Open StrCmd, Cnn
